...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПрименятьСкидкиКЦене Тогда 
		...
		Элементы.СуммаРучнойСкидки.Видимость	= Ложь;
		Элементы.СуммаСкидки.Видимость			= Ложь;
		Элементы.ВалютаСкидки.Видимость			= Ложь;
	...
	Элементы.ИнформацияОРасчетеСкидокПоСтрокеПроцентСкидкиНаценки.Видимость = ПрименятьСкидкиКЦене;									 
	// }
	// 4D
...
Процедура РазложитьПоКолонкам(СтрокаДерева, НайденнаяСтрока)
	...
	Для каждого СтрокаРасшифровки Из НайденнаяСтрока.Расшифровка Цикл
		...
		// Расчет скидки к цене, №10910
		// {
		СтрокаДерева.ПроцентСкидкиНаценки		= СтрокаДерева.ПроцентСкидкиНаценки + СтрокаРасшифровки.ПроцентСкидкиНаценки;
		// }
		// 4D
...
Процедура СформироватьИнформациюОРасчетеСкидокПоСтроке(ВходящееДеревоСкидок, КлючСвязи)
	...
	// Расчет скидки к цене, №10910
	// {
	ДеревоСкидок.Колонки.Добавить("ПроцентСкидкиНаценки", Новый ОписаниеТипов("Число"));
	// }
	// 4D
