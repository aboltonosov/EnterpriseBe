...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СохраненныйВидОтчета <> Неопределено И НЕ Параметры.Свойство("Организация") И НЕ Параметры.Свойство("ВидОтчета") Тогда
		...
		ЗаполнитьСписокВыбораВидамиОтчетов(Элементы.ОтборВидОтчета.СписокВыбора, МассивВидовОтчетов);
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСписокВыбораВидамиОтчетов(СписокВыбора, КоллекцияВидовОтчетов)
	
	СписокВыбора.Очистить();
	
	Если ТипЗнч(КоллекцияВидовОтчетов) = Тип("СписокЗначений") Тогда
		Для Каждого ЭлементВидОтчета Из КоллекцияВидовОтчетов Цикл
			СписокВыбора.Добавить(ЭлементВидОтчета.Значение, ЭлементВидОтчета.Значение);
		КонецЦикла;
	ИначеЕсли ТипЗнч(КоллекцияВидовОтчетов) = Тип("Массив") Тогда
		Для Каждого СтрВидОтчета Из КоллекцияВидовОтчетов Цикл
			СписокВыбора.Добавить(СтрВидОтчета, СтрВидОтчета);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьОтборы(Форма)
	...
	Если ТипЗнч(Форма.ВидОтчета) = Тип("СписокЗначений") Тогда
		Для Каждого ЭлементВидОтчета Из Форма.ВидОтчета Цикл
			Если СокрЛП(ЭлементВидОтчета.Значение) = "Макет пенсионного дела" Тогда
	...
	Иначе
		Если СокрЛП(Форма.ВидОтчета) = "Макет пенсионного дела" Тогда
			ВидОтчетаМакетПенсионногоДела = Форма.ВидОтчета;
