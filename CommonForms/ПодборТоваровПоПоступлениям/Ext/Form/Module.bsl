...
Процедура ЗаполнитьТаблицуТары()
	...
	|			И (ДокументПоступления.НалогообложениеНДС = &НалогообложениеНДС Или Не ДокументПоступления Ссылка Документ.ПоступлениеТоваровУслуг)
...
Функция ТекстЗапросаЗакупкаПоИмпорту()
	...
	Возврат "ВЫБРАТЬ
		...
		|	ПоступлениеТоваровУслуг.ЗакупкаПодДеятельность КАК ЗакупкаПодДеятельность,
		|	ПоступлениеТоваровУслугТовары.Номенклатура КАК Номенклатура,
		|	ПоступлениеТоваровУслугТовары.Назначение КАК Назначение,
		...
		|			И ВТОстатки.АналитикаУчетаНоменклатуры = ПоступлениеТоваровУслугТовары.АналитикаУчетаНоменклатуры
		|ГДЕ
