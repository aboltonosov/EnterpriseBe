...
Процедура ВывестиОтчетСоблюдениеШтатногоРасписания(ДокументРезультат, Данные)
	...
	Для каждого СтрокаПодразделения Из Данные.Строки Цикл
		...
		Если НастройкиПечатныхФорм.ВыводитьПолнуюИерархиюПодразделений И ЗначениеЗаполнено(СтрокаПодразделения.Подразделение) Тогда
			Подразделения.Параметры.Подразделение = СтрокаПодразделения.Подразделение.ПолноеНаименование();
		КонецЕсли;
	...
	Если Отклонение < 0 Тогда
		...
		Если ?(КоличествоСтавок = Null, 0, КоличествоСтавок) = 0 Тогда
			...
			Если МакетПодразделения Тогда
				КомментарийКПозиции = НСТр("ru='Нет позиций штатного расписания'");
			Иначе
				КомментарийКПозиции = НСТр("ru='Не используется в штатном расписании'");
			КонецЕсли;
		Иначе
			КомментарийКПозиции = НСТр("ru='Превышена штатная численность'");
		КонецЕсли; 
