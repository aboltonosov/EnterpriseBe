...
Процедура СформироватьСтраницыИнвентарнойКнигиОС(ПараметрыОтчета)
	...
	|	ЕСТЬNULL(СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПредпоследнееМОЛ.Представление, """") КАК ПредпоследнеМОЛПредставление,
	...
	|			СчетаБухгалтерскогоУчетаОС.СчетНачисления" + ?(ИДКонфигурации = "БАУ" ИЛИ ИДКонфигурации = "БАУКОРП", "Износа", "Амортизации") + " КАК СчетНачисленияАмортизации,
	...
	|						ИЗ
	|							Документ.ПодготовкаКПередачеОС КАК ПередачаОС ГДЕ ПередачаОС.ПередачаВыполнена
	|						
	|						ОБЪЕДИНИТЬ ВСЕ
	|						
	|						ВЫБРАТЬ
