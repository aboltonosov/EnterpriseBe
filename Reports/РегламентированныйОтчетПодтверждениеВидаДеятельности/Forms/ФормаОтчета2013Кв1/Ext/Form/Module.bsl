...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мВерсияФормы = "25/01/2017";
...
Процедура ВывестиНаПечатьПриложение1(ПараметрыПечати)
	...
	ЗаполняемыйМакет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение1");
...
Процедура ВывестиНаПечатьПриложение2(ПараметрыПечати)
	...
	ЗаполняемыйМакет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение2");
...
Процедура ВывестиНаПечатьПриложение3(ПараметрыПечати)
	...
	Если СтруктураРеквизитовФормы.Редакция75н Тогда
		ЗаполняемыйМакет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение3_1_ВРедакцииПриказа75н");
	Иначе
		ЗаполняемыйМакет = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение3_1");
	КонецЕсли;	
	
	ЗаполнитьМакетПечати(ПараметрыПечати, ЗаполняемыйМакет);
	
	ИтоговыйМакет = Новый ТабличныйДокумент;
	ЗаполнитьЗначенияСвойств(ИтоговыйМакет, ЗаполняемыйМакет, "ОриентацияСтраницы, ПолеСверху, ПолеСлева, ПолеСнизу, ПолеСправа, АвтоМасштаб", "Макет");
	...
	Если СтруктураРеквизитовФормы.Редакция75н Тогда
		МакетОбразец = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение3_2_ВРедакцииПриказа75н");
	Иначе
		МакетОбразец = ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ПечатнаяФормаОтчета2013Кв1_Приложение3_2");
	КонецЕсли;	
...
Функция Сохранить(Автосохранение = Ложь, КодИФНС = "не применимо", Вариант, ВидДокументаНомерКорректировкиИзменен = Неопределено, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("Редакция75н", СтруктураРеквизитовФормы.Редакция75н);
