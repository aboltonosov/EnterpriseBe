...
// { Удалены росс. предопределенные счета
...
Функция ПолучитьПараметрыИсполненияОтчета() Экспорт
	...
	Структура.Вставить("ИспользоватьПослеВыводаРезультата",  Ложь);
	Возврат Структура;
...
Процедура СформироватьРасчетНалогаНаПрибыль(ПараметрыОтчета, Документ, КэшПараметровОтчета)
	...
	// {
	НалогНаПрибыль = ?(ПоддержкаБУ18, ПланыСчетов.Хозрасчетный.НалогНаПрибыль, 
	...
	Если СтрокаСтавки = Неопределено Тогда
		СтавкаНалогаФБ = НалоговыйУчет.НалоговаяСтавкаФедеральныйБюджет(КонецДня(ПараметрыОтчета.КонецПериода));
	...
	|					ТОГДА ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидДеятельностиДляНалоговогоУчетаЗатрат
	|				КОНЕЦ <> ЗНАЧЕНИЕ(Перечисление.ВидыДеятельностиДляНалоговогоУчетаЗатрат.ОсобыйПорядокНалогообложения),
	|			КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДС)),
	|			) КАК ХозрасчетныйОбороты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|			Организация В (&Организация)
	|				И ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРБП В 
