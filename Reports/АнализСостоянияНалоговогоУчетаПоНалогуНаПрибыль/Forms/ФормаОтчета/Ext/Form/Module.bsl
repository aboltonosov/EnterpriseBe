...
Процедура РезультатСхемыВыбор(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ТекстДанныеНеАктуальны" Тогда
		
		ПериодРегистрации = ?(ЗначениеЗаполнено(НезакрытыйПериодФинансовыйРезультат), НезакрытыйПериодФинансовыйРезультат, Отчет.НачалоПериода);
		ПараметрыФормы = Новый Структура("Организация, ПериодРегистрации", Отчет.Организация, ПериодРегистрации);
...
Процедура СформироватьРасшифровку(УправляющаяСтруктура)
	...
	РезультатВыполнения = ОбновитьТаблицуНаСервере(УправляющаяСтруктура, ДанныеОбновлены);
	Если РезультатВыполнения = Неопределено Тогда
		Возврат;
	ИначеЕсли РезультатВыполнения.ЗаданиеВыполнено Тогда
		Если НЕ ДанныеОбновлены Тогда
			ОбновитьСтраницы(УправляющаяСтруктура);	
		КонецЕсли;
	Иначе
...
&НаСервере
Функция ОбновитьТаблицуНаСервере(УправляющаяСтруктура, ДанныеОбновлены)
...
Процедура УстановитьНадписьПоСтатусамРегламентныхОпераций()
	...
	СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.НеТребуется;
	...
	Если ЗначениеЗаполнено(Отчет.Организация) 
		...
		СписокОрганизаций.Добавить(Отчет.Организация);
		
		СписокПодразделений = СписокПодразделений();
		
		ОбработкаОбъект = Обработки.ОперацииЗакрытияМесяца.Создать();
		
		Период = Отчет.НачалоПериода;
		Пока Период < КонецМесяца(Отчет.КонецПериода) Цикл
			...
			СостоянияПредыдущихЭтапов = Новый Структура;
			СостоянияПредыдущихЭтапов.Вставить("ДолиСписанияКосвенныхРасходов");
			
			СостояниеРеглУчет = ОбработкаОбъект.СостояниеЭтапа(Неопределено, ПараметрыРасчета, "РеглУчет");
			СостоянияПредыдущихЭтапов.Вставить("РеглУчет", СостояниеРеглУчет);
			
			СостояниеСписаниеКосвенныхРасходов = ОбработкаОбъект.СостояниеЭтапа(СостоянияПредыдущихЭтапов, ПараметрыРасчета, "СписаниеКосвенныхРасходов");
			СостоянияПредыдущихЭтапов.Вставить("СписаниеКосвенныхРасходов", СостояниеСписаниеКосвенныхРасходов);
			
			СостояниеРезервыПоДолгам = ОбработкаОбъект.СостояниеЭтапа(СостоянияПредыдущихЭтапов, ПараметрыРасчета, "РезервыПоДолгам");
			СостоянияПредыдущихЭтапов.Вставить("РезервыПоДолгам", СостояниеРезервыПоДолгам);
			
			СостояниеФинансовыйРезультат = ОбработкаОбъект.СостояниеЭтапа(СостоянияПредыдущихЭтапов, ПараметрыРасчета, "ФинансовыйРезультат");
			УстановитьПривилегированныйРежим(Ложь);
			
			Если СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.ВыполненоСОшибками
				Или СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.НеВыполнено
				Или СостояниеФинансовыйРезультат = Перечисления.СостоянияОперацийЗакрытияМесяца.ОжидаетВыполненияПредыдущихОпераций Тогда
				
				НезакрытыйПериодФинансовыйРезультат = Период;
				
				НадписьДанныеНеАктуальны.Наименование = 
					СтрШаблон(НСтр("ru = 'Данные отчета могут быть не актуальны, так как не выполнены регламентные операции за %1'"), Формат(Период, "ДФ='MMММ yyyy'"));
			...
			Если ЗначениеЗаполнено(НезакрытыйПериодФинансовыйРезультат) Тогда
				Прервать;
			КонецЕсли;
