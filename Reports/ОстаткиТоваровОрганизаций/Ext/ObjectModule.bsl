...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если ДляПомощникаОформленияСкладскихАктов Тогда
		...
		ДокументОбъект = Новый Структура("Дата, Склад, Организация", ТекущаяДата(), Склад, Неопределено);
	...
	|	КОНЕЦ КАК Партнер,
	|
	|	ТаблицаОстатков.ВидЗапасовВладельца.НалогообложениеНДС КАК НалогообложениеНДС,
	|
...
Процедура ВидыЗапасовНеОбособленныеИОбособленные(Сделки, Менеджер, Подразделение, МенеджерВременныхТаблиц, ДоступнаПередачаНаКомиссию = Ложь)
