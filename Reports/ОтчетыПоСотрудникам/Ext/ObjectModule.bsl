...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если КлючВарианта = "Т2" ИЛИ КадровыйУчетВнутренний.ОтчетВидаКарточкаСотрудника(КлючВарианта) Тогда
		...
		ЭтоВариантОтчетаТ2ГСМС = (ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба")
			И ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба").ЭтоВариантОтчетаТ2ГСМС(КлючВарианта));
		
		Если (КлючВарианта = "Т2" Или ЭтоВариантОтчетаТ2ГСМС)
			И Не ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.ПлановыеНачисления) Тогда
			
			ВызватьИсключение НСтр("ru='Недостаточно прав на доступ к данным'");
...
Процедура ИнициализироватьОтчет() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина") Тогда
		ОбщегоНазначения.ОбщийМодуль("Медицина").ДоработатьСхемуОтчетаПоСотрудникам(ЭтотОбъект);
	КонецЕсли; 
...
Процедура ВывестиВложенныеГруппировкиСМакетамиT2(ДокументРезультат, Данные, Группировки, Уровень, ДатаОтчета, СоответствиеПользовательскихПолей, РаботникКадровойСлужбыРасшифровкаПодписи, КлючВарианта)
	...
	Если Группировки.Количество() > Уровень Тогда 
		...
		Для Каждого СтрокаДанных Из Данные.Строки Цикл
			...
			ВывестиВложенныеГруппировкиСМакетамиT2(ДокументРезультат, СтрокаДанных, Группировки, Уровень + 1, ДатаОтчета,  СоответствиеПользовательскихПолей, РаботникКадровойСлужбыРасшифровкаПодписи, КлючВарианта);
...
Процедура ВывестиГруппировкуT2(ДокументРезультат, СтрокаДанных, Поле, Уровень, СоответствиеПользовательскихПолей)
	...
	// {   	
	МакетГруппировки  = УправлениеПечатью.МакетПечатнойФормы("Отчет.ОтчетыПоСотрудникам.ПФ_MXL_Т2_Локализация");  	
	// }
	// 4D
...
Процедура ВывестиМакетT2(ДокументРезультат, СтрокаДанных, Уровень, ДатаОтчета, ДополнительныеСведения, СоответствиеПользовательскихПолей, РаботникКадровойСлужбыРасшифровкаПодписи, КлючВарианта) 
	...
	// Перенести макеты печатных форм из 2.1.3.266 в 2.2.3.138, № 15621
	// {	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Отчет.ОтчетыПоСотрудникам.ПФ_MXL_Т2_Локализация");	
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Дмитрий, 11.09.2017 5:55:45 
	// Страна Беларусь
	// {
	Если ЗначениеЗаполнено(СтрокаДанных.ЛичныеДанныеСтрана) И СтрокаДанных.ЛичныеДанныеСтрана <> Справочники.СтраныМира.Беларусь Тогда
...
Функция ПолучитьПредставлениеСтатьиТКРФ(СтатьяТКРФ)
	...
	Пока Выборка.Следующий() Цикл
		СтатьяТКРФПредставление = Выборка.ТекстОснования + ", " + Выборка.Наименование;
	КонецЦикла;
