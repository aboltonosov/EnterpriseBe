	...
	Если ИмяТекРаздела = "ДвижениеСредств" Тогда
		...
		Для НомСтрокиВДереве = 1 По ДеревоДанныхРаздела.Строки.Количество() Цикл
			...
			Для каждого Обл Из ТД_МнЧасть1.Области Цикл
				...
				Если ПозИнд > 0 Тогда
					Обл.Имя = Лев(Обл.Имя, ПозИнд) + Формат(НомСтрокиВДереве, "ЧГ=");
				КонецЕсли;
			КонецЦикла;
			ВставитьОбластьВТабличныйДокументПриПечати(ТД_МнЧасть1, "Преемник", МакетМЧБ, СтруктураПараметров);
			// Вывод данных в строки 1.
			Обл = ТабличныйДокумент.Области["П0000104_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = ?(Ветка.Данные["П0000104"] = 0, "-", Ветка.Данные["П0000104"]);
			Обл = ТабличныйДокумент.Области["П0000105_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			...
			Обл = ТабличныйДокумент.Области["П0000204_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = ?(Ветка.Данные["П0000204"] = 0, "-", Ветка.Данные["П0000204"]);
			Обл = ТабличныйДокумент.Области["П0000205_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = Ветка.Данные["КодВалюты"];
			...
			Обл = ТабличныйДокумент.Области["П0000304_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = ?(Ветка.Данные["П0000304"] = 0, "-", Ветка.Данные["П0000304"]);
			Обл = ТабличныйДокумент.Области["П0000305_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = Ветка.Данные["КодВалюты"];
			...
			Обл = ТабличныйДокумент.Области["П0000404_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = ?(Ветка.Данные["П0000404"] = 0, "-", Ветка.Данные["П0000404"]);
			Обл = ТабличныйДокумент.Области["П0000405_" + Формат(НомСтрокиВДереве, "ЧГ=")];
			Обл.Текст = Ветка.Данные["КодВалюты"];
