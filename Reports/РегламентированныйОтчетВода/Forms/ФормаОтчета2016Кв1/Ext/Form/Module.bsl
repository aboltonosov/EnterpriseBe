...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки, ДеревоВыгрузки)
	...
	Для Каждого СтраницаРаздела1 Из Раздел1 Цикл
		...
		НомерСтроки1 = 1;
		Пока Данные1.Свойство("П000100002003_" + Формат(НомерСтроки1, "ЧГ=")) Цикл
			ОКТМО = СокрЛП(Данные1["П000100002003_" + Формат(НомерСтроки1, "ЧГ=")]);
			Сумма = Данные1["П000100003003_" + Формат(НомерСтроки1, "ЧГ=")];
			...
			Если НЕ (ОКТМОДлина = 11 ИЛИ ОКТМОДлина = 8) Тогда
				...
				+ Формат(НомерСтроки1, "ЧГ=") + " не" + ?(ОКТМОДлина = 0, "", "верно") + " задан код по ОКТМО.",
				"Раздел1", "П000100002003_" + Формат(НомерСтроки1, "ЧГ="), НомерСтраницы1);
				СтатусПроверки = Ложь;
			КонецЕсли;
			НомерСтроки1 = НомерСтроки1 + 1;
		КонецЦикла;
...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	Для Каждого СтраницаРаздела Из ДанныеРаздела1 Цикл
		...
		НомерСтроки = 1;
		Пока ДанныеСтраницы.Свойство("П000100002003_" + Формат(НомерСтроки, "ЧГ=")) Цикл
...
Процедура СформироватьРаздел1()
	...
	Если СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Раздел1" Тогда
		...
		Для Инд = 0 По ПромТЗ.Количество() - 1 Цикл
			...
			ТабличныйДокумент.Области["П000100002003_" + Формат(ИндДопСтроки + 1, "ЧГ=")].Значение
			= ПромТЗ[Инд]["П000100002003"];
			ТабличныйДокумент.Области["П000100003003_" + Формат(ИндДопСтроки + 1, "ЧГ=")].Значение
			= ПромТЗ[Инд]["П000100003003"];
	...
	Иначе
		...
		Для Инд = 0 По ПромТЗ.Количество() - 1 Цикл
			...
			"П000100002003_" + Формат(ИндДопСтроки + 1, "ЧГ="), ПромТЗ[Инд]["П000100002003"]);
			ТаблСтраницРаздела1[ИндСтраницыР1].Данные[0].Значение.Вставить(
			"П000100003003_" + Формат(ИндДопСтроки + 1, "ЧГ="), ПромТЗ[Инд]["П000100003003"]);
...
Процедура ПроверитьКС_2_9(СтруктураРезультата)
	...
	ОписаниеКС      = НСтр("ru='Коэффициент в строке 150 подраздела 2.1 раздела 2 должен иметь значение 10, если строка 140 заполнена'");
	...
	Для Каждого СтраницаРаздела21 Из Раздел21 Цикл
		...
		КС = ПоказателиКС(Р21Ст150Гр03.Значение, 10);
		Расшифровка = КС.ЛеваяЧастьСтр + " = " + КС.ПраваяЧастьСтр;
...
Процедура ПроверитьКС_2_10(СтруктураРезультата)
	...
	Для Каждого СтраницаРаздела21 Из Раздел21 Цикл
		...
		КС = ПоказателиКС(Р21Ст160Гр03.Значение, Р21Ст101Гр03.Значение
			* Окр(Р21Ст110Гр03.Значение * Р21Ст130Гр03.Значение * ?(Р21Ст140Гр03.Значение = 0, 1, Р21Ст140Гр03.Значение)
			* ?(Р21Ст150Гр03.Значение = 0, 1, Р21Ст150Гр03.Значение)) + Р21Ст102Гр03.Значение * Окр(Р21Ст120Гр03.Значение
			* ?(Р21Ст130Гр03.Значение = 0, 1, Р21Ст130Гр03.Значение) * ?(Р21Ст140Гр03.Значение = 0, 1, Р21Ст140Гр03.Значение)
			* ?(Р21Ст150Гр03.Значение = 0, 1, Р21Ст150Гр03.Значение)), Истина, Истина);
			
		Расшифровка = КС.ЛеваяЧастьСтр + " = " + Р21Ст101Гр03.ЗначениеСтр + " * окр.(" + Р21Ст110Гр03.ЗначениеСтр + " * "
			+ Р21Ст130Гр03.ЗначениеСтр + ?(Р21Ст140Гр03.Значение = 0, "", " * " + Р21Ст140Гр03.ЗначениеСтр)
