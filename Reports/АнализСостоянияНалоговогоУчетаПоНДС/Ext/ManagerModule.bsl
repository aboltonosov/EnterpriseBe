...
Функция ПолучитьНаборДанныхНачислениеРеализацияСложныйУчетНДС(Запрос)
	...
	|				И НЕ Событие В (ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПродажи.ПодтвержденаСтавка0), 
...
Функция ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:34:15 
	// Ставки НДС 20% и 25%
	// {
	|		КОГДА ВТРеализации.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
	|		КОГДА ВТРеализации.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25_125)
	// }
	// 4D
	|	КОНЕЦ КАК СтавкаНДС,
	...
	|	КлючиАналитикиУчетаПоПартнерам.Договор КАК ДоговорКонтрагента,
	|	РасчетыСКлиентами.РасчетныйДокумент КАК СчетФактура,
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:33:18 
	// Ставки НДС 20% и 25%
	// {
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120) КАК СтавкаНДСПредположительная,
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток - РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * 20 / 120 КАК ЧИСЛО(15, 2))) КАК СуммаБезНДС,
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * 20 / 120 КАК ЧИСЛО(15, 2))) КАК СуммаНДС
	// }
	// 4D
	|ПОМЕСТИТЬ ВТХозрасчетныйВсеОстаткиАвансыПолученные
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоДокументам.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, , , ) КАК РасчетыСКлиентами
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаПоПартнерам КАК КлючиАналитикиУчетаПоПартнерам
	|		ПО РасчетыСКлиентами.АналитикаУчетаПоПартнерам = КлючиАналитикиУчетаПоПартнерам.Ссылка
	|			И (КлючиАналитикиУчетаПоПартнерам.Организация В (&Организация))
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25_125)
	// }
	// 4D
...
Функция ПолучитьНаборДанныхНачислениеНалоговыйАгент(Запрос)
	...
	|	ВЫРАЗИТЬ(СУММА(ВложенныйЗапрос.Сумма) * 0.18 КАК ЧИСЛО(15, 2)) КАК СуммаНДСОборот
...
Функция ПолучитьНаборДанныхВычетАвансыВыданныеУпрощенныйУчетНДС(Запрос)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:30:24 
	// Ставки НДС 20% и 25%
	// {
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120) КАК СтавкаНДСПредположительная,
	|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотДт - ХозрасчетныйОбороты.СуммаОборотДт * 20 / 120 КАК ЧИСЛО(15, 2)) КАК СуммаБезНДС,
	|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотДт * 20 / 120 КАК ЧИСЛО(15, 2)) КАК СуммаНДС
	// }
	// 4D
	|ПОМЕСТИТЬ ВТАвансыВыданныеВременнаяТаблица
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериодаГраница, 
	|												Регистратор, 
	...
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10_110)
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:27:29 
	// Ставки НДС 20% и 25%
	// {
	|		КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20_120)
