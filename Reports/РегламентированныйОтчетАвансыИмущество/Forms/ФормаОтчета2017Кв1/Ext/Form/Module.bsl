...
Процедура ПроверитьКС_1_9(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма авансового платежа (строка 180 Раздела 2) должна быть равна 1/4 (строка 120 Раздела 2 - строка 140 Раздела 2) * строка 170 Раздела 2 / 100, если код вида имущества (строка 001 Раздела 2) = ""01"", ""03"", ""05"", ""08"" или ""10""'");
	...
	Для Каждого СтраницаР2 Из СтраницыРаздела2 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст140Гр03);
		ВнестиПоказательВРасшифровку(Р2Ст140Гр03, "Раздел2", НомерСтраницыСтр);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст170Гр03);
		ВнестиПоказательВРасшифровку(Р2Ст170Гр03, "Раздел2", НомерСтраницыСтр);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст1Гр01);
		ВнестиПоказательВРасшифровку(Р2Ст1Гр01, "Раздел2", НомерСтраницыСтр);
		КС = ПоказателиКС(Р2Ст180Гр03.Значение, 1 / 4 * (Р2Ст120Гр03.Значение - Р2Ст140Гр03.Значение) * Р2Ст170Гр03.Значение / 100);
		Расшифровка = КС.ЛеваяЧастьСтр + " =  1/4 * (" + Р2Ст120Гр03.ЗначениеСтр + " - " + Р2Ст140Гр03.ЗначениеСтр + ") * "
			+ Р2Ст170Гр03.ЗначениеСтр + " / 100 Итог[" + КС.ПраваяЧастьСтр + "], когда """ + Р2Ст1Гр01.ЗначениеСтр + """ = ""01,03,05,08 или 10""";
		УсловиеВыполнено = КодНайден(Р2Ст1Гр01.ЗначениеСтр, "01,03,05,08,10");
		Если НЕ УсловиеВыполнено ИЛИ (УсловиеВыполнено И КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
...
Процедура ПроверитьКС_1_10(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма авансового платежа (строка 180 Раздела 2) должна быть равна 1/4 (строка 120 Раздела 2 - строка 140 Раздела 2) * строка 150 Раздела 2 * строка 170 Раздела 2 / 100, если код вида имущества (строка 001 Раздела 2) = ""02""'");
	Нарушение      = НСтр("ru='НК РФ ст.382, п. 4'");
	ОписаниеОшибки1 = НСтр("ru='Завышена сумма авансового платежа'");
	ОписаниеОшибки2 = НСтр("ru='Занижена сумма авансового платежа'");
	Р2Ст180Гр03 = ПоказательПоСтроке("Р2Ст180Гр03");
	...
	Для Каждого СтраницаР2 Из СтраницыРаздела2 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст140Гр03);
		ВнестиПоказательВРасшифровку(Р2Ст140Гр03, "Раздел2", НомерСтраницыСтр);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст150Гр03Знам);
		ВнестиПоказательВРасшифровку(Р2Ст150Гр03Знам, "Раздел2", НомерСтраницыСтр);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст150Гр03Числ);
		...
		КС = ПоказателиКС(Р2Ст180Гр03.Значение, (1 / 4 * (Р2Ст120Гр03.Значение - Р2Ст140Гр03.Значение) * Р2Ст150Гр03Зн * Р2Ст170Гр03.Значение
			* 2 / 100));
		Расшифровка = КС.ЛеваяЧастьСтр + " = (1 / 4 * (" + Р2Ст120Гр03.ЗначениеСтр + " - " + Р2Ст140Гр03.ЗначениеСтр + ") * "
			+ Р2Ст150Гр03Числ.ЗначениеСтр + " / " + Р2Ст150Гр03Знам.ЗначениеСтр + " * " + Р2Ст170Гр03.ЗначениеСтр + " * 2 / 100) Итог["
			+ КС.ПраваяЧастьСтр + "], когда """ + Р2Ст1Гр01.ЗначениеСтр + """ = ""02""";
		УсловиеВыполнено = (Р2Ст1Гр01.Значение = "02");
		Если НЕ УсловиеВыполнено ИЛИ (УсловиеВыполнено И КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
...
Процедура ПроверитьКС_1_11(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Сумма авансового платежа (строка 180 раздела 2) должна быть равна 1/4 (строка 120 Раздела 2 - строка 140 Раздела 2) * строка 170 Раздела 2 * строка 175 Раздела 2 / 100'");
	...
	Для Каждого СтраницаР2 Из СтраницыРаздела2 Цикл
		...
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст170Гр03);
		ВнестиПоказательВРасшифровку(Р2Ст170Гр03, "Раздел2", НомерСтраницыСтр);
		ВставитьЗначениеПоказателя(ДанныеСтраницы, Р2Ст175Гр03);
		ВнестиПоказательВРасшифровку(Р2Ст175Гр03, "Раздел2", НомерСтраницыСтр);
		КС = ПоказателиКС(Р2Ст180Гр03.Значение, 1 / 4 * (Р2Ст120Гр03.Значение - Р2Ст140Гр03.Значение) * Р2Ст170Гр03.Значение
			* ?(ЗначениеЗаполнено(Р2Ст175Гр03.Значение), Р2Ст175Гр03.Значение, 0) / 100);
		Расшифровка = КС.ЛеваяЧастьСтр + " = 1 / 4 * (" + Р2Ст120Гр03.ЗначениеСтр + " - " + Р2Ст140Гр03.ЗначениеСтр + ") * "
			+ Р2Ст170Гр03.ЗначениеСтр + " * " + Р2Ст175Гр03.ЗначениеСтр + " / 100 Итог [" + КС.ПраваяЧастьСтр + "]";
		Если КС.ЛеваяЧасть = КС.ПраваяЧасть Тогда
			ЭтоОшибка = Ложь;
			ОбщееЧислоБезОшибок = ОбщееЧислоБезОшибок + 1;
		Иначе
...
Процедура ПроверитьКС_1_14(СтруктураРезультата)
	...
	ОписаниеКС     = НСтр("ru='Средняя стоимость необлагаемого налогом имущества (строка 140 Раздела 2) должна быть равна нулю'");
	Нарушение      = НСтр("ru='НК РФ ст. 381; ст. 372, п. 2; ст. 346.35, п. 7'");
	ОписаниеОшибки = НСтр("ru='Возможно неправомерное применение льгот'");
	Р2Ст140Гр03 = ПоказательПоСтроке("Р2Ст140Гр03");
	ДанныеПоказателей = СтруктураРеквизитовФормы.мСохраненныйДок.ДанныеОтчета.Получить();
...
Процедура ПроверитьКС_1_18(СтруктураРезультата)
	...
	Для Каждого СтраницаР3 Из СтраницыРаздела3 Цикл
		...
		Расшифровка = КС.ЛеваяЧастьСтр + " = 1 / 4 * (" + Р3Ст20Гр03.ЗначениеСтр + " - " + Р3Ст25Гр03.ЗначениеСтр + ") * "
			+ Р3Ст30Гр03Числ.ЗначениеСтр + " / " + Р3Ст30Гр03Знам.Значение + " * " + Р3Ст50Гр03Числ.ЗначениеСтр + " / " + Р3Ст50Гр03Числ.ЗначениеСтр
...
Процедура ПроверитьКС_1_19(СтруктураРезультата)
	...
	Для Каждого СтраницаР3 Из СтраницыРаздела3 Цикл
		...
		Расшифровка = КС.ЛеваяЧастьСтр + " = 1 / 4 * (" + Р3Ст20Гр03.ЗначениеСтр + " - " + Р3Ст25Гр03.ЗначениеСтр + ") * "
			+ Р3Ст50Гр03Числ.ЗначениеСтр + " / " + Р3Ст50Гр03Числ.ЗначениеСтр + " * " + Р3Ст70Гр03.ЗначениеСтр + " / 100 Итог ["
			+ КС.ПраваяЧастьСтр + "]";
		УсловиеВыполнено = ЗначенияЗаполнены(Р3Ст30Гр03Знам.Значение, Р3Ст30Гр03Числ.Значение, Р3Ст80Гр03Знам.Значение, Р3Ст80Гр03Числ.Значение);
		Если НЕ УсловиеВыполнено ИЛИ (УсловиеВыполнено И КС.ЛеваяЧасть = КС.ПраваяЧасть) Тогда
			ЭтоОшибка = Ложь;
...
Процедура ПроверитьКС_1_20(СтруктураРезультата)
	...
	Для Каждого СтраницаР3 Из СтраницыРаздела3 Цикл
		...
		КС = ПоказателиКС(Р3Ст90Гр03.Значение, 1 / 4 * (Р3Ст20Гр03.Значение - Р3Ст25Гр03.Значение) * Р3Ст50Гр03Зн
			* Р3Ст70Гр03.Значение * Р3Ст80Гр03Зн / 100);
		Расшифровка = КС.ЛеваяЧастьСтр + " = 1 / 4 * (" + Р3Ст20Гр03.ЗначениеСтр + " - " + Р3Ст25Гр03.ЗначениеСтр + ") * "
			+ Р3Ст50Гр03Числ.ЗначениеСтр + " / " + Р3Ст50Гр03Числ.ЗначениеСтр + " * " + Р3Ст70Гр03.ЗначениеСтр + " * " + Р3Ст80Гр03Числ.ЗначениеСтр
...
Функция ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р1СтМ2090Гр01" Тогда
		Результат.Вставить("Имя", "П00001М209001");
		Результат.Вставить("Представление", Нстр("ru=''"));
