...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2017_1(Данные, УникальныйИдентификатор)
	...
	Если Не ЗначениеЗаполнено(Титульная.ДатаНачПат) И ЗначениеЗаполнено(Титульная.ОГРНИП) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указана дата начала действия патента", "Форма2017_1_Титульная", "КолМес"));
	КонецЕсли;
	...
	Для Каждого Стр Из Данные.ДанныеМногостраничныхРазделов.Форма2017_1_ЛистВ Цикл 
		...
		Если Не ЗначениеЗаполнено(СтрЗначение.КодНОУчет) Тогда 
			ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указан налоговый орган", "Форма2017_1_ЛистВ", "КодНОУчет", СтрЗначение.УИД));
		ИначеЕсли СтрДлина(СокрЛП(СтрЗначение.КодНОУчет)) <> 4 Тогда 
...
Функция ОсновныеСведенияЭлектронногоПредставления_Форма2017_1(Объект, УникальныйИдентификатор)
	...
	ОсновныеСведения.Вставить("ЧислНаемРаб",Формат(ДанныеУведомления.Форма2017_1_Лист2.ЧислНаемРаб, "ЧН=0"));
...
Процедура ЗаполнитьЛистыВ(ДанныеУведомления, ДеревоВыгрузки)
	...
	Для Каждого Стр Из ЛистыВСвернутые Цикл 
		...
		Для Каждого Данные Из ОтборСтроки Цикл
			...
			Если Не (Данные.КодОбъект = "16" Или Данные.КодОбъект = "17" Или Данные.КодОбъект = "18" Или Данные.КодОбъект = "19"
					 Или Данные.КодОбъект = "20" Или Данные.КодОбъект = "21" Или Данные.КодОбъект = "22" Или Данные.КодОбъект = "29") Тогда 
