	...
	Для Каждого ИсходныйПоказатель ИЗ ДанныеБухОтчетностиТекущийПериод Цикл
		...
		Иначе // Прогнозные данные делаем только для отчета о прибылях и убытках
			...
			Если ДанныеБухОтчетностиПредыдущийПериод[ИмяИсходногоПоказателя] <> Неопределено
				...
				// Для того чтобы пользователь мог проверить рассчитанные значения вручную
...
Процедура ДобавитьПоказателиФинансовойУстойчивости(Показатели, СтрокаОтчетности, ДанныеФинансовогоАнализа)
	...
	// Расчитаем процент наличия собственных средств
...
Процедура ВывестиРазделРискБанкротства(ТабличныйДокумент, СтрокаДанных, ПараметрыВывода, ЭтоОсновнаяФормаОтчета)
	...
	Если НЕ ЭтоОсновнаяФормаОтчета Тогда
		ОбластиМакета.РискБанкростваПояснение.Параметры.Заполнить(Показатели);
		ОбластиМакета.РискБанкростваПояснение.Параметры.ИндексАльтмана = ИндексАльтмана;
		ОбластиМакета.РискБанкростваПояснение.Параметры.ПояснениеОценкиРискБанкротства = Показатели.ПояснениеОценкиРискБанкротства;
		ТабличныйДокумент.Вывести(ОбластиМакета.РискБанкростваПояснение);
	Иначе
		// Диаграмма
		...
		ВывестиОбластьПоказатьРасчет("РезультатОценки", "РискБанкростваПояснение",ОбластиМакета, ТабличныйДокумент);
		
		СтрокаОбласти = ПараметрыВывода.ОбластиРасшифровки.Добавить();
		СтрокаОбласти.ИмяОбласти   = "РискБанкростваПояснение";
		СтрокаОбласти.ПерваяСтрока = ТабличныйДокумент.ВысотаТаблицы + 1;
	КонецЕсли;
...
Процедура ДобавитьВидДеятельности(ДанныеФинансовогоАнализа)
	...
	Если ЗначениеЗаполнено(ДанныеФинансовогоАнализа.Организация.КодОКВЭД) Тогда
