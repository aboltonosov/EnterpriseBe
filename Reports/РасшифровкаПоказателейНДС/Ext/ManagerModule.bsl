...
Процедура ПолучитьНаборДанныхНачислениеАвансыПолученныеУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток - РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * 18 / 118 КАК ЧИСЛО(15, 2))) КАК СуммаБезНДС,
	|	-(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * 18 / 118 КАК ЧИСЛО(15, 2))) КАК СуммаНДС
	|ПОМЕСТИТЬ ВТХозрасчетныйВсеОстаткиАвансыПолученные
	|ИЗ
	|	РегистрНакопления.РасчетыСКлиентамиПоДокументам.ОстаткиИОбороты(&НачалоПериода, &КонецПериодаГраница, , , ) КАК РасчетыСКлиентами
	...
	|	И -(ВЫРАЗИТЬ(РасчетыСКлиентами.ПредоплатаРеглКонечныйОстаток * 18 / 118 КАК ЧИСЛО(15, 2)))  <> 0 
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностямРасшифровка130СтрокиДекларации(Запрос, СтруктураВнешнихНаборов) 
	...
	|		СчетФактураПолученный.Дата КАК ДатаВходящегоСчетаФактуры
	|	ИЗ
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям(Запрос, СтруктураВнешнихНаборов) 
	...
	|			И (НЕ ЗаявлениеОВвозеТоваров.ОплатаПодтверждена 
	|					ИЛИ ЗаявлениеОВвозеТоваров.ДатаПодтвержденияОплаты > &КонецПериода) ТОГДА
...
Процедура ПолучитьНаборДанныхВычетНалоговыйАгент(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ЕСТЬNULL(СчетФактураНалоговыйАгентДокументыПоступления.Ссылка, НДСЗаписиКнигиПокупокОбороты.СчетФактура) КАК СчетФактура,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураНалоговыйАгент.ДокументыПоступления КАК СчетФактураНалоговыйАгентДокументыПоступления
	|		ПО НДСЗаписиКнигиПокупокОбороты.ДокументОплаты = СчетФактураНалоговыйАгентДокументыПоступления.Документ
