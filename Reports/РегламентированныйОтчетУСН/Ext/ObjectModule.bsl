...
Функция УниверсальнаяВыгрузка(Параметры) Экспорт
	...
	Результат.Вставить("ТекстВыгрузки", "");
	Результат.Вставить("КодировкаВыгрузки", "");
	...
	Если НЕ ВыгрузкаВозможна(СохраненныйОтчет, ПараметрыВыгрузки, РазделыОтчета, Результат) Тогда
		Результат.Вставить("ОтчетВыгружен", Ложь);
		Возврат Результат;
	КонецЕсли;
	
	ЗаполнитьВыгрузкуДанными(ДеревоВыгрузки, ПараметрыВыгрузки, РазделыОтчета, Результат);
	Если НЕ РегламентированнаяОтчетность.ПроверитьДанные(Результат, ДеревоВыгрузки) Тогда
		Результат.Вставить("ОтчетВыгружен", Ложь);
		Возврат Результат;
	КонецЕсли;
	
	ТекстВыгрузки = ВыгрузитьДеревоВXML(ДеревоВыгрузки, ПараметрыВыгрузки);
	КодировкаВыгрузки = "windows-1251";
	
	Результат.Вставить("ОтчетВыгружен", Истина);
	Результат.Вставить("ТекстВыгрузки", ТекстВыгрузки);
	Результат.Вставить("ИмяФайла", ПараметрыВыгрузки.ИдФайл);
	Результат.Вставить("КодировкаВыгрузки", КодировкаВыгрузки);
...
Функция НовыйТаблицаСообщенийОбОшибкахВыгрузки()
