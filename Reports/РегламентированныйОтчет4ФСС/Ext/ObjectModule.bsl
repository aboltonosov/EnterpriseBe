...
Процедура ЗаполнитьСведенияПП(ИмяФормы, ПрототипУзла_PAYM_ORDER, ДанныеПП)
	...
	Пока РегламентированнаяОтчетностьКлиентСервер.СвойствоОпределено(ДанныеМСЧ,
							ИмяПоказателяНомераПП + "_" + Формат(НомСтр, "ЧГ=0")) Цикл
...
Функция ВыгрузкаВозможна(ИмяФормы,
	...
	Если ВРег(СокрЛП(ИмяФормы)) = ВРег("ФормаОтчета2017Кв3") Тогда
		...
		Если ЗначениеЗаполнено(БюджетнаяОрганизация) Тогда
			
			Если ВозможныеЗначенияПризнакаБюджетнойОрганизации.Найти(БюджетнаяОрганизация) = Неопределено Тогда
		...
		Иначе
			...
			ВидПлательщикаСтраховыхВзносов = Неопределено;
			ОргСвед = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
			СтруктураРеквизитовФормы.мСохраненныйДок.Организация, ТекущаяДатаСеанса(), "ВидПлательщикаСтраховыхВзносов");
			ОргСвед.Свойство("ВидПлательщикаСтраховыхВзносов", ВидПлательщикаСтраховыхВзносов);
			
			Если ВидПлательщикаСтраховыхВзносов = "01" Тогда
				
				РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
				НСтр("ru='Не задан признак страхователя бюджетной организации.'"), "Титульный", "БюджетнаяОрганизация");
