...
Процедура ЗаполнитьСодержательнуюЧасть(УзелРодитель)
	...
	НомерСтроки = 1;
	Пока Данные11.Свойство("П000110000101_" + Формат(НомерСтроки, "ЧГ=")) Цикл
		...
		Для Каждого Эл Из ЭкзСумНалПУКод.Строки Цикл
			Если Эл.Строки.Количество() = 0 Тогда
				Если Эл.Ключ = "П100110000102" Тогда
					СуммаКУплате = Данные11["П000110000102_" + Формат(НомерСтроки, "ЧГ=")];
					СуммаКУменьш = Данные11["П000110000103_" + Формат(НомерСтроки, "ЧГ=")];
					ВывестиПоказательВXML(Эл, СуммаКУплате - СуммаКУменьш);
				Иначе
					ВывестиПоказательВXML(Эл, Данные11[Эл.Ключ + "_" + Формат(НомерСтроки, "ЧГ=")]);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		НомерСтроки = НомерСтроки + 1;
