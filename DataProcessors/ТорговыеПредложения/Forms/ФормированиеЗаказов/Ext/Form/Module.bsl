...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ТорговыеПредложения_ИзменитьКоличествоВКорзине"
		...
		ИзменитьКоличествоВКорзинеПоИдентификатору(Параметр.ПредложениеИдентификатор, Количество);
...
Процедура ПередЗагрузкойДанныхИзНастроекНаСервере(Настройки)
	...
	Если Организация.Пустая() Тогда
		...
		ПоказатьСкрытьКомандуПодключенияОрганизации(Элементы, Организация);
	КонецЕсли;
...
Процедура ОрганизацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ПоказатьСкрытьКомандуПодключенияОрганизации(Элементы, ВыбранноеЗначение);
...
Процедура ТоварыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.ТорговоеПредложение", ПараметрыФормы, ЭтаФорма,
...
Процедура ПроверкаЗаполненияСтраницаПодготовкаЗаказов(Отказ)
	...
	Если Элементы.ГруппаПодключитьОрганизацию.Видимость
		И Не ПодключитьОрганизацию Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не получено согласие на подключение организации к сервису 1С:Бизнес-сеть'"),,
			"ПодключитьОрганизацию",,Отказ);
	КонецЕсли;
...
Процедура ПоказатьСкрытьКомандуПодключенияОрганизации(Элементы, Знач Организация)
	
	Показать = Не Организация.Пустая() И Не ОрганизацияПодключена(Организация);
	Элементы.НадписьПодключитьОрганизацию.Видимость = Показать;
	Элементы.ГруппаПодключитьОрганизацию.Видимость = Показать;
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ОрганизацияПодключена(Знач Организация)
...
Процедура ВыполнитьДлительнуюОперацию(Знач ТекущаяСтраница, СледующаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаПодготовкаЗаказов
		...
		ДополнительныеПараметрыПроцедуры.Вставить("ПодключитьОрганизацию", ПодключитьОрганизацию);
...
Процедура ЗаполнитьПараметрыФормы()
	...
	ИначеЕсли Параметры.Свойство("МассивКорзины") Тогда
		...
		Для Каждого СоставПозиции Из Параметры.МассивКорзины Цикл
			...
			СтрокаКорзины.ИспользованиеХарактеристик = Не СтрокаКорзины.ХарактеристикаНоменклатуры.Пустая();
...
Процедура УстановитьУсловноеОформление()
	...
	// Устанавливаем условное оформление для характеристики
...
&НаСервере
Процедура ИзменитьКоличествоВКорзинеПоИдентификатору(ПредложениеИдентификатор, Количество)
...
Процедура ЗаполнитьНоменклатуруПоНоменклатуреПоставщика(СтрокаТовары = Неопределено)
	...
	Для Каждого СтрокаТоваров Из СтрокиТоваров Цикл
		Если Не ЗначениеЗаполнено(СтрокаТоваров.Номенклатура) Тогда
			...
			ДополнительныеРеквизиты.Вставить("Идентификатор", Сред(СтрокаТоваров.ПредложениеИдентификатор,
...
Процедура ОповеститьОбИзмененииКоличества(ПредложениеИдентификатор, ПоставщикИдентификатор, Количество)
	...
	Оповестить("ТорговыеПредложения_ИзменитьКоличествоВКорзине", ПараметрыОповещения, ЭтаФорма);
