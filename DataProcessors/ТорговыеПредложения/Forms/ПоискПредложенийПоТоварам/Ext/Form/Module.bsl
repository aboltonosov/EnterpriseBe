...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не БизнесСеть.ОрганизацияПодключена() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Требуется подключение к сервису 1С:Бизнес-сеть.'"),,,, Отказ);
		Возврат;
	КонецЕсли;
	...
	ОбновитьНадписьПодобраноТоваров(ЭтаФорма);
	...
	ТорговыеПредложенияПереопределяемый.ПриСозданииНаСервере(ЭтаФорма);
...
Процедура ОбработкаОповещенияНаСервере(ИмяСобытия)
	...
	ТорговыеПредложенияПереопределяемый.ОбработатьОповещение(ЭтаФорма, ИмяСобытия);
...
Процедура ПоискПредложений(Команда)
	...
	Обработчик = Новый ОписаниеОповещения("ПоискПредложенийПродолжение", ЭтаФорма);
...
Процедура Подключаемый_ПереопределяемаяКоманда(Команда)
	...
	ТорговыеПредложенияКлиентПереопределяемый.ОбработатьКомандуФормы(ЭтаФорма, Команда);
	ОбновитьНадписьПодобраноТоваров(ЭтаФорма);
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Для каждого ЭлементСписка Из Поставщики Цикл
		...
		НовыйЭлемент.КартинкаЗначений = БиблиотекаКартинок.БизнесСеть;
...
Процедура ОткрытьФормуОформленияЗаказов()
	...
	ОткрытьФорму("Обработка.ТорговыеПредложения.Форма.ФормированиеЗаказов", ПараметрыОткрытия, ЭтаФорма);
...
Процедура ОбработкаВыбораНаСервере(ВыбранноеЗначение, ДополнительныеПараметры = Неопределено)
	...
	ТорговыеПредложенияПереопределяемый.ОбработатьВыбор(ЭтаФорма, ВыбранноеЗначение, ДополнительныеПараметры);
	ОбновитьНадписьПодобраноТоваров(ЭтаФорма);
...
Процедура ИнформацияПоТоваруНажатие(Элемент, СтандартнаяОбработка)
	...
	Оповестить("ОбновитьСписокТорговыхПредложенийПоПараметрам", ПараметрыОткрытия);
...
Процедура УстановитьУсловноеОформление()
	...
	ТорговыеПредложенияПереопределяемый.УстановитьУсловноеОформлениеЕдиницИзмерения(ЭтаФорма, "СписокНоменклатураЕдиницаИзмерения", "Список.Упаковка");
