...
//       ** ПодключитьОрганизацию - Булево - необходимость подключения организации к сервису 1С:Бизнес-сеть.
...
Процедура СформироватьЗаказы(Знач ПараметрыПроцедуры, Знач АдресРезультата) Экспорт
	...
	Отказ = Ложь;
	Если Не БизнесСеть.ОрганизацияПодключена(Организация) Тогда // Подключение организации к сервису, если она не подключена.
		Если ПараметрыПроцедуры.ДополнительныеПараметры.ПодключитьОрганизацию Тогда
			...
			Если Не ЗначениеЗаполнено(СтрокаЗаказа.НоменклатураПоставщика) Тогда
				...
				Если ЗначенияРеквизитовНоменклатурыПоставщика <> Неопределено
					...
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("Реквизит, СсылкаНаОбъект",
							"Номенклатура", СтрокаЗаказа.Номенклатура));
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("Реквизит, ЗначениеРеквизита",
					...
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("Реквизит, ЗначениеРеквизита",
							"Ид", ИдентификаторНоменклатурыПоставщика));
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("Реквизит, ЗначениеРеквизита",
							"Упаковка", СтрокаЗаказа.Упаковка));
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("Реквизит, ЗначениеРеквизита",
							"Владелец", Партнер));
					ЗаполнитьЗначенияСвойств(ДеревоДанных.Строки.Добавить(),
						Новый Структура("ТипОбъекта, СсылкаНаОбъект",
							"Партнеры", Партнер));
...
// Получение списка корневых категорий рубрикатора.
//
// Параметры:
...
//
Процедура ПолучитьКатегорииПервогоУровня(СтруктураПараметров, АдресХранилища) Экспорт
