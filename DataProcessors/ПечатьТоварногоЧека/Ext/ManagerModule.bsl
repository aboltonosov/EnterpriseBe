...
Функция СформироватьПечатнуюФормуТоварныйЧек(СтруктураТипов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		// 4D:ERP для Беларуси, Петр, 01.08.2017 16:23:50 
		// Перенести макеты печатных форм из 2.1.3.266 в 2.2.3.138, №15620
		// {
		ЗаполнитьТабличныйДокументТоварныйЧек_Локализация(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати);
		// }
		// 4D
...
// 4D:ERP для Беларуси, Петр, 01.08.2017 16:25:04 
// <Перенести макеты печатных форм из 2.1.3.266 в 2.2.3.138>, №15620
// {
Процедура ЗаполнитьТабличныйДокументТоварныйЧек_Локализация(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	
	ШаблонОшибкиТовары = НСтр("ru = 'В документе %1 отсутствуют товары. Печать товарного чека не требуется'");
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьТоварногоЧека.ПФ_MXL_ТоварныйЧек_Локализация");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			Иначе
				...
				ОбластьДанных.Параметры.Цена = 
					Формат(Окр(СтрокаТовары.Цена, 2), 
					"ЧЦ=15; ЧДЦ=" + 2);
			КонецЕсли;
			...
			Если Не ДанныеПечати.ЦенаВключаетНДС Тогда
				СуммаСНДС = Окр(СтрокаТовары.Сумма + СтрокаТовары.СуммаНДС, 2);
			Иначе
				СуммаСНДС = Окр(СтрокаТовары.Сумма, 2);
			КонецЕсли;
			
			Если НаборыСервер.ВыводитьТолькоЗаголовок(СтрокаТовары, ИспользоватьНаборы) Тогда
				ОбластьСуммы.Параметры.Заполнить(ПустыеДанные);
			Иначе
				УстановитьПараметр(ОбластьСуммы, "Сумма", Формат(Окр(СуммаСНДС, 2), "ЧЦ=15; ЧДЦ=" + 2));
			КонецЕсли;
			
			ТабличныйДокумент.Присоединить(ОбластьСуммы);
			
			Если Не НаборыСервер.ИспользоватьОбластьКомплектующие(СтрокаТовары, ИспользоватьНаборы) Тогда
				Сумма    = Сумма    + Окр(СтрокаТовары.Сумма, 2);
				СуммаНДС = СуммаНДС + Окр(СтрокаТовары.СуммаНДС, 2);
			КонецЕсли;
		...
		ОбластьСуммы.Параметры.Сумма = Формат(СуммаДокумента, "ЧЦ=15; ЧДЦ=" + 2);
		ТабличныйДокумент.Присоединить(ОбластьСуммы);
		
		// Вывести Сумму прописью.
		ОбластьМакета = Макет.ПолучитьОбласть("СуммаПрописью");
		ОбластьМакета.Параметры.ИтоговаяСтрока = "Всего наименований " + НомерСтроки
		                                       + ", на сумму " + ФормированиеПечатныхФорм.ФорматСумм(ДанныеПечати.СуммаДокумента, ДанныеПечати.Валюта);
		ОбластьМакета.Параметры.СуммаПрописью  = РаботаСКурсамиВалют.СформироватьСуммуПрописью(ДанныеПечати.СуммаДокумента, ДанныеПечати.Валюта);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		ОбластьМакета = Макет.ПолучитьОбласть("Подписи");
		ОбластьМакета.Параметры.Заполнить(ДанныеПечати);
		ОбластьМакета.Параметры.ОтветственныйПредставление = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.Ответственный);
