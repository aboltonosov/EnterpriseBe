...
Процедура ПродолжитьТестирование()
	...
	Если ЗакрытиеМесяцаУТВызовСервера.ФоновоеЗаданиеЗакрытияМесяцаАктивно() Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьЗапись(ПутьКТекущемуФайлу, ТекущаяЗаписьXML)
	ПутьКТекущемуФайлу = "" + РабочийКаталог + Формат(ТекущаяДата(), "ДФ=dd-MM-yyyy") + "_" + Строка(Новый УникальныйИдентификатор()) + ".xml";
...
Функция ЭталонныеДанные21(ТестируемыйМесяц)
	...
	Если Результат = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'В каталоге движений ERP 2.1 не найден файл выгрузки движений за %ТестируемыйМесяц'");
		ТекстИсключения = СтрЗаменить(ТекстИсключения, "%ТестируемыйМесяц", Формат(ТестируемыйМесяц,"ДФ='MMMM yyyy'"));
