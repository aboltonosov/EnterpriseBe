	...
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		...
		Если Заказы.Количество() = 1 Тогда
			...
			Если РеквизитыЗаказа.УслугиПоПереработке = Перечисления.ВариантыОформленияУслугДокументовПереработки.УказываютсяВЗаказеОтчете Тогда
				...
				ТекстОтгрузка = СтрЗаменить(ТекстОтгрузка, "%СуммаПоступления%",   Формат(СтруктураСостоянияРасчетов.СуммаПоступления, "ЧЦ=15; ЧДЦ=2; ЧН="));
				ТекстОтгрузка = СтрЗаменить(ТекстОтгрузка, "%Валюта%",          РеквизитыЗаказа.Валюта);
...
//-- НЕ УТКА
	...
	Заголовок = НСтр("ru='Помощник закрытия '") + ТекстРП;
...
//++ НЕ УТКА
	ИначеЕсли ТипЗаказа = "ЭтапПроизводства2_2" Тогда
		ТекстТЧОтгрузка = НСтр("ru='обеспечения материалами и работами'");
		ТекстТЧПриемка = НСтр("ru='выходных и побочных изделий'");
	ИначеЕсли ТипЗаказа = "ЗаказДавальца" Тогда
		ТекстТЧОтгрузка = НСтр("ru='выпускаемой продукции'");
		ТекстТЧПриемка = НСтр("ru='сырья и материалов для производства'");
//-- НЕ УТКА
//++ НЕ УТ
	ИначеЕсли ТипЗаказа = "ЗаказПереработчику" Тогда
		ТекстТЧОтгрузка = НСтр("ru='сырья и материалов'");
		ТекстТЧПриемка = НСтр("ru='выходных изделий'");
//-- НЕ УТ
	КонецЕсли;
...
//++ НЕ УТ
	ИначеЕсли ТипЗнч(Заказы[0].Значение) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ИСТИНА КАК ПереработкаПоЗаказу
		|ИЗ
		|	Документ.ЗаказПереработчику КАК ЗаказПереработчику
		|ГДЕ
		|	ЗаказПереработчику.ПереработкаПоЗаказу = ИСТИНА
		|	И ЗаказПереработчику.Ссылка В (&Заказы)";
		
		ПереработкаПоЗаказу = Не Запрос.Выполнить().Пустой();
		
		ИспользуетсяСтатусЗакрыт = ПолучитьФункциональнуюОпцию("ИспользоватьСтатусыЗаказовПереработчикам") Или ПереработкаПоЗаказу;
...
Процедура ЗаполнитьОтклоненияПоЗаказамПереработчикам()
	...
	Распоряжения = Заказы.ВыгрузитьЗначения();
	...
	|ГДЕ
	|	Заказ.Ссылка В(&Распоряжения)
	|	И Заказ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.Закрыт)
	|	И &ИспользоватьСтатусЗакрыт";

	ТекстЗапросаТаблицыДокумента = Документы.ЗаказПереработчику.ТекстЗапросаТаблицыДокументаДляКорректировки();
	
	ТекстыЗапроса = Новый Массив();
	ТекстыЗапроса.Добавить(ТекстЗапросаРаспоряжения);
	ТекстыЗапроса.Добавить(НакладныеСервер.ТекстРазделителяЗапросов());
	ТекстыЗапроса.Добавить(ТекстЗапросаТаблицыДокумента);
	
	Запрос.Текст = СтрСоединить(ТекстыЗапроса);
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	ВыборкаЗаказы           = РезультатЗапроса[0];
	ТаблицаМатериалы        = РезультатЗапроса[1].Выгрузить();
	ТаблицаПродукция        = РезультатЗапроса[2].Выгрузить();
	ТаблицаВозвратныеОтходы = РезультатЗапроса[3].Выгрузить();
	...
	ПараметрыЗаполнения.ЗаполнятьКоличествоОтклонением          = Истина;
	...
	ЗаказыСервер.СкорректироватьОтгрузкуПриемку(Распоряжения, ТаблицаВозвратныеОтходы, ПараметрыЗаполнения);
	
	ВыделитьТоварыСОтклонениями(ТаблицаМатериалы);
	ВыделитьТоварыСОтклонениями(ТаблицаПродукция);
	ВыделитьТоварыСОтклонениями(ТаблицаВозвратныеОтходы);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаМатериалы,        МерныеТоварыККорректировке);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПродукция,        МерныеТоварыККорректировке);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаВозвратныеОтходы, МерныеТоварыККорректировке);
	
	Если ВыборкаЗаказы.Пустой()
...
Процедура ЗаполнитьОтклоненияПоЗаказамМатериаловВПроизводство()
	...
	ТекстЗапросаТаблицыДокумента = Документы.ЗаказМатериаловВПроизводство.ТекстЗапросаТаблицыДокументаДляКорректировки();
	...
	ЗаказыСервер.СкорректироватьОтгрузкуПриемку(Распоряжения, ТаблицаТовары, ПараметрыЗаполнения);
	
	ВыделитьТоварыСОтклонениями(ТаблицаТовары);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаТовары, МерныеТоварыККорректировке);
...
Процедура ЗаполнитьОтклоненияПоЗаказамДавальца()
	...
	|ГДЕ
	|	Заказ.Ссылка В(&Распоряжения)
	|	И Заказ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.Закрыт)
	|	И &ИспользоватьСтатусЗакрыт";

	ТекстЗапросаТаблицыДокумента = Документы.ЗаказДавальца.ТекстЗапросаТаблицыДокументаДляКорректировки();
...
Процедура ЗаполнитьОтклоненияПоЭтапамПроизводства()
	...
	|	Заказ.Ссылка КАК Заказ
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Заказ
	|ГДЕ
	|	Заказ.Ссылка В(&Распоряжения)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	Заказ.ЗаказПереработчику КАК Заказ
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК Заказ
	|ГДЕ
	|	Заказ.Ссылка В(&Распоряжения)
	|	И Заказ.ЗаказПереработчику <> ЗНАЧЕНИЕ(Документ.ЗаказПереработчику.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|
	...
	|ГДЕ
	|	Заказ.Ссылка В(&Распоряжения)
	|	И Заказ.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Завершен)
	|	И &ИспользоватьСтатусЗакрыт";
	
	ТекстЗапросаТаблицыДокумента = Документы.ЭтапПроизводства2_2.ТекстЗапросаТаблицыДокументаДляКорректировки();
	...
	ВсеРаспоряжения          = РезультатЗапроса[0].Выгрузить();
	ВыборкаЗаказы            = РезультатЗапроса[1];
	ТаблицаМатериалы         = РезультатЗапроса[2].Выгрузить();
	ТаблицаПродукция         = РезультатЗапроса[3].Выгрузить();
	ТаблицаПобочнаяПродукция = РезультатЗапроса[4].Выгрузить();
	
	ИспользуетсяПередачаПереработчику = Ложь;
	Для Каждого Распоряжение Из Распоряжения Цикл
		Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
			ИспользуетсяПередачаПереработчику = Истина;
			Прервать;
		ИначеЕсли ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЭтапПроизводства2_2")
			И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Распоряжение, "ПроизводствоНаСтороне") Тогда
			ИспользуетсяПередачаПереработчику = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	ЗаказыСервер.СкорректироватьОтгрузкуПриемку(ВсеРаспоряжения, ТаблицаМатериалы, ПараметрыЗаполнения);
	...
	ЗаказыСервер.СкорректироватьОтгрузкуПриемку(ВсеРаспоряжения, ТаблицаПродукция, ПараметрыЗаполнения);
	
	ПараметрыЗаполнения.ИмяТабличнойЧасти                       = "ПобочныеИзделия";
	
	ЗаказыСервер.СкорректироватьОтгрузкуПриемку(ВсеРаспоряжения, ТаблицаПобочнаяПродукция, ПараметрыЗаполнения);
	
	ВыделитьТоварыСОтклонениями(ТаблицаМатериалы);
	ВыделитьТоварыСОтклонениями(ТаблицаПродукция);
	ВыделитьТоварыСОтклонениями(ТаблицаПобочнаяПродукция);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаМатериалы,         МерныеТоварыККорректировке);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПродукция,         МерныеТоварыККорректировке);
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаПобочнаяПродукция, МерныеТоварыККорректировке);
	...
	Если ИспользуетсяПередачаПереработчику Тогда
		ИндексовРаспоряжений = Заказы.Количество() - 1;
		Для Индекс = 0 По ИндексовРаспоряжений Цикл
			Строка = Заказы[ИндексовРаспоряжений - Индекс];
			Распоряжение = Строка.Значение;
			Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
...
//++ НЕ УТ
&НаСервере
Процедура ВыделитьТоварыСОтклонениями(Таблица)
	
	КоличествоИндексов = Таблица.Количество() - 1;
	Для Счетчик = 0 По КоличествоИндексов Цикл
		Индекс = КоличествоИндексов - Счетчик;
...
Функция СформироватьТекстОбработано()
	...
	Если ТипЗаказа = "ЗаказКлиента" Тогда
		ТекстДля = НСтр("ru='Для заказов клиентов (%КоличествоОбработанных%) '");
	ИначеЕсли ТипЗаказа = "ЗаказПоставщику" Тогда
		ТекстДля = НСтр("ru='Для заказов поставщикам (%КоличествоОбработанных%) '");
	ИначеЕсли ТипЗаказа = "ЗаказНаСборку" Тогда
		ТекстДля = НСтр("ru='Для заказов на сборку (%КоличествоОбработанных%) '");
	ИначеЕсли ТипЗаказа = "ЗаказНаВнутреннееПотребеление" Тогда
		ТекстДля = НСтр("ru='Для заказов на внутреннее потребеление (%КоличествоОбработанных%) '");
	ИначеЕсли ТипЗаказа = "ЗаказНаПеремещение" Тогда
		ТекстДля = НСтр("ru='Для заказов на перемещение (%КоличествоОбработанных%) '");
	ИначеЕсли ТипЗаказа = "ЗаявкаНаВозвратТоваровОтКлиента" Тогда
		ТекстДля = НСтр("ru='Для заявок на возврат товаров от клиента (%КоличествоОбработанных%) '");
	КонецЕсли;
	...
	Возврат ТекстДля + ТекстДействия;
...
Функция ТекстНеудачнойОбработки(Закрывать)
	...
	ТекстДействия = НСтр("ru='Не удалось '");
	
	Если Закрывать Тогда 
		ТекстДействия = ТекстДействия + НСтр("ru='установить статус ""Закрыт"".'");
	Иначе
		
		Если СкорректироватьМерныеТовары Тогда
			ТекстДействия = ТекстДействия + НСтр("ru='скорректировать строки мерных товаров'");
		КонецЕсли;
		
		Если СкорректироватьЦены Тогда
			ТекстДействия = ТекстДействия + ?(ТекстДействия="","",", ") + НСтр("ru='скорректировать цены'");
		КонецЕсли;
		
		Если ОтменитьНеотработанныеСтроки Тогда
			ТекстДействия = ТекстДействия + ?(ТекстДействия="","",", ") + НСтр("ru='отменить непоставленные строки'");
		КонецЕсли;
		
		ТекстДействия = ТекстДействия + ".";
