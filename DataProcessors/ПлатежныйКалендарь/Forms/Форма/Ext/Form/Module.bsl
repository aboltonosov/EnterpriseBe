...
&НаКлиенте
Процедура ГруппировкаПриоритетСтатьиДДС(Команда)
	
	ИзменитьГруппировкуПлатежей("ПриоритетСтатьиДДС");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаПоСтатьеДДС(Команда)
	
	ИзменитьГруппировкуПлатежей("СтатьяДДС");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаПоПриоритетуЗаявки(Команда)
	
	ИзменитьГруппировкуПлатежей("ПриоритетЗаявки");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаПоХозОперации(Команда)
	
	ИзменитьГруппировкуПлатежей("ХозяйственнаяОперация");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаПоПодразделению(Команда)
	
	ИзменитьГруппировкуПлатежей("Подразделение");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаПоПлательщикуПолучателю(Команда)
	
	ИзменитьГруппировкуПлатежей("ПлательщикПолучатель");
	
КонецПроцедуры

&НаКлиенте
Процедура ГруппировкаБезГруппировки(Команда)
	
	ИзменитьГруппировкуПлатежей();
...
Процедура СформироватьСписанияДС(Команда)
	...
	ПараметрыПомощника.Вставить("АдресДокументовДляФормированияПлатежей", 
		АдресДокументовДляФормированияПлатежей());
	
	ОткрытьФорму("Обработка.ПлатежныйКалендарь.Форма.ПомощникФормированияПлатежныхПоручений",
		ПараметрыПомощника,
		ЭтаФорма);
...
&НаСервере
Функция АдресДокументовДляФормированияПлатежей()
	...
	ВыбранныеПоля.Добавить("НазначениеПлатежа");
	ВыбранныеПоля.Добавить("ПлательщикПолучатель");
	ВыбранныеПоля.Добавить("ХозяйственнаяОперация");
	...
	ТаблицаЗаявок.Колонки.ОбъектОплаты.Имя = "Ссылка";
	ТаблицаЗаявок.Колонки.Сумма.Имя = "СуммаКОплате";
	ТаблицаЗаявок.Колонки.БанковскийСчетКасса.Имя = "БанковскийСчет";
	
	Для каждого СтрокаЗаявки Из ТаблицаЗаявок Цикл
		Если СтрокаЗаявки.СуммаКОплате < 0 Тогда
			СтрокаЗаявки.СуммаКОплате = -СтрокаЗаявки.СуммаКОплате;
		КонецЕсли;
	КонецЦикла;
	
	Возврат ПоместитьВоВременноеХранилище(ТаблицаЗаявок, УникальныйИдентификатор);
...
&НаКлиенте
Процедура ИзменитьГруппировкуПлатежей(ИмяГруппировки = "")
