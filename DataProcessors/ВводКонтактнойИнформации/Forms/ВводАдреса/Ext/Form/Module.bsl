...
// 4D:ERP для Беларуси, ВладимирР, 27.06.2017 11:31:01 
// Адресный и банковский классификаторы, №10377 (2)
// {
&НаСервереБезКонтекста
Функция АдресПоКлассификатору(ТекущаяСтрана) Экспорт
	Возврат АдресныйКлассификаторСлужебный_Локализация.АдресПоКлассификатору(ТекущаяСтрана);
КонецФункции
// }
// 4D
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если АдресТолькоРоссийский  Тогда
		...
		// Адресный и банковский классификаторы, №10377 (2)
		// {
		Если Не АдресПоКлассификатору(Страна) Тогда
		// }
		// 4D
			...
			// Адресный и банковский классификаторы, №10377 (2)
			// {
			ТекстПредупрежденияПриОткрытии = НСтр("ru = 'Адрес введен некорректно: допускается ввод только белорусских и российских адресов. Значение поля ""Страна"" было изменено на ""Беларусь"", необходимо проверить остальные поля.'"); 
			// }
			// 4D
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если АдресПоКлассификатору(Страна) Тогда
	// }
	// 4D
...
Процедура ФормаВыбораУлицПоИндексу(Знач Текст, СтандартнаяОбработка)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если СтрДлина(СокрЛП(Текст)) = 6 И Индекс <> Текст И АдресПоКлассификатору(Страна) Тогда
	// }
	// 4D
		...
		Если ЕстьКлассификатор Тогда
			...
			ИначеЕсли ЕстьПравоЗагружатьКлассификатор Тогда
				...
				// Адресный и банковский классификаторы, №10377 (2)
				// {
				Если СтранаРоссия <> ПредопределенноеЗначение("Справочник.СтраныМира.Беларусь") И ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
				// }
				// 4D
			...
			Если НаселенныйПунктДетально <> Неопределено Тогда
				...
				Для каждого ЧастьАдреса Из НовыеДанныеПоНаселенномуПунктуДетально Цикл
					Если ЧастьАдреса.Значение.Уровень < 7 Тогда
						...
						// 4D:ERP для Беларуси, ВладимирР, 14.08.2017 10:03:05 
						// Загрузка адресного классификатора, №15204
						// {
						Если НаселенныйПунктДетально.Свойство(ЧастьАдреса.Ключ) Тогда
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	ИначеЕсли АдресПоКлассификатору(Контекст.Страна) И ПустаяСтрока(XDTOИнформация.Представление) Тогда
	// }
	// 4D
...
Процедура ОбработкаИзмененияСтраныКлиент()
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	НаселенныйПунктДетальноКопия = НаселенныйПунктДетально;
	
	Если Страна = СтранаРоссия Тогда
		НаселенныйПунктДетально = АдресныйКлассификаторСлужебный_Локализация.СтруктураЧастейАдресаНаселенногоПункта();
	Иначе
		НаселенныйПунктДетально = УправлениеКонтактнойИнформациейКлиентСервер.СтруктураЧастейАдресаНаселенногоПункта("ФИАС");
	КонецЕсли;
	
	Для каждого Стр Из НаселенныйПунктДетальноКопия Цикл
		Если НаселенныйПунктДетально.Свойство(Стр.Ключ) Тогда
			НаселенныйПунктДетально[Стр.Ключ].Идентификатор = Стр.Значение.Идентификатор;
			НаселенныйПунктДетально[Стр.Ключ].Наименование = Стр.Значение.Наименование;
			НаселенныйПунктДетально[Стр.Ключ].Представление = Стр.Значение.Представление;
			НаселенныйПунктДетально[Стр.Ключ].Сокращение = Стр.Значение.Сокращение;
			НаселенныйПунктДетально[Стр.Ключ].Уровень = Стр.Значение.Уровень;
		КонецЕсли;
	КонецЦикла;
	
	ЭтоРоссийскийАдрес = АдресПоКлассификатору(Страна);
	// }
	// 4D
...
Процедура УстановитьИндексИПредставление(Контекст, XDTOКонтактная = Неопределено)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если АдресПоКлассификатору(Контекст.Страна) Тогда
	// }
	// 4D
...
Процедура ЗначениеРеквизитовПоКонтактнойИнформации(Контекст, РедактируемаяИнформация)
	...
	Иначе
		...
		// Адресный и банковский классификаторы, №10377 (2)
		// {
		СсылкаНаРоссию = Справочники.СтраныМира.Беларусь;
		// }
		// 4D
	...
	Иначе
		...
		Если Контекст.Страна = Контекст.СтранаРоссия Тогда
			Контекст.НаселенныйПунктДетально = АдресныйКлассификаторСлужебный_Локализация.СтруктураЧастейАдресаНаселенногоПункта();
		Иначе
...
Функция КонтактнаяИнформацияПоЗначениюРеквизитов(Контекст)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если Не АдресПоКлассификатору(Контекст.Страна) Тогда
	// }
	// 4D
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 1 Тогда
		...
		ИначеЕсли ПустаяСтрока(МестоОшибки) Или МестоОшибки = "/" Тогда
			...
			// Адресный и банковский классификаторы, №10377 (2)
			// {
			Если СтранаРоссия <> ПредопределенноеЗначение("Справочник.СтраныМира.Беларусь") Тогда
...
Функция КонтекстФормыКлиент(НовыеДанные = Неопределено)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	НовыеДанные.ПредставлениеАдреса = СтрЗаменить(НовыеДанные.ПредставлениеАдреса, "Беларусь Респ, ", "");
	// }
	// 4D
