...
//
// Параметры
//	МесяцНачисленияПараметр - тип Дата, месяц начисления зарплаты.
//	ОрганизацияПараметр - тип СправочникСсылка.Организации
//
Процедура Инициализировать(МесяцНачисленияПараметр, ОрганизацияПараметр) Экспорт
	
	МесяцНачисления = МесяцНачисленияПараметр;
	...
	Зарплата.Взносы = Новый ТаблицаЗначений;
...
//		если не задан - для выявления удержаний будут использованы сотрудники из ТаблицаСотрудников за период МесяцНачисления.
//
Процедура ЗаполнитьНачислениеЗарплаты(ТаблицаСотрудников, Отборы = Неопределено, ТаблицаУдержаний = Неопределено) Экспорт
	
	Начисления = НачисленияЗарплаты(ТаблицаСотрудников, Отборы);
	...
	Если ТаблицаУдержаний = Неопределено Тогда
		...
		ТаблицаУдержаний.ЗаполнитьЗначения(НачалоМесяца(МесяцНачисления), "ДатаНачала");
		ТаблицаУдержаний.ЗаполнитьЗначения(КонецМесяца(МесяцНачисления), "ДатаОкончания");
	КонецЕсли;
...
Функция ТаблицаИсходныеДанныеНачисленияЗарплатыПоНачислениям() Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 29.11.2017 15:27:29 
	// Документ "Больничный лист. Доработать заполнение процента оплаты, №15884 
	// {
	Начисления.Колонки.Добавить("ПроцентОплаты", Новый ОписаниеТипов("Число"));
	// }
	// 4D
...
Функция НачисленияЗарплаты(ТаблицаСотрудников, Знач Отборы = Неопределено) 
...
Процедура РазбитьВТНачисленияПоДатамНачисленийЗависящихОтСтажа(УсловияРазбиения)
	...
	Запрос = НовыйЗапрос();
	Запрос.Текст = 
	// 4D:ERP для Беларуси, Яна, 23.08.2017 12:35:45 
	...
	Запрос.Текст = 
		...
		|	МАКСИМУМ(ПериодыИзмененияСтажаСледующие.ДатаСтажа) КАК ДатаСтажаПредыдущая
		|ПОМЕСТИТЬ ВТПериодыИзмененияСтажаСПредыдущимиДатами
		|ИЗ
		|	ВТПериодыИзмененияСтажа КАК ПериодыИзмененияСтажа
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПериодыИзмененияСтажа КАК ПериодыИзмененияСтажаСледующие
		|		ПО ПериодыИзмененияСтажа.Сотрудник = ПериодыИзмененияСтажаСледующие.Сотрудник
		|			И ПериодыИзмененияСтажа.ВидСтажа = ПериодыИзмененияСтажаСледующие.ВидСтажа
		|			И ПериодыИзмененияСтажа.ДатаСтажа > ПериодыИзмененияСтажаСледующие.ДатаСтажа
		|
		|СГРУППИРОВАТЬ ПО
		|	ПериодыИзмененияСтажа.ДатаСтажа,
		|	ПериодыИзмененияСтажа.Сотрудник,
		...
		|	ПериодыИзмененияСтажа.ВидСтажа,
		|	ПериодыИзмененияСтажа.ЗначениеСтажа,
		|	ПериодыИзмененияСтажаПредыдущие.ЗначениеСтажа КАК ЗначениеСтажаПредыдущее
		|ПОМЕСТИТЬ ВТПериодыИзмененияСтажаСПредыдущими
		|ИЗ
		|	ВТПериодыИзмененияСтажаСПредыдущимиДатами КАК ПериодыИзмененияСтажа
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПериодыИзмененияСтажа КАК ПериодыИзмененияСтажаПредыдущие
		|		ПО ПериодыИзмененияСтажа.Сотрудник = ПериодыИзмененияСтажаПредыдущие.Сотрудник
		|			И ПериодыИзмененияСтажа.ВидСтажа = ПериодыИзмененияСтажаПредыдущие.ВидСтажа
		|			И ПериодыИзмененияСтажа.ДатаСтажаПредыдущая = ПериодыИзмененияСтажаПредыдущие.ДатаСтажа";
	Запрос.Выполнить();
	УдалитьВТ.Добавить("ВТПериодыИзмененияСтажаСПредыдущимиДатами");
	УдалитьВТ.Добавить("ВТПериодыИзмененияСтажаСПредыдущими");
	
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Начисления.ИдентификаторСтроки,
		|	ДатыИзмененияСтажаСотрудников.ДатаСтажа КАК Дата
		|ПОМЕСТИТЬ ВТДатыПоИдентификатору
		|ИЗ
		|	ВТНачисления КАК Начисления
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПоказателиРасчета КАК ПоказателиРасчета
		|		ПО (ПоказателиРасчета.ВидРасчета = Начисления.Начисление)
		|			И (ПоказателиРасчета.ПоказательЧисловойЗависящийОтСтажа = ИСТИНА)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПериодыИзмененияСтажаСПредыдущими КАК ДатыИзмененияСтажаСотрудников
		|		ПО Начисления.Сотрудник = ДатыИзмененияСтажаСотрудников.Сотрудник
		|			И (ПоказателиРасчета.Показатель.ВидСтажа = ДатыИзмененияСтажаСотрудников.ВидСтажа)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПоказателиРасчетаЗарплаты.ШкалаОценкиСтажа КАК ШкалаОценкиСтажа
		|		ПО (ШкалаОценкиСтажа.Ссылка = ПоказателиРасчета.Показатель)
		|			И (ВЫБОР
		|				КОГДА ШкалаОценкиСтажа.ВерхняяГраницаИнтервалаСтажа <= ДатыИзмененияСтажаСотрудников.ЗначениеСтажа
		|						И ШкалаОценкиСтажа.ВерхняяГраницаИнтервалаСтажа > ЕСТЬNULL(ДатыИзмененияСтажаСотрудников.ЗначениеСтажаПредыдущее, ДатыИзмененияСтажаСотрудников.ЗначениеСтажа)
		|					ТОГДА ИСТИНА
		|				КОГДА ДатыИзмененияСтажаСотрудников.ЗначениеСтажа = 0
		|					ТОГДА ИСТИНА
		|				ИНАЧЕ ЛОЖЬ
		|			КОНЕЦ)";
	Запрос.Выполнить();
...
Функция РанееОплаченныеПериодыОкончательныйРасчет(ОписаниеВТСПериодами)
	...
	Запрос.Текст = 
		...
		|	ВТСотрудникиПериоды КАК СотрудникиПериоды
		|ГДЕ
		|	СотрудникиПериоды.ДатаНачала >= &МесяцНачисления
		|	И 1 В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				1
		|			ИЗ
		...
		|			ГДЕ
		|				Начисления.ПериодРегистрации <= &МесяцНачисления
		|				И Начисления.Регистратор <> &ИсключаемыйРегистратор
		|				И Начисления.Регистратор <> &ИсправленныйДокумент
		|				И Начисления.Сотрудник = СотрудникиПериоды.Сотрудник
		|				И Начисления.ПериодДействия = НАЧАЛОПЕРИОДА(СотрудникиПериоды.ДатаНачала, МЕСЯЦ))";
	...
	Если РезультатЗапроса.Пустой() Тогда
		Возврат НовыйРанееОплаченныеПериоды();	
	КонецЕсли;
...
Функция НовыйРанееОплаченныеПериоды()
	РанееОплаченныеПериоды = Новый ТаблицаЗначений;
	РанееОплаченныеПериоды.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники")); 
	РанееОплаченныеПериоды.Колонки.Добавить("ДатаНачала", Новый ОписаниеТипов("Дата")); 
	РанееОплаченныеПериоды.Колонки.Добавить("ДатаОкончания", Новый ОписаниеТипов("Дата"));
...
Процедура ЗаполнитьЗначенияПереопределяемыхПоказателейНачислений(ТребованияНачислений, СтрокиПоИдентификатору)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.РассчитатьПоказательСреднемесячноеКоличествоКалендарныхДнейОтпускаГосслужащих(МенеджерВременныхТаблиц, ДополнительныеПоказатели);
	КонецЕсли;
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДни(СтрокиПоИдентификатору, ОписаниеВТНачисления)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ПоказателиРасчета.Показатель = &КалендарныеДни
		|			ТОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Начисления.ПериодДействияНачало, ДЕНЬ), НАЧАЛОПЕРИОДА(Начисления.ПериодДействияКонец, ДЕНЬ), ДЕНЬ) + 1
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК Значение
		|ИЗ
		|	ВТРегистрРасчета_Начисления_ФактическийПериодДействия КАК Начисления
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниБезУчетаПраздников(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	МенеджерУчетаВремени = МенеджерУчетаВремени();
	ОписаниеФильтра = МенеджерУчетаВремени.ОписаниеФильтраВТДанныеПроизводственногоКалендаряПоСотрудникам();
	ОписаниеФильтра.ИмяТаблицы = "ВТФильтрДанныеПроизводственногоКалендаряПоСотрудникам";
	ОписаниеФильтра.ИмяПоляСотрудник = "Сотрудник";
	ОписаниеФильтра.ИмяПоляДатаНачала = "ДатаНачала";
	ОписаниеФильтра.ИмяПоляДатаОкончания = "ДатаОкончания";
	МенеджерУчетаВремени.СоздатьВТДанныеПроизводственногоКалендаряПоСотрудникам(ОписаниеФильтра);
	
	УдалитьВТ.Добавить("ВТДанныеПроизводственногоКалендаряПоСотрудникам");
	...
	Запрос.Текст = 
		...
		|			И (НЕ Начисления.ФиксРасчетВремени)
		|			И (НЕ Начисления.ФиксСторно)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДанныеПроизводственногоКалендаряПоСотрудникам КАК ДанныеКалендаря
		|		ПО (ДанныеКалендаря.Сотрудник = Начисления.Сотрудник)
...
Процедура РассчитатьЗначенияПоказателейКоэффициентИндексацииСреднегоЗаработка(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	Запрос.Текст = 
		...
		|			И (КоэффициентыИндексации.Период МЕЖДУ ЕСТЬNULL(ДатыНачалаСобытияВТ.Дата, ДатыНачалаСобытия.ДатаНачалаСобытия) И Начисления.ПериодДействияКонец)
		|ГДЕ
...
Процедура РассчитатьЗначениеПоказателяРасчетнаяБазаУдержаний(ОчередностьРасчета, СтрокиПоИдентификатору)
	...
	Запрос.Выполнить();	
	УдалитьВТ.Добавить("ВТЗаписиУдержаний");
	...
	Если НастройкиУдержаний.РассчитыватьТолькоПоТекущемуДокументу Тогда
		ОтборБазовыхЗаписей.Добавить(РасчетЗарплатыРасширенный.ЭлементОтбораБазовыхЗаписей("Регистратор", ВременныйРегистратор));
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	ИмяТаблицыБазовыхВР = Метаданные.ПланыВидовРасчета.Удержания.ПолноеИмя() + ".БазовыеВидыРасчета";
	РасчетЗарплатыРасширенный.СоздатьВТРасчетнаяБаза(МенеджерВременныхТаблиц, ИменаИзмерений, ИмяТаблицыБазовыхВР, ОтборБазовыхЗаписей, МенеджерКадровогоУчета());
	УстановитьПривилегированныйРежим(Ложь);
	УдалитьВТ.Добавить("ВТРасчетнаяБаза");
	
	ОписаниеВТ = ОписаниеВТУдержания();
	ОписаниеВТ.ИмяВТ = "ВТЗаписиУдержаний";
	ОписаниеВТ.ИмяПоляНачалоПериода = "БазовыйПериодНачало";
	ОписаниеВТ.ИмяПоляОкончаниеПериода = "БазовыйПериодКонец";
	СоздатьВТДополнениеРасчетнойБазыУдержаний(ОписаниеВТ);
	УдалитьВТ.Добавить("ВТДополнениеРасчетнойБазыУдержаний");
	...
	Запрос.Текст = 
		...
		|				И (РасчетнаяБаза.ПериодРегистрацииРазрез = ОсновныеЗаписи.МесяцУдержания)
		|				И (ВЫБОР
		|					КОГДА ОсновныеЗаписи.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
		|						ТОГДА ОсновныеЗаписи.Сотрудник = РасчетнаяБаза.СотрудникРазрез
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ)
		...
		|		ЦеломесячнаяБаза.ВидРасчетаРазрез,
		|		ЦеломесячнаяБаза.НомерСтрокиРазрез,
		|		ЦеломесячнаяБаза.РегистраторРазрез,
		|		0,
		|		ЦеломесячнаяБаза.РезультатБаза
		|	ИЗ
		|		ВТОсновныеЗаписи КАК ИскусственныеЗаписи
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРасчетнаяБаза КАК ЦеломесячнаяБаза
		|			ПО ИскусственныеЗаписи.НомерСтроки = ЦеломесячнаяБаза.НомерСтроки
		|				И ИскусственныеЗаписи.Регистратор = ЦеломесячнаяБаза.Регистратор
		|				И (ЦеломесячнаяБаза.ПериодРегистрацииРазрез = ИскусственныеЗаписи.МесяцУдержания)
		|				И (ВЫБОР
		|					КОГДА ИскусственныеЗаписи.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
		|						ТОГДА ИскусственныеЗаписи.Сотрудник = ЦеломесячнаяБаза.СотрудникРазрез
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ)
		|				И (ИскусственныеЗаписи.ОригинальнаяЗапись = ЛОЖЬ)) КАК БазаУдержаний
		|
		|СГРУППИРОВАТЬ ПО
		|	БазаУдержаний.НомерСтроки,
		|	БазаУдержаний.БазовоеНачисление,
		...
		|
		|ВЫБРАТЬ
		|	Начисления.НомерСтрокиУдержания,
		|	Начисления.ВидРасчета,
		|	Начисления.НомерСтрокиНачисления,
		|	Начисления.РегистраторНачисления,
		|	Начисления.РасчетнаяБаза,
		|	Начисления.РасчетнаяБаза
		|ИЗ
		|	(ВЫБРАТЬ
		|		ОсновныеЗаписи.НомерСтроки КАК НомерСтрокиУдержания,
		|		Начисления.ВидРасчета КАК ВидРасчета,
		|		Начисления.НомерСтроки КАК НомерСтрокиНачисления,
		|		Начисления.Регистратор КАК РегистраторНачисления,
		|		ВЫБОР
		|			КОГДА ОсновныеЗаписи.Начинается
		|					И НЕ ОсновныеЗаписи.Заканчивается
		|				ТОГДА ВЫБОР
		|						КОГДА Начисления.ПериодДействия > ОсновныеЗаписи.МесяцУдержания
		|							ТОГДА Начисления.Результат
		|						ИНАЧЕ 0
		|					КОНЕЦ
		|			КОГДА ОсновныеЗаписи.Заканчивается
		|					И НЕ ОсновныеЗаписи.Начинается
		|				ТОГДА ВЫБОР
		|						КОГДА Начисления.ПериодДействия < ОсновныеЗаписи.МесяцУдержания
		|							ТОГДА Начисления.Результат
		|						ИНАЧЕ 0
		|					КОНЕЦ
		|			КОГДА ОсновныеЗаписи.Начинается
		|					И ОсновныеЗаписи.Заканчивается
		|				ТОГДА 0
		...
		|			ПО (Начисления.ПериодРегистрации = ОсновныеЗаписи.МесяцУдержания)
		|				И (Начисления.ПериодДействия <> ОсновныеЗаписи.МесяцУдержания)
		|				И (Начисления.ФизическоеЛицо = ОсновныеЗаписи.ФизическоеЛицо)
		|				И (Начисления.ГоловнаяОрганизация = ОсновныеЗаписи.Организация)
		|				И (ВЫБОР
		|					КОГДА ОсновныеЗаписи.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
		|						ТОГДА ОсновныеЗаписи.Сотрудник = Начисления.Сотрудник
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ)
		|				И (Начисления.Регистратор <> &ИсключаемыйРегистратор)
		...
		|			ПО (БазовыеНачисления.Удержание = ОсновныеЗаписи.ВидРасчета)
		|				И (БазовыеНачисления.Начисление = Начисления.ВидРасчета)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТБазовыеНачисления КАК БазовыеНачисления
		|			ПО (БазовыеНачисления.Удержание = ОсновныеЗаписи.ВидРасчета)
		|				И (БазовыеНачисления.Начисление = Начисления.ВидРасчета)) КАК Начисления
		|ГДЕ
		|	Начисления.РасчетнаяБаза <> 0";
	Если Не НастройкиУдержаний.РассчитыватьТолькоПоТекущемуДокументу Тогда
	...
	Запрос.Текст = 
		...
		|	Удержания.ДокументОснование,
		|	Удержания.ИдентификаторСтроки,
		|	ЕСТЬNULL(ДополнениеРасчетнойБазы.СуммаНачислений, 0) КАК СуммаНачислений,
		|	ЕСТЬNULL(ДополнениеРасчетнойБазы.СуммаНДФЛ, 0) КАК СуммаНДФЛ,
		...
		|					ИСТИНА
		|				ИЗ
		|					ВТОсновныеЗаписи КАК ЗаписиНеполныйМесяц
		|				ГДЕ
		...
		|	ВЫБОР
		|		КОГДА ДополнениеРасчетнойБазы.Месяц ЕСТЬ НЕ NULL 
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ЕстьДополнениеРасчетнойБазы
		|ИЗ
		|	ВТРегистрРасчета_Удержания КАК Удержания
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИдентификаторыСтрок КАК ИдентификаторыСтрок
		|		ПО (ИдентификаторыСтрок.ИдентификаторСтроки = Удержания.ИдентификаторСтроки)
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДополнениеРасчетнойБазыУдержаний КАК ДополнениеРасчетнойБазы
		|		ПО (ДополнениеРасчетнойБазы.ДокументОснование = Удержания.ДокументОснование)
		|			И (ДополнениеРасчетнойБазы.Месяц = НАЧАЛОПЕРИОДА(Удержания.БазовыйПериодНачало, МЕСЯЦ))
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.СледующийПоЗначениюПоля("Месяц") Цикл
		Пока Выборка.СледующийПоЗначениюПоля("ДокументОснование") Цикл
			...
			СтрокиДополненияБазы = Новый Массив;
			Пока Выборка.СледующийПоЗначениюПоля("ИдентификаторСтроки") Цикл
				Строка = СтрокиПоИдентификатору[Выборка.ИдентификаторСтроки];
				...
				Если Выборка.НеполныйМесяц = Ложь Тогда
					...
					Если Выборка.ЕстьДополнениеРасчетнойБазы Тогда
						СтрокиДополненияБазы.Добавить(Строка.Дополнительно);
				...
				Строка.Дополнительно.Вставить("СуммаДополнения", 0);
				Строка.Дополнительно.Вставить("СуммаНДФЛ", 0);
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.НеполныйМесяц = Истина Тогда
						...
						Если Выборка.ЕстьДополнениеРасчетнойБазы Тогда
							СтрокиДополненияБазы.Добавить(НоваяСтрока);
							РазностьДат.Добавить(Выборка.РазностьДат);
						КонецЕсли;
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			Если Не Выборка.ЕстьДополнениеРасчетнойБазы Тогда
				Продолжить;
			КонецЕсли;
			Если СтрокиДополненияБазы.Количество() = 1 Тогда
				ЗаполнитьСтрокуДополненияРасчетнойБазы(СтрокиДополненияБазы[0], Выборка.СуммаНачислений, Выборка.СуммаНДФЛ);
				Продолжить;
			КонецЕсли;
			РаспределениеСуммыДополнения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаНачислений, КоэффициентыРаспределения);
			Если РаспределениеСуммыДополнения = Неопределено Тогда
				РаспределениеСуммыДополнения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаНачислений, РазностьДат);
			КонецЕсли;
			РаспределениеСуммыНДФЛ = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаНДФЛ, КоэффициентыРаспределения);
			Если РаспределениеСуммыНДФЛ = Неопределено Тогда
				РаспределениеСуммыНДФЛ = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаНДФЛ, РазностьДат);
			КонецЕсли;
			Индекс = 0;
			Для Каждого Строка Из СтрокиДополненияБазы Цикл
				СуммаДополнения = 0;
				Если РаспределениеСуммыДополнения <> Неопределено Тогда
...
Процедура ЗаполнитьСтрокуДополненияРасчетнойБазы(Строка, РаспределениеСуммыДополнения, РаспределениеСуммыНДФЛ)
	...
	КонецЕсли;
	Если РаспределениеСуммыДополнения <> Неопределено Тогда
		Строка.СуммаДополнения = РаспределениеСуммыДополнения * КоэффициентНеполногоМесяца;
	КонецЕсли;
	Если РаспределениеСуммыНДФЛ <> Неопределено Тогда
		Строка.СуммаНДФЛ = РаспределениеСуммыНДФЛ * КоэффициентНеполногоМесяца;
	КонецЕсли;
...
//	- Месяц
//
Процедура СоздатьВТДополнениеРасчетнойБазыУдержаний(ОписаниеВТ)
	...
	Если Не ВТСодержитСтроки("ВТФильтрДополненияРасчетнойБазы") Тогда
		Запрос.Текст = 
			...
			|	0 КАК СуммаНДФЛ
			|ПОМЕСТИТЬ ВТДополнениеРасчетнойБазыУдержаний
			|ИЗ
			|	ВТФильтрДополненияРасчетнойБазы КАК Фильтр";
		Запрос.Выполнить();
		УничтожитьВТ(УдалитьВТ);
		Возврат;
	КонецЕсли;
	...
	ВидыДополненияРасчетнойБазы = Новый Массив;
	ВидыНачисленийДополнения = ВидыНачисленийДополненияРасчетнойБазыУдержаний();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыДополненияРасчетнойБазы, ВидыНачисленийДополнения);
	ВидыНДФЛ = ВидыНДФЛДополненияРасчетнойБазыУдержаний();
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВидыДополненияРасчетнойБазы, ВидыНДФЛ);
	...
	Запрос.Текст =
		...
		|	СУММА(ДанныеДляРасчета.СуммаНДФЛ) КАК СуммаНДФЛ
		|ПОМЕСТИТЬ ВТДополнениеРасчетнойБазыУдержаний
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|		ВЫБОР
		|			КОГДА Начисления.НачислениеУдержание В (&ВидыНачисленийДополнения)
		...
		|				И (Начисления.НачислениеУдержание В (&ВидыДополненияРасчетнойБазы))
		|				И (Начисления.Регистратор <> &ИсключаемыйРегистратор)
		|				И (Начисления.Регистратор = &ВременныйРегистратор)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		...
		|		Фильтр.ДокументОснование,
		|		ВЫБОР
		|			КОГДА НЕ Начисления.НачислениеУдержание В (&ВидыНДФЛ)
		|				ТОГДА Начисления.Сумма
		|			ИНАЧЕ 0
		|		КОНЕЦ,
		|		ВЫБОР
		...
		|				И (Фильтр.Организация = Начисления.Организация.ГоловнаяОрганизация)
		|				И (Начисления.Регистратор <> &ИсключаемыйРегистратор)
		|				И (Начисления.Регистратор = &ВременныйРегистратор)) КАК ДанныеДляРасчета
		|
		|СГРУППИРОВАТЬ ПО
		|	ДанныеДляРасчета.Месяц,
		|	ДанныеДляРасчета.ДокументОснование
		|
		|ИМЕЮЩИЕ
		|	(СУММА(ДанныеДляРасчета.СуммаНачислений) <> 0
		|		ИЛИ СУММА(ДанныеДляРасчета.СуммаНДФЛ) <> 0)";
	Если Не НастройкиУдержаний.РассчитыватьТолькоПоТекущемуДокументу Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (Начисления.Регистратор = &ВременныйРегистратор)", "");
	КонецЕсли;
	УстановитьПривилегированныйРежим(Истина);
...
Процедура РассчитатьНачисления()
	...
	ОбработатьНачисленияПослеРасчета(СтрокиПоИдентификатору);
...
Процедура ОбработатьНачисленияПослеРасчета(СтрокиПоИдентификатору)
	УчетПособийСоциальногоСтрахования.ОбработатьНачисленияПослеРасчета(МенеджерВременныхТаблиц, Организация, МесяцНачисления, СтрокиПоИдентификатору);
КонецПроцедуры
...
Процедура РассчитатьНачисленияОчередности(СтрокиПоИдентификатору)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		РассчитатьЗначениеПоказателяРасчетнаяБазаНачислений(Выборка.ОчередностьРасчета, СтрокиПоИдентификатору);
...
Процедура РассчитатьЗначениеПоказателяРасчетнаяБазаНачислений(ОчередностьРасчета, СтрокиПоИдентификатору)
	...
	Запрос.Текст = 
		...
		|				ВТПоказателиРасчета КАК ПоказателиРасчета
		|			ГДЕ
		|				ПоказателиРасчета.ВидРасчета = Начисления.Ссылка
		|				И ПоказателиРасчета.Показатель = &РасчетнаяБаза)";
	Запрос.Выполнить();	
	УдалитьВТ.Добавить("ВТВидыРасчета");
	
	Если Не ВТСодержитСтроки("ВТВидыРасчета") Тогда
		УничтожитьВТ(УдалитьВТ);
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	И НачисленияФПД.ВидРасчета В
		|			(ВЫБРАТЬ
		|				ВидыРасчета.ВидРасчета
		|			ИЗ
		|				ВТВидыРасчета КАК ВидыРасчета
		|			ГДЕ
		|				ВидыРасчета.ПериодДействияБазовый = ИСТИНА)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Запрос.Текст = 
		...
		|		ПО (ПоказателиРасчета.ВидРасчета = Начисления.ВидРасчета)
		|			И (ПоказателиРасчета.Показатель = &РасчетнаяБаза)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасчетнаяБаза КАК РасчетнаяБаза
...
Процедура РассчитатьСтрокиНачисленийПредопределеннымСпособом(СпособРасчета, СтрокиРасчета)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплата");
		Модуль.ПриРасчетеЗарплатыПредопределеннымСпособом(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц, ИсключаемыйРегистратор);
	КонецЕсли;
...
Процедура РассчитатьСтрокиУдержанийПредопределеннымСпособом(СпособРасчета, СтрокиРасчета)
	...
	ИсполнительныеЛисты.РассчитатьУдержанияПоИсполнительнымЛистам(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц, ИсключаемыйРегистратор);
...
Процедура ЗавершитьРасчетУдержаний(УдалитьВТ, СтрокиПоИдентификатору)
	...
	Если Зарплата.Удержания.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура СоздатьВТУдержанныеСуммы()
	...
	Запрос.Текст =
		...
		|	СУММА(УдержаноЗаМесяц.Удержано) КАК Сумма
		...
		|				И УдержаноЗаМесяц.Период = Удержания.Месяц
		|				И (УдержаноЗаМесяц.Регистратор <> &ИсключаемыйРегистратор)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		|		НАЧАЛОПЕРИОДА(УдержаноЗаМесяц.Период, МЕСЯЦ),
		|		УдержаноЗаМесяц.Организация.ГоловнаяОрганизация,
		|		УдержаноЗаМесяц.ФизическоеЛицо,
		|		УдержаноЗаМесяц.НачислениеУдержание,
		|		УдержаноЗаМесяц.ДокументОснование,
		|		УдержаноЗаМесяц.Сумма
		|	ИЗ
		|		ВТРегистрНакопления_НачисленияУдержанияПоСотрудникам КАК УдержаноЗаМесяц
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТФильтр КАК Удержания
		|			ПО УдержаноЗаМесяц.Организация.ГоловнаяОрганизация = Удержания.Организация
		|				И УдержаноЗаМесяц.ФизическоеЛицо = Удержания.ФизическоеЛицо
		|				И УдержаноЗаМесяц.НачислениеУдержание = Удержания.ВидРасчета
		|				И УдержаноЗаМесяц.ДокументОснование = Удержания.ДокументОснование
		|				И УдержаноЗаМесяц.Период = Удержания.Месяц) КАК УдержаноЗаМесяц
		|
		|СГРУППИРОВАТЬ ПО
		|	УдержаноЗаМесяц.Месяц,
		|	УдержаноЗаМесяц.Организация,
...
Процедура ЗаполнитьНастройкуПересчетаПоУдержаниям(СтрокаИзменений)
	...
	НастройкаПересчета = ПересчетФизическихЛиц.Добавить();
	НастройкаПересчета.ФизическоеЛицо = СтрокаИзменений.ФизическоеЛицо;
	НастройкаПересчета.ПересчитыватьУдержанияОтНачислений = НастройкиУчета.ИспользоватьСтатьиФинансирования;
...
Процедура НачатьПересчетУдержаний(Удержания)
	...
	Для Каждого СтрокаУдержаний Из Зарплата.Удержания Цикл
...
Процедура ОбъединитьНДФЛ(НДФЛ)
	...
	ОбъединитьТаблицыСохраняяИсправления(НДФЛ, Зарплата.НДФЛ, ПризнакиИсправления, ПоляКлюча);
...
Процедура ЗаменитьИсправленныеСтроки(СтрокиПриемника, СтрокиИсточника, ТаблицаИсточник, ПризнакиИсправления, ПоляФиксЗаполнение = Неопределено, ПроверятьФиксЗаполнение = Истина)
	...
	Для Каждого СтрокаПриемника Из СтрокиПриемника Цикл
		Если Не СтрокаИсправлена(СтрокаПриемника, ПризнакиИсправления) Тогда
			УдалитьСтрокиПриемника.Добавить(СтрокаПриемника);
			Продолжить;
		КонецЕсли;
...
Функция ДоговорыПодряда(Сотрудники, Отборы = Неопределено)
	...
	ДоговорыПодряда = РасчетЗарплатыРасширенный.РезультатНачисленияПоДоговорам(
		Организация, 
		МесяцНачисления, 
		ИсключаемыйРегистратор, 
...
Процедура РассчитатьДоговорыПодряда()
	...
	ДоговорыПодряда = Зарплата.ДоговорыПодряда;
...
Процедура НачатьПересчетДоговоровПодряда(ДоговорыПодряда)
	...
	Для Каждого СтрокаНачислений Из Зарплата.ДоговорыПодряда Цикл
		Если Сотрудники.Найти(СтрокаНачислений.Сотрудник) <> Неопределено Тогда
...
Процедура ЗавершитьПересчетДоговоровПодряда()
	...
	Если ФизическиеЛицаБезРасчета.Количество() > 0 Тогда
		ДоговорыПодряда = НайтиСтрокиОтборПоМассиву(Зарплата.ДоговорыПодряда, "ФизическоеЛицо", ФизическиеЛицаБезРасчета);
...
Функция РезультатРасчетаНДФЛ()
	...
	// 4D:ERP для Беларуси, АлексейЧ, 11.01.2018 12:58:20 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	//*******************************************************************************************
	// Яна исключила данную функцию из кода. Без неё не отображается расчет ПН в форма документа.
	// Включим данную функцию. Документ увольнения рассмотрим позже.
	// ******************************************************************************************
	// {
	
	// 4D:ERP для Беларуси, Яна, 24.08.2017 12:34:48 
	// Ошибка при формировании документа увольнения, №14902 
	// {
...
Процедура НачатьРегистрациюНДФЛ(НДФЛ)
	...
	Для Каждого СтрокаТаблицы Из Зарплата.НДФЛ Цикл
...
Процедура НачатьПересчетНДФЛ(Начисления, ДоговорыПодряда)
	...
	ТаблицуНДФЛВРезультатРасчета(Зарплата.НДФЛ);
...
Процедура ЗавершитьПересчетНДФЛ(НДФЛ)
	...
	НайденныеСтроки = НайтиСтрокиОтборПоМассиву(Зарплата.НДФЛ, "ФизическоеЛицо", НастройкиПересчета.ФизическиеЛицаРасчетаНДФЛ);
	ОбъединитьМассивыСтрокСохраняяИсправления(НДФЛ, Зарплата.НДФЛ, НайденныеСтроки, ПоляКлючаОбъединенияНДФЛ(), ПризнакиИсправленияНДФЛ());
...
Процедура СоздатьВТНачисленияДляРегистрацииДоходовНДФЛ(Начисления)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ГрантыНеоблагаемыеНДФЛ") Тогда
		...
		Если Модуль.ИспользуютсяГрантыНеоблагаемыеНДФЛ() Тогда
			...
			СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, "БухучетНачисленийДляРегистрацииДоходовНДФЛ");
...
Процедура СформироватьДоходыНДФЛ(Начисления, ДоговорыПодряда)
	...
	ЗарплатаКадры.СоздатьВТПоНаборуЗаписей(МенеджерВременныхТаблиц, НаборыЗаписей.СведенияОДоходахНДФЛ, Истина);
	ЗакончитьЗамерВремени(ИмяЗамера, Замер);
...
Процедура СформироватьДоходыНДФЛПоДоговорамПодряда(ДоговорыПодряда, ДатаОперации)
	...
	Если НастройкиУчета.ИспользоватьТерритории Тогда
		...
		ДоговорыСТерриториями.Колонки.Добавить("Подразделение", Метаданные.ОпределяемыеТипы.ТерриторииВыполненияРаботВОрганизации.Тип);
...
Функция ДоходыПоДоговорамПодрядаОблагаемыеНДФЛ(ДоговорыПодряда)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ГрантыНеоблагаемыеНДФЛ") Тогда
		Возврат ДоговорыПодряда;
	КонецЕсли;
	...
	Если Не ВТСуществует(ПараметрыБухучета.ИмяВТБухучетНачисленийПоДоговорам) Тогда
		...
		СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетДоговорыГПХ, ПараметрыБухучета.ИмяВТБухучетНачисленийПоДоговорам);
		УдалитьВТ.Добавить(ПараметрыБухучета.ИмяВТБухучетНачисленийПоДоговорам);
...
Процедура ЗавершиьРасчетВзносовПриПересчете(Взносы)
	...
	НайденныеСтроки = НайтиСтрокиОтборПоМассиву(Зарплата.Взносы, "ФизическоеЛицо", НастройкиПересчета.ФизическиеЛицаРасчетаВзносов);;
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		Зарплата.Взносы.Удалить(НайденнаяСтрока);
	КонецЦикла;
	
	// Переносим рассчитанные строки.
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(Взносы, Зарплата.Взносы);
...
Процедура РассчитатьКорректировкиВыплаты()
	...
	ТекущиеКорректировки = ВзаиморасчетыССотрудниками.ОстаткиВзаиморасчетов(
		Организация, 
		НачалоМесяца(МесяцНачисления), 
...
Процедура ЗавершитьРасчетКорректировокВыплаты(КорректировкиВыплаты)
	...
	Если Не НастройкиПересчета.ВыполнятьПересчет Тогда
		Зарплата.КорректировкиВыплаты = КорректировкиВыплаты;
		Возврат;
	КонецЕсли;
	...
	// Удаляем из коллекции строки по физическим лицам, по которым был выполнен пересчет.
	НайденныеСтроки = НайтиСтрокиОтборПоМассиву(Зарплата.КорректировкиВыплаты, "ФизическоеЛицо", НастройкиПересчета.ФизическиеЛицаКорректировокВыплаты);
	ОбъединитьМассивыСтрокСохраняяИсправления(КорректировкиВыплаты, Зарплата.КорректировкиВыплаты, НайденныеСтроки, "ФизическоеЛицо", ПризнакиИсправленияНДФЛ());
...
Процедура ЗарегистрироватьНалогНаМатериальнуюВыгоду()
	...
	Если Зарплата.ПогашениеЗаймов.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ПогашениеЗаймов = Зарплата.ПогашениеЗаймов;
...
Процедура ЗавершитьПересчетПогашенияЗаймов(ПогашениеЗаймов)
	...
	Если Не НастройкиПересчета.ВыполнятьПересчет Тогда
		Зарплата.ПогашениеЗаймов = ПогашениеЗаймов;
		Возврат;
	КонецЕсли;
	...
	// Удаляем из коллекции строки по физическим лицам, по которым был выполнен пересчет.
	НайденныеСтроки = НайтиСтрокиОтборПоМассиву(Зарплата.ПогашениеЗаймов, "ФизическоеЛицо", НастройкиПересчета.ФизическиеЛицаПогашенияЗаймов);;
	Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
		Зарплата.ПогашениеЗаймов.Удалить(НайденнаяСтрока);
	КонецЦикла;
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ПогашениеЗаймов, Зарплата.ПогашениеЗаймов);
...
Процедура НачатьРегистрациюНалогаНаМатериальнуюВыгоду(ПогашениеЗаймов)
	...
	Для Каждого СтрокаТаблицы Из Зарплата.ПогашениеЗаймов Цикл
		Если ФизическиеЛицаМассив.Найти(СтрокаТаблицы.ФизическоеЛицо) <> Неопределено Тогда
...
Процедура ВыполнитьОтражениеНачисленийВБухучете()
	...
	Если НастройкиОбновленияБухучета.ПроверятьНеобходимостьПересчетаНДФЛ Тогда
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ГрантыНеоблагаемыеНДФЛ") Тогда
			...
			Если Модуль.СоздатьВТПолучателиГрантов(МенеджерВременныхТаблиц, ПараметрыБухучета.ИмяВТБухучетНачислений) Тогда
				...
				СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(ТекущийБухучетНачислений, "ВТТекущийБухучетНачислений");
	...
	Иначе
		...
		Для каждого СтрокаТЗ Из Зарплата.Начисления Цикл
...
Процедура ВыполнитьОтражениеНачисленийПоДоговорамВБухучете()
	...
	Иначе
		...
		Для каждого СтрокаТЗ Из Зарплата.ДоговорыПодряда Цикл
			Отбор.ИдентификаторСтроки = СтрокаТЗ.ИдентификаторСтрокиБухучет;
			БухучетСтроки = БухучетНачислений.Скопировать(Отбор);
			Если НастройкиОбновленияБухучета.ПроверятьНеобходимостьПересчетаВзносов Тогда
...
Процедура ВыполнитьОтражениеНДФЛИКорректировокВыплатыВБухучете()
	...
	Если БухучетВыполняемыеДействия.ВыполнятьБухучетНДФЛ И Зарплата.НДФЛ.Количество() > 0 Тогда
		
		НДФЛРезультатыРасчета = Зарплата.НДФЛ;
	...
	Если БухучетВыполняемыеДействия.ВыполнятьБухучетКорректировокВыплаты И Зарплата.КорректировкиВыплаты.Количество() > 0 Тогда
		
		КорректировкиВыплаты = Зарплата.КорректировкиВыплаты;
	...
	Если БухучетНДФЛ.Количество() > 0 Тогда
		...
		Если СоответствиеСтрокНДФЛ.Количество() > 0 Тогда
			...
			КонецЦикла;
		Иначе
			Для каждого СтрокаТЗ Из Зарплата.НДФЛ Цикл
				Отбор.ИдентификаторСтроки = СтрокаТЗ.ИдентификаторСтрокиБухучет;
	...
	Если БухучетКорректировкиВыплаты.Количество() > 0 Тогда
		...
		Если СоответствиеСтрокКорректировокВыплаты.Количество() > 0 Тогда
			...
			КонецЦикла;
		Иначе
			Для каждого СтрокаТЗ Из Зарплата.КорректировкиВыплаты Цикл
				Отбор.ИдентификаторСтроки = СтрокаТЗ.ИдентификаторСтрокиБухучет;
...
Процедура ВыполнитьОтражениеУдержанийИЗаймовВБухучете()
	...
	Если БухучетВыполняемыеДействия.ВыполнятьБухучетУдержаний И Зарплата.Удержания.Количество() > 0 Тогда
	...
	Если БухучетВыполняемыеДействия.ВыполнятьБухучетЗаймов И Зарплата.ПогашениеЗаймов.Количество() > 0 Тогда
	...
	ИсходныеДанные.ВидыНачисленийДополненияРасчетнойБазы = ВидыНачисленийДополненияРасчетнойБазыУдержаний();
	...
	Если БухучетУдержаний.Количество() > 0 Тогда
		...
		Если СоответствиеСтрокУдержаний.Количество() > 0 Тогда
			...
			КонецЦикла;
		Иначе
			Для каждого СтрокаТЗ Из Зарплата.Удержания Цикл
				Отбор.ИдентификаторСтроки = СтрокаТЗ.ИдентификаторСтрокиБухучет;
	...
	Если БухучетЗаймов.Количество() > 0 Тогда
		...
		Если СоответствиеСтрокЗаймов.Количество() > 0 Тогда
			...
			КонецЦикла;
		Иначе
			Для каждого СтрокаТЗ Из Зарплата.ПогашениеЗаймов Цикл
				Отбор.ИдентификаторСтроки = СтрокаТЗ.ИдентификаторСтрокиБухучет;
...
Функция НоваяТаблицаТерриторииДляБухучета()
	...
	Территории.Колонки.Добавить("Территория", Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.ТерриторииВыполненияРаботВОрганизации.Тип));
...
Функция БухучетНачисленийИзКоллекцииСтрок(Начисления)
	...
	БухучетНачислений = ПустаяТаблицаБухучетНачислений();
...
Процедура СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, ИмяВТ)
	...
	Иначе
		...
		|	БухучетНачислений.ПериодДействия,
...
Процедура СоздатьВТБухучетВсехНачисленийДляБухучетаУдержанийНДФЛ()
	...
	Если Не СуществуетВТБухучетДоговорыГПХ Тогда
		БухучетДоговорыГПХ = БухучетНачисленийИзКоллекцииСтрок(Зарплата.ДоговорыПодряда);
	КонецЕсли;
	...
	Если Не СуществуетВТБухучетНачислений И Не СуществуетВТБухучетДоговорыГПХ Тогда
		...
		СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, ПараметрыБухучета.ИмяВТБухучетВсехНачислений);
		Возврат;
	КонецЕсли;
			
	Если Не СуществуетВТБухучетНачислений Тогда
		СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, ПараметрыБухучета.ИмяВТБухучетНачислений);
		УдалитьВТ.Добавить(ПараметрыБухучета.ИмяВТБухучетНачислений);
...
Процедура СоздатьВТБухучетНачисленийДляРегистрацииДоходовСтраховыхВзносов(Начисления, ИмяВТБухучетНачислений)
	...
	Если НастройкиУчета.ИспользоватьСтатьиФинансирования Тогда
		...
		БухучетНачислений = БухучетНачисленийИзКоллекцииСтрок(Начисления);
		СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, ИмяВТБухучетНачислений);
...
Функция ПустаяТаблицаБухучетНачислений()
...
Процедура ПодготовитьДанныеДляОбновленияБухучетаРезультатовРасчета()
	...
	Если Не НастройкиОбновленияБухучета.ОбновитьВсеТаблицы Тогда
		...
		Если БухучетВыполняемыеДействия.ВыполнятьБухучетНачислений Тогда
			...
			БухучетНачислений = БухучетНачисленийИзКоллекцииСтрок(Зарплата.Начисления);
			СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(БухучетНачислений, ИмяВТ);
...
Процедура СоздатьВТУдержания(Удержания, СтрокиПоИдентификатору)
