...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СнятиеРезерва
		...
		ТекстИсключения = НСтр("ru = 'Для заказа давальца запрещена операция снятия резерва.'");
		ВызватьИсключение ТекстИсключения;
		Возврат;
	КонецЕсли;
	//-- НЕ УТКА
...
Процедура НастроитьЭлементыФормыПриСоздании()
	...
	Если ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезерв
		...
		Если ИспользоватьУчетЗатратПоНаправлениямДеятельности Тогда
			Заголовок = СтрЗаменить(НСтр("ru = 'Снятие резерва (Назначение: %1)'"), "%1", НазначениеЗаказВШапке);
		Иначе
			Заголовок = СтрЗаменить(НСтр("ru = 'Снятие резерва (%1)'"), "%1", НазначениеЗаказВШапке);
		КонецЕсли;
	...
	Иначе
		...
		Если ИспользоватьУчетЗатратПоНаправлениямДеятельности  Тогда
			Заголовок = СтрЗаменить(НСтр("ru = 'Резервирование (Назначение: %1)'"), "%1", НазначениеЗаказВШапке);
		Иначе
			Заголовок = СтрЗаменить(НСтр("ru = 'Резервирование (%1)'"), "%1", НазначениеЗаказВШапке);
		КонецЕсли;
		...
		Если РезервироватьИКорректировать
		...
		ИначеЕсли РезервироватьИКорректировать 
...
Процедура ЗаполнитьТаблицу()
	...
	СтруктураЗаполнения = Документы.КорректировкаНазначенияТоваров.ЗаполнитьТаблицуСнятияУстановкиРезервов(ВидОперации, Назначение, Заказ, 
		КорректировкаНазначения, ОтборРезервированияПоСкладуОтгрузки Или ТолькоСкладОтгрузки, ?(ТолькоЗаказыОрганизации, Организация, Неопределено));
