...
Процедура РаспределитьПоОрганизациямЗавершение(Знач Результат, Знач ДополнительныеПараметры) Экспорт
	...
	Если СозданоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не удалось провести автоматическое распределение по организациям. Возможно у выбранных организаций нет остатков по выбранным товарам.'"));
	КонецЕсли;
...
Функция ПолучитьТаблицуРаспределенийСервер()
	...
	ЗапасыСервер.ТаблицаОстатковТоваровОрганизаций(Неопределено, Неопределено, 
		?(ДатаОформленияСкладскихАктов = Дата(1, 1, 1), ТекущаяДатаСеанса(), ДатаОформленияСкладскихАктов),
...
Процедура ЗаписатьСкладскойАктСервер(СкладскойАкт)
	...
	Если СкладскойАкт.ПроверитьЗаполнение() Тогда
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Документ %Документ% не проведен: %ТекстОшибки%'");
