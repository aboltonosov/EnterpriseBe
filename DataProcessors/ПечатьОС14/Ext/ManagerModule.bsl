...
Процедура ЗаполнитьТабличныйДокументОС14(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, КомплектыПечати)
	...
	// 4D:ERP Для Беларуси, Евгений, 15.12.2016 14:35:12 	
	// Печатные формы по ОС, №13507
	// {	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьОС14.ПФ_MXL_ОС14_Локализация");
	// }
	// 4D
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		// Печатные формы по ОС, №13507
		// {	
		ПодразделениеОтветственныхЛиц = ВыборкаДокументы.ПредставлениеПодразделения;
		...
		ОрганизацияГрузоотправительНаименование = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(
			БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
				ВыборкаДокументы.Поставщик, ВыборкаДокументы.ДатаДокумента), "НаименованиеДляПечатныхФорм,");
		ОрганизацияГрузоотправительБанковскиеРеквизиты = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(
			БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(
				ВыборкаДокументы.Поставщик, ВыборкаДокументы.ДатаДокумента), "Телефоны,НомерСчета,Банк,БИК,КоррСчет,");
				
		ОбластьШапка.Параметры.Грузоотправитель = ОрганизацияГрузоотправительНаименование;
		ОбластьШапка.Параметры.ОрганизацияГрузоотправительБанковскиеРеквизиты  = ОрганизацияГрузоотправительБанковскиеРеквизиты;	
		
		ОбластьПодвал.Параметры.ГлавныйБухгалтерНаименование = ОтветственныеЛица.ГлавныйБухгалтерПредставление;
		// }
        // 4D
