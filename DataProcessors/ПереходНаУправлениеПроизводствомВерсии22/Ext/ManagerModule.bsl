...
Функция ПроверитьВозможностьКонвертации(ОтмеченныеСтроки)
	...
	Результат.ЕстьОшибки = ОтсутствуютЦеховыеКладовые(ОтмеченныеСтроки);
...
Процедура ДополнитьЗаказНаПроизводство22(Заказ22Объект, Заказ21Объект, КлючСвязиПродукция)
	...
	Для каждого Строка Из НайденныеСтроки Цикл
		...
		Если ВыпускающийЭтап <> Неопределено
			...
			НоваяСтрока.Количество = РассчитатьКоличество22(Строка, ВыпускающийЭтап);
...
Функция ВыходныеИзделияКонструктор(Источник)
	...
	Результат.Колонки.Добавить("КлючСвязиНазначение",ОписаниеТипов);
	
	ОписаниеТипов = Новый ОписаниеТипов("УникальныйИдентификатор");
	Результат.Колонки.Добавить("Этап",          ОписаниеТипов);
	Результат.Колонки.Добавить("ЭтапНазначение",ОписаниеТипов);
	...
	Для каждого СтрокаЭтап Из КоллекцияЭтапов Цикл
		...
		Для каждого ДанныеСтроки Из НайденныеСтроки Цикл
			...
			Если ЭтоСтрокаВыпускаПолуфабриката(ДанныеСтроки, Заказ21Объект) Тогда
				...
				НоваяСтрока.КлючСвязиНазначение = СтруктураИзделия.КлючСвязиРодитель;
				НоваяСтрока.ЭтапНазначение      = СтруктураИзделия.ЭтапПотребитель;
...
Функция МатериалыИУслугиКонструктор(Источник)
	...
	ОписаниеТипов = Новый ОписаниеТипов("Булево");
	Результат.Колонки.Добавить("ЗапрещеноИзменятьПараметрыОбеспечения", ОписаниеТипов);
...
Функция МатериалыИУслугиЗаказа21(Заказ21Объект, ВыпускающийЭтап, КоллекцияЭтапов, СтруктураИзделия)
	...
	Для каждого СтрокаЭтап Из КоллекцияЭтапов Цикл
		...
		Для каждого ДанныеСтроки Из НайденныеСтроки Цикл
			...
			НоваяСтрока.ЗапрещеноИзменятьПараметрыОбеспечения = Истина;
...
Процедура ДобавитьДанныеСпецификацииЦепочкиЭтапов(Результат, Заказ21Объект, КоллекцияЭтапов, СтруктураИзделия)
	...
	ДанныеСпецификации.Вставить("ЕдиницПартийИзделий", ВыпускающийЭтап.Количество22);
	...
	ДанныеСпецификации.Вставить("ОптимальнаяПартияЗапуска", 0);
...
Функция РезультатКонвертации()
	...
	Возврат Новый Структура("ЕстьОшибки,ОбработаноЭтапов,ДеревоЗаказов",Ложь,0);
