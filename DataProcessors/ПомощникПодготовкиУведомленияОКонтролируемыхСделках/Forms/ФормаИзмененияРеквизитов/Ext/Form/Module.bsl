...
// Производит заполнение таблицы изменяемых реквизитов значениями из соответствующих справочников (в зависимости от выбранного реквизита, который изменяем).
&НаСервере
...
Процедура ИзменитьОбъектНаСервере(ОбновлятьРеквизиты)
	...
	Если СтруктураНастроекОбъекта.Свойство("ТекстЗапроса") Тогда
		СписокИзменяемыхОбъектовТекущегоУсловия.ТекстЗапроса = СтруктураНастроекОбъекта.ТекстЗапроса;
	КонецЕсли;
	
	Если СтруктураНастроекОбъекта.Свойство("ОсновнаяТаблица") Тогда
		СписокИзменяемыхОбъектовТекущегоУсловия.ОсновнаяТаблица = СтруктураНастроекОбъекта.ОсновнаяТаблица;
	КонецЕсли;
	
	Если Не СтруктураНастроекОбъекта.Свойство("ТекстЗапроса") И Не СтруктураНастроекОбъекта.Свойство("ОсновнаяТаблица") Тогда
		СписокИзменяемыхОбъектовТекущегоУсловия.ОсновнаяТаблица = ИзменяемыйОбъект;
	КонецЕсли;
...
Функция ДобавитьЭлементВГруппуДинамическогоСпискаФормы(ГруппаДинамическогоСписка, ИмяЭлемента, ЭтоГруппа = Ложь, СвойстваЭлемента = Неопределено)
	...
	// Добавляем или удаляем элемент, в зависимости от присутствия его на форме или отсутствия среди реквизитов динамического списка:
...
Функция СписокСсылокПоВыбраннымСтрокамДинамическогоСписка()
	...
	// наличие групп и так далее), если в процессе формирования выясняется, что в массиве только обычные элементы справочника -
	// этот  массив без обработки через скд передаем на изменение.
