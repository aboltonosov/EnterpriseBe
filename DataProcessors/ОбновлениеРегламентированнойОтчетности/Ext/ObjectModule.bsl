...
Функция ПолучитьСписокОтчетов() Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 25.11.2015 14:52:01 
	// , №10629
	// {
	СписокРеглОтчетовИспользуемыхВТекущейКонфигурации = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСписокРегламентированныхОтчетов_Локализация();
	
	// Получим макет со списком отчетов.
	МакетСписокОтчетов = ЭтотОбъект.ПолучитьМакет("СписокОтчетов_Локализация");
	// }
	// 4D
