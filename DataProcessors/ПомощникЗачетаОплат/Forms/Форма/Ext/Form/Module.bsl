...
Процедура ЗачтеноПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущийЭлемент.Имя="ЗачтеноСуммаЗачтено" И ТекущиеДанные.Валюта = ВалютаВзаиморасчетов Тогда
		Отказ = Истина;
	КонецЕсли;
	
	СуммаПередРедактированием = ТекущиеДанные.СуммаЗачтено;
	СуммаПлатежаПередРедактированием = ТекущиеДанные.СуммаВВалютеПлатежа;
	КоэффициентПересчетаСуммы = СуммаПлатежаПередРедактированием/СуммаПередРедактированием;
...
Процедура ЗачтеноПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ТекущиеДанные.СуммаЗачтено <> СуммаПередРедактированием Тогда
		...
		КоэффициентПересчетаСуммы = СуммаПлатежаПередРедактированием/ТекущиеДанные.СуммаЗачтено;
		...
		ПересчитатьОстатокПлатежаНаСервере(СоответствиеСписания, Отказ);
		
		Если Отказ Тогда
			ТекущиеДанные.СуммаЗачтено = СуммаПередРедактированием;
		КонецЕсли;
			
	ИначеЕсли ТекущиеДанные.СуммаВВалютеПлатежа <> СуммаПлатежаПередРедактированием Тогда
		
		ОчиститьСообщения();
		
		Отказ = Ложь;
		
		СуммаКОтменеВВалютеПлатежа = СуммаПлатежаПередРедактированием - ТекущиеДанные.СуммаВВалютеПлатежа;
		
		ТекущиеДанные.СуммаЗачтено = ТекущиеДанные.СуммаВВалютеПлатежа / КоэффициентПересчетаСуммы;
		
		ИзменениеСуммы = СуммаПередРедактированием - ТекущиеДанные.СуммаЗачтено;
		
		Если СуммаКОтменеВВалютеПлатежа < 0 Тогда
			ТекущиеДанные.СуммаВВалютеПлатежа = СуммаПлатежаПередРедактированием;
		...
		СоответствиеСписания = Новый Соответствие;
		СоответствиеСписания.Вставить(СтрокаТаблицы, ИзменениеСуммы);
		СоответствиеСписанияВВалютеПлатежа = Новый Соответствие;
		СоответствиеСписанияВВалютеПлатежа.Вставить(СтрокаТаблицы, СуммаКОтменеВВалютеПлатежа);

		ОтменитьЗачетНаСервере(СоответствиеСписания, СоответствиеСписанияВВалютеПлатежа, , Отказ);
...
Процедура ОтменитьЗачет(Команда)
	...
	Если МассивСтрок.Количество() > 0 Тогда
		...
		Для Каждого Строка Из МассивСтрок Цикл
			...
			СоответствиеСписанияВВалютеПлатежа.Вставить(Строка, 0);
		КонецЦикла;
		
		ОтменитьЗачетНаСервере(СоответствиеСписания, СоответствиеСписанияВВалютеПлатежа, Ложь);
...
Процедура ПолучитьРеквизитыРасчетовСПартнером()
	...
	Если СтруктураРеквизитов <> Неопределено Тогда
		...
		МассивРасчетыСПоставщиками.Добавить(Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
...
Функция ТекстЗапросаПоРасчетамСКлиентами()
	...
	|		И ТаблицаОстатков.Валюта = ТаблицаКРаспределению.ВалютаВзаиморасчетов
	|ГДЕ 
	|	НЕ (&ЭтоУправленческаяОрганизация И ТаблицаОстатков.ЗаказКлиента = Неопределено)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаОстатков.ЗаказКлиента,
	|	ТаблицаОстатков.Валюта,
...
&НаСервере
Процедура ОтменитьЗачетНаСервере(СоответствиеСтрок, СоответствиеСтрокВВалютеПлатежа, СуммаСтрокиИзменена = Истина, Отказ = Ложь)
	...
	Для Каждого ЭлементСтроки Из СоответствиеСтрок Цикл
		...
		СуммаКОтменеВВалютеПлатежа = СоответствиеСтрокВВалютеПлатежа[ЭлементСтроки.Ключ];
		...
		Если СуммаКОтмене = 0 Тогда
			...
			Если СуммаКОтменеВВалютеПлатежа = 0 Тогда
				СуммаКОтменеВВалютеПлатежа = СтрокаТаблицы.СуммаВВалютеПлатежа;
			КонецЕсли;
		ИначеЕсли Не СуммаСтрокиИзменена Тогда
			СтрокаТаблицы.СуммаЗачтено = СтрокаТаблицы.СуммаЗачтено - СуммаКОтмене;
			СтрокаТаблицы.СуммаВВалютеПлатежа= СтрокаТаблицы.СуммаВВалютеПлатежа - СуммаКОтменеВВалютеПлатежа;
		КонецЕсли;
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если СуммаКОтменеВВалютеПлатежа = 0 Тогда
				СуммаВВалютеПлатежа = СтрокаТаблицы.СуммаВВалютеПлатежа;
				СтрокаОплаты.СуммаВВалютеПлатежа = ВзаиморасчетыСервер.СписатьСуммуПропорционально(
					СуммаВВалютеПлатежа,
					СуммаКОтмене,
					СуммаБазиса);
			КонецЕсли;
		...
		Иначе
			СтрокаОплаты.ДоступноКЗачету = СуммаКОтмене;
		КонецЕсли;
...
Процедура ИсправитьПревышениеНаСервере(Отказ)
	...
	Для Сч = 1 По РазмерТаблицы Цикл
		...
		СуммаКОтменеВВалютеПлатежа = СтрокаТаблицы.СуммаВВалютеПлатежа/СтрокаТаблицы.СуммаЗачтено * СуммаКОтмене;
		
		СоответствиеСписания.Вставить(СтрокаТаблицы.ПолучитьИдентификатор(), СуммаКОтмене);
		СоответствиеСписанияВВалютеПлатежа.Вставить(СтрокаТаблицы.ПолучитьИдентификатор(), СуммаКОтменеВВалютеПлатежа);
	...
	ОтменитьЗачетНаСервере(СоответствиеСписания, СоответствиеСписанияВВалютеПлатежа, Ложь, Отказ);
...
Процедура ОтразитьЗачетОплатыВДокументе(ДокументСсылка, ДанныеДокумента, СтруктураСумм, ПолеОшибки, Отказ)
	...
	КоэффициентПересчетаОстатка = СтруктураСумм.СуммаЗачтено / СтруктураСумм.СуммаВВалютеПлатежа;
	...
	Для Каждого СтрокаДокумента Из МассивСтрок Цикл
		...
		ИначеЕсли СтрокаДокумента.Заказ = ОбъектРасчетов
			...
			Если ВалютаВзаиморасчетов = СтрокаДокумента.ВалютаВзаиморасчетов Тогда
				СписаноВВалютеВзаиморасчетов = Окр(ВзаиморасчетыСервер.СписатьСумму(
					Списано * КоэффициентПересчетаОстатка,
					СтруктураСумм.СуммаЗачтено),2);
					
				СтрокаДокумента.СуммаВзаиморасчетов = СтрокаДокумента.СуммаВзаиморасчетов - СписаноВВалютеВзаиморасчетов;
	...
	Если ДанныеИзменены Тогда
		...
		Для Каждого СтрокаДокумента Из МассивСтрок Цикл
			...
			Если НЕ ЗначениеЗаполнено(СтрокаДокумента.Заказ) Тогда
				...
				СтрокаДокумента.СуммаВзаиморасчетов = СтрокаДокумента.Сумма * КоэффициентПересчетаОстатка;
...
&НаСервере
Процедура ПересчитатьОстатокПлатежаНаСервере(СоответствиеСтрок, Отказ = Ложь)
	...
	Для Каждого ЭлементСтроки Из СоответствиеСтрок Цикл
		...
		СуммаКОтмене = ЭлементСтроки.Значение;
		СуммаБазиса  = СтрокаТаблицы.СуммаЗачтено + СуммаКОтмене;
		...
		НайденныеСтроки = ТаблицаПоиска.НайтиСтроки(СтруктураПоиска);
		Если НайденныеСтроки.Количество() = 0 ИЛИ ЭтоОтменаАванса Тогда
			СтрокаТаблицы.ДанныеИзменены = Истина;
			Продолжить;
		Иначе
			СтрокаОплаты = НайденныеСтроки[0];
			СтрокаОплаты.ДоступноКЗачету = СтрокаОплаты.СуммаВВалютеПлатежа / КоэффициентПересчетаСуммы;
		КонецЕсли;
