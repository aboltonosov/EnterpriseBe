	...
	Для Ш = 1 по 3 Цикл 
		...
		Если Область <> Неопределено Тогда 
			...
			Для Нс = НомерСтроки по МакетСтиля.ВысотаТаблицы Цикл
				...
				Попытка
					Объект[Свойство] = РаботаВБезопасномРежиме.ВычислитьВБезопасномРежиме(Значение);
				Исключение
				КонецПопытки;
...
Функция ИПИспользуетТрудНаемныхРаботников(Организация) Экспорт
	...
	Иначе
		...
		Попытка
			РаботаВБезопасномРежиме.ВыполнитьВБезопасномРежиме("РегламентированнаяОтчетностьПереопределяемый.ПолучитьПризнакИспользованияИПТрудаНаемныхРаботников(Параметры.Организация, Параметры.ИПИспользуетТрудНаемныхРаботников)", ПараметрыБР); //BRO119
		Исключение
			РаботаВБезопасномРежиме.ВыполнитьВБезопасномРежиме("Параметры.ИПИспользуетТрудНаемныхРаботников = РегламентированнаяОтчетностьПереопределяемый.ИПИспользуетТрудНаемныхРаботников(Параметры.Организация)", ПараметрыБР); //BRO118
		КонецПопытки;		
