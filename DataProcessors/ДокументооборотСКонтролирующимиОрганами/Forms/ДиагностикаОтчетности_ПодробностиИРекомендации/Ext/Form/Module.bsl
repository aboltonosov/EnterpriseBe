...
Процедура ОбщееДействиеЗагрузкаКриптопро(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка) Экспорт
	...
	СтандартнаяОбработка = Ложь; 
	ПерейтиПоНавигационнойСсылке("https://www.cryptopro.ru/products/csp/downloads");
...
Функция ПолучитьКриптопровайдераОтчета(Отчет, Автонастройка, ПараметрыКП = Неопределено)
	...
	Если Отчет.Проверки.ПроверкаУчетнойЗаписи.ИнформацияЛичногоСертификата.Криптопровайдер = Неопределено Тогда 
		Если Автонастройка.ВладелецЭП.КриптопровайдерENUM = Перечисления.ТипыКриптоПровайдеров.CryptoPro Тогда 
			ПараметрыКП = CryptoProCSP;
		ИначеЕсли Автонастройка.ВладелецЭП.КриптопровайдерENUM = Перечисления.ТипыКриптоПровайдеров.VipNet Тогда 
	...
	Если (CryptoProCSP.Имя = НеизвестныйКП.Имя И CryptoProCSP.Тип = НеизвестныйКП.Тип) Тогда 
		ПараметрыКП = CryptoProCSP;
		Возврат Перечисления.ТипыКриптоПровайдеров.CryptoPro;
	КонецЕсли;
	Если (ViPNetCSP.Имя = НеизвестныйКП.Имя И ViPNetCSP.Тип = НеизвестныйКП.Тип) Тогда 
		ПараметрыКП = ViPNetCSP;
		Возврат Перечисления.ТипыКриптоПровайдеров.VipNet;
	КонецЕсли;
