...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Клиент-банк, №11678
	// {
	Объект.ЗагрузитьПредварительно = Истина;
	УстановитьЗаголовокКомандыЗагрузки();
	// }
	// 4D
...
Процедура СоздатьНастройкуПрямойОбменНажатие(Элемент)
	...
	Обработчик = Новый ОписаниеОповещения("ПослеСозданияНастройкиЭДО", ЭтаФорма);
	ОбменСБанкамиКлиент.ОткрытьСоздатьНастройкуОбмена(
		ТекущиеДанные.Организация,
		ТекущиеДанные.Банк,
		ТекущиеДанные.НомерСчета,
...
// {
&НаКлиенте
Процедура НайденныеОбъектыДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекСтрока = Элементы.НайденныеОбъекты.ТекущиеДанные;
	Отбор = Новый Структура;
	
	Если ЗначениеЗаполнено(ТекСтрока.БанковскийСчет) Тогда
		Отбор.Вставить("БанковскийСчет", ТекСтрока.БанковскийСчет);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекСтрока.Валюта) Тогда
		Отбор.Вставить("Валюта", ТекСтрока.Валюта);
	КонецЕсли;
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Отбор", Отбор);
	ПараметрыФормы.Вставить("РежимВыбора", Истина);
	ПараметрыФормы.Вставить("ТекущаяСтрока", ТекСтрока.Документ);
	
	Если ТекСтрока.Исходящий Тогда
		Форма = ПолучитьФорму("Документ.СписаниеБезналичныхДенежныхСредств.ФормаВыбора", ПараметрыФормы, Элемент);
	Иначе
		Форма = ПолучитьФорму("Документ.ПоступлениеБезналичныхДенежныхСредств.ФормаВыбора", ПараметрыФормы, Элемент);
	КонецЕсли;
...
&НаКлиенте
Процедура НайденныеОбъектыДокументПриИзменении(Элемент)
	ТекСтрока = Элементы.НайденныеОбъекты.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТекСтрока.Документ) Тогда
		ТекСтрока.СуммаДокумента = ПолучитьСуммуДокумента(ТекСтрока.Документ);
	Иначе
...
&НаКлиенте
Процедура НайденныеОбъектыСнятьФлажки(Команда)
	Для каждого ЗначОбъект Из Объект.НайденныеОбъекты Цикл
		ЗначОбъект.Пометка = Ложь;
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура НайденныеОбъектыУстановитьФлажки(Команда)
	Для каждого ЗначОбъект Из Объект.НайденныеОбъекты Цикл
		ЗначОбъект.Пометка = Истина;
	КонецЦикла;
КонецПроцедуры

&НаКлиенте
Процедура НайденныеОбъектыСброситьПоиск(Команда)
	Объект.ЗагрузитьПредварительно = Истина;
	Объект.НайденныеОбъекты.Очистить();
	УстановитьЗаголовокКомандыЗагрузки();
КонецПроцедуры

Процедура УстановитьЗаголовокКомандыЗагрузки()
	Если Объект.ЗагрузитьПредварительно Тогда
		Элементы.ЗагрузитьДокументы.Заголовок = "Загрузить предварительно";
	Иначе
		Элементы.ЗагрузитьДокументы.Заголовок = "";
	КонецЕсли;
КонецПроцедуры
// }
// 4D
...
Процедура ВывестиОтчет(Команда)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.БанковскиеСчета.ВыделенныеСтроки Цикл
		...
		Если Не ДанныеСтроки.Загружен И ДанныеСтроки.ОбменЧерезФайл Тогда
			...
			ПараметрыЧтения.Вставить("Программа", ЗначениеРеквизитаОбъекта(ДанныеСтроки.Ссылка, "Программа"));
			СтрокиВыписки = ПрочитатьФайлВыписки(ДанныеСтроки.НастройкаОбмена, ДанныеСтроки.Кодировка, ТекстСообщения, ПараметрыЧтения);
			// }
			// 4D
...
Функция ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита)
	Возврат Обработки.КлиентБанк_Локализация.ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита);
КонецФункции
...
Процедура ПрочитатьДанныеВыписки(Счет)
	...
	// {
	ПараметрыЧтения = Новый Структура;
	ПараметрыЧтения.Вставить("Программа", ЗначениеРеквизитаОбъекта(Счет.Ссылка, "Программа"));
	СтрокиВыписки = ПрочитатьФайлВыписки(Счет.НастройкаОбмена, Счет.Кодировка, ТекстСообщения, ПараметрыЧтения);
	// }
	// 4D
...
Процедура ОбработатьЗавершениеЗагрузки()
	...
	// Клиент-банк, №11678
	// {
	Если ЕстьЗавершенныеЗагрузки И Объект.ЗагрузитьПредварительно Тогда
		ЗавершитьЗагрузку();
	КонецЕсли;
	// }
	// 4D
...
Процедура СоздатьКонтрагентаНаСервере(ИмяКонтрагента, ДанныеДокумента, БанковскийСчет)
	...
	НовыйКонтрагент = Обработки.КлиентБанк_Локализация.СоздатьКонтрагента(ДокументыВыписки, СтрокаДокумента);
	НовыйСчет = Обработки.КлиентБанк_Локализация.СоздатьБанковскийСчетКонтрагента(ДокументыВыписки, СтрокаДокумента, НовыйКонтрагент);
...
Процедура ПодставитьКонтрагентаНаСервере(ИмяКонтрагента, НайденныйКонтрагент, ДанныеДокумента, БанковскийСчет)
	...
	Если СчетКонтрагента = Неопределено Тогда
		СчетКонтрагента =
			Обработки.КлиентБанк_Локализация.СоздатьБанковскийСчетКонтрагента(ДокументыВыписки, ДокументыВыписки[0], НайденныйКонтрагент);
	КонецЕсли;
...
Функция НайтиКонтрагента(ИмяКонтрагента, ДанныеДокумента)
	...
	Обработки.КлиентБанк_Локализация.ЗаполнитьКонтрагентов(ДокументыВыписки, ДокументыВыписки);
...
Функция РазобратьДанныеВыпискиДокумента(ДанныеВыписки, БанковскийСчет)
	...
	// {
	ПараметрыЗагрузки = Новый Структура;
	ПараметрыЗагрузки.Вставить("РасчетныйСчет", БанковскийСчет.Ссылка);
	ДанныеВыпискиСтруктура = ДенежныеСредстваСервер_Локализация.РазобратьФайлВыписки(СтрокиДокумента, Истина, ПараметрыЗагрузки);
	// }
	// 4D
...
Процедура ВыборФайлаОбменаСБанком(ТекущиеДанные, Режим) Экспорт
	...
	// Клиент-банк, №11678
	// {
	ДиалогВыбораФайла.Фильтр = НСтр("ru='Текстовый файл (*.txt)|*.txt|XML-документ (*.xml)|*.xml'");
	// }
	// 4D
...
Функция ПрочитатьФайлВыписки(ИмяФайла, Кодировка, ТекстСообщения = "", ПараметрыЧтения)
...
// 4D
	...
	Иначе
		...
		Попытка
			...
			// Клиент-банк, №11678
			// {
			Если ПараметрыЧтения.Программа = "БТАБАНК" Тогда
				ПотокЧтения = Новый ЧтениеТекста(ИмяФайла, КодировкаТекста.UTF8,,, Ложь);
			Иначе
				ПотокЧтения = Новый ЧтениеТекста(ИмяФайла,,,, Ложь);
			КонецЕсли;
			// }
			// 4D
		...
		Иначе
			...
			// Клиент-банк, №11678
			// {
			Если Найти(ТекущаяСтрока, "^Type=") = 0 И Найти(ТекущаяСтрока, "*****") = 0 И Найти(ТекущаяСтрока, "?xml version") = 0 Тогда
			// }
			// 4D
				ТекстСообщения =
...
Процедура ПрочитатьДанныеВыпискиИЗагрузитьНаСервере(ИдСчета, АдресФайла)
	...
	// {
	ПараметрыЧтения = Новый Структура;
	ПараметрыЧтения.Вставить("Программа", Счет.Программа);
	СтрокиВыписки = ПрочитатьФайлВыписки(ИмяФайла, Счет.Кодировка, ТекстСообщения, ПараметрыЧтения);
	// }
	// 4D
