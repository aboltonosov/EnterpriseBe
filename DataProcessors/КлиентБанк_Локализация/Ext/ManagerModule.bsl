...
Процедура Выгрузить(ПараметрыВыгрузки, АдресХранилища) Экспорт
	...
	// Клиент-банк, №11678
	// {
	ДенежныеСредстваСервер_Локализация.Выгрузить(ПараметрыВыгрузки, АдресХранилища);
	Возврат;
	// }
	// 4D
	...
	ПотокВыгрузки.ДобавитьСтроку("ДатаСоздания="  + Формат(ТекущаяДата(), "ДФ=дд.ММ.гггг"));
	ПотокВыгрузки.ДобавитьСтроку("ВремяСоздания=" + Формат(ТекущаяДата(), "ДФ=ЧЧ:мм:сс"));
...
Процедура Загрузить(ПараметрыЗагрузки, АдресХранилища) Экспорт
	...
	Иначе
		...
		ДопПараметрыЗагрузки.Вставить("РасчетныйСчет", ПараметрыЗагрузки.БанковскийСчет);
		ДанныеВыписки = ДенежныеСредстваСервер_Локализация.РазобратьФайлВыписки(ПараметрыЗагрузки.СтрокиВыписки,, ДопПараметрыЗагрузки);
		// }
		// 4D
	...
	Если ДанныеВыписки <> Неопределено Тогда
		...
		// Клиент-банк, №11678
		// {
		Для каждого СтрокаДокумента Из ПараметрыЗагрузки.НайденныеОбъекты Цикл
			Отбор = Новый Структура;
			Отбор.Вставить("Номер", СтрокаДокумента.Номер);
			Отбор.Вставить("Исходящий", СтрокаДокумента.Исходящий);
			НайденныеДокументы = ДокументыКЗагрузке.НайтиСтроки(Отбор);
			Если НайденныеДокументы.Количество() > 0 И ЗначениеЗаполнено(СтрокаДокумента.Документ) Тогда
				НайденныеДокументы[0].Документ = СтрокаДокумента.Документ;
				НайденныеДокументы[0].БанковскийСчет = СтрокаДокумента.БанковскийСчет;
				НайденныеДокументы[0].НайденДокументВБазе = Истина;
				НайденныеДокументы[0].Загружать = Истина;
			КонецЕсли;
		КонецЦикла;
		
		ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, НомерСчета, ДопПараметрыЗагрузки);
		ОбновитьДокументы(ДокументыКЗагрузке, БанковскийСчет, ПараметрыЗагрузки.СоздаватьКонтрагентов, ПараметрыЗагрузки.ПроводитьДокументы, ПараметрыЗагрузки);
		СоздатьДокументы(ДокументыКЗагрузке, БанковскийСчет, ПараметрыЗагрузки.СоздаватьКонтрагентов, ПараметрыЗагрузки.ПроводитьДокументы, ПараметрыЗагрузки);
		// }
		// 4D
		...
		// Клиент-банк, №11678
		// {
		ТД = ТаблицаДокументов.СкопироватьКолонки();
		Для каждого Стр Из ТаблицаДокументов Цикл
			Отбор = Новый Структура;
			Отбор.Вставить("Номер", Стр.Номер);
			Отбор.Вставить("Пометка", Истина);
			Отбор.Вставить("Исходящий", Стр.Исходящий);
			
			НайденныеДокументы = ПараметрыЗагрузки.НайденныеОбъекты.НайтиСтроки(Отбор);
			Если НайденныеДокументы.Количество() > 0 Тогда
				НоваяСтрока = ТД.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Стр);
			КонецЕсли;
		КонецЦикла;
		
		ПоместитьВоВременноеХранилище(ТД, АдресХранилища);
		// }
		// 4D
...
Функция СоздатьКонтрагента(ДокументыКЗагрузке, СтрокаДокумента) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 01.12.2017 5:24:08 
	// Клиент-банк
	// {
	Если СтрНачинаетсяС(ВРег(СтруктураНаименования.Наименование), "ИП") ИЛИ СтрНачинаетсяС(ВРег(СтруктураНаименования.Наименование), "ИНДИВИДУАЛЬНЫЙ ПРЕДПРИНИМАТЕЛЬ") Тогда
...
Функция СоздатьБанковскийСчетКонтрагента(ДокументыКЗагрузке, СтрокаДокумента, ВладелецСчета) Экспорт
	...
	Если ПрямыеРасчеты Тогда
		...
		Если Не ЗначениеЗаполнено(Банк) И Не ПустаяСтрока(КоррСчетБанка) Тогда
			Банк = Справочники.КлассификаторБанковРФ.НайтиПоРеквизиту("КоррСчет", КоррСчетБанка);
		КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(РЦБанка) И Не ПустаяСтрока(БикРЦБанка) Тогда
			РЦБанк = Справочники.КлассификаторБанковРФ.НайтиПоКоду(БикРЦБанка);
			Если Не ЗначениеЗаполнено(РЦБанк) Тогда
				РЦБанк = Справочники.КлассификаторБанковРФ.НайтиПоРеквизиту("КоррСчет", КоррСчетРЦБанка);
			КонецЕсли;
		КонецЕсли;
...
Функция ТекстЗапросаПлатежныеПорученияТретьиЛица()
	...
	|	ДанныеДокумента.Валюта.Наименование КАК ВалютаНаименование,
	|	ДанныеДокумента.Валюта.НаименованиеПолное КАК ВалютаНаименованиеПолное,
	|	ДанныеДокумента.БанковскийСчет.Банк.СВИФТБИК КАК ОрганизацияБанкСВИФТБИК,
	|	ДанныеДокумента.БанковскийСчет.Банк.Код КАК ОрганизацияБанкКод,
	|	ДанныеДокумента.БанковскийСчет.Банк.Город КАК ОрганизацияБанкГород,
	|	ДанныеДокумента.БанковскийСчет.БанкДляРасчетов.Наименование КАК ОрганизацияБанкДляРасчетовНаименование,
	|	ДанныеДокумента.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное,
	|	ДанныеДокумента.БанковскийСчет.БанкДляРасчетов КАК ОрганизацияБанкДляРасчетовСсылка,
	|	ДанныеДокумента.БанковскийСчет.Банк КАК ОрганизацияБанкСсылка,
	|	ДанныеДокумента.Организация.СтранаРегистрации.Код КАК ОрганизацияСтранаРегистрацииКод,
	|	ДанныеДокумента.Контрагент.СтранаРегистрации.Код КАК КонтрагентСтранаРегистрацииКод,
	|	ДанныеДокумента.Контрагент.НаименованиеПолное КАК КонтрагентНаименованиеПолное,
	|	ДанныеДокумента.БанковскийСчетКонтрагента.БанкДляРасчетов КАК КонтрагентБанкДляРасчетовСсылка,
	|	ДанныеДокумента.БанковскийСчетКонтрагента.Банк КАК КонтрагентБанкСсылка,
	|	ДанныеДокумента.БанковскийСчетКонтрагента.Банк.СВИФТБИК КАК КонтрагентБанкСВИФТБИК,
	// }
	// 4D
...
// Клиент-банк, №11678
// 
Процедура ПодготовитьСтрокиВыписки(ДокументыКЗагрузке, БанковскийСчет, НомерСчета, ДопПараметрыЗагрузки)
// }
// 4D
	...
	Для каждого СтрокаДокумента Из ДокументыКЗагрузке Цикл
		...
		Исходящий = СтрокаДокумента.Исходящий;
		// }
		// 4D
		...
		Если ДопПараметрыЗагрузки.ПрограммаБанк = "ФРАНСАБАНК" И ДопПараметрыЗагрузки.Свойство("ДатаВыписки", ДатаВыписки) Тогда
			СтрокаДокумента.Дата = ДатаВыписки;
			СтрокаДокумента.ДатаДок = ДатаВыписки;
		КонецЕсли;
		// }
		// 4D
	...
	ТаблицаДокументов = ДокументыКЗагрузке.Скопировать(
		...
		|ПолучательИНН, ПлательщикИНН, ПолучательКПП, ПлательщикКПП, ПроверятьИНН, ПроверятьКПП, СуммаДокумента");
	ЗаполнитьСсылкиНаДокументы_Локализация(ДокументыКЗагрузке, ТаблицаДокументов, БанковскийСчет);
	// }
	// 4D
...
// Клиент-банк, №11678
// {
Процедура ОбновитьДокументы(ДокументыКЗагрузке, БанковскийСчет, СоздаватьКонтрагентов, ПроводитьДокументы, ПараметрыЗагрузки)
// }
// 4D
	...
	Для каждого СтрокаДокумента Из СтрокиКЗагрузке Цикл
		...
		// 4D:ERP для Беларуси, ВладимирР, 15.02.2017 15:14:01 
		// Клиент-банк, №11678
		// {
		СуммыРазличаются = (СтрокаДокумента.СуммаДокумента <> ДокументОбъект.Ссылка.СуммаДокумента);
		// }
		// 4D
		...
		// Клиент-банк, №11678
		// {
		УстановитьСвойство(ДокументОбъект, "Комментарий", НСтр("ru = '#Загружен из Клиент-Банка'") + "; Контрагент: " + СтрокаДокумента.Плательщик + "; Расчетный счет: " + СтрокаДокумента.ПлательщикСчет);
		// }
		// 4D
		...
		Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ПоступлениеБезналичныхДенежныхСредств") Тогда
			Для каждого СтрокаРасшифровки Из ДокументОбъект.РасшифровкаПлатежа Цикл
				Если СтрокаРасшифровки.СуммаВзаиморасчетов = 0 Тогда
...
// Клиент-банк, №11678
// {
Процедура СоздатьДокументы(ДокументыКЗагрузке, БанковскийСчет, СоздаватьКонтрагентов, ПроводитьДокументы, ПараметрыЗагрузки)
// }
// 4D
	...
	Для каждого СтрокаДокумента Из СтрокиКЗагрузке Цикл
		...
		Если ЗначениеЗаполнено(НайденныеДокументы[0].Документ) Тогда
			НомерВходящегоДокумента = СтрокаДокумента.Номер;
			ДатаВходящегоДокумента = ДенежныеСредстваКлиентСервер.ПолучитьДатуИзСтроки(СтрокаДокумента.Дата);
			Если Не ЗначениеЗаполнено(ДатаВходящегоДокумента) Тогда
				ДатаВходящегоДокумента = СтрокаДокумента.ДатаДок;
			КонецЕсли;
			ДокументОбъект = НайденныеДокументы[0].Документ.ПолучитьОбъект();
		Иначе
			Если СтрокаДокумента.Исходящий Тогда
		...
		Если Не ДокументОбъект.ЭтоНовый() Тогда
			ДокументОбъект.СуммаДокумента = СтрокаДокумента.СуммаДокумента;
		...
		Если СтрокаДокумента.Исходящий Тогда
			...
			// Клиент-банк, №11678
			// {
			Если Не ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
...
Процедура ЗаполнитьКонтрагентов(ДокументыКЗагрузке, ТаблицаДокументов) Экспорт
	...
	// Клиент-банк, №11678
	// {
	|		ДанныеСправочника.НаименованиеПолное = ТаблицаДокументов.Наименование И ДанныеСправочника.НаименованиеПолное <> """"
	// }
	// 4D
	...
	// Клиент-банк, №11678
	// {
	|		ДанныеСправочника.Наименование = ТаблицаДокументов.Наименование И ДанныеСправочника.Наименование <> """"
	// }
	// 4D
...
// 4D:ERP для Беларуси, ВладимирР, 25.02.2018 20:31:25 
// Клиент-банк, №11678
// {
Функция ЗначениеРеквизитаОбъекта(ЗначСсылка, ЗначИмяРеквизита) Экспорт
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗначСсылка, ЗначИмяРеквизита);
КонецФункции

Процедура ЗагрузитьПроверка(ПараметрыЗагрузки, АдресХранилища) Экспорт
	Если ПараметрыЗагрузки.ПрямойОбмен Тогда
		ДанныеВыписки = РазобратьДеревоВыписки(ПараметрыЗагрузки.СтрокиВыписки);
	Иначе
		ДопПараметрыЗагрузки = Новый Структура;
	...
	Если ДанныеВыписки <> Неопределено Тогда
		...
		ОбъектВозврата = Новый Структура;
		ОбъектВозврата.Вставить("ТаблицаДокументов", ТаблицаДокументов);
		ОбъектВозврата.Вставить("НайденныеДокументы", ДокументыКЗагрузке);
		
		ПоместитьВоВременноеХранилище(ОбъектВозврата, АдресХранилища);
	КонецЕсли;
КонецПроцедуры

Процедура ЗаполнитьСсылкиНаДокументы_Локализация(ДокументыКЗагрузке, ТаблицаДокументов, БанковскийСчет)
	Запрос = Новый Запрос;
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	...
	Пока Выборка.Следующий() Цикл
		СтрокаДокумента = ДокументыКЗагрузке.Найти(Выборка.НомерСтроки, "НомерСтроки");
		Если СтрокаДокумента <> Неопределено Тогда
			// 4D:ERP для Беларуси, Алексей4Д, 05.01.2018 23:35:33 
			// Исправление ошибки. Снятие признака загружать не трубуется.
			// {
			// Если документ найден, создавать новый не будем. Если найдено несколько документов, то обновлять данные документа не нужно.
			//Если ЗначениеЗаполнено(СтрокаДокумента.Документ) Тогда
			//	СтрокаДокумента.Загружать = Ложь;
