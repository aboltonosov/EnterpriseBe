...
Функция ВыгрузкаНастроекУчета()
	...
	Попытка
		...
		Если ЭтоНеФайловыйВариант() Тогда
...
Процедура СохранитьВХранилище(ВременныйФайл, ТипФайла)
	...
	Если ЭтоНеФайловыйВариант() И Файл.Существует() Тогда
...
Процедура СохранитьЗагруженныеДляБП()
	...
	Если ЭтоНеФайловыйВариант() И ЗначениеЗаполнено(АдресДанныеИзБП) Тогда
		ИмяФайла = ПолучитьИмяВременногоФайла(".xml");
...
Функция ОтобразитьНастройкиУчета(ОчиститьИОбновить = Ложь)
	...
	Если ОчиститьИОбновить Или ДеревоНастроекУчета.ПолучитьЭлементы().Количество() = 0 Тогда
		...
		Если ЭтоНеФайловыйВариант() И ЗначениеЗаполнено(АдресФайлаНастройкиИсточника) Тогда
		...
		Если Файл.Существует() Тогда
			...
			ТаблицаЗначенийКонстант = ПолучитьТаблицуЗначенийКонстант(ДеревоНастроек);
...
&НаСервере
Функция ПолучитьТаблицуЗначенийКонстант(Дерево)
				...
				ИначеЕсли Строка.КартинкаИсточник = -1 Тогда
					...
					ИначеЕсли Строка.КартинкаПриемник = -1 Тогда
						...
						Иначе
							...
							ЗначенияПеречисления.Вставить("Не выводить",
															Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.НеВыводить);
							ЗначенияПеречисления.Вставить("Артикул", Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Артикул);
							ЗначенияПеречисления.Вставить("Код", Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Код);
...
Процедура ОбновитьНаСервере(ДатаФайлаДанных)
	...
	Если ЭтоНеФайловыйВариант() И ЗначениеЗаполнено(АдресТекущийЭтап) Тогда
	...
	Иначе
		...
		ИначеЕсли СтруктураНастроек.Этап = 4 Тогда
			...
			Если ЭтоНеФайловыйВариант() И ЗначениеЗаполнено(АдресПериодДокументов) Тогда
				ИмяФайлаНастроек = ПолучитьИмяВременногоФайла(".xml");
...
//
&НаКлиенте
Процедура ОбработчикВыбораФайловогоКаталога(Знач ИмяСвойства, СтандартнаяОбработка = Ложь, Знач ПараметрыДиалога = Неопределено, ОповещениеЗавершения = Неопределено) Экспорт
