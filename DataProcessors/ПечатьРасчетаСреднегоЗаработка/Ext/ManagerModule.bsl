...
Процедура ВывестиОтработанноеВремя(ТабличныйДокумент, НомерРаздела, ОбластьШапкаОтработанногоВремени, ОбластьСтрокаОтработанногоВремени, ОбластьПодвалОтработанногоВремени, ОтработанноеВремя, ИспользоватьСреднеЧасовойЗаработок)
	...
	// 4D:ERP для Беларуси, Алеся, 25.08.2017 13:01:03 
	...
	ЗначенияПараметров.Вставить("Отработано", 	УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ОтработанноеВремя, "Отработано"));
	ЗначенияПараметров.Вставить("Норма", 			УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ОтработанноеВремя, "Норма"));
	// }
	// 4D
...
Процедура ВывестиДолюОтработанногоВремени(ТабличныйДокумент, ОбластьДоляВремени, ОтработанноеВремя, ИспользоватьСреднеЧасовойЗаработок)
	...
	// 4D:ERP для Беларуси, Алеся, 25.08.2017 15:55:51 
	// Локализация ЗП
	// {
	ЗначенияПараметров.Вставить("ВсегоОтработано", 	УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ОтработанноеВремя, "Отработано"));
	ЗначенияПараметров.Вставить("ВсегоПоНорме", 	УчетСреднегоЗаработкаКлиентСервер.ИтогиПоПолю(ОтработанноеВремя, "Норма"));
	// }
	// 4D		
...
Функция ТабличныйДокументРасчетаСреднегоЗаработка0504425(ДанныеДокументов, ОбъектыПечати, ИдентификаторШаблона = "ПФ_MXL_ЗапискаРасчетФорма0504425с2015") Экспорт
	...
	Если ИдентификаторШаблона = "ПФ_MXL_ЗапискаРасчетФорма0504425с2015" Тогда
		ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасчетСреднегоЗаработкаФорма0504425с2015";
	Иначе
		ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_РасчетСреднегоЗаработкаФорма0504425";
	КонецЕсли;
