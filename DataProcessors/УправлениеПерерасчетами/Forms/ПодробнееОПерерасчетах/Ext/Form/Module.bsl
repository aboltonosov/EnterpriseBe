...
Процедура ПрочитатьПерерасчеты()
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.Основание) Тогда
				ПричиныПерерасчетаЗарплаты.Добавить(Выборка.Основание, ПерерасчетЗарплаты.КраткоеПредставлениеДокумента(Выборка.Основание));
			КонецЕсли;
	...
	ФОИспользоватьЛьготыСотрудников = ПолучитьФункциональнуюОпциюФормы("ИспользоватьЛьготыСотрудников");
	...
	Если ФОИспользоватьЛьготыСотрудников Тогда
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.СледующийПоЗначениюПоля("Льгота") Цикл
				...
				НоваяСтрока = ПерерасчетыЛьгот.Добавить();
				НоваяСтрока.Льгота = Выборка.Льгота;
				
				Пока Выборка.Следующий() Цикл
					
					Если ЗначениеЗаполнено(Выборка.Основание) Тогда
						НоваяСтрока.Причины.Добавить(Выборка.Основание, ПерерасчетЗарплаты.КраткоеПредставлениеДокумента(Выборка.Основание));
					КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ФОИспользоватьЛьготыСотрудников И Не РезультатЗапроса.Пустой());
