...
Процедура ЗарегистрироватьНеобходимостьПерерасчетаЛьгот()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ОткрытьФорму("Обработка.УправлениеПерерасчетами.Форма.ФормаНовойЗаписиПерерасчетовЛьгот", ПараметрыОткрытия, ЭтаФорма, , , , Оповещение);
...
Процедура ЗаполнитьПерерасчетыЗарплаты(Сотрудник = Неопределено, Период = Неопределено)
	...
	Запрос.Текст =
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ДанныеСотрудников
		...
		|			И (ДанныеСотрудников.ОсновноеРабочееМестоВОрганизации)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПерерасчетЗарплаты.Организация,
		|	ПерерасчетЗарплаты.ФизическоеЛицо,
				...
				Пока Выборка.СледующийПоЗначениюПоля("ДокументНачисления") Цикл
					...
					Пока Выборка.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(Выборка.Основание) Тогда
							НоваяСтрока.Причина.Добавить(Выборка.Основание, ПерерасчетЗарплаты.КраткоеПредставлениеДокумента(Выборка.Основание));
						КонецЕсли;
				...
				Пока Выборка.СледующийПоЗначениюПоля("ПериодРегистрации") Цикл
					...
					Пока Выборка.Следующий() Цикл
						Если ЗначениеЗаполнено(Выборка.ДокументОснование) Тогда
							НоваяСтрока.Причина.Добавить(Выборка.ДокументОснование, ПерерасчетЗарплаты.КраткоеПредставлениеДокумента(Выборка.ДокументОснование));
						КонецЕсли; 
					КонецЦикла; 
