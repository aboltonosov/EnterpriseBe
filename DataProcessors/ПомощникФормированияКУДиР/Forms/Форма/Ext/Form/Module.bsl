...
Процедура КонецПериодаРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	УстановитьСписокОрганизацийНаУСНЗаПериод(Объект.НачалоПериода, Объект.КонецПериода, Элементы.Организация.СписокВыбора, Объект.Организация);
КонецПроцедуры
...
Процедура ДекорацияНастройкаГруппНоменклатурыНажатие(Элемент)
	...
	ПараметрыФормы.Вставить("МассивСкрываемыхЭлементов", СтрРазделить("СчетаУчетаНоменклатурыСтатьяДоходовРегл,ОбщиеСчетаУчетаНоменклатурыСтатьяДоходовРегл", ","));
...
Процедура ДействиеРасчетНалогаОтменитьНажатие(Элемент)
	...
	ОбновитьСтраницыВыполненияЗаданий(Истина);
КонецПроцедуры
...
Процедура УправлениеФормой()
	...
	Если ПрименяетсяУСНДоходыМинусРасходы Тогда
		Элементы.ДекорацияПризнаниеРасходов.Заголовок = НСтр("ru = 'Автоматическое признание расходов'");
		Элементы.ПояснениеПодготовкаАвтоматическоеПризнаниеРасходов.Заголовок =
		НСтр("ru = 'Автоматическое признание расходов для отражения в Книге учета доходов и расходов, сторнирование доходов по деятельности ЕНВД.'");
	Иначе
		Элементы.ДекорацияПризнаниеРасходов.Заголовок = НСтр("ru = 'Сторно доходов по деятельности ЕНВД'");
		Элементы.ПояснениеПодготовкаАвтоматическоеПризнаниеРасходов.Заголовок =
		НСтр("ru = 'Автоматическое сторнирование доходов по деятельности ЕНВД, ранее записанных в Книгу учета доходов и расходов.'");
	КонецЕсли;
	Элементы.ДекорацияПризнаниеРасходов.Гиперссылка = ПрименяетсяУСНДоходыМинусРасходы;
	
	УстановитьПривилегированныйРежим(Истина);
	
	НачалоРасчетаОперацийПоУСН = УчетУСНСервер.НачалоРасчетаОперацийПоУСН(Объект.КонецПериода, Объект.Организация);
	ВыполненныеРасчетыАктуальны = НачалоРасчетаОперацийПоУСН.НачалоРасчетаПризнаниеРасходовПриУСН > КонецМесяца(Объект.КонецПериода)
		И НачалоРасчетаОперацийПоУСН.НачалоРасчетаСторноДоходовКУДиРПоЕНВД > КонецМесяца(Объект.КонецПериода);
	СтатусАвтоматическогоПризнанияРасходов = ?(ВыполненныеРасчетыАктуальны, 2, 0);
	...
	ИначеЕсли НачалоРасчетаОперацийПоУСН.НачалоРасчетаСторноДоходовКУДиРПоЕНВД < Объект.НачалоПериода Тогда
		...
		МесяцНачалаРасчета = Формат(НачалоРасчетаОперацийПоУСН.НачалоРасчетаСторноДоходовКУДиРПоЕНВД, "ДФ='ММММ гггг'");
		ПредупреждениеБолееРаннегоПризнания = СтрШаблон(ТекстПредупреждения, МесяцНачалаРасчета);
	Иначе
...
Процедура УстановитьСписокОрганизацийНаУСНЗаПериод(НачалоПериода, КонецПериода, СписокОрганизаций, ТекущаяОрганизация)
	...
	СписокОрганизаций.ЗагрузитьЗначения(ВернутьОрганизацииНаУСНЗаПериод(НачалоПериода, КонецПериода));
...
&НаКлиенте
Процедура ОбновитьСтраницыВыполненияЗаданий(ТолькоСтраницуНалога = Ложь)
	
	#Область ПризнаниеРасходов
	Если Не ТолькоСтраницуНалога Тогда 
	...
	ПояснениеРасчетНалога = ПояснениеРасчетаНалога(Объект.КонецПериода, СтатусАвтоматическогоПризнанияРасходов);
	...
	ИначеЕсли Не СтатусАвтоматическогоПризнанияРасходов = 2 И Элементы.ГруппаПодготовкаАвтоматическоеПризнаниеРасходов.Видимость Тогда // Ожидает выполнения признания расходов
...
Процедура ЗапуститьВыполнениеФоновогоЗадания(ИмяЗадания, СтатусВыполнения)
	...
	ПараметрыФоновогоЗадания = Новый Структура("НачалоПериода, КонецПериода, Организация", Объект.НачалоПериода, КонецМесяца(Объект.КонецПериода), Объект.Организация);
	
	Попытка
		
		Если ИмяЗадания = "ПризнаниеРасходов" Тогда
			РезультатФоновогоЗадания = ВыполнитьПризнаниеРасходовНаСервере(УникальныйИдентификатор, ПараметрыФоновогоЗадания, ПрименяетсяУСНДоходыМинусРасходы);
		Иначе
	...
	Если ИмяЗадания = "ПризнаниеРасходов" Тогда
		...
		СтатусРасчетаНалога = 0;
		ОбновитьСтраницыВыполненияЗаданий(Ложь);
	Иначе
...
&НаСервереБезКонтекста
Функция ВыполнитьПризнаниеРасходовНаСервере(УникальныйИдентификатор, ПараметрыЭкспортнойПроцедуры, ПрименяетсяУСНДоходыМинусРасходы)
	
	Если ПрименяетсяУСНДоходыМинусРасходы Тогда
		НаименованиеЗадания = НСтр("ru = 'Выполняется признание расходов по УСН'");
		ВыполняемыйМетод = "УчетУСНСервер.ПризнатьИОтразитьРасходыКУДиР";
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Выполняется сторно доходов в КУДиР по деяетельности ЕНВД'");
		ВыполняемыйМетод = "УчетУСНСервер.СторноДоходовКУДиРПоЕНВД";
	КонецЕсли;
	
	РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(УникальныйИдентификатор, 
		ВыполняемыйМетод, ПараметрыЭкспортнойПроцедуры, НаименованиеЗадания);
...
&НаКлиенте
Функция ПояснениеРасчетаНалога(КонецПериода, СтатусПризнания)
