...
Процедура ОбновитьДвиженияПоРасчетамСПартнерами(РегистраторыКОтражению, ПараметрыОтладки = Неопределено) Экспорт
	...
	ПараметрыДвижений.Вставить("ИмяВременнойТаблицыРезультата", "втДвиженияАктивовПассивов");
	...
	|	Т.Источник
	|ИЗ
	|	втДвиженияАктивовПассивов КАК Т
	|
	|ИТОГИ ПО
	|	Регистратор";
...
//
Функция БалансовыеРегистры() Экспорт
...
Функция ЗапросДвиженийНаправлений(Параметры)
	...
	Для Каждого Движения Из ПервичныеДвижения Цикл
		...
		ИначеЕсли ИмяТаблицы = "РасчетыСКлиентами" И НЕ ЕстьРасчетыСКлиентамиПоДокументам Тогда
		...
		ИначеЕсли ИмяТаблицы = "РасчетыСПоставщиками" И НЕ ЕстьРасчетыСПоставщикамиПоДокументам Тогда
...
Процедура ДобавитьТекстЗапросаДенежныеСредства(Запрос, ТаблицаДвижений, ТипДС)
	...
	ИначеЕсли ТипЗнч(ТаблицаДвижений) = Тип("ТаблицаЗначений") Тогда
		...
		Запрос.Текст = Запрос.Текст + ТекстЗапроса + РазделительЗапросов();
...
Процедура ДобавитьТекстЗапросаДенежныеДокументы(Запрос, ТаблицаДвижений)
	...
	ИначеЕсли ТипЗнч(ТаблицаДвижений) = Тип("ТаблицаЗначений") Тогда
		...
		|	Т.Организация КАК Организация,
		|	Т.Подразделение КАК Подразделение,
		|	Т.ДенежныйДокумент КАК ДенежныйДокумент,
		|	Т.СуммаУпр КАК СуммаУпр
		|ПОМЕСТИТЬ ПервичныеДвижения
		|ИЗ
		|	&ИмяТаблицы КАК Т
...
Процедура ДобавитьТекстЗапросаПринятаяВозвратнаяТара(Запрос, ТаблицаДвижений)
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(Т.ДокументПоступления) = ТИП(Документ.ПоступлениеТоваровУслуг)
	|			ТОГДА ВЫРАЗИТЬ(Т.ДокументПоступления КАК Документ.ПоступлениеТоваровУслуг).НаправлениеДеятельности
	|		ИНАЧЕ
	|			ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)
	|	КОНЕЦ КАК НаправлениеДеятельности,
	|	СУММА(Т.Сумма) КАК Сумма,
...
Процедура ДобавитьТекстЗапросаПрочиеРасходы(Запрос, ТаблицаДвижений)
	...
	|	Т.НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА СтатьиРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов)
...
Процедура ДобавитьТекстЗапросаСебестоимостьТоваров(Запрос, ТаблицаДвижений)
	...
	|	КОНЕЦ КАК Статья,
	|	ЗНАЧЕНИЕ(Перечисление.СтатьиБезАналитики.ПустаяСсылка) КАК Аналитика,
	|	СУММА(Т.Стоимость + Т.СуммаДопРасходов + Т.Трудозатраты + Т.ПостатейныеСНДС) КАК Сумма,
	|	СУММА(ВЫБОР КОГДА Т.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) ТОГДА -(Т.Стоимость + Т.СуммаДопРасходов + Т.Трудозатраты + Т.ПостатейныеСНДС) ИНАЧЕ (Т.Стоимость + Т.СуммаДопРасходов + Т.Трудозатраты + Т.ПостатейныеСНДС) КОНЕЦ) КАК СуммаБаланса,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыИсточниковУправленческогоБаланса.РасчетСебестоимости) КАК ВидИсточника,
	|	&Источник КАК Источник
	|
	|ПОМЕСТИТЬ ИмяТаблицы
	...
	|	И (Т.Стоимость + Т.СуммаДопРасходов + Т.Трудозатраты + Т.ПостатейныеСНДС) <> 0
	|	//&Отбор
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Регистратор,
...
Процедура ДобавитьТекстЗапросаТоварыКОформлениюОтчетовКомитенту(Запрос, ТаблицаДвижений)
	...
	|	ЕСТЬNULL(ВидыЗапасов.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК Подразделение,
...
Процедура ДобавитьТекстЗапросаТрудозатратыНезавершенногоПроизводства(Запрос, ТаблицаДвижений)
	...
	Если ПолучитьФункциональнуюОпцию("УправлениеПредприятием") Тогда
		ТекстЗапроса = СтрЗаменить(
						ТекстЗапроса,
						"&НаправлениеДеятельности",
						"ЕСТЬNULL(Т.ЗаказНаПроизводство.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))");
	Иначе
...
Процедура ДобавитьТекстЗапросаВременныхТаблицКонтроляБаланса(Запрос)
	...
	|	Т.ВидИсточника КАК ВидИсточника,
	|	ЗНАЧЕНИЕ(Перечисление.ИсточникиУправленческогоБаланса.%1) КАК Источник
	|ИЗ
	|	%1 КАК Т";
	...
	Для Каждого ИмяТаблицы Из Запрос.Параметры.ТаблицыДвиженийНаправлений Цикл
		...
		ТекстЗапроса = СтрШаблон(ШаблонЗапросаКонтроляБаланса, ИмяТаблицы);
	...
	Запрос.Текст = Запрос.Текст + ТекстЗапросаДвижений + РазделительЗапросов();
	...
	Если ФормироватьФинансовыйРезультат Тогда
		...
		Запрос.Текст = Запрос.Текст + ТекстЗапросаКонтрольБаланса + РазделительЗапросов();
		...
		КонецЕсли;
		Запрос.Текст = Запрос.Текст + ТекстЗапросаДенежныхСредств + РазделительЗапросов();
...
Процедура ДобавитьТекстЗапросаДвиженийДенежныхСредств(Запрос)
	...
	Запрос.Текст = Запрос.Текст + ТекстЗапросаДенежныхСредств + РазделительЗапросов();
	Запрос.УстановитьПараметр("ДвижениеКосвенныхДС", "Движение косвенных ДС");
	Запрос.УстановитьПараметр("ВыделениеКосвенныхДС", "Выделение косвенных ДС");
	Запрос.УстановитьПараметр("ДвижениеПрямыхДС", "Движения прямых ДС");
	Запрос.УстановитьПараметр("ДвиженияПрямыхДСпоПусто", "Движения прямых ДС по пусто");
...
Процедура ДобавитьТекстЗапросаДвиженийВводаВыводаПоНаправлению(Запрос)
	...
	Запрос.Текст = Запрос.Текст + ТекстЗапросаВводВыводПоНД + РазделительЗапросов();
...
Процедура ДобавитьТекстЗапросаДвиженийАктивовПассивов(Запрос)
	...
	Запрос.Текст = Запрос.Текст + ТекстЗапросаДвиженияАктивовПассивов;
...
Функция ЗаписатьСформированныеДвижения(ИмяДокумента, ЗапросДвижений, НеЗаписыватьДвижения = Ложь)
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		Попытка
			...
			Если НеЗаписыватьДвижения Тогда
				ОтменитьТранзакцию();
			Иначе
...
Функция ТаблицыОтложенногоРасчета(ВидОтложенногоРасчета)
	...
	Если ВидОтложенногоРасчета = Перечисления.ВидыИсточниковУправленческогоБаланса.РасчетСебестоимости Тогда
		...
		//++ НЕ УТ
		Таблицы.Вставить(МетаданныеРегистров.ТрудозатратыНезавершенногоПроизводства.Имя);
		//-- НЕ УТ
