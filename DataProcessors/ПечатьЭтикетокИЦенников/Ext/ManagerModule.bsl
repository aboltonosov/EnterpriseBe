...
Процедура СформироватьПечатныеФормыЭтикетокИЦенниковТоваров(ДанныеПечати, ОбъектыПечати, КоллекцияПечатныхФормБСП)
	...
	// 4D:ERP для Беларуси, Алеся, 01.08.2017 16:25:14 
	// Локализация макета
	// {
	СтруктураНастроек.ИмяМакетаСхемыКомпоновкиДанных = "ПоляШаблонаПечатьТовары_Локализация";
	// }
	// 4D
...
Функция ДанныеДляПечатиЭтикетокДоставки(МассивОбъектов) Экспорт
	...
	Если ТипЗнч(ТекущийОбъект) = Тип("ДокументСсылка.ЗаданиеНаПеревозку") Тогда
		...
		|	РаспоряженияПоЗаданию.Объем,
		|	ВЫБОР
		|		КОГДА ДокументРаспоряжение.ЗаказДавальца.ПеревозчикПартнер.ШаблонЭтикетки.Ссылка ЕСТЬ NULL
		...
		|	РаспоряженияПоЗаданию.Объем,
		|	ВЫБОР
		|		КОГДА ДокументРаспоряжение.ЗаказПереработчику.ПеревозчикПартнер.ШаблонЭтикетки.Ссылка ЕСТЬ NULL
		...
		|		ПО ДокументРаспоряжение.Ссылка = ЭтапПроизводства.ЗаказПереработчику
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЭтапПроизводства2_2.ОбеспечениеМатериаламиИРаботами КАК ДокументРаспоряжениеТовары
		|		ПО ЭтапПроизводства.Ссылка = ДокументРаспоряжениеТовары.Ссылка,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|
		|СГРУППИРОВАТЬ ПО
		|	РаспоряженияПоЗаданию.Распоряжение,
	...
	Иначе
		...
		|	ВЫБОР
		|		КОГДА ДокументПередачаСырьяПереработчику.ПеревозчикПартнер.ШаблонЭтикетки.Ссылка ЕСТЬ NULL
		|				ИЛИ ДокументПередачаСырьяПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументПередачаСырьяПереработчику.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументПередачаСырьяПереработчику.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ
		|ИЗ
		|	Документ.ПередачаСырьяПереработчику.Товары КАК ПередачаСырьяПереработчикуТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаСырьяПереработчику КАК ДокументПередачаСырьяПереработчику
		|		ПО ПередачаСырьяПереработчикуТовары.Ссылка = ДокументПередачаСырьяПереработчику.Ссылка,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|ГДЕ
		...
		|	ВЫБОР
		|		КОГДА ДокументПередачаДавальцу.ПеревозчикПартнер.ШаблонЭтикетки.Ссылка ЕСТЬ NULL
		|				ИЛИ ДокументПередачаДавальцу.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ИЛИ ДокументПередачаДавальцу.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ДоКлиента)
		|			ТОГДА ШаблонЭтикеткиДляДоставки.Значение
		|		ИНАЧЕ ДокументПередачаДавальцу.ПеревозчикПартнер.ШаблонЭтикетки
		|	КОНЕЦ
		|ИЗ
		|	Документ.ПередачаДавальцу.Товары КАК ПередачаДавальцуТовары
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаДавальцу КАК ДокументПередачаДавальцу
		|		ПО ПередачаДавальцуТовары.Ссылка = ДокументПередачаДавальцу.Ссылка,
		|	Константа.ШаблонЭтикеткиДляДоставки КАК ШаблонЭтикеткиДляДоставки
		|ГДЕ
