...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураКонтакнтыхДанных = БизнесСеть.ОписаниеКонтактнойИнформацииПользователя();
	БизнесСетьПереопределяемый.ПолучитьКонтактнуюИнформациюПользователя(Пользователи.ТекущийПользователь(), СтруктураКонтакнтыхДанных);
	КонтактноеЛицо = СтруктураКонтакнтыхДанных.ФИО;
	Телефон = СтруктураКонтакнтыхДанных.Телефон;
	ЭлектроннаяПочта = СтруктураКонтакнтыхДанных.ЭлектроннаяПочта;
...
Процедура ОтправитьДокумент(Команда)
	...
	Если УведомлятьПоПочте И ПустаяСтрока(ЭлектроннаяПочта) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Введите адрес электронной почты'"),, "ЭлектроннаяПочта",,);
		Возврат;
	КонецЕсли;
	Если Не ПустаяСтрока(ЭлектроннаяПочта) 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Адрес электронной почты введен неверно'"),, "ЭлектроннаяПочта",,);
		Возврат;
	КонецЕсли;
