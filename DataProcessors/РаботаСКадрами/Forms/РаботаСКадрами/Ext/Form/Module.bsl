...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Петр, 23.08.2017 10:32:12 
	// Рабочее место "Работа с кадрами", №15739
	// {
	СотрудникОтборПоОрганизации();
	// }
	// 4D
...
Процедура СотрудникСправкаОЗаработке(Команда)
	...
	// 4D:ERP для Беларуси, Юлия, 04.08.2017 16:26:35 
	// Локализованные макеты по кадрам и ЗП, № 15656
	// {
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьКадровыхПриказовРасширенная", "ПФ_MXL_СправкаОДоходахПроизвольнаяФорма_Локализация",
...
Процедура СотрудникСправкаСМестаРаботы(Команда)
	...
	// Локализованные макеты по кадрам и ЗП, № 15656
	// {
	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Обработка.ПечатьКадровыхПриказовРасширенная", "ПФ_MXL_СправкаСМестаРаботы_Локализация",
			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Сотрудник), ЭтаФорма);
	// }
	// 4D
...
Процедура ПрочитатьДанныеСотрудника()
	...
	Если ЗначениеЗаполнено(Сотрудник) Тогда
		...
		Иначе
			...
			Иначе
				СотрудникСНИЛС = "<" + НСтр("ru='Страховой номер не заполнен'") + ">";
			КонецЕсли;
			// }
			// 4D
...
// {
&НаСервере
Процедура СотрудникОтборПоОрганизации()
	
	ОтборПоОрганизации = Новый ПараметрВыбора("Отбор.Организация", Организация);
	
	ОтборПоОрганизацииМассив = Новый Массив;
	ОтборПоОрганизацииМассив.Добавить(ОтборПоОрганизации);
	
	ОтборПоОрганизацииМассивФиксированныйМассив = Новый ФиксированныйМассив(ОтборПоОрганизацииМассив);
	Элементы.Сотрудник.ПараметрыВыбора = ОтборПоОрганизацииМассивФиксированныйМассив;
