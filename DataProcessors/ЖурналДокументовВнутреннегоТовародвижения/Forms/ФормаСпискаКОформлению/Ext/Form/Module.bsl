...
Функция ОформитьРядНакладныхПоТаблицам(ДокументыПоТипамНакладных, ПоОрдерам, СписокОшибок)
	...
	Для Каждого Таблица Из ДокументыПоТипамНакладных Цикл
		...
		Если ПерваяСтрока.СостояниеНакладной > 0 Тогда
			...
			//++ НЕ УТКА
			Если ПолноеИмяНакладной = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя() Тогда
				ОтборСтрок = ПерваяСтрока.Склад;
				РеквизитыШапки = МенеджерНакладной.ДанныеЗаполненияНакладной(Заказы, ПерваяСтрока.ХозяйственнаяОперация, ОтборСтрок);
			ИначеЕсли ПолноеИмяНакладной = Метаданные.Документы.ПередачаМатериаловВПроизводство.ПолноеИмя() Тогда
				СвойстваЗаказов = Новый Структура("Ссылка, Склад, Подразделение, Организация");
				ЗаполнитьЗначенияСвойств(СвойстваЗаказов, ПерваяСтрока);
				МассивСвойств = Новый Массив();
				МассивСвойств.Добавить(СвойстваЗаказов);
				
				РеквизитыШапки = МенеджерНакладной.ДанныеЗаполненияНакладной(Заказы, МассивСвойств);
			Иначе
			//-- НЕ УТКА
...
Функция ОформитьЧерезФормуПерезаполнения(ДокументыПоТипамНакладных, ПоОрдерам, СписокОшибок)
	...
	Если ПравоДоступа("Добавление", МетаданныеНакладной) Или ПравоДоступа("Изменение", МетаданныеНакладной) Тогда
		...
		ИначеЕсли ВыделеннаяСтрока.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВнутренняяПередачаТоваров Тогда
			ИмяПереоформляемогоДокумента = НСтр("ru = 'внутренней передачи товаров'");
		...
		//++ НЕ УТКА
		ИначеЕсли НайденнаяСтрока.ПолноеИмяДокумента = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя()
			И (НайденнаяСтрока.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыпускПродукцииНаСклад
				Или НайденнаяСтрока.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства) Тогда
		...
		//++ НЕ УТКА
		Если НайденнаяСтрока.ПолноеИмяДокумента = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя() Тогда
			РеквизитыШапки = МенеджерНакладной.ДанныеЗаполненияНакладной(Заказы, ВыделеннаяСтрока.ХозяйственнаяОперация, ВыделеннаяСтрока.Склад);
		ИначеЕсли ПолноеИмяНакладной = Метаданные.Документы.ПередачаМатериаловВПроизводство.ПолноеИмя() Тогда
			СвойстваЗаказов = Новый Структура("Ссылка, Склад, Подразделение, Организация");
			ЗаполнитьЗначенияСвойств(СвойстваЗаказов, ВыделеннаяСтрока);
			МассивСвойств = Новый Массив();
...
Процедура НастроитьФормуПоНастройкамХозяйственныхОперацийИДокументов()
	...
	ЗаголовокПоУмолчанию = НСтр("ru = 'Внутренние документы (накладные к оформлению)'");
...
Процедура ПеречитатьКОформлению(ДокументДляПересчета = Неопределено)
	...
	Для Каждого ТаблицаЗапроса Из ТаблицыЗапроса Цикл
		...
		//++ НЕ УТ
		ИначеЕсли ТаблицаЗапроса.Ключ = ИмяРегистраЗаказыМатериаловВПроизводство Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаЗапроса.Значение, ТаблицаКОформлению);
		//-- НЕ УТ
		//++ НЕ УТКА
		ИначеЕсли ТаблицаЗапроса.Ключ = ИмяРегистраРаспоряженияНаПередачуИзПроизводства Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаЗапроса.Значение, ТаблицаКОформлению);
		//-- НЕ УТКА
	...
	//++ НЕ УТКА
	// Для последующего соединения по хоз.операции в методе ДобавитьТаблицуОтгрузкиВИтоговую
	// Соединение по хоз.операции производится исключительно для документа ЭтапПроизводства2_2
	ДополнитьХозяйственнойОперацией(КОформлениюТоварыКОтгрузке, Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство);
	//-- НЕ УТКА
...
Процедура ДобавитьТаблицуОтгрузкиВИтоговую(ТаблицаИсточник, ТаблицаПриемник, ДобавлятьНовыеСтроки = Истина)
	...
	Для Каждого Строка Из ТаблицаИсточник Цикл
		...
		Если ПроизводствоДоступно
			И ТаблицаИсточник.Колонки.Найти("СкладПолучатель") <> Неопределено 
			И ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			Отбор.Вставить("Склад", Строка.СкладПолучатель);
		КонецЕсли;
		Если ПроизводствоДоступно
			И ЗначениеЗаполнено(Строка.ХозяйственнаяОперация)
			И ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			Отбор.Вставить("ХозяйственнаяОперация", Строка.ХозяйственнаяОперация);
		КонецЕсли;
		//-- НЕ УТКА
		...
		ИначеЕсли ДобавлятьНовыеСтроки 
			...
			Если ПолныеИменаНакладных.НайтиПоЗначению(Строка.Ссылка.Метаданные().ПолноеИмя()) <> Неопределено Тогда
...
Процедура ВосстановитьНастройки()
	...
	Если ТипЗнч(Настройки) = Тип("Структура") Тогда
		...
		Настройки.Свойство("ОтборПериод",ОтборПериод);
...
Функция РазделитьЗаказыПоПолнымИменамНакладных(СписокРаспоряжений, ПоОрдерам, СписокОшибок) 
	...
	Для Каждого Строка Из СписокРаспоряжений Цикл
		...
		//++ НЕ УТКА
		ИначеЕсли ПолноеИмяНакладной = Метаданные.Документы.ПередачаМатериаловВПроизводство.ПолноеИмя()
			И Не ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством")
			И ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2") Тогда
				ТекстОшибки = НСтр("ru = 'По распоряжению %1 невозможно оформить накладную, т.к. выключено управление производством версии 2.1.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
		...
		ИначеЕсли ПоОрдерам
			//++ НЕ УТКА
			И ПолноеИмяНакладной <> Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя()
			//-- НЕ УТКА
			...
			ИначеЕсли Строка.СостояниеОрдера = 4 Тогда
				...
				ТекстОшибки = НСтр("ru = 'Для распоряжения %1 невозможно оформить накладную по отгрузке (приемке) т.к. склад не использует ордерную схему.'");
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Строка.Ссылка);
		...
		//++ НЕ УТКА
		ИначеЕсли ПоОрдерам И ПолноеИмяНакладной = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя() Тогда
			...
			ОперацияОтгрузки = (Строка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаВПроизводство
				...
				Или Строка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВКладовую);
			...
			ИначеЕсли Не ОперацияОтгрузки И Строка.СостояниеПриходногоОрдера = 0 Тогда
			...
			ИначеЕсли Не ОперацияОтгрузки И Строка.СостояниеПриходногоОрдера = 4 И НакладнаяЯвляетсяРаспоряжением Тогда
			...
			ИначеЕсли Не ОперацияОтгрузки И Строка.СостояниеПриходногоОрдера = 4 Тогда
...
Функция ЗаполнитьПоляШапкиДокументов(Таблица)
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаРемонт) Тогда
		...
		|ПОМЕСТИТЬ ВтДанныеШапкиДокументов
		|ИЗ
		|	Документ.ЗаказНаРемонт КАК ТаблицаДокумента
		|ГДЕ
		|	ТаблицаДокумента.Ссылка В
		|			(ВЫБРАТЬ
		|				ВтТовары.Ссылка
	...
	//++ НЕ УТ
	Если ПравоДоступа("Чтение", Метаданные.Документы.ПередачаМатериаловВПроизводство) Тогда
		МассивТекстовЗапросов.Добавить("
		|ВЫБРАТЬ
		|	ТаблицаДокумента.Ссылка                КАК Ссылка,
		|	ТаблицаДокумента.Организация           КАК Организация,
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказМатериаловВПроизводство) Тогда
		...
		|	ТаблицаДокумента.Ответственный         КАК Ответственный,
		|	ТаблицаДокумента.Подразделение         КАК Подразделение,
		|	ТаблицаДокумента.ЦеховаяКладовая       КАК СкладПолучатель,
		|	NULL                                   КАК Склад,
		|	ТаблицаДокумента.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|	ТаблицаДокумента.Комментарий           КАК Комментарий,
		|	ТаблицаДокумента.Номер                 КАК Номер,
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказНаПроизводство) Тогда
		МассивТекстовЗапросов.Добавить("
		|ВЫБРАТЬ
		|	ТаблицаДокумента.Ссылка        КАК Ссылка,
		|	ТаблицаДокумента.Организация   КАК Организация,
	...
	|		4
	//++ НЕ УТКА
	// Для выпуска продукции в новой концепции производства отключаем расчет состояния расходного ордера, т.к. документ не использует отгрузку
	|	КОГДА (Таблица.Ссылка ССЫЛКА Документ.ДвижениеПродукцииИМатериалов
	|		ИЛИ Таблица.Ссылка ССЫЛКА Документ.ЭтапПроизводства2_2)
	|		И (ВтДанныеШапки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад)
	|			Или ВтДанныеШапки.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства)) Тогда
	|		4
	//-- НЕ УТКА
	|	ИНАЧЕ
	|		Таблица.СостояниеОрдера
...
Функция ПолноеИмяНакладнойПоПолномуИмениРаспоряжения(ПолноеИмяРаспоряжения, СкладПолучатель)
	...
	ИначеЕсли ПолноеИмяРаспоряжения = Метаданные.Документы.ЗаказНаВнутреннееПотребление.ПолноеИмя()
		//++ НЕ УТКА
		Или ПолноеИмяРаспоряжения = Метаданные.Документы.ЗаказНаРемонт.ПолноеИмя()
		//-- НЕ УТКА
		Тогда
	...
	//++ НЕ УТ
	ИначеЕсли ПолноеИмяРаспоряжения = Метаданные.Документы.ЗаказМатериаловВПроизводство.ПолноеИмя() Тогда
		//++ НЕ УТКА
		Если ЗначениеЗаполнено(СкладПолучатель) Тогда
			ПолноеИмяНакладной = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя();
		Иначе
		//-- НЕ УТКА
			ПолноеИмяНакладной = Метаданные.Документы.ПередачаМатериаловВПроизводство.ПолноеИмя();
		//++ НЕ УТКА
		КонецЕсли;
		//-- НЕ УТКА
	//-- НЕ УТ
	//++ НЕ УТКА
	ИначеЕсли ПолноеИмяРаспоряжения = Метаданные.Документы.ЗаказНаПроизводство.ПолноеИмя() Тогда
		ПолноеИмяНакладной = Метаданные.Документы.ПередачаМатериаловВПроизводство.ПолноеИмя();
	ИначеЕсли ПолноеИмяРаспоряжения = Метаданные.Документы.ЭтапПроизводства2_2.ПолноеИмя() Тогда
		ПолноеИмяНакладной = Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя();
	//-- НЕ УТКА
	Иначе
...
//++ НЕ УТКА
&НаСервере
Процедура ДополнитьХозяйственнойОперацией(Таблица, ХозяйственнаяОперация)
	
	Если Таблица = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Таблица.Колонки.Найти("ХозяйственнаяОперация") = Неопределено Тогда
		Таблица.Колонки.Добавить("ХозяйственнаяОперация", Новый ОписаниеТипов("ПеречислениеСсылка.ХозяйственныеОперации"));
	КонецЕсли;
	
	Для Каждого Строка Из Таблица Цикл
		Если ТипЗнч(Строка.Ссылка) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
