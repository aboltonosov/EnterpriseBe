...
Процедура ТаблицаНастройкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураПараметры = Новый Структура("Организация, Продавец, ТипЗапасов, СпособПередачиТоваров, Валюта, Договор, ВидЦены");
	...
	Если СтрокаТаблицы.Свойство("СпособПередачи" + ИндексКолонки) Тогда
		...
		Если СтрокаТаблицы["СпособПередачи" + ИндексКолонки] <> СпособПередачиНеТребуется Тогда
			...
			СтруктураПараметры.Договор = СтрокаТаблицы["Договор" + ИндексКолонки];
			СтруктураПараметры.ВидЦены = СтрокаТаблицы["ВидЦены" + ИндексКолонки];
...
Процедура ТаблицаНастройкиВыборЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ВозвращаемыеПараметры <> Неопределено Тогда
		...
		СтрокаТаблицы["Договор" + ИндексКолонки] = ВозвращаемыеПараметры.Договор;
		СтрокаТаблицы["ВидЦены" + ИндексКолонки] = ВозвращаемыеПараметры.ВидЦены;
	КонецЕсли;
