...
//++ НЕ УТКА
		...
		|		 И Т.Склад          = Товары.Склад
		|		 И Т.ДатаОтгрузки   = Товары.ДатаОтгрузки
		|		 И (Т.Назначение    = Товары.Ссылка.НазначениеМатериалы
		|			ИЛИ Товары.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется))
		|		 И Товары.ВариантОбеспечения В(
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Обособленно),
		|			ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется))
...
Функция ТаблицаПотребностейПоЗаказамИОстатков(Отбор, ТипОтбора, ТаблицаСпособовОбеспечения = Неопределено, ТаблицаЗапасы = Неопределено) Экспорт
	...
	Иначе
		...
		ОбособленноеОбеспечениеНаСкладеОтличномОтСкладаОтгрузкиВозможно = ПолучитьФункциональнуюОпцию("РазрешитьОбособлениеТоваровСверхПотребности")
		//++ НЕ УТКА
		...
		Если ОбособленноеОбеспечениеНаСкладеОтличномОтСкладаОтгрузкиВозможно Тогда
...
Функция ВременнаяТаблицаТоваровДляОбеспеченияЗаказов()
	...
	ТекстЗапроса =
		...
		|{ГДЕ
		|	Товары.Заказ.НаправлениеДеятельности.* КАК НаправлениеДеятельности}
		|;
		|
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	Товары.Номенклатура      КАК Номенклатура,
...
//++ НЕ УТКА
		...
		|			ПО КорректировкиЗаказов.Ссылка = Т.Регистратор
		|			
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		...
		|		И (Т.Регистратор ССЫЛКА Документ.ЗаказНаПроизводство
		|			ИЛИ Т.Регистратор ССЫЛКА Документ.КорректировкаЗаказаМатериаловВПроизводство)
		|		И Т.КОбеспечению <> 0
		|	
		|	{ГДЕ
		|		(ЕСТЬNULL(СпрСпособ.Ссылка,
		...
		|					ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
		|			КОНЕЦ КАК Справочник.СтруктураПредприятия).*                                   КАК Подразделение,
		|		(ЕСТЬNULL(СпрСпособ.ИсточникОбеспеченияПотребностей, НЕОПРЕДЕЛЕНО)).*              КАК ИсточникОбеспечения}) КАК РаботыЗаказовНаПроизводство
		|СГРУППИРОВАТЬ ПО
...
//-- НЕ УТКА
		...
		|	И Т.КОбеспечению > 0
		//++ НЕ УТКА
		|	И НЕ (Т.Регистратор ССЫЛКА Документ.ЗаказНаПроизводство
		|		ИЛИ Т.Регистратор ССЫЛКА Документ.КорректировкаЗаказаМатериаловВПроизводство)
		//-- НЕ УТКА
		|
		|{ГДЕ
	...
	Если ДляВыборки Тогда
		...
		ТекстЗапроса = ТекстЗапроса +
			...
			|	СпособОбеспечения.* КАК СпособОбеспечения}
			|{ГДЕ
			|	Т.Заказ.НаправлениеДеятельности КАК НаправлениеДеятельности}
			|ИЗ
...
Функция ВременнаяТаблицаСпособовОбеспечения()
	...
	ТекстЗапроса =
		...
		|	Т.Назначение.*                               КАК Назначение,
		|	Т.Склад.*                                    КАК Склад,
		|	Т.Регистратор.НаправлениеДеятельности.*      КАК НаправлениеДеятельности}
		|
		|{ГДЕ (Т.Номенклатура, Т.Характеристика) В(
		|	ВЫБРАТЬ
		|		ТаблицаОтбора.Номенклатура   КАК Номенклатура,
		...
		|		 И НЕ ОбеспечениеРаботами.Регистратор.НаправлениеДеятельности ЕСТЬ NULL}
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		|		ПО &ПодстановкаОсновногоСпособаОбеспечения
		|ГДЕ
		...
		|//отборы компоновщика
		|{ГДЕ
		|	ОбеспечениеРаботами.Регистратор.НаправлениеДеятельности.* КАК НаправлениеДеятельности}
		|{ГДЕ
		|	(ЕСТЬNULL(СпрСпособ.Ссылка,
		|		ЗНАЧЕНИЕ(Справочник.СпособыОбеспеченияПотребностей.ПустаяСсылка))).*           КАК СпособОбеспечения,
		|	ЕСТЬNULL(СпрСпособ.ТипОбеспечения, ЗНАЧЕНИЕ(Перечисление.ТипыОбеспечения.Покупка)) КАК ТипОбеспечения,
		...
		|		 И НЕ ОбеспечениеЗаказов.Регистратор.НаправлениеДеятельности ЕСТЬ NULL}
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СпособыОбеспеченияПотребностей КАК СпрСпособ
		|		ПО &ПодстановкаОсновногоСпособаОбеспечения
		|
		...
		|//отборы компоновщика
		|{ГДЕ
		|	ОбеспечениеЗаказов.Регистратор.НаправлениеДеятельности.* КАК НаправлениеДеятельности}
		|{ГДЕ
		|	Т.Номенклатура.*                             КАК Номенклатура,
		|	Т.Характеристика.*                           КАК Характеристика,
		|	Т.Назначение.*                               КАК Назначение,
...
Функция ТекстПолейУсловияЗакупок()
	...
	Текст =
		...
		|								ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25_125)) ТОГДА
		|							1.25
		// }
		// 4D
...
Функция ТаблицаПолуфабрикатов(МенеджерВременныхТаблиц)
	...
	Возврат РегистрыНакопления.ПродукцияИПолуфабрикатыВПроизводстве.ПолуфабрикатыВПроизводстве(
