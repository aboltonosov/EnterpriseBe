...
//++ НЕ УТ
	...
	ПоляКлючаЗаказа = "Организация, Подразделение, Склад, ЦеховаяКладовая";
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		ДанныеЗаполнения.Вставить("Комментарий", ТекстКомментарий);
		Документ = Обработки.ОбеспечениеПотребностей.ЗаполнитьИПровестиДокумент(
			Документы.ЗаказМатериаловВПроизводство, ДанныеЗаполнения, Перечисления.СтатусыЗаказовМатериаловВПроизводство.КОбеспечению, Сообщения);
		ДокументыОбъекты.Добавить(Документ);
...
//++ НЕ УТКА
	...
	ПоляКлючаЗаказа = "НаправлениеДеятельности, Организация, Подразделение, Партнер, Договор, ДатаПотребности";
	...
	Для Каждого Элемент Из МассивТаблиц Цикл
		...
		ДанныеЗаполнения.Вставить("Комментарий", ТекстКомментарий);
		Документ = Обработки.ОбеспечениеПотребностей.ЗаполнитьИПровестиДокумент(
			Документы.ЗаказНаПроизводство2_2, ДанныеЗаполнения, Перечисления.СтатусыЗаказовНаПроизводство2_2.КПроизводству, Сообщения);
...
Функция ТаблицаЗаказыНаПроизводство2_2КОформлению()
	...
	ТекстЗапроса =
		...
		|УПОРЯДОЧИТЬ ПО
		|	Организация, Подразделение, НаправлениеДеятельности, Партнер, Договор, ДатаПотребности, // порядок обхода заказов
