...
// 4D:ERP для Беларуси, ВладимирР, 02.11.2017 12:00:43 
// МножественнаяВыгрузка, ЗаявленияОВвозеТоваров, №15631
// {

&НаСервереБезКонтекста
Функция ПолучитьСписокЗаявленияОВвозеТоваров(Знач ВыделенныеСтроки)
	ОбщиеДанные = Новый Массив;
	
	Для каждого Стр Из ВыделенныеСтроки Цикл
		ОбщиеДанные.Добавить(Стр.Ссылка);
	КонецЦикла;
...
Процедура ВыгрузитьЗаявлениеОВвозе(Команда)
	...
	СписокЗаявленияОВвозеТоваров = ПолучитьСписокЗаявленияОВвозеТоваров(Список.ВыделенныеСтроки);
	
	Режим = РежимДиалогаВыбораФайла.ВыборКаталога;
	ДиалогОткрытияФайла = Новый ДиалогВыбораФайла(Режим);
	ДиалогОткрытияФайла.Выбрать();
	КаталогВыгрузки = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ДиалогОткрытияФайла.Каталог);
	
	Для каждого СписокТекущиеДанные Из СписокЗаявленияОВвозеТоваров Цикл
		ВыгружаемыеДанные = ВыгрузитьЗаявлениеОВвозеТоваровНаСервере(УникальныйИдентификатор, СписокТекущиеДанные);
		Если Не ВыгружаемыеДанные = Неопределено Тогда
			СохранитьВыгружаемыеДанные(ВыгружаемыеДанные, Истина, КаталогВыгрузки);
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Выгружен документ: " + СписокТекущиеДанные + " - " + ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ВыгружаемыеДанные[0].ИмяФайлаВыгрузки).Имя;
			Сообщение.Сообщить();
		КонецЕсли;
	КонецЦикла;
	Возврат;
...
Функция ТекстЗапросаСписокДокументыЗакупки()
	...
	// 4D:ERP для Беларуси, Дмитрий, 07.10.2017 13:05:48 
	// Серия в реестре документов
	// {
	|	РеестрДокументов.СерияПервичногоДокумента КАК СерияВходящегоДокумента,
	// }
	// 4D
...
Функция ТекстЗапросаЗаявленияНаВвозТоваров()
	...
	// Серия в реестре документов
	// {
	|	РеестрДокументов.СерияПервичногоДокумента,
	// }
	// 4D
...
Процедура СохранитьВыгружаемыеДанные(ВыгружаемыеДанные
	...
	// МножественнаяВыгрузка, ЗаявленияОВвозеТоваров, №15631
	// {
	, МножественнаяВыгрузка = Ложь, КаталогВыгрузки = ""
	// }
	// 4D
	...
	Для Каждого ФайлВыгрузки Из ВыгружаемыеДанные Цикл
		...
		Попытка
			...
			// МножественнаяВыгрузка, ЗаявленияОВвозеТоваров, №15631
			// {
			Если МножественнаяВыгрузка И Не ПустаяСтрока(КаталогВыгрузки) Тогда
				ИмяТекущегоФайла = КаталогВыгрузки + ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ФайлВыгрузки.ИмяФайлаВыгрузки).Имя;
				ПолучитьФайл(ФайлВыгрузки.АдресФайлаВыгрузки, ИмяТекущегоФайла, Ложь);
			Иначе
