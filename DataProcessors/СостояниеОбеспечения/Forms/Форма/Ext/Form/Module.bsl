...
//-- НЕ УТ
		...
		ТекстЗапроса = ТекстЗапроса
			+ РегистрыНакопления.ЗаказыКлиентов.ТекстЗапросаЗаказовКОбеспечению(ЕстьФильтр, ПолучатьНесогласованные);
...
Функция ЭтоРазборка(Ссылка)
	...
	ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "ХозяйственнаяОперация")
		= Перечисления.ХозяйственныеОперации.СборкаТоваров Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция РекомендацияПоИндексуКартинки(Индекс, СтрокаТовары) Экспорт
...
//++ НЕ УТКА
	...
	Если ПравоДоступа(Право, Метаданные.Документы.ЭтапПроизводства2_2)
		И (ПравоДоступа(Право, Метаданные.РегистрыНакопления.ЗаказыМатериаловВПроизводство) Или Право = "Изменение") Тогда
