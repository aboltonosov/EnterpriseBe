...
Процедура ЗаписатьРезультат()
	...
	Если СтатусВыполненнойЗагрузки Тогда
		...
		Если Не РезультатВыгрузкиИзФайла.ФайлВыгрузки = Неопределено Тогда
			ПолноеИмяАрхивируемогоФайла = ВременныйКаталог + "ВыгрузкаМодели.xml";
			РезультатВыгрузкиИзФайла.ФайлВыгрузки.Записать(ПолноеИмяАрхивируемогоФайла);
		КонецЕсли;
		
		ПолноеИмяАрхивируемогоФайлаКА = ВременныйКаталог + "ВыгрузкаМоделиКА.xml";
		РезультатВыгрузкиИзФайла.ФайлВыгрузкиКА.Записать(ПолноеИмяАрхивируемогоФайлаКА);
		...
		Попытка		
			...
			КонецЕсли;
			ОбъектЗаписьZip.Добавить(ПолноеИмяАрхивируемогоФайлаКА);
