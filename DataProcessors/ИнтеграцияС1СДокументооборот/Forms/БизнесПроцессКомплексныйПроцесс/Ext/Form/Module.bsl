...
Функция ПодготовитьБизнесПроцесс(Прокси)
	...
	Для каждого Этап из Этапы Цикл
		...
		Если ЗначениеЗаполнено(Этап.ЗапущенныйБизнесПроцессID) Тогда
			Ответ = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(Прокси, Этап.ЗапущенныйБизнесПроцессТип, Этап.ЗапущенныйБизнесПроцессID);
			ЭтапXDTO.businessProcess = Ответ.objects[0];
		КонецЕсли;
