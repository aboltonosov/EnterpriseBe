...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Дмитрий, 14.08.2017 16:01:21 
	// Отключаем
	// {
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Если ВидимостьГруппыНастройки Тогда
			...
			Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") Тогда
				// 4D:ERP для Беларуси, Дмитрий, 07.09.2017 14:22:43 
	...
	// 4D:ERP для Беларуси, Дмитрий, 14.08.2017 16:02:10 
	// Отключаем
	// {
	Элементы.ГруппаЗащитаПерсональныхДанных.Видимость 									  = Ложь;
	// }
	...
	Если РежимРаботы.БазоваяВерсия Тогда
		Элементы.ОписаниеРаздела.Заголовок =
			 НСтр("ru = 'Контроль за состоянием программы, резервное копирование и восстановление, обновление версии.'");
	КонецЕсли;
