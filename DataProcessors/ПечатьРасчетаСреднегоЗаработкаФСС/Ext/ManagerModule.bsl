...
Процедура ВывестиТаблицуЗаработка(ТабличныйДокумент, ОбластиМакета, ДанныеДокумента)
	...
	Для каждого РасчетныйГод Из ДанныеДокумента.ПараметрыРасчета.РасчетныеГоды Цикл
		...
		Для каждого СтрокаПоСтрахователю Из СтрокаПоГоду.Страхователи Цикл	
			...
			ЗначенияПараметров.Вставить("РасчетныйГод", СтрокаПоГоду.РасчетныйГод);
		...
		Если СтрокаПоГоду.Страхователи.Количество() = 0 Тогда
			ЗначенияПараметров.Очистить();
			ЗначенияПараметров.Вставить("РасчетныйГод", СтрокаПоГоду.РасчетныйГод);
			ЗначенияПараметров.Вставить("Заработок", 	0);
			ЗначенияПараметров.Вставить("ДнейБолезниУходаЗаДетьми", 0);
			ЗначенияПараметров.Вставить("Страхователь", "");
			ВывестиСтрокуПоСтрахователю(ТабличныйДокумент, ДанныеДокумента, ОбластиМакета, ЗначенияПараметров)
		КонецЕсли;
		
		ЗначенияПараметров.Вставить("ПредельнаяВеличина", 	СтрокаПоГоду.ПредельнаяВеличина);
		ЗначенияПараметров.Вставить("Заработок", 			СтрокаПоГоду.Заработок);
