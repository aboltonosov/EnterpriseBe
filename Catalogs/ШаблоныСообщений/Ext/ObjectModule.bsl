...
Процедура ЗаполнитьНаОснованииСтруктуры(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ЗначениеЗаполнено(ПараметрыШаблона.Назначение) Тогда
		
		НастройкиШаблона = ШаблоныСообщенийСлужебный.ПриОпределенииНастроек();
		
		Для каждого ОписаниеШаблона Из НастройкиШаблона.ПредметыШаблонов Цикл
			Если СтрСравнить(ПараметрыШаблона.Назначение, ОписаниеШаблона.Представление) = 0
				ИЛИ СтрСравнить(ПараметрыШаблона.Назначение, ОписаниеШаблона.Имя) = 0 Тогда
				
				МетаданныеОбъекта                                 = Метаданные.НайтиПоПолномуИмени(ОписаниеШаблона.Имя);
				ЭтотОбъект.ПолноеИмяТипаПараметраВводаНаОсновании = ОписаниеШаблона.Имя;
				ЭтотОбъект.Назначение                             = ОписаниеШаблона.Представление;
				ЭтотОбъект.ПредназначенДляВводаНаОсновании        = Истина;
				ПараметрыШаблона.ПолноеИмяТипаНазначения          = ОписаниеШаблона.Имя;
				Прервать;
