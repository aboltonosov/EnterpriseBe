...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Клиент-банк, №11678
	// {
	ПриСозданииФормыЭлементаБанковскиеСчетаОрганизаций();
	// }
	// 4D
...
// {
&НаСервере
Процедура ПриСозданииФормыЭлементаБанковскиеСчетаОрганизаций()
	
	СписокПрограмм = ДенежныеСредстваСервер_Локализация.ПолучитьСписокПоддерживаемыхПрограммКлиентБанка();
	Для каждого ТекущиеЭлементСписка Из СписокПрограмм Цикл
		Элементы.Программа.СписокВыбора.Добавить(ТекущиеЭлементСписка.Значение, ТекущиеЭлементСписка.Представление);
	КонецЦикла;
	
	Элементы.Программа.СписокВыбора.СортироватьПоПредставлению();
...
Процедура НомерСчетаПриИзменении(Элемент)
	...
	Если Не ПустаяСтрока(Объект.НомерСчета) Тогда
		...
		// 4D:ERP для Беларуси, Дмитрий, 11.11.2017 7:29:08 
		// контроль номера счета
		// {
		Если ИностранныйБанк = 0 Тогда
			
			Если СтрДлина(СокрЛП(Объект.НомерСчета)) = 28 Тогда
				Если НЕ БанковскиеСчетаКлиент.ПроверитьНомерСчета(Объект.НомерСчета) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
						НСтр("ru='Номер счета введен некорректно'"),, "НомерСчета", "Объект"); 
				КонецЕсли;	
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Введено недостаточно символов в номере счета'"),, "НомерСчета", "Объект"); 
			КонецЕсли;	
...
Функция ЗаполнитьРеквизитыБанкаПоБИК(ТипБанка, БИК = "", ПеренестиЗначенияРеквизитов = Ложь)
	...
	Если ТипБанка = "Банк" Тогда
		...
		Если НЕ ПустаяСтрока(ЗаписьОБанке) Тогда
			...
			// 4D:ERP для Беларуси, Дмитрий, 28.04.2017 11:31:59 
			// Загрузка перечня банков, №14618 
			// {
			Объект.СВИФТБанка = ЗаписьОБанке.СВИФТБИК;
			// }
			// 4D     
	...
	ИначеЕсли ТипБанка = "БанкДляРасчетов" Тогда
		...
		РаботаСБанками.ПолучитьДанныеКлассификатораРФ(БИК,,ЗаписьОБанке);
	 	Если НЕ ПустаяСтрока(ЗаписьОБанке) Тогда
			...
			// Загрузка перечня банков, №14618 
			// {
			Объект.СВИФТБанкаДляРасчетов = ЗаписьОБанке.СВИФТБИК;
			// }
			// 4D
	...
	Если ТипЗнч(Банк) = Тип("СправочникСсылка.КлассификаторБанковРФ") Тогда
		Если ТипБанка = "Банк" Тогда
			...
			Если ПеренестиЗначенияРеквизитов Тогда
				...
				// Загрузка перечня банков, №14618 
				// {
				Объект.СВИФТБанка 		 = Банк.СВИФТБИК;
				// }
				// 4D
...
Процедура УправлениеЭлементамиФормы()
	...
	//++ НЕ УТ
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ГосударственныеКонтракты) Тогда
		Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ГосударственныйКонтракт, "Состояние") = Перечисления.СостоянияГосударственныхКонтрактов.Завершен Тогда
	...
	Если Не Объект.ИностранныйБанк Тогда
		...
		// 4D:ERP для Беларуси, Дмитрий, 19.06.2017 17:50:53 
		// IBAN
		// {
	...
	// Клиент-банк, №11678
	// {
	Элементы.Программа.Доступность = Объект.ОбменСБанкомВключен;
	// }
	// 4D
	...
	// Клиент-банк, №11678
	// {
	Если Не Элементы.Найти("Программа") = Неопределено Тогда
...
Функция СформироватьАвтоНаименование()
	...
	// 4D:ERP для Беларуси, Дмитрий, 11.11.2017 8:01:04 
	// поправим наименование
	// {
	Если Объект.РучноеИзменениеРеквизитовБанка Тогда
		СтрокаНаименования = СокрЛП(Объект.НомерСчета) 
