...
// 4D:ERP для Беларуси, ВалентинМ, 22.06.2017 17:03:01 
// Общая функциональность
// {

// Получает страны ЕАЭС в виде структуры согласно макета "Классификатор"
Функция ПолучитьСтруктуруСтраныЕАЭС(Знач Дата = '00010101') Экспорт
	
	Если Дата = '00010101' Тогда
		Дата = ТекущаяДатаСеанса();
	КонецЕсли;
	
	струкЗначений = Новый Структура("Наименование, КодАльфа2, КодАльфа3, Код, ПолноеНаименование", "", "", "", "", "");
	
	Если Дата >= '20150101' Тогда
		струкЗначений.Наименование = "РОССИЯ,БЕЛАРУСЬ,КАЗАХСТАН";
		струкЗначений.КодАльфа2 = "RU,BY,KZ";
		струкЗначений.КодАльфа3 = "RUS,BLR,KAZ";
		струкЗначений.Код = "643,112,398";
		струкЗначений.ПолноеНаименование = "Российская Федерация,Республика Беларусь,Республика Казахстан";
	КонецЕсли;
	
	Если Дата >= '20150102' Тогда
		струкЗначений.Наименование = струкЗначений.Наименование + ",АРМЕНИЯ";
		струкЗначений.КодАльфа2 = струкЗначений.КодАльфа2 + ",AM";
		струкЗначений.КодАльфа3 = струкЗначений.КодАльфа3 + ",ARM";
		струкЗначений.Код = струкЗначений.Код + ",051";
		струкЗначений.ПолноеНаименование = струкЗначений.ПолноеНаименование + ",Республика Армения";
	КонецЕсли;
	
	Если Дата >= '20150812' Тогда
		струкЗначений.Наименование = струкЗначений.Наименование + ",КИРГИЗИЯ";
		струкЗначений.КодАльфа2 = струкЗначений.КодАльфа2 + ",KG";
		струкЗначений.КодАльфа3 = струкЗначений.КодАльфа3 + ",KGZ";
		струкЗначений.Код = струкЗначений.Код + ",417";
		струкЗначений.ПолноеНаименование = струкЗначений.ПолноеНаименование + ",Киргизская Республика";
	КонецЕсли;
	
	Возврат струкЗначений;
