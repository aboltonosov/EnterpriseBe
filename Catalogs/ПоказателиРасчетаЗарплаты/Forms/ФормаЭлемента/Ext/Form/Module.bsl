...
Процедура УстановитьЗначенияРеквизитовПоказателяТарификации(Форма)
	...
	#Если Клиент Тогда
		ПодсистемаТарификационнойОтчетностиСуществует = ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина.ТарификационнаяОтчетностьУчрежденийФМБА");
	#Иначе
		ПодсистемаТарификационнойОтчетностиСуществует = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина.ТарификационнаяОтчетностьУчрежденийФМБА");
	#КонецЕсли
	
	Если ПодсистемаТарификационнойОтчетностиСуществует Тогда
		
		#Если Клиент Тогда
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("ТарификационнаяОтчетностьУчрежденийФМБАКлиентСервер");
		#Иначе
			Модуль = ОбщегоНазначения.ОбщийМодуль("ТарификационнаяОтчетностьУчрежденийФМБАКлиентСервер");
		#КонецЕсли
