...
Процедура НаборЗаписейВременноПребывающиеПринятыеПоДолгосрочнымДоговорамПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НоваяСтрока Тогда
		...
		СтрукутраПоиска = Новый Структура("ГоловнаяОрганизация,Период", СтрокаНаборЗаписейВременноПребывающиеПринятыеПоДолгосрочнымДоговорам.ГоловнаяОрганизация, СтрокаНаборЗаписейВременноПребывающиеПринятыеПоДолгосрочнымДоговорам.Период);
		Если НаборЗаписейВременноПребывающиеПринятыеПоДолгосрочнымДоговорам.НайтиСтроки(СтрукутраПоиска).Количество() > 1 Тогда
			Отказ = Истина;
...
Процедура УстановитьОтображениеОсобенностиРасчетаСтраховыхВзносов()
	...
	// 4D:ERP для Беларуси, Яна, 18.04.2017 16:21:10 
	// Исправление справочников по подсистемам "Кадры" и "Зарплата", № 14769 
	// {
	ИнформационнаяНадписьКПолюВидЗастрахованногоЛица =
	ИнформационнаяНадписьДляФормРедактированияГражданства(СтатусыЗастрахованныхФизическихЛиц.ВидЗастрахованногоЛица);
	// }
	// 4D
...
// {
&НаСервере
Функция ИнформационнаяНадписьДляФормРедактированияГражданства(ВидЗастрахованногоЛица)	
	Возврат "";
КонецФункции
// }
// 4D

// 4D:ERP для Беларуси, Юлия, 04.11.2017 14:38:36 
// Редактировать форму "Страхование спр. "Сотрудники", № 16523 
// {
&НаКлиенте
Процедура СведенияОбИнвалидностиФизическихЛицЕстьСправкаПриИзменении(Элемент)
	ОбновитьПолеСведенияОбИнвалидностиПериод(ЭтаФорма, ОбщегоНазначенияКлиент.ДатаСеанса()); 
КонецПроцедуры
// }
// 4D
