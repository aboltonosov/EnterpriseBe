...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Яна, 06.06.2017 12:29:44 
	// Спр. "Виды доходов подоходного налога" элементы на форме не доступны для редактирования, № 15051 
	// {
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ЗапроситьРежимИзмененияВычетаПоДоходамНДФЛ(ЭтаФорма, ВычетыПоДоходамНДФЛ.Период, Отказ);
...
Функция ЗаписатьРедактируемыеРеквизитыКодаДоходаНДФЛ()
	...
	Возврат РедактированиеПериодическихСведений.ЗаписатьЗаписьПослеРедактированияВФорме(ЭтаФорма, "РедактируемыеРеквизитыКодаДоходаНДФЛ", СсылкаНаОбъект);
...
Процедура ЗаписатьНаКлиентеЗавершение(Отказ, ДополнительныеПараметры) Экспорт 
	...
	Иначе
		ЗаписатьНаСервере(ДополнительныеПараметры.ЗакрытьПослеЗаписи);
...
// {
&НаКлиенте
Процедура ЗапроситьРежимИзмененияВычетаПоДоходамНДФЛ(ФормаИсточник, ДатаИзменения, Отказ)
	
	ТекстКнопкиДа = НСтр("ru = 'Изменился вычет к доходам по НДФЛ'");
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При редактировании Вы изменили вычет. 
                  |Если Вы исправили прежнюю запись вычета к доходам по НДФЛ (она была ошибочной), нажмите ""Исправлена ошибка"".
                  |Если у изменился вычет к доходам по НДФЛ с %1, нажмите ""%2""'"), 
			Формат(ДатаИзменения, "ДФ='д ММММ гггг ""г""'"),
			ТекстКнопкиДа);
	
	РедактированиеПериодическихСведенийКлиент.ЗапроситьРежимИзмененияРегистра(ФормаИсточник, "ВычетыПоДоходамНДФЛ", ТекстВопроса, ТекстКнопкиДа, Отказ);
...
// { 
&НаКлиенте
Процедура НеОблагаетсяУНалоговогоАгентаПриИзменении(Элемент)
	
	ДоступностьЭлементовНалогообложения();
	
КонецПроцедуры

Процедура ДоступностьЭлементовНалогообложения()
	
	Если Объект.НеОблагаетсяУНалоговогоАгента Тогда
		Объект.СтавкаНалогообложенияРезидента = Перечисления.НДФЛСтавкиНалогообложенияРезидента.НеОблагается;
		Элементы.СтавкаНалогообложенияРезидента.Доступность = Ложь;
	Иначе 
		Элементы.СтавкаНалогообложенияРезидента.Доступность = Истина;
	КонецЕсли;
