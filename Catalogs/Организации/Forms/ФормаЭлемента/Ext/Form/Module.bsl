...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// {
	КонтрагентБелгосстрах				= Объект.КонтрагентБелгосстрах;
	ДоговорБелгосстрах					= Объект.ДоговорБелгосстрах;
	// }
	// 4D
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ДанныеРегистрацийПФРИзменены Тогда
		...
		// Локализовать документ "Отражение зарплаты в финансовом учете", № 15826
		// {
		ЗаполнитьЗначенияСвойств(ТекущийОбъект, ЭтаФорма, "КодОрганаПФР, НаименованиеТерриториальногоОрганаПФР, РегистрационныйНомерПФР, ИПРегистрационныйНомерПФР, ДоговорБелгосстрах, КонтрагентБелгосстрах");
		// }
		// 4D
...
Процедура ЗаполнитьОписаниеПФР()
	...
	// 4D:ERP для Беларуси, Петр, 03.11.2017 14:54:35 
	// Редактирование закладок "Регистрационные данные" и "Зарплата и кадры" спр. Организации, №16293
	// {
	СтрокаОписанияРеквизитовПФР = НСтр("ru = 'Регистрационный номер: %РегистрационныйНомерПФР%. Наименование территориального органа: %НаименованиеТерриториальногоОрганаПФР%.%ИПРегистрационныйНомерПФР%'");
...
Процедура ЗаполнитьОписаниеФСС()
	...
	// 4D:ERP для Беларуси, Петр, 02.11.2017 11:54:30 
	// Редактирование закладок "Регистрационные данные" и "Зарплата и кадры" спр. Организации, №16293
	// {
	СтрокаОписанияРеквизитовФСС = НСтр("ru = 'Регистрационный номер: %РегистрационныйНомерФСС%. Наименование территориального органа: %НаименованиеТерриториальногоОрганаФСС%. %ДанныеИП%'");
	...
	Если ВидОрганизации = "ИндивидуальныйПредприниматель" Тогда
		...
		СтрокаОписанияРеквизитовФСС = СтрЗаменить(СтрокаОписанияРеквизитовФСС, 
												"%ДанныеИП%",   
												Символы.ПС + НСтр("ru = 'Регистрационный номер ИП: %ИПРегистрационныйНомерФСС%.'"));
...
Процедура ЗаполнитьОписаниеПараметровРасчетаЗарплаты()
	...
	// 4D:ERP для Беларуси, Петр, 30.10.2017 11:21:58 
	// Редактирование закладок "Регистрационные данные" и "Зарплата и кадры" спр. Организации, №16293
	// {
	СтрокаОписания = НСтр("ru = 'График работы сотрудников: %ГрафикРаботыСотрудников%.'");
	// }
	// 4D
...
Процедура ЗаполнитьОписаниеКадровогоУчетаИРасчетаЗарплаты()
	...
	// 4D:ERP для Беларуси, Яна, 27.06.2017 17:13:34 
	// Зарплата и кадры, № 14167 
	// {
	...
	Если ИспользоватьНачислениеЗарплаты Тогда  
		СтрокаОписания = СтрЗаменить(
				СтрокаОписания, 
				"%ПродолжениеСтроки%",
				НСтр("ru = 'Ставка взносов в Белгосстрах : %Ставка%. 
					|%ПродолжениеСтроки%'"));
		
	КонецЕсли;			
	
	// 4D:ERP для Беларуси, Петр, 30.10.2017 11:24:54 
	// Редактирование закладок "Регистрационные данные" и "Зарплата и кадры" спр. Организации, №16293
	// {
...
Процедура ЗаполнитьОписаниеБухучетаИВыплатыЗарплат()
	...
	СтрокаОписания = "%ПродолжениеСтроки%";
	// 4D:ERP для Беларуси, Петр, 30.10.2017 11:26:53 
	// Редактирование закладок "Регистрационные данные" и "Зарплата и кадры" спр. Организации, №16293
	// {
	Если ИспользоватьНачислениеЗарплаты Тогда
		СтрокаОписания = СтрЗаменить(
			СтрокаОписания, 
			"%ПродолжениеСтроки%", 
			НСтр("ru = 'Статья финансирования: %СтатьяФинансирования%. Способ отражения зарплаты в бухучете: %СпособОтраженияЗарплатыВБухучете%.
				|%ПродолжениеСтроки%'"));	
	КонецЕсли;
...
Процедура ВосстановитьДанныеРегистрацийВПФР()
	...
	// Локализовать документ "Отражение зарплаты в финансовом учете", № 15826
	// {
	ЗаполнитьЗначенияСвойств(ЭтаФорма, Объект, "КодОрганаПФР, НаименованиеТерриториальногоОрганаПФР, РегистрационныйНомерПФР, ИПРегистрационныйНомерПФР, ДоговорБелгосстрах, КонтрагентБелгосстрах");
	// }
	// 4D
