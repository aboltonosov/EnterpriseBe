...
Процедура ПередЗаписью(Отказ)
	...
	// 4D:ERP для Беларуси, Яна, 24.05.2017 11:28:13 
	// Отражение суммы ФОТ при создании новой позиции штатного расписания, №14918 
	// {	
	
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ШтатноеРасписание.Ссылка,
		|	ШтатноеРасписание.Закрыта
		|ИЗ
	...
	ПодразделениеЕсть = Выборка.Следующий();
		
	Если ЭтоНовый() Тогда
		
		Если ПодразделениеЕсть Тогда
			Родитель = Выборка.Ссылка;
			УстановитьПодразделениеЗакрыто(Выборка);	
		Иначе
			...
			ГруппаРодитель.Закрыта = Закрыта;
			ГруппаРодитель.Записать();
			
			Родитель = ГруппаРодитель.Ссылка;
		КонецЕсли;
	Иначе
		УстановитьПодразделениеЗакрыто(Выборка);	
	КонецЕсли; 
...
// Отражение суммы ФОТ при создании новой позиции штатного расписания, №14918 
// {
Процедура УстановитьПодразделениеЗакрыто(Выборка)
	ПодразделениеСсылка = Выборка.Ссылка;
	ПодразделениеЗакрытоБыло = Выборка.Закрыта;
	...
	РезультатЗапроса = Запрос.Выполнить();
	ПодразделениеЗакрытоСтало = Ложь;
	Если РезультатЗапроса.Пустой() И Закрыта Тогда
		ПодразделениеЗакрытоСтало = Истина;
	КонецЕсли;
	Если ПодразделениеЗакрытоСтало <> ПодразделениеЗакрытоБыло Тогда
		ПодразделениеОбъект = ПодразделениеСсылка.ПолучитьОбъект();
		ПодразделениеОбъект.Закрыта = ПодразделениеЗакрытоСтало;
