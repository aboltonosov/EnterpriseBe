...
Процедура НастроитьПолеКалендаря()
	...
	// 4D:ERP для Беларуси, АлексейЧ, 11.12.2017 10:17:33 
	// При заполнении производственного календаря для РБ на 1 января программа ставит рабочий день, №17038
	// {
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ЕСТЬNULL(ДанныеПроизводственногоКалендаря.Год, 0) КАК Год
		|ИЗ
		|	РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаря
		|ГДЕ
		|	ДанныеПроизводственногоКалендаря.ПроизводственныйКалендарь = &ПроизводственныйКалендарь";
	
	Запрос.УстановитьПараметр("ПроизводственныйКалендарь", Объект.Ссылка);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НомерГодаТекущегоЭлементакКалендаря =  ВыборкаДетальныеЗаписи.Год;
	КонецЦикла;
	
	Если НомерТекущегоГода = 0 И НомерГодаТекущегоЭлементакКалендаря = 0 Тогда
		НомерТекущегоГода = Год(ТекущаяДатаСеанса());
	ИначеЕсли НомерТекущегоГода = 0 И НомерГодаТекущегоЭлементакКалендаря <> 0 Тогда 	
		Если НомерГодаТекущегоЭлементакКалендаря = Неопределено Тогда 
			НомерТекущегоГода = Год(ТекущаяДатаСеанса());		
		Иначе	
			НомерТекущегоГода = НомерГодаТекущегоЭлементакКалендаря;
		КонецЕсли;	
	КонецЕсли;
	// }
	// 4D
