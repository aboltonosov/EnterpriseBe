...
Функция РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(КодПроизводственногоКалендаря, НомерГода) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ПраздничныеДни Цикл
		...
		Если Год(ДатаПредпраздничногоДня) <> НомерГода Тогда
			...
			// 4D:ERP для Беларуси, АлексейЧ, 07.12.2017 15:40:24 
			// При заполнении производственного календаря для РБ на 1 января программа ставит рабочий день, № 17038
			// Предпраздничные дни другого года пропускаем, кроме 31 декабря, т.к. 1 января - НОВЫЙ ГОД
			// {
			Если Месяц(ДатаПредпраздничногоДня) = 12 И День(ДатаПредпраздничногоДня) = 31 Тогда
...
Функция ПраздничныеДниПроизводственногоКалендаря(КодПроизводственногоКалендаря, НомерГода)
	...
	// 4D:ERP для Беларуси, ВладимирР, 11.05.2016 10:40:06 
	// Производственный календарь, №10820
	// {
	// 1 января - Новый год (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "01.01", НомерГода, Ложь);
	
	// 7 января - Рождество Христово (православное) (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "07.01", НомерГода, Ложь);
	
	// 8 марта - Международный женский день (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "08.03", НомерГода, Ложь);
	
	// 1 мая - Праздник труда (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "01.05", НомерГода, Ложь);
	
	// 9 мая - День Победы (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "09.05", НомерГода, Ложь);
	
	// 3 июля - День Независимости Республики Беларусь (День Республики) (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "03.07", НомерГода, Ложь);
	
	// 7 ноября - День Октябрьской революции (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "07.11", НомерГода, Ложь);
	
	// 25 декабря - Рождество Христово (католическое) (нерабочий)
	ДобавитьПраздничныйДень(ПраздничныеДни, "25.12", НомерГода, Ложь);
	// }
	// 4D
