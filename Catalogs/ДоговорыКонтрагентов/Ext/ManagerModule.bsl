...
Функция ТипДоговора(ХозяйственнаяОперация) Экспорт
	...
	Возврат
		...
		?((ХозяйственнаяОперация = Операции.ЗакупкаУПоставщика
			ИЛИ ХозяйственнаяОперация = Операции.ЗакупкаВСтранахЕАЭС) 
...
// Переносит файлы из справочника ""УдалитьГосударственныеКонтрактыПрисоединенныеФайлы"" в справочник ""ДоговорыКонтрагентовПрисоединенныеФайлы""
// Заполняет реквизит "Файл" табличной части "ПодтверждающиеДокументы" договора ссылкой на новый файл
// Очищает реквизит "УдалитьПодтверждающийДокумент" табличной части "ПодтверждающиеДокументы" договора
//
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	...
	|	Договоры.Ссылка
	|ИЗ
	|	Справочник.ДоговорыКонтрагентов.ПодтверждающиеДокументы КАК Договоры
	|ГДЕ
	|	Договоры.Ссылка.ПлатежиПо275ФЗ
	|	И НЕ Договоры.УдалитьПодтверждающийДокумент.Ссылка ЕСТЬ NULL 
	|	И Договоры.Файл.Ссылка ЕСТЬ NULL");
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПодтверждающийДокумент Из ДоговорОбъект.ПодтверждающиеДокументы Цикл
