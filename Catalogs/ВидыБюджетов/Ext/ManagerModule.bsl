...
//		* БлокуируемыйРеквизит - Строка - Имя блокируемого реквизита.
//
		...
		Для Каждого СтрокаОшибки из КлючиИЗначение.Значение Цикл
			...
			Если КлючиИЗначение.Ключ = "Статьи" Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, 
										НСтр("ru = 'статьи'"), СтрокаОшибки.СтатьяПоказатель, НСтр("ru = 'статье'"), СтрокаОшибки.ВидАналитики);
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, 
										НСтр("ru = 'показателя'"), СтрокаОшибки.СтатьяПоказатель, НСтр("ru = 'показателе'"), СтрокаОшибки.ВидАналитики);
			КонецЕсли;
	...
	Для Сч = 1 По МассивЭлементов.ВГраница() Цикл
		...
		Если МассивЭлементов[Сч].Свойство("СтатьиБюджетов") Тогда
			Для Каждого КлючИЗначение из МассивЭлементов[Сч].СтатьиБюджетов Цикл
				Если МассивЭлементов[Сч-1].СтатьиБюджетов[КлючИЗначение.Ключ] = Неопределено ИЛИ МассивЭлементов[Сч-1].СтатьиБюджетов[КлючИЗначение.Ключ].СтатьяБюджетов <> КлючИЗначение.Значение.СтатьяБюджетов
...
Процедура ЗарегистироватьДанныеДляОчисткиКэшВспомогательныхДанных(Параметры) Экспорт
...
Функция ИспользуемыеСтатьиПоказателиВидаБюджета(ВидБюджета) 
	...
	Запрос.УстановитьПараметр("Реквизит_ПривилегированныйРасчет", ПланыВидовХарактеристик.РеквизитыЭлементовФинансовыхОтчетов.ПривилегированныйРасчет);
