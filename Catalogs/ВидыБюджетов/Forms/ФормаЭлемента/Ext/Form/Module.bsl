...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ЕстьПривилегированныйДоступКДанным = ТекущийОбъект.КэшСтатейИПоказателей.НайтиСтроки(
									Новый Структура("ПривилегированныйРасчет", Истина)).Количество() > 0;
...
Процедура ИспользоватьДляВводаПланаПриИзмененииНаСервере()
	...
	// Обнулим параметры рекдактирования
...
Процедура ВывестиДеревоЭлементовВДеревоОтображения(ЭлементыДерева, РодительскаяСтрока, ГлубинаУровней)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	ИСТИНА КАК ПривилегированныйРасчет
	               ...
	               |			И (ЭлементыФинансовыхОтчетовРеквизитыВидаЭлемента.Реквизит = &ПривилегированныйРасчет)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	Операнды.Операнд,
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	МАКСИМУМ(ЕСТЬNULL(ЭлементыФинансовыхОтчетовРеквизитыВидаЭлемента.Значение, ЛОЖЬ)) КАК ЭлементПривилегированныйРасчет,
	               |	ЭлементыФинансовыхОтчетовЭлементыТаблицы.Ссылка КАК Родитель,
	               |	МАКСИМУМ(ЕСТЬNULL(ОперандыФормулыПривилегированно.ПривилегированныйРасчет, ЛОЖЬ)) КАК ОперандыПривилегированныйРасчет
	               |ИЗ
	               ...
	               |		ИЛИ МАКСИМУМ(ЕСТЬNULL(ОперандыФормулыПривилегированно.ПривилегированныйРасчет, ЛОЖЬ)) = ИСТИНА)";
				   
	Запрос.УстановитьПараметр("ПривилегированныйРасчет", ПланыВидовХарактеристик.РеквизитыЭлементовФинансовыхОтчетов.ПривилегированныйРасчет);
		...
		Если Не НоваяСтрока = Неопределено Тогда
			Если НоваяСтрока.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НастройкаЯчеек Тогда
				...
				Если СложнаяТаблицаПривилегированно.НайтиСтроки(Новый Структура("Родитель",СтрокаДерева.ЭлементОтчета)).Количество() > 0 Тогда
					ЕстьПривилегированныйРасчет = Истина;
				КонецЕсли;
			Иначе
				ЕстьПривилегированныйРасчет = Ложь;
				Если ЗначениеЗаполнено(НоваяСтрока.ПривилегированныйРасчет) И Не НоваяСтрока.ПривилегированныйРасчет Тогда
					Если ОперандыФормулыПривилегированно.НайтиСтроки(Новый Структура("Родитель",СтрокаДерева.ЭлементОтчета)).Количество() > 0 Тогда
						ЕстьПривилегированныйРасчет = Истина;
					КонецЕсли;
				Иначе
					ЕстьПривилегированныйРасчет = ?(ЗначениеЗаполнено(НоваяСтрока.ПривилегированныйРасчет),НоваяСтрока.ПривилегированныйРасчет, Ложь)
				КонецЕсли;
			КонецЕсли;
			НоваяСтрока.ПривилегированныйРасчетКартинка = Не Число(ЕстьПривилегированныйРасчет);
		КонецЕсли;
...
Процедура ОбновитьСтрокуВидаБюджетаПослеИзменения(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено Тогда
		...
		КонецЕсли;
		ЕстьПривилегированныйРасчет = Ложь;
		Если Результат.Свойство("ЕстьПривилегированныйРасчет") И Результат.ЕстьПривилегированныйРасчет Тогда
			ЕстьПривилегированныйРасчет = Результат.ЕстьПривилегированныйРасчет;
		Иначе
			ЕстьПривилегированныйРасчет = ЗначениеЗаполнено(СтрокаОтражение.ПривилегированныйРасчет) И СтрокаОтражение.ПривилегированныйРасчет;
		КонецЕсли;			
		СтрокаОтражение.ПривилегированныйРасчетКартинка = Не Число(ЕстьПривилегированныйРасчет);
...
Процедура ДобавитьСтрокуОтражение(ЭлементыДобавления, ИсточникЭлементОтчета, Форма)
	...
	Для Сч = 2 по Форма.Элементы.НаименованиеДляПечати.ПодчиненныеЭлементы.Количество() Цикл
		...
		КонецЕсли;
	КонецЦикла;
	НоваяСтрокаОтражения.ПривилегированныйРасчетКартинка = Не Число(НоваяСтрокаОтражения.ПривилегированныйРасчет);
...
Процедура ЗаполнитьКэшСтатейИПоказателей(СтрокаДереваЭлементов, ТзКэшСтатейИПоказателей)
	...
	Если СтрокаДереваЭлементов.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов 
		...
		КонецЕсли;
		НоваяСтрока.ПривилегированныйРасчет = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеЭлемента, "ПривилегированныйРасчет");
	КонецЕсли;
	...
	Для Каждого Стр Из ДанныеЭлемента.ЭлементыТаблицы Цикл
		...
		КонецЕсли;
		НоваяСтрока.ПривилегированныйРасчет = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеТаблицы, "ПривилегированныйРасчет");
	...
	Для Каждого ЭлементДляПроверки Из МассивЭлементовДляПроверки Цикл
		...
		Для Каждого Стр Из ЭлементДляПроверки.ОперандыФормулы Цикл
			...
			КонецЕсли;
			НоваяСтрока.ПривилегированныйРасчет = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеОперанд, "ПривилегированныйРасчет");

		КонецЦикла;
		
		Для Каждого Стр Из ЭлементДляПроверки.ИсточникиЗначений Цикл
			
			ДанныеИсточник = Стр.Источник;
			
			Если ТипЗнч(ДанныеИсточник) = Тип("Строка") Тогда
				ДанныеИсточник = ПолучитьИзВременногоХранилища(ДанныеИсточник);
			КонецЕсли;
			
			Если Не (ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов 
				ИЛИ ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов 
				ИЛИ ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель) Тогда
				
				Продолжить;
			КонецЕсли;
			
			НоваяСтрока = ТзКэшСтатейИПоказателей.Добавить();
			Если ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеИсточник, "СтатьяБюджетов");
			ИначеЕсли ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ПоказательБюджетов Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеИсточник, "ПоказательБюджетов");
			ИначеЕсли ДанныеИсточник.ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель Тогда
				НоваяСтрока.СтатьяПоказатель = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеИсточник, "НефинансовыйПоказатель");
			КонецЕсли;
			НоваяСтрока.ПривилегированныйРасчет = ФинансоваяОтчетностьВызовСервера.ЗначениеДополнительногоРеквизита(ДанныеИсточник, "ПривилегированныйРасчет");
...
Функция ПолучитьКэшСтатейИПоказателей(ДеревоЭлементов)
	...
	ТзКэшСтатейИПоказателей.Колонки.Добавить("ПривилегированныйРасчет", Новый ОписаниеТипов("Булево"));
	...
	ТзКэшСтатейИПоказателей.Свернуть("СтатьяПоказатель,ПривилегированныйРасчет");
