...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаОперанда из ДанныеОбъекта.ОперандыФормулы Цикл
		...
		НоваяСтрока.ПривилегированныйРасчетКартинка = Не Число(НоваяСтрока.ПривилегированныйРасчет);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ Отказ Тогда
		...
		Если ЗначениеЗаполнено(ЭтаФорма.АдресЭлементаВХранилище) Тогда
			...
			КонецЦикла;
			СтруктураЭлемента.Вставить("ЕстьПривилегированныйРасчет", ЕстьПривилегированныйРасчет);
...
Процедура ОперандыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Отказ = ДублирующийИдентификатор И Не ОтменаРедактирования;
КонецПроцедуры
...
Процедура ЗавершитьРедактирование(Команда)
	...
	ЕстьПривилегированныйРасчет = Операнды.НайтиСтроки(Новый Структура("ПривилегированныйРасчет",Истина)).Количество() > 0;
	ДополнительныеСведения = Новый Структура();
	ДополнительныеСведения.Вставить("ЕстьПривилегированныйРасчет", ЕстьПривилегированныйРасчет);
...
Процедура ОбновитьЗаголовокФормы(Запись = Ложь) Экспорт
	...
	ПредставлениеТипа = НСтр("ru = 'Производный показатель'");
...
Процедура ОбновитьСтрокуВидаБюджетаПослеИзменения(Результат, ДополнительныеПараметры) Экспорт
	...
	ТекущиеДанные = Элементы.Операнды.ТекущиеДанные;
	ТекущиеДанные.ПривилегированныйРасчетКартинка = Не Число(ТекущиеДанные.ПривилегированныйРасчет);
