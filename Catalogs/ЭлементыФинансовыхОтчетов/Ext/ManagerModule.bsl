...
//
// Параметры:
//  ПериодОтчета - Структура - описание периода со следующимим ключами - значениями:
//                           НачалоПериода - Дата - начало периода
//                           КонецПериода - Дата - конец периода
//                           Периодичность - Перечисление.Периодичность - массив предопределнных интервалов
//
// Возвращаемое значение:
//   ТаблицаЗначений - в таблице содержатся конкретные даты из переданного периода для предопределенных интервалов год, полугодие, квартал, месяц и т.д.
//                     Если в параметрах периода передан пустой массив интервалов,
...
//
// Параметры:
//  ЭлементОтчета - Справочник.ЭлементыФинансовыхОтчетов - текущий элмент отчета.
//
// Возвращаемое значение:
...
Функция СхемаОтбораИзмерения(СтруктураИменИзмерений)
	...
	Для Каждого КлючИЗначение из СтруктураИменИзмерений Цикл
		...
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("ПланВидовХарактеристикСсылка.АналитикиСтатейБюджетов") Тогда
			...
			БюджетнаяОтчетностьРасчетКэшаСервер.ПолучитьТекстПоляВидаАналитики(КлючИЗначение.Ключ) + ")" + ?(ДобавлятьСуффикс, ".*", "") + "
	...
	Если ТипЗнч(ВидОтчета) = Тип("СправочникСсылка.ВидыБюджетов") И ДанныеЭлемента <> "" Тогда
		...
		Если НужноЗаполнениеИсточников Тогда
			...
			Если ЭтоЗаполнение Тогда
				...
				КонецЦикла;
				СписокДоступныхЭлементов = БюджетнаяОтчетностьРасчетКэшаСервер.ИсточникиЗначенийПоУмолчанию(Кэш, Элемент, , , Истина); 
