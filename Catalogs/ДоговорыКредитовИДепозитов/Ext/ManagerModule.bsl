...
Функция СоздатьЗаявкиНаРасходДС(Параметры) Экспорт
	...
	//++ НЕ УТ
	// по каждому типу платежей 275-ФЗ,
	//-- НЕ УТ
	...
	//++ НЕ УТ
										ПлатежиПо275ФЗ = ДатаОплаты.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
										Пока ПлатежиПо275ФЗ.Следующий() Цикл
											ТипыПлатежей275ФЗ = ПлатежиПо275ФЗ.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
											Пока ТипыПлатежей275ФЗ.Следующий() Цикл
												
												Если ПоследнийУровень275ФЗ Тогда
													
													СформироватьЗаявкуНаОплату(ТипыПлатежей275ФЗ, Параметры);
	...
	//++ НЕ УТ
											...
											КонецЦикла;//по Типам платежей 275-ФЗ
										КонецЦикла;//по наличию платежей 275-ФЗ
	//-- НЕ УТ
...
Функция ТекстЗапросаПоОплатам()
	...
	|			ИЛИ  ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.Депозит)
	|			ТОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПлатежиПо275ФЗ,
	|	ВЫБОР
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.КредитИлиЗайм)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеСуммКредитовПеречисленныхУполномоченнымБанком)
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.Депозит)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеСвязаннноеСРазмещениемНаДепозитеУполномоченногоБанка)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПустаяСсылка)
	|	КОНЕЦ КАК ТипПлатежаФЗ275,
	//-- НЕ УТ
	|	ДоговорыКредитовИДепозитов.Партнер,
	...
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.КредитИлиЗайм)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПеречисленияПрибылиИсполнителю)
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.Депозит)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеСвязаннноеСРазмещениемНаДепозитеУполномоченногоБанка)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПустаяСсылка)
	...
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.КредитИлиЗайм)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеНаОтдельныйСчет)
	|		КОГДА ДоговорыКредитовИДепозитов.БанковскийСчет.ОтдельныйСчетГОЗ
	|				И ДоговорыКредитовИДепозитов.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.Депозит)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеСвязаннноеСРазмещениемНаДепозитеУполномоченногоБанка)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПустаяСсылка)
	...
	//++ НЕ УТ
	|	ОплатыПоГрафику.ПлатежиПо275ФЗ КАК ПлатежиПо275ФЗ,
	|	ОплатыПоГрафику.ТипПлатежаФЗ275 КАК ТипПлатежаФЗ275,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ОплатыПоГрафику.ПлатежиПо275ФЗ,
	|	ОплатыПоГрафику.ТипПлатежаФЗ275,
	//-- НЕ УТ
	|	ОплатыПоГрафику.Партнер,
	|	ОплатыПоГрафику.Контрагент,
...
Процедура СформироватьЗаявкуНаОплату(ДатаОплаты, Параметры)
	...
	//++ НЕ УТ
	ДенежныеСредстваСервер.ЗаполнитьПодтверждающиеДокументы(НоваяЗаявка, НоваяЗаявка.ПодтверждающиеДокументы);
	//-- НЕ УТ
