...
Процедура ЗаполнитьСписокНалоговыхОрганов(АктивироватьОрган = Неопределено)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РегистрацииВНалоговомОргане КАК РегистрацииУпр
	|		ПО (РегистрацииУпр.РегистрацияВНалоговомОргане = НалоговыеОрганы.Ссылка)
	|			И (РегистрацииУпр.Организация = &Организация)
	//++ НЕ УТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИсторияРегистрацийВНалоговомОргане.СрезПоследних КАК РегистрацииРегл
	|		ПО (РегистрацииРегл.РегистрацияВНалоговомОргане = НалоговыеОрганы.Ссылка)
	|			И (РегистрацииРегл.СтруктурнаяЕдиница ССЫЛКА Справочник.ПодразделенияОрганизаций)
	//-- НЕ УТ
	|ГДЕ
	|	НалоговыеОрганы.Владелец = &Организация
	|
	|СГРУППИРОВАТЬ ПО
...
Процедура ОткрытьФормуВыбораНалоговогоОргана(ВыделенныеСтроки, Периодичность = Истина)
	...
	ПараметрыФормы = Новый Структура("СписокПодразделений, Организация, Периодичность", СписокПодразделений, Организация, Периодичность);
