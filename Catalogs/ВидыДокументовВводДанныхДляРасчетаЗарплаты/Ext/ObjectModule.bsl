...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Показатели.Количество() = 0
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не выбраны ни показатели расчета зарплаты, ни виды времени, ни виды работ'"), , , , Отказ);
	КонецЕсли;
	...
	ТекстОшибки = НСтр("ru = 'Изменение шаблона невозможно. Выберите другие настройки или скопируйте шаблон'");
	...
	Пока Выборка.СледующийПоЗначениюПоля("ВидОшибки") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Документ") Цикл
			...
			Если КоличествоДокументовВОшибке > 2 Тогда
				...
				ТекстОшибкиДокументов = ТекстОшибкиДокументов + " " + НСтр("ru = 'и других'") + ", ";
				Прервать;
			КонецЕсли;
			ТекстОшибкиДокументов = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1%2 №%3 от %4г, '"),
		...
		Если Выборка.ВидОшибки = "Показатели" Или Выборка.ВидОшибки = "ВидыВремени" Или Выборка.ВидОшибки = "ВидыРабот" Тогда
			Пока Выборка.Следующий() Цикл
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.Таб + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1'"), Выборка.ЗначениеОшибки);
			КонецЦикла;
		КонецЕсли;
