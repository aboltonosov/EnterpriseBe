...
Процедура БИКБанкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Элемент.Имя = "БИКБанка" И Не ИностранныйБанк
		...
		ОткрытьФорму("Справочник.КлассификаторБанковРФ.ФормаВыбора", СтруктураПараметров, Элемент);
	КонецЕсли;
...
Процедура ПриИзмененииБанковскихРеквизитов(Элемент, ЗначениеПоля, ТекущийБанк, ТекущийБИК, ТекущийСВИФТ, ТекущийКоррСчет, ТекущееПредставление)
	...
	// 4D:ERP для Беларуси, Дмитрий, 13.11.2017 6:10:24 
	// пока отключим
	// {
	Возврат;
	// }
...
Функция СообщениеДубльИНН(ПредставлениеИНН_КПП, НайденныйДубльПоИНН)
	...
	// 4D:ERP для Беларуси, Дмитрий, 23.09.2017 10:42:54 
	// У нас УНП
	// {
	СтрСообщение = НСтр("ru='Контрагент создан не будет. УНП - %1 уже указаны для контрагента %2 с кодом %3, ответственный - %4.'");
	// }
	// 4D
...
Функция ПервичнаяИнформацияЗаполненаНеправильно()
	...
	Если НЕ Отказ Тогда
		...
		// 4D:ERP для Беларуси, Юлия, 10.11.2017 16:41:13 
		// Ошибки при создании Контрагента, № 16577
		// {
...
Процедура ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ИспользуютсяТолькоПартнеры Или (ДействиеСКонтрагентом = 1 И НЕ НаОснованииКонтрагента И УказыватьЮридическиеРеквизиты) Тогда
		...
		// 4D:ERP для Беларуси, Дмитрий, 11.09.2017 5:51:35 
		// Страна Беларусь
		// {
		ОбъектКонтрагент.СтранаРегистрации		= ?(ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент, СтранаРегистрации, Справочники.СтраныМира.Беларусь);
		// }
		// 4D
		...
		Если ЭтоКомпания = 0 И НЕ ПустаяСтрока(ТелефонПартнера) И ИспользуютсяТолькоПартнеры Тогда
			...
			ЗаполнитьРеквизитыТабличнойЧастиДляТелефона(НоваяСтрокаКИ, ЗначенияПолейВСписокЗначений(ТелефонПартнераЗначенияПолей));
		КонецЕсли;
