...
Процедура ЗаполнитьПредставлениеСтатейКалькуляцииВзносов(Форма)
	...
	Если Не ЗначениеЗаполнено(Форма.Объект.СтатьяКалькуляцииПФР) Тогда
		Заголовок = НСтр("ru = 'Отражать страховые взносы по статье калькуляции оплаты труда.'");
	ИначеЕсли Форма.Объект.СтатьяКалькуляцииПФР = Форма.Объект.СтатьяКалькуляцииФОМС И 
		Форма.Объект.СтатьяКалькуляцииПФР = Форма.Объект.СтатьяКалькуляцииФОМС Тогда
