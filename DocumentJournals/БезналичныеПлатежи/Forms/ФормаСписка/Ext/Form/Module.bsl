...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Клиент-банк, №11678
	// {
	ПриСозданииФормыЖурналДокументовБезналичныеПлатежиФормаСписка();
	// }
	// 4D
...
&НаСервере
Процедура ПриСозданииФормыЖурналДокументовБезналичныеПлатежиФормаСписка()
	Элементы.ЗагрузитьВыписку.Заголовок = "Загрузка выписки (для РФ)";
	Элементы.ВыгрузитьВБанк.Заголовок = "Выгрузка в банк (для РФ)";
	
	НоваяГруппа = Элементы.Добавить("СтандартныеКоманды_Локализация", Тип("ГруппаФормы"), Элементы.ПодменюВыгрузкаЗагрузка);
	НоваяГруппа.Заголовок = "Стандартные команды (локализация)";
	НоваяГруппа.Вид = ВидГруппыФормы.ГруппаКнопок;
	Элементы.Переместить(НоваяГруппа, Элементы.ПодменюВыгрузкаЗагрузка, Элементы.СтандартныеКоманды);
	
	НоваяКоманда = ЭтаФорма.Команды.Добавить("ВыгрузкаВБанк_Локализация");
	НоваяКоманда.Действие = "ВыгрузкаВБанк_Локализация";
	НовыйЭлемент = Элементы.Добавить("ВыгрузкаВБанк_Локализация", Тип("КнопкаФормы"), НоваяГруппа);
	НовыйЭлемент.ИмяКоманды = "ВыгрузкаВБанк_Локализация";
	НовыйЭлемент.Заголовок = "Выгрузка в банк (для Беларуси)";
	
	НоваяКоманда = ЭтаФорма.Команды.Добавить("ЗагрузкаВыписки_Локализация");
	НоваяКоманда.Действие = "ЗагрузкаВыписки_Локализация";
	НовыйЭлемент = Элементы.Добавить("ЗагрузкаВыписки_Локализация", Тип("КнопкаФормы"), НоваяГруппа);
	НовыйЭлемент.ИмяКоманды = "ЗагрузкаВыписки_Локализация";
	НовыйЭлемент.Заголовок = "Загрузка выписки (для Беларуси)";
КонецПроцедуры
...
&НаКлиенте
Процедура ВыгрузкаВБанк_Локализация(Команда)
	ОткрытьФорму("Обработка.КлиентБанк_Локализация.Форма.ВыгрузкаВБанк", Новый Структура("БанковскийСчет", БанковскийСчетОтбор), Элементы.СписокПлатежей);
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузкаВыписки_Локализация(Команда)
	ОткрытьФорму("Обработка.КлиентБанк_Локализация.Форма.ЗагрузкаВыписки", Новый Структура("БанковскийСчет", БанковскийСчетОтбор), Элементы.СписокПлатежей);
КонецПроцедуры
...
Процедура Оплатить(Команда)
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		...
		СтруктураОснование.Вставить("ЗаказПоставщику", СтрокаТаблицы.Ссылка);
		СтруктураОснование.Вставить("СуммаКОплате", СтрокаТаблицы.СуммаКОплате);
		СтруктураОснование.Вставить("БанковскийСчет", СтрокаТаблицы.БанковскийСчет);
		...
		КонецЕсли;
		//-- НЕ УТ
		ОткрытьФорму("Документ.СписаниеБезналичныхДенежныхСредств.ФормаОбъекта", СтруктураПараметры, Элементы.СписокПлатежей);
	КонецЕсли;
...
Процедура ОплатитьЗаявку(Команда)
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		...
		Иначе
			...
			СтруктураОснование.Вставить("БанковскийСчетКасса", СтрокаТаблицы.БанковскийСчет);
...
Процедура Подключаемый_ОбновитьКоманды()
	...
	УправлениеПечатьюКлиентСервер.ОбновитьКоманды(ЭтотОбъект, Элементы.СписокПлатежей);
...
Процедура ОбработкаЗавершенияСозданияОпераций(Результат, ДополнительныеПараметры) Экспорт 
	...
	Если ЗначениеЗаполнено(Результат) Тогда 
		...
		Если ЭтоАдресВременногоХранилища(Результат) Тогда 
			...
			"e1cib/list/ЖурналДокументов.БезналичныеПлатежи",
			ТекстСообщения);
