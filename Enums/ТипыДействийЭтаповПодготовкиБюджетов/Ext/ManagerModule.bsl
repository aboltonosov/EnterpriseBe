...
Функция ПолучитьПредставлениеДействия(Знач НастройкиДействия) Экспорт
	...
	Если ЗначениеЗаполнено(ВидыБюджетов) Тогда
		...
		Текст = СтрЗаменить(Шаблон, "%Представление", НСтр("ru = 'Вид бюджета'"));
		Текст = СтрЗаменить(Текст, "%Значение", ВидыБюджетов[0].ВидБюджета);
		...
		Строки = НастройкиДействия.НайтиСтроки(Отбор);
		Если Строки.Количество() > 0 И ТипЗнч(Строки[0].Значение) = Тип("ТаблицаЗначений") 
			...
			Для Каждого Колонка Из НастройкиАналитик.Колонки Цикл
				...
				Если ЗначениеЗаполнено(НастройкаАналитик[Колонка.Имя]) Тогда
					...
					Пока Сч <= РеквизитыБюджета.Количество() Цикл
						...
						Сч = Сч + 1;
					КонецЦикла;
					Текст = СтрЗаменить(Шаблон, "%Представление", КолонкаИмя);
					Текст = СтрЗаменить(Текст, "%Значение", НастройкаАналитик[Колонка.Имя]);
					Результат = Результат + ?(ЗначениеЗаполнено(Результат), ", ", "") + Текст;
				КонецЕсли;
				
			КонецЦикла;
			Если ЗначениеЗаполнено(ТекстАналитикиДляРедактирования) Тогда
				Результат = Результат + НСтр("ru = ', Разрешено для редактирования: '") + ТекстАналитикиДляРедактирования;
			КонецЕсли;
		КонецЕсли;
