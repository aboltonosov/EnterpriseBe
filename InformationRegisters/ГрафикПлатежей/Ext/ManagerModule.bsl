...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Расчеты.ЗаказКлиента КАК ОбъектОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Поступление) КАК ПоступлениеСписание
	|ИЗ
	...
	|	ПО
	|		ГрафикПлатежей.ОбъектОплаты = Расчеты.ЗаказКлиента
	...
	|	И ТИПЗНАЧЕНИЯ(Расчеты.ЗаказКлиента) НЕ В (
	|		ЗНАЧЕНИЕ(Документ.ВозвратТоваровОтКлиента),
	|		ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
	|	)
	|	И Расчеты.ЗаказКлиента <> НЕОПРЕДЕЛЕНО
	|	И Расчеты.ЗаказКлиента <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|";
	...
	|	Расчеты.КОплатеОстаток - Расчеты.ОплачиваетсяОстаток < 0
	|	И ГрафикПлатежей.ОбъектОплаты ЕСТЬ NULL
	|	И ТИПЗНАЧЕНИЯ(Расчеты.ЗаказКлиента) В (
	|		ЗНАЧЕНИЕ(Документ.ВозвратТоваровОтКлиента),
	|		ЗНАЧЕНИЕ(Документ.ЗаявкаНаВозвратТоваровОтКлиента)
	|	)
	|	И Расчеты.ЗаказКлиента <> НЕОПРЕДЕЛЕНО
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	Расчеты.ЗаказПоставщику КАК ОбъектОплаты,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Списание) КАК ПоступлениеСписание
	|ИЗ
	...
	|		РегистрСведений.ГрафикПлатежей КАК ГрафикПлатежей
	|	ПО
	|		ГрафикПлатежей.ОбъектОплаты = Расчеты.ЗаказПоставщику
	|		И ГрафикПлатежей.ПоступлениеСписание = ЗНАЧЕНИЕ(Перечисление.ТипыДвиженияДенежныхСредств.Списание)
	|ГДЕ
	|	Расчеты.КОплатеОстаток - Расчеты.ОплачиваетсяОстаток < 0
	|	И ГрафикПлатежей.ОбъектОплаты ЕСТЬ NULL
	|	И ТИПЗНАЧЕНИЯ(Расчеты.ЗаказПоставщику) НЕ В (
	|		ЗНАЧЕНИЕ(Документ.ВозвратТоваровПоставщику)
	|	)
	|	И Расчеты.ЗаказПоставщику <> НЕОПРЕДЕЛЕНО
	|	И Расчеты.ЗаказПоставщику <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|";
	...
	|	Расчеты.КОплатеОстаток - Расчеты.ОплачиваетсяОстаток > 0
	|	И ГрафикПлатежей.ОбъектОплаты ЕСТЬ NULL
	|	И ТИПЗНАЧЕНИЯ(Расчеты.ЗаказПоставщику) В (
	|		ЗНАЧЕНИЕ(Документ.ВозвратТоваровПоставщику)
	|	)
	|	И Расчеты.ЗаказПоставщику <> НЕОПРЕДЕЛЕНО
	|	И Расчеты.ЗаказПоставщику <> ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Попытка
		ТаблицаОбъектовОплаты = Запрос.Выполнить().Выгрузить();
		УстановитьБлокировкиДанныхДляРасчетаГрафика(ТаблицаОбъектовОплаты, "РегистрНакопления.РасчетыСКлиентами", "ЗаказКлиента");
		РассчитатьГрафикПлатежейПоРасчетамСКлиентами(ТаблицаОбъектовОплаты.ВыгрузитьКолонку("ОбъектОплаты"), Параметры.Очередь);
	...
	Попытка
		ТаблицаОбъектовОплаты = Запрос.Выполнить().Выгрузить();
		УстановитьБлокировкиДанныхДляРасчетаГрафика(ТаблицаОбъектовОплаты, "РегистрНакопления.РасчетыСПоставщиками", "ЗаказПоставщику");
		РассчитатьГрафикПлатежейПоРасчетамСПоставщиками(ТаблицаОбъектовОплаты.ВыгрузитьКолонку("ОбъектОплаты"), Параметры.Очередь);
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
		ТекстСообщения = НСтр("ru = 'Не удалось заполнить график платежей по причине: %Причина%'");
