...
Функция РезультатЗапросаПоНастройкамОтраженияВУчете(МассивОрганизаций, Период) Экспорт
	...
	|			ТОГДА ЛОЖЬ
	|		КОГДА ПрочиеРасходы.СтатьяРасходов.ВариантРаспределенияРасходов В (&ВариантыРаспределенияРасходовНеСписываются)
	|			ТОГДА ЛОЖЬ
	|		КОГДА ПрочиеРасходы.СтатьяРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
	...
	|			ТОГДА ЛОЖЬ
	|		КОГДА ПорядокОтражения.СтатьяРасходов.ВариантРаспределенияРасходов В (&ВариантыРаспределенияРасходовНеСписываются)
	|			ТОГДА ЛОЖЬ
	|		КОГДА ПорядокОтражения.СтатьяРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
...
Функция ВременнаяТаблицаДвиженийСоСчетами(ВременнаяТаблицаДвижений, МассивОрганизаций)
	...
	|			ТОГДА ЛОЖЬ
	|		КОГДА АналитикаУчета.ВариантРаспределенияРасходов В (&ВариантыРаспределенияРасходовНеСписываются)
	|			ТОГДА ЛОЖЬ
	|		КОГДА АналитикаУчета.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты)
...
// Выполняет анализ операций отражения расходов, в зависимости от параметров и дополняет исходную таблицу операций новыми строками.
//	
//	Параметры:
...
// Возвращает настройки отражения статьи расходов в регламентированном учете. Если записей в регистре нет - будут получаться данные по умолчанию из статьи расходов.
//
// Параметры:
...
// переносит данные по настройке счетов производства из регистра ПорядокОтраженияРасходов в регистр ПорядокОтраженияПроизводства.
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	
	ДополнительныеПараметры = ОбновлениеИнформационнойБазы.ДополнительныеПараметрыОтметкиОбработки();
	ДополнительныеПараметры.ЭтоНезависимыйРегистрСведений = Истина;
	ДополнительныеПараметры.ПолноеИмяРегистра = Метаданные.РегистрыСведений.ПорядокОтраженияРасходов.ПолноеИмя();
	...
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияРасходов КАК Данные
	|ГДЕ
	...
	ДополнительныеПараметры.ПолноеИмяРегистра = Метаданные.РегистрыСведений.ПорядокОтраженияРасходов.ПолноеИмя();
	Таблица = Результат.Получить(0).Выгрузить();
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Таблица, ДополнительныеПараметры);
	
	ДополнительныеПараметры.ПолноеИмяРегистра = Метаданные.РегистрыСведений.ПорядокОтраженияПроизводства.ПолноеИмя();
	Таблица = Результат.Получить(1).Выгрузить();
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, Таблица, ДополнительныеПараметры);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	МетаданныеРегистра = Метаданные.РегистрыСведений.ПорядокОтраженияРасходов;
	...
	|	УдаляемыйРегистр.Подразделение,
	|	УдаляемыйРегистр.Подразделение КАК МестоПроизводства,
	|	УдаляемыйРегистр.СтатьяРасходов,
	|	УдаляемыйРегистр.СчетУчета
	|ИЗ
	|	РегистрСведений.ПорядокОтраженияРасходов КАК УдаляемыйРегистр
	...
	|			И УдаляемыйРегистр.Подразделение = ВТИзмеренийРегистра.Подразделение
	|			И УдаляемыйРегистр.СтатьяРасходов = ВТИзмеренийРегистра.СтатьяРасходов";
	#КонецОбласти
	
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуИзмеренийНезависимогоРегистраСведенийДляОбработки(
		Параметры.Очередь, ПолноеИмяРегистра, Запрос.МенеджерВременныхТаблиц, ДополнительныеПараметрыВыборкиДанныхДляОбработки);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировкиПорядокОтраженияРасходов = Блокировка.Добавить(ПолноеИмяРегистра);
			ЭлементБлокировкиПорядокОтраженияРасходов.Режим = РежимБлокировкиДанных.Исключительный;
			ЭлементБлокировкиПорядокОтраженияРасходов.УстановитьЗначение("Организация", Выборка.Организация);
			ЭлементБлокировкиПорядокОтраженияРасходов.УстановитьЗначение("Подразделение", Выборка.Подразделение);
			ЭлементБлокировкиПорядокОтраженияРасходов.УстановитьЗначение("СтатьяРасходов", Выборка.СтатьяРасходов);
			
			ЭлементБлокировкиПорядокОтраженияПроизводства = Блокировка.Добавить("РегистрСведений.ПорядокОтраженияПроизводства");
			ЭлементБлокировкиПорядокОтраженияПроизводства.Режим = РежимБлокировкиДанных.Исключительный;
			ЭлементБлокировкиПорядокОтраженияПроизводства.УстановитьЗначение("Организация", Выборка.Организация);
			ЭлементБлокировкиПорядокОтраженияПроизводства.УстановитьЗначение("МестоПроизводства", Выборка.МестоПроизводства);

			Блокировка.Заблокировать();
			
			НаборЗаписейПорядокОтраженияПроизводства = РегистрыСведений.ПорядокОтраженияПроизводства.СоздатьНаборЗаписей();
			...
			НаборЗаписейПорядокОтраженияРасходов = РегистрыСведений.ПорядокОтраженияРасходов.СоздатьНаборЗаписей();
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось записать данные в регистр %ИмяРегистра%, по организации %Организация% и месту производства %МестоПроизводства%, по причине: %Причина%'");
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ИмяРегистра%", "РегистрСведений.ПорядокОтраженияПроизводства");
