...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	Для Каждого Запись Из ЭтотОбъект Цикл
		...
		Если Не ЗначениеЗаполнено(Запись.СчетУчетаНДСПриЗакупке) Тогда
			...
			// 4D:ERP для Беларуси, Екатерина, 21.10.2015 18:52:52 
			// Локализация плана счетов, №8969
			// {
			Если Запись.СчетУчетаНаСкладе = ПланыСчетов.Хозрасчетный.ОборудованиеКУстановке
				ИЛИ Запись.СчетУчетаНаСкладе = ПланыСчетов.Хозрасчетный.ОборудованиеКУстановкеПереданноеВМонтаж
				ИЛИ Запись.СчетУчетаНаСкладе = ПланыСчетов.Хозрасчетный.СтроительныеМатериалы
				ИЛИ Запись.СчетУчетаНаСкладе = ПланыСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств Тогда
