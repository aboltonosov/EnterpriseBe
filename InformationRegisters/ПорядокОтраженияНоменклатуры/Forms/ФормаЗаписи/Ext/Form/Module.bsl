...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ВозвратЗначенияБезЗаписи Тогда
		...
		СтруктураВозврата = Новый Структура("Организация, Склад, ГруппаФинансовогоУчета, СчетУчетаНаСкладе, СчетУчетаВыручкиОтПродаж,
			|	СчетУчетаСебестоимостиПродаж, СчетУчетаНДСПриПродаже, СтатьяДоходовРегл, СчетУчетаНДСПриЗакупке, СчетУчетаВПути, СчетУчетаНДСВПути");
