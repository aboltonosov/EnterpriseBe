	...
	Пока ВыборкаОснований.Следующий() Цикл
		...
		Пока ДетальнаяВыборка.Следующий() Цикл
			...
			Если ДетальнаяВыборка.Требуется Тогда
				...
				// 4D:ERP для Беларуси, Дмитрий, 06.04.2017 10:37:20 
				// Склад, серия и номер БСО для ЭСЧФ
				// {
				Если ДополнительныеСвойства.Свойство("Склад") Тогда
					СтрокаНабора.Склад = ДополнительныеСвойства.Склад;
				КонецЕсли;	
				Если ДополнительныеСвойства.Свойство("СерияИсходящегоДокумента") Тогда
					СтрокаНабора.СерияИсходящегоДокумента = ДополнительныеСвойства.СерияИсходящегоДокумента;
				КонецЕсли;	
				Если ДополнительныеСвойства.Свойство("НомерИсходящегоДокумента") Тогда
					СтрокаНабора.НомерИсходящегоДокумента = ДополнительныеСвойства.НомерИсходящегоДокумента;
				КонецЕсли;	
				// }
				// 4D
