...
Функция ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, Имя, Представление, Экземпляров = 0) Экспорт
	...
	// {
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьРоссийскиеМакеты") Тогда
		...
		Если СписокОтключаемыхФорм.Найти(Имя) <> Неопределено Тогда
			Возврат Неопределено
		КонецЕсли;
		
		//переименование
		ЛокализованноеПредставление = СписокЛокализованныхПредставлений.Получить(Имя);
		Если ЛокализованноеПредставление <> Неопределено Тогда 
			Представление = ЛокализованноеПредставление;
		КонецЕсли;
