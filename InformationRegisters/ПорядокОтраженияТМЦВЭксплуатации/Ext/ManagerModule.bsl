...
Функция ВременнаяТаблицаДвиженийСоСчетами(ВременнаяТаблицаДвижений)
	...
	|		ТОГДА ЕСТЬNULL(ОбщаяНастройка.СчетУчета, ЕСТЬNULL(АналитикаУчета.СчетУчета, &ПустойСчет))
	...
	|		ТОГДА ЕСТЬNULL(ОбщаяНастройка.СчетЗабалансовогоУчета, ЕСТЬNULL(АналитикаУчета.СчетЗабалансовогоУчета, &ПустойСчет))
	|		ИНАЧЕ ПорядокОтражения.СчетЗабалансовогоУчета КОНЕЦ КАК СчетЗабалансовогоУчета,
	|
	|	ЕСТЬNULL(ОбщаяНастройка.СчетУчета, ЕСТЬNULL(АналитикаУчета.СчетУчета, &ПустойСчет)) <> &ПустойСчет
	|		И ЕСТЬNULL(ПорядокОтражения.СчетУчета, &ПустойСчет) = &ПустойСчет
	|		ИЛИ ЕСТЬNULL(ОбщаяНастройка.СчетУчета, ЕСТЬNULL(АналитикаУчета.СчетУчета, &ПустойСчет))
	|		= ЕСТЬNULL(ПорядокОтражения.СчетУчета, &ПустойСчет) КАК СчетУчетаПоУмолчанию,
	|	ЕСТЬNULL(ОбщаяНастройка.СчетЗабалансовогоУчета, ЕСТЬNULL(АналитикаУчета.СчетЗабалансовогоУчета, &ПустойСчет)) <> &ПустойСчет
	|		И ЕСТЬNULL(ПорядокОтражения.СчетЗабалансовогоУчета, &ПустойСчет) = &ПустойСчет
	|		ИЛИ ЕСТЬNULL(ОбщаяНастройка.СчетЗабалансовогоУчета, ЕСТЬNULL(АналитикаУчета.СчетЗабалансовогоУчета, &ПустойСчет))
	|		= ЕСТЬNULL(ПорядокОтражения.СчетЗабалансовогоУчета, &ПустойСчет) КАК СчетЗабалансовогоУчетаПоУмолчанию
	|
	...
	|			И ТаблицаДвижений.КатегорияЭксплуатации = ОбщаяНастройка.КатегорияЭксплуатации
	|			И ТаблицаДвижений.КатегорияЭксплуатации = ЗНАЧЕНИЕ(Справочник.КатегорииЭксплуатации.ПустаяСсылка)";
...
// Выполняет анализ операций отражения ТМЦ в эксплуатации, в зависимости от параметров и дополняет исходную таблицу операций новыми строками.
//	
//	Параметры:
