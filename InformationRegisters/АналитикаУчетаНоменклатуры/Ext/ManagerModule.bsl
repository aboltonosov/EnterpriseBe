...
Функция ПараметрыЗаполненияКлючейАналитики() Экспорт
	Возврат Новый Структура("ИзмененаАналитика", Ложь);
КонецФункции
...
Процедура ЗаполнитьВКоллекции(Коллекция, МестаУчета, ИменаПолей = Неопределено, ДополнитьКлюч = Ложь, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ИменаПолей.Назначение) И ТипЗнч(ИменаПолей.Назначение) = Тип("Структура") Тогда
...
Функция ПолучитьНаборЗаписей(ПараметрыАналитики)
	...
	Иначе
		...
		ПроверитьЗаполнениеПоляСерия(НоваяСтрока);
...
Процедура ПроверитьЗаполнениеПоляСерия(МенеджерЗаписи)

	Если ТипЗнч(МенеджерЗаписи.Склад) = Тип("СправочникСсылка.Партнеры")
	 И ЗначениеЗаполнено(МенеджерЗаписи.Серия) Тогда
		ВызватьИсключение НСтр("ru = 'Параметр ""Серия"" должен быть не заполнен'");
	КонецЕсли;
...
Функция ТекстДополнениеКлючейАналитикиВКоллекции(ИменаПолей)
	ТекстЗапроса = "
		...
		|		И Ключи.Назначение = СпрВидыЗапасов.УдалитьНазначение
