...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаТаблицаПланыЗакупок(Запрос, ТекстыЗапроса, Регистры, ДокументСсылка);
...
Функция ТекстЗапросаТаблицаПланыЗакупок(Запрос, ТекстыЗапроса, Регистры, ДокументСсылка)
	...
	Если Замещающий и СтатусЗамещения <> Неопределено Тогда
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПланЗакупокТовары.Ссылка КАК ПланЗакупок,
		|		ПланЗакупокТовары.НомерСтроки КАК НомерСтроки,
		|		ВЫБОР
		|			КОГДА ПланЗакупокТовары.ДатаПоступления = ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА &НачалоПериода
		...
		|			ИНАЧЕ ПланЗакупокТовары.Назначение
		...
		|			ИНАЧЕ ПланЗакупокТовары.Склад
		|		КОНЕЦ КАК Склад,
		|		ПланЗакупокТовары.Партнер КАК Партнер,
		|		ПланЗакупокТовары.Соглашение КАК Соглашение,
		|		&СтатусЗамещения КАК Статус,
		|		ВЫБОР
		|			КОГДА ПланЗакупокТовары.Отменено
		|				ТОГДА 0
		|			ИНАЧЕ ПланЗакупокТовары.Количество
		|		КОНЕЦ КАК Количество,
		|		ВЫБОР
		|			КОГДА ПланЗакупокТовары.Отменено
		|				ТОГДА 0
		|			ИНАЧЕ ПланЗакупокТовары.Сумма
		|		КОНЕЦ КАК Сумма
		|	ИЗ
		|		Документ.ПланЗакупок.Товары КАК ПланЗакупокТовары
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
		|			ПО ПланЗакупокТовары.Номенклатура = СпрНоменклатура.Ссылка
		|	ГДЕ
		...
		|		-ПланыЗакупокОбороты.СуммаОборот
		|	ИЗ
		...
		|					И ПланЗакупок <> &Ссылка) КАК ПланыЗакупокОбороты) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.ПланЗакупок,
		|	ВложенныйЗапрос.Период,
