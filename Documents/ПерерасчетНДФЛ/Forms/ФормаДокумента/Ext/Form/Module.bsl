...
Функция ДанныеПоНДФЛ(Сотрудники = Неопределено)
	...
	ТекущиеОстаткиВзаиморасчетов = ВзаиморасчетыССотрудниками.ОстаткиВзаиморасчетов(Объект.Организация, НачалоМесяца(Объект.МесяцПерерасчета), ИсчисленныйНалогФизическиеЛица, Объект.Ссылка);
	...
	СтрокаГруппировки = "ФизическоеЛицо, МесяцНалоговогоПериода, Подразделение, КодВычетаЛичный";
...
Процедура ОбработкаПодбораНаСервере(Сотрудники)
	...
	Для Каждого Сотрудник Из Сотрудники Цикл
		...
		СтрокиНДФЛП = РезультатРасчетаНДФЛ.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник));
