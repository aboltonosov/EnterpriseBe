...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// 4D:ERP для Беларуси, Яна, 08.09.2017 11:22:32 
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ УдержанияПерерасчет
...
Процедура НДФЛПриАктивизацииСтроки(Элемент)
	...
	// 4D:ERP для Беларуси, АлексейЧ, 15.02.2018 10:32:52 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
...
Процедура ПересчитатьНДФЛ(Команда)
	...
	Если Не РасчетЗарплатыРасширенныйКлиентСервер.ФормаДокументаГотоваДляРасчетаЗарплаты(ЭтаФорма, ОписаниеДокумента(ЭтотОбъект)) Тогда
		Возврат;
	КонецЕсли;
	// 4D:ERP для Беларуси, АлексейЧ, 14.02.2018 9:46:21 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
		Сотрудники = СотрудникиДокумента();
		СотрудникиДокумента = УдалитьПовторяющиесяЭлементыМассива(Сотрудники);
 
		РежимПересчетаНДФЛ = Истина;
		ПерезаполнитьДанныеФормыНаКлиенте(СотрудникиДокумента, );
...
Процедура ПриПолученииДанныхНаСервере(ТекущийОбъект, ДополнятьФорму = Истина)
	...
	// 4D:ERP для Беларуси, Яна, 08.09.2017 11:09:36 
	// Локализовать документ "Начисление зарплаты и взносов", №15928
	// {
	...
	Элементы.ГруппаПерерасчетПоУходуЗаПервымРебенком.Видимость = Ложь;
	Элементы.ГруппаПерерасчетПоХодуЗаПоследующимиДетьми.Видимость = Ложь;
	Элементы.ГруппаПерерасчетКоличествоДетей.Видимость = Ложь;
	Элементы.ГруппаПерерасчетДанныеДляРасчетаПособия.Видимость = Ложь;
	// }
	// 4D
...
Процедура ЗавершитьИнициализациюФормы(ТекущийОбъект, ДополнятьФорму, НовыйДокумент = Истина)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			"ИтогУдержано",
			"Видимость",
			// 4D:ERP для Беларуси, АлексейЧ, 21.12.2017 13:40:34 
			// Документ "Начисление зарплаты и взносов", №16993
			// {
			//ИспользоватьУдержания Или ИспользоватьНДФЛ);
			ИспользоватьУдержания Или ИспользоватьНДФЛ Или ИспользоватьВзносы);
			// }
			// 4D
...
Процедура ВзносыПриОкончанииРедактированияНаСервере()
	...
	// 4D:ERP для Беларуси, АлексейЧ, 20.12.2017 17:20:32 
	// Документ "Начисление зарплаты и взносов", №16993
	// {
	РассчитатьИтогУдержано(ЭтаФорма);	
	// }
...
Процедура ВзносыПослеУдаленияНаСервере()
	...
	// 4D:ERP для Беларуси, АлексейЧ, 20.12.2017 17:20:17 
	// Документ "Начисление зарплаты и взносов", №16993
	// {
	РассчитатьИтогУдержано(ЭтаФорма);	
	// }
...
Процедура СортироватьРезультатРасчетаНачислений(ДанныеЗаполнения, ИспользоватьСортировкуПоУмолчанию = Истина)	
	...
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "Начисление.ОчередностьРасчета", МетаданныеРеквизитов.Начисление.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "Начисление.РеквизитДопУпорядочивания", МетаданныеРеквизитов.Начисление.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "ДокументОснование", МетаданныеРеквизитов.ДокументОснование.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "ДатаНачала", МетаданныеРеквизитов.ДатаНачала.Тип);	
...
Процедура СортироватьРезультатыРасчетаПоФизическимЛицам(ДанныеЗаполнения)	
	...
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "Удержание.ОчередностьРасчета", МетаданныеРеквизитов.Удержание.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "Удержание", МетаданныеРеквизитов.Удержание.Тип);
	...
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "МесяцНалоговогоПериода", МетаданныеРеквизитов.МесяцНалоговогоПериода.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "Подразделение", МетаданныеРеквизитов.Подразделение.Тип);
	...
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "МесяцНалоговогоПериода", МетаданныеРеквизитов.ДатаПолученияДохода.Тип);
	...
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "ФизическоеЛицо", МетаданныеРеквизитов.ФизическоеЛицо.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "ДоговорЗайма", МетаданныеРеквизитов.ДоговорЗайма.Тип);
	СотрудникиФормыРасширенный.ДобавитьДополнительноеПоллеСортировки(Коллекция, "ДатаНачала", Новый ОписаниеТипов("Дата"));
...
// 4D:ERP для Беларуси, АлексейЧ, 14.02.2018 12:29:05 
// Локализовать документ "Начисление зарплаты и взносов", № 15928
// {
...
Процедура СоздатьВТРаспределениеРезультатовНачислений(МенеджерВременныхТаблиц, ТаблицаРаспределенияНачислений)
	...
	|	РаспределениеНачислений.Результат,
...
Функция РасчетныйЛистокНаСервереБезКонтекста(ДокументСсылка, СписокФизическихЛиц)
	...
	Возврат УчетНачисленнойЗарплаты.РасчетныйЛистокПодробнее(ДокументОбъект, Ложь, СписокФизическихЛиц, ДокументОбъект.МесяцНачисления);
...
Функция РасчетныйЛистокНаСервере(ИдентификаторыВыделенныхСтрок, ИмяТабличнойЧасти, СписокСотрудников)
	...
	ДокументОбъект.ДополнительныеСвойства.Вставить("ВидыУчетов", "Начисления,УчетНачисленнойЗарплаты,ИсчисленныеСтраховыеВзносы,ОстальныеВидыУчета");
	...
	Возврат УчетНачисленнойЗарплаты.РасчетныйЛистокПодробнее(ДокументОбъект, Истина, МассивФизическихЛиц, Объект.МесяцНачисления, ИсходныйДокументОбъект);
	...
	Для Каждого СтрокаТаблицы Из Объект[ОписаниеТаблицы.ИмяТаблицы] Цикл 
		Если МассивСотрудников.Найти(СтрокаТаблицы[ОписаниеТаблицы.ИмяРеквизитаСотрудник]) <> Неопределено Тогда 
			...
			Если ОписаниеТаблицы.ИмяТаблицы = "НДФЛ" Тогда
				...
				СтрокиВычетов = Объект.ПримененныеВычетыНаДетейИИмущественные.НайтиСтроки(Отбор);
				Для Каждого СтрокаВычета Из СтрокиВычетов Цикл 
					ЗаполнитьЗначенияСвойств(ТекущийОбъект.ПримененныеВычетыНаДетейИИмущественные.Добавить(), СтрокаВычета);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура РассчитатьИтогУдержано(Форма)
	...
	// 4D:ERP для Беларуси, АлексейЧ, 20.12.2017 16:36:43 
	// Документ "Начисление зарплаты и взносов", №16993
	// {
		ЗарплатаКадрыКлиентСервер.ИтогиТабличныхЧастейОписаниеСуммируемогоРеквизита("Налог", НСтр("ru='Подоходный налог'")));
	// }
	// 4D
	...
	// Документ "Начисление зарплаты и взносов", №16993
	// {
	ОписанияСоставляющихПФ = Новый Массив;
	ОписанияСоставляющихПФ.Добавить(
		ЗарплатаКадрыКлиентСервер.ИтогиТабличныхЧастейОписаниеСуммируемогоРеквизита("ПенсионныйФонд", НСтр("ru='Отчисления в Пенсионный фонд'")));
	// }
	// 4D
	...
	// {
	ОписанияСоставляющих.Добавить(
		ЗарплатаКадрыКлиентСервер.ИтогиТабличныхЧастейОписаниеТабличнойЧасти("Объект.Взносы", ОписанияСоставляющихПФ));
	// }
	// 4D
