...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	// {
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИнвентаризацияБСО") Тогда
		ИнициализироватьДокумент(ДанныеЗаполнения);
		
		Организация = ДанныеЗаполнения.Организация;
		Подразделение = ДанныеЗаполнения.Подразделение;
		РежимИспользованияБСО = Истина;
		
		ТЗПриход = УчетБланковСтрогойОтчетностиСервер.СтруктураСлужебнойТаблицыБСО();
		
		Для Каждого ТекСтрокаТовары Из ДанныеЗаполнения.Товары Цикл
			НомераПоУчету = Новый Массив;
			НомераФакт = Новый Массив;
			
			НомерМин = ?(ПустаяСтрока(ТекСтрокаТовары.НомерС), 0, Число(ТекСтрокаТовары.НомерС));
			НомерМакс = ?(ПустаяСтрока(ТекСтрокаТовары.НомерПо), 0, Число(ТекСтрокаТовары.НомерПо));
			НомерМинФакт = ?(ПустаяСтрока(ТекСтрокаТовары.НомерСФакт), 0, Число(ТекСтрокаТовары.НомерСФакт));
			НомерМаксФакт = ?(ПустаяСтрока(ТекСтрокаТовары.НомерПоФакт), 0, Число(ТекСтрокаТовары.НомерПоФакт));
			
			Для К = НомерМин По НомерМакс Цикл
				Если К <> 0 Тогда
					НомераПоУчету.Добавить(К);
				КонецЕсли;
			КонецЦикла;
			
			Для К = НомерМинФакт По НомерМаксФакт Цикл
				Если К <> 0 Тогда
					НомераФакт.Добавить(К);
				КонецЕсли;
			КонецЦикла;
			
			Для каждого Стр Из НомераПоУчету Цикл
				ИндексФакт = НомераФакт.Найти(Стр);
				Если ИндексФакт <> Неопределено Тогда
					НомераФакт.Удалить(ИндексФакт);
				КонецЕсли;
			КонецЦикла;
			
			Для каждого Стр Из НомераФакт Цикл
				ДлинаНомера = Макс(СтрДлина(ТекСтрокаТовары.НомерС), СтрДлина(ТекСтрокаТовары.НомерПо));
				НомерБланкаСтр = УчетБланковСтрогойОтчетностиКлиентСервер.ОтформатироватьНомерБланка(Стр, ДлинаНомера);
				
				НоваяСтрока = ТЗПриход.Добавить();
				...
				НоваяСтрока.Номер = НомерБланкаСтр;
				НоваяСтрока.Количество = 1;
				НоваяСтрока.КоличествоУпаковок = 1;
				НоваяСтрока.НомерБланка = Стр;
				НоваяСтрока.ДлинаНомера = ДлинаНомера;
				НоваяСтрока.НачальныйНомерБСО = НомерБланкаСтр;
				НоваяСтрока.КонечныйНомерБСО = НомерБланкаСтр;
			КонецЦикла;
		КонецЦикла;
		
		ТЗПриход = УчетБланковСтрогойОтчетностиСервер.СвернутьТаблицуБСО(ТЗПриход, Ложь);
		...
		Для каждого Стр Из ТЗПриход Цикл
			...
			НоваяСтрока.НачальныйНомерБСО = Стр.НачальныйНомерБСО;
			НоваяСтрока.КонечныйНомерБСО = Стр.КонечныйНомерБСО;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	// 4D:ERP для Беларуси, Петр, 14.02.2018 16:20:14 
