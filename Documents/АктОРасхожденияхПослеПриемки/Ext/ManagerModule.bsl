...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	ПостфиксИмениДокумента = ?(Данные.ТипОснованияАктаОРасхождении = Перечисления.ТипыОснованияАктаОРасхождении.ПоступлениеТоваровУслуг,
	                           НСтр("ru = 'после поступления'"),
	                           НСтр("ru = 'после возврата от клиента'"));
...
Функция ТекстЗапросаВтТовары(Запрос, ТекстыЗапроса)
	...
	|		ИЛИ ВЫБОР КОГДА ЕСТЬNULL(ТаблицаТовары.Ссылка.Соглашение.ВариантПриемкиТоваров,ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка))
	|							= ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|					ТОГДА ВариантПриемкиТоваровКонстанта.Значение
	|				ИНАЧЕ ЕСТЬNULL(ТаблицаТовары.Ссылка.Соглашение.ВариантПриемкиТоваров,ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка))
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Константа.ВариантПриемкиТоваров КАК ВариантПриемкиТоваровКонстанта
	|		ПО ИСТИНА
	|ГДЕ
...
Функция ТекстЗапросаТаблицаДвижениеТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|		КОГДА ВЫБОР КОГДА ЕСТЬNULL(ТаблицаТовары.Ссылка.Соглашение.ВариантПриемкиТоваров,ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка))
	|							= ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка)
	|					ТОГДА ВариантПриемкиТоваровКонстанта.Значение
	|				ИНАЧЕ ЕСТЬNULL(ТаблицаТовары.Ссылка.Соглашение.ВариантПриемкиТоваров,ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.ПустаяСсылка))
	|			КОНЕЦ = ЗНАЧЕНИЕ(Перечисление.ВариантыПриемкиТоваров.РазделенаТолькоПоНакладным)
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ПО АктОРасхожденияхПослеПриемки.Ссылка = ДанныеОснований.Ссылка
	|ГДЕ
	|	АктОРасхожденияхПослеПриемки.Ссылка = &Ссылка";
...
Функция ТекстЗапросаВтТаблицаДанныхОснований(Запрос, ТекстыЗапроса)
	...
	|		ТаблицаОснований.Склад КАК МестоХранения
	|	ИЗ
	|		Документ.АктОРасхожденияхПослеПриемки.Товары КАК ТаблицаОснований
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	ДобавитьКомандуТОРГ1(КомандыПечати, "ПодменюПечатьВозвраты");
	ДобавитьКомандуТОРГ2(КомандыПечати, "ПодменюПечатьВозвраты");
	ДобавитьКомандуТОРГ3(КомандыПечати, "ПодменюПечатьВозвраты");
