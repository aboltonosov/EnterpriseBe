...
// Регистрирует данные для обработчика обновления УП 2.2.2:
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ПоручениеЭкспедитору КАК ПоручениеЭкспедитору
	|ГДЕ
	|	(ПоручениеЭкспедитору.Удалить2ДатаВыполнения <> &ПустаяДата
	|				И ПоручениеЭкспедитору.Удалить2ДатаВыполнения <> ПоручениеЭкспедитору.ДатаВыполнения
	|			ИЛИ ПоручениеЭкспедитору.Удалить1ДатаВыполнения <> &ПустаяДата
	|				И ПоручениеЭкспедитору.Удалить1ДатаВыполнения <> ПоручениеЭкспедитору.ДатаВыполнения)
	|	И ПоручениеЭкспедитору.ДатаВыполнения = &ПустаяДата
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяДокумента = "Документ.ПоручениеЭкспедитору";
	МетаданныеДокумента = Метаданные.Документы.ПоручениеЭкспедитору;
		
	МенеджерВТ = Новый МенеджерВременныхТаблиц;
	
	Результат = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуСсылокДляОбработки(Параметры.Очередь, ПолноеИмяДокумента, МенеджерВТ);
	...
	"ВЫБРАТЬ
	|	ПоручениеЭкспедитору.Ссылка КАК Ссылка,
	|	ПоручениеЭкспедитору.ВерсияДанных КАК ВерсияДанных
	|ИЗ
	|	Документ.ПоручениеЭкспедитору КАК ПоручениеЭкспедитору
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДляОбработки КАК ДокументыКОбработке
	|		ПО ПоручениеЭкспедитору.Ссылка = ДокументыКОбработке.Ссылка
	|ГДЕ
	|	(ПоручениеЭкспедитору.Удалить2ДатаВыполнения <> &ПустаяДата
	|				И ПоручениеЭкспедитору.Удалить2ДатаВыполнения <> ПоручениеЭкспедитору.ДатаВыполнения
	|			ИЛИ ПоручениеЭкспедитору.Удалить1ДатаВыполнения <> &ПустаяДата
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(ДокОбъект.ДатаВыполнения) Тогда  
			Если ЗначениеЗаполнено(ДокОбъект.Удалить2ДатаВыполнения) 
				И ДокОбъект.Удалить2ДатаВыполнения <> ДокОбъект.ДатаВыполнения Тогда
				ДокОбъект.ДатаВыполнения = ДокОбъект.Удалить2ДатаВыполнения;
				ЕстьИзменения = Истина;
			ИначеЕсли ЗначениеЗаполнено(ДокОбъект.Удалить1ДатаВыполнения) 
				И ДокОбъект.Удалить1ДатаВыполнения <> ДокОбъект.ДатаВыполнения Тогда 
				ДокОбъект.ДатаВыполнения = ДокОбъект.Удалить1ДатаВыполнения;
				ЕстьИзменения = Истина;
			КонецЕсли;
		КонецЕсли;
