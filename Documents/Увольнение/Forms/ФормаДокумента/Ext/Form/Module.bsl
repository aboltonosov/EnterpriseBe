...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПроведениеДокументаУвольнение");
	КонецЕсли;
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ПримененныеВычетыКДоходам
...
////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ ПримененныеВычетыИИмущественные (на детей и имущественные).
...
Процедура ОткрытьСреднийЗаработок(Команда)	
	...
	// 4D:ERP для Беларуси, Яна, 01.09.2017 16:30:48 
	...
	УчетСреднегоЗаработкаКлиент.ОткрытьФормуВводаСреднегоЗаработкаЛокализация(ПараметрыРедактированияСреднегоЗаработка(ЭтоСреднечасовойЗаработок(), Ложь), ЭтаФорма, Оповещение);
	// }
	// 4D
...
Функция ПараметрыРедактированияСреднегоЗаработка(ЭтоСреднечасовойЗаработок, ЭтоВыходноеПособие)	
	...
	// 4D:ERP для Беларуси,Яна, 01.09.2017 16:32:33 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	УчетСреднегоЗаработка.ЗаполнитьТаблицыДанныхСреднегоЗаработкаОтпускПоДокументу(Объект, ПараметрыРедактирования);	
	// }
	// 4D
...
Процедура НачисленияПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.НачисленияПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыНачислений(РегистрацияНачисленийДоступна));
КонецПроцедуры
...
Процедура ПособияПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.ПособияПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыПособия(РегистрацияНачисленийДоступна));
КонецПроцедуры
...
Процедура ПособияПерерасчетПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.ПособияПерерасчетПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыПособияПерерасчеты(РегистрацияНачисленийДоступна));
КонецПроцедуры
...
Процедура ЛьготыПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.ЛьготыПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыЛьгот(РегистрацияНачисленийДоступна));
...
Процедура УдержанияПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.УдержанияПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыУдержаний(РегистрацияНачисленийДоступна));
КонецПроцедуры
...
Процедура ПерерасчетПодробно(Команда)
	...
	РасчетЗарплатыРасширенныйВызовСервера.СохранитьНастройкуРежимаОтображенияПодробно(Объект.Ссылка, Элементы.НачисленияПерерасчетПодробно.Пометка, КадровыйУчетРасширенныйКлиентСервер.ОписаниеТаблицыПерерасчетов(РегистрацияНачисленийДоступна));
...
Процедура ПриПолученииДанныхНаСервере()
	...
	// 4D:ERP для Беларуси, АлексейЧ, 02.01.2018 13:40:34 
	// Ошибка при работе с документом "Увольнение" под профилем "Кадровик (без расчета зарплаты), №17234
	// Добавляем проверку на наличие элемента формы формы документа, 
	// в связи с тем, что, в зависимости от роли текущего пользователя элементы формы
	// могут присутствовать, или отсутствовать.
	// {
	
	// 4D:ERP для Беларуси, Яна, 07.09.2017 16:13:20 
	// Локализация документа "Увольнение ", №15180 
	// {
	
	Если ЭтаФорма.Элементы.Найти("ПособияВТомЧислеЗаСчетФБ") <> Неопределено Тогда
		Элементы.ПособияВТомЧислеЗаСчетФБ.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ГруппаПоУходуЗаПервымРебенком") <> Неопределено Тогда
		Элементы.ГруппаПоУходуЗаПервымРебенком.Видимость = Ложь;
	КонецЕсли;

	Если ЭтаФорма.Элементы.Найти("ГруппаПоХодуЗаПоследующимиДетьми") <> Неопределено Тогда
		Элементы.ГруппаПоХодуЗаПоследующимиДетьми.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ГруппаКоличествоДетей") <> Неопределено Тогда
		Элементы.ГруппаКоличествоДетей.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ГруппаДанныеДляРасчетаПособия") <> Неопределено Тогда
		Элементы.ГруппаДанныеДляРасчетаПособия.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ПособияПерерасчетРезультатВТомЧислеЗаСчетФБ") <> Неопределено Тогда
		Элементы.ПособияПерерасчетРезультатВТомЧислеЗаСчетФБ.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ПособияПерерасчетГруппаПоУходуЗаПервымРебенком") <> Неопределено Тогда
		Элементы.ПособияПерерасчетГруппаПоУходуЗаПервымРебенком.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ПособияПерерасчетГруппаПоХодуЗаПоследующимиДетьми") <> Неопределено Тогда
		Элементы.ПособияПерерасчетГруппаПоХодуЗаПоследующимиДетьми.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ПособияПерерасчетГруппаКоличествоДетей") <> Неопределено Тогда
		Элементы.ПособияПерерасчетГруппаКоличествоДетей.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ПособияПерерасчетГруппаДанныеДляРасчетаПособия") <> Неопределено Тогда
		Элементы.ПособияПерерасчетГруппаДанныеДляРасчетаПособия.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыОПС") <> Неопределено Тогда
		Элементы.ВзносыОПС.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРДоПредельнойВеличины") <> Неопределено Тогда
		Элементы.ВзносыПФРДоПредельнойВеличины.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРСПревышения") <> Неопределено Тогда
		Элементы.ВзносыПФРСПревышения.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРПоСуммарномуТарифу") <> Неопределено Тогда
		Элементы.ВзносыПФРПоСуммарномуТарифу.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРНакопительная") <> Неопределено Тогда
		Элементы.ВзносыПФРНакопительная.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРСтраховая") <> Неопределено Тогда
		Элементы.ВзносыПФРСтраховая.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРДоПредельнойВеличиныЕНВД") <> Неопределено Тогда
		Элементы.ВзносыПФРДоПредельнойВеличиныЕНВД.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРСПревышенияЕНВД") <> Неопределено Тогда
		Элементы.ВзносыПФРСПревышенияЕНВД.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРПоСуммарномуТарифуЕНВД") <> Неопределено Тогда
		Элементы.ВзносыПФРПоСуммарномуТарифуЕНВД.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРНакопительнаяЕНВД") <> Неопределено Тогда
		Элементы.ВзносыПФРНакопительнаяЕНВД.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыПФРСтраховаяЕНВД") <> Неопределено Тогда
		Элементы.ВзносыПФРСтраховаяЕНВД.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыФСС") <> Неопределено Тогда
		Элементы.ВзносыФСС.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыФССНесчастныеСлучаи") <> Неопределено Тогда
		Элементы.ВзносыФССНесчастныеСлучаи.Видимость = Ложь;
	КонецЕсли;
	
	Если ЭтаФорма.Элементы.Найти("ВзносыФФОМС") <> Неопределено Тогда
		Элементы.ВзносыФФОМС.Видимость = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьВидыРасчетовПоВидамОтпусков()
	...
	Иначе
		...
		Для каждого ДополнительныйОтпуск Из Объект.ДополнительныеОтпуска Цикл
			...
			И Не (СпособУдержанияИзлишнеНачисленныхОтпускных = ПредопределенноеЗначение("Перечисление.СпособыУдержанияИзлишнеНачисленныхОтпускных.СторнированиеРанееНачисленныхСумм")
			И ДополнительныйОтпуск.ПризнакКомпенсацииУдержания = ПредопределенноеЗначение("Перечисление.КомпенсацияУдержаниеОтпускаПриУвольнении.УдержатьЗаИспользованныеАвансом"))
...
Процедура РассчитатьСотрудникаНаСервере(Сотрудник, ОписаниеТаблицы = Неопределено)
	...
	МенеджерРасчета.ДобавитьДатуЗавершенияРаботы(Объект.Сотрудник, Объект.ДатаУвольнения);
...
Процедура ЗаполнитьНаСервере()
	...
	ЗаполнитьУдержания(ТаблицаУдержаний, МенеджерРасчета);
	МенеджерРасчета.ЗаполнитьУдержанияСотрудникаЗаПериод(Объект.Сотрудник, Объект.ПериодРегистрации, КонецМесяца(Объект.ПериодРегистрации), ТаблицаУдержаний);
	...
	ЗаполнитьНастройкиМенеджераРасчета(МенеджерРасчета);
	МенеджерРасчета.НастройкиБухучета.КоэффициентыСреднегоЗаработкаДокумента = УчетСреднегоЗаработка.КоэффициентыРаспределенияСреднегоЗаработкаДокумента(Объект, КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
	МенеджерРасчета.РассчитатьЗарплату();
...
&НаСервере
Процедура ЗаполнитьУдержания(ТаблицаУдержаний, МенеджерРасчета)
...
Процедура СформироватьЗаписиПерерасчета(МенеджерРасчета)
	...
	Если ЗначениеЗаполнено(СсылкаНаИсправленныйДокумент) Тогда
		...
		ИсправлениеДокументовРасчетЗарплаты.СформироватьДанныеПерерасчетаДляИсправления(Объект.Организация, Объект.ПериодРегистрации, СсылкаНаИсправленныйДокумент, Объект.Начисления, Объект.НачисленияПерерасчет, СсылкаНаОбъект, ОписаниеТаблицы, ПолучитьКонтролируемыеПоля(), КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)), СотрудникДляИсправления);
	...
	Иначе
		ИсправлениеДокументовРасчетЗарплаты.СформироватьДанныеПерерасчета(Объект.Организация, Объект.ПериодРегистрации, Объект.Начисления, Объект.НачисленияПерерасчет, СсылкаНаОбъект, ПолучитьКонтролируемыеПоля(), КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
	КонецЕсли;
...
Процедура ЗаполнитьОтрицательныеНачисленияЗаОтпуск(ТаблицаНачислений, МенеджерРасчета)
	...
	НачисленныеОтпуска = УчетНачисленнойЗарплатыРасширенный.НачисленныеОтпускаПоПериодамИВидам(Объект.Сотрудник, МассивВидыОтпусков, СсылкаНаОбъект);
...
Процедура ДанныеВРеквизиты()
	...
	ПрочитатьРеквизитыИсправления(Не ЭтаФорма.Параметры.Свойство("Ключ") Или ЭтаФорма.Параметры.Ключ.Пустая());
	Если Не РедактированиеСтрокиСписочногоДокумента Тогда
...
// 4D:ERP для Беларуси, Яна, 01.09.2017 16:33:34 
// Ошибка при вводе данных для расчета среднего заработка, №15832
// {
&НаКлиентеНаСервереБезКонтекста
...
Процедура РассчитатьСохраняемоеДенежноеСодержание()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
		...
		Модуль.РассчитатьСохраняемоеДенежноеСодержаниеДокумента(ЭтаФорма, КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
	КонецЕсли;
...
Процедура ОбновитьДанныеДляРасчетаСохраняемогоДенежногоСодержания()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
		Модуль.ОбновитьДанныеСохраняемогоДенежногоСодержанияДокумента(ЭтаФорма, КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
	КонецЕсли;
...
Процедура ОбновитьФормуПоВидуДоговора()
	...
	Если ИспользоватьГосударственнуюМуниципальнуюСлужбу Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
			Модуль = ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба");
			ВидыДоговоровГосударственнойСлужбы = Модуль.ВидыДоговоровГосударственнойСлужбы();
			ЭтоГосслужащий = ВидыДоговоровГосударственнойСлужбы.Найти(Объект.ВидДоговора) <> Неопределено;
		КонецЕсли;
...
Процедура ЗагрузитьНастройки()
	...
	Если РегистрацияНачисленийДоступна Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
			Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплата");
			Модуль.ПриЗавершенииИнициализацииФормыДокументаОплатыТруда(ЭтаФорма, КадровыйУчетРасширенныйКлиентСервер.ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьПересечениеПериодовОтсутствияНаСервере()
	...
	НоваяСтрока.Начало = Объект.ДатаУвольнения + 86400;
