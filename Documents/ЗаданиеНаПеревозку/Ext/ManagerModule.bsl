...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТТН")
		...
		// 4D:ERP для Беларуси, Дмитрий, 09.02.2016 11:54:57 
		// Д.Товарно-транспортная накладная (задача №10783)
		// {
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 09.02.2016 14:24:57 
	// Д.Товарно-транспортная накладная (задача №10783)
	// {
	...
	КонецЕсли;
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
				КоллекцияПечатныхФорм,
				"ТТН_ГоризонтальнаяСПриложением",
				НСтр("ru = 'Товарно-транспортная накладная'"),
				Обработки.ПечатьТТН.СформироватьПечатнуюФормуТТН(
					СтруктураРаспоряжения.Распоряжения,
					ОбъектыПечати,
...
Функция СформироватьПечатнуюФормуМаршрутныйЛист(МассивОбъектов, ОбъектыПечати)
	...
	|	ЗаданиеНаПеревозкуМаршрут.Ссылка КАК Ссылка,
	|	ВЫБОР
	|		КОГДА ЗаданиеНаПеревозкуРаспоряжения.Распоряжение ССЫЛКА Документ.ПоручениеЭкспедитору
	|				И ВЫРАЗИТЬ(ЗаданиеНаПеревозкуРаспоряжения.Распоряжение КАК Документ.ПоручениеЭкспедитору).СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуНаСклад)
	|			ТОГДА ЗаданиеНаПеревозкуРаспоряжения.Склад
	|		ИНАЧЕ ЗаданиеНаПеревозкуРаспоряжения.ПолучательОтправитель
...
Процедура ОбъединитьОбластьСоСтрокамиНиже(ТабличныйДокумент, ИмяОбласти, КоличествоДопСтрок)
	...
	// 4D:ERP для Беларуси, ВладимирР, 24.09.2017 13:53:49 
	// Обновление НПС 2.1.3.266 и 2.2.3.138, №15869
	// {
	Область = ТабличныйДокумент.Область(СтрЗаменить(НовыйАдрес, Символ(160), ""));
	// }
	// 4D
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ЗаданиеНаПеревозку КАК ЗаданиеНаПеревозку
	|ГДЕ
	|	ЗаданиеНаПеревозку.УдалитьКонтрагент <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)";
	Запрос = Новый Запрос(ТекстЗапроса);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДокументОбъект.Контрагент = ДокументОбъект.УдалитьКонтрагент;
		ДокументОбъект.УдалитьКонтрагент = Справочники.Контрагенты.ПустаяСсылка();
		
		Попытка
			
			ОбновлениеИнформационнойБазы.ЗаписатьОбъект(ДокументОбъект, Истина, Ложь, РежимЗаписиДокумента.Запись);
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru = 'Не удалось записать данные в документ %ИмяДокумента% по причине: %Причина%'");
			...
			ЗаписьЖурналаРегистрации(
				...
				Метаданные.Документы.ЗаданиеНаПеревозку,
				Неопределено,
				ТекстСообщения);
