...
Функция ПолучитьДанныеДляПечатнойФормыИНВ3(ПараметрыПечати, ДокументОснование) Экспорт
	...
	Если ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныйРасчет Тогда
			...
			|	И ПорчаТоваровТоварыОприходованные.Ссылка.Организация = &Организация
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		+ ЦеныНоменклатуры.ПостатейныеРегл) КАК СтоимостьРегл
		...
		Иначе
			...
			|				+ СебестоимостьТоваровОстатки.ПостатейныеРеглОстаток)
			...
			|		+ СтоимостьТоваровСрезПоследних.ПостатейныеРегл) КАК Цена
...
Функция ПолучитьДанныеДляПечатнойФормыИНВ19(ПараметрыПечати, ДокументОснование) Экспорт
	...
	Если ПредварительныеДанныеРезультат.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныеДанныеРезультат.ПредварительныйРасчет Тогда
			...
			|
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Ключи
			|		ПО Ключи.КлючАналитики = ВидыЗапасов.АналитикаУчетаНоменклатуры
			|
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
			|		ПО ВидыЗапасов.НоменклатураОприходование = Аналитика.Номенклатура
			|			И ВидыЗапасов.ХарактеристикаОприходование = Аналитика.Характеристика
			//++ НЕ УТ 
			|		И ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = Аналитика.СтатьяКалькуляции
			//-- НЕ УТ
			|			И ВидыЗапасов.Серия = Аналитика.Серия
			|			И Ключи.Назначение = Аналитика.Назначение
			|			И ВидыЗапасов.Ссылка.Склад = Аналитика.Склад
			|ГДЕ
			|	ВидыЗапасов.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|	И ВидыЗапасов.Ссылка.Склад = &Склад
			|	И ВидыЗапасов.Ссылка.Организация = &Организация
		...
		Иначе
			...
			|	
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Ключи
			|		ПО ПорчаТоваровВидыЗапасов.АналитикаУчетаНоменклатуры = Ключи.КлючАналитики
			|
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АналитикаУчетаНоменклатуры КАК Аналитика
			|		ПО ПорчаТоваровВидыЗапасов.НоменклатураОприходование = Аналитика.Номенклатура
			//++ НЕ УТ 
			|		И ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) = Аналитика.СтатьяКалькуляции
			//-- НЕ УТ
			|			И ПорчаТоваровВидыЗапасов.ХарактеристикаОприходование = Аналитика.Характеристика
			|			И Ключи.Назначение = Аналитика.Назначение
			|			И ПорчаТоваровВидыЗапасов.Серия = Аналитика.Серия
			|			И ПорчаТоваровВидыЗапасов.Ссылка.Склад = Аналитика.Склад
			|ГДЕ
			|	ПорчаТоваровВидыЗапасов.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
			|	И ПорчаТоваровВидыЗапасов.Ссылка.Склад = &Склад
			|	И ПорчаТоваровВидыЗапасов.Ссылка.Организация = &Организация
