		...
		Иначе
			...
			Если ЗначениеЗаполнено(Объект.ДокументОснование)
				...
				Если ПолучитьФункциональнуюОпцию("ИспользоватьТорговыеНадбавки") Тогда
					ЦенаСклада = Неопределено;
					Если ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
						ЦенаСклада = Объект.ДокументОснование.СкладПолучатель.РозничныйВидЦены;
					ИначеЕсли ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда 
						ЦенаСклада = Объект.ДокументОснование.Склад.РозничныйВидЦены;
					КонецЕсли;
					
					Если ЦенаСклада <> Неопределено Тогда
						Для Каждого ТекСтрока Из ВыбранныеЦены Цикл
							Если ТекСтрока.Ссылка = ЦенаСклада  Тогда
								ТекСтрока.Выбрана = Истина;
								ТекСтрока.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБ;
								СхемаКомпоновкиДанных = Документы.УстановкаЦенНоменклатуры.ПолучитьМакет("ЦеныРозничныеВводНаОсновании");
								ТекСтрока.АдресСхемыКомпоновкиДанных = ПоместитьВоВременноеХранилище(СхемаКомпоновкиДанных, УникальныйИдентификатор);
								НоваяСтрока = Объект.ВидыЦен.Добавить();
								НоваяСтрока.ВидЦены = ТекСтрока.Ссылка;
							КонецЕсли;
						КонецЦикла;
