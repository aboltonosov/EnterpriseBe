...
Процедура ЗаполнитьДокументНаОснованииПередачи(Знач Основание)
...
Процедура ЗаполнитьДокументНаОснованииЗаказа(Знач Основание)
...
Процедура СообщитьОбОшибкахЗаполненияВидовЗапасов(ТаблицаОшибок, МенеджерВременныхТаблиц)
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Возврат превышает остаток сырья и материалов у переработчика %1 %2 %3'"),
			Партнер,                            // %1
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если СтрокаТаблицы.НеУказанНомерГТД Тогда
				ТекстСообщения = НСтр("ru = 'Номенклатура: %1, остатков у переработчика %2 недостаточно %3 %4 с указанными номерами ГТД'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Номенклатура: %1, остатков у переработчика %2 недостаточно %3 %4'");
			КонецЕсли;
