...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ТекстЗадолженностьПокупателяЗаУслуги()
		...
		// 4D:ERP для Беларуси, ВладимирР, 22.08.2017 12:13:35 
		// Ошибки ERP, №15567
		// {
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	// Локализация печатных форм, №7762
	// {
	|	Шапка.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	Шапка.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	Шапка.Отпустил КАК Отпустил,
	|	Шапка.ОтпустилДолжность КАК ОтпустилДолжность,
	|	Шапка.Руководитель.Наименование КАК Разрешил,
	|	Шапка.Руководитель.Должность КАК РазрешилДолжность,
	|	Шапка.Принял КАК Принял,
	|	Шапка.ПереданыДокументы КАК ПереданыДокументы,
	// }
	// 4D
	|	Шапка.Ссылка КАК Ссылка,
	|	Шапка.Номер КАК Номер,
