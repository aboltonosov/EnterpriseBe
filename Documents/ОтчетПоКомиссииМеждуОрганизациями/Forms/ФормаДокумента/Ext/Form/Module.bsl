...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если КонецДня(Объект.Дата) <> КонецДня(Объект.КонецПериода)
		И ЗначениеЗаполнено(Объект.КонецПериода) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Конец периода, за который отчитывается комиссионер, не может отличаться от даты документа.'"),
			,
			"КонецПериода",
			"Объект",
			Отказ);
...
&НаКлиенте
Процедура ВыполнитьЗеркальныйЗачетОплатыЗавершение(РезультатВопроса, ЗеркальныйЗачетОплатыМеждуОрганизациями) Экспорт
    
    КодОтвета = РезультатВопроса;
	Если КодОтвета = КодВозвратаДиалога.Да Тогда
		ПараметрыФормы = Новый Структура("Документ, ЗеркальныйЗачетОплатыМеждуОрганизациями", Объект.Ссылка, ЗеркальныйЗачетОплатыМеждуОрганизациями);
