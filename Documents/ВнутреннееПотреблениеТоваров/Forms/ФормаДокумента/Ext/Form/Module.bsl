...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	Документы.ВнутреннееПотреблениеТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Объект.Ссылка, МассивЗаказов, ПараметрыЗаполнения, ДопОтборы);
...
Процедура УстановитьВидимость(ОчищатьСтатьи = Ложь)
	...
	// Учет бланков строгой отчетности, №10038
	// {
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СписаниеТоваровПоТребованию") И Объект.РежимИспользованияБСО Тогда
