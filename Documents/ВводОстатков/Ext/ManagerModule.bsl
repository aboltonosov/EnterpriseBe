...
#Область ТекстОстаткиВыплаченныхПроцентовПоКредитам        // Дт 66.02 :: Кт 000
	...
	|
	|	ВЫБОР
	// 4D:ERP для Беларуси, Екатерина, 08.12.2015 11:10:06 
	// , №10719
	// {
	|// Кредиты
	|		КОГДА ДанныеДоговора.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговораКредитовИДепозитов.КредитВБанке)
...
#Область ТекстОстаткиНачисленныхПроцентовПоКредитам        // Дт 000 :: Кт 66.02
	...
	|	ДанныеДоговора.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|
	// 4D:ERP для Беларуси, Екатерина, 08.12.2015 11:07:56 
	// , №10719
	// {
	|	ВЫБОР
	|// Кредиты
...
#Область ТекстПередачаВЭксплуатацию                                 // Дт 10.11.Х :: Кт 000
	...
	|	)
	|	И НЕ Операция.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыВводаНачальныхОстатков.ОперативныйУчет)
	// 4D:ERP для Беларуси, Дмитрий, 20.11.2017 7:47:00 
	...
	|		ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПриПередаче50),
	|		ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПриСписании)
	|	)
	// }
	// 4D
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 17.12.2015 17:51:08 
	// Печатные формы по спецодежде, №10070
	// {
	
	// Задание на размещение товаров
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.СписокФорм = "ФормаТМЦВЭксплуатации";
	КомандаПечати.Идентификатор = "ВводОстатков";
	КомандаПечати.Представление = НСтр("ru = 'Ввод остатков ТМЦ в эксплуатации'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	// }
	// 4D
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 21.09.2015 14:30:43 
	// Печатные формы по спецодежде, № 10070
	// {
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ВводОстатков") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ВводОстатков", НСтр("ru = 'Ввод остатков ТМЦ в эксплуатации'"), СформироватьПечатнуюФормуВводаОстатковМатериалов(МассивОбъектов, ОбъектыПечати, Неопределено));
	КонецЕсли;
	// }
	// 4D
...
// Печатные формы по спецодежде, № 10070
// {
Функция СформироватьПечатнуюФормуВводаОстатковМатериалов(МассивОбъектов, ОбъектыПечати, КомплектыПечати)
	Запрос = Новый Запрос;
	...
	|	ДокВводОстатков.ТМЦВЭксплуатации.(
	|		НомерСтроки КАК НомерСтроки,
	|		Номенклатура.ЕдиницаДляОтчетов КАК ЕдИзм,
	|		ПРЕДСТАВЛЕНИЕ(Номенклатура.ЕдиницаДляОтчетов) КАК ПечЕдИзм,
	|		ПартияТМЦВЭксплуатации.Документ КАК ДокПередачи,
	|		Номенклатура КАК Материал,
	|		Номенклатура.Код КАК Код,
	|		Номенклатура.Артикул КАК Артикул,
	|		ПРЕДСТАВЛЕНИЕ(ДокВводОстатков.ТМЦВЭксплуатации.Номенклатура) КАК ПечМатериал,
	|		СуммаВР КАК ВременнаяРазница,
	|		СуммаПР КАК ПостояннаяРазница,
	|		Сумма КАК СтоимостьРегл,
	|		Количество,
	|		ПервоначальнаяСумма КАК Стоимость,
	|		ФизическоеЛицо КАК Работник,
	|		ПРЕДСТАВЛЕНИЕ(ДокВводОстатков.ТМЦВЭксплуатации.ФизическоеЛицо) КАК ПечРаботник,
	|		ДатаПередачиВЭксплуатацию КАК ДатаПередачи,
	|		ПРЕДСТАВЛЕНИЕ(ДокВводОстатков.ТМЦВЭксплуатации.ПартияТМЦВЭксплуатации.Документ) КАК ПечДокПередачи
	|	)
	|ИЗ
	|	Документ.ВводОстатков КАК ДокВводОстатков
	|ГДЕ
	|	ДокВводОстатков.Ссылка В(&МассивОбъектов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ВводОстатков.ПФ_MXL_ВводОстатков");
	
	Область = Макет.ПолучитьОбласть("Заголовок");
	
	РеквизитыДокумента = Новый Структура("Номер, Дата, Префикс");
	
	СинонимДокумента = Метаданные.Документы.ВводОстатков.Синоним;
	
	Область = Макет.ПолучитьОбласть("Заголовок");
	
	ЗаполнитьЗначенияСвойств(РеквизитыДокумента, Шапка);
	Область.Параметры.ТекстЗаголовок = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(РеквизитыДокумента, СинонимДокумента);
	...
	ТабЧасть = Шапка.ТМЦВЭксплуатации.Выбрать();
	Пока ТабЧасть.Следующий() Цикл
		...
		Область.Параметры.ПогашеннаяСтоимость = (ТабЧасть.Стоимость - ТабЧасть.СтоимостьРегл );
		
		Область.Параметры.ТекущаяСтоимость            = ТабЧасть.СтоимостьРегл;	
