...
Процедура РазделУчетаПриИзменении(Элемент)
	...
	Если Элементы.ГруппаКассы.Видимость Тогда
		Элементы.КассыЗаполнитьПоОстаткам.Видимость = 
...
Процедура УстановитьВидимость()
	...
	Элементы.КассыЗаполнитьПоОстаткам.Видимость = Объект.ТипОперации = Перечисления.ТипыОперацийВводаОстатков.ОстаткиВКассах
		И Объект.РазделУчета = ПредопределенноеЗначение("Перечисление.РазделыВводаНачальныхОстатков.РегламентированныйУчет");
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура КассыСуммаПриИзмененииСервер(Сумма, СуммаРегл, ИмяОперации, ВалютаРегламентированногоУчета, ДатаДокумента, Касса = Неопределено, КассаККМ = Неопределено)
