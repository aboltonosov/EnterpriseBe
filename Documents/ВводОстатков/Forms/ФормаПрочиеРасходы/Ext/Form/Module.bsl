...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Петр, 19.02.2018 11:30:01 
...
Процедура ПрочиеРасходыСуммаПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Петр, 19.02.2018 10:11:00 
	// Механизм ввода остатков, № 17856
	// {
...
Процедура ПрочиеРасходыСтавкаНДСПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Петр, 19.02.2018 9:45:31 
	// Механизм ввода остатков, № 17856
	// {
	СтруктураДействий.Вставить("ПересчитатьСуммуНДСРегл", СтруктураПересчетаСуммыРегл);
...
Процедура ПрочиеРасходыСуммаРеглПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Петр, 16.02.2018 17:50:10 
	// Механизм ввода остатков, № 17856
	// {
	СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаСуммыНДСВСтрокеТЧ(Объект);
	СтруктураПересчетаСуммыРегл = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаСуммыНДСВСтрокеТЧ(Объект);
...
&НаСервере
Процедура УстановитьВидимостьПРВР()
	...
	КонецЕсли;
	Элементы.ПрочиеРасходыСуммаПР.Видимость = ПрименяетсяПБУ18;
	Элементы.ПрочиеРасходыСуммаВР.Видимость = ПрименяетсяПБУ18;
