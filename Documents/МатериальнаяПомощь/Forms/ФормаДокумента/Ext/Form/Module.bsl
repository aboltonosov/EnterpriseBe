...
Процедура РассчитатьСотрудниковНаСервере(Знач Сотрудники, ОписаниеТаблицы, СохранятьИсправления = Ложь)
	...
	// 4D:ERP для Беларуси, Яна, 24.07.2017 11:34:55 
	...
	ДополнительныеПараметры.РассчитыватьНДФЛБезусловно = Истина;
	ДополнительныеПараметры.ОкончательныйРасчетНДФЛ = ОкончательныйРасчетНДФЛ;	
	
	// Выполняем пересчет коллекций документа.	

	// 4D:ERP для Беларуси, АлексейЧ, 15.02.2018 15:52:13 
	// Локализовать документ "Начисление зарплаты и взносов">, №  15928
	// {
	// Неправильная отработка документа. Оставляем типовой функционал!!!!
	///***************************************************
	//РасчетЗарплатыРасширенный.РассчитатьДанныеФормыПоСотрудникам(Сотрудники, ЭтаФорма, ОписаниеДокумента(ЭтаФорма),ДополнительныеПараметры,ОписаниеТаблицы,СохранятьИсправления);
	// }
	// 4D
