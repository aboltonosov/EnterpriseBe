...
Процедура ПересчитатьНаКлиенте(ПараметрыСтроки, ИмяРеквизита, Показатель = "")
	...
	Если ИмяРеквизита = "ОстатокОтпуска" ИЛИ ИмяРеквизита = "ОтпускАвансом" ИЛИ ИмяРеквизита = "СреднийЗаработок" Тогда
		...
		ИначеЕсли Не ФормироватьРезервыОтпусковВНалоговомУчете Тогда
	...
	Если ИмяРеквизита = "ТекущаяСтавкаСтраховыхВзносов" Тогда
		...
		Если ФормироватьРезервыОтпусковВНалоговомУчете Тогда
			Если Инвентаризация Тогда
				ПараметрыСтроки.СуммаРезерваСтраховыхВзносовИсчисленоНУ = ПараметрыСтроки.СуммаРезерваСтраховыхВзносовИсчислено;
			Иначе
				ПараметрыСтроки.СуммаРезерваСтраховыхВзносовИсчисленоНУ = ПараметрыСтроки.СуммаРезерваИсчисленоНУ*ПараметрыСтроки.ТекущаяСтавкаСтраховыхВзносов/100;
			КонецЕсли;
			ПараметрыСтроки.СуммаРезерваСтраховыхВзносовНУ            = ПараметрыСтроки.СуммаРезерваСтраховыхВзносовИсчисленоНУ;
		Иначе
			ПараметрыСтроки.СуммаРезерваСтраховыхВзносовИсчисленоНУ   = 0;
			ПараметрыСтроки.СуммаРезерваСтраховыхВзносовНУ            = 0;
		КонецЕсли;
	...
	Если ИмяРеквизита = "ТекущаяСтавкаФССНесчастныеСлучаи" Тогда
		...
		Если ФормироватьРезервыОтпусковВНалоговомУчете Тогда
			Если Инвентаризация Тогда
				ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ = ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиИсчислено ;
			Иначе
				ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ = ПараметрыСтроки.СуммаРезерваИсчисленоНУ*ПараметрыСтроки.ТекущаяСтавкаФССНесчастныеСлучаи/100;
			КонецЕсли;
			ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиНУ          = ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ;
		Иначе
			ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиИсчисленоНУ = 0;
			ПараметрыСтроки.СуммаРезерваФССНесчастныеСлучаиНУ          = 0;
		КонецЕсли;
	...
	Если МетодОбязательств И Показатель = "БУ" Тогда
