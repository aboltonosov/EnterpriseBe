...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТорговыеНадбавки") Тогда
		...
		РозничныеПродажиВызовСервера_Локализация.ВставитьРеквизитВТабличнуюЧасть(ЭтаФорма, ЭтаФорма.Элементы, "Товары", "ЦенаИзготовителя", "Цена изготовителя (импортера)", "Цена");
		Элементы["ТоварыЦенаИзготовителя"].УстановитьДействие("ПриИзменении", "Подключаемый_ТоварыЦенаИзготовителяПриИзменении");
	...
	РозничныеПродажиВызовСервера_Локализация.УстановитьНастройкиВидимостиРеквизитовКомандДляРозницы(ЭтаФорма, "Поступление");
	
	Если ЗначениеЗаполнено(Объект.Склад) И Объект.Склад.ЭтоГруппа Тогда
		ЭтаФорма.ЭтоРозничныйСклад = РозничныеПродажи_Локализация.ПроверитьСклады(Объект.Склад);	
	КонецЕсли;  
	// }
	// 4D
...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ФормаСтавкиВРознице" Тогда	
		Если РезультатВыбора <> Неопределено Тогда
			Если РезультатВыбора.СтавкаТН = 0 Тогда
				Если РезультатВыбора.РозничнаяСтавкаНДС <> Неопределено Тогда
					Если РезультатВыбора.ВыделенныеСтроки Тогда
						НазначитьСтавкуРозничногоНДСНаСервере(РезультатВыбора.РозничнаяСтавкаНДС, Элементы.Товары.ВыделенныеСтроки);
					Иначе
					    НазначитьСтавкуРозничногоНДСНаСервере(РезультатВыбора.РозничнаяСтавкаНДС);
					КонецЕсли; 					
					РозничныеПродажиКлиент_Локализация.ОповеститьОбОкончанииНазначенияРозничныхСтавокНДС(РезультатВыбора.РозничнаяСтавкаНДС);
				КонецЕсли;
			Иначе
				Если РезультатВыбора.ВыделенныеСтроки Тогда
					НазначитьСтавкуТорговойНадбавкиНаСервере(РезультатВыбора.СтавкаТН, Элементы.Товары.ВыделенныеСтроки);
				Иначе	
				    НазначитьСтавкуТорговойНадбавкиНаСервере(РезультатВыбора.СтавкаТН);
				КонецЕсли; 
				РозничныеПродажиКлиент_Локализация.ОповеститьОбОкончанииНазначенияТорговыхНадбавок(РезультатВыбора.СтавкаТН);	
			КонецЕсли; 		
		КонецЕсли;			
	КонецЕсли; 
	// }
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если Объект.РежимИспользованияБСО Тогда
			...
			СтруктураВозврата = УчетБланковСтрогойОтчетностиСервер.ПроверитьВозможностьПоступленияБСО(Объект.Товары, Объект.Дата);
...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажиКлиент_Локализация.ЗаполнитьСтруктуруТорговыеНадбавкиРозничныйНДС(Объект, ЭтаФорма, СтруктураДействий);
	// }
	// 4D
...
Процедура ТоварыКоличествоУпаковокПриИзменении(Элемент)
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажиКлиент_Локализация.ДобавитьВСтруктуруРассчитатьРозничнуюЦену(Объект, ЭтаФорма, СтруктураДействий);
	// }
	// 4D
...
Процедура Подключаемый_ТоварыРозничнаяЦенаСУчетомОкругленияПриИзменении(Элемент) 	
	...
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("РассчитатьСтавкуСуммуТорговойНадбавки", Новый Структура("ИмяДокумента,ЦенаВключаетНДС","Поступление", Объект.ЦенаВключаетНДС));
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТоварыЦенаИзготовителяПриИзменении(Элемент) 
	...
	Если ТекущаяСтрока.Цена = 0 Тогда
		...
		СтруктураДействий.Вставить("ЗаполнитьЦенуИзЦеныИзготовителя");
	КонецЕсли;
...
Процедура Подключаемый_ТоварыСтавкаТНПриИзменении(Элемент)  
	...
	РозничныеПродажиКлиент_Локализация.РассчитатьРозничнуюЦену("Поступление", Элементы, ЭтаФорма, Объект, КэшированныеЗначения); 
...
Процедура НазначитьСтавкуТорговойНадбавкиНаСервере(СтавкаТорговойНадбавки, Знач ВыделенныеСтроки = Неопределено) 
	...
	РозничныеПродажи_Локализация.НазначитьРозничныеСтавки(Объект, "Товары", СтавкаТорговойНадбавки, "СтавкаТН", "Поступление", ЭтаФорма["РозничныйВидЦен"], ВыделенныеСтроки);
...
Процедура НазначитьСтавкуРозничногоНДСНаСервере(СтавкаРозничногоНДС, Знач ВыделенныеСтроки = Неопределено)   
	...
	РозничныеПродажи_Локализация.НазначитьРозничныеСтавки(Объект, "Товары", СтавкаРозничногоНДС, "РозничнаяСтавкаНДС","Поступление", ЭтаФорма["РозничныйВидЦен"], ВыделенныеСтроки);
...
Процедура ПриИзмененииСкладаСервер()
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажи_Локализация.ГруппаСкладов(СкладГруппа, Склад, ЭтаФорма, Объект);
	// }
	// 4D
...
Процедура ЦенаВключаетНДСПриИзмененииСервер(КэшированныеЗначения)
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажи_Локализация.ДобавитьВСтруктуруРассчитатьРозничнуюЦену(Объект, ЭтаФорма, СтруктураДействий);
	// }
	// 4D
...
Процедура КонтрольНеСогласованныхИзмененийОбработатьСобытиеПриИзменении(Элемент)
	...
	ИначеЕсли Элемент.Имя = "ТоварыРозничнаяСтавкаНДС" Тогда
		Подключаемый_ТоварыРозничнаяСтавкаНДСПриИзменении(Элемент);	
	// Локализация_УправлениеТорговлейДляБеларуси }}
	Иначе
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажи_Локализация.ПриЧтенииСозданииНаСервере(ЭтаФорма, Объект);
	// }
	// 4D
	...
	// Розничная торговля, №15729
	// {
	РозничныеПродажи_Локализация.ЗаполнитьРозничныеСтавкиНДС(ЭтаФорма, Объект);
	// }
	// 4D
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		// 4D:ERP для Беларуси, Петр, 21.02.2018 15:07:22 
		// Ошибка подбора номенклатуры в документах поступления при заполнения документа, № 17997
		// {
		СтруктураДействий.Вставить("ПересчитатьСуммуСУчетомРучнойСкидки", Новый Структура("Очищать", Ложь));
		// }
		// 4D
		...
		// Розничная торговля, №15729
		// {
		РозничныеПродажи_Локализация.ЗаполнитьСтруктуруТорговыеНадбавкиРозничныйНДС(Объект, ЭтаФорма, СтруктураДействий);
		// }
		// 4D
