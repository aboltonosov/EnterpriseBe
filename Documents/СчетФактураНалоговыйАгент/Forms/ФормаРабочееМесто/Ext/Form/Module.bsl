...
// 4D:ERP для Беларуси, ВладимирР, 19.04.2016 10:52:01 
// Ставка НДС в счет-фактурах, №11298
// {

&НаКлиенте
Процедура УстановитьСтавкуНДСОбщая(Команда)
	...
	ТекСтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС." + Команда.Имя);
...
Процедура УстановитьСтавкуНДС(ТекстСтавкаНДС)
	...
	// Ставка НДС 25%, №11449
	// {
	ИначеЕсли ТекстСтавкаНДС = "20%" Тогда
		ТекСтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС20");
	ИначеЕсли ТекстСтавкаНДС = "25%" Тогда
		ТекСтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС25");
	// }
	// 4D
