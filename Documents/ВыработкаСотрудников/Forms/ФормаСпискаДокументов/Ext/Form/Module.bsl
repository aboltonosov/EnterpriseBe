...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОтборВидНаряда.СписокВыбора.Добавить(Перечисления.ВидыБригадныхНарядов.ПустаяСсылка(), НСтр("ru = 'Любой'"));
	Для Каждого Строка Из ДоступныеВидыНарядов Цикл
		Элементы.ОтборВидНаряда.СписокВыбора.Добавить(Строка.Значение);
	КонецЦикла;
	
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(БригадныеНаряды, "ПроизводствоПоРаспоряжениям", НСтр("ru = 'Производство (по распоряжениям)'"));
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(БригадныеНаряды, "ПроизводствоБезРаспоряжений", НСтр("ru = 'Производство (без распоряжений)'"));
	...
	ТолькоУправлениеПроизводством2_2 =
		Не ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством")
		И ПолучитьФункциональнуюОпцию("ИспользоватьУправлениеПроизводством2_2");
	
	Если ТолькоУправлениеПроизводством2_2 Тогда
		Элементы.БригадныеНарядыСоздатьНарядНаПроизводствоБезРаспоряжения.Видимость = Ложь;
		Элементы.БригадныеНарядыСоздатьНарядНаПроизводство.Заголовок = НСтр("ru = 'По производству'");
	КонецЕсли;
...
Процедура КомандаОформитьПоРаспоряжению(Команда)
	...
	Для Каждого НомерСтроки из ВыделенныеСтроки Цикл
		...
		Если ТекБригада <> Строка.Бригада Тогда
			ТекстПредупреждения = НСтр("ru = 'Документ может быть сформирован только по одной бригаде.'");
			Отказ = Истина;
		ИначеЕсли ТекОрганизация <> Строка.Организация Тогда
			ТекстПредупреждения = НСтр("ru = 'Документ может быть сформирован только по одной организации.'");
			Отказ = Истина;
		ИначеЕсли ТекВидНаряда <> Строка.ВидНаряда Тогда
			ТекстПредупреждения = НСтр("ru = 'Документ может быть сформирован только по одному виду наряда.'");
			Отказ = Истина;
		КонецЕсли;
		...
		Периоды.Добавить(НачалоМесяца(Строка.Период));
	...
	ЗначенияЗаполнения.Вставить("ВидыРабот",          МассивВидовРабот);
	ЗначенияЗаполнения.Вставить("Распоряжения",       МассивРаспоряжений);
	ЗначенияЗаполнения.Вставить("Периоды",            Периоды);
...
Процедура ЗаполнитьТрудозатратыКОформлению()
	...
	Если НЕ ОтображатьРаспоряжения Тогда
		ТаблицаТрудозатратыКОформлению.Свернуть("Период, Бригада,ВидНаряда,ВидРабот,Подразделение,Организация", "Количество");
		ТаблицаТрудозатратыКОформлению.Сортировать("Период, Бригада,ВидНаряда,ВидРабот,Подразделение,Организация");
	Иначе
		ТаблицаТрудозатратыКОформлению.Сортировать("Период, Бригада,ВидНаряда,ВидРабот,Подразделение,Организация,Распоряжение");
	КонецЕсли;
