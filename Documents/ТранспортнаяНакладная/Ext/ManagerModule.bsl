...
Функция СоздатьТранспортныеНакладныеДляЗаданийНаПеревозку(МассивОбъектов, ВыделенныеСтроки = Неопределено) Экспорт
	...
	|
	// 4D:ERP для Беларуси, ВладимирР, 23.03.2016 16:12:03 
	// Д.Товарно-транспортная накладная, №10783
	// {
	|	ЕСТЬNULL(ТранспортнаяНакладная.ПутевойЛист, """") КАК ПутевойЛист,
	|	ПРЕДСТАВЛЕНИЕ(НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.ТранспортноеСредство.ВладелецТранспорта) КАК ВладелецТранспорта,
	|	НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.Водитель.Наименование КАК Водитель,
	|	НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.ТранспортноеСредство.Код КАК АвтомобильГосударственныйНомер,
	|	НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.ТранспортноеСредство.Марка КАК АвтомобильМарка,
	|	НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.ТранспортноеСредство.ГосударственныйНомерПрицепа КАК ГосударственныйНомерПрицепа,
	|	НакладныеПоЗаданиямНаПеревозку.ЗаданиеНаПеревозку.ТранспортноеСредство.Прицеп КАК Прицеп,
	// }
	// 4D
	|
...
//-- НЕ УТКА
	...
	Пока ТранспортныеНакладныеПоНомерамСтрок.Следующий() Цикл
		...
		// Д.Товарно-транспортная накладная, №10783
		// {
		ЗначенияЗаполнения.Вставить("ПутевойЛист", ТранспортныеНакладныеПоНомерамСтрок.ПутевойЛист);
		ЗначенияЗаполнения.Вставить("ВладелецТранспорта", ТранспортныеНакладныеПоНомерамСтрок.ВладелецТранспорта);
		ЗначенияЗаполнения.Вставить("Водитель", ТранспортныеНакладныеПоНомерамСтрок.Водитель);
		ЗначенияЗаполнения.Вставить("АвтомобильМарка", ТранспортныеНакладныеПоНомерамСтрок.АвтомобильМарка);
		ЗначенияЗаполнения.Вставить("АвтомобильГосударственныйНомер", ТранспортныеНакладныеПоНомерамСтрок.АвтомобильГосударственныйНомер);
		ЗначенияЗаполнения.Вставить("Прицеп", ТранспортныеНакладныеПоНомерамСтрок.Прицеп);
		ЗначенияЗаполнения.Вставить("ГосударственныйНомерПрицепа", ТранспортныеНакладныеПоНомерамСтрок.ГосударственныйНомерПрицепа);
		// }
		// 4D
...
Функция СоздатьТранспортныеНакладные(МассивОбъектов, ЭтоПечать = Ложь) Экспорт
	...
	|	ГДЕ
	|		ВозвратТоваровМеждуОрганизациями.Ссылка В(&МассивОбъектов)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// }
...
//-- НЕ УТ
	...
	// {
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.РеализацияТоваровУслуг)
	|				ИЛИ ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ВозвратТоваровПоставщику)
	|				ИЛИ ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ПеремещениеТоваров)
	|			ТОГДА ПараметрыТТН.Ссылка.НомерИсходящегоДокумента
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	...
	|				ИЛИ ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ВозвратТоваровПоставщику)
	|				ИЛИ ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ПеремещениеТоваров)
	|			ТОГДА ПараметрыТТН.Ссылка.СерияИсходящегоДокумента
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК СерияИсходящегоДокумента,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.РеализацияТоваровУслуг)
	|				ИЛИ ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ВозвратТоваровПоставщику)
	|			ТОГДА ЕСТЬNULL(ПараметрыТТН.Ссылка.Договор.Наименование, НЕОПРЕДЕЛЕНО)
	|		КОГДА ТИПЗНАЧЕНИЯ(ПараметрыТТН.Ссылка) = ТИП(Документ.ПеремещениеТоваров)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Основание,
	// }
	...
	// Серия и номер БСО в накладных, №11113
	// {
	НомерИсходящегоДокумента = Неопределено;
	СерияИсходящегоДокумента = Неопределено;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПервоеРаспоряжение 
			...
			Если ЗначениеЗаполнено(МассивРаспоряжений) Тогда
				...
				Если ЗначениеЗаполнено(МассивРаспоряженийСТоварами) Тогда 
					...
					// Серия и номер БСО в накладных, №11113
					// {
					ЗначенияЗаполнения.Вставить("НомерИсходящегоДокумента", Выборка.НомерИсходящегоДокумента);
					ЗначенияЗаполнения.Вставить("СерияИсходящегоДокумента", ВРег(Выборка.СерияИсходящегоДокумента));
					ЗначенияЗаполнения.Вставить("Основание", Выборка.Основание);
					// }
					// 4D
		...
		// Серия и номер БСО в накладных, №11113
		// {
		НомерИсходящегоДокумента = Выборка.НомерИсходящегоДокумента;
		СерияИсходящегоДокумента = Выборка.СерияИсходящегоДокумента;
	...
	Если ЗначениеЗаполнено(МассивРаспоряжений) Тогда	
		...
		Если ЗначениеЗаполнено(МассивРаспоряженийСТоварами) Тогда
			...
			// Серия и номер БСО в накладных, №11113
			// {
			ЗначенияЗаполнения.Вставить("НомерИсходящегоДокумента", НомерИсходящегоДокумента);
			ЗначенияЗаполнения.Вставить("СерияИсходящегоДокумента", ВРег(СерияИсходящегоДокумента));
...
Функция РеквизитыТранспортнойНакладной(МассивРаспоряжений = Неопределено, ЗаданиеНаПеревозку = Неопределено, ЗначенияЗаполнения = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 24.11.2014 6:27:53 
	// Локализация печатных форм, №7762
	// {
	ДанныеЗаполнения.Вставить("ПутевойЛист");
	ДанныеЗаполнения.Вставить("ВладелецТранспорта");
	ДанныеЗаполнения.Вставить("Принял");
	ДанныеЗаполнения.Вставить("ПогрузкаИсполнитель");
	ДанныеЗаполнения.Вставить("ПогрузкаСпособРабот");
	ДанныеЗаполнения.Вставить("ПогрузкаДатаПрибытия");
	ДанныеЗаполнения.Вставить("ПогрузкаДатаУбытия");
	ДанныеЗаполнения.Вставить("ПогрузкаПростой");
	ДанныеЗаполнения.Вставить("ПогрузкаДопОперацииВремя");
	ДанныеЗаполнения.Вставить("ПогрузкаДопОперацииНаименование");
	ДанныеЗаполнения.Вставить("РазгрузкаИсполнитель");
	ДанныеЗаполнения.Вставить("РазгрузкаСпособРабот");
	ДанныеЗаполнения.Вставить("РазгрузкаДатаПрибытия");
	ДанныеЗаполнения.Вставить("РазгрузкаДатаУбытия");
	ДанныеЗаполнения.Вставить("РазгрузкаПростой");
	ДанныеЗаполнения.Вставить("РазгрузкаДопОперацииВремя");
	ДанныеЗаполнения.Вставить("РазгрузкаДопОперацииНаименование");
	ДанныеЗаполнения.Вставить("ТранпортныеУслуги");
	ДанныеЗаполнения.Вставить("КоличествоЕздок");
	ДанныеЗаполнения.Вставить("НомерИсходящегоДокумента");
	ДанныеЗаполнения.Вставить("СерияИсходящегоДокумента");
	ДанныеЗаполнения.Вставить("Основание");
	ДанныеЗаполнения.Вставить("Переадресовка");
	ДанныеЗаполнения.Вставить("ПереданыДокументы");
	// }
	// 4D
...
Функция ПолучитьТолькоДокументыОснованияСТоварами(МассивВсехРаспоряжений, Отказ = Неопределено) Экспорт
	...
	Документы.ВозвратТоваровМеждуОрганизациями.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения);
	// }
	// 4D
...
//-- НЕ УТКА
	...
	|	ДокументТовары.Ссылка
	|ИЗ
	|	ПередачаТоваровМеждуОрганизациямиТаблицаТоваров КАК ДокументТовары
	|ГДЕ
	|	ДокументТовары.ЭтоТовар
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДокументТовары.Ссылка
	|ИЗ
	|	ВозвратТоваровМеждуОрганизациямиТаблицаТоваров КАК ДокументТовары
	|ГДЕ
	|	ДокументТовары.ЭтоТовар
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция ПолучитьПараметрыОтбораТТН(ДокументСсылка) Экспорт 
	...
	|		Документ.ПередачаТоваровМеждуОрганизациями КАК ПередачаТоваровМеждуОрганизациями
	|	ГДЕ
	|		ПередачаТоваровМеждуОрганизациями.Ссылка = &ДокументСсылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		Документ.ВозвратТоваровМеждуОрганизациями КАК ВозвратТоваровМеждуОрганизациями
	|	ГДЕ
	|		ВозвратТоваровМеждуОрганизациями.Ссылка = &ДокументСсылка
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// }
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТТН") Тогда
		...
		// 4D:ERP для Беларуси, ВладимирР, 11.02.2015 11:54:57 
		// Задача №9058, Обновление до релиза 2.0.10
		// {
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "";
...
//-- НЕ УТКА
	...
	// 4D:ERP для Беларуси, Дмитрий, 02.05.2015 0:24:02 
	// Локализация печатных форм, №7762
	// {
	Документы.РеализацияТоваровУслуг.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	Документы.ВозвратТоваровПоставщику.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	...
	//++ НЕ УТ
	Документы.ПередачаСырьяПереработчику.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	//-- НЕ УТ
	
	//++ НЕ УТКА
	Документы.ПередачаДавальцу.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	Документы.ВозвратСырьяДавальцу.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	//-- НЕ УТКА
	// }
	// 4D
	...
	// ТТН-1 и ТН-2, №15629
	// {
	Документы.ПередачаТоваровМеждуОрганизациями.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	Документы.ВозвратТоваровМеждуОрганизациями.ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц);
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, ВладимирР, 07.06.2016 11:07:41
	// Печать наборов в ТТН, №11591
	// {
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ТаблицаТоваровКорректировкаРеализацииНаборы
	|ИЗ
	|	ТаблицаТоваровКорректировкаРеализации КАК ТаблицаТоваров
	...
	|			Т.Ссылка КАК Ссылка
	|		ИЗ
	|			РеализацияТоваровУслугТаблицаТоваров КАК Т) КАК Т
	|		ПО (ИСТИНА)
	|ГДЕ
	...
	|			ИЗ
	|				РеализацияТоваровУслугТаблицаТоваров КАК Т)
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Ссылка,
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка,
	...
	|	Товары.ВариантПредставленияНабораВПечатныхФормах,
	|	Товары.ВариантРасчетаЦеныНабора,
	|	Товары.НоменклатураНабора,
	|	Товары.ХарактеристикаНабора,
	|	Товары.Номенклатура,
	|	Товары.Характеристика,
	...
	|	Товары.Количество
	|ИЗ
	|	ТаблицаТоваровКорректировкаРеализацииНаборы КАК Товары
	|ГДЕ
	|	Товары.НоменклатураНабора <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|			Т.Ссылка КАК Ссылка
	|		ИЗ
	|			ТаблицаТоваровКорректировкаРеализацииНаборы КАК Т) КАК Т
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				Т.ВариантКомплектацииНоменклатуры
	|			ИЗ
	|				ТаблицаТоваровКорректировкаРеализацииНаборы КАК Т)
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Ссылка,
	|	ВариантыКомплектацииНоменклатурыТовары.Ссылка,
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(Таблица.Ссылка КАК Документ.ПоступлениеТоваровУслуг).ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаНаКомиссию)
	|			ТОГДА ВЫБОР
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВЫРАЗИТЬ(Таблица.Ссылка КАК Документ.ПоступлениеТоваровУслуг).ХозяйственнаяОперация", "Таблица.Ссылка.ХозяйственнаяОперация");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаВесНоменклатуры", Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаВесУпаковки("ТаблицаТоваров.Номенклатура.ЕдиницаИзмерения", "ТаблицаТоваров.Номенклатура"));
	...
	|
	// 4D:ERP для Беларуси, Дмитрий, 15.12.2014 0:55:32 
	// Локализация печатных форм, №7762
	// {
	|	ЕСТЬNULL(ТаблицаТранспортныхНакладных.ДокументОснование.Валюта, &ВалютаРегламентированногоУчета) КАК Валюта,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПутевойЛист КАК ПутевойЛист,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ВладелецТранспорта КАК ВладелецТранспорта,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Руководитель КАК Разрешил,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Руководитель.Должность КАК РазрешилДолжность,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Принял КАК Принял,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаИсполнитель КАК ПогрузкаИсполнитель,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаСпособРабот КАК ПогрузкаСпособРабот,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаДатаПрибытия КАК ПогрузкаДатаПрибытия,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаДатаУбытия КАК ПогрузкаДатаУбытия,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаПростой КАК ПогрузкаПростой,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаДопОперацииВремя КАК ПогрузкаДопОперацииВремя,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПогрузкаДопОперацииНаименование КАК ПогрузкаДопОперацииНаименование,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаИсполнитель КАК РазгрузкаИсполнитель,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаСпособРабот КАК РазгрузкаСпособРабот,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаДатаПрибытия КАК РазгрузкаДатаПрибытия,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаДатаУбытия КАК РазгрузкаДатаУбытия,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаПростой КАК РазгрузкаПростой,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаДопОперацииВремя КАК РазгрузкаДопОперацииВремя,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.РазгрузкаДопОперацииНаименование КАК РазгрузкаДопОперацииНаименование,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ТранпортныеУслуги КАК ТранпортныеУслуги,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.КоличествоЕздок КАК КоличествоЕздок,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Основание КАК Основание,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Переадресовка КАК Переадресовка,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ПереданыДокументы КАК ПереданыДокументы,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.Отпустил КАК Отпустил,
	|	ТаблицаТранспортныхНакладных.ТранспортнаяНакладная.ОтпустилДолжность КАК ОтпустилДолжность,
	// }
	// 4D
	|
	...
	|	ЕСТЬNULL(ТаблицаТранспортныхНакладных.ДокументОснование.Валюта, &ВалютаРегламентированногоУчета),
	...
	|		ПО ТаблицаТоваров.НоменклатураНабора = ВременнаяТаблицаНаборы.НоменклатураНабора
	|			И ТаблицаТоваров.ХарактеристикаНабора = ВременнаяТаблицаНаборы.ХарактеристикаНабора
	|			И ТаблицаТоваров.Ссылка = ВременнаяТаблицаНаборы.Ссылка
	|ГДЕ
	|	(ТаблицаТоваров.НоменклатураНабора = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	...
	|
	|ВЫБРАТЬ
	|	ВременнаяТаблицаНаборы.СуммаБезНДС + ВременнаяТаблицаНаборы.СуммаНДС,
	...
	|	ВременнаяТаблицаНаборы.НоменклатураНабора,
	|	ВременнаяТаблицаНаборы.НоменклатураНабора.Код,
	|	ВременнаяТаблицаНаборы.НоменклатураНабора.НаименованиеПолное,
	|	ВременнаяТаблицаНаборы.ХарактеристикаНабора.НаименованиеПолное,
	|	ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка),
	|	ВЫБОР
	|		КОГДА НЕ &ВыводитьБазовыеЕдиницыИзмерения
	|			ТОГДА ВременнаяТаблицаНаборы.КоличествоУпаковок
	|		ИНАЧЕ ВременнаяТаблицаНаборы.Количество
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка),
	|	ВременнаяТаблицаНаборы.НоменклатураНабора.ЕдиницаИзмерения.Представление,
	|	ВЫБОР
	|		КОГДА НЕ &ВыводитьБазовыеЕдиницыИзмерения
	|			ТОГДА ВременнаяТаблицаНаборы.СуммаБезНДС / ВременнаяТаблицаНаборы.КоличествоУпаковок
	|		ИНАЧЕ ВременнаяТаблицаНаборы.СуммаБезНДС / ВременнаяТаблицаНаборы.Количество
	|	КОНЕЦ,
	|	0,
	|	0,
	|	0,
...
//-- НЕ УТКА
	...
	|ВЫБРАТЬ
	|	
	// 4D:ERP для Беларуси, Дмитрий, 19.12.2014 5:54:54 
...
Функция ПолучитьДанныеДляПечатнойФормыТранспортнаяНакладная(ТаблицаНакладных) Экспорт
	...
	|	ИЗ
	|		ТаблицаТранспортныхНакладных КАК ДокументыОснования
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровМеждуОрганизациями.Товары КАК ДокументТовары
	|			ПО ДокументыОснования.ДокументОснование = ДокументТовары.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|			ПО (ВидыНоменклатуры.Ссылка = ДокументТовары.Номенклатура.ВидНоменклатуры)
	|	
	...
	|	ИЗ
	|		ТаблицаТранспортныхНакладных КАК ДокументыОснования
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями.Товары КАК ДокументТовары
	|			ПО ДокументыОснования.ДокументОснование = ДокументТовары.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|			ПО (ВидыНоменклатуры.Ссылка = ДокументТовары.Номенклатура.ВидНоменклатуры)
	|	
...
Процедура ЗаполнитьРеквизитыТранспортнойНакладной(МассивРаспоряжений, РеквизитыТранспортнойНакладной, ЗначенияЗаполнения = Неопределено, ЗаданиеНаПеревозку = Неопределено)
	...
	|ВЫБРАТЬ
	|
	// 4D:ERP для Беларуси, Дмитрий, 19.12.2014 5:31:22 
	// Локализация печатных форм, №7762
	// {
	|	ЕСТЬNULL(ДокументРаспоряжение.НомерИсходящегоДокумента, """") КАК НомерИсходящегоДокумента,
	|	ЕСТЬNULL(ДокументРаспоряжение.СерияИсходящегоДокумента, """") КАК СерияИсходящегоДокумента,
	|	ЕСТЬNULL(ДокументРаспоряжение.Руководитель, """") КАК Разрешил,
	|	ЕСТЬNULL(ДокументРаспоряжение.Принял, """") КАК Принял,
	|	ЕСТЬNULL(ДокументРаспоряжение.ПереданыДокументы, """") КАК ПереданыДокументы,
	// }
	// 4D
	|
	|	ВЫБОР
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ДокументРаспоряжение.НомерИсходящегоДокумента, """"),
	|	ЕСТЬNULL(ДокументРаспоряжение.СерияИсходящегоДокумента, """"),
	|	ЕСТЬNULL(ДокументРаспоряжение.Руководитель, """"),
	|	ЕСТЬNULL(ДокументРаспоряжение.Принял, """"),
	|	ЕСТЬNULL(ДокументРаспоряжение.ПереданыДокументы, """"),
	|	"""",
	|	"""",
	|	"""",
	|	"""",
	...
	|	""""
	|ИЗ
	|	Документ.ПередачаТоваровМеждуОрганизациями КАК ДокументРаспоряжение
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады.КонтактнаяИнформация КАК СкладыКонтактнаяОтправитель
	|		ПО ДокументРаспоряжение.Склад = СкладыКонтактнаяОтправитель.Ссылка
	|			И (СкладыКонтактнаяОтправитель.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.АдресСклада))
	|ГДЕ
	...
	|	""""
	|ИЗ
	|	Документ.ВозвратТоваровМеждуОрганизациями КАК ДокументРаспоряжение
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады.КонтактнаяИнформация КАК СкладыКонтактнаяОтправитель
	|		ПО ДокументРаспоряжение.Склад = СкладыКонтактнаяОтправитель.Ссылка
	|			И (СкладыКонтактнаяОтправитель.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.АдресСклада))
	|ГДЕ
