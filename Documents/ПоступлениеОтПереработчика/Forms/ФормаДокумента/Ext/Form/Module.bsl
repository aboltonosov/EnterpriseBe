...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьПоступлениеПоНесколькимЗаказам Тогда
		Элементы.ТоварыПодобратьТоварыИзЗаказа.Заголовок = НСтр("ru = 'Подобрать из заказов'");
		Элементы.ВозвратныеОтходыПодобратьТоварыИзЗаказа.Заголовок = НСтр("ru = 'Подобрать из заказов'");
	КонецЕсли;
...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора) 
	...
	ИначеЕсли НоменклатураКлиент.ЭтоУказаниеСерий(ИсточникВыбора) Тогда
		НоменклатураКлиент.ОбработатьУказаниеСерии(ЭтаФорма, ПараметрыУказанияСерий[РезультатВыбора.ИмяТЧ], РезультатВыбора);
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник) 
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияКлиентПереопределяемый.ЕстьНеобработанноеСобытие() Тогда
			ОбработатьШтрихкоды(ПодключаемоеОборудованиеУТКлиент.ПреобразоватьДанныеСоСканераВМассив(Параметр), "Товары");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЧтенииСозданиНаСервере()
	...
	Элементы.Товары.АвтоОтметкаНезаполненного = НЕ Объект.ПоступлениеПоЗаказам;
...
Процедура ОбновитьИнформациюПоЗаказам() 
	...
	Если СписокЗаказов.Количество() > 0 Тогда
		...
		|ГДЕ
		|	ЗаказПереработчикуУслуги.Ссылка В (&Распоряжения)
		|	И ЗаказПереработчикуУслуги.Распоряжение ССЫЛКА Документ.ЭтапПроизводства2_2";
		Запрос.УстановитьПараметр("Распоряжения", СписокЗаказов.ВыгрузитьЗначения());
		
		ОтображатьГиперссылкуЗакрытияЗаказов = Запрос.Выполнить().Пустой();
		
		Элементы.ЗакрытьЗаказ.Видимость  = ОтображатьГиперссылкуЗакрытияЗаказов;
		Элементы.ЗакрытьЗаказы.Видимость = ОтображатьГиперссылкуЗакрытияЗаказов;
...
&НаСервере
Процедура ПолучитьТоварыИзХранилища(ДанныеЗаполнения) 
	...
	ТоварыИзХранилища = ПолучитьИзВременногоХранилища(ДанныеЗаполнения.АдресТоваровВХранилище);
	СтруктураПоискаТоваров = Новый Структура("КодСтроки, ЗаказПереработчику");
	
	Для Каждого СтрокаТоваров Из ТоварыИзХранилища Цикл
		
		ИмяТЧ = ?(СтрокаТоваров.ВозвратныйОтход И НЕ ПродукцияИОтходыВОднойТЧ, "ВозвратныеОтходы", "Товары");
...
Функция ПривязатьСтрокиКЗаказам(ОтобратьПоЗаказу)
	...
	|	ЕСТЬNULL(ЗаказПереработчикуПродукция.НомерСтроки, ЗаказПереработчикуОтходы.НомерСтроки) КАК НомерСтроки,
	|	0                                       КАК Распределено,
	...
	|	НЕ &ПродукцияИОтходыВОднойТЧ И НЕ ЗаказПереработчикуОтходы.Ссылка ЕСТЬ NULL КАК ВозвратныйОтход
	|ИЗ
	|	ЗаказыПереработчикам КАК ЗаказыПереработчикам
...
Процедура УстановитьВидимостьКомандВводНаОсновании()
	...
	ВводНаОсновании.УстановитьВидимостьЭлементаФормыСервер(ЭтотОбъект, "Документ.ОтчетПереработчика", 				Не Объект.ПоступлениеПоЗаказам);
