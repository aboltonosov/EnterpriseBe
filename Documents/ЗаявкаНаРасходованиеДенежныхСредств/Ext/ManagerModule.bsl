...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Если НоваяКоманда <> Неопределено Тогда
		...
		НоваяКоманда.СписокФорм = "ФормаДокумента, ФормаСпискаДокументов, ФормаСписка, ФормаСпискаЗаявокКСогласованию";
	...
	НоваяКоманда = Обработки.ПлатежныйКалендарь.ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании);
	Если НоваяКоманда <> Неопределено Тогда
		НоваяКоманда.Обработчик = "ВводНаОснованииУТКлиент.ОткрытьПомощникФормированияДокументовОплаты";
		НоваяКоманда.СписокФорм = "ФормаСпискаДокументов, ФормаСписка, ФормаСпискаЗаявокКСогласованию";
		НоваяКоманда.РежимИспользованияПараметра = РежимИспользованияПараметраКоманды.Множественный;
	КонецЕсли;
...
Функция ДобавитьКомандуСоздатьНаОснованииОтчетаПоКомиссии_Вознаграждение(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеБезналичныхДенежныхСредств) Тогда		
		КомандаСоздатьНаОсновании = КомандыСоздатьНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Обработчик = "ВводНаОснованииУТКлиент.ЗаявкаНаРасходованиеДсНаОснованииОтчетаПоКомиссии_Вознаграждение";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры,
		ДенежныеСредстваСервер.ДокументыКЗаполнениюФайлаПодтверждающихДокументов());
	//-- НЕ УТ
	...
	|	Документ.ЗаявкаНаРасходованиеДенежныхСредств КАК ЗаявкаНаРасходованиеДенежныхСредств
	|ГДЕ
	|	ЗаявкаНаРасходованиеДенежныхСредств.РегистрацияВНалоговомОргане = ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
	|	И ЗаявкаНаРасходованиеДенежныхСредств.УдалитьРегистрацияВНалоговомОргане <> ЗНАЧЕНИЕ(Справочник.РегистрацииВНалоговомОргане.ПустаяСсылка)
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("ИмяВременнойТаблицы", "ВТкОбработкеЗаяки");
	...
	Запрос = Новый Запрос("
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЗаявкаНаРасходованиеДенежныхСредствПодтверждающиеДокументы.УдалитьПодтверждающийДокумент КАК ИсходныйФайл,
	|	ЗаявкаНаРасходованиеДенежныхСредствПодтверждающиеДокументы.УдалитьПодтверждающийДокумент.Контракт КАК Контракт
	|ИЗ
	|	ВТкОбработкеЗаяки КАК ВТкОбработкеЗаяки
	...
	|		ПО ВТкОбработкеЗаяки.Ссылка = ЗаявкаНаРасходованиеДенежныхСредствПодтверждающиеДокументы.Ссылка");
	
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	РезультатыЗапросов = Запрос.ВыполнитьПакет();
	ВыборкаРеквизитовТЧ = Запрос.Выполнить().Выбрать();
	//-- НЕ УТ
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ДокументОбъект.УдалитьПриоритет);
			...
			//++ НЕ УТ
			// Конвертация ссылок присоединенных файлов из реквизита "УдалитьПодтверждающийДокумент" в "Файл" табличной части "ПодтверждающиеДокументы"
			Для Каждого ПодтверждающийДокумент Из ДокументОбъект.ПодтверждающиеДокументы Цикл
				Если НЕ ЗначениеЗаполнено(ПодтверждающийДокумент.Файл)
					И ЗначениеЗаполнено(ПодтверждающийДокумент.УдалитьПодтверждающийДокумент) Тогда
					
					ВыборкаРеквизитовТЧ.Сбросить();
					СтруктураПоиска = Новый Структура("ИсходныйФайл", ПодтверждающийДокумент.УдалитьПодтверждающийДокумент); 
					Если ВыборкаРеквизитовТЧ.НайтиСледующий(СтруктураПоиска)
						И ЗначениеЗаполнено(ВыборкаРеквизитовТЧ.Контракт) Тогда
					
						ОбщегоНазначенияУТ.СкопироватьПрисоединенныйФайл(
							ВыборкаРеквизитовТЧ.Контракт, 
							ПодтверждающийДокумент.Файл, 
							ПодтверждающийДокумент.УдалитьПодтверждающийДокумент,
							Ложь);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			...
			Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоставщику
				...
				Для Каждого Расшифровка Из ДокументОбъект.РасшифровкаПлатежа Цикл
					...
					Если ЗначениеЗаполнено(Расшифровка.Заказ) И НЕ ЗначениеЗаполнено(Расшифровка.Организация) Тогда
						Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Расшифровка.Заказ, "Организация");
					КонецЕсли;
			...
			Если Не ЗначениеЗаполнено(ДокументОбъект.РегистрацияВНалоговомОргане) Тогда
				ДокументОбъект.РегистрацияВНалоговомОргане = ДокументОбъект.УдалитьРегистрацияВНалоговомОргане;
			КонецЕсли;
