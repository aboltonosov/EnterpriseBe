...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаТаблицы Из ТекущийОбъект.СведенияОДоходах Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТаблицы Из ТекущийОбъект.СведенияОВычетах Цикл
		Если СотрудникиДокумента.Найти(СтрокаТаблицы.Сотрудник) = Неопределено Тогда
			Если УдаляемыеСотрудники.Найти(СтрокаТаблицы.Сотрудник) = Неопределено Тогда
				УдаляемыеСотрудники.Добавить(СтрокаТаблицы.Сотрудник);
			КонецЕсли;
	...
	Для каждого УдаляемыйСотрудник Из УдаляемыеСотрудники Цикл
		НайденныеСтроки = ТекущийОбъект.СведенияОДоходах.НайтиСтроки(Новый Структура("Сотрудник", УдаляемыйСотрудник));
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ТекущийОбъект.СведенияОДоходах.Удалить(НайденнаяСтрока);
		КонецЦикла;
		НайденныеСтроки = ТекущийОбъект.СведенияОВычетах.НайтиСтроки(Новый Структура("Сотрудник", УдаляемыйСотрудник));
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ТекущийОбъект.СведенияОДоходах.Удалить(НайденнаяСтрока);
		КонецЦикла;
	КонецЦикла;
...
Процедура ПроверкаСтороннейПрограммойКаталогПрограммыНеОбнаружен(ПараметрыПроверки)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаВыбораПроверочнойПрограммы", , ЭтотОбъект, , , ,Оповещение , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СотрудникиПередУдалениемНаСервере()
	...
	Для Каждого ИдентификаторСтроки Из Элементы.Сотрудники.ВыделенныеСтроки Цикл
		...
		НайденныеСтроки = Объект.СведенияОВычетах.НайтиСтроки(СтруктураОтбора);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Объект.СведенияОВычетах.Удалить(НайденнаяСтрока);
		КонецЦикла;
		
		НайденныеСтроки = Объект.СведенияОДоходах.НайтиСтроки(СтруктураОтбора);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Объект.СведенияОДоходах.Удалить(НайденнаяСтрока);
		КонецЦикла;
...
Функция СотрудникиДляЗаполненияСправки()
	...
	Запрос.УстановитьПараметр("ГоловнаяОрганизация", ЗарплатаКадрыПовтИсп.ГоловнаяОрганизация(Объект.Организация));
	...
	ИначеЕсли Объект.НомерКорректировки > 0 Тогда
		...
		// Выбираем тех, по кому ранее подавали справки (соотв. документ помечен как переданный), на после этого вводились данные о доходах или налогах
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Сотрудник,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СправкиНДФЛДляПередачиВНалоговыйОрган.Сотрудники КАК СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники
		|		ПО ДатыПоследнихСправок.Сотрудник = СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Сотрудник
		|			И ДатыПоследнихСправок.ДатаДокумента = СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Ссылка.Дата
		|			И (СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Ссылка.Организация = &Организация)
		|			И (СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Ссылка.РегистрацияВНалоговомОргане = &РегистрацияВНалоговомОргане)
		|			И (СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Ссылка.КППОтправки = &КППОтправки)
		|			И (СправкиНДФЛДляПередачиВНалоговыйОрганСотрудники.Ссылка.НалоговыйПериод = &НалоговыйПериод)
	...
	Иначе // исходные (обычные) справки
		...
		|				И (СправкиНДФЛДляПередачиВНалоговыйОрган.ОснованиеДляПредставленияСправок = ЗНАЧЕНИЕ(Перечисление.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетность))
