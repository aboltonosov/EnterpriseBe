...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаТаблицаПланыСборкиРазборки(Запрос, ТекстыЗапроса, Регистры, ДокументСсылка);
...
Функция ТекстЗапросаТаблицаПланыСборкиРазборки(Запрос, ТекстыЗапроса, Регистры, ДокументСсылка)
	...
	Замещающий = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Замещающий");
	СтатусЗамещения = Планирование.ПолучитьСтатусЗамещения(ДокументСсылка);
	
	Если Замещающий и СтатусЗамещения <> Неопределено Тогда
		
		Запрос.УстановитьПараметр("СтатусЗамещения", СтатусЗамещения);
		
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ВложенныйЗапрос.ПланСборкиРазборки,
		...
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПланСборкиРазборкиТовары.Ссылка КАК ПланСборкиРазборки,
		|		ПланСборкиРазборкиТовары.НомерСтроки КАК НомерСтроки,
		|		ВЫБОР
		|			КОГДА ПланСборкиРазборкиТовары.ДатаСборкиРазборки = ДАТАВРЕМЯ(1, 1, 1)
		|				ТОГДА &НачалоПериода
		|			ИНАЧЕ ПланСборкиРазборкиТовары.ДатаСборкиРазборки
		|		КОНЕЦ КАК Период,
		|		&Сценарий КАК Сценарий,
		|		ПланСборкиРазборкиТовары.Номенклатура КАК Номенклатура,
		|		ПланСборкиРазборкиТовары.Характеристика КАК Характеристика,
		|		ВЫБОР
		...
		|			ИНАЧЕ ПланСборкиРазборкиТовары.Назначение
		|		КОНЕЦ КАК Назначение,
		|		ВЫБОР
		|			КОГДА СпрНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
		|					ИЛИ СпрНоменклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
		|				ТОГДА ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
		|			ИНАЧЕ ПланСборкиРазборкиТовары.Склад
		|		КОНЕЦ КАК Склад,
		|		&СтатусЗамещения КАК Статус,
		|		&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
		|		ВЫБОР
		|			КОГДА ПланСборкиРазборкиТовары.Отменено
		|				ТОГДА 0
		|			ИНАЧЕ ПланСборкиРазборкиТовары.Количество
		|		КОНЕЦ КАК Количество
		|	ИЗ
		|		Документ.ПланСборкиРазборки.Товары КАК ПланСборкиРазборкиТовары
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СпрНоменклатура
		|			ПО ПланСборкиРазборкиТовары.Номенклатура = СпрНоменклатура.Ссылка
		|	ГДЕ
		|		ПланСборкиРазборкиТовары.Ссылка = &Ссылка
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		ПланыСборкиРазборкиОбороты.ХозяйственнаяОперация,
		...
		|				&ОкончаниеПериода,
		|				Запись,
		|				Статус = &СтатусЗамещения
		|					И Сценарий = &Сценарий
		|					И ХозяйственнаяОперация = &ХозяйственнаяОперация
		|					И (&ЗаполнятьНазначениеВТЧ Или Назначение = &Назначение)
		|					И ПланСборкиРазборки <> &Ссылка) КАК ПланыСборкиРазборкиОбороты) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.ПланСборкиРазборки,
		|	ВложенныйЗапрос.Период,
...
Функция ЗапросЗамещения(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|				И (&ЗаполнятьНазначениеВТЧ Или Назначение = &Назначение)
		|				И Статус В (&Статусы)
		|				И Сценарий = &Сценарий) КАК ПланыСборкиРазборкиОбороты";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ТекстСообщения = НСтр("ru = 'Не удалось обработать: %ПланСборкиРазборки% по причине: %Причина%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПланСборкиРазборки%", Выборка.Ссылка);
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Причина%", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Предупреждение,
									Метаданные.Документы.ПланСборкиРазборки, ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
