...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	|	ДанныеДокумента.Руководитель.Наименование КАК Разрешил,
	|	ДанныеДокумента.Руководитель.Должность КАК РазрешилДолжность,
	|	ДанныеДокумента.Принял КАК Принял,
	|	ДанныеДокумента.ПереданыДокументы КАК ПереданыДокументы,
	// }
	// 4D
	|	ДанныеДокумента.Ссылка КАК Ссылка,
	...
	|	Ссылка
	|;
	// 4D:ERP для Беларуси, ВладимирР, 01.03.2017 15:28:31 
	// ERP. Печатная форма ТН документ Передача товара, №14423
	// {
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	КоэффициентыУпаковок.Номенклатура,
	|	КоэффициентыУпаковок.КоличествоУпаковок КАК КоличествоУпаковок,
	|	КоэффициентыУпаковок.Количество КАК Количество,
	|	КоэффициентыУпаковок.Упаковка,
	|	КоэффициентыУпаковок.НомерСтроки КАК НомерСтроки,
	|	КоэффициентыУпаковок.КоэффициентУпаковки КАК КоэффициентУпаковки,
	|	КоэффициентыУпаковок.КоэффициентВложеннойУпаковки КАК КоэффициентВложеннойУпаковки,
	|	КоэффициентыУпаковок.ВидУпаковки,
	|	КоэффициентыУпаковок.Ссылка
	|ПОМЕСТИТЬ ТЗКоэффициентыУпаковок
	|ИЗ
	...
	// ERP. Печатная форма ТН документ Передача товара, №14423
	// {
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТЗКоэффициентыУпаковок КАК КоэффициентыУпаковок
	|		ПО ТаблицаТоваров.Ссылка = КоэффициентыУпаковок.Ссылка
	|			И ТаблицаТоваров.НомерСтроки = КоэффициентыУпаковок.НомерСтроки
	|			И ТаблицаТоваров.Количество = КоэффициентыУпаковок.Количество
...
Функция СтруктураОснованияДляПечати(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ
			|	ДоговорыМеждуОрганизациями.НаименованиеДляПечати КАК Основание,
			|	ДоговорыМеждуОрганизациями.Дата КАК ОснованиеДата,
			|	ДоговорыМеждуОрганизациями.Номер КАК ОснованиеНомер
			|ИЗ
