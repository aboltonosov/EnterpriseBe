	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл 
			...
			ИначеЕсли Выборка.ИсточникОтчета = "РегламентированныйОтчетЛьготаНДПИ" Тогда 
				Продолжить;
				Отчеты["РегламентированноеУведомлениеНалоговаяЛьготаРегИнвестПроекты"].СоздатьСконвертированныйОтчетЛьготаНДПИ(Выборка);
			КонецЕсли;
		КонецЦикла;
	Исключение
...
Процедура ОбработкаПолученияПолейПредставления(Поля, СтандартнаяОбработка) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 07.12.2015 9:26:40 
