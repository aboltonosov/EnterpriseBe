...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// 4D:ERP для Беларуси, Яна, 08.09.2017 12:29:56 
	// Не считается ФОТ в документе "прием на работу списком" , №15983
	// {
	//Если ИмяСобытия = "ИзмененСтажФизическогоЛица" И Источник.ВладелецФормы = ЭтаФорма Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПроведениеДокументаПриемНаРаботу");
...
Процедура ДополнитьФорму(ОтложенноеИзменение = Неопределено)
	...
	Иначе
		...
		// 4D:ERP для Беларуси, Яна, 12.06.2017 12:37:46 
...
Процедура ДанныеВРеквизит()
	...
	Если Параметры.Свойство("Ключ") И Не ЭтаФорма.Параметры.Ключ.Пустая() Тогда
		ИсправлениеДокументовЗарплатаКадры.ПрочитатьРеквизитыИсправления(ЭтаФорма, "ПериодическиеСведения");
	КонецЕсли;
	ИсправлениеДокументовЗарплатаКадры.УстановитьПоляИсправления(ЭтаФорма, "ПериодическиеСведения");
...
Процедура ЗаполнитьПоПозицииНаСервере()
	...
	// 4D:ERP для Беларуси, Яна, 12.06.2017 10:34:30 
	// Документ "Прием на работу", №14926 
	// {
...
Процедура ЗаполнитьРазрядПоУмолчанию()
	...
	// 4D:ERP для Беларуси, Петр, 28.08.2017 15:18:49 
	// Прием на работу. Неправильно рассчитана сумма ФОТ, №15712
	// {
	Объект.РазрядКатегория = Объект.ДолжностьПоШтатномуРасписанию.РазрядКатегория;
	// }
	// 4D
...
Процедура РассчитатьИтогиПоФОТ(Форма)
	...
	#Если НЕ ТонкийКлиент И НЕ ВебКлиент И НЕ ТолстыйКлиентУправляемоеПриложение Тогда
		ПодсистемаУправленческаяЗарплатаСуществует = ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата");
	#Иначе
		ПодсистемаУправленческаяЗарплатаСуществует = ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата");
	#КонецЕсли
...
// 4D:ERP для Беларуси, Яна, 07.08.2017 10:56:37 
// Результат тестирования конфигурации, №15540 
// {
&НаКлиенте
Процедура ПриИзмененииКоличестваСтавок(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура УстановитьОтображениеПодсказкиДлительностиИспытательногоСрока(Форма)
	...
	Если ЗначениеЗаполнено(Форма.Объект.ДлительностьИспытательногоСрока) Тогда
		...
		// 4D:ERP для Беларуси, Анна, 06.06.2017 17:44:16 
		// Изменить надпись, если испытательный срок превышает 3 месяца, №15084
		// {
		Если Форма.Объект.ДлительностьИспытательногоСрока > 3 Тогда			
			ТекстПодсказки = НСтр("ru='Срок предварительного испытания не должен превышать трех месяцев, не считая периода временной нетрудоспособности и других периодов, когда работник отсутствовал на работе'");
		КонецЕсли;
		// }
		// 4D 
...
// 4D:ERP для Беларуси, Яна, 12.06.2017 10:31:55 
// Документ "Прием на работу", №14926 
// {
&НаСервере
Процедура УстановитьЗначениеРайонногоКоэффициентаВФорме()
	
	ЗарплатаКадрыРасширенный.УстановитьЗначениеРайонногоКоэффициентаВФорме(
		ЭтаФорма, Объект.Сотрудник, ВремяРегистрации, Объект.Подразделение, Объект.Организация, Объект.Территория, Объект.Должность);
