...
#Область КорректировкаПоступления
	...
	Для Каждого СтрТЧ Из ДокументыОснования Цикл
		...
		Если ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПоступления") Тогда
	...
	ИначеЕсли ЕстьИсправления И Не Исправление И Не СводныйКорректировочный Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	// ЭСЧФ
	// {
	Для Каждого ТекущийДокумент Из ДокументыОснования Цикл
		ДокументОснование = ТекущийДокумент.ДокументОснование;
		Прервать;
	КонецЦикла;
	// }
	// 4D
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		ИначеЕсли ДанныеЗаполнения.Свойство("Корректировочный") И ДанныеЗаполнения.Корректировочный
			...
			И ТипЗнч(ДанныеЗаполнения.ДокументОснование) <> Тип("ДокументСсылка.КорректировкаПоступления") 
...
Функция ПолучитьПараметрыСчетаФактурыПоОснованиям()
	...
	Результат = Новый Структура("Организация, Контрагент, КППКонтрагента, Валюта,
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ИмяОбъекта = "КорректировкаПоступления" Тогда
			...
			|ИЗ
			|	Документ.КорректировкаПоступления КАК ТекущийДокумент
			|ГДЕ
			|	ТекущийДокумент.Ссылка В (&ДокументОснование_КорректировкаПоступления)
			...
			|		ПО ОснованияИсходных.ДокументОснование = КорректировкаПоступления.ДокументОснование
	...
	Если ВыборкаИсходныхДанных <> Неопределено Тогда
		...
		Если Результат.Корректировочный Тогда
			...
			Если Результат.Исправление Тогда
				...
				МассивКорректировок = Запрос.Параметры.ДокументОснование_КорректировкаПоступления;
