...
Процедура ТекстПродавцыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылка = "ОткрытьФормуПродавцы" Тогда
		...
		Для каждого Строка Из Объект.Продавцы Цикл
			СписокПродавцов.Добавить(Строка.Продавец);
		КонецЦикла;
...
Процедура ДатаПолученияПриИзмененииКлиент()
	...
	// СтандартныеПодсистемы.РаботаСКонтрагентами
	Если Не ЗначениеЗаполнено(Объект.ДатаСоставления) Тогда
...
Процедура ПродавецЗавершениеВыбора(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьПродавцов(СписокПродавцов);
	
КонецПроцедуры

&НаСервере 
Процедура ЗаполнитьПродавцов(СписокПродавцов)
...
Процедура ГруппаСтраницыПриСменеСтраницыНаСервере()
	...
	ЭД = ЭлектронныеСчетаФактуры.ПолучитьЭДПоВладельцу(Объект.Ссылка);
	Если ЗначениеЗаполнено(ЭД) Тогда
		ТабличныйДокументЭСЧФ = ЭлектронныеСчетаФактуры.ПолучитьДанныеДляПечати(ЭД);
	КонецЕсли;	
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	Если ТекущаяСтраница = Элементы.ГруппаСтраницаЭСЧФ Тогда
		ГруппаСтраницыПриСменеСтраницыНаСервере();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ДокументыОснованияДокументОснованиеПриИзмененииНаСервере(ДокументОснование)
	
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		Попытка
			ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОснование, "СуммаДокумента");
			//ТекущаяСтрока.СуммаНДС = ЗначенияРеквизитов.СуммаНДС;
			//ТекущаяСтрока.СуммаДокумента = ЗначенияРеквизитов.СуммаДокумента;
		Исключение
		КонецПопытки;	
	Иначе
		//ТекущаяСтрока.СуммаНДС = 0;
		//ТекущаяСтрока.СуммаДокумента = 0;
	КонецЕсли;	
