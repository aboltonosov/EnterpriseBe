...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	ПродажиСервер.ПоместитьВременнуюТаблицуКоэффициентыУпаковок(МенеджерВременныхТаблиц, "ВозвратСырьяДавальцуТаблицаТоваров");
	ОтветственныеЛицаСервер.СформироватьВременнуюТаблицуОтветственныхЛицДокументов(МассивОбъектов, МенеджерВременныхТаблиц);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВозвратСырьяДавальцу.Ссылка КАК Ссылка,
	|	ВозвратСырьяДавальцу.Номер КАК Номер,
	|	ВозвратСырьяДавальцу.Дата КАК Дата,
	|	ВозвратСырьяДавальцу.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	ВозвратСырьяДавальцу.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	ВозвратСырьяДавальцу.Отпустил КАК Отпустил,
	|	ВозвратСырьяДавальцу.ОтпустилДолжность КАК ОтпустилДолжность,
	|	ВозвратСырьяДавальцу.Руководитель.Наименование КАК Разрешил,
	|	ВозвратСырьяДавальцу.Руководитель.Должность КАК РазрешилДолжность,
	|	ВозвратСырьяДавальцу.Принял КАК Принял,
	|	ВозвратСырьяДавальцу.ПереданыДокументы КАК ПереданыДокументы,
	|	ВозвратСырьяДавальцу.Организация КАК Организация,
	|	ВозвратСырьяДавальцу.Грузополучатель КАК Грузополучатель,
	|	ВозвратСырьяДавальцу.Грузоотправитель КАК Грузоотправитель,
	|	ТаблицаОтветственныеЛица.РуководительНаименование КАК Руководитель,
	|	ТаблицаОтветственныеЛица.РуководительДолжность КАК ДолжностьРуководителя,
	|	ТаблицаОтветственныеЛица.ГлавныйБухгалтерНаименование КАК ГлавныйБухгалтер,
	|	ВозвратСырьяДавальцу.Организация.Префикс КАК Префикс,
	|	ВозвратСырьяДавальцу.Основание КАК Основание,
	|	ВозвратСырьяДавальцу.ОснованиеДата КАК ОснованиеДата,
	|	ВозвратСырьяДавальцу.ОснованиеНомер КАК ОснованиеНомер,
	|	ВозвратСырьяДавальцу.Валюта КАК Валюта,
	|	ВозвратСырьяДавальцу.ДоверенностьНомер КАК ДоверенностьНомер,
	|	ВозвратСырьяДавальцу.ДоверенностьДата КАК ДоверенностьДата,
	|	ВозвратСырьяДавальцу.ДоверенностьВыдана КАК ДоверенностьВыдана,
	|	ВозвратСырьяДавальцу.ДоверенностьЛицо КАК ДоверенностьЛицо,
	|	&ЕдиницаИзмеренияВеса КАК ЕдиницаИзмеренияВеса
	|ИЗ
	|	Документ.ВозвратСырьяДавальцу КАК ВозвратСырьяДавальцу
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ПО ВозвратСырьяДавальцу.Ссылка = ДанныеДокументов.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОтветственныеЛица КАК ТаблицаОтветственныеЛица
	|		ПО ВозвратСырьяДавальцу.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка
	|;
	...
	|ВЫБРАТЬ
	|	ВозвратСырьяДавальцу.Ссылка КАК Ссылка,
	|	ВозвратСырьяДавальцу.Номенклатура КАК Номенклатура,
	|	ВозвратСырьяДавальцу.Характеристика.НаименованиеПолное КАК Характеристика,
	|	&ТекстЗапросаНаименованиеЕдиницыИзмерения КАК ЕдиницаИзмеренияНаименование,
	|	ВозвратСырьяДавальцу.КоличествоУпаковок КАК Количество,
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС) КАК СтавкаНДС,
	|	ВозвратСырьяДавальцу.СуммаБезНДС / ВозвратСырьяДавальцу.КоличествоУпаковок КАК Цена,
	|	ВозвратСырьяДавальцу.СуммаБезНДС КАК СуммаБезНДС,
	|	ВозвратСырьяДавальцу.СуммаНДС КАК СуммаНДС,
	|	ВозвратСырьяДавальцу.СуммаБезНДС + ВозвратСырьяДавальцу.СуммаНДС КАК СуммаСНДС,
	|	ВозвратСырьяДавальцу.ЭтоВозвратнаяТара КАК ЭтоВозвратнаяТара
	|ИЗ
	|	ВозвратСырьяДавальцуТаблицаТоваров КАК ВозвратСырьяДавальцу
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоэффициентыУпаковок КАК КоэффициентыУпаковок
	|		ПО ВозвратСырьяДавальцу.Ссылка = КоэффициентыУпаковок.Ссылка
	|			И ВозвратСырьяДавальцу.НомерСтроки = КоэффициентыУпаковок.НомерСтроки
	|			И ВозвратСырьяДавальцу.КоличествоУпаковок = КоэффициентыУпаковок.КоличествоУпаковок
	|			И (НЕ &ВыводитьБазовыеЕдиницыИзмерения)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВозвратСырьяДавальцу.Ссылка,
	|	ВозвратСырьяДавальцу.НомерСтроки
	|ИТОГИ ПО
	|	Ссылка";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаНаименованиеЕдиницыИзмерения",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения("Наименование", "ВозвратСырьяДавальцу.Упаковка", "ВозвратСырьяДавальцу.Номенклатура"));
	...
	РезультатПоТабличнойЧасти = РезультатПакетаЗапросов[1];
