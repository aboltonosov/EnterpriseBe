...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Дмитрий, 04.11.2017 11:33:51 
	// отключаем
	// {
...
&НаКлиенте
Процедура ВывестиСообщенияПользователюВыгружаемыеДанные(ВыгружаемыеДанные)
	Для Каждого ФайлВыгрузки Из ВыгружаемыеДанные Цикл
		Для каждого ТекстМассива Из ФайлВыгрузки.ЗамечанияОбОшибках Цикл
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ТекстМассива;
			Сообщение.Сообщить();
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры
...
Процедура ВыгрузитьЗаявлениеОВвозе(Команда)
	...
	Если НЕ ВыгружаемыеДанные = Неопределено Тогда
		...
		// Выгрузка заявления о ввозе товаров из ЕАЭС, №10718
		// {
		ВывестиСообщенияПользователюВыгружаемыеДанные(ВыгружаемыеДанные);
		// }
		// 4D
...
Процедура ОткрытьДокументОплаты(СсылкаНаДокумент, НаимнованиеОбъектаМетаданных)
	...
	Иначе
		ОткрытьФорму("Документ." + НаимнованиеОбъектаМетаданных + ".ФормаОбъекта", Новый Структура("Основание", Объект.Ссылка));
	КонецЕсли;
