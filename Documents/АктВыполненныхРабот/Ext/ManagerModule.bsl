...
Функция РеквизитыДокумента(ДокументСсылка) Экспорт
	...
	СтруктураРеквизитов = Новый Структура("Дата, Организация, Партнер, Контрагент, Договор, ПорядокРасчетов, Валюта, ВалютаВзаиморасчетов, ПоЗаказу, СуммаДокумента, СуммаВзаиморасчетов, ХозяйственнаяОперация, Курс, Кратность",
		Дата,
		Организация,
		Партнер,
		Контрагент,
...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	|
	// 4D:ERP для Беларуси, Екатерина, 26.04.2016 13:39:42 
	// Расчет скидки к цене, №10910
	// {
	|	СкидкиНаценки.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	// }
	// 4D
	|
	...
	|	СкидкиНаценки.СкидкаНаценка КАК СкидкаНаценка,
	|
	// 4D:ERP для Беларуси, Екатерина, 26.04.2016 13:39:52 
	// Расчет скидки к цене, №10910
	// {
	|	СкидкиНаценки.ПроцентСкидкиНаценки КАК ПроцентСкидкиНаценки,
	// }
	...
	Пока ВыборкаУслуги.Следующий() Цикл
		...
		Если ЗаказНайден И РеквизитыЗаказов.ПересчитатьВВалютуДокумента Тогда
			...
			Если КурсВалюты.Количество() = 1 Тогда
				...
				// 4D:ERP для Беларуси, Екатерина, 26.04.2016 13:45:40 
				// Расчет скидки к цене, №10910
				// {
				СтрокаТаб.ЦенаСоСкидкой = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(
					СтрокаТаб.ЦенаСоСкидкой,
					РеквизитыЗаказов.Валюта, ДанныеОтбора.Валюта,
					КурсВалюты[0].Курс,СтруктураКурсовНовойВалюты.Курс,
					КурсВалюты[0].Кратность,СтруктураКурсовНовойВалюты.Кратность);
				// }
...
Процедура ИнициализироватьВидыЗапасов(Запрос)
	...
	ЗапросВидовЗапасов.УстановитьПараметр("АктПоЗаказам",	Запрос.Параметры.АктПоЗаказам);
...
Процедура ИнициализироватьКлючиАналитикиНоменклатуры(Запрос)
	...
	|			ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) КАК Серия,
	|			ВЫБОР
	|				КОГДА ТаблицаУслуги.ЗаказКлиента = НЕОПРЕДЕЛЕНО
	...
	|			ПО ТоварыЗаказа.Ссылка = ТаблицаУслуги.ЗаказКлиента
	|				И ТоварыЗаказа.КодСтроки = ТаблицаУслуги.КодСтроки
	|		ГДЕ
	|			ТаблицаУслуги.Ссылка = &Ссылка
...
Функция ТекстЗапросаВтАналитика(Запрос, ТекстыЗапроса)
	...
	|			ВЫБОР
	|				КОГДА ТаблицаУслуги.ЗаказКлиента = НЕОПРЕДЕЛЕНО
	|						ИЛИ ТаблицаУслуги.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|					ТОГДА ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|				КОГДА ТоварыЗаказа.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно)
	|					ТОГДА ТоварыЗаказа.Ссылка.Назначение
	|				ИНАЧЕ
	...
	|		Документ.ЗаказКлиента.Товары КАК ТоварыЗаказа
	|	ПО
	|		ТоварыЗаказа.Ссылка = ТаблицаУслуги.ЗаказКлиента
	|		И ТоварыЗаказа.КодСтроки = ТаблицаУслуги.КодСтроки
	|
	|ГДЕ
	|	ТаблицаУслуги.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО ТаблицаУслуги.АналитикаУчетаНоменклатуры = Аналитика.КлючАналитики
	|ГДЕ
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, МассивЗаказов = Неопределено)
	...
	|
	// 4D:ERP для Беларуси, Екатерина, 26.04.2016 13:42:30 
	// Расчет скидки к цене, №10910
	// {
	...
	|	ЗаказТовары.Цена,
	|
	// 4D:ERP для Беларуси, Екатерина, 26.04.2016 13:43:24 
	// Расчет скидки к цене, №10910
	// {
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 08.08.2015 7:54:47 
	// ТТН-1 и ТН-2 в валюте, №10143
	// {
	ПересчитыватьВВалютуРегл = Ложь;
	// }
	// 4D
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	НастройкаПечати = КомплектПечатныхФорм.Найти("АктВыполненныхРаботБезСкидок", "Имя");
	Если НастройкаПечати <> Неопределено И НастройкаПечати.Печатать Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекцию(КоллекцияПечатныхФорм, НастройкаПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			НастройкаПечати.Имя,
			НастройкаПечати.Представление,
			СформироватьПечатнуюФормуАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати, Новый Структура("ОтображатьСкидки", Ложь)));
	КонецЕсли;
	...
	НастройкаПечати = КомплектПечатныхФорм.Найти("УПДАктВыполненныхРабот", "Имя");
	Если НастройкаПечати <> Неопределено И НастройкаПечати.Печатать Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекцию(КоллекцияПечатныхФорм, НастройкаПечати);
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
...
// заполняет реквизиты "Порядок оплаты", "Курс" и "Кратность" документа "АктВыполненныхРабот".
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.АктВыполненныхРабот КАК АктВыполненныхРабот
	|ГДЕ АктВыполненныхРабот.ПорядокОплаты = Значение(Перечисление.ПорядокОплатыПоСоглашениям.ПустаяСсылка)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.АктВыполненныхРабот";
