...
// 4D:ERP для Беларуси, ВладимирР, 28.07.2015 14:55:07 
// Списание НМА (+ акт о списании НМА), №10112
// {
#Область Печать

#Область Печать_АктНаСписаниеНМА

Функция СформироватьПечатнуюФормуАктНаСписаниеНМА(МассивОбъектов, ОбъектыПечати, КомплектыПечати, ПараметрыПечати, ИмяМакета)
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_СписаниеНМА_Локализация";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.СписаниеНМА.ПФ_MXL_Списание_НМА_Локализация");
	ВалютаРегл = Константы.ВалютаРегламентированногоУчета.Получить();
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СписаниеНМА.Ссылка КАК Ссылка,
	|	СписаниеНМА.Номер КАК НомерАкта,
	|	СписаниеНМА.Организация КАК Организация,
	|	СписаниеНМА.Подразделение КАК Подразделение,
	|	СписаниеНМА.НематериальныйАктив КАК НематериальныйАктив,
	...
	|	ПервоначальныеСведенияНМА.ПервоначальнаяСтоимость КАК БалансоваяСтоимость,
	|	ОтражениеДокументовВРеглУчете.Статус КАК Статус
	// 4D:ERP для Беларуси, Петр, 13.11.2017 15:23:24 
	...
	|		ГДЕ
	|			(ТИПЗНАЧЕНИЯ(ПервоначальныеСведенияНМАБухгалтерскийУчет.Регистратор) = ТИП(Документ.ПринятиеКУчетуНМА)
	|					ИЛИ ТИПЗНАЧЕНИЯ(ПервоначальныеСведенияНМАБухгалтерскийУчет.Регистратор) = ТИП(Документ.ВводОстатковВнеоборотныхАктивов))) КАК ПервоначальныеСведенияНМА
	|		ПО СписаниеНМА.Организация = ПервоначальныеСведенияНМА.Организация
	|			И СписаниеНМА.НематериальныйАктив = ПервоначальныеСведенияНМА.НематериальныйАктив
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтражениеДокументовВРеглУчете КАК ОтражениеДокументовВРеглУчете
	|		ПО СписаниеНМА.Ссылка = ОтражениеДокументовВРеглУчете.Регистратор
	|ГДЕ
	|	СписаниеНМА.Ссылка В(&МассивСсылок)
	|
	|УПОРЯДОЧИТЬ ПО
	|	СписаниеНМА.МоментВремени";
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		ДатаДок = Формат(ВыборкаДокумент.ДатаДок, "ДФ=dd.MM.yyyy");
		СведенияОбОрганизации = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ВыборкаДокумент.Организация, ВыборкаДокумент.ДатаДок);
		
		ОблШапка.Параметры.Заполнить(ВыборкаДокумент);
		ОблШапка.Параметры.Организация = СведенияОбОрганизации.ПолноеНаименование;
		ОблШапка.Параметры.НомерАкта = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаДокумент.НомерАкта, Истина, Истина);
		ОблШапка.Параметры.НМА = ВыборкаДокумент.НаименованиеПолное;
		ОблШапка.Параметры.ДатаДок = ДатаДок;
		
		ОтветственныеЛица = ОтветственныеЛицаБП.ОтветственныеЛица(ВыборкаДокумент.Организация, ВыборкаДокумент.ДатаДок);
		ОблШапка.Параметры.Руководитель = ОтветственныеЛица.РуководительПредставление;
		ОблШапка.Параметры.РуководительДолжность = ОтветственныеЛица.РуководительДолжностьПредставление;
		
		ТабДокумент.Вывести(ОблШапка);
		
		ОблСтрока.Параметры.Заполнить(ВыборкаДокумент);
		
		Если ВыборкаДокумент.Статус = Перечисления.СтатусыОтраженияДокументовВРеглУчете.ОтраженоВРеглУчете Тогда
			ВидыСубконто = Новый Массив;
			ВидыСубконто.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.НематериальныеАктивы);
			
			ЗапросБУ = Новый Запрос;
			ЗапросБУ.УстановитьПараметр("ВидыСубконто", ВидыСубконто);
			ЗапросБУ.УстановитьПараметр("НематериальныйАктив", ВыборкаДокумент.НематериальныйАктив);
			ЗапросБУ.УстановитьПараметр("Организация", ВыборкаДокумент.Организация);
			ЗапросБУ.УстановитьПараметр("Подразделение", ВыборкаДокумент.Подразделение);
			ЗапросБУ.Текст =
			"ВЫБРАТЬ
			...
			|			,
			|			,
			|			СчетКт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.АмортизацияНематериальныхАктивов),
			|			&ВидыСубконто,
			|			Организация = &Организация
			|				И ПодразделениеКт = &Подразделение
			|				И СубконтоКт1 = &НематериальныйАктив) КАК ХозрасчетныйОборотыДтКт";
			РезультатБУ = ЗапросБУ.Выполнить();
			ВыборкаБУ = РезультатБУ.Выбрать();
			ВыборкаБУ.Следующий();
			АмортизацияРасход = ВыборкаБУ.СуммаОборот;
			
			Если АмортизацияРасход > 0 Тогда
				ОблСтрока.Параметры.АмортизацияРасход = АмортизацияРасход;
				СтоимостьСписания = ВыборкаДокумент.БалансоваяСтоимость - АмортизацияРасход;
				ОблПодвал2.Параметры.ТекстОст = Строка(СтоимостьСписания) + " (" + СокрЛП(РаботаСКурсамиВалют.СформироватьСуммуПрописью(СтоимостьСписания, ВалютаРегл, Истина)) + ")";
			КонецЕсли;
			
			ТабДокумент.Вывести(ОблСтрока);
		Иначе
			ОблСтрока.Параметры.АмортизацияРасход = "не отражено в регл. учете...";
			ТабДокумент.Вывести(ОблСтрока);
			
			ТабДокумент.Область(21, 53, 21, 59).ГоризонтальноеПоложение = ГоризонтальноеПоложение.Центр;
			ТабДокумент.Область(21, 53, 21, 59).ЦветТекста = Новый Цвет(255, 0, 0);
		КонецЕсли;
		
		ТабДокумент.Вывести(ОблПодвал1);
		
		ОблПодвал2.Параметры.ГлавБух = ОтветственныеЛица.ГлавныйБухгалтерПредставление;
		ТабДокумент.Вывести(ОблПодвал2);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ПФ_MXL_Списание_НМА_Локализация";
	КомандаПечати.Представление = НСтр("ru = 'Акт на списание'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_Списание_НМА_Локализация") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ПФ_MXL_Списание_НМА_Локализация", НСтр("ru='Акт на списание'"),
			СформироватьПечатнуюФормуАктНаСписаниеНМА(МассивОбъектов, ОбъектыПечати, Неопределено, ПараметрыПечати, "ПФ_MXL_Списание_НМА_Локализация"));
	КонецЕсли;
