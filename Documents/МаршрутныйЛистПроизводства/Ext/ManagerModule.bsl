...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ИспользуетсяСправочноеУказаниеСерий = НоменклатураСервер.ИспользуетсяСправочноеУказаниеСерийВМаршуртныхЛистах(
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийМатериалы()
	...
	|					ИНАЧЕ 5
	|				КОНЕЦ
	|		КОГДА ПолитикиУчетаСерий.ПолитикаУчетаСерий.УказыватьДляМатериаловПриВыполненииМаршрутныхЛистов
	|			ТОГДА ВЫБОР
	|					КОГДА Товары.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка) И НЕ &ФактОтбора И НЕ Товары.МатериалИзрасходован
	|						ТОГДА 21
	|					КОГДА Товары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийИзделия()
	...
	|					И ПолитикиУчетаСерий.ПолитикаУчетаСерий.УказыватьДляИзделийПриВыполненииМаршрутныхЛистов)
	|			ТОГДА ВЫБОР
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		МестаУчета = РегистрыСведений.АналитикаУчетаНоменклатуры.МестаУчета(Неопределено, ДокОбъект.Подразделение, ДокОбъект.Подразделение, Неопределено);
		
		ПараметрыЗаполненияКлючейМатериалов = РегистрыСведений.АналитикаУчетаНоменклатуры.ПараметрыЗаполненияКлючейАналитики();
		РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокОбъект.МатериалыИУслуги, МестаУчета, Неопределено, Ложь, ПараметрыЗаполненияКлючейМатериалов);
		
		ПараметрыЗаполненияКлючейПродукции = РегистрыСведений.АналитикаУчетаНоменклатуры.ПараметрыЗаполненияКлючейАналитики();
		РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокОбъект.ВыходныеИзделия, МестаУчета, Неопределено, Ложь, ПараметрыЗаполненияКлючейПродукции);
		
		Попытка
			Если ПараметрыЗаполненияКлючейМатериалов.ИзмененаАналитика ИЛИ ПараметрыЗаполненияКлючейПродукции.ИзмененаАналитика Тогда
