...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.Ключ) Тогда
		...
		// 4D:ERP для Беларуси, Яна, 17.07.2017 12:57:33 
		// Ошибка открытия документа "Перевод к другому работадателю" , №15464
		// {
		Объект.СтатьяТКРФ = Справочники.ОснованияУвольнения.НайтиПоНаименованию("п. 4 ст. 35 ТК");
		// }
		// 4D
...
Процедура СообщитьОНевозможностиСозданияУвольнения()
	// 4D:ERP для Беларуси, Яна, 17.07.2017 13:10:38 
	// Ошибка открытия документа "Перевод к другому работадателю", № 15464 
	// {
	ТекстСообщения = Нстр("ru = 'Перед созданием увольнения необходимо заполнить организацию, сотрудника, дату увольнения и статью ТК РБ.'");
	// }
	// 4D
