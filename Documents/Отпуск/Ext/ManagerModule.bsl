...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если РеквизитыДляПроведения.ДокументРассчитан Тогда 
		Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
			ИсправлениеДокументовЗарплатаКадры.СторнироватьДвиженияРегистраОстатков(Движения.ДниЧасыОтгулов, Объект.ИсправленныйДокумент);
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если Пользователи.РолиДоступны("ДобавлениеИзменениеНачисленнойЗарплатыРасширенная,ПолныеПрава,ЧтениеНачисленнойЗарплатыРасширенная", , Ложь) 
		...
		// 4D:ERP для Беларуси, Петр, 09.10.2017 8:13:06 
		// Редактирование списка печатных форм для печати из документа "Отпуск", №16192
		// Сообщения об ошибках при формировании печатных форм из док-та "Отпуск", № 16404
		// {
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Обработчик = "ЗарплатаКадрыКлиент.ВыполнитьКомандуПечати";
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказовРасширенная";
		КомандаПечати.Идентификатор = "ПФ_MXL_Т60_Локализация";
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета = Неопределено) Экспорт 
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			КадровыеДанныеСотрудника = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Выборка.Сотрудник, "ВидДоговора", Выборка.ДатаНачалаПериодаОтсутствия);
...
Функция ДанныеДокументовДляПечатиРасчетаСреднегоЗаработка(МассивСсылок, ПоСтатьямФинансирования = Ложь) Экспорт
	...
	|	ЕСТЬNULL(ОтпускаСотрудников.Дата, Документ.Дата) КАК ДатаДокумента,
	|	ЕСТЬNULL(ОтпускаСотрудников.Номер, Документ.Номер) КАК НомерДокумента,
	...
	|		ПО Документ.ДокументЗаполнения = ОтпускаСотрудников.Ссылка
	|ГДЕ
...
Функция ДанныеДокументовДляПечатиРасчетаСреднегоЗаработка0504425(МассивСсылок) Экспорт
	...
	"ВЫБРАТЬ
	|	Документ.Ссылка,
	|	Документ.ДатаНачалаПериодаОтсутствия КАК Дата,
	...
	|ВЫБРАТЬ
	|	Документ.Ссылка КАК Ссылка,
	|	Документ.ВидРасчетаОсновногоОтпуска КАК Начисление,
	|	Документ.НачалоПериодаЗаКоторыйПредоставляетсяОтпуск,
	|	Документ.КонецПериодаЗаКоторыйПредоставляетсяОтпуск,
	|	Документ.ДатаНачалаПериодаОтсутствия КАК ДатаНачалаОтсутствия,
	|	Документ.ДатаОкончанияПериодаОтсутствия КАК ДатаОкончанияОтсутствия,
	...
	|	Документ.КоличествоДнейОсновногоОтпуска + ЕСТЬNULL(ВТДополнительныеОтпуска.КоличествоДней, 0) КАК ДнейОтпускаВсего,
	...
	|	Документ.Основание,
	|	Документ.РассчитатьЗарплату,
	|	ВЫБОР
	|		КОГДА Документ.ИсправленныйДокумент <> ЗНАЧЕНИЕ(Документ.Отпуск.ПустаяСсылка)
	|			ТОГДА Документ.ИсправленныйДокумент.ПериодРегистрации
	|		ИНАЧЕ Документ.ПериодРегистрации
