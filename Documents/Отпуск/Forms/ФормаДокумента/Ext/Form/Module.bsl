...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "ПроведениеДокументаОтпуск");
	КонецЕсли;
...
Процедура ОткрытьСреднийЗаработок(Команда)
	...
	// 4D:ERP для Беларуси, Яна, 01.09.2017 14:28:57 
...
Функция ПараметрыРедактированияСреднегоЗаработка()
	...
	// 4D:ERP для Беларуси, Яна, 01.09.2017 14:41:10 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
...
Процедура ДополнитьНачисленияВыплачиваемыеКОтпуску(ТаблицаНачислений, МенеджерРасчета)
	...
	Если Объект.ПредоставитьЕдиновременнуюВыплатуКОтпуску И ЗначениеЗаполнено(Объект.ВидРасчетаЕдиновременнойВыплатыКОтпуску) Тогда
		...
		НовыйИнтервал.ДатаНачала = Объект.ДатаНачалаСобытия;
		НовыйИнтервал.ДатаОкончания = Объект.ДатаНачалаСобытия;
	КонецЕсли;
...
Процедура ПеренестиДанныеУчетаСреднегоЗаработкаВДокумент(РезультатРедактирования)
	...
	// 4D:ERP для Беларуси, Петр, 22.02.2018 17:46:17 
	// Документ "Отпуск". Сохранение изменений при повторном открытии формы "Ввод данных для расчета среднего заработка", № 16429
	// {
	Объект.СреднийЗаработокОтпуск.Очистить();
	Объект.ОтработанноеВремяДляСреднегоОтпуск.Очистить();
	Объект.ДанныеОбИндексации.Очистить();
	
	УчетСреднегоЗаработка.ЗаполнитьДанныеУчетаОбщегоСреднегоЗаработкаОтпуск(
		Объект.СреднийЗаработокОтпуск, 
		Объект.ОтработанноеВремяДляСреднегоОтпуск, 
...
Процедура РассчитатьСреднийЗаработок()
	...
	УстановитьПривилегированныйРежим(Истина);	
	// 4D:ERP для Беларуси, Яна, 01.09.2017 14:48:46 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	Для Каждого СпособРасчета Из СпособыРасчетаСреднегоЗаработка Цикл
		Объект[СпособРасчета.Ключ] = УчетСреднегоЗаработкаКлиентСервер.РассчитатьСреднийЗаработокОтпуск(
		Объект.СреднийЗаработокОтпуск, 
		Объект.ОтработанноеВремяДляСреднегоОтпуск, 
		Объект.ДанныеОбИндексации, 
		...
		Новый Структура("СпособРасчетаОтпуска, ДатаНачалаСобытия", СпособРасчета.Значение, Объект.ДатаНачалаСобытия));
	КонецЦикла;
	// }
	// 4D
...
Процедура УстановитьВидимостьУдержаний(Форма)
	...
	// 4D:ERP для Беларуси, Петр, 20.11.2017 9:31:38 
	// Редактировать форму документа "отпуск", №16689
	// {
...
Процедура ЗаполнитьИнформациюЗаполненностиДанныхРасчетаДенежногоСодержания(Форма)
	...
	#Если Клиент Тогда
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
			Модуль = ОбщегоНазначенияКлиент.ОбщийМодуль("РасчетДенежногоСодержанияКлиентСервер");
		КонецЕсли;
	#Иначе
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
			Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержанияКлиентСервер");
		КонецЕсли;
	#КонецЕсли
...
Процедура ЗаполнитьСтруктуруРеквизитовИСпособовРасчетаОтпуска()
	...
	// 4D:ERP для Беларуси, Яна, 01.09.2017 14:50:22 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
...
Процедура УстановитьВидимостьРасчетныхПолей()
	...
	// 4D:ERP для Беларуси, Юлия, 14.12.2017 16:27:28 
	// На закладке "Начислено" док-та "Отпуск" добавить возможность изменить вид начисления, № 17137
	// {
