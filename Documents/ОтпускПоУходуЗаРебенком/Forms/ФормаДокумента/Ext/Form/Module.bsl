...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		// 4D:ERP для Беларуси, Яна, 15.08.2017 10:31:01 
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИсправленДокумент" И Источник = Объект.Ссылка Тогда
		ОбработкаОповещенияИсправленДокументНаСервере();
...
&НаКлиенте
Процедура ВыплачиватьПособиеДоТрехЛетНаВторогоРебенкаПриИзменении(Элемент)
	...
	// {
	ВыплачиватьПособиеДоТрехЛетНаВторогоРебенкаПриИзмененииНаСервере(); 
	// }
	// 4D
...
// {
&НаКлиенте
Процедура ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалидаПриИзменении(Элемент)
	ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалидаПриИзмененииНаСервере();	
КонецПроцедуры
// }
// 4D
...
&НаКлиенте
Процедура ПособиеДоТрехЛетНаВторогоРебенкаПриИзменении(Элемент)
	
	// 4D:ERP для Беларуси, Яна, 14.06.2017 11:59:01 
	// Добавление варианта оплаты отпуска по уходу за ребенком инвалидом до 3-х лет, №15145 
	// {
	ПриИзмененииВидаПособияДоТрехЛетНаВторогоРебенка();
	// }
	// 4D
...
// {
&НаКлиенте
Процедура ДатаОкончанияПособияДоТрехЛетНаРебенкаИнвалидаПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПособиеДоТрехЛетНаРебенкаИнвалидаПриИзменении(Элемент)
	ПриИзмененииВидаПособияДоТрехЛетНаРебенкаИнвалида();
КонецПроцедуры
// }
// 4D
...
&НаСервере
Процедура ВыплачиватьПособиеДоТрехЛетНаВторогоРебенкаПриИзмененииНаСервере()
	...
	Если Объект.ВыплачиватьПособиеДоТрехЛетНаВторогоРебенка
		И Не ЗначениеЗаполнено(Объект.ПособиеДоТрехЛетНаВторогоРебенка) Тогда
		ЗаполнитьВидПособияДоТрехЛетНаВторогоРебенка();
	КонецЕсли;
	
	УстановитьДоступностьЭлементовЗависимыхОтВыплатыПособияДоТрехЛетНаВторогоРебенка(ЭтаФорма);
...
// {
&НаСервере
Процедура ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалидаПриИзмененииНаСервере()

	ОбновитьКомментарийКДокументу(ЭтаФорма); 
	
	Если Объект.ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалида
		И Не ЗначениеЗаполнено(Объект.ПособиеДоТрехЛетНаРебенкаИнвалида) Тогда
		ЗаполнитьВидПособияДоТрехЛетНаРебенкаИнвалида();
	КонецЕсли;
	
	УстановитьДоступностьЭлементовЗависимыхОтВыплатыПособияДоТрехЛетНаРебенкаИнвалида(ЭтаФорма);
...
&НаСервере
Процедура ЗаполнитьВидПособияДоТрехЛетНаВторогоРебенка()
	...
	РасчетЗарплатыРасширенныйФормы.ЗаполнитьНачислениеВФормеДокументаПоКатегории(
		ЭтаФорма, Объект.ПособиеДоТрехЛетНаВторогоРебенка, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехЛетНаВторогоРебенка);
...
// {
&НаСервере
Процедура ЗаполнитьВидПособияДоТрехЛетНаРебенкаИнвалида()
	
	УстановитьПривилегированныйРежим(Истина);
	
	РасчетЗарплатыРасширенныйФормы.ЗаполнитьНачислениеВФормеДокументаПоКатегории(
		ЭтаФорма, Объект.ПособиеДоТрехЛетНаРебенкаИнвалида, Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехНаРебенкаИнвалида);
...
&НаСервере
Процедура ПриИзмененииВидаПособияДоТрехЛетНаВторогоРебенка()
	
	// 4D:ERP для Беларуси, Яна, 14.06.2017 11:59:01 
	// Добавление варианта оплаты отпуска по уходу за ребенком инвалидом до 3-х лет, №15145 
	// {
	УстановитьСвойствоНачисленияУтверждены();
	РасчетЗарплатыРасширенныйФормы.ОбновитьПоляВводаПоказателейПособияДоТрехЛетНаВторогоРебенка(ЭтаФорма);
	// }
	// 4D
...
// {
&НаСервере
Процедура ПриИзмененииВидаПособияДоТрехЛетНаРебенкаИнвалида()

	УстановитьСвойствоНачисленияУтверждены();
	
	РасчетЗарплатыРасширенныйФормы.ОбновитьПоляВводаПоказателейПособияДоТрехЛетНаРебенкаИнвалида(ЭтаФорма);
...
&НаСервере
Процедура УстановитьВидимостьРасчетныхПолей(ОбновлятьПоляНесколькихРабочихМест = Истина)
	...
	Если ОграниченияНаУровнеЗаписей.ПлановыеНачисленияЧтениеБезОграничений Тогда
		...
		Если ОбновлятьПоляНесколькихРабочихМест Тогда
			НесколькоРабочихМестОбработкаФормы(Ложь);
		КонецЕсли;
...
Процедура УстановитьДоступностьРегистрацииНачислений()
	...
	ЕстьПравоПросмотраСреднегоЗаработка = КадровыйУчет.ПравоНаРазделУчета("Чтение", "НачисленнаяЗарплата");
	Элементы.ДанныеОДетяхЗаявление.Видимость = ЕстьПравоПросмотраСреднегоЗаработка;
...
Процедура ДополнитьСтрокиЗаявлениями()
	...
	Если Элементы.ДанныеОДетяхЗаявление.Видимость
		И ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(Объект.Организация, Объект.ДатаОкончанияПособияДоПолутораЛет) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
...
&НаСервере
Процедура ОбработкаОповещенияИсправленДокументНаСервере()
	
	ДокументОбъект = РеквизитФормыВЗначение("Объект");
	ДанныеНачисленийВРеквизит(ДокументОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьЭлементовЗависимыхОтВыплатыПособияДоТрехЛетНаВторогоРебенка(Форма)
	...
	МассивЭлементов.Добавить("ДатаОкончанияПособияДоТрехЛетНаВторогоРебенка");	
	МассивЭлементов.Добавить("ПособиеДоТрехЛетНаВторогоРебенка");	
	МассивЭлементов.Добавить("ГруппаПоказателиПособияДоТрехЛетНаВторогоРебенка");	
	
	УстановитьДоступностьМассиваЭлементов(Форма, МассивЭлементов, Форма.Объект.ВыплачиватьПособиеДоТрехЛетНаВторогоРебенка);
	// }
	// 4D
...
// {
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьДоступностьЭлементовЗависимыхОтВыплатыПособияДоТрехЛетНаРебенкаИнвалида(Форма)
	МассивЭлементов = Новый Массив;
	
	МассивЭлементов.Добавить("ДатаОкончанияПособияДоТрехЛетНаРебенкаИнвалида");	
	МассивЭлементов.Добавить("ПособиеДоТрехЛетНаРебенкаИнвалида");	
	МассивЭлементов.Добавить("ГруппаПоказателиПособияДоТрехЛетНаРебенкаИнвалида");	
	
	УстановитьДоступностьМассиваЭлементов(Форма, МассивЭлементов, Форма.Объект.ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалида);	
КонецПроцедуры
// }
// 4D
...
Процедура ДанныеОДетяхПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		ТекущаяОчередность = 0;
		Если Объект.ДанныеОДетях.Количество() > 0 Тогда
			Для каждого ДанныеОРебенке Из Объект.ДанныеОДетях Цикл
				Если ТекущаяОчередность < ДанныеОРебенке.Очередность Тогда
					ТекущаяОчередность 	= ДанныеОРебенке.Очередность;
					ТекущаяФамилия 		= ДанныеОРебенке.Фамилия; 
					ТекущееОтчество 	= ДанныеОРебенке.Отчество; 
				КонецЕсли;
			КонецЦикла;
			
			Элементы.ДанныеОДетях.ТекущиеДанные.Фамилия 	= ТекущаяФамилия;
			Элементы.ДанныеОДетях.ТекущиеДанные.Отчество 	= ТекущееОтчество;
		КонецЕсли;
		
		Элементы.ДанныеОДетях.ТекущиеДанные.ИдентификаторСтроки	= Строка(Новый УникальныйИдентификатор);
		Элементы.ДанныеОДетях.ТекущиеДанные.Очередность = ТекущаяОчередность + 1;
		Элементы.ДанныеОДетях.ТекущиеДанные.ВидПодтверждающегоДокумента = ПредопределенноеЗначение("Перечисление.ВидыПодтверждающихДокументовОтпускаПоУходу.СвидетельствоОРождении");
		Элементы.ДанныеОДетях.ТекущиеДанные.НаименованиеПодтверждающегоДокумента = "Свидетельство о рождении";
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗавершитьИнициализациюФормы(ТекущийОбъект)
	...
	УстановитьВидимостьРасчетныхПолей(Ложь);
	УстановитьВидимостьГруппыВыплата();
