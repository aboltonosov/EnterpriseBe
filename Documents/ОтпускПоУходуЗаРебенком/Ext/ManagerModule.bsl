...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 30.11.2017 10:21:32 
	// Редактировать список печатных форм док-та "Отпуск по уходу за ребенком", № 16918
	...
	КомандаПечати.Идентификатор = "ПФ_MXL_ОПредоставленииОтпускаПоУходуЗаРебенком";
...
Функция КадровыеДанныеОсновногоСотрудника(ФизическоеЛицо, Организация, Дата)
	...
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОсновныеСотрудники[0].Сотрудник), "ФизическоеЛицо,ФИОПолные,ТабельныйНомер,Подразделение,Должность,ВидЗанятости", Дата);
...
// 4D:ERP для Беларуси, Яна, 16.08.2017 14:48:02 
// Документ "Изменение условий оплаты отпуска по уходу за ребенком", №15091 
// {
Функция ИдентификаторСтрокПоказателейПособияДоТрехЛетНаВторогоРебенка() Экспорт
	Возврат 9 * 1000000;
КонецФункции
...
Процедура ЗаполнитьПлановыеНачисленияИПоказатели(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	// 4D:ERP для Беларуси, Яна, 15.08.2017 11:38:28 
	...
	// {
	Если РеквизитыДляПроведения.ВыплачиватьПособиеДоТрехЛетНаВторогоРебенка Тогда
		ДобавитьПособиеПоУходуДоТрехЛетНаВторогоРебенка(РеквизитыДляПроведения, ПлановыеНачисления,  ГоловнаяОрганизация);
		ДобавитьПоказателиПособияПоУходуДоТрехЛетНаВторогоРебенка(РеквизитыДляПроведения, ЗначенияПоказателей);
	КонецЕсли;	
	
	Если РеквизитыДляПроведения.ВыплачиватьПособиеДоТрехЛетНаРебенкаИнвалида Тогда
		ДобавитьПособиеПоУходуДоТрехЛетНаРебенкаИнвалида(РеквизитыДляПроведения, ПлановыеНачисления,  ГоловнаяОрганизация);
		ДобавитьПоказателиПособияПоУходуДоТрехЛетНаРебенкаИнвалида(РеквизитыДляПроведения, ЗначенияПоказателей);
	КонецЕсли;
	// }
	// 4D
...
// 4D:ERP для Беларуси, Яна, 15.08.2017 11:50:11 
// Документ "Отпуск по уходу за ребенком", №15745 
// Добавление варианта оплаты отпуска по уходу за ребенком инвалидом до 3-х лет, №15145
// {
Процедура ДобавитьПособиеПоУходуДоТрехЛетНаВторогоРебенка(РеквизитыДляПроведения, ПлановыеНачисления, ГоловнаяОрганизация)
	НовоеПлановоеНачисление = ПлановыеНачисления.Добавить();
	НовоеПлановоеНачисление.ДатаСобытия = РеквизитыДляПроведения.ДатаНачала;
	НовоеПлановоеНачисление.ДействуетДо = КонецДня(РеквизитыДляПроведения.ДатаОкончанияПособияДоТрехЛетНаВторогоРебенка) + 1;
	НовоеПлановоеНачисление.Сотрудник = РеквизитыДляПроведения.ОсновнойСотрудник;
	НовоеПлановоеНачисление.ФизическоеЛицо = РеквизитыДляПроведения.Сотрудник;
	НовоеПлановоеНачисление.ГоловнаяОрганизация = ГоловнаяОрганизация;
	НовоеПлановоеНачисление.Начисление = РеквизитыДляПроведения.ПособиеДоТрехЛетНаВторогоРебенка;
	НовоеПлановоеНачисление.Размер = РеквизитыДляПроведения.РазмерПособияДоТрехЛетНаВторогоРебенкаФиксированнойСуммой;
	НовоеПлановоеНачисление.Используется = Истина;
КонецПроцедуры

Процедура ДобавитьПоказателиПособияПоУходуДоТрехЛетНаВторогоРебенка(РеквизитыДляПроведения, ЗначенияПоказателей)
	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		НоваяСтрока 				= ЗначенияПоказателей.Добавить();
		НоваяСтрока.ДатаСобытия 	= РеквизитыДляПроведения.ДатаНачала;
		НоваяСтрока.ДействуетДо 	= КонецДня(РеквизитыДляПроведения.ДатаОкончанияПособияДоТрехЛетНаВторогоРебенка) + 1;
		НоваяСтрока.Сотрудник 		= РеквизитыДляПроведения.ОсновнойСотрудник;
		НоваяСтрока.ФизическоеЛицо 	= РеквизитыДляПроведения.Сотрудник;
		НоваяСтрока.Организация 	= РеквизитыДляПроведения.Организация;
		НоваяСтрока.Показатель 		= Справочники.ПоказателиРасчетаЗарплаты.ПроцентПособияДоТрехЛетНаВторогоРебенка;
		НоваяСтрока.Значение 		= СтрокаТаблицы.Значение;
		//?(не ЗначениеЗаполнено(ПроцентПособияДоТрехЛетНаВторогоРебенка), 100, ПроцентПособияДоТрехЛетНаВторогоРебенка)
	КонецЦикла;
	
КонецПроцедуры

Процедура ДобавитьПособиеПоУходуДоТрехЛетНаРебенкаИнвалида(РеквизитыДляПроведения, ПлановыеНачисления, ГоловнаяОрганизация)
	НовоеПлановоеНачисление = ПлановыеНачисления.Добавить();
	НовоеПлановоеНачисление.ДатаСобытия = РеквизитыДляПроведения.ДатаНачала;
	НовоеПлановоеНачисление.ДействуетДо = КонецДня(РеквизитыДляПроведения.ДатаОкончанияПособияДоТрехЛетНаРебенкаИнвалида) + 1;
	НовоеПлановоеНачисление.Сотрудник = РеквизитыДляПроведения.ОсновнойСотрудник;
	НовоеПлановоеНачисление.ФизическоеЛицо = РеквизитыДляПроведения.Сотрудник;
	НовоеПлановоеНачисление.ГоловнаяОрганизация = ГоловнаяОрганизация;
	НовоеПлановоеНачисление.Начисление = РеквизитыДляПроведения.ПособиеДоТрехЛетНаРебенкаИнвалида;
	НовоеПлановоеНачисление.Размер = РеквизитыДляПроведения.РазмерПособияДоТрехЛетНаРебенкаИнвалидаФиксированнойСуммой;
	НовоеПлановоеНачисление.Используется = Истина;
КонецПроцедуры

Процедура ДобавитьПоказателиПособияПоУходуДоТрехЛетНаРебенкаИнвалида(РеквизитыДляПроведения, ЗначенияПоказателей)
	...
	Для Каждого СтрокаТаблицы Из НайденныеСтроки Цикл
		НоваяСтрока 				= ЗначенияПоказателей.Добавить();
		НоваяСтрока.ДатаСобытия 	= РеквизитыДляПроведения.ДатаНачала;
		НоваяСтрока.ДействуетДо 	= КонецДня(РеквизитыДляПроведения.ДатаОкончанияПособияДоТрехЛетНаРебенкаИнвалида) + 1;
		НоваяСтрока.Сотрудник 		= РеквизитыДляПроведения.ОсновнойСотрудник;
		НоваяСтрока.ФизическоеЛицо 	= РеквизитыДляПроведения.Сотрудник;
		НоваяСтрока.Организация 	= РеквизитыДляПроведения.Организация;
		НоваяСтрока.Показатель 		= Справочники.ПоказателиРасчетаЗарплаты.ПроцентПособияДоТрехЛетНаРебенкаИнвалида;
		НоваяСтрока.Значение 		= СтрокаТаблицы.Значение;
		//?(не ЗначениеЗаполнено(ПроцентПособияДоТрехЛетНаРебенкаИнвалида), 100, ПроцентПособияДоТрехЛетНаРебенкаИнвалида)
	КонецЦикла;
