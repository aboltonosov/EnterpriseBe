...
&НаКлиентеНаСервереБезКонтекста
Функция ПолучитьСохраненноеЗначение(Форма, ИмяРеквизита)
	
	Перем Результат;
	
	Если Форма.СохраненныеЗначения <> Неопределено Тогда
		Результат = Форма.СохраненныеЗначения[ИмяРеквизита];
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСохраненноеЗначение(Форма, ИмяРеквизита, Значение)
	
	Если Форма.СохраненныеЗначения = Неопределено Тогда
		Форма.СохраненныеЗначения = Новый Структура(ИменаСохраняемыхРеквизитов());
	КонецЕсли;
	
	Форма.СохраненныеЗначения[ИмяРеквизита] = Значение;
