...
Процедура ЗаполнитьПоПлатежномуДокументу(РеквизитыЗаполнения)
	...
	Если МассивАналитикУчетаПоПартнерам.Количество() > 0 Тогда
		...
		Попытка
			РаспределениеВзаиморасчетов.РаспределитьВсеРасчетыСКлиентами(КонецМесяца(РеквизитыЗаполнения.Дата),АналитикиРасчета);
		Исключение
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьЗаказыКлиентов") Тогда
		...
		Если Товары.Количество() <> 0 Тогда
			...
			Документы.СчетФактураВыданныйАванс.РаспределитьАвасыПоТоварам(ТаблицаАвансы, Товары, ДокументОснование);
