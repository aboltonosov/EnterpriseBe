...
Процедура ОбработкаВыбора(Результат, ИсточникВыбора)
	...
	Если ТипЗнч(Результат) = Тип("СправочникСсылка.ЭкземплярБюджетаПрисоединенныеФайлы") Тогда
		Если СлужебныйПрисоединенныйФайл(Результат) Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Файл является служебным, выбор запрещен.'"));
		Иначе
...
&НаСервереБезКонтекста
Функция СлужебныйПрисоединенныйФайл(ПрисоединенныйФайл)
	
	ТипСлужебногоФайла = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл, "ТипСлужебногоФайла");
	Возврат ЗначениеЗаполнено(ТипСлужебногоФайла);
...
Функция МожноОбновитьБюджет()
	...
	Если НезаполненныеПоля.Количество() > 0 Тогда
		...
		ШаблонСообщенияСостояния = ?(НезаполненныеПоля.Количество() = 1,
			НСтр("ru = 'Заполните поле %СписокПолей'"), НСтр("ru = 'Заполните поля: %СписокПолей'"));
...
Процедура СкопироватьПоПериодамПоРасшифровке(Расшифровка, Значение, ИсключаяПериоды = Неопределено)
	...
	Для Каждого Период из МассивПериодов Цикл
		...
		Если Период = Расшифровка["Период" + МинимальнаяПериодичность] 
		...
		Для Каждого Периодичность из Периодичности Цикл
			РабочаяРасшифровка.Вставить("Период" + Периодичность, БюджетированиеКлиентСервер.ДатаНачалаПериода(Период, Периодичность));
		КонецЦикла;
		...
		Для Строка = АктивнаяОбласть.Верх по АктивнаяОбласть.Низ Цикл
			Для Колонка = АктивнаяОбласть.Лево по АктивнаяОбласть.Право Цикл
				...
				Если ОбластьЗначения.СодержитЗначение Тогда
					...
					Период = Расшифровка["Период" + МинимальнаяПериодичность];
		...
		Для Строка = АктивнаяОбласть.Верх по АктивнаяОбласть.Низ Цикл
			Для Колонка = АктивнаяОбласть.Лево по АктивнаяОбласть.Право Цикл
				...
				Если ОбластьЗначения.СодержитЗначение Тогда
					...
					Расшифровка = БюджетнаяОтчетностьВызовСервера.ЗначениеРасшифровки(ОбластьЗначения.Расшифровка, АдресДанныхРасшифровки);
					СкопироватьПоПериодамПоРасшифровке(Расшифровка, ОбластьЗначения.Значение, ИсключаяПериоды);
	...
	Для Колонка = АктивнаяОбласть.Лево по АктивнаяОбласть.Право Цикл
		Для Строка = АктивнаяОбласть.Верх По АктивнаяОбласть.Низ Цикл
			...
			Если Область.СодержитЗначение Тогда
				...
				ОтразитьИзменениеЯчейки(Расшифровка, Область.Текст, ВведенныйТекст);
...
Функция ВставитьОбластьНаСервере(СоответствиеИзмерений, ДополнительныеПараметры)
	...
	Для Каждого СтрокаСтруктурыЯчеек из НайденныеСтроки Цикл
		...
		Для Каждого СтрокаПериодов из ТаблицаПериодов Цикл
			...
			Для Каждого Периодичность из Периодичности Цикл
				СтруктураПоискаПроверка.Вставить("Период" + Периодичность, СтрокаПериодов["Период" + Периодичность]);

				Если ДанныеЯчеек.НайтиСтроки(СтруктураПоискаПроверка).Количество() > 0 Тогда
					СтруктураПоиска.Вставить("Период" + Периодичность, СтрокаПериодов["Период" + Периодичность]);
					ПериодСтрокой = ПериодСтрокой + "Период" + Периодичность +"{" + Строка(СтрокаПериодов["Период" + Периодичность]) + "};";
				Иначе
					Если СтруктураПоиска.Свойство("Период" + Периодичность) Тогда
						СтруктураПоиска.Удалить("Период" + Периодичность);
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			
			Если ДобавленныеПериоды.Получить(ПериодСтрокой) = Неопределено Тогда
				ДобавленнаяСтрока = БюджетнаяОтчетностьВыводСервер.ДобавитьСтрокуДанныхЯчеек(ДанныеЯчеек, СтрокаСтруктурыЯчеек, 
																				СтруктураПоиска, СтрокаСтруктурыЯчеек.Колонка);
	...
	ДанныеОтчетаДляОформления = БюджетнаяОтчетностьРасчетКэшаСервер.ДанныеОтчетаДляОформления(Таблица.ПараметрыТаблицы, КэшДопРеквизиты);
	...
	ПараметрыПериодов = БюджетнаяОтчетностьВыводСервер.ПреобразоватьПараметрыДанныхВДанныеПериодов(
														СтруктураПараметровВывода.СтруктураКолонокТаблицыДанных, ПараметрыДанных);
	ПараметрыВывода.Вставить("ПараметрыПериодов", ПараметрыПериодов);
	ПараметрыВывода.Вставить("СчетчикСтрок", 0);
	
	КэшПараметровВалют = БюджетнаяОтчетностьВыводСервер.ПолучитьКэшПараметровВалют(ДанныеЯчеек, СтруктураОписанияОтчета.ПараметрыЯчеек, 
																							ПараметрыВывода.ПодчиненныеЭлементыВидаБюджета);
	ПараметрыВывода.Вставить("КэшПараметровВалют", КэшПараметровВалют);
	
	КэшПараметровЕдиницИзмерения = БюджетнаяОтчетностьВыводСервер.ПолучитьКэшПараметровЕдиницИзмерения(ДанныеЯчеек, СтруктураОписанияОтчета.ПараметрыЯчеек);
	...
	ПредставленияИзмерений = БюджетнаяОтчетностьВыводСервер.ПредставлениеИзмеренийОтчета(ДеревоВывода, ДанныеЯчеек, СтруктураКолонокТаблицыДанных);
	ПараметрыВывода.Вставить("ПредставленияИзмерений", ПредставленияИзмерений);

	СтруктураКолонок = БюджетнаяОтчетностьВыводСервер.ПолучитьДеревоГруппировокКолонок(ПараметрыВывода, 
	...
	//Удаляется пустая строка, которая при выводе добавляется "по-умолчанию"
	АдресДобавленияСтрокСпискаРедактирования.Строки.Удалить(АдресДобавленияСтрокСпискаРедактирования.Строки.Количество() - 1);
	
	ЗначениеВРеквизитФормы(ДеревоДляВыводаОтчета, "ДеревоЭлементовБюджета");
	
	ВГраницаВставляемойОбласти = ПараметрыВывода.ДокументРезультат.ВысотаТаблицы / 2 - 1;
...
Процедура ПолучитьДоступностьЭлементовПоБюджетнойЗадаче()
	...
	Если Не ИспользоватьБюджетныйПроцесс ИЛИ РольДоступна("РедактированиеДанныхБюджетированияВнеПроцессов")
