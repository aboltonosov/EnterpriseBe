...
Функция ОтменитьНеотработанныеСервер()
	...
	ПараметрыЗаполнения.Вставить("ИмяТабличнойЧасти", "МатериалыИРаботы");
	...
	Таблица = ОбеспечениеСервер.ТаблицаЗаполнениеОбеспеченияДокумента(Объект, ПереченьВариантов, ПараметрыЗаполнения, ВыделенныеСтроки);
	...
	Если ТипЗнч(Таблица) = Тип("ТаблицаЗначений") Тогда
		...
		ЗаказыСервер.ПеренестиВТаблицуДокументаОтмененныеСтроки(Объект.МатериалыИРаботы, Таблица);
		
		РезультатОтмены = ЗаказыСервер.РезультатОтменыНеотработанныхСтрок(КоличествоСтрокКОтмене, Модифицированность);
	Иначе
		Ошибки = ОбеспечениеКлиентСервер.ОшибкиКонтроляОтгрузкиИОбеспечения(Таблица, "МатериалыИРаботы", НСтр("ru = 'Материалы и работы'"));
		ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Ошибки);
