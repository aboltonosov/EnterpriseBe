...
Функция ПолучитьГрафикВыпускаПродукции(ДанныеДокумента, ПараметрыОтладки = Неопределено) Экспорт
	...
	ОписаниеЗамера = Производительность.НачатьЗамерВремени("МежцеховоеУправление.РасчетГрафикаПроизводства");
	
	РезультатРасчета = РассчитатьГрафикВыпускаПродукции(ДанныеДокумента, ПараметрыОтладки);
	
	Производительность.ЗакончитьЗамерВремени(ОписаниеЗамера, ДанныеДокумента.Этапы.Количество());
