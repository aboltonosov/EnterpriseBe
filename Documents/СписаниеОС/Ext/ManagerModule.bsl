...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, ДополнительныеСвойства)
	...
	Запрос.УстановитьПараметр("НазваниеДокумента", НСтр("ru='Списание ОС'"));
...
Процедура ВременнаяТаблицаОсновныхСредств(ТекстыЗапроса)
	...
	|				* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляБУЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|				- (ВЫРАЗИТЬ(втТекущееСостояниеОбъектов.НакопленнаяАмортизацияБУЦФ 
	|							* (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаСписанияБУ 
	|										* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляБУЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|					/ втТекущееСостояниеОбъектов.ВосстановительнаяСтоимостьБУЦФ КАК ЧИСЛО(15, 2)))
	...
	|				* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляНУЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|				- (ВЫРАЗИТЬ(втТекущееСостояниеОбъектов.НакопленнаяАмортизацияНУЦФ 
	|							* (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаСписанияНУ 
	|										* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляНУЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|					/ втТекущееСостояниеОбъектов.ВосстановительнаяСтоимостьНУЦФ КАК ЧИСЛО(15, 2)))
	...
	|				* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляПРЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|				- (ВЫРАЗИТЬ(втТекущееСостояниеОбъектов.НакопленнаяАмортизацияПРЦФ 
	|							* (ВЫРАЗИТЬ(ТаблицаДокумента.СуммаСписанияПР 
	|										* ЕСТЬNULL(втТекущееСостояниеОбъектов.ДоляПРЦФ, 0) КАК ЧИСЛО(15, 2))) 
	|					/ втТекущееСостояниеОбъектов.ВосстановительнаяСтоимостьПРЦФ КАК ЧИСЛО(15, 2)))
	...
	|					/ втТекущееСостояниеОбъектов.ПолнаяВосстановительнаяСтоимостьБУ КАК ЧИСЛО(15, 2)))
	|	КОНЕЦ КАК РасходБУ,
	|
	|	ВЫБОР КОГДА ЕСТЬNULL(втТекущееСостояниеОбъектов.ПолнаяВосстановительнаяСтоимостьБУ, 0) = 0 ТОГДА 0
...
Функция ПрочиеРасходы(ТекстыЗапроса, Регистры)
	...
	|	0 КАК СуммаБезНДС,
	|	ТаблицаОС.РасходБУ КАК СуммаРегл,
	|	ТаблицаОС.РасходБУ КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
	|	ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка) КАК АналитикаУчетаНоменклатуры,
...
Функция ПрочиеДоходы(ТекстыЗапроса, Регистры)
	...
	|	
	|	ВЫРАЗИТЬ((втЦелевыеСредстваУпр.Сумма * (ТаблицаОС.СуммаСписанияБУЦФ/втЦелевыеСредстваРеглСгруппированныеПоНаправлениям.Сумма)) КАК ЧИСЛО(15,2)) КАК Сумма,
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	Возврат УчетОСВызовСервера.ТекстЗапросаВТОтраженияВРеглУчетеНачисленнойАмортизации("СписаниеОС")
...
Функция ТекстЗапросаВТСуммыЧастичнойЛиквидации()
	...
	|	
	|	Строки.СуммаСписанияБУ - ВЫРАЗИТЬ(Строки.СуммаСписанияБУ * втОстаткиПоСчетам.ДоляЦФБУ КАК ЧИСЛО(15,2)) КАК СуммаБУ,
	|	Строки.СуммаСписанияНУ - ВЫРАЗИТЬ(Строки.СуммаСписанияНУ * втОстаткиПоСчетам.ДоляЦФНУ КАК ЧИСЛО(15,2)) КАК СуммаНУ,
	|	Строки.СуммаСписанияПР - ВЫРАЗИТЬ(Строки.СуммаСписанияПР * втОстаткиПоСчетам.ДоляЦФБУ КАК ЧИСЛО(15,2)) КАК СуммаПР,
	|	
	|	ВЫРАЗИТЬ(Строки.СуммаСписанияБУ * втОстаткиПоСчетам.ДоляЦФБУ КАК ЧИСЛО(15,2)) КАК СуммаБУЦФ,
	|	ВЫРАЗИТЬ(Строки.СуммаСписанияНУ * втОстаткиПоСчетам.ДоляЦФНУ КАК ЧИСЛО(15,2)) КАК СуммаНУЦФ,
	|	ВЫРАЗИТЬ(Строки.СуммаСписанияПР * втОстаткиПоСчетам.ДоляЦФБУ КАК ЧИСЛО(15,2)) КАК СуммаПРЦФ
	|	
	...
	|				/ ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьБУ, 0) КАК ЧИСЛО(15,2))
	...
	|				/ ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьНУ, 0) КАК ЧИСЛО(15,2)) 
	...
	|				/ ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьБУЦФ, 0) КАК ЧИСЛО(15,2)) 
	...
	|				/ ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьНУЦФ, 0) КАК ЧИСЛО(15,2)) 
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат УчетОСВызовСервера.ТекстОтраженияВРеглУчетеНачисленнойАмортизации()
		...
		+ Разделитель + СписаниеПервоначальнойСтоимостиОСЦФ()
		...
		+ Разделитель + ЧастичнаяЛиквидацияПервоначальнойСтоимостиОСЦФ()
		+ Разделитель + ЧастичнаяЛиквидацияНачисленнойАмортизации()
		+ Разделитель + ЧастичнаяЛиквидацияНачисленнойАмортизацииЦФ()
		+ Разделитель + ЧастичнаяЛиквидацияНачисленногоИзноса()
		+ Разделитель + ЧастичнаяЛиквидацияСоСчетаВыбытияОС()
		+ Разделитель + ЧастичнаяЛиквидацияДоходыОтЦелевогоФинансирования()
		+ Разделитель + СписаниеРезерваДооценки()
		+ Разделитель + ВыбытиеАрендованногоОС();
...
Функция ЧастичнаяЛиквидацияНачисленногоИзноса()
	...
	|	
	|	ВЫБОР КОГДА ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьБУ, 0) = 0 ТОГДА 0
	|	ИНАЧЕ ВЫРАЗИТЬ(ЕСТЬNULL(втОстаткиПоСчетам.НакопленнаяАмортизацияБУ, 0) * Строки.СуммаСписанияБУ / ЕСТЬNULL(втОстаткиПоСчетам.ВосстановительнаяСтоимостьБУ, 0) КАК ЧИСЛО(15,2))
	|	КОНЕЦ КАК Сумма,
	|	
	|	// Дт ///////////////////////////////////////////////////////////////////////////////////////////
	|	
...
Функция ЧастичнаяЛиквидацияСоСчетаВыбытияОС()
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУ + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУЦФ КАК СуммаНУДт,
	|	втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПР + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПРЦФ КАК СуммаПРДт,
	...
	|	- (втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПР + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПРЦФ) КАК СуммаВРДт,
	|	
	|	// Кт ///////////////////////////////////////////////////////////////////////////////////////////
	|	
	|	НЕОПРЕДЕЛЕНО КАК ВидСчетаКт,
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУ + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУЦФ КАК СуммаНУКт,
	|	втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПР + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПРЦФ КАК СуммаПРКт,
	|	(втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьБУ + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьБУЦФ)
	|	- (втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУ + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьНУЦФ)
	|	- (втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПР + втСуммыЧастичнойЛиквидации.ОстаточнаяСтоимостьПРЦФ) КАК СуммаВРКт,
	|	
...
Функция ДанныеПечатнойФормыОС4(МассивОбъектов)
	...
	|		ДрагоценныйМатериал.ЕдиницаИзмерения КАК ДМЕдиницаИзмерения
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	// 4D:ERP для Беларуси, ВладимирР, 23.08.2017 14:14:48 
	// Приходуемые материальные ценности, №15565
	// {
	|ВЫБРАТЬ
	|	ДемонтируемыеУзлы.Ссылка КАК Ссылка,
	|	ДемонтируемыеУзлы.УзелДокумент,
	|	ДемонтируемыеУзлы.УзелДокументНомер,
	|	ДемонтируемыеУзлы.УзелДокументДата,
	|	ДемонтируемыеУзлы.УзелДокументНаименование,
	|	ДемонтируемыеУзлы.УзелНоменклатура,
	|	ДемонтируемыеУзлы.УзелНоменклатураНаименованиеПолное,
	|	ДемонтируемыеУзлы.УзелХарактеристика,
	|	ДемонтируемыеУзлы.УзелХарактеристикаНаименованиеПолное,
	|	ДемонтируемыеУзлы.УзелКоличество,
	|	ДемонтируемыеУзлы.УзелЕдиницаИзмерения,
	|	ДемонтируемыеУзлы.УзелЦена,
	|	ДемонтируемыеУзлы.УзелСумма
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		И ДемонтируемыеУзлы.Ссылка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПрочееПоступлениеТоваров)
	...
	|	
	|	ВЫБРАТЬ
	|		ДемонтируемыеУзлы.Ссылка,
	|		ДемонтируемыеУзлы.Ссылка,
	|		ДемонтируемыеУзлы.Ссылка.Номер,
	|		ДемонтируемыеУзлы.Ссылка.Дата,
	|		ТИПЗНАЧЕНИЯ(ДемонтируемыеУзлы.Ссылка),
	|		ДемонтируемыеУзлы.Номенклатура,
	|		ДемонтируемыеУзлы.Номенклатура.НаименованиеПолное,
	|		ДемонтируемыеУзлы.Характеристика,
	|		ДемонтируемыеУзлы.Характеристика.НаименованиеПолное,
	|		ДемонтируемыеУзлы.Количество,
	|		ДемонтируемыеУзлы.Номенклатура.ЕдиницаИзмерения,
	|		0,
	|		0
	|	ИЗ
	|		Документ.СписаниеОС.ПриходуемыеМЦ КАК ДемонтируемыеУзлы
	|	ГДЕ
	|		ДемонтируемыеУзлы.Ссылка В(&МассивОбъектов)) КАК ДемонтируемыеУзлы
	|ИТОГИ ПО
	|	Ссылка";
	// }
	// 4D
...
Функция ПечатьОС4(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Если ВыборкаСтрок.Количество() = 1 Тогда
			...
			Страница1.Параметры.ДолжностьРуководителя = ОтветственныеЛица.РуководительДолжность;
