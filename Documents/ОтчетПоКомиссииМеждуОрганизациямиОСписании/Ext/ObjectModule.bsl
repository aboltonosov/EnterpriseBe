...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
	   И ДанныеЗаполнения.Свойство("ЗаполнятьПоТоварамКОформлению") Тогда
		...
		Если ТекущаяДата() > КонецМесяца(КонецПериода)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
		...
		Если СтрокаТаблицы.Количество <> 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				|Отчет по комиссии превышает остаток товара комитента %2, списанного комиссионером %3, на %4 %5'"),
		...
		Если СтрокаТаблицы.Сумма <> 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Номенклатура: %1 
				|Сумма документа превышает сумму списанного товара комиссионером %2, на %3 %4'"),
				НоменклатураКлиентСервер.ПредставлениеНоменклатуры(СтрокаТаблицы.Номенклатура, СтрокаТаблицы.Характеристика),
				Комиссионер,
				СтрокаТаблицы.Сумма,
				Валюта);
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов() Экспорт
	...
	ТаблицаТовары = Товары.Выгрузить(, "АналитикаУчетаНоменклатуры, Упаковка, ДатаСчетаФактурыКомиссионера, СчетФактураВыставленныйКомиссионера, Покупатель, Количество, КоличествоУпаковок, СуммаПродажи, СуммаПродажиНДС, СуммаВознаграждения, СуммаНДСВознаграждения");
	ТаблицаТовары.Свернуть("АналитикаУчетаНоменклатуры, Упаковка, ДатаСчетаФактурыКомиссионера, СчетФактураВыставленныйКомиссионера, Покупатель", "Количество, КоличествоУпаковок, СуммаПродажи, СуммаПродажиНДС, СуммаВознаграждения, СуммаНДСВознаграждения");
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НоваяСтрока.СчетФактураВыставленныйКомиссионера = СтрокаТоваров.СчетФактураВыставленныйКомиссионера;
...
Функция ЗначенияПолейВидаЗапасовПоУмолчанию()
	...
	ЗначенияПоУмолчанию.НалогообложениеНДС					= НалогообложениеНДС;
	ЗначенияПоУмолчанию.НалогообложениеОрганизации			= НалогообложениеНДС;
...
Процедура ДозаполнитьВидыЗапасов(ВременныеТаблицы)
	...
	ОтборОстатков.Вставить("Партнер", Комиссионер);
	ОтборОстатков.Вставить("Соглашение", Справочники.СоглашенияСКлиентами.ПустаяСсылка());
