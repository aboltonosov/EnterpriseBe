...
Процедура ЗаполнитьПеремещения(РезультатЗапроса, ТаблицаДоступныхСтоимостей, ТаблицаЗабалансовыхСтоимостей, Отказ)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ДополнитьТаблицуПеремещений(Выборка, ТаблицаДоступныхСтоимостей, СтруктураПоиска) Тогда
			СтруктураПоиска.ФизическоеЛицо = Справочники.ФизическиеЛица.ПустаяСсылка();
			ДополнитьТаблицуПеремещений(Выборка, ТаблицаДоступныхСтоимостей, СтруктураПоиска);
		КонецЕсли;
...
Функция ДополнитьТаблицуВозвратов(СтруктураСтроки, ТаблицаДоступныхСтоимостей, СтруктураПоиска)
	...
	СтрокаВозвратов = Возвраты.Добавить();
	СтрокаДоступныеСтоимости = СтрокиДоступныхСтоимостей[0];
	ЗаполнитьЗначенияСвойств(СтрокаВозвратов, СтруктураСтроки);
	ЗаполнитьЗначенияСвойств(СтрокаВозвратов, СтрокаДоступныеСтоимости);
	
	Если СтрокаДоступныеСтоимости.Количество > СтруктураСтроки.Количество Тогда
		
		Коэффициент = (СтруктураСтроки.Количество/СтрокаДоступныеСтоимости.Количество);
		
		СтрокаВозвратов.Количество = СтруктураСтроки.Количество;
		СтрокаВозвратов.СуммаБУ = СтрокаВозвратов.СуммаБУ * Коэффициент;
		СтрокаВозвратов.СуммаНУ = СтрокаВозвратов.СуммаНУ * Коэффициент;
		СтрокаВозвратов.СуммаПР = СтрокаВозвратов.СуммаПР * Коэффициент;
		СтрокаВозвратов.СуммаВР = СтрокаВозвратов.СуммаБУ - СтрокаВозвратов.СуммаНУ - СтрокаВозвратов.СуммаПР;
		
	КонецЕсли;
	
	СтрокаДоступныеСтоимости.Количество = СтрокаДоступныеСтоимости.Количество - СтрокаВозвратов.Количество;
	
	СтрокаДоступныеСтоимости.СуммаБУ = СтрокаДоступныеСтоимости.СуммаБУ - СтрокаВозвратов.СуммаБУ;
	СтрокаДоступныеСтоимости.СуммаНУ = СтрокаДоступныеСтоимости.СуммаНУ - СтрокаВозвратов.СуммаНУ;
	СтрокаДоступныеСтоимости.СуммаПР = СтрокаДоступныеСтоимости.СуммаПР - СтрокаВозвратов.СуммаПР;
	СтрокаДоступныеСтоимости.СуммаВР = СтрокаДоступныеСтоимости.СуммаВР - СтрокаВозвратов.СуммаВР;
	
	Если СтрокаДоступныеСтоимости.Количество = 0 Тогда
		ТаблицаДоступныхСтоимостей.Удалить(СтрокаДоступныеСтоимости);
	КонецЕсли;
	
	Если СтрокаВозвратов.СуммаБУ = СтруктураСтроки.Сумма Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Если СтрокаВозвратов.СуммаБУ > СтруктураСтроки.Сумма Тогда
		...
		ЗаполнитьЗначенияСвойств(СтрокаРасходов, СтрокаВозвратов,, "СуммаБУ, СуммаНУ, СуммаПР, СуммаВР, Количество");
		
		Коэффициент = (СтруктураСтроки.Сумма/СтрокаВозвратов.СуммаБУ);
		СтрокаРасходов.СуммаБУ = СтрокаВозвратов.СуммаБУ - СтруктураСтроки.Сумма;
		СтрокаРасходов.СуммаНУ = СтрокаВозвратов.СуммаНУ - СтрокаВозвратов.СуммаНУ * Коэффициент;
		СтрокаРасходов.СуммаПР = СтрокаВозвратов.СуммаПР - СтрокаВозвратов.СуммаПР * Коэффициент;
		СтрокаРасходов.СуммаВР = СтрокаРасходов.СуммаБУ - СтрокаРасходов.СуммаНУ - СтрокаРасходов.СуммаПР;
		
		СтрокаВозвратов.СуммаБУ = СтрокаВозвратов.СуммаБУ - СтрокаРасходов.СуммаБУ;
		СтрокаВозвратов.СуммаНУ = СтрокаВозвратов.СуммаНУ - СтрокаРасходов.СуммаНУ;
		СтрокаВозвратов.СуммаПР = СтрокаВозвратов.СуммаПР - СтрокаРасходов.СуммаПР;
		СтрокаВозвратов.СуммаВР = СтрокаВозвратов.СуммаВР - СтрокаРасходов.СуммаВР;
	Иначе
		СтрокаДоходов = Доходы.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДоходов, СтруктураСтроки, "СтатьяДоходов, АналитикаДоходов");
		ЗаполнитьЗначенияСвойств(СтрокаДоходов, СтрокаВозвратов,, "СуммаБУ, СуммаНУ, СуммаПР, СуммаВР, Количество");
		СтрокаДоходов.СуммаБУ = СтруктураСтроки.Сумма - СтрокаВозвратов.СуммаБУ;
...
Функция ВыполнитьЗапросПогашенияСтоимости()
	...
	|	ЕСТЬNULL(НАЧАЛОПЕРИОДА(Партии.Дата, МЕСЯЦ), ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)) = &НачДатаПрошлогоМесяца КАК ПартияПолученаПрошлымМесяцем,
	...
	|				ИнвентарныйНомер В
	|					(ВЫБРАТЬ
	|						ОстаткиВЭксплуатации.ПартияТМЦВЭксплуатации.ИнвентарныйНомер
	|					ИЗ
	|						ОстаткиВЭксплуатации КАК ОстаткиВЭксплуатации)) КАК НаработкиНачалоПериода
	|		ПО ОстаткиВЭксплуатации.ПартияТМЦВЭксплуатации.ИнвентарныйНомер = НаработкиНачалоПериода.ИнвентарныйНомер
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НаработкиТМЦВЭксплуатации.СрезПоследних(
	...
	|		ПО ОстаткиВЭксплуатации.ПартияТМЦВЭксплуатации.ИнвентарныйНомер = НаработкиНачалоПрошлогоМесяца.ИнвентарныйНомер
	...
	|		ПО ОстаткиВЭксплуатации.ПартияТМЦВЭксплуатации.ИнвентарныйНомер = НаработкиОкончаниеПериода.ИнвентарныйНомер
	|ГДЕ
	|	ОстаткиВЭксплуатации.ПартияТМЦВЭксплуатации.СпособПогашенияСтоимостиБУ = ЗНАЧЕНИЕ(Перечисление.СпособыПогашенияСтоимостиТМЦ.ПоНаработке)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВозвратИзЭксплуатацииТовары.СтатьяДоходов КАК СтатьяДоходов,
	...
	|				ТОГДА ИСТИНА
	|			КОГДА ВозвратИзЭксплуатации.Дата МЕЖДУ &НачДатаПрошлогоМесяца И &НачДата
	...
	|				ТОГДА ИСТИНА
	|			КОГДА ПеремещениеВЭксплуатации.Дата МЕЖДУ &НачДатаПрошлогоМесяца И &НачДата
	...
	|				ТОГДА ИСТИНА
	|			КОГДА СписаниеИзЭксплуатации.Дата МЕЖДУ &НачДатаПрошлогоМесяца И &НачДата
