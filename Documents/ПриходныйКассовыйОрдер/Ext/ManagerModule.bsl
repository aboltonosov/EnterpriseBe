...
#Область ВнутренняяПередачаДС
	...
	|
	// 4D:ERP для Беларуси, Екатерина, 04.08.2015 16:30:38 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 26.10.2017 11:28:20 
	// Локализация печатных форм. Документ ПКО., №16377
	// {
	Если ПравоДоступа("Изменение", Метаданные.Документы.ПриходныйКассовыйОрдер) Тогда
		// Приходный кассовый ордер
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "КО1_2016";
		...
		// Приходный кассовый ордер
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "КО1_ПЭВМ_2016";
		КомандаПечати.Представление = НСтр("ru = 'Приходный кассовый ордер (ПЭВМ)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		//++ НЕ УТ
		КомандаПечати.Обработчик = "УправлениеПечатьюУПКлиент.ПроверкаОтраженияВРеглУчете";
		//-- НЕ УТ
		...
		// Приходный кассовый ордер
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "КО1в_2016";
		КомандаПечати.Представление = НСтр("ru = 'Приходный кассовый ордер (валютный)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		//++ НЕ УТ
		КомандаПечати.Обработчик = "УправлениеПечатьюУПКлиент.ПроверкаОтраженияВРеглУчете";
		//-- НЕ УТ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 22.06.2015 14:23:32 
	// Локализация печатных форм, №7762
	// {
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КО1_ПЭВМ") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КО1_ПЭВМ",
			НСтр("ru='Приходный кассовый ордер'"),
			СформироватьПечатнуюФормуКО1(МассивОбъектов, ОбъектыПечати, Неопределено, ПараметрыПечати, "ПФ_MXL_КО1_ПЭВМ_Локализация"));
	...
	// Локализация печатных форм. Документ ПКО., №16377
	// {
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КО1_2016") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
			"КО1_2016",
			"Приходный кассовый ордер КО-1 2016",
			СформироватьПечатнуюФормуКО1_2016(МассивОбъектов, ОбъектыПечати, Неопределено)
		);
		
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КО1_ПЭВМ_2016") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КО1_ПЭВМ_2016",
			"Приходный кассовый ордер КО-1 ПЭВМ",
			СформироватьПечатнуюФормуКО1_ПЭВМ_2016(МассивОбъектов, ОбъектыПечати, Неопределено)
		);
		
	КонецЕсли;

	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КО1в_2016") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КО1в_2016",
			"Приходный кассовый ордер КО-1в",
			СформироватьПечатнуюФормуКО1В_2016(МассивОбъектов, ОбъектыПечати, Неопределено)
		);
...
// 4D:ERP для Беларуси, ВладимирР, 07.12.2015 15:52:21 
// Локализация печатных форм, №7762
// {
Функция СформироватьПечатнуюФормуКО1(МассивОбъектов, ОбъектыПечати, КомплектыПечати, ПараметрыПечати, ИмяМакета = "ПФ_MXL_КО1_Локализация")
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КО1";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходныйКассовыйОрдер." + ИмяМакета);
// }
// 4D
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.Сумма = ФормированиеПечатныхФорм.СуммаРубКоп(
			Выборка.Сумма,
			Выборка.Валюта,
		...
		// Локализация печатных форм, №7762
		// {
		Если ИмяМакета = "ПФ_MXL_КО1_Локализация" Тогда
...
// Локализация печатных форм. Документ ПКО., №16377
// {
Функция СформироватьПечатнуюФормуКО1_2016(МассивОбъектов, ОбъектыПечати, КомплектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КО1";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходныйКассовыйОрдер.ПФ_MXL_КО1_2016_Локализация");
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ДанныеДокумента.РасшифровкаПлатежа.(
	               |		СтатьяДвиженияДенежныхСредств
	               |	)
	               |ИЗ
	               |	Документ.ПриходныйКассовыйОрдер КАК ДанныеДокумента
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.НДС = ЗаполнитьНДС(СокрЛП(Выборка.ВТомЧислеНДС));
		СуммаНДС = Формат(ЗаполнитьСуммуНДС(Выборка.ВТомЧислеНДС),"ЧДЦ=2");
		
		Попытка
			СуммаНДС = Число(СуммаНДС);
			Если СуммаНДС = ЦЕЛ(СуммаНДС) Тогда
				ОбластьМакета.Параметры.СуммаНДСКоп = "00";	
			Иначе
				ОбластьМакета.Параметры.СуммаНДСКоп = (СуммаНДС - ЦЕЛ(СуммаНДС)) * 100;
			КонецЕсли;
	        ОбластьМакета.Параметры.СуммаНДС = ЦЕЛ(СуммаНДС);	
		Исключение
		    ОбластьМакета.Параметры.СуммаНДС = СуммаНДС;
			ОбластьМакета.Параметры.СуммаНДСКоп =  СуммаНДС;
		КонецПопытки;
				
		Сумма = Строка(Выборка.Сумма);
		...
		Если НЕ ЗначениеЗаполнено(СтатьяДвиженияДенежныхСредств) Тогда
			СтатьяДДС = Выборка.РасшифровкаПлатежа.Выбрать();
			СтатьяДДС.Следующий();
		    СтатьяДвиженияДенежныхСредств = СтатьяДДС.СтатьяДвиженияДенежныхСредств;
		КонецЕсли; 
...
Функция СформироватьПечатнуюФормуКО1В_2016(МассивОбъектов, ОбъектыПечати, КомплектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КО1В";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходныйКассовыйОрдер.ПФ_MXL_КО1в_2016");
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.НДС		 = ЗаполнитьНДС(СокрЛП(Выборка.ВТомЧислеНДС));
        ОбластьМакета.Параметры.СуммаНДС = Формат(ЗаполнитьСуммуНДС(Выборка.ВТомЧислеНДС), "ЧЦ=15; ЧДЦ=2");		
		...
		ОбластьМакета.Параметры.ВалютаКурс = Формат(ВалютаКурс.Курс,"ЧДЦ=4");//ВалютаКурс.Курс;
...
Функция СформироватьПечатнуюФормуКО1_ПЭВМ_2016(МассивОбъектов, ОбъектыПечати, КомплектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.АвтоМасштаб = Истина;
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходныйКассовыйОрдер.ПФ_MXL_КО1_ПЭВМ_2016_Локализация");
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.Сумма = Формат(Выборка.Сумма, "ЧЦ=15; ЧДЦ=2");
...
Функция ЗаполнитьНДС(ВТомЧислеНДС)

	Если ВТомЧислеНДС = "Без налога (НДС)" ИЛИ ПустаяСтрока(ВТомЧислеНДС)Тогда
			Возврат "—";		
	Иначе	
		КолНДС = СтрЧислоВхождений(ВТомЧислеНДС, "(");
		НДС = "";
		СтрокаНДС = ВТомЧислеНДС;
		Индекс = 1;
		
		Пока Индекс <= КолНДС Цикл
			НачПозСкобки = Найти(СтрокаНДС, "(");
			КонПозСкобки = Найти(СтрокаНДС, ")");
			ПозЗапятой   = Найти(СтрокаНДС, ", ");
			Если Не ЗначениеЗаполнено(НДС) Тогда 
				НДС = Сред(СтрокаНДС, НачПозСкобки+1, КонПозСкобки-НачПозСкобки-1);
		   	Иначе 
				НДС = НДС + ", " + Сред(СтрокаНДС, НачПозСкобки + 1, КонПозСкобки-НачПозСкобки-1);
		   	КонецЕсли;

			СтрокаНДС = Сред(СтрокаНДС, ПозЗапятой + 1);
			Индекс = Индекс + 1;
		КонецЦикла; 
...
Функция ЗаполнитьСуммуНДС(ВТомЧислеНДС)

	Если СокрЛП(ВТомЧислеНДС) = "Без налога (НДС)"  ИЛИ ПустаяСтрока(ВТомЧислеНДС)Тогда
		Возврат "—";		
	Иначе	
		КолНДС = СтрЧислоВхождений(ВТомЧислеНДС, "(");
		СуммаНДС = "";
		...
		Пока Индекс <= КолНДС Цикл
			...
			КонПозСкобки = Найти(СтрокаНДС, ")");
			ПозЗапятой   = Найти(СтрокаНДС, ", ");
			Если Найти(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2), "-") > 0 Тогда
				Если Не ЗначениеЗаполнено(СуммаНДС) Тогда 
					СуммаНДС = Число(СтрЗаменить(Лев(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2), Найти(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2), " ")), "-", ".")); 
			   	Иначе 
					СуммаНДС = СуммаНДС + Число(СтрЗаменить(Лев(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2), Найти(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2), " ")), "-", "."));
			   	КонецЕсли;
			Иначе
				Если Не ЗначениеЗаполнено(СуммаНДС) Тогда 
					СуммаНДС = Число(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2)); 
			   	Иначе 
					СуммаНДС = СуммаНДС + Число(Сред(СтрокаНДС, КонПозСкобки + 2, ПозЗапятой - КонПозСкобки - 2));
			   	КонецЕсли;
			КонецЕсли;
...
КонецФункции // ЗаполнитьСуммуНДС()
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 27.10.2017 9:22:34 
	// Локализация печатных форм. Документ ПКО., №16377
	// {
	ИмяМакета = "КО1_2016";
	...
	Если ТекущийКомплект.Количество() > 0 Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			СформироватьПечатнуюФормуКО1_2016(ТекущийМассивОбъектов, ОбъектыПечати, ТекущийКомплект)
		);
	КонецЕсли;
	
	ИмяМакета = "КО1_ПЭВМ_2016";
	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
	Если ТекущийКомплект.Количество() > 0 Тогда
		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			СформироватьПечатнуюФормуКО1_ПЭВМ_2016(ТекущийМассивОбъектов, ОбъектыПечати, ТекущийКомплект)
		);
	КонецЕсли;
	...
	Если ТекущийКомплект.Количество() > 0 Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			СформироватьПечатнуюФормуКО1В_2016(ТекущийМассивОбъектов, ОбъектыПечати, ТекущийКомплект)
		);
	КонецЕсли;
	// }
	// 4D
...
Функция КомплектПечатныхФорм() Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 27.10.2017 9:27:09 
	// Локализация печатных форм. Документ ПКО., №16377
	// {
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "КО1_2016", "Приходный кассовый ордер КО-1", 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "КО1_ПЭВМ_2016", "Приходный кассовый ордер КО-1 ПЭВМ", 1);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "КО1в_2016", "Приходный кассовый ордер КО-1 (валютный)", 1);
	// }
	// 4D
