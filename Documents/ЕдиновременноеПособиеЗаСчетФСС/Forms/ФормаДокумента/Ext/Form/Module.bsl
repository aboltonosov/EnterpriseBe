...
Процедура ВидПособияПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Яна, 07.06.2017 10:17:31 
...
Процедура РассчитатьПособие(ВыводитьСообщения = Ложь)
	...
	// 4D:ERP для Беларуси, Яна, 07.06.2017 10:21:09 
	// Документ "Единовременные пособия за счет ФСЗН", №14887 
	// {
	...
	ИначеЕсли Объект.ВидПособия  = Перечисления.ПереченьПособийСоциальногоСтрахования.ПриРожденииВторогоИПоследующихДетей Тогда	
		ГосударственноеПособиеВТвердыхСуммах = "ПриРожденииВторогоИПоследующихДетей";		
	КонецЕсли;
	// }
	// 4D
...
Функция СписокВыбораВидаПособия()
	...
	// 4D:ERP для Беларуси, Яна, 07.06.2017 12:24:09 
	// Документ "Единовременные пособия за счет ФСЗН", №14887
	// {
	СписокВыбора = Новый СписокЗначений;
	СписокВыбора.Добавить(Перечисления.ПереченьПособийСоциальногоСтрахования.ВСвязиСоСмертью);
	Если НЕ ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(Объект.Организация, Объект.ПериодРегистрации)  Тогда
		СписокВыбора.Добавить(Перечисления.ПереченьПособийСоциальногоСтрахования.ПриПостановкеНаУчетВРанниеСрокиБеременности);
		СписокВыбора.Добавить(Перечисления.ПереченьПособийСоциальногоСтрахования.ПриРожденииРебенка);
		СписокВыбора.Добавить(Перечисления.ПереченьПособийСоциальногоСтрахования.ПриРожденииВторогоИПоследующихДетей);		
	КонецЕсли;	
