...
Процедура ЗаполнитьПервоначальныеСуммыПриОтражении(МенеджерВременныхТаблиц) Экспорт
	...
	ПроведениеСерверУТ.ИницализироватьТаблицыДляДвижений(Запрос, ТекстыЗапроса, ТаблицыДляДвижений, Ложь, Ложь, Истина);
	...
	Попытка
		...
		НаборЗаписей.Отбор.Регистратор.Установить(РеквизитыДокумента.Ссылка);
		НаборЗаписей.Загрузить(ТаблицыДляДвижений.ПервоначальныеСведенияНМАБухгалтерскийУчет);
		НаборЗаписей.Записать();
		
		НаборЗаписей = РегистрыСведений.ПервоначальныеСведенияНМАНалоговыйУчет.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Регистратор.Установить(РеквизитыДокумента.Ссылка);
		НаборЗаписей.Загрузить(ТаблицыДляДвижений.ПервоначальныеСведенияНМАНалоговыйУчет);
		НаборЗаписей.Записать();
...
Процедура ДанныеДокументаИУчета(ТекстыЗапроса)
	...
	|				&ГраницаКонецМесяца,
	|
	// 4D:ERP для Беларуси, Евгений, 27.01.2017 12:22:29 
	// Ошибка проведения и отражения в регл.учете документа ПринятиеКУчетуНМА, №13899
	// {
...
Процедура ПрочиеРасходы(ТекстыЗапроса, Регистры)
	...
	|		ТОГДА 0
	|		ИНАЧЕ ТаблицаНМА.ПервоначальнаяСтоимостьНУ
...
Функция ВременнаяТаблицаОстаткиСчетаКапитализации()
	...
	|			Регистратор,
	|
	// 4D:ERP для Беларуси, Евгений, 27.01.2017 12:26:45 
	// Ошибка проведения и отражения в регл.учете документа ПринятиеКУчетуНМА, №13899
	// {
	|			Счет В (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПриобретениеНематериальныхАктивов)),
	// }
...
Функция ВременнаяТаблицаЦелевоеФинансирование()
	...
	|			ТОГДА ВЫРАЗИТЬ(втЦелевоеФинансированиеНМА.Сумма / втИсточникиЦелевогоФинансированияСгруппированные.Количество КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ ВЫРАЗИТЬ(втЦелевоеФинансированиеНМА.Сумма * (втИсточникиЦелевогоФинансирования.Сумма / втИсточникиЦелевогоФинансированияСгруппированные.Сумма) КАК ЧИСЛО(15, 2))
	|	КОНЕЦ КАК Сумма,
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	Возврат ПринятиеКУчету()
		+ Разделитель + ПринятиеКУчетуЦелевыеСредства()
		+ Разделитель + ПринятиеКУчетуЧастичныеЦелевыеСредства()
		+ Разделитель + ВключениеВРасходыПриПринятииКУчету()
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапроса =
 	...
 	|	ПринятиеКУчетуНМА.ВерсияДанных,
 	|	ЕСТЬNULL(ПринятиеКУчетуНМА.НематериальныйАктив.ВидОбъектаУчета, ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовУчетаНМА.НематериальныйАктив)) КАК ВидОбъектаУчета,
 	|	НЕ ЗаблокированоНематериальныйАктив.Ссылка ЕСТЬ NULL КАК ЧитаемыеДанныеЗаблокированы
 	|ИЗ
 	|	&ВТДляОбработкиСсылка КАК СсылкиДляОбработки
 	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПринятиеКУчетуНМА КАК ПринятиеКУчетуНМА
 	|		ПО (ПринятиеКУчетуНМА.Ссылка = СсылкиДляОбработки.Ссылка)
 	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтЗаблокированоНематериальныеАктивы КАК ЗаблокированоНематериальныйАктив
 	|		ПО (ЗаблокированоНематериальныйАктив.Ссылка = ПринятиеКУчетуНМА.НематериальныйАктив)";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ВТДляОбработкиСсылка", ПараметрыДляОбработки.ИмяВременнойТаблицы);
	
	ПараметрыВТ = ОбновлениеИнформационнойБазы.СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(
		Параметры.Очередь, "Справочник.НематериальныеАктивы", МенеджерВременныхТаблиц);
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ВтЗаблокированоНематериальныеАктивы", ПараметрыВТ.ИмяВременнойТаблицы);
	...
	Выборка = Результат.Выбрать();
 	...
 	Пока Выборка.Следующий() Цикл
 		...
 		Попытка
			...
			Если Выборка.ВидОбъектаУчета = Перечисления.ВидыОбъектовУчетаНМА.НематериальныйАктив
				И НЕ ЗначениеЗаполнено(ДанныеОбъекта.ПорядокУчетаНУ) Тогда
			
				ДанныеОбъекта.ПорядокУчетаНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.НачислениеАмортизации;
			КонецЕсли; 
			
			Если НЕ ЗначениеЗаполнено(ДанныеОбъекта.ВидОбъектаУчета) Тогда
				ДанныеОбъекта.ВидОбъектаУчета = Выборка.ВидОбъектаУчета;
			КонецЕсли; 
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ПринятиеКУчетуНМА КАК ПринятиеКУчетуНМА
	|ГДЕ
	|	(ПринятиеКУчетуНМА.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПустаяСсылка)
	|				И ПринятиеКУчетуНМА.НематериальныйАктив.ВидОбъектаУчета = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовУчетаНМА.НематериальныйАктив)
	|			ИЛИ ПринятиеКУчетуНМА.ВидОбъектаУчета = ЗНАЧЕНИЕ(Перечисление.ВидыОбъектовУчетаНМА.ПустаяСсылка))";
