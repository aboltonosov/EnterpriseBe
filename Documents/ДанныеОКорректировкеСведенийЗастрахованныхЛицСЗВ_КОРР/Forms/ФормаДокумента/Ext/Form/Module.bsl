...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Объект.КорректируемыйПериод = '20160101';
...
Процедура КорректируемыйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ПерсонифицированныйУчетКлиент.ОтчетныйПериодРегулирование(Объект.КорректируемыйПериод, КорректируемыйПериодСтрока, Направление, ,'20161001');
...
Процедура ЗаполнитьДанныеОЗаработкеСотрудников(СписокСотрудников)
	...
	Если Год(Объект.КорректируемыйПериод) < 2014 Тогда 
		Возврат;
	КонецЕсли;
