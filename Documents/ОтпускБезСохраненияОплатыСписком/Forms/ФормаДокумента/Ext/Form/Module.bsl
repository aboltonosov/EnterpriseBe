...
Процедура ДополнитьФорму(ОтложенноеИзменение = Неопределено)
	...
	Иначе
		...
		ИсправлениеДокументовЗарплатаКадры.ГруппаИсправлениеДополнитьФорму(
			ЭтаФорма, Ложь,, ДобавлятьЭлементыФормы, ДобавлятьРеквизитыФормы, ОтложенноеИзменение);
...
Процедура ЗаполнитьКоличествоДнейОтпускаПоСтроке(СтрокаСотрудника)
	...
	Если ЗначениеЗаполнено(СтрокаСотрудника.ДатаНачала)
		...
		СтрокаСотрудника.КоличествоДнейОтпуска = (КонецДня(СтрокаСотрудника.ДатаОкончания) - НачалоДня(СтрокаСотрудника.ДатаНачала))/86400;
	Иначе
...
Функция ЕстьФиксированныеДанные(Форма)
	...
	Возврат РасчетЗарплатыРасширенныйКлиентСервер.ЕстьФиксированныеДанныеВТаблице(Форма, ОписаниеТаблицыПерерасчетов(Форма.РегистрацияНачисленийДоступна));
