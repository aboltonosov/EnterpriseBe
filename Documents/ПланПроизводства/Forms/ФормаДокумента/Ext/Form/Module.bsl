...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.Проведен Тогда
		...
		ПодключитьОбработчикОжидания("КонтрольРассчетаПотребностей", 3);
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		СтруктураДействий.Вставить("ПредставлениеТаблицы",НСтр("ru='Продукция'"));
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если Объект.Проведен Тогда
		...
		ПодключитьОбработчикОжидания("КонтрольРассчетаПотребностей", 3);
	Иначе
		Элементы.РассчетПотребностиВМатериалах.ТекущаяСтраница = Элементы.СтраницаНеРассчитана;
	КонецЕсли;
...
Процедура ВидПланаПриИзмененииЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Объект.Замещающий и Объект.Продукция.Количество() = 0 Тогда
...
Процедура ПереключитьПериодыВКолонки(Команда)
	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru='В строках списка ""Продукция"" указаны даты за границами планирования - строки будут удалены. Удалить строки?'");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Реквизиты = "ОтображатьНомерПериода, ИспользоватьДляПланированияМатериалов, ПланированиеПоНазначениям";
	...
	ПланированиеПоНазначениям = ПараметрыСценария.ПланированиеПоНазначениям;
	...
	ЗамещающийСуммирующий = ?(Объект.Замещающий, "Замещающий", "");
...
Процедура СценарийПриИзмененииСервер()
	...
	Реквизиты = "Периодичность, ОтображатьНомерПериода, ИспользоватьДляПланированияМатериалов, ПланированиеПоНазначениям";
	//++ НЕ УТ
...
Процедура УстановитьВидимость()
	...
	Элементы.ПродукцияНазначение.Видимость = ПланированиеПоНазначениям И ЗаполнятьНазначениеВТЧ И ИспользоватьОбособленноеОбеспечениеЗаказов;
	Элементы.ПродукцияПоДатамНазначение.Видимость = ПланированиеПоНазначениям И ЗаполнятьНазначениеВТЧ И ИспользоватьОбособленноеОбеспечениеЗаказов;
...
Функция ПолучитьПараметрыВидаПлана(Знач ВидПлана, Знач ОбновлятьПравило, АдресПравилаЗаполнения, АдресПользовательскихНастроек)
	...
	Реквизиты = "КоличествоПериодов, ЗаполнятьПодразделение, Замещающий, ЗапретитьРедактированиеПравила, ЗаполнятьПоФормуле, ЗаполнятьНазначениеВТЧ, ПланироватьПолуфабрикатыАвтоматически";
	//++ НЕ УТ
...
Процедура КонтрольРассчетаПотребностей()
	...
	ДанныеОчереди = ДанныеОчередиПересчетаПотребностей(Объект.Ссылка);
	
	Если ДанныеОчереди.ЕстьЗаписи Тогда
		
		Если ДанныеОчереди.ЕстьОшибки Тогда
			
			Элементы.РассчетПотребностиВМатериалах.ТекущаяСтраница = Элементы.СтраницаНеРассчитанаЕстьОшибки;
			
		Иначе
			
			Элементы.РассчетПотребностиВМатериалах.ТекущаяСтраница = Элементы.СтраницаРассчитывается;
				
		КонецЕсли;
		
	Иначе
				
		Элементы.РассчетПотребностиВМатериалах.ТекущаяСтраница = Элементы.СтраницаРассчитана;	
...
&НаСервереБезКонтекста
Функция ДанныеОчередиПересчетаПотребностей(ПланПроизводства)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ОчередьРасчетаПланаПроизводства.ЕстьОшибкиРасчета
		|ИЗ
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ОчередьРасчетаПланаПроизводства.Разделитель УБЫВ");
		
	Запрос.УстановитьПараметр("ПланПроизводства", ПланПроизводства);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Результат = Новый Структура("ЕстьЗаписи, ЕстьОшибки", Ложь, Ложь);
	
	Если Выборка.Следующий() Тогда
		
		Результат.ЕстьЗаписи = Истина;
		Результат.ЕстьОшибки = Выборка.ЕстьОшибкиРасчета;
...
Процедура ПровестиИПересчитатьПотребностиЗавершение(ОтветНаВопрос, ДополнительныеПараметры) Экспорт
	...
	Если ОтветНаВопрос = КодВозвратаДиалога.Да Тогда
		...
		ПровестиИПересчитатьПотребности();
...
&НаСервере
Процедура ПровестиИПересчитатьПотребности()
	
	Если Объект.Проведен Тогда
		
		ЗаписатьНеобходимостьПересчетаПотребностей();
...
&НаСервере
Процедура ЗаписатьНеобходимостьПересчетаПотребностей()
	
	Документы.ПланПроизводства.ЗаписатьОчередьФоновогоПроведения(Объект.Ссылка, Объект.Сценарий, Истина);
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	СтруктураПоиска = Новый Структура("ИдентификаторНоменклатура, ИдентификаторХарактеристика, ИдентификаторУпаковка, 
		|ИдентификаторСпецификация, ИдентификаторНазначение", 0, 0, 0, 0, 0);
...
Функция ПараметрыВидаПлана(Форма)
	...
	ПараметрыВидаПлана = Новый Структура("ЗаполнятьПодразделение, ПланированиеПоНазначениям");
...
Функция ПараметрыОтбора(Форма)
	...
	ПараметрыОтбора = Новый Структура("Подразделение, Назначение");
...
Процедура ОбработатьРезультатЗаполненияПоФормуле()
	...
	Если Объект.КроссТаблица 
		...
		Для каждого Период Из ЭтаФорма.Периоды.НайтиСтроки(Новый Структура("Активная", Истина)) Цикл
			Для Каждого СтрокаТоваров Из ПродукцияПоДатам Цикл
...
Функция ПолучитьРезультатЗаполненияНаСервере(КэшированныеЗначения)
	...
	Для каждого СтрокаТовара Из Объект.Продукция Цикл
		...
		ПараметрыСпецификации.Вставить("Подразделение",	Объект.Подразделение);
		Если ЗаполнятьСпецификациюПоПериодам Тогда
			ПараметрыСпецификации.Вставить("НачалоПериода",	СтрокаТовара.ДатаВыпуска);
		Иначе
			ПараметрыСпецификации.Вставить("НачалоПериода",	Объект.НачалоПериода);
		КонецЕсли;
