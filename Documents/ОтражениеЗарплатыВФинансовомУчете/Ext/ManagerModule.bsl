...
Функция БазаРаспределенияТрудозатрат()
	...
	|	НЕОПРЕДЕЛЕНО КАК ПартияПроизводства,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) КАК СтатьяКалькуляцииПФР,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) КАК СтатьяКалькуляцииФСС,
	|	ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка) КАК СтатьяКалькуляцииФОМС
	...
	|	НЕОПРЕДЕЛЕНО КАК ПартияПроизводства,
	|	РесурсныеСпецификации.СтатьяКалькуляцииПФР,
	|	РесурсныеСпецификации.СтатьяКалькуляцииФСС,
	|	РесурсныеСпецификации.СтатьяКалькуляцииФОМС
	|ИЗ
	...
	|	И РесурсныеСпецификации.СтатьяКалькуляцииПФР <> ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ЕСТЬNULL(СтатьиВзносов.СтатьяКалькуляцииПФР, ВТТрудозатраты.СтатьяКалькуляции) КАК СтатьяКалькуляцииПФР,
	|	ЕСТЬNULL(СтатьиВзносов.СтатьяКалькуляцииФСС, ВТТрудозатраты.СтатьяКалькуляции) КАК СтатьяКалькуляцииФСС,
	|	ЕСТЬNULL(СтатьиВзносов.СтатьяКалькуляцииФОМС, ВТТрудозатраты.СтатьяКалькуляции) КАК СтатьяКалькуляцииФОМС,
	...
	|	База.СтатьяКалькуляцииПФР КАК СтатьяКалькуляцииПФР,
	|	База.СтатьяКалькуляцииФСС КАК СтатьяКалькуляцииФСС,
	|	База.СтатьяКалькуляцииФОМС КАК СтатьяКалькуляцииФОМС,
	...
	|	Начислено.ПФРСтраховая КАК ПФРСтраховая,
	|	Начислено.ПФРНакопительная КАК ПФРНакопительная,
	|	Начислено.ПФРПоСуммарномуТарифу КАК ПФРПоСуммарномуТарифу,
	|	Начислено.ПФРДоПредельнойВеличины КАК ПФРДоПредельнойВеличины,
	|	Начислено.ПФРСПревышения КАК ПФРСПревышения,
	|	Начислено.ПФРНаДоплатуЛетчикам КАК ПФРНаДоплатуЛетчикам,
	|	Начислено.ПФРНаДоплатуШахтерам КАК ПФРНаДоплатуШахтерам,
	|	Начислено.ПФРЗаЗанятыхНаПодземныхИВредныхРаботах КАК ПФРЗаЗанятыхНаПодземныхИВредныхРаботах,
	|	Начислено.ПФРЗаЗанятыхНаТяжелыхИПрочихРаботах КАК ПФРЗаЗанятыхНаТяжелыхИПрочихРаботах,
	|	Начислено.ФСС КАК ФСС,
	|	Начислено.ФФОМС КАК ФФОМС,
	|	Начислено.ТФОМС КАК ТФОМС,
	|	Начислено.ФССНесчастныеСлучаи КАК ФССНесчастныеСлучаи,
	|	Начислено.ПФРЗаЗанятыхНаПодземныхИВредныхРаботахБезСпецОценки КАК ПФРЗаЗанятыхНаПодземныхИВредныхРаботахБезСпецОценки,
	|	Начислено.ПФРЗаЗанятыхНаПодземныхИВредныхРаботахСпецОценка КАК ПФРЗаЗанятыхНаПодземныхИВредныхРаботахСпецОценка,
	|	Начислено.ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахБезСпецОценки КАК ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахБезСпецОценки,
	|	Начислено.ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахСпецОценка КАК ПФРЗаЗанятыхНаТяжелыхИПрочихРаботахСпецОценка,
...
Функция ПараметрыРаспределенияСтроки(ВидСуммы, СтрокаБазы, ДанныеСпособа)
	...
	Если ВидСуммы.ВидФондаВзносов = Перечисления.ВидыФондовВзносов.ПФР Тогда
		Параметры.Вставить("СтатьяКалькуляции", СтрокаБазы.СтатьяКалькуляцииПФР);
	ИначеЕсли ВидСуммы.ВидФондаВзносов = Перечисления.ВидыФондовВзносов.ФСС Тогда
		Параметры.Вставить("СтатьяКалькуляции", СтрокаБазы.СтатьяКалькуляцииФСС);
	ИначеЕсли ВидСуммы.ВидФондаВзносов = Перечисления.ВидыФондовВзносов.ФОМС Тогда
		Параметры.Вставить("СтатьяКалькуляции", СтрокаБазы.СтатьяКалькуляцииФОМС);
	КонецЕсли;
