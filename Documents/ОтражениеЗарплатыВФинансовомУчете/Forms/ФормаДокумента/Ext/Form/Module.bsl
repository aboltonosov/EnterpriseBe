...
Процедура СводноПриИзменении(Элемент)
	...
	ИначеЕсли Не Сводно И (Объект.НачисленнаяЗарплатаИВзносы.Количество() > 0 Или Объект.НачисленныйНДФЛ.Количество() > 0) Тогда
		...
		// 4D:ERP для Беларуси, Яна, 25.08.2017 14:59:40 
		// Локализовать документ "Отражение зарплаты в финансовом учете", №15826 
		// {
		ТекстВопроса = НСтр("ru = 'Списки ""Начисленная зарплата и взносы"" и ""Начисленный ПН"" будет очищены. Продолжить?'");
		// }
		// 4D
...
Процедура ЗаполнитьНачисленныйНДФЛ(Команда)
	СписокТаблиц = Новый СписокЗначений();
	// 4D:ERP для Беларуси, , 04.09.2017 10:28:50 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826 
	// {
	СписокТаблиц.Добавить("НачисленныйНДФЛ",             Нстр("ru = 'Начисленый ПН'"));
	// }
	// 4D	
	ЗаполнитьТаблицу(СписокТаблиц);
КонецПроцедуры
...
Процедура ПоказатьВзносыПодробно(Команда)
	...
	// 4D:ERP для Беларуси, Яна, 04.09.2017 10:38:24 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826 
	// {
...
Процедура УстановитьУсловноеОформление()
	...
	СписокЗначений = Новый СписокЗначений;
	// 4D:ERP для Беларуси, АлексейЧ, 17.12.2017 22:54:10 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	...
	// 4D:ERP для Беларуси, Яна, 04.09.2017 11:10:06 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НачисленнаяЗарплатаИВзносыПенсионныйФонд.Имя);

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НачисленнаяЗарплатаИВзносыБелгосстрах.Имя);
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НачисленнаяЗарплатаИВзносыПС.Имя);

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.НачисленнаяЗарплатаИВзносыСС.Имя);
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, АлексейЧ, 14.12.2017 11:26:31 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	СписокОпераций.Добавить(ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЗарплате.ПенсионныйФонд"));
	// }
	// 4D
...
Процедура УстановитьУсловноеОформлениеБелгосстрахФСЗН()
	...
	// суммы ФСЗН не должны отражаться при начислении за счет фонда ФСЗН
	...
	// суммы Белгосстрах не должны отражаться при начислении за счет фонда ФСЗН
...
Процедура ОбновитьСводнуюТаблицуПоДетальнымЗаписям()
	...
	// 4D:ERP для Беларуси, Яна, 04.09.2017 11:11:47 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	|	ДетальныеЗаписи.ФСЗН,
	//|	ДетальныеЗаписи.ПенсионныйФонд,
	|	ДетальныеЗаписи.Белгосстрах,
	//|	ДетальныеЗаписи.ПС,
	//|	ДетальныеЗаписи.СС,
	...
	|	СУММА(ВТДетальныеЗаписи.ФСЗН) КАК ФСЗН,
	//|	СУММА(ВТДетальныеЗаписи.ПенсионныйФонд) КАК ПенсионныйФонд,
	|	СУММА(ВТДетальныеЗаписи.Белгосстрах) КАК Белгосстрах,
	//|	СУММА(ВТДетальныеЗаписи.ПС) КАК ПС,
	//|	СУММА(ВТДетальныеЗаписи.СС) КАК СС,
...
Функция СпособОтраженияТребуется(ВидОперации, ВзносыВсего)
	...
	// 4D:ERP для Беларуси, АлексейЧ, 17.12.2017 19:46:03 
	// Локализовать документ "Отражение зарплаты в финансовом учете", № 15826
	// {
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюФСЗН") Тогда
	// }
	// 4D
		Возврат Ложь;
