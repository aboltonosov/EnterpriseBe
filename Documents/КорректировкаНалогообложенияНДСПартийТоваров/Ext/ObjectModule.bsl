...
Функция ВременныеТаблицыДанныхДокумента() Экспорт
	...
	Запрос.УстановитьПараметр("ТаблицаТовары",				ЗапасыСервер.ТаблицаДополненнаяОбязательнымиКолонками(Товары));
...
Процедура СформироватьДоступныеВидыЗапасов(МенеджерВременныхТаблиц) Экспорт
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТаблицаТоваров.ВидЗапасов КАК ВидЗапасов,
	|	ТаблицаТоваров.ВидЗапасов КАК ВидЗапасовПродавца
	|
...
Процедура СообщитьОбОшибкахЗаполненияВидовЗапасов(ТаблицаОшибок, МенеджерВременныхТаблиц)
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Корректировка НДС превышает остаток товара организации %1 на складе'"),
			Организация);
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Номенклатура: %1, недостаточно %2 %3 на складе %4'"),
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	|ИЗ
	|	Документ.КорректировкаНалогообложенияНДСПартийТоваров КАК ДанныеДокумента
	|ГДЕ
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
	 ...
	 ИЛИ ПерезаполнитьВидыЗапасов Тогда
