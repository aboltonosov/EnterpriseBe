...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВнеоборотныеАктивыПовтИсп.ТребуетсяНастройкаОтраженияРасходовПоНалогам(ГруппаОС) Тогда
...
Процедура ОбновитьСвойстваЭлементовФормы(ИзмененныеРеквизиты=Неопределено)
	...
	Если ОбновитьВсе Или Реквизиты.Свойство("ГруппаОС") Тогда
		...
		ТребуетсяНастройка = ВнеоборотныеАктивыПовтИсп.ТребуетсяНастройкаОтраженияРасходовПоНалогам(ГруппаОС);
...
// 4D:ERP для Беларуси, Яна, 10.04.2017 11:31:20 
// Задача №14610
// {
&НаСервере
Процедура ШифрЕНАОФПриИзмененииНаСервере()
			
	НормаАмортизационныхОтчислений = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ШифрПоЕНАОФ, "НормаАмортизационныхОтчислений");
	Если НормаАмортизационныхОтчислений <> Неопределено Тогда 
		НормаАмортизационныхОтчислений = НормаАмортизационныхОтчислений  * 12;
		СрокИспользованияНУ = НормаАмортизационныхОтчислений;
		СрокИспользованияБУ = НормаАмортизационныхОтчислений;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ШифрЕНАОФПриИзменении(Элемент)
	
	ШифрЕНАОФПриИзмененииНаСервере();
