...
Функция КонтекстВыполненияВыгрузки502(ОсновныеСведенияВыгрузки)
	...
	ВременныйКаталог = ?(Прав(ВременныйКаталог, 1) = "\", ВременныйКаталог, ВременныйКаталог + "\");
...
Процедура ЗаполнитьСведенияОПредметеСделки502(Контекст, ТекстСделки, Листы1Б, Сделка)
	...
	Для Каждого ТипПредмета ИЗ ТипыПредметовСделок Цикл
		...
		Для Каждого Лист1Б ИЗ ТипПредмета.Значение Цикл
			...
			ЗаполненОКП = ЗначениеЗаполнено(Лист1Б.Строка043КодПоОКП);
			...
			Если ТипПредСд = 1 Тогда
				...
				Если ЗаполненТНВЭД Тогда
					УстановитьНеобязательныйАтрибут(Шаблон_ПерПредСд, "ОКП", Лист1Б.Строка043КодПоОКП); // Атрибут может присутствовать
				Иначе
					УстановитьАтрибут(Шаблон_ПерПредСд, "ОКП", Лист1Б.Строка043КодПоОКП); // Атрибут должен присутствовать
				КонецЕсли;
...
Процедура ЗаполнитьСведенияОбУчастникахФизЛицах502(Контекст, ТекстСделки, Листы3, ДанныеРаздела3, Сделка)
	...
	Для Каждого Лист3 Из СписокЛистов3 Цикл
		...
		Иначе
			...
			АдресСтруктурой = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(ФизЛицо.КонтактнаяИнформацияЗначенияПолей);
...
Функция УсловиеВыполнено(Параметры, Условие)
	...
	Попытка
		РезультатВычисленияВыражения = РаботаВБезопасномРежиме.ВычислитьВБезопасномРежиме(СтрЗаменить(Условие, "&", "Параметры."), Параметры);
