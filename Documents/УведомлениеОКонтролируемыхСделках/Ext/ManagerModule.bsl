...
Процедура ПодготовитьДанныеАвтоматическогоЗаполнения(Уведомление) Экспорт
	...
	СтруктураПараметров = ПолучитьСтруктуруПараметровЗапросаКДокументамИсточникам(Уведомление);
	...
	// Документы покупки:
	ДобавитьСделкиПоступленияТоваровУслуг(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок);
...
// На основании служебных данных по контролируемым сделкам, определяет какие сделки относятся в разряд контролируемых и какие нет, возвращает эти данные в виде временных таблиц.
//	Параметры:
//		Уведомление - ДокументСсылка.УведомлениеОКонтролируемыхСделках - ссылка на документ уведомления,
...
// Возвращает текст настроек, с данными о методе выставления листа 1Б (для каждой сделки или для нескольких) и коде места представления.
//	Возвращаемое значение:
//		Строка - текст.
//
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.УведомлениеОКонтролируемыхСделках) Тогда
		...
		КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.УведомлениеОКонтролируемыхСделках.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ВводНаОсновании.ПредставлениеОбъекта(Метаданные.Документы.УведомлениеОКонтролируемыхСделках);
...
// Подготавливает основные параметры запроса к документам - источникам, на основании которых будет формироваться таблица контролируемых сделок.
Функция ПолучитьСтруктуруПараметровЗапросаКДокументамИсточникам(Уведомление)
...
// Создает запрос и заполняет его переданными параметрами для последующего запроса к документам - источникам, на основании которых будет формироваться таблица контролируемых сделок.
...
Процедура ДобавитьСделкиПоступленияТоваровУслуг(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИностранныеКонтрагенты КАК ИностранныеКонтрагенты
	|		ПО ПоступлениеТоваровУслуг.Контрагент = ИностранныеКонтрагенты.Контрагент
	|ГДЕ
	|	(НЕ ВзаимозависимыеЛицаПоПериодам.Контрагент ЕСТЬ NULL 
	|			ИЛИ НЕ ИностранныеКонтрагенты.Контрагент ЕСТЬ NULL )
	|	И ПоступлениеТоваровУслуг.Организация В(&СтруктураОрганизации)
	...
	|				КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты)
	|	И ВЫБОР
	|			КОГДА ПоступлениеТоваровУслуг.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПоступлениеТоваровУслуг.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|					ИЛИ ПоступлениеТоваровУслуг.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.Импорт)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыПоступления.Ссылка.Соглашение.НалогообложениеНДС, ТоварыПоступления.Ссылка.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ВЫБОР
	...
	|	КОНЕЦ КАК ТоварМировойБиржевойТорговли,
	|	ВЫБОР
	|		КОГДА ТоварыПоступления.ВидЗапасов.ТипЗапасов В (ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПустаяСсылка), ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар))
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.ПриобретениеТоваров)
	|		КОГДА ТоварыПоступления.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Услуга)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.ПолучениеУслуг)
	|		КОГДА ТоварыПоступления.ВидЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.АгентскаяУслуга)
	...
	|	ЕСТЬNULL(ТоварыПоступления.НомерГТД.СтранаПроисхождения, ЕСТЬNULL(ТоварыОрганизацийКОформлению.НомерГТД.СтранаПроисхождения, ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка))) КАК СтранаПроисхождения,
	|	ДокументыКонтролируемыхСделок.ВзаимозависимыеЛица,
	|	ДокументыКонтролируемыхСделок.ИностранныйКонтрагент,
	|	ДокументыКонтролируемыхСделок.ЗарегистрированВСтранеСЛьготнымНалогообложением
	|ПОМЕСТИТЬ ТоварыКонтролируемыхСделок
	...
	|		ПО ТоварыПоступления.Ссылка = ТоварыОрганизацийКОформлению.ДокументПоступления
	|			И ТоварыПоступления.ВидЗапасов = ТоварыОрганизацийКОформлению.ВидЗапасов
	|			И ТоварыПоступления.АналитикаУчетаНоменклатуры = ТоварыОрганизацийКОформлению.АналитикаУчетаНоменклатуры
...
Процедура ДобавитьСделкиПоступленияУслугПрочихАктивов(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПоступлениеУслугПрочихАктивов.Ссылка КАК РасчетныйДокумент,
	|	ВЫБОР
	|		КОГДА ВзаимозависимыеЛицаПоПериодам.Контрагент ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВзаимозависимыеЛицаПоПериодам КАК ВзаимозависимыеЛицаПоПериодам
	|		ПО (ВзаимозависимыеЛицаПоПериодам.Организация = &Организация)
	|			И (ВзаимозависимыеЛицаПоПериодам.Контрагент = ПоступлениеУслугПрочихАктивов.Контрагент)
	|			И ПоступлениеУслугПрочихАктивов.Дата >= ВзаимозависимыеЛицаПоПериодам.НачалоПериода
	|			И ПоступлениеУслугПрочихАктивов.Дата <= ВзаимозависимыеЛицаПоПериодам.ОкончаниеПериода
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИностранныеКонтрагенты КАК ИностранныеКонтрагенты
	|		ПО ПоступлениеУслугПрочихАктивов.Контрагент = ИностранныеКонтрагенты.Контрагент
	|ГДЕ
	|	(НЕ ВзаимозависимыеЛицаПоПериодам.Контрагент ЕСТЬ NULL 
	|			ИЛИ НЕ ИностранныеКонтрагенты.Контрагент ЕСТЬ NULL )
	|	И ПоступлениеУслугПрочихАктивов.Организация В(&СтруктураОрганизации)
	|	И ПоступлениеУслугПрочихАктивов.Дата >= &НачалоПериода
	|	И ПоступлениеУслугПрочихАктивов.Дата <= &ОкончаниеПериода
	|	И ПоступлениеУслугПрочихАктивов.Проведен
	|	И ПоступлениеУслугПрочихАктивов.Контрагент В
	|			(ВЫБРАТЬ
	|				КонтролируемыеКонтрагенты.Контрагент
	|			ИЗ
	|				КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты)
	|	И ВЫБОР
	|			КОГДА ПоступлениеУслугПрочихАктивов.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ПоступлениеУслугПрочихАктивов.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	РасчетныйДокумент
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(УслугиПрочиеАктивы.Ссылка.Соглашение.НалогообложениеНДС, УслугиПрочиеАктивы.Ссылка.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ЛОЖЬ КАК ТоварМировойБиржевойТорговли,
	...
	|ПОМЕСТИТЬ ТоварыКонтролируемыхСделок
	|ИЗ
	|	Документ.ПоступлениеУслугПрочихАктивов.Расходы КАК УслугиПрочиеАктивы
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументыКонтролируемыхСделок КАК ДокументыКонтролируемыхСделок
	|		ПО УслугиПрочиеАктивы.Ссылка = ДокументыКонтролируемыхСделок.РасчетныйДокумент
	|;
	|
...
Процедура ДобавитьСделкиВозвратаТоваровПоставщику(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|		ПО ВозвратТоваровПоставщику.ДокументПоступления = ПоступлениеТоваровУслуг.Ссылка
	|ГДЕ
	...
	|	И ПоступлениеТоваровУслуг.Дата <= &ОкончаниеПериода
	|	И ПоступлениеТоваровУслуг.Проведен
	|	И ВозвратТоваровПоставщику.ДокументПоступления <> ЗНАЧЕНИЕ(Документ.ПоступлениеТоваровУслуг.ПустаяСсылка)
	|	И ПоступлениеТоваровУслуг.Контрагент В
	|			(ВЫБРАТЬ
	|				КонтролируемыеКонтрагенты.Контрагент
	|			ИЗ
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыВозврата.Ссылка.Соглашение.НалогообложениеНДС, ТоварыВозврата.Ссылка.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ВЫБОР
...
Процедура ДобавитьСделкиКорректировкиПоступления(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	КорректировкаПоступления.ДокументОснование КАК Поступление
	|ПОМЕСТИТЬ ДокументыКорректировкиИРеализации
	|ИЗ
	...
	|				КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты)
	|	И ВЫБОР
	|			КОГДА КорректировкаПоступления.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ КорректировкаПоступления.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	|		КОНЕЦ
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Корректировка,
	...
	|	ВЫБОР
	|		КОГДА ПоступлениеТоваровУслуг.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА &ИспользованиеДоговоров
	|			ТОГДА ПоступлениеТоваровУслуг.Договор
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Договор,
	|	ВЫБОР
	|		КОГДА &ИспользованиеСоглашений
	|			ТОГДА ПоступлениеТоваровУслуг.Соглашение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Соглашение,
	|	ДокументыКорректировкиИРеализации.Корректировка КАК Корректировка,
	...
	|	ДокументыКорректировкиИРеализации КАК ДокументыКорректировкиИРеализации
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг КАК ПоступлениеТоваровУслуг
	|		ПО (ПоступлениеТоваровУслуг.Дата >= &НачалоПериода)
	|			И (ПоступлениеТоваровУслуг.Дата <= &ОкончаниеПериода)
	|			И (ПоступлениеТоваровУслуг.Проведен)
	|			И (ПоступлениеТоваровУслуг.Контрагент В
	|				(ВЫБРАТЬ
	...
	|				ИЗ
	|					КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты))
	|			И ДокументыКорректировкиИРеализации.Поступление = ПоступлениеТоваровУслуг.Ссылка
	|			И (ВЫБОР
	|				КОГДА ПоступлениеТоваровУслуг.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ПоступлениеТоваровУслуг.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	...
	|	ПоступлениеУслугПрочихАктивов.Валюта,
	|	ВЫБОР
	|		КОГДА ПоступлениеУслугПрочихАктивов.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	...
	|	ВЫБОР
	|		КОГДА &ИспользованиеДоговоров
	|			ТОГДА ПоступлениеУслугПрочихАктивов.Договор
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Договор,
	|	ВЫБОР
	|		КОГДА &ИспользованиеСоглашений
	|			ТОГДА ПоступлениеУслугПрочихАктивов.Соглашение
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Соглашение,
	|	ДокументыКорректировкиИРеализации.Корректировка,
	|	ЛОЖЬ
	...
	|		ПО (ПоступлениеУслугПрочихАктивов.Дата <= &ОкончаниеПериода)
	|			И (ПоступлениеУслугПрочихАктивов.Контрагент В
	|				(ВЫБРАТЬ
	|					КонтролируемыеКонтрагенты.Контрагент
	|				ИЗ
	...
	|			И (ПоступлениеУслугПрочихАктивов.Дата >= &НачалоПериода)
	|			И (ПоступлениеУслугПрочихАктивов.Проведен)
	|			И ДокументыКорректировкиИРеализации.Поступление = ПоступлениеУслугПрочихАктивов.Ссылка
	|			И (ВЫБОР
	|				КОГДА ПоступлениеУслугПрочихАктивов.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ПоступлениеУслугПрочихАктивов.Договор.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоров.СПоставщиком)
	...
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ДокументыКонтролируемыхСделок.РасчетныйДокумент КАК РасчетныйДокумент,
	|	КорректировкаПоступленияРасхождения.Ссылка КАК РегистраторРегистраСумм,
	|	ДокументыКонтролируемыхСделок.Период КАК Период,
	|	ДокументыКонтролируемыхСделок.Организация,
	|	ДокументыКонтролируемыхСделок.Контрагент,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход) КАК ТипКонтролируемойСделки,
	|	ЕСТЬNULL(КорректировкаПоступленияРасхождения.Номенклатура, КорректировкаПоступленияРасхождения.Содержание) КАК ПредметСделки,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияРасхождения.Номенклатура ЕСТЬ NULL 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.ИнойОбъектГражданскихПрав)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА КорректировкаПоступленияРасхождения.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|						ИЛИ КорректировкаПоступленияРасхождения.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|					ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.РаботаУслуга)
	|				ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.Товар)
	|			КОНЕЦ
	|	КОНЕЦ КАК ТипПредметаСделки,
	|	ЕСТЬNULL(КорректировкаПоступленияРасхождения.Номенклатура.НаименованиеПолное, КорректировкаПоступленияРасхождения.Содержание) КАК НаименованиеПредметаСделки,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияРасхождения.Номенклатура ЕСТЬ NULL 
	|				ИЛИ КорректировкаПоступленияРасхождения.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|				ИЛИ КорректировкаПоступленияРасхождения.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(КорректировкаПоступленияРасхождения.НомерГТД.СтранаПроисхождения, ЗНАЧЕНИЕ(Справочник.СтраныМира.Россия))
	|	КОНЕЦ КАК СтранаПроисхожденияПредметаСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.Корректировка) КАК ХозяйственнаяОперация,
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияРасхождения.Номенклатура ЕСТЬ NULL 
	|				ИЛИ ЕСТЬNULL(КорректировкаПоступленияРасхождения.Характеристика.Принципал, КорректировкаПоступленияРасхождения.Номенклатура.Принципал) = НЕОПРЕДЕЛЕНО
	|				ИЛИ ЕСТЬNULL(КорректировкаПоступленияРасхождения.Характеристика.Принципал, КорректировкаПоступленияРасхождения.Номенклатура.Принципал) = КорректировкаПоступленияРасхождения.Ссылка.Организация
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВключатьСоставСделок,
	|	ДокументыКонтролируемыхСделок.Валюта,
	...
	|		КОГДА КорректировкаПоступленияРасхождения.Номенклатура ЕСТЬ NULL 
	|			ТОГДА &ЕдиницаИзмеренияШтука
	|		ИНАЧЕ КорректировкаПоступленияРасхождения.Номенклатура.ЕдиницаИзмерения
	|	КОНЕЦ КАК ЕдиницаИзмерения,
	|	СУММА(КорректировкаПоступленияРасхождения.Количество) КАК Количество,
	|	КорректировкаПоступленияРасхождения.СтавкаНДС,
	|	СУММА(КорректировкаПоступленияРасхождения.СуммаСНДС - КорректировкаПоступленияРасхождения.СуммаНДС) КАК СуммаБезНДС,
	|	СУММА(КорректировкаПоступленияРасхождения.СуммаНДС) КАК СуммаНДС,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(КорректировкаПоступленияРасхождения.Ссылка.Соглашение.НалогообложениеНДС, КорректировкаПоступленияРасхождения.Ссылка.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ВЫБОР
	...
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ТоварМировойБиржевойТорговли,
	|	КорректировкаПоступленияРасхождения.ИдентификаторСтроки,
	|	ДокументыКонтролируемыхСделок.ИностранныйКонтрагент,
	|	ДокументыКонтролируемыхСделок.ЗарегистрированВСтранеСЛьготнымНалогообложением,
	|	ДокументыКонтролируемыхСделок.ВзаимозависимыеЛица,
	|	ДокументыКонтролируемыхСделок.Грузополучатель,
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ ТоварыМировойБиржевойТорговли КАК ТоварыМировойБиржевойТорговли
	|			ПО КорректировкаПоступленияРасхождения.Номенклатура = ТоварыМировойБиржевойТорговли.ПредметСделки
	|		ПО ДокументыКонтролируемыхСделок.Корректировка = КорректировкаПоступленияРасхождения.Ссылка
	|ГДЕ
	|	ВЫБОР
	|			КОГДА ДокументыКонтролируемыхСделок.ВернутьМногооборотнуюТару
	|				ТОГДА КорректировкаПоступленияРасхождения.Номенклатура.ТипНоменклатуры <> ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	|	КорректировкаПоступленияРасхождения.Ссылка,
	|	КорректировкаПоступленияРасхождения.СтавкаНДС,
	|	КорректировкаПоступленияРасхождения.ИдентификаторСтроки,
	|	ДокументыКонтролируемыхСделок.ИностранныйКонтрагент,
	|	ДокументыКонтролируемыхСделок.ЗарегистрированВСтранеСЛьготнымНалогообложением,
	...
	|	ДокументыКонтролируемыхСделок.Договор,
	|	ДокументыКонтролируемыхСделок.Соглашение,
	|	ЕСТЬNULL(КорректировкаПоступленияРасхождения.Номенклатура, КорректировкаПоступленияРасхождения.Содержание),
	|	ЕСТЬNULL(КорректировкаПоступленияРасхождения.Номенклатура.НаименованиеПолное, КорректировкаПоступленияРасхождения.Содержание),
	|	ВЫБОР
	|		КОГДА КорректировкаПоступленияРасхождения.Номенклатура ЕСТЬ NULL 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.ИнойОбъектГражданскихПрав)
	|		ИНАЧЕ ВЫБОР
...
Процедура ДобавитьСделкиОтчетаКомитентуОПродажах(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	ЛОЖЬ КАК ТоварМировойБиржевойТорговли,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ОтчетКомитенту.Соглашение.НалогообложениеНДС, ОтчетКомитенту.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
...
Процедура ДобавитьСделкиОтчетаКомитентуОСписании(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыКомитента.Ссылка.Соглашение.НалогообложениеНДС, ТоварыКомитента.Ссылка.НалогообложениеНДС) = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ВЫБОР
...
Процедура ДобавитьСделкиНачисленияКредитовИДепозитов(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	|	0 КАК СуммаНДС,
	|	ВЫБОР
	|		КОГДА НачисленияКредитовИДепозитовНачисления.Договор.ХарактерДоговора = ЗНАЧЕНИЕ(Перечисление.ХарактерДоговораКредитовИДепозитов.КредитИлиЗайм)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ПолученДоход)
	|	КОНЕЦ КАК ТипКонтролируемойСделки,
	|	ЛОЖЬ КАК ТоварМировойБиржевойТорговли,
...
Процедура ДобавитьСделкиПоступленияУслугПоЛизингу(СтруктураПараметров, МенеджерВременныхТаблиц, ТаблицаСделок)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПоступлениеУслугПоЛизингу.Ссылка КАК РасчетныйДокумент,
	|	ВЫБОР
	|		КОГДА ВзаимозависимыеЛицаПоПериодам.Контрагент ЕСТЬ NULL 
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВзаимозависимыеЛицаПоПериодам КАК ВзаимозависимыеЛицаПоПериодам
	|		ПО (ВзаимозависимыеЛицаПоПериодам.Организация = &Организация)
	|			И (ВзаимозависимыеЛицаПоПериодам.Контрагент = ПоступлениеУслугПоЛизингу.Контрагент)
	|			И ПоступлениеУслугПоЛизингу.Дата >= ВзаимозависимыеЛицаПоПериодам.НачалоПериода
	|			И ПоступлениеУслугПоЛизингу.Дата <= ВзаимозависимыеЛицаПоПериодам.ОкончаниеПериода
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИностранныеКонтрагенты КАК ИностранныеКонтрагенты
	|		ПО (ИностранныеКонтрагенты.Контрагент = ПоступлениеУслугПоЛизингу.Контрагент)
	|ГДЕ
	|	ПоступлениеУслугПоЛизингу.Организация В(&СтруктураОрганизации)
	|	И ПоступлениеУслугПоЛизингу.Дата >= &НачалоПериода
	|	И ПоступлениеУслугПоЛизингу.Дата <= &ОкончаниеПериода
	|	И ПоступлениеУслугПоЛизингу.Проведен = ИСТИНА
	|	И ПоступлениеУслугПоЛизингу.Контрагент В
	|			(ВЫБРАТЬ
	|				КонтролируемыеКонтрагенты.Контрагент
	|			ИЗ
	|				КонтролируемыеКонтрагенты КАК КонтролируемыеКонтрагенты)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка КАК РасчетныйДокумент,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка.Дата КАК Период,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка.Организация,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка.Контрагент,
	|	ПоступлениеУслугПоЛизингуНачисления.Содержание КАК ПредметСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыПредметовКонтролируемыхСделок.ИнойОбъектГражданскихПрав) КАК ТипПредметаСделки,
	|	ПоступлениеУслугПоЛизингуНачисления.Содержание КАК НаименованиеПредметаСделки,
	|	ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка) КАК СтранаПроисхожденияПредметаСделки,
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперацииКонтролируемыхСделок.ПолучениеУслуг) КАК ХозяйственнаяОперация,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка.Валюта,
	|	&ЕдиницаИзмеренияШтука КАК ЕдиницаИзмерения,
	|	1 КАК Количество,
	|	ПоступлениеУслугПоЛизингуНачисления.СтавкаНДС,
	|	ПоступлениеУслугПоЛизингуНачисления.СуммаСНДС - ПоступлениеУслугПоЛизингуНачисления.СуммаНДС КАК СуммаБезНДС,
	|	ПоступлениеУслугПоЛизингуНачисления.СуммаНДС,
	|	ЗНАЧЕНИЕ(Перечисление.ТипыКонтролируемыхСделок.ОсуществленРасход) КАК ТипКонтролируемойСделки,
	|	ЛОЖЬ КАК ТоварМировойБиржевойТорговли,
	|	ВЫБОР
	|		КОГДА ПоступлениеУслугПоЛизингуНачисления.Ссылка.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОперацияОблагаетсяЕНВД,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Грузоотправитель,
	|	ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка) КАК Грузополучатель,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка.Договор КАК Договор,
	|	НЕОПРЕДЕЛЕНО КАК Соглашение,
	|	ПоступлениеУслугПоЛизингуНачисления.Ссылка КАК РегистраторРегистраСумм,
	|	1 КАК МножительСуммовыхДанных,
	|	ПоступлениеУслугПоЛизингуНачисления.ИдентификаторСтроки КАК ИдентификаторСтроки
	|ПОМЕСТИТЬ ТаблицаСделок
	|ИЗ
	|	ДокументыКонтролируемыхСделок КАК ДокументыКонтролируемыхСделок
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеУслугПоЛизингу.Начисления КАК ПоступлениеУслугПоЛизингуНачисления
	|		ПО ДокументыКонтролируемыхСделок.РасчетныйДокумент = ПоступлениеУслугПоЛизингуНачисления.Ссылка
	|ГДЕ
	|	(ДокументыКонтролируемыхСделок.ВзаимозависимыеЛица
...
Функция ПолучитьОсновныеСведенияУведомленияДляВыгрузки(Уведомление) Экспорт
	...
	ПараметрыОрганизации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Организация,
		"ИНН, КПП, НаименованиеПолное, КодНалоговогоОргана, ЮридическоеФизическоеЛицо, ИндивидуальныйПредприниматель, КодОКВЭД, РегистрацияВНалоговомОргане");
	...
	ЭлектроннаяПочтаОрганизации = ?(ОрганизацияФизЛицо, "", УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации));
	...
	Если ОрганизацияФизЛицо Тогда
		...
		Если КадровыеДанныеФизЛиц.Количество()>0 Тогда
			...
			Если ЗначениеЗаполнено(ДанныеФизЛица.АдресПоПрописке) Тогда
				...
				АдресСтруктурой = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(ДанныеФизЛица.АдресПоПрописке);
			...
			Если ЗначениеЗаполнено(ДанныеФизЛица.АдресЗаПределамиРФ) Тогда
				АдресСтруктурой = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(ДанныеФизЛица.АдресЗаПределамиРФ);
...
Функция ИдентификаторФайлаЭлектронногоПредставления(СведенияОтправки) Экспорт
	...
	КонецЕсли;
	ИдентификаторПолучателя = Строка(СведенияОтправки.КодНО) + "_" + Строка(СведенияОтправки.КодНО);
	...
	Для Каждого Лист Из ЛистыУведомления.ДанныеРаздела3 Цикл
		...
		Если Не ЗначениеЗаполнено(Лист.Строка030ИНН) Тогда
			...
			Если Лист.ГражданствоФизЛицСтрана = Справочники.СтраныМира.Россия ИЛИ Не ЗначениеЗаполнено(Лист.ГражданствоФизЛицСтрана) Тогда
				...
				// Адрес места жительства/прибывания
				АдресСтруктурой = УправлениеКонтактнойИнформацией.ПредыдущаяСтруктураКонтактнойИнформацииXML(Лист.КонтактнаяИнформацияЗначенияПолей);
