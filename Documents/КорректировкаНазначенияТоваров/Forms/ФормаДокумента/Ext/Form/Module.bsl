...
Процедура ПриЧтенииСозданииНаСервере()
	...
	//++ НЕ УТКА
	ЗапретСнятияРезерва = ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца");
	//-- НЕ УТКА
...
Процедура ЗаполнитьДоступныеДействия()
	...
	ДоступныеДействия = Документы.КорректировкаНазначенияТоваров.ДоступныеДействия(Заказ);
...
Процедура ПриИзмененииКлючевыхПолейШапки()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.ПроизвольнаяКорректировкаНазначений") Тогда
		...
		Если ЗначениеЗаполнено(Заказ) Тогда
			Документы.КорректировкаНазначенияТоваров.ЗаполнитьПоЗаказу(Заказ, Объект.Ссылка, Объект.Товары);
			НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(Объект, ПараметрыУказанияСерий);
		КонецЕсли;
