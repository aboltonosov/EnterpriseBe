...
Функция ВременныеТаблицыДанныхДокумента() Экспорт
	...
	|		И СпрВидЗапасов.Комитент = НЕОПРЕДЕЛЕНО)
	|
	|ИНДЕКСИРОВАТЬ ПО
...
//-- НЕ УТКА
	...
	Если ЗначениеЗаполнено(Заказ) Тогда
		...
		Документы.КорректировкаНазначенияТоваров.ЗаполнитьПоОснованию(Назначение, Заказ, Ссылка, ВидОперации, Товары, Ложь);
...
//++ НЕ УТКА
	...
	Если ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказДавальца") Тогда
		...
		ШаблонСообщенияПустое = НСтр("ru='Не заполнена колонка ""Новое назначение"" в строке %НомерСтроки% списка ""Товары"".'");
		ШаблонСообщенияРавные = НСтр("ru='Значение в колонке ""Новое назначение"" в строке %НомерСтроки% списка ""Товары"" не должно совпадать со значением в колонке ""Исходное назначение"".'");
		...
		Если Таблица.Количество() > 0 Тогда
			...
			ШаблонСообщения = НСтр("ru='В колонке ""Новое назначение"" в строке %НомерСтроки% должно быть указано назначение заказа давальца ""%Клиент%"".'");
