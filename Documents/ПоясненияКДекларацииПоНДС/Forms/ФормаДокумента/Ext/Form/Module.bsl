...
Процедура УстановитьУсловноеОформлениеСтрокиСДругимПериодом(ИмяЭлемента, ПутьКДанным)
	...
	Если ЗначениеЗаполнено(ПериодОтраженияЗаписи) Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИ, ПутьКДанным, ВидСравненияКомпоновкиДанных.НеРавно, ПериодОтраженияЗаписиПриЛиквидации);
...
Функция ПериодДляОрганизацииПриЛиквидации(Знач ПериодОтраженияЗаписи)
	
	Результат = ПериодОтраженияЗаписи;
	
	Если ЗначениеЗаполнено(ПериодОтраженияЗаписи) Тогда
		
		ВидПериодаЧислом = Число(Лев(ПериодОтраженияЗаписи, 2));
		ГодСтрокой 		 = Сред(ПериодОтраженияЗаписи, 4);
		
		// Код периода 01-12 преобразуем в 71-82 прибавлением 70
		Если ВидПериодаЧислом >= 1 И ВидПериодаЧислом <= 12 Тогда
			Результат = Строка(ВидПериодаЧислом + 70) + ";" + ГодСтрокой;
		КонецЕсли;
		
		// Код периода 71-82 преобразуем в 01-12 вычитанием 70
		Если ВидПериодаЧислом >= 71 И ВидПериодаЧислом <= 82 Тогда
			Результат = Строка(ВидПериодаЧислом - 70) + ";" + ГодСтрокой;
		КонецЕсли;
		
		// Код периода 21-24 преобразуем в 51-56
		
		// 1 квартал
		Если ВидПериодаЧислом = 21 Тогда
			Результат = "51;" + ГодСтрокой;
		КонецЕсли;
		
		// 2 квартал
		Если ВидПериодаЧислом = 22 Тогда
			Результат = "54;" + ГодСтрокой;
		КонецЕсли;
		
		// 3 квартал
		Если ВидПериодаЧислом = 23 Тогда
			Результат = "55;" + ГодСтрокой;
		КонецЕсли;
		
		// 4 квартал
		Если ВидПериодаЧислом = 24 Тогда
			Результат = "56;" + ГодСтрокой;
		КонецЕсли;
		
		// Код периода 51-56 преобразуем в 21-24
		
		// 1 квартал
		Если ВидПериодаЧислом = 51 Тогда
			Результат = "21;" + ГодСтрокой;
		КонецЕсли;
		
		// 2 квартал
		Если ВидПериодаЧислом = 54 Тогда
			Результат = "22;" + ГодСтрокой;
		КонецЕсли;
		
		// 3 квартал
		Если ВидПериодаЧислом = 55 Тогда
			Результат = "23;" + ГодСтрокой;
		КонецЕсли;
		
		// 4 квартал
		Если ВидПериодаЧислом = 56 Тогда
			Результат = "24;" + ГодСтрокой;
		КонецЕсли;
...
Функция ЗапретитьРедактированиеСтрокиСДругимПериодом(Элемент, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные <> Неопределено 
		...
		И Элемент.ТекущиеДанные.ПериодОтраженияЗаписи <> ПериодОтраженияЗаписи
		И Элемент.ТекущиеДанные.ПериодОтраженияЗаписи <> ПериодОтраженияЗаписиПриЛиквидации Тогда
...
Процедура ПодготовитьФормуНаСервере()
	...
	ПериодОтраженияЗаписиПриЛиквидации 	= ПериодДляОрганизацииПриЛиквидации(ПериодОтраженияЗаписи);
...
Процедура УстановитьУсловноеОформлениеВыставленныеСчетаФактуры()
	...
	#Область ЗапретРедактированияЗаписейСДругимПериодом
		УстановитьУсловноеОформлениеСтрокиСДругимПериодом("ВыставленныеСчетаФактурыНеплательщиковНДСРезультатПроверки", "Объект.ВыставленныеСчетаФактурыНеплательщиковНДС.ПериодОтраженияЗаписи");
	#КонецОбласти
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы.ПериодОтраженияЗаписи) Тогда
		...
		Иначе
			...
			ИначеЕсли ВидПериода = 51 Тогда
				ПредставлениеПериода = "I квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
			ИначеЕсли ВидПериода = 54 Тогда
				ПредставлениеПериода = "II квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
			ИначеЕсли ВидПериода = 55 Тогда
				ПредставлениеПериода = "III квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
			ИначеЕсли ВидПериода = 56 Тогда
				ПредставлениеПериода = "IV квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
