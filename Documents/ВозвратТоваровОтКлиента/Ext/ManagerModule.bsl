...
Функция ТекстУсловияТребуетсяСчетФактура(ОграничитьПоТипуНалогообложения = Истина) Экспорт
	...
	ТекстУсловия = "ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиента) ИЛИ ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиентаПрошлыхПериодов)";
	
	Если ОграничитьПоТипуНалогообложения Тогда
		ТекстУсловия = ТекстУсловия + "
		|И ДанныеДокумента.НалогообложениеНДС В (&ТипыНалогообложения)";
	КонецЕсли;
...
#Область ТекстСторноНДС // (Дт 90.03 :: Кт 19.03)
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)              КАК НаправлениеДеятельностиКт,
	|
	// 4D:ERP для Беларуси, ВладимирР, 08.09.2016 11:25:21 
	// Проводки по возврату товаров от клиента, №12450
	// {
	...
	|ГДЕ
	// 4D:ERP для Беларуси, ВладимирР, 14.03.2017 10:29:07 
	// Возврат товаров от клиента по реализациям прошлых периодов, №14478
	// {
	|	Операция.НалогообложениеНДС В (ЗНАЧЕНИЕ (Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС), 
...
#Область ТекстСторноВыручкиОтРеализацииКомисионногоТовара // (Дт 62.01 :: Кт 76.ОК)
...
#Область ТекстВозвратПереданнойТары // (Дт 41.03 :: Кт 76.09)
	...
	|	-ЕСТЬNULL(Стоимости.Знак * Стоимости.Сумма, Строки.Знак * Строки.Сумма) КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	-(Строки.Знак *Строки.Количество) КАК КоличествоКт,
	|	-ЕСТЬNULL(Стоимости.Знак * Стоимости.Сумма, Строки.Знак * Строки.Сумма) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	...
	|ВЫБРАТЬ //// Отклонение в стоимости возвращамой тары на расходы (Дт 91.02 :: Кт 76)
	|	
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	0 КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	(Стоимости.Знак * Стоимости.Сумма) + ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСРегл + СуммыВВалютеРегл.СуммаНДСРегл, ТаблицаВидыЗапасов.СуммаСНДС) КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.РасчетыСКлиентамиТара) КАК ВидСчетаКт,
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	(Стоимости.Знак * Стоимости.Сумма) + ЕСТЬNULL(СуммыВВалютеРегл.СуммаБезНДСРегл + СуммыВВалютеРегл.СуммаНДСРегл, ТаблицаВидыЗапасов.СуммаСНДС) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	...
	|		И Стоимости.ХозяйственнаяОперация В (
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноПереданнойТары),
	|				ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТарыОтКлиентаПрошлыхПериодов))
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
#Область ТекстПринятиеНДСКВычету // (Дт <68.2> :: Кт <19.х>)
	...
	|			(ВЫБОР
	|				КОГДА НДСЗаписи.ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ОбъектыНезавершенногоСтроительства)
	|					ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСприСтроительствеОсновныхСредств)
	...
	|
	|	ВЫБОР 
	|		КОГДА Операция.ПокупательНеПлательщикНДС
	|			ТОГДА НДСЗаписи.Организация
	|		ИНАЧЕ НДСЗаписи.Поставщик
	|	КОНЕЦ                           КАК СубконтоКт1,
...
#КонецОбласти
	...
	Возврат ТекстВозвратОтКомиссионера
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратОтКомиссионераСкладПроизводства
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСторноСебестоимостиРеализованногоТовараСкладПроизводства
		// 4D:ERP для Беларуси, ВладимирР, 14.03.2017 10:29:07 
		// Возврат товаров от клиента по реализациям прошлых периодов, №14478
		// {
		// Действие не требуется
		// }
		// 4D
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратСебестоимостиТоваровПрошлыхЛетСкладПроизводства
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратТоваровДругогоКачества
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратТоваровДругогоКачестваСкладПроизводства
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВозвратПереданнойТарыСкладПроизводства
		...
		+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПринятиеНДСКВычету
		+ "";
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюОтчетовКомитенту(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ 
	|		И КлючиКомитента.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА - ВидыЗапасов.Себестоимость
	|		ИНАЧЕ 0 КОНЕЦ КАК Себестоимость,
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		 ИЛИ ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента)
	...
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА - ВидыЗапасов.СебестоимостьБезНДС
	|		ИНАЧЕ 0 КОНЕЦ КАК СебестоимостьБезНДС,
	|
	|	ВЫБОР
	|		КОГДА ВидыЗапасов.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|		 ИЛИ ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.ИзТекущегоДокумента)
	...
	|		КОГДА ВидыЗапасов.СпособОпределенияСебестоимости = ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимости.Вручную)
	|			ТОГДА - ВидыЗапасов.СебестоимостьРегл
	|		ИНАЧЕ 0 КОНЕЦ КАК СебестоимостьРегл,
	|
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	-(ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.БазаНДСРегл = 0
	|		ТОГДА ЕСТЬNULL(ТаблицаДокументаРегл.СуммаБезНДС, ТаблицаВидыЗапасов.СуммаБезНДСРегл)
	|		ИНАЧЕ ТаблицаВидыЗапасов.БазаНДСРегл
	|	КОНЕЦ)								   КАК СуммаБезНДС,
	|	-(ЕСТЬNULL(ТаблицаДокументаРегл.СуммаНДС,
...
Процедура УстановитьПараметрыЗапросаКоэффициентыВалют(Запрос)
	...
	// 4D:ERP для Беларуси, ВладимирР, 22.08.2017 16:07:24 
	// "Возврат от клиентов" от нерезидента РБ, №15608
	// {
	Если ТипЗнч(Запрос.Параметры.Ссылка) = Тип("ДокументСсылка.ВозвратТоваровОтКлиента") И ЗначениеЗаполнено(Запрос.Параметры.Ссылка.ДокументРеализации) Тогда
		Коэффициенты = РаботаСКурсамивалютУТ.ПолучитьКоэффициентыПересчетаВалюты(Запрос.Параметры.Валюта, Неопределено, Запрос.Параметры.Ссылка.ДокументРеализации.Дата);
	Иначе
...
// - заполняет реквизит "СпособОпределенияСебестоимости" в ТЧ "Товары" документа "ВозвратТоваровОтКлиента";
// - заполняет назначение в аналитике учета номенклатуры из вида запасов.
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ ВидыЗапасов.Ссылка.ПорядокОплаты = Значение(Перечисление.ПорядокОплатыПоСоглашениям.ПустаяСсылка)
	|	ИЛИ (ВидыЗапасов.АналитикаУчетаНоменклатурыОтгрузки.Назначение <> ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение
	|		И ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		РегистрыСведений.АналитикаУчетаНоменклатуры.ЗаполнитьВКоллекции(ДокументОбъект.ВидыЗапасов, МестаУчета, ИменаПолей, ДополнитьКлюч, ПараметрыЗаполненияКлючейВидовЗапасов);
		...
		Попытка
			Если ОбъектИзменен ИЛИ ПараметрыЗаполненияКлючейВидовЗапасов.ИзмененаАналитика Тогда
...
Процедура ЗарегистрироватьДанныеКОбновлениюКлючейАналитики(Параметры) Экспорт
	...
	|		И КлючиКГенерации.Назначение = ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение
	|ГДЕ
	|	ВидыЗапасов.АналитикаУчетаНоменклатурыОтгрузки.Назначение <> ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение
	|	И ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение <> ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|	И ВидыЗапасов.Ссылка.Проведен
...
Процедура СгенерироватьКлючиАналитикиНоменклатуры(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		|		Поля.Серия,
		|		Поля.Склад,
		|		ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение КАК Назначение,
		|		Поля.СтатьяКалькуляции
		|	ИЗ
		|		Документ.ВозвратТоваровОтКлиента.ВидыЗапасов КАК ВидыЗапасов
		|
		...
		|			И ВидыЗапасов.ВидЗапасовОтгрузки.УдалитьНазначение = Аналитика.Назначение
