...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаявкаНаВозвратТоваровОтКлиента") Тогда
		...
		// 4D:ERP для Беларуси, ВладимирР, 22.06.2016 16:53:45 
		// Возврат товаров от клиентов, №11660
		// {
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		|	И ТИПЗНАЧЕНИЯ(СвязанныеДокументы.Ссылка) = ТИП(Документ.ДоверенностьВыданная)";
		Запрос.УстановитьПараметр("ЗначениеКритерияОтбора", ДанныеЗаполнения);
		...
		Если Не Результат.Пустой() Тогда
			...
			ДоверенностьВыданная = Выборка.Ссылка;
		КонецЕсли;
		// }
		// 4D
	...
	Если Не Проведен
		...
		Если СтрокиБезДокументаРеализации.Количество() Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='В документе нельзя указывать строки по одной и той же позиции номенклатуры и с документом продажи и без него.'"),
				ЭтотОбъект,
				,
				,
				Отказ);
			...
			Пока СтрокиБезДокументаРеализации.Следующий() Цикл
				...
				АдресОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru=' в строке %1 списка ""Товары""'"),
				...
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Не заполнен ""Документ продажи""'") + АдресОшибки,
					ЭтотОбъект,
					Поле,
					,
					Отказ);
...
Процедура ЗаполнитьВидыЗапасовПоОстаткамУКомиссионера(МенеджерВременныхТаблиц, Отказ)
	...
	|	ПО ТаблицаВидыЗапасов.АналитикаУчетаНоменклатурыОтгрузки = АналитикаОтгрузки.КлючАналитики
	|;
	|УНИЧТОЖИТЬ ВтТаблицаВидыЗапасов
	|");
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", ХозяйственнаяОперация);
	Запрос.УстановитьПараметр("ТаблицаВидыЗапасов", ВидыЗапасов.Выгрузить());
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	...
	ОтборОстатков.Вставить("Дата", КонецМесяца(Дата));
	ОтборОстатков.Вставить("Склад", Склад);
...
Процедура СообщитьОбОшибкахЗаполненияКомиссионныхВидовЗапасов(ТаблицаОшибок, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Номенклатура: %1, у партнера %2 недостаточно %3 %4'"),
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента
		...
		Пока СуммаКРаспределению > 0 И ВыборкаРасчетыПоОснованию.Следующий() Цикл
			...
			Если ВыборкаРасчетыПоОснованию.СуммаОстаток > 0 Тогда
				...
				Коэффициенты  = РаботаСКурсамивалютУТ.ПолучитьКоэффициентыПересчетаВалюты(Валюта, ВыборкаРасчетыПоОснованию.ВалютаВзаиморасчетов, ТекущаяДата());
...
Процедура ЗаполнитьСебестоимостьВидовЗапасов()
	...
	Для Каждого СтрокаТоваров Из Товары Цикл
		Если ЗначениеЗаполнено(СтрокаТоваров.ДокументРеализации)
		 И СтрокаТоваров.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПродажи Тогда
			Продолжить;
		КонецЕсли;
