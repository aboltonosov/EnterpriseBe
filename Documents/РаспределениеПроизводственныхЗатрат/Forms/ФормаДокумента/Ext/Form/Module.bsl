...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа Тогда
		Элементы.Наличие.Доступность = Ложь;
	...
	//++ НЕ УТКА
	Элементы.НадписьВходящийОстаток.Заголовок = УправлениеПроизводствомКлиентСервер.РасшифровкаВходящегоОстатка(ЭтаФорма);
	//-- НЕ УТКА
	
	// конец новое производство

	Если ПолучитьФункциональнуюОпцию("КомплекснаяАвтоматизация") Тогда
		Элементы.СтраницаВыпускиБезРаспоряжения.Заголовок = НСтр("ru = 'По выпускам'");
	КонецЕсли;
	...
	Если Элементы.Найти("ПодменюОтчетыКомандаОтчет1_Отчет_БазаРаспределенияМатериаловИРабот") <> Неопределено И Не Объект.НовоеПроизводство Тогда
		Элементы.ПодменюОтчетыКомандаОтчет1_Отчет_БазаРаспределенияМатериаловИРабот.Видимость = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
	...
	СвойстваДокумента.Вставить("ТипНоменклатуры", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Номенклатура, "ТипНоменклатуры"));
