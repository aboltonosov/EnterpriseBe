...
Функция РезультатЗапросаПоИзменениямПозицийШтатногоРасписания(ТарифнаяСетка, ДатаВступленияВСилу, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Если ФОИспользоватьШтатноеРасписание Тогда
		...
		ЗарплатаКадры.УничтожитьВТ(Запрос.МенеджерВременныхТаблиц, "ВТПозицииШтатногоРасписания", Истина);
		...
		УправлениеШтатнымРасписанием.СоздатьВТНачисленияПозицийШтатногоРасписания(Ложь, ОписательВременныхТаблиц, "ВТНачисленияПозицийШтатногоРасписания", Истина);
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
			...
			Запрос.Текст =
				...
				|	И (ПозицииШтатногоРасписания.РазрядКатегория = ЗНАЧЕНИЕ(Справочник.РазрядыКатегорииДолжностей.ПустаяСсылка)
...
Функция РезультатЗапросаПоИзменениямПлановыхНачислений(ТарифнаяСетка, ДатаВступленияВСилу, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ИзменениеУчтено,
		|	ИзменениеПлановыхНачисленийСотрудники.ДатаИзменения
		|ИЗ
		|	ВТСотрудникиОрганизацииСПоказателями КАК СотрудникиОрганизации
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИзменениеПлановыхНачислений.Сотрудники КАК ИзменениеПлановыхНачисленийСотрудники
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИзменениеПлановыхНачислений.ПоказателиСотрудников КАК ИзменениеПлановыхНачисленийПоказателиСотрудников
		|			ПО ИзменениеПлановыхНачисленийСотрудники.Ссылка = ИзменениеПлановыхНачисленийПоказателиСотрудников.Ссылка
		|				И ИзменениеПлановыхНачисленийСотрудники.ИдентификаторСтрокиСотрудника = ИзменениеПлановыхНачисленийПоказателиСотрудников.ИдентификаторСтрокиСотрудника
		|				И (ИзменениеПлановыхНачисленийПоказателиСотрудников.Показатель В (&Показатели))
		|		ПО СотрудникиОрганизации.Организация = ИзменениеПлановыхНачисленийСотрудники.Ссылка.Организация
		|			И СотрудникиОрганизации.Сотрудник = ИзменениеПлановыхНачисленийСотрудники.Сотрудник
		|			И (ИзменениеПлановыхНачисленийСотрудники.ДатаИзменения = &ДатаВступленияВСилу)
		|			И (ИзменениеПлановыхНачисленийСотрудники.Ссылка.Проведен)
		...
		|	И (СотрудникиОрганизации.Значение <> Тарифы.Тариф
		|			ИЛИ НЕ ИзменениеПлановыхНачисленийПоказателиСотрудников.Ссылка ЕСТЬ NULL )
		|
		|УПОРЯДОЧИТЬ ПО
		|	Организация,
...
Процедура ИзменитьФОТ(ПараметрыВыполнения, АдресВоВременномХранилище = Неопределено) Экспорт
				...
				Для каждого СтрокаИзмененияШтатногоРасписания Из ИзмененияШтатногоРасписания Цикл
					...
					Для каждого СтрокаПозиции Из СтрокиПозиций Цикл
						...
						Если СтрокаПозицииДокумента = Неопределено Тогда
							...
							Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
								...
								Модуль.ЗаполнитьТекущиеДанныеПозицииОбъекта(ИзменениеШтатногоРасписания, СтрокаПозицииДокумента, ДанныеПозиции);
							КонецЕсли;
					...
					Исключение
						...
						Если ЗначениеЗаполнено(ИзменениеШтатногоРасписания.Ссылка) Тогда
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось сохранить изменения в документе'") + " - " + Строка(ИзменениеШтатногоРасписания.Ссылка));
						Иначе
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось сохранить изменения позиций штатного расписания во вновь созданном документе'"));
						КонецЕсли;
				...
				Для каждого СтрокаОрганизации Из СтрокиОрганизации Цикл
					...
					Если ИзмененныеПозиции.Количество() > 0  Тогда
						...
						Для каждого ПозицияОбъект Из ИзмененныеПозиции Цикл
							...
							Исключение
								ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось сохранить изменения в позиции'") + " """ + ПозицияОбъект.Наименование + """");
							КонецПопытки;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		ИзмененияПлановыхНачислений = ТаблицаИзменений.Скопировать(, "Организация,ИзменениеПлановыхНачислений,ДатаИзменения");
		ИзмененияПлановыхНачислений.Свернуть("Организация,ИзменениеПлановыхНачислений,ДатаИзменения");
		...
		Для каждого СтрокаИзмененияПлановыхНачислений Из ИзмененияПлановыхНачислений Цикл
			...
			СтруктураПоиска = Новый Структура("Организация,ИзменениеПлановыхНачислений,ДатаИзменения");
			...
			Для каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
				...
				Если ИзменениеПлановыхНачислений.Сотрудники.НайтиСтроки(
						Новый Структура("ДатаИзменения,Сотрудник", ДатаВступленияВСилу, СтрокаСотрудника.Сотрудник)).Количество() = 0 Тогда
			...
			Для каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
				...
				СтрокиСотрудникаДокумента = ИзменениеПлановыхНачислений.Сотрудники.НайтиСтроки(Новый Структура("ДатаИзменения,Сотрудник", ДатаВступленияВСилу, СтрокаСотрудника.Сотрудник));
			...
			Если СотрудникиДляРасчетаФОТ.Количество() > 0 Тогда
				...
				ВремяРегистрацииСотрудников = ЗарплатаКадрыРасширенный.ВремяРегистрацииСотрудниковДокумента(
					ИзменениеПлановыхНачислений.Ссылка, СотрудникиДляРасчетаФОТ, ИзменениеПлановыхНачислений.ДатаИзменения);
				
				СтрокиСотрудников = Новый Массив;
								
				ИдентификаторыСтрокПоСотрцдникам = Новый Соответствие;
				...
				Для Каждого Сотрудник Из СотрудникиДляРасчетаФОТ Цикл
					...
					СтрокиСотрудника = ИзменениеПлановыхНачислений.Сотрудники.НайтиСтроки(Новый Структура("ДатаИзменения,Сотрудник", ДатаВступленияВСилу, Сотрудник));
					...
					ГоловнаяОрганизация = ЗарплатаКадрыПовтИсп.ГоловнаяОрганизация(ИзменениеПлановыхНачислений.Организация);
					...
					СтрокаТаблицыКадровыхДанных.Организация = ИзменениеПлановыхНачислений.Организация;
					
					СтрокиСотрудников.Добавить(СтрокаСотрудника);
					
					ИдентификаторыСтрокПоСотрцдникам.Вставить(Сотрудник, СтрокаСотрудника.ИдентификаторСтрокиСотрудника);
				...
				Для каждого СтрокаРассчитанногоНачисления Из РасчитанныеДанныеНачислений.ПлановыйФОТ Цикл
					Отбор.ИдентификаторСтрокиСотрудника = ИдентификаторыСтрокПоСотрцдникам[СтрокаРассчитанногоНачисления.Сотрудник];
			...
			Исключение
				...
				Если ЗначениеЗаполнено(ИзменениеПлановыхНачислений.Ссылка) Тогда
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось сохранить изменения в документе'") + " - " + Строка(ИзменениеПлановыхНачислений.Ссылка));
				Иначе
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не удалось сохранить изменения начислений сотрудников во вновь созданном документе'"));
				КонецЕсли;
...
// 4D:ERP для Беларуси, Яна, 16.05.2017 12:35:06 
