...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 4D:ERP для Беларуси, Яна, 16.06.2017 09:24:40 
	// Документ "Утверждение тарифной сетки" и справочник "Тарифная сетка", № 14918 
	// {		
	МенюОтчеты.ПриСозданииНаСервере(ЭтаФорма, Элементы.ПодменюОтчеты);  
	ВидимостьКолонокЕТСНаСервере(ЭтаФорма);
	// }
	// 4D
...
Процедура ТарифнаяСеткаПриИзменении(Элемент)	
	...
	// 4D:ERP для Беларуси, Яна, 15.05.2017 11:16:03 
	// // Документ "Утверждение тарифной сетки" и справочник "Тарифная сетка", № 14918
	// {
	ВидимостьКолонокЕТС();
	// }
...
// {
&НаКлиенте
Процедура ТарифнаяСеткаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)	
	ВидимостьКолонокЕТС();	
КонецПроцедуры

&НаСервере
Процедура ВидимостьКолонокЕТС()	
	ВидимостьКолонокЕТСНаСервере(ЭтаФорма);
КонецПроцедуры
// }
// 4D
...
// 4D:ERP для Беларуси, Яна, 25.05.2017 11:55:12 
// Документ "Утверждение тарифной сетки" и справочник "Тарифная сетка", № 14918
// {
&НаКлиенте
Процедура ТарифыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ТарифыПриРедактировании(Элемент, НоваяСтрока, Копирование);
...
Процедура ТарифыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ТарифыПриРедактировании(Элемент, НоваяСтрока, ОтменаРедактирования);
	
КонецПроцедуры

&НаКлиенте
Процедура ТарифыПриРедактировании(Элемент, НоваяСтрока, Действия = Неопределено)
	...
	РазрядыКатегории = РазрядыКатегорииДолжностейСтруктура(ТекущиеДанные.РазрядКатегория);

	РассчитатьДолжностнойОклад(ТекущиеДанные,Объект.СпособОкругления, РазрядыКатегории );
...
Процедура ТарифыРазрядКатегорияПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Юлия, 16.08.2017 10:35:33 
	// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
	// { 	
	РассчитатьЗначенияТарифов();
	РассчитатьДолжностнойОкладБезПривязкиКТарифнойСетке();
	// }
	// 4D
...
// 4D:ERP для Беларуси, Яна, 16.06.2017 13:04:38 
// Документ "Утверждение тарифной сетки" и справочник "Тарифная сетка", № 14918
// {
// МенюОтчеты
&НаКлиенте
...
Процедура ЗаполнитьЗначенияДействующихТарифов(ОчиститьЗначения = Ложь)
	...
	Для Каждого ДанныеТарифов Из ДанныеТарифнойСетки.ЗначенияТарифов Цикл 
		...
		// 4D:ERP для Беларуси, Юлия, 18.08.2017 16:26:40 
		// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
		// {
...
Процедура РассчитатьЗначенияТарифов()
	...
	// 4D:ERP для Беларуси, Петр, 15.08.2017 13:01:30 
	// Форматирование кода, № 15734
	// {
		...
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			ОписаниеСпособаОкругления = ЗарплатаКадрыРасширенныйПовтИсп.ОписаниеСпособаОкругления(Объект.СпособОкругления);
		#Иначе
			ОписаниеСпособаОкругления = ЗарплатаКадрыРасширенныйКлиентПовтИсп.ОписаниеСпособаОкругления(Объект.СпособОкругления);
		#КонецЕсли
		
		Для Каждого ДанныеРазрядаТарифнойСетки Из Объект.Тарифы Цикл 
			ДанныеРазрядаТарифнойСетки.ТарифныйОклад = ДанныеРазрядаТарифнойСетки.РазрядКатегория.Коэффициент * БазовоеЗначение * ДанныеРазрядаТарифнойСетки.РазрядКатегория.ПовышающийКоэффициент;
			ДанныеРазрядаТарифнойСетки.ТарифныйОклад = ЗарплатаКадрыКлиентСервер.Округлить(ДанныеРазрядаТарифнойСетки.ТарифныйОклад, ОписаниеСпособаОкругления.Точность, ОписаниеСпособаОкругления.ПравилоОкругления);
			
			ДанныеРазрядаТарифнойСетки.ДолжностнойОклад = ДанныеРазрядаТарифнойСетки.ТарифныйОклад + ДанныеРазрядаТарифнойСетки.ТарифныйОклад*ДанныеРазрядаТарифнойСетки.РазрядКатегория.ПроцентПовышения/100;
			ДанныеРазрядаТарифнойСетки.ДолжностнойОклад = ЗарплатаКадрыКлиентСервер.Округлить(ДанныеРазрядаТарифнойСетки.ДолжностнойОклад, ОписаниеСпособаОкругления.Точность, ОписаниеСпособаОкругления.ПравилоОкругления);
		КонецЦикла;
...
Процедура УстановитьИспользованиеТарифныхКоэффициентов()
	...
	// 4D:ERP для Беларуси, Юлия, 15.08.2017 15:03:12 
	// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
	// {
...
Функция РассчитатьИзменениеФОТНаСервере()
	...
	НаименованиеЗадания = НСтр("ru = 'Расчет ФОТ при изменении тарифной группы'");
...
Процедура ТарифыРазрядКатегорияПриИзмененииНаСервере()
	...
	// 4D:ERP для Беларуси, Юлия, 15.08.2017 16:16:25 
	// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
	// {
	РассчитатьЗначенияТарифов(); 
	// }
...
Процедура ЗаполнитьИзмененияФОТ()
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ Тарифы.Отменить
		|	И Тарифы.Тариф <> Тарифы.ТекущийТариф";
				...
				Пока Выборка.СледующийПоЗначениюПоля("ИзменениеШтатногоРасписания") Цикл
					...
					Иначе
						...
						Если ФОИспользоватьИсториюИзмененияШтатногоРасписания Тогда
							...
							СтрокаИзменениеШтатногоРасписания.Представление = НСтр("ru='Не найдены изменения позиций штатного расписания'");
						Иначе
							СтрокаИзменениеШтатногоРасписания.ИзменениеУчтено = Истина;
							СтрокаИзменениеШтатногоРасписания.Представление = НСтр("ru='Позиции штатного расписания'");
						КонецЕсли;
						...
						Если Не СтрокаПозиции.ИзменениеУчтено Тогда
							...
							Если СтрокаИзменениеШтатногоРасписания.ИзменениеУчтено = Истина Тогда
								...
								СтрокаИзменениеШтатногоРасписания.Представление = СтрокаИзменениеШтатногоРасписания.Представление
									+ " (" + НСтр("ru='не все изменения учтены'") + ")";
							...
							Если ЗначениеЗаполнено(Выборка.РазрядКатегория)
								...
								СтрокаПозиции.Представление = СтрокаПозиции.Представление + ", " + НСтр("ru='размер тарифа должен быть'")
									+ " " + Формат(Выборка.Тариф, "ЧДЦ=2");
								
							Иначе
								
								СтрокаПозиции.Представление = СтрокаПозиции.Представление + ", " + НСтр("ru='размер тарифа должен быть в пределах от'")
									+ " " + Формат(Выборка.ТарифМин, "ЧДЦ=2") + " " + НСтр("ru='до'") + " " + Формат(Выборка.ТарифМакс, "ЧДЦ=2");
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("ИзменениеПлановыхНачислений") Цикл
			...
			Иначе
				...
				СтрокаИзменениеПлановыхНачислений.Представление = НСтр("ru='Не найдены изменения начислений сотрудников'");
			КонецЕсли;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не Выборка.ИзменениеУчтено И ЗначениеЗаполнено(Выборка.ИзменениеПлановыхНачислений) Тогда
					...
					СтрокаСотрудника.Представление = СтрокаСотрудника.Представление + ", " + НСтр("ru='размер тарифа должен быть'")
						+ " = " + Формат(Выборка.Тариф, "ЧДЦ=2");
...
// 4D:ERP для Беларуси, Юлия, 16.08.2017 10:33:08 
// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
// {
&НаСервере
Процедура ВидимостьКолонокЕТСНаСервере(ЭтаФорма)
	
	ЭтаФорма.Элементы.ТарифыРазрядКатегорияРазрядЕТС.Видимость = Истина;
	ЭтаФорма.Элементы.ТарифыРазрядКатегорияКоэффициентЕТС.Видимость = Истина;
	ЭтаФорма.Элементы.ТарифыРазрядКатегорияПовышающийКоэффициент.Видимость = Истина;	
	
КонецПроцедуры
// }
// 4D

// 4D:ERP для Беларуси, Яна, 15.05.2017 10:44:40 
// Документ "Утверждение тарифной сетки" и справочник "Тарифная сетка", № 14918 
// {
&НаКлиентеНаСервереБезКонтекста
Процедура РассчитатьДолжностнойОклад (ТекущиеДанные,СпособОкругления, СтруктураРазрядКатегория)
	...
	Если ТекущиеДанные.РазрядКатегория <> Неопределено И СтруктураРазрядКатегория.ПустаяСсылка = Ложь Тогда 
		ТекущиеДанные.ДолжностнойОклад = ТекущиеДанные.ТарифныйОклад + (ТекущиеДанные.ТарифныйОклад *  СтруктураРазрядКатегория.ПроцентПовышения/100); 
	КонецЕсли;
	
	ТекущиеДанные.ДолжностнойОклад = ЗарплатаКадрыКлиентСервер.Округлить(ТекущиеДанные.ДолжностнойОклад, ОписаниеСпособаОкругления.Точность, ОписаниеСпособаОкругления.ПравилоОкругления);
	
КонецПроцедуры

&НаСервере
Функция РазрядыКатегорииДолжностейСтруктура(РазрядКатегория) 
	
	СтруктураРазрядКатегория =  Новый Структура;
	
	СтруктураРазрядКатегория.Вставить("Коэффициент", РазрядКатегория.Коэффициент);
	СтруктураРазрядКатегория.Вставить("ПовышающийКоэффициент", РазрядКатегория.ПовышающийКоэффициент);
	СтруктураРазрядКатегория.Вставить("ПроцентПовышения", РазрядКатегория.ПроцентПовышения);
		
	Если РазрядКатегория = Справочники.РазрядыКатегорииДолжностей.ПустаяСсылка() Тогда 
		СтруктураРазрядКатегория.Вставить("ПустаяСсылка", Истина);
	Иначе 
		СтруктураРазрядКатегория.Вставить("ПустаяСсылка", Ложь);
	КонецЕсли;
	
	Возврат СтруктураРазрядКатегория;
	
КонецФункции
// }
// 4D

// 4D:ERP для Беларуси, Юлия, 18.08.2017 16:25:00 
// Утверждение тарифной группы. Ошибка расчета тарифного и должностного оклада, № 15709
// {
&НаСервере
Процедура ТарифыТарифныйОкладПриИзмененииНаСервере()
...
&НаКлиенте
Процедура ТарифыТарифныйОкладПриИзменении(Элемент)
	
	ТарифыТарифныйОкладПриИзмененииНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура РассчитатьДолжностнойОкладБезПривязкиКТарифнойСетке()
	БазовоеЗначение = Объект.БазовыйТарифГруппы;
