...
Процедура ПриОткрытии(Отказ) 
	...
	Если Объект.ПередачаПоЗаказам
		...
		ТекстСообщения = НСтр("ru='Уточните склад передачи и заполните сырье и материалы с помощью команды ""Заполнить по заказу""'");
...
Процедура ПриИзмененииПартнераСервер() 
	...
	КонецЕсли;
	ТаблицаОснованийДляПечати.Загрузить(Документы.ПередачаСырьяПереработчику.ПолучитьТаблицуОснованийДляПечати(Объект));
...
Процедура УстановитьЗаголовокЗаполнитьПоЗаказамОрдерам()
	...
	Если НЕ ОрдернаяСхемаПриОтгрузке Тогда
		ЗаголовокЗаполнить = НСтр("ru = 'Подобрать из заказа'");
	Иначе
		ЗаголовокЗаполнить = НСтр("ru = 'Подобрать по заказу/ордерам'");
	КонецЕсли;
...
Функция ПривязатьСтрокиКЗаказам()
	...
	Запрос = Новый Запрос("
		...
		|		ПО ЭтапПроизводства.Ссылка = ТаблицаУслуги.Распоряжение
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	ДопОтборы.Вставить("Склад", Объект.Склад);
	
	Документы.ПередачаСырьяПереработчику.ЗаполнитьПоЗаказамОрдерам(
			ТаблицаНакладная, 
			Объект.Ссылка, 
			МассивЗаказов, 
			ПараметрыЗаполнения, 
