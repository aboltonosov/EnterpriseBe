...
Процедура ОбновитьИнформациюПоЗаказам(ТоварыДокумента)
	...
	ПараметрыОбновления.ПутьЗаказаВШапке                  = "ЗаказПереработчику";
	...
	ПараметрыОбновления.ИспользоватьЗаказыВТабличнойЧасти = Ложь;
	
	ЗаказыСервер.ОбновитьИнформациюПоЗаказамВФорме(ЭтаФорма, ТоварыДокумента, ПараметрыОбновления);
...
Процедура УстановитьЗаголовокЗаполнитьПоЗаказамОрдерам()
	...
	Если НЕ ИспользоватьОрдернуюСхемуПриОтгрузке Тогда
		ЗаголовокЗаполнить = НСтр("ru = 'Подбор сырья и материалов по заказам'");
	Иначе
		ЗаголовокЗаполнить = НСтр("ru = 'Подбор сырья и материалов по заказу/ордерам'");
	КонецЕсли;
