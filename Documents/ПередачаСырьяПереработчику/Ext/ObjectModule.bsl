...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ДокументОснование")
			...
			ЗаполнитьНаОснованииЗаказаПереработчика(
	...
	Иначе 
		...
		Если ТипЗнч(ДанныеЗаполнения) = Тип("Массив") Тогда // команда ввода на основании
			...
			Иначе
				...
				Если ПродажиВызовСервера.СформироватьДанныеЗаполненияРеализации(ДанныеЗаполнения, РеквизитыШапки) Тогда
...
Процедура ЗаполнитьНаОснованииЗаказаПереработчика(Знач ДокументОснование,
	...
	СтруктураОснование = Документы.ПередачаСырьяПереработчику.СтруктураОснованияДляПечати(ЭтотОбъект);
	...
	Если ТаблицаОшибок.Количество() > 0 Тогда
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			Если СтрокаТаблицы.НеУказанНомерГТД Тогда
				ТекстСообщения = НСтр("ru = 'Номенклатура: %1, на складе %2 недостаточно %3 %4 с указанными номерами ГТД'");
			Иначе
				ТекстСообщения = НСтр("ru = 'Номенклатура: %1, на складе %2 недостаточно %3 %4'");
			КонецЕсли;
