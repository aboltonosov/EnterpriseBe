...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	КонтролироватьНомераГТД = ЗапретитьПоступлениеТоваровБезНомеровГТД
		И ((ХозяйственнаяОперация = ХозяйственныеОперации.СборкаТоваров) И ВестиУчетПоГТД И Не ЗначениеЗаполнено(НомерГТД)
	...
	ИначеЕсли (ХозяйственнаяОперация = ХозяйственныеОперации.РазборкаТоваров) И КонтролироватьНомераГТД Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Номер ГТД"" в строке %НомерСтроки% списка ""Товары"".'");
...
Процедура ЗаполнитьПоЗаказу(СтруктураЗаполнения)
	...
	ПараметрыЗаполнения.НакладнаяПоЗаказу           = МассивЗаказов.Количество() > 0;
...
Процедура ЗаполнитьНомераГТД()
	Если Не ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров Тогда
		Возврат;
	КонецЕсли;
