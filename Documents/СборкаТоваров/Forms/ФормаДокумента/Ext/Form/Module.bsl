...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	ПараметрыЗаполнения.НакладнаяПоЗаказу           = ЗначениеЗаполнено(Объект.ЗаказНаСборку);
	ПараметрыЗаполнения.ОрдернаяСхемаПриПоступлении = ОрдернаяСхемаПриПоступлении;
	...
	ПоРасходнымОрдерам = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров);
	...
	ПоРасходнымОрдерам = (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров);
	ПараметрыЗаполнения.ПоШапке = Истина;
	ПараметрыЗаполнения.ПоРасходнымОрдерам = ПоРасходнымОрдерам;
	
	Документы.СборкаТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладнаяПоШапке, 
		Объект.Ссылка, 
		МассивРаспоряжений, 
		ПараметрыЗаполнения);
...
Процедура ОткрытьПодборТоваровПоЗаказамОрдерам()
	...
	ЭтоСборка = Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.СборкаТоваров");
	
	РеквизитыШапки = Новый Структура("Организация, Подразделение, Склад, ХозяйственныеОперации, Ссылка, НаправлениеДеятельности, Номенклатура, Характеристика");
...
Функция ТоварыНакладной()
	...
	Таблица.Колонки.Добавить("РаспоряжениеПриходногоОрдера", Новый ОписаниеТипов("ДокументСсылка.ЗаказНаСборку, ДокументСсылка.СборкаТоваров"));
	...
	РаспоряжениеТоваровЭтоНакладная   = Не СборкаПоЗаказу 
		Или (НакладнаяЯвляетсяРаспоряжением И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РазборкаТоваров);
	РаспоряжениеКомплектаЭтоНакладная = Не СборкаПоЗаказу 
		Или (НакладнаяЯвляетсяРаспоряжением И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СборкаТоваров);
	...
	НоваяСтрока.РаспоряжениеПриходногоОрдера = ?(РаспоряжениеКомплектаЭтоНакладная, Объект.Ссылка, Объект.ЗаказНаСборку);
	НоваяСтрока.ЗаказНаСборку = ?(ЗначениеЗаполнено(Объект.ЗаказНаСборку), Объект.ЗаказНаСборку, Объект.Ссылка);
	НоваяСтрока.Сделка = Объект.Сделка;
	...
	Для Каждого Строка Из Объект.Товары Цикл
		...
		НоваяСтрока.РаспоряжениеПриходногоОрдера = ?(РаспоряжениеТоваровЭтоНакладная, Объект.Ссылка, Объект.ЗаказНаСборку);
		НоваяСтрока.ЗаказНаСборку = ?(ЗначениеЗаполнено(Объект.ЗаказНаСборку), Объект.ЗаказНаСборку, Объект.Ссылка);
		НоваяСтрока.Сделка = Объект.Сделка;
...
Процедура ОбработкаПодбораТоваровИзЗаказа(АдресВХранилище)
	...
	ЗаполнитьЗначенияСвойств(КлючКомплекта, Объект);
	
	НайденныеСтроки = ПодобранныеТовары.НайтиСтроки(КлючКомплекта);
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		ПодобранныеТовары.Удалить(НайденныеСтроки[0]);
	КонецЕсли;
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.НомерГТДСтранаПроисхождения.Видимость = Элементы.НомерГТД.Видимость;
