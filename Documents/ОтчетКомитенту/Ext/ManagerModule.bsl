...
Функция ТекстЗапросаТаблицаУслугиКОформлениюОтчетовПринципалу(Запрос, ТекстыЗапроса, Регистры)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:43:26 
	// Ставки НДС 20% и 25%
	// {
	|		КОГДА КОформлению.СтавкаНДС В (&СтавкиНДС20) 
	|			ТОГДА КОформлению.СуммаПродажи - КОформлению.СуммаПродажи / 1.20
	|		КОГДА КОформлению.СтавкаНДС В (&СтавкиНДС25) 
	|			ТОГДА КОформлению.СуммаПродажи - КОформлению.СуммаПродажи / 1.25
	// }
	// 4D
	|		ИНАЧЕ 0
	|	КОНЕЦ   								  КАК СуммаНДС,
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:44:21 
	// Ставки НДС 20% и 25%
	// {
	...
	СтавкиНДС25 = Новый Массив();
	СтавкиНДС25.Добавить(Перечисления.СтавкиНДС.НДС25);
	СтавкиНДС25.Добавить(Перечисления.СтавкиНДС.НДС25_125);
	
	Запрос.УстановитьПараметр("СтавкиНДС20",  СтавкиНДС20);
	Запрос.УстановитьПараметр("СтавкиНДС25",  СтавкиНДС25);
	// }
	// 4D
