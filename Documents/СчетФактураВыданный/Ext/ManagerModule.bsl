...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Документы.СчетФактураВыданный.ДобавитьКомандуСоздатьНаОснованииКорректировочныйСчетФактура(КомандыСоздатьНаОсновании);
...
Функция ДобавитьКомандуСоздатьНаОснованииКорректировочныйСчетФактура(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураВыданный) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Дополнительный/исправленный счет-фактура'");
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураВыданный) Тогда
		КомандаСоздатьНаОсновании = КомандыСоздатьНаОсновании.Добавить();
		КомандаСоздатьНаОсновании.Идентификатор = Метаданные.Документы.СчетФактураВыданный.ПолноеИмя();
		КомандаСоздатьНаОсновании.Представление = ВводНаОсновании.ПредставлениеОбъекта(Метаданные.Документы.СчетФактураВыданный);
		КомандаСоздатьНаОсновании.ПроверкаПроведенияПередСозданиемНаОсновании = Истина;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.КППКонтрагента ПОДОБНО """"
	|			ТОГДА ЕСТЬNULL(ДанныеДокумента.Контрагент.КПП, """")
	|		ИНАЧЕ ДанныеДокумента.КППКонтрагента
	|	КОНЕЦ КАК КонтрагентКПП,
	|	ЕСТЬNULL(ДанныеДокумента.Организация.КПП, """") КАК ОрганизацияКПП,
	|	ДанныеДокумента.Подразделение   КАК Подразделение,
	|	ДанныеДокумента.Ответственный   КАК Ответственный,
	|	ДанныеДокумента.Проведен        КАК Проведен,
	// 4D:ERP для Беларуси, Дмитрий, 09.10.2017 22:32:57 
	// ЭСЧФ
	// {
	...
	// 4D:ERP для Беларуси, Дмитрий, 25.09.2017 16:14:20 
	// ЭСЧФ
	// {
...
//++ НЕ УТКА
#Область ТекстОтчетДавальцу
	ТекстОтчетДавальцу = "
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги),
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги),
	|	ТаблицаУслуги.Ссылка.СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл,0) = 0
	|			ТОГДА ВЫРАЗИТЬ((ТаблицаУслуги.СуммаСНДС - ТаблицаУслуги.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ СуммыДокументовВВалютеРегл.БазаНДСРегл
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.МенеджерПечати = "Обработка.ЭлектронныеСчетФактуры";
	КомандаПечати.Идентификатор = "ЭСЧФ";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст =
	// 4D:ERP для Беларуси, ВалентинМ, 27.06.2017 16:29:49 
	// Новые реквизиты итоговых сумм
	// {
	"ВЫБРАТЬ
	|	СчетФактураВыданный.Ссылка
	|ИЗ
	...
	|
	|ИМЕЮЩИЕ
	|	(СчетФактураВыданный.СуммаАкциза <> ЕСТЬNULL(СУММА(СчетФактураВыданныйТовары.СуммаАкциза), 0)
	|		ИЛИ СчетФактураВыданный.СуммаНДС <> ЕСТЬNULL(СУММА(СчетФактураВыданныйТовары.СуммаНДС), 0)
	|		ИЛИ СчетФактураВыданный.СуммаДокумента <> ЕСТЬNULL(СУММА(СчетФактураВыданныйТовары.СуммаСНДС), 0))";
	// }
	// 4D
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// 4D:ERP для Беларуси, ВалентинМ, 27.06.2017 16:31:29 
	// Новые реквизиты итоговых сумм
	// {
	ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры);
	...
	Пока Выборка.Следующий() Цикл
		...
		струкРеквизитов = Новый Структура("СуммаАкциза, СуммаНДС, СуммаДокумента", "СуммаАкциза", "СуммаНДС", "СуммаСНДС");
		Для Каждого КлЗн Из струкРеквизитов Цикл
			струкРеквизитов[КлЗн.Ключ] = ДокументОбъект.Товары.Итог(КлЗн.Значение);
			Если ДокументОбъект[КлЗн.Ключ] <> струкРеквизитов[КлЗн.Ключ] Тогда
				ДокументОбъект[КлЗн.Ключ] = струкРеквизитов[КлЗн.Ключ];
...
Функция ТекстЗапросаВтТаблицаДанныхОснований(Запрос, ТекстыЗапроса)
	...
	|		ЕСТЬNULL(ШапкаОснования.ДокументОснование.ХозяйственнаяОперация, ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту)),
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	// Серия в реестре документов
	// {
	|	МАКСИМУМ(ВложенныйЗапрос.СерияПервичногоДокумента) КАК СерияПервичногоДокумента,
	// }
	// 4D
