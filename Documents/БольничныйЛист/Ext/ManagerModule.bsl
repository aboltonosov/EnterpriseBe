...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 07.12.2017 14:49:26 
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_РасчетПособия") Тогда
		...
		"ПФ_MXL_РасчетПособия", НСтр("ru = 'Расчет пособия к листку нетрудоспособности'"),
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ДоплатаЗаДниНетрудоспособности") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_MXL_ДоплатаЗаДниНетрудоспособности", НСтр("ru = 'Доплата за дни нетрудоспособности'"),
...
Функция ДанныеДокументовДляПечатиРасчетаСреднегоЗаработкаФСС(МассивСсылок) Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 01.03.2018 16:55:49 
	// Доработать печатные формы док-та "Больничный лист", № 17952
	// {
	|	БольничныйЛист.РасчетПремии,
	// }
	// 4D
...
Функция ДанныеДляПечатиРасчетаПособия(МассивСсылок)
	...
	|	БольничныеЛисты.ДатаНачалаСобытия КАК Период,
	...
	|	ВЫБОР
	|		КОГДА БольничныеЛисты.ДатаНарушенияРежима >= БольничныеЛисты.ДатаНачалаОплаты
	|				И БольничныеЛисты.ДатаНарушенияРежима <= БольничныеЛисты.ДатаОкончанияОплаты
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Снижено,
	|	ВЫБОР
	|		КОГДА БольничныеЛисты.ДатаНарушенияРежима = ДАТАВРЕМЯ(1, 1, 1)
	|				ИЛИ БольничныеЛисты.ДатаОкончанияОплаты < БольничныеЛисты.ДатаНарушенияРежима
	|			ТОГДА РАЗНОСТЬДАТ(БольничныеЛисты.ДатаНачалаОплаты, БольничныеЛисты.ДатаОкончанияОплаты, ДЕНЬ)
	|		ИНАЧЕ РАЗНОСТЬДАТ(БольничныеЛисты.ДатаНачалаОплаты, БольничныеЛисты.ДатаНарушенияРежима, ДЕНЬ)
	|	КОНЕЦ + 1 КАК ДнейВПолномРазмере,
	|	ВЫБОР
	|		КОГДА РАЗНОСТЬДАТ(БольничныеЛисты.ДатаНачалаОплаты, БольничныеЛисты.ДатаОкончанияОплаты, ДЕНЬ) < 12
	|			ТОГДА РАЗНОСТЬДАТ(БольничныеЛисты.ДатаНачалаОплаты, БольничныеЛисты.ДатаОкончанияОплаты, ДЕНЬ)
	|		ИНАЧЕ 12
	|	КОНЕЦ КАК ДнейПо80процентов,
	|	ВЫБОР
	|		КОГДА БольничныеЛисты.ДатаНарушенияРежима <> ДАТАВРЕМЯ(1, 1, 1)
	|				И БольничныеЛисты.ДатаОкончанияОплаты >= БольничныеЛисты.ДатаНарушенияРежима
	|			ТОГДА РАЗНОСТЬДАТ(БольничныеЛисты.ДатаНарушенияРежима, БольничныеЛисты.ДатаОкончанияОплаты, ДЕНЬ) + 1
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК ДнейПо50процентов,
	|	БольничныеЛисты.ПричинаНетрудоспособности КАК ПричинаНетрудоспособности,
	|	ВЫБОР
	|		КОГДА БольничныеЛисты.ПричинаНетрудоспособности = ЗНАЧЕНИЕ(Перечисление.ПричиныНетрудоспособности.ПоБеременностиИРодам)
	|			ТОГДА ""по беременности и родам""
	|		КОГДА БольничныеЛисты.ПричинаНетрудоспособности = ЗНАЧЕНИЕ(Перечисление.ПричиныНетрудоспособности.ПоУходуЗаРебенком)
	|			ТОГДА ""по уходу за больным ребенком""
	|		КОГДА БольничныеЛисты.ПричинаНетрудоспособности = ЗНАЧЕНИЕ(Перечисление.ПричиныНетрудоспособности.ПоУходуЗаВзрослым)
	|			ТОГДА ""по уходу за больным членом семьи""
	|		ИНАЧЕ ""по временной нетрудоспособности""
	|	КОНЕЦ КАК ПричинаНетрудоспособностиСтрокой,
	...
	|	БольничныеЛисты.СреднийДневнойЗаработок,
	|	БольничныеЛисты.МинимальныйСреднедневнойЗаработок,
	|	БольничныеЛисты.ОграничениеПособия,
	|	БольничныеЛисты.ПорядокВыплаты,
	|	БольничныеЛисты.ДоляНеполногоВремени,
	|	БольничныеЛисты.ДатаНачалаПоловиннойОплаты,
	|	БольничныеЛисты.Дата КАК ДатаЛисткаНетрудоспособности,
	|	БольничныеЛисты.ВидОплатыПособия.КатегорияНачисленияИлиНеоплаченногоВремени КАК КатегорияНачисления
	...
	|	ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(БольничныеЛисты.ДатаНачалаСобытия, ГОД), МЕСЯЦ, -12) КАК Период
	|ПОМЕСТИТЬ ВТДваПоследнихГода
	|ИЗ
	...
	|ВЫБРАТЬ
	|	БольничныеЛисты.Ссылка,
	|	ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(БольничныеЛисты.ДатаНачалаСобытия, ГОД), МЕСЯЦ, -24)
	|ИЗ
	|	ВТБольничныеЛисты КАК БольничныеЛисты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(БольничныеЛисты.ДатаНачалаОплаты, МЕСЯЦ), ДЕНЬ, -1) КАК Период,
	|	БольничныеЛисты.Ссылка КАК Ссылка
	|ПОМЕСТИТЬ ВТПериодыНачала
	|ИЗ
	|	ВТБольничныеЛисты КАК БольничныеЛисты";
		
	Запрос.Выполнить();
	
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра("ВТПериодыНачала");
	...
	|	БольничныеЛисты.Организация,
	|	БольничныеЛисты.НомерЛисткаНетрудоспособности,
	|	БольничныеЛисты.ДатаЛисткаНетрудоспособности,
	...
	|	БольничныеЛисты.ДнейПо80процентов,
	|	БольничныеЛисты.ПричинаНетрудоспособности КАК ПричинаНетрудоспособности,
	|	БольничныеЛисты.ПричинаНетрудоспособностиСтрокой,
	|	БольничныеЛисты.ДатаНачала,
	|	БольничныеЛисты.ДатаОкончания,
	...
	|	БольничныеЛисты.МинимальныйСреднедневнойЗаработок,
	|	БольничныеЛисты.КатегорияНачисления КАК КатегорияНачисления
	|ИЗ
	|	ВТБольничныеЛисты КАК БольничныеЛисты
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	БольничныйЛистНачисления.ДатаНачала КАК ДатаНачалаПериода,
	|	БольничныйЛистНачисления.ДатаОкончания КАК ДатаОкончанияПериода,
	|	БольничныйЛистНачисления.ОплаченоДней КАК ЧислоДнейОплаты,
	|	БольничныйЛистНачисления.Ссылка КАК Ссылка,
	|	БольничныйЛистНачисления.ПроцентОплаты КАК РазмерПособияВПроцентах,
	|	БольничныеЛисты.СреднийДневнойЗаработок КАК СреднедневнойЗаработок,
	|	БольничныйЛистНачисления.Результат КАК Результат,
	|	МаксимальнаяОплатаТруда.Размер / ДЕНЬ(КОНЕЦПЕРИОДА(БольничныйЛистНачисления.ДатаНачала, МЕСЯЦ)) * БольничныйЛистНачисления.ОплаченоДней КАК МаксимальныйРазмерПособия,
	|	БольничныйЛистПоказатели.Значение КАК ТарифныйОклад
	|ИЗ
	|	ВТБольничныеЛисты КАК БольничныеЛисты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТМаксимальнаяОплатаТруда КАК МаксимальнаяОплатаТруда
	|		ПО (ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(БольничныеЛисты.ДатаНачалаОплаты, МЕСЯЦ), ДЕНЬ, -1) = МаксимальнаяОплатаТруда.Период)
	...
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.БольничныйЛист.Показатели КАК БольничныйЛистПоказатели
	|			ПО БольничныйЛистНачисления.Ссылка = БольничныйЛистПоказатели.Ссылка
	|				И БольничныйЛистНачисления.НомерСтроки = БольничныйЛистПоказатели.ИдентификаторСтрокиВидаРасчета
	|				И (БольничныйЛистПоказатели.Показатель = ЗНАЧЕНИЕ(Справочник.ПоказателиРасчетаЗарплаты.ТарифныйОклад))
	|		ПО БольничныеЛисты.Ссылка = БольничныйЛистНачисления.Ссылка
	|ГДЕ
	|	ПВРНачисления.КатегорияНачисленияИлиНеоплаченногоВремени В (ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛиста), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛистаЗаСчетРаботодателя), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоПрофзаболевание), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоНесчастныйСлучайНаПроизводстве), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОтпускПоБеременностиИРодам), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛистаОтТарифногоОклада), ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ОтпускПоБеременностиИРодамОтТарифногоОклада))
	|
	|УПОРЯДОЧИТЬ ПО
	|	БольничныеЛисты.ДатаНачала
	|ИТОГИ
	|	СУММА(ЧислоДнейОплаты),
	|	СУММА(Результат),
	|	СУММА(МаксимальныйРазмерПособия)
	|ПО
	|	Ссылка
	|;
	|
	...
	|	БольничныеЛисты.Ссылка КАК Ссылка,
	|	ЕСТЬNULL(ВЫБОР
	|			КОГДА БольничныеЛисты.ПорядокВыплаты = ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Межрасчет)
	|					ИЛИ БольничныеЛисты.ПорядокВыплаты = ЗНАЧЕНИЕ(Перечисление.ХарактерВыплатыЗарплаты.Аванс)
	|				ТОГДА БольничныйЛистНДФЛ.Налог
	|			ИНАЧЕ 0
	|		КОНЕЦ, 0) КАК НДФЛ
	|ИЗ
	|	ВТБольничныеЛисты КАК БольничныеЛисты
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.БольничныйЛист.НДФЛ КАК БольничныйЛистНДФЛ
	|		ПО БольничныеЛисты.Ссылка = БольничныйЛистНДФЛ.Ссылка
	|ИТОГИ
	|	СУММА(НДФЛ)
	|ПО
	|	Ссылка";
	...
	СписокСотрудников = Результат[КоличествоРезультатов - 4].Выгрузить().ВыгрузитьКолонку("Сотрудник");
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, СписокСотрудников, "ФИОПолные,ФамилияИО,ИНН,СтраховойНомерПФР,Подразделение,Должность,ТабельныйНомер,Пол");
	
	ВыборкаДляШапки = Результат[КоличествоРезультатов - 3].Выбрать();
	ВыборкаНачислений = Результат[КоличествоРезультатов - 2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);						
	ВыборкаНДФЛ = Результат[КоличествоРезультатов - 1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);						
	...
	ДанныеДляПечатиРасчетаПособия.Вставить("ВыборкаНДФЛ", ВыборкаНДФЛ);
...
Процедура ВывестиДанныеРасчетаПособияВТабличныйДокумент(Макет, ДокументРезультат, ДанныеДляПечатиРасчетаПособия, ОбъектыПечати)
	...
	ВыборкаНДФЛ                 = ДанныеДляПечатиРасчетаПособия.ВыборкаНДФЛ;
	...
	Пока ВыборкаДляШапки.Следующий() Цикл
		...
		СекцияСведенияОЗаработке = Макет.ПолучитьОбласть("СведенияОЗаработке");
		СекцияРасчетПособияИтог = Макет.ПолучитьОбласть("РасчетПособияПодвал");
		...
		СекцияШапка.Параметры.ДатаЛисткаНетрудоспособности = Формат(СекцияШапка.Параметры.ДатаЛисткаНетрудоспособности,"ДЛФ=D");
		...
		Иначе
			...
			ДнейПо80процентов 	= ВыборкаДляШапки.ДнейПо80процентов;
			ДнейПо50процентов   = ВыборкаДляШапки.ДнейПо50процентов;
			ПроцентОплаты 		= ВыборкаДляШапки.ПроцентОплаты;
			
			//"Назначено пособие по временной нетрудоспособности в размере 80% среднего заработка за первые 12 календарных дней и 100% среднего заработка за последующие дни с учетом ограничения общего размера.";
			
			ОписаниеНазначениеПособия = "Назначено пособие " + ВыборкаДляШапки.ПричинаНетрудоспособностиСтрокой + " за " + ДнейВПолномРазмере + ПерсонифицированныйУчет.ФормаМножественногоЧисла(" календарный день"," календарных дня"," календарных дней", ДнейВПолномРазмере) + " из них в размере ";
			Если ПроцентОплаты = 80 Тогда 
				ОписаниеНазначениеПособия = ОписаниеНазначениеПособия + ПроцентОплаты + "% за первые " + ДнейПо80процентов + ПерсонифицированныйУчет.ФормаМножественногоЧисла(" календарный день"," календарных дня"," календарных дней", ДнейПо80процентов) + " и 100% за " + (ДнейВПолномРазмере - ДнейПо80процентов) + ПерсонифицированныйУчет.ФормаМножественногоЧисла(" календарный день"," календарных дня"," календарных дней", ДнейПо50процентов);
			ИначеЕсли ПроцентОплаты = 100 Тогда
				ОписаниеНазначениеПособия = ОписаниеНазначениеПособия + ПроцентОплаты + "% за " + ДнейВПолномРазмере + ПерсонифицированныйУчет.ФормаМножественногоЧисла(" календарный день"," календарных дня"," календарных дней", ДнейВПолномРазмере); 
			КонецЕсли;
			
			Если ВыборкаДляШапки.Снижено Тогда
				ОписаниеНазначениеПособия = ОписаниеНазначениеПособия + Символы.ПС + "Пособие снижено с " + Формат(ВыборкаДляШапки.ДатаНарушенияРежима, "ДЛФ=DD");
			КонецЕсли;
			
		КонецЕсли;
		СекцияШапка.Параметры.ОписаниеНазначенияПособия = ОписаниеНазначениеПособия;
		...
		Если ВыборкаДляШапки.КатегорияНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОплатаБольничногоЛистаОтТарифногоОклада Или ВыборкаДляШапки.КатегорияНачисления = Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ОтпускПоБеременностиИРодамОтТарифногоОклада Тогда
			
			ВыборкаНачислений.Сбросить();
			ЗначенияЗаполнения = Новый Структура;
			
			НачисленоВсего = 0;
			РазмерДневногоПособияИтого = 0;
			СекцияРасчетПособияШапка = Макет.ПолучитьОбласть("РасчетПособияШапкаТО");
			...
			Если ВыборкаНачислений.НайтиСледующий(Новый Структура("Ссылка", ВыборкаДляШапки.Ссылка)) Тогда
				...
				НачисленоВсего = ВыборкаНачислений.Результат; 
				
				Пока ВыборкаНачисленийПоДокументу.Следующий() Цикл
					
					СекцияРасчетПособияСтрока = Макет.ПолучитьОбласть("РасчетПособияСтрокаТО");
					ЗаполнитьЗначенияСвойств(СекцияРасчетПособияСтрока.Параметры, ВыборкаНачисленийПоДокументу);
					КаледарныеДниМесяца = День(КонецМесяца(ВыборкаНачисленийПоДокументу.ДатаНачалаПериода));
					//РазмерДневногоПособия = Окр(ВыборкаНачисленийПоДокументу.ЧислоДнейОплаты * (ВыборкаНачисленийПоДокументу.ТарифныйОклад/КаледарныеДниМесяца) * ВыборкаНачисленийПоДокументу.РазмерПособияВПроцентах / 100,2);
					//РазмерДневногоПособияИтого = РазмерДневногоПособияИтого + РазмерДневногоПособия; 				
					ЗначенияЗаполнения.Вставить("РазмерПособияВПроцентах", 	ВыборкаНачисленийПоДокументу.РазмерПособияВПроцентах);
				...
				КонецЦикла;
				СекцияРасчетПособияИтог = Макет.ПолучитьОбласть("РасчетПособияИтогоТО");
				//ЗначенияЗаполнения.Вставить("РазмерДневногоПособия", 	РазмерДневногоПособияИтого);
				
				ЗаполнитьЗначенияСвойств(СекцияРасчетПособияИтог.Параметры, ВыборкаНачислений);
				ЗаполнитьЗначенияСвойств(СекцияРасчетПособияИтог.Параметры, ЗначенияЗаполнения);
		...
		Иначе
			...
			ДокументРезультат.Вывести(СекцияСведенияОЗаработке);
			...
			Если ВыборкаНачислений.НайтиСледующий(Новый Структура("Ссылка", ВыборкаДляШапки.Ссылка)) Тогда
				...
				Пока ВыборкаНачисленийПоДокументу.Следующий() Цикл
					...
					РазмерДневногоПособия = Окр(ВыборкаНачисленийПоДокументу.ЧислоДнейОплаты * ВыборкаНачисленийПоДокументу.СреднедневнойЗаработок * ВыборкаНачисленийПоДокументу.РазмерПособияВПроцентах / 100,2);
					РазмерДневногоПособияИтого = РазмерДневногоПособияИтого + РазмерДневногоПособия; 				
					ЗначенияЗаполнения.Вставить("РазмерПособияВПроцентах", 	ВыборкаНачисленийПоДокументу.РазмерПособияВПроцентах);
					ЗначенияЗаполнения.Вставить("РазмерДневногоПособия", 	РазмерДневногоПособия);
				...
				КонецЦикла;
				СекцияРасчетПособияИтог = Макет.ПолучитьОбласть("РасчетПособияИтого");
				ЗначенияЗаполнения.Вставить("РазмерДневногоПособия", 	РазмерДневногоПособияИтого);
		...
		РасчетПособияПодвал = Макет.ПолучитьОбласть("РасчетПособияПодвал");
		РасчетПособияПодвал.Параметры.НачисленоВсего 					= НачисленоВсего;
		РасчетПособияПодвал.Параметры.НачисленоВсегоПрописью 			= РаботаСКурсамиВалют.СформироватьСуммуПрописью(НачисленоВсего, ВалютаУчета);
		ДокументРезультат.Вывести(РасчетПособияПодвал);
...
Функция СоответствиеПолейЭЛН() Экспорт
	...
	СоответствиеПолей.Вставить("ДатаНачалаРодственник1",    "SERV1_DT1");
	СоответствиеПолей.Вставить("ДатаОкончанияРодственник1", "SERV1_DT2");
	...
	СоответствиеПолей.Вставить("ДатаНачалаРодственник2",    "SERV2_DT1");
	СоответствиеПолей.Вставить("ДатаОкончанияРодственник2", "SERV2_DT2");
