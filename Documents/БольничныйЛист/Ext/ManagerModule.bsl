...
Функция ДанныеДляПечатиРасчетаПособия(МассивСсылок)
	...
	|	ВЫБОР
	|		КОГДА БольничныеЛисты.СтажРасширенныйМесяцев = 0
	...
	Пока ВыборкаДляШапки.Следующий() Цикл
		...
		Иначе
			...
			Если ВыборкаДляШапки.Снижено Тогда
				ОписаниеНазначениеПособия = СтрШаблон(Нстр("ru = '%1%2Пособие снижено с %3'"), ОписаниеНазначениеПособия, Символы.ПС, Формат(ВыборкаДляШапки.ДатаНачалаПоловиннойОплаты, "ДЛФ=DD"));
			КонецЕсли;
