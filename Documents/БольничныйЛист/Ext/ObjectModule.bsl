...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	И ПрямыеВыплатыПособийСоциальногоСтрахованияРасширенный.КоллекцияСодержитПособияЗаСчетФСС(Начисления.ВыгрузитьКолонку("Начисление"));
...
Процедура ПроверитьЗаполнениеРеквизитовНеобходимыхДляРасчета(КонтейнерОшибок, ПроверкаПередРасчетом = Ложь)
	...
	Если НазначитьПособие Тогда
		...
		Иначе
			...
			// Ошибка при проведении документа "Больничный лист", № 16117
			// {
			...
			Если ПрименятьЛьготыПриНачисленииПособия И Не ЗначениеЗаполнено(ЛьготыОплатыПособий) Тогда
