...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Накладная") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "Накладная", НСтр("ru = 'Выкуп возвратной тары'"), СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати, Неопределено));
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрНомеровГТД") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "РеестрНомеровГТД", НСтр("ru = 'Реестр номеров ГТД'"), СформироватьПечатнуюФормуРеестрНомеровГТД(МассивОбъектов, ОбъектыПечати, Неопределено));
	КонецЕсли;
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Если ТекущийКомплект.Количество() > 0 Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			Обработки.ПечатьОбщихФорм.СформироватьПечатнуюФормуУПД(СтруктураТипов, ОбъектыПечати, Неопределено, ТекущийКомплект));
	КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	// Локализация печатных форм, №7762
	// {
	|	ВыкупВозвратнойТарыКлиентом.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	ВыкупВозвратнойТарыКлиентом.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	ВыкупВозвратнойТарыКлиентом.Отпустил КАК Отпустил,
	|	ВыкупВозвратнойТарыКлиентом.ОтпустилДолжность КАК ОтпустилДолжность,
	|	ВыкупВозвратнойТарыКлиентом.Руководитель.Наименование КАК Разрешил,
	|	ВыкупВозвратнойТарыКлиентом.Руководитель.Должность КАК РазрешилДолжность,
	|	ВыкупВозвратнойТарыКлиентом.Принял КАК Принял,
	|	ВыкупВозвратнойТарыКлиентом.ПереданыДокументы КАК ПереданыДокументы,
	// }
	// 4D
	|
...
// заполняет реквизиты "Порядок оплаты", "Курс" и "Кратность" документа "ВыкупВозвратнойТарыКлиентом".
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ВыкупВозвратнойТарыКлиентом КАК ВыкупВозвратнойТарыКлиентом
	|ГДЕ ВыкупВозвратнойТарыКлиентом.ПорядокОплаты = Значение(Перечисление.ПорядокОплатыПоСоглашениям.ПустаяСсылка)
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ВыкупВозвратнойТарыКлиентом";
