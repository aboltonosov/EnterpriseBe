...
Процедура ЗаполнитьРасчетныеДокументы()
	...
	|		КОГДА НЕ ПроведенныеОтпуска.Отпуск ЕСТЬ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(ПроведенныеОтпуска.Отпуск) = ТИП(Документ.Отпуск)
	|						ТОГДА ПроведенныеОтпуска.Отпуск.ДокументРассчитан
	|					ИНАЧЕ ПроведенныеОтпуска.Отпуск.ПерерасчетВыполнен
	|				КОНЕЦ
	|		КОГДА НЕ НеПроведенныеОтпуска.Отпуск ЕСТЬ NULL 
	|			ТОГДА ВЫБОР
	|					КОГДА ТИПЗНАЧЕНИЯ(НеПроведенныеОтпуска.Отпуск) = ТИП(Документ.Отпуск)
	|						ТОГДА НеПроведенныеОтпуска.Отпуск.ДокументРассчитан
	|					ИНАЧЕ НеПроведенныеОтпуска.Отпуск.ПерерасчетВыполнен
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Рассчитан,
	|	ВЫБОР
	...
	Если СтрокиСотрудника.Количество() > 0 Тогда
		...
		Для Каждого СтрокаСотрудника Из ВсеСтрокиСотрудника Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаСотрудника.Отпуск) 
				Или СтрокаСотрудника.Рассчитан 
...
Функция ДанныеОРасхожденияхСУчетом()
	...
	Для Каждого СтрокСотрудника Из ТаблицаСотрудников Цикл
		...
		Если СтрокиПоСотруднику.Количество() > 0 Тогда
			ОстаткиОтпуска = ОстаткиОтпусков.ОстатокОтпускаСотрудникаНаДату(СтрокСотрудника.Сотрудник, Объект.Дата, Объект.Ссылка);
