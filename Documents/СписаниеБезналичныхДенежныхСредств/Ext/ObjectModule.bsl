...
Функция ПодобратьРеквизитыПлатежаВБюджет(ИсключаяРеквизиты = "") Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 31.01.2018 18:04:34 
	...
	//|	КОНЕЦ КАК Субконто3,
	|	СписаниеРасшифровкаПлатежа.СчетУчета,
	|	СписаниеРасшифровкаПлатежа.Субконто1,
	|	СписаниеРасшифровкаПлатежа.Субконто2,
	|	СписаниеРасшифровкаПлатежа.Субконто3,
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Петр, 01.02.2018 9:01:41 
	// Локализовать документ Списание безналичных ДС, № 16881
	// {
	//|	ВЫБОР КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВБюджет) ТОГДА
	//|		НЕОПРЕДЕЛЕНО
	...
	|	СписаниеРасшифровкаПлатежа.СтатьяРасходов,
	// }
	// 4D
	...
	Если Не Результат.Пустой() Тогда
		...
		// 4D:ERP для Беларуси, Петр, 01.02.2018 9:03:29 
		// Локализовать документ Списание безналичных ДС, № 16881
		// {
		...
		ЗаполнитьЗначенияСвойств(РасшифровкаПлатежа[0], РеквизитыПлатежа, "СтатьяДвиженияДенежныхСредств, СтатьяРасходов, АналитикаАктивовПассивов, Подразделение");
		//++ НЕ УТ
		ЗаполнитьЗначенияСвойств(РасшифровкаПлатежа[0], РеквизитыПлатежа, "СчетУчета, Субконто1, Субконто2, Субконто3");
		//-- НЕ УТ
		// }
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипОснования = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств")
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// {
	РеквизитыКУдалению = Документы.СписаниеБезналичныхДенежныхСредств.ПолучитьРеквизитыКУдалению();
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьНачислениеЗарплатыУТ") И ПроведеноБанком Тогда
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаЗаймаСотруднику Тогда
			Для Каждого Строка Из РасшифровкаПлатежа Цикл
				ЗаймыСотрудникам.ЗарегистрироватьПредоставлениеЗайма(Движения, Строка.ДоговорЗаймаСотруднику, Строка.Сумма, ДатаПроведенияБанком, Организация, ПодотчетноеЛицо, Отказ);
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	// 4D:ERP для Беларуси, Алексей4Д, 10.10.2017 11:16:17 
	// Исключить очередность платежа российскую
	// {
