...
//                                                         "ИменаПолейДляОпределенияРаспоряжения" параметров указания серий этого документа.
// 
// Возвращаемое значение:
...
#Область ТекстВыпускПродукцииДавальцаЗабаланс // (Дт 002 :: Кт )
	...
	|			ТОГДА Строки.Склад
	|		ИНАЧЕ ЕСТЬNULL(Строки.Склад.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))
	|	КОНЕЦ";
#КонецОбласти
...
#КонецОбласти
	...
	Возврат
	ТекстВыпускСобственнойПродукции
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускСобственнойПродукцииСкладПроизводства
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускВПодразделение
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускПродукцииДавальцаБаланс
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускПродукцииДавальцаЗабаланс
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускПродукцииДавальцаБалансСИспользованием40СчетПлановая
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускПродукцииДавальцаБалансСИспользованием40СчетФактическая
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускПродукцииДавальцаБалансСИспользованием40СчетОтклонения
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыполнениеРаботПоВыпускуПродукцииДавальца
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускСобственнойПродукцииНаСкладСИспользованием40СчетаПлановаяСкладПроизводства
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстВыпускСобственнойПродукцииНаСкладСИспользованием40СчетаОтклонениеСкладПроизводства
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ИзменениеНаправленияДеятельностиМатериальныхЗатрат
	+ " ОБЪЕДИНИТЬ ВСЕ " + ВключениеИсключениеНДСВСтоимость
	;
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	ТекстСтрокиСУчетомАктивовПассивов = "
	...
	|		КОГДА СтрокиСебестоимости.СтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|				И СтрокиСебестоимости.СтатьяРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	...
	ТекстЗапроса = ТекстСтрокиСУчетомАктивовПассивов;
...
Процедура УстановитьПараметрыЗапросаКоэффициентыВалют(Запрос)
	...
	ПараметрыУчетнойПолитики = РегистрыСведений.УчетнаяПолитикаОрганизаций.ПараметрыУчетнойПолитики(Запрос.Параметры.Организация,
...
Процедура УстановитьПараметрыЗапросаВидЗапасовРаботы(Запрос)
	...
	ДопКолонки.Вставить("ХозяйственнаяОперация", Перечисления.ХозяйственныеОперации.ВыпускПродукции);
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса)
	...
	|		И АналитикаРасчетов.Партнер = ТаблицаТовары.ВидЗапасов.Комитент
...
Функция ТекстЗапросаТаблицаТоварыКОформлениюПоступления(Запрос, ТекстыЗапроса, Регистры)
	...
	|	Документ.ВыпускПродукции.Товары КАК ТаблицаТовары
	|ГДЕ
	|	(&ВыпускПоРаспоряжениям И НЕ &НакладнаяЯвляетсяРаспоряжением)
	...
	|	Документ.ВыпускПродукции.Товары КАК ТаблицаТовары
	|ГДЕ
	|	(НЕ &ВыпускПоРаспоряжениям ИЛИ &НакладнаяЯвляетсяРаспоряжением)
	|	И ВЫБОР КОГДА ТаблицаТовары.Склад.ИспользоватьОрдернуюСхемуПриПоступлении
	|			И &Дата >= ТаблицаТовары.Склад.ДатаНачалаОрдернойСхемыПриПоступлении
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|
	//++ НЕ УТКА
	// работы по выпуску давальческой продукции
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	6                                              КАК Порядок,
	|	ТаблицаТовары.НомерСтроки                      КАК НомерСтрокиДокумента,
...
Функция ТекстЗапросаТаблицаТоварыКПоступлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ
	|	И (Не &ВыпускПоРаспоряжениям ИЛИ &НакладнаяЯвляетсяРаспоряжением)
	|	И &НаправлениеВыпуска = Значение(Перечисление.ХозяйственныеОперации.ВыпускПродукцииНаСклад)";
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|		&НаправлениеВыпуска      КАК ТипСсылки,
	|		&Организация             КАК Организация,
	|		&ХозяйственнаяОперация   КАК ХозяйственнаяОперация,
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ПараметрыЗаполненияКлючей.ИзмененаАналитика Или ИзмененРеквизитПередатьДавальцу Тогда
