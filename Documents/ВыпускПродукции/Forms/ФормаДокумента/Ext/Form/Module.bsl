...
Процедура ОбновитьДанныеСпецификацииНаСервере(Поля, НайтиСпецификацию, Идентификаторы = Неопределено)
	...
	Если МассивСтрок.Количество() > 0 И НайтиСпецификацию Тогда
		УправлениеДаннымиОбИзделиях.ЗаполнитьСпецификациюВСтроках(МассивСтрок, МассивДанных, Истина);
	КонецЕсли;
...
Процедура АктуализироватьВыпускПодДеятельность(Заполнить = Истина)
	...
	Если Заполнить Тогда
		...
		Если Объект.НаправлениеВыпуска = Перечисления.ХозяйственныеОперации.ВыпускПродукцииНаСклад Тогда
			ПараметрСклад = Объект.Склад;
		КонецЕсли;
		Объект.ВыпускПодДеятельность = Справочники.Организации.НалогообложениеНДС(Объект.Организация, ПараметрСклад, Объект.Дата);
	КонецЕсли;
