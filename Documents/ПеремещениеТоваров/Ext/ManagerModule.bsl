...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПеремещениеПродукцииДавальцевБаланс()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПеремещениеПродукцииДавальцевЗабаланс()
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПередачаПродукцииДавальцевСОбособленногоПодразделенияБаланс()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПередачаПродукцииДавальцевСОбособленногоПодразделенияЗабаланс()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПередачаПродукцииДавальцевНаОбособленноеПодразделениеБаланс()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстПередачаПродукцииДавальцевНаОбособленноеПодразделениеЗабаланс()
...
//     * ЗаказНаПеремещение             - ДокументСсылка.ЗаказНаПеремещение, ДокументСсылка.ПеремещениеТоваров - 
//     * РаспоряжениеПриходногоОрдера   - ДокументСсылка.ЗаказНаПеремещение, ДокументСсылка.ПеремещениеТоваров - 
...
Функция ДанныеТаблицыТоварыДокумента(Накладная) Экспорт
	...
	|		ИЛИ НЕ Таблица.Ссылка.ПеремещениеПоЗаказам
	|		ТОГДА Таблица.Ссылка
	|		ИНАЧЕ Таблица.ЗаказНаПеремещение
...
//    * ХозяйственнаяОперация       - ПеречислениеСсылка.ХозяйственныеОперации - 
//    * Менеджер                    - ДокументМенеджер.ПеремещениеТоваров - 
//    * ИмяРегистраЗаказ            - Строка - 
//    * МенеджерРегистраЗаказ       - РегистрНакопленияМенеджер.ЗаказыНаПеремещение - 
//    * ИмяПоляЗаказ                - Строка - 
//    * Дата                        - Дата - 
//    * КлючевыеПоля                - Строка - 
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	НакладнаяЯвляетсяРаспоряжением = ЗакупкиСервер.РаспоряжениеНаПриемкуТовараНакладная(Неопределено);
	КорОрганизация = ?(Реквизиты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВнутренняяПередачаТоваров, Реквизиты.КорОрганизация, Неопределено);
	...
	Если ЗначениеЗаполнено(Реквизиты.СкладОтправитель) Тогда
		ИнформацияПоСкладуОтправителю = НСтр("ru='Перемещение с ""%СкладОтправитель""'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ИнформацияПоСкладуОтправителю = СтрЗаменить(ИнформацияПоСкладуОтправителю, "%СкладОтправитель", Реквизиты.СкладОтправительПредставление);
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Реквизиты.СкладПолучатель) Тогда
		ИнформацияПоСкладуПолучателю = НСтр("ru='Перемещение на ""%СкладПолучатель""'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ИнформацияПоСкладуПолучателю = СтрЗаменить(ИнформацияПоСкладуПолучателю, "%СкладПолучатель", Реквизиты.СкладПолучательПредставление);
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ИЗ
	|		Документ.ПеремещениеТоваров КАК ДанныеДокумента
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.Товары КАК ТоварыДокумента
...
Функция ТекстПередачаКомиссионныхТоваровНаОбособленноеПодразделение()
	...
	|		И Строки.РазделУчета = Стоимости.РазделУчета
	|ГДЕ
	|	Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВнутренняяПередачаТоваров)
...
Функция ТекстПередачаСобственныхТоваровСОбособленногоПодразделения()
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиДт,
	|
	// 4D:ERP для Беларуси, Дмитрий, 12.01.2016 12:56:38 
	// Локализация плана счетов
	// {
...
Функция ТекстПередачаСобственныхТоваровНаОбособленноеПодразделение()
	...
	|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиКт,
	|
	// 4D:ERP для Беларуси, Дмитрий, 12.01.2016 12:57:36 
	// Локализация плана счетов
	// {
...
Функция ТекстПеремещениеПродукцииДавальцевЗабаланс()

	ТекстПеремещениеПродукцииДавальцевЗабаланс = "
	|ВЫБРАТЬ //// Перемещение давальческой продукции (Дт 002 :: Кт 002)
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	Возврат ТекстПеремещениеПродукцииДавальцевЗабаланс;
...
Функция СформироватьПечатнуюФормуНакладнаяБСО(МассивОбъектов, ОбъектыПечати, КомплектыПечати)
	...
	|	ПРЕДСТАВЛЕНИЕ(ПеремещениеТоваров.ОрганизацияПолучатель.НаименованиеПолное) КАК ОрганизацияПолучательНаименованиеПолное,
	|	ПеремещениеТоваров.СкладПолучатель КАК СкладПолучатель,
	|	ПРЕДСТАВЛЕНИЕ(ПеремещениеТоваров.СкладПолучатель.Подразделение) КАК СкладПолучательПодразделение,
	|	ПеремещениеТоваров.СкладПолучатель.ТекущаяДолжностьОтветственного КАК СкладПолучательДолжность,
	|	ПеремещениеТоваров.СкладПолучатель.ТекущийОтветственный КАК СкладПолучательОтветственный,
	|	ПеремещениеТоваров.СкладОтправитель КАК СкладОтправитель,
	|	ПРЕДСТАВЛЕНИЕ(ПеремещениеТоваров.СкладОтправитель.Подразделение) КАК СкладОтправительПодразделение,
	|	ПеремещениеТоваров.СкладОтправитель.ТекущаяДолжностьОтветственного КАК СкладОтправительДолжность,
	|	ПеремещениеТоваров.СкладОтправитель.ТекущийОтветственный КАК СкладОтправительОтветственный
	|ИЗ
	...
	|	ТранспортнаяНакладнаяДокументыОснования.Ссылка.ДоверенностьНомер КАК ДоверенностьНомер,
	|	ТранспортнаяНакладнаяДокументыОснования.Ссылка.ДоверенностьЛицо КАК ДоверенностьЛицо,
	|	ТранспортнаяНакладнаяДокументыОснования.Ссылка.ДоверенностьДата КАК ДоверенностьДата,
	|	ТранспортнаяНакладнаяДокументыОснования.Ссылка.ДоверенностьВыдана КАК ДоверенностьВыдана
	|ИЗ
	...
	Если Результат[0].Пустой() Или Результат[1].Пустой() Или Результат[2].Пустой() Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	ТабДокумент = Новый ТабличныйДокумент;
	ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПеремещениеТоваров_НакладнаяБСО";
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПеремещениеТоваров.ПФ_MXL_НакладнаяБСО");
	
	ПервыйДокумент = Истина;
	
	Шапка = Результат[0].Выбрать();
	ВыборкаНоменклатура = Результат[1].Выбрать();
	ТранспортнаяНакладная = Результат[2].Выбрать();
	...
	Пока Шапка.Следующий() Цикл
		...
		ТранспортнаяНакладная.Сбросить();
		СтуктураПоиска = Новый Структура("ДокументОснование", Шапка.Ссылка);
		Пока ТранспортнаяНакладная.НайтиСледующий(СтуктураПоиска) Цикл
			ОбластьМакетаШапка.Параметры.ЧерезКого = ТранспортнаяНакладная.ДоверенностьЛицо;
			Если ЗначениеЗаполнено(ТранспортнаяНакладная.ДоверенностьНомер) И ЗначениеЗаполнено(ТранспортнаяНакладная.ДоверенностьДата) Тогда
				ОбластьМакетаШапка.Параметры.ДоверенностьНомер = ТранспортнаяНакладная.ДоверенностьНомер + " от " + Формат(ТранспортнаяНакладная.ДоверенностьДата, "ДФ=dd.MM.yyyy");
			КонецЕсли;
		КонецЦикла;
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Если ТекущийКомплект.Количество() > 0 Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			Обработки.ПечатьОбщихФорм.СформироватьПечатнуюФормуМХ1(ТекущаяСтруктураТипов, ОбъектыПечати, ПараметрыПечати, ТекущийКомплект));
	КонецЕсли;
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	ПродажиСервер.ПоместитьВременнуюТаблицуКоэффициентыУпаковок(МенеджерВременныхТаблиц, "ПеремещениеТоваровТаблицаТоваров");
	...
	|	ПеремещениеТоваров.Номер КАК Номер,
	|	ПеремещениеТоваров.Дата КАК Дата,
	|	ПеремещениеТоваров.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	ПеремещениеТоваров.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	ПеремещениеТоваров.Отпустил КАК Отпустил,
	|	ПеремещениеТоваров.ОтпустилДолжность КАК ОтпустилДолжность,
	|	ПеремещениеТоваров.Руководитель.Наименование КАК Разрешил,
	|	ПеремещениеТоваров.Руководитель.Должность КАК РазрешилДолжность,
	|	ПеремещениеТоваров.Принял КАК Принял,
	|	ПеремещениеТоваров.ПереданыДокументы КАК ПереданыДокументы,
	|	ПеремещениеТоваров.Организация КАК Организация,
	...
	|	ПеремещениеТоваров.ОснованиеДата КАК ОснованиеДата,
	|	ПеремещениеТоваров.ОснованиеНомер КАК ОснованиеНомер,
	|	ПеремещениеТоваров.АдресДоставки КАК АдресДоставки,
	|	&ВалютаРеглУчета КАК Валюта,
	|	ПеремещениеТоваров.ДоверенностьНомер КАК ДоверенностьНомер,
	|	ПеремещениеТоваров.ДоверенностьДата КАК ДоверенностьДата,
	|	ПеремещениеТоваров.ДоверенностьВыдана КАК ДоверенностьВыдана,
	|	ПеремещениеТоваров.ДоверенностьЛицо КАК ДоверенностьЛицо,
	...
	|	ВЫРАЗИТЬ(ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) КАК ЧИСЛО(15, 2)) КАК Цена,
	|	ВЫБОР
	...
	|	КОНЕЦ * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) * ПеремещениеТоваров.КоличествоУпаковок КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА НЕ &ТекстЗапросаКоэффициентУпаковки2 ЕСТЬ NULL
	|				И &ТекстЗапросаКоэффициентУпаковки2 <> 0
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) / &ТекстЗапросаКоэффициентУпаковки2
	|		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0)
	|	КОНЕЦ * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) * ПеремещениеТоваров.КоличествоУпаковок КАК СуммаСНДС,
	|	0 КАК СуммаНДС
	|ИЗ
	...
	|ИТОГИ ПО
	|	Ссылка";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки1", Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ПеремещениеТоваров.Упаковка", "ПеремещениеТоваров.Номенклатура"));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаКоэффициентУпаковки2", Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки("ТаблицаПериодовЦен.Упаковка", "ТаблицаПериодовЦен.Номенклатура"));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаНаименованиеЕдиницыИзмерения",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения("Наименование", "ПеремещениеТоваров.Упаковка", "ПеремещениеТоваров.Номенклатура"));
...
Функция СформироватьПечатнуюФормуТОРГ13(МассивОбъектов, ОбъектыПечати, КомплектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		// 4D:ERP для Беларуси, Евгений, 27.01.2017 14:13:42 
        // Исправление ошибки печати накладной на внутреннее перемещение в документе "Перемещение товаров", №14010
        // {
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НужноПересчитатьСтатусУказанияСерий Тогда
			ПараметрыУказанияСерий = Новый ФиксированнаяСтруктура(НоменклатураСервер.ПараметрыУказанияСерий(ДокОбъект, Документы.ПеремещениеТоваров)); 
			НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ДокОбъект, ПараметрыУказанияСерий);
		КонецЕсли;
			
		Попытка
			Если ИзмененаАналитика Или Выборка.НужноПересчитатьСтатусУказанияСерий Тогда
