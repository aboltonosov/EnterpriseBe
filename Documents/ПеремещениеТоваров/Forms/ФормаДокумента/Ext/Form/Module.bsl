...
Процедура НачальныйНомерБСОПриИзменении(Элемент)
	УчетБланковСтрогойОтчетностиКлиент.ПересчитатьНачальныйКонечныйНомерПриИзменении(Элементы, "Товары", "НачальныйНомерБСО", "КонечныйНомерБСО", "КоличествоУпаковок", "НачальныйНомерБСОПриИзменении", Ложь);
КонецПроцедуры

&НаКлиенте
Процедура КонечныйНомерБСОПриИзменении(Элемент)
	УчетБланковСтрогойОтчетностиКлиент.ПересчитатьНачальныйКонечныйНомерПриИзменении(Элементы, "Товары", "НачальныйНомерБСО", "КонечныйНомерБСО", "КоличествоУпаковок", "КонечныйНомерБСОПриИзменении", Ложь);
КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// {
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТорговыеНадбавки") Тогда
		
		РозничныеПродажиВызовСервера_Локализация.ВставитьРеквизитВТабличнуюЧасть(ЭтаФорма, ЭтаФорма.Элементы, "Товары", "Цена", "Цена", "Цена");
		Элементы["ТоварыЦена"].УстановитьДействие("ПриИзменении", "Подключаемый_ТоварыЦенаПриИзменении");
		РозничныеПродажиВызовСервера_Локализация.ДополнитьРеквизитыРозницы(ЭтаФорма);
	...
	Элементы["ВидЦены"].УстановитьДействие("ПриИзменении", "Подключаемый_ВидЦеныПриИзменении");

	Если ЗначениеЗаполнено(Объект.СкладОтправитель) Тогда
		Если Объект.СкладОтправитель.ЭтоГруппа Тогда
			ЭтаФорма.ЭтоРозничныйСклад = РозничныеПродажи_Локализация.ПроверитьСклады(Объект.СкладОтправитель);
		Иначе
		    ЭтаФорма.ЭтоРозничныйСклад = Объект.СкладПолучатель.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин;
		КонецЕсли;   		
	КонецЕсли; 
	
	Если ЗначениеЗаполнено(Объект.СкладОтправитель) И НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ЭтаФорма.Объект.ВидЦены = Объект.СкладОтправитель.УчетныйВидЦены;	
	КонецЕсли; 	
	
	ЭтаФорма["РассчитыватьРеквизитыРозницы"] = ПолучитьФункциональнуюОпцию("ИспользоватьТорговыеНадбавки");
	ЭтаФорма["РозничныйВидЦен"] = Объект.СкладПолучатель.РозничныйВидЦены;
	
	РозничныеПродажиВызовСервера_Локализация.УстановитьНастройкиВидимостиРеквизитовКомандДляРозницы(ЭтаФорма, "Перемещение");
	// }
	// 4D
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	// Розничная торговля, №15729
	// {
	Если ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ФормаСтавкиВРознице" Тогда	
		Если ВыбранноеЗначение <> Неопределено Тогда
			Если ВыбранноеЗначение.СтавкаТН = 0 Тогда
				Если ВыбранноеЗначение.РозничнаяСтавкаНДС <> Неопределено Тогда
					Если ВыбранноеЗначение.ВыделенныеСтроки Тогда
						НазначитьСтавкуРозничногоНДСНаСервере(ВыбранноеЗначение.РозничнаяСтавкаНДС, Элементы.Товары.ВыделенныеСтроки);
					Иначе
					    НазначитьСтавкуРозничногоНДСНаСервере(ВыбранноеЗначение.РозничнаяСтавкаНДС);
					КонецЕсли; 					
					РозничныеПродажиКлиент_Локализация.ОповеститьОбОкончанииНазначенияРозничныхСтавокНДС(ВыбранноеЗначение.РозничнаяСтавкаНДС);
				КонецЕсли;
			Иначе
				Если ВыбранноеЗначение.ВыделенныеСтроки Тогда
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	Если ЭтаФорма.РассчитыватьРеквизитыРозницы И ЭтаФорма.ЭтоРозничныйСклад Тогда
		СтруктураДействий = Новый Структура;
		СтруктураДействий.Вставить("ЗаполнитьЦенуПродажи", ПолучитьСтруктуруЗаполненияЦеныРозницаВСтрокеТЧ());
...
Процедура СкладОтправительПриИзмененииСервер(ОтвязатьВсеСтрокиОтЗаказа, ОчищатьСтроки, КэшированныеЗначения)
	...
	// Розничная торговля, №15729
	// {
	Если НЕ ЗначениеЗаполнено(Объект.СкладПолучатель) ИЛИ Объект.СкладПолучатель.ТипСклада = Перечисления.ТипыСкладов.ОптовыйСклад Тогда
		Объект.ВидЦены = Объект.СкладОтправитель.УчетныйВидЦены;
	ИначеЕсли Объект.СкладПолучатель.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин Тогда
		Объект.ВидЦены = Объект.СкладОтправитель.РозничныйВидЦены;
	КонецЕсли; 

	ЭтаФорма.ЭтоРозничныйСклад = (Объект.СкладОтправитель.ТипСклада = Перечисления.ТипыСкладов.ОптовыйСклад 
		И  Объект.СкладПолучатель.ТипСклада = Перечисления.ТипыСкладов.РозничныйМагазин);
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	// {{ Локализация_УправлениеТорговлейДляБеларуси
	РозничныеПродажиКлиентСервер_Локализация.ОбработкаВыбораПодборНаСервере(ЭтаФорма, Объект, СтруктураДействий); 
	// Локализация_УправлениеТорговлейДляБеларуси }}
...
Функция ТоварыНакладной()
	...
	ТипыРаспоряжений = Новый ОписаниеТипов("ДокументСсылка.ЗаказНаПеремещение, ДокументСсылка.ПеремещениеТоваров");
	
	Таблица.Колонки.Добавить("РаспоряжениеПриходногоОрдера", ТипыРаспоряжений);
	...
	Для Каждого Строка Из Объект.Товары Цикл
		...
		НоваяСтрока.РаспоряжениеПриходногоОрдера = ?(НакладнаяЯвляетсяРаспоряжением Или Не Объект.ПеремещениеПоЗаказам,
			Объект.Ссылка,
			НоваяСтрока.ЗаказНаПеремещение);
		
		НоваяСтрока.ЗаказНаПеремещение = ?(Объект.ПеремещениеПоЗаказам, НоваяСтрока.ЗаказНаПеремещение, Объект.Ссылка);
...
Процедура ПерезаполнитьПоОрдерам(МассивЗаказов)
	...
	ПараметрыЗаполнения.ХозяйственнаяОперация = Объект.ХозяйственнаяОперация;
	
	ТаблицаНакладная = Документы.ПеремещениеТоваров.ДанныеТаблицыТоварыДокумента(Объект.Ссылка);
	
	Документы.ПеремещениеТоваров.ЗаполнитьПоЗаказамОрдерам(ТаблицаНакладная, Объект.Ссылка, МассивЗаказов, ПараметрыЗаполнения);
...
// {
&НаКлиенте
Процедура Подключаемый_ТоварыРозничнаяЦенаСУчетомОкругленияПриИзменении(Элемент) 	
	
	ТекущаяСтрока = Элементы.Товары.ТекущиеДанные;
		
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("РассчитатьСтавкуСуммуТорговойНадбавки",Новый Структура("ИмяДокумента","Перемещение"));
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
КонецПроцедуры 

&НаКлиенте
Процедура Подключаемый_ТоварыЦенаПриИзменении(Элемент)  
	
	РозничныеПродажиКлиент_Локализация.РассчитатьРозничнуюЦену("Перемещение", Элементы, ЭтаФорма, Объект, КэшированныеЗначения, РозничныеПродажиВызовСервера_Локализация.ПолучитьЦенаВключаетНДС(Объект.ВидЦены)); 
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТоварыСтавкаТНПриИзменении(Элемент) 
	
	РозничныеПродажиКлиент_Локализация.РассчитатьРозничнуюЦену("Перемещение", Элементы, ЭтаФорма, Объект, КэшированныеЗначения); 
		
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ТоварыРозничнаяСтавкаНДСПриИзменении(Элемент) 
...
&НаКлиенте
Процедура Подключаемый_ВидЦеныПриИзменении(Элемент) 
...
&НаКлиенте
Функция ПолучитьСтруктуруЗаполненияЦеныРозницаВСтрокеТЧ()
...
&НаКлиенте
Процедура ЗаполнитьСтавкуРозничногоНДС(Команда) 
	
	Если Не РозничныеПродажиКлиент_Локализация.ВозможноНазначениеТорговойНадбавки(Объект, "Товары", НСтр("ru='Товары'")) Тогда
		Возврат;
	КонецЕсли;
	
	РозничныеПродажиКлиент_Локализация.ОткрытьФормуФормаСтавкиВРознице(ЭтаФорма, Ложь, Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьСтавкуРозничногоНДСДляВыделенныхСтрок(Команда)
	...
	Если Элементы.Товары.ВыделенныеСтроки <> Неопределено  Тогда
		...
		Если СтрокиТабличнойЧасти.Количество() = 0 Тогда
			ТекстПредупреждения = НСтр("ru='Ставка розничного НДС не может быть назначена.'");
			Предупреждение(ТекстПредупреждения);
			Возврат;
		КонецЕсли;
	...
	РозничныеПродажиКлиент_Локализация.ОткрытьФормуФормаСтавкиВРознице(ЭтаФорма, Ложь, Истина);

КонецПроцедуры

&НаСервере
Процедура НазначитьСтавкуРозничногоНДСНаСервере(СтавкаРозничногоНДС, Знач ВыделенныеСтроки = Неопределено) 

	РозничныеПродажиВызовСервера_Локализация.НазначитьРозничныеСтавки(Объект, "Товары", СтавкаРозничногоНДС, "РозничнаяСтавкаНДС","Перемещение",ЭтаФорма["РозничныйВидЦен"], ВыделенныеСтроки);
		
КонецПроцедуры

&НаСервере
Процедура НазначитьСтавкуТорговойНадбавкиНаСервере(СтавкаТорговойНадбавки, Знач ВыделенныеСтроки = Неопределено)//1С-Минск

	РозничныеПродажиВызовСервера_Локализация.НазначитьРозничныеСтавки(Объект, "Товары", СтавкаТорговойНадбавки, "СтавкаТН", "Перемещение", ЭтаФорма["РозничныйВидЦен"], ВыделенныеСтроки);
