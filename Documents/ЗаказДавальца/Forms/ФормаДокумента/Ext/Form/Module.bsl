	...
	Если Не НоваяСтрока Тогда
		...
		Если Не КешСтроки = Неопределено 
			...
			Если СпецификацииКУдалению.Количество() Тогда
				...
				ПоказатьВопрос(ОписаниеОповещения, 
					НСтр("ru='Изменилась основаная спецификация. В табличной части ""Сырье и материалы для производства"" будут удалены строки 
						|по спецификации " + СпецификацииКУдалению[0] + ". Продолжить?'"),
					РежимДиалогаВопрос.ДаНет);
...
Процедура ПродукцияЗаполнитьОсновнойСпецификациейЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ОчиститьМатериалыПоСпецификациям(ДополнительныеПараметры.СпецификацииКУдалению);
	ОбновитьСписокВыбораСпецификаций(Элементы, Объект.Продукция);
	РассчитатьИтоговыеПоказателиЗаказа(ЭтаФорма);
...
Процедура ПродукцияПередУдалениемЗавершение(Результат, Параметры) Экспорт
	...
	ОчиститьМатериалыПоСпецификациям(Параметры.СпецификацииКУдалению);
...
Процедура ОбновитьСписокВыбораСпецификаций(Элементы, Продукция)
	...
	Для Каждого СтрокаПродукции из Продукция Цикл
		...
		Спецификации.Добавить(СтрокаПродукции.Спецификация,
			?(Не ЗначениеЗаполнено(СтрокаПродукции.Спецификация), "<Не заполнена>", ""));
...
//
&НаКлиенте
Процедура ОчиститьМатериалыПоСпецификациям(Спецификации)
	
	СтруктураПоиска = Новый Структура;
	СтруктураПоиска.Вставить("Спецификация");
	Для Каждого СпецификацияКУдалению Из Спецификации Цикл
		
		СтруктураПоиска.Спецификация = СпецификацияКУдалению;
		СтрокиКУдалению = Объект.Материалы.НайтиСтроки(СтруктураПоиска);
		Для Каждого СтрокаКУдалению Из СтрокиКУдалению Цикл
			Объект.Материалы.Удалить(СтрокаКУдалению);
		КонецЦикла;
...
Процедура РассчитатьИтоговыеПоказателиЗаказа(Форма)
	...
	Если Не Форма.Объект.НалогообложениеНДС = ПродажаОблагаетсяНДС Тогда
...
Процедура УстановитьДоступностьКомандыЗаполнитьПоФактическойПотребности()
	...
	Если ВариантОбособления = Перечисления.ВариантыОбособленияМатериаловВПроизводстве.ЗаказНаПроизводство
		...
		Элементы.МатериалыПерезаполнитьПоПотребностямПроизводтсва.Видимость = Истина;
	Иначе
		
		Элементы.МатериалыПерезаполнитьПоПотребностямПроизводтсва.Видимость = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьКомандВводаНаОсновании()
	...
	ВводНаОсновании.УстановитьВидимостьЭлементаФормыСервер(ЭтотОбъект, "Документ.ЗаказНаПроизводство2.2", Объект.УправлениеПроизводством2_2);
	ВводНаОсновании.УстановитьВидимостьЭлементаФормыСервер(ЭтотОбъект, "Документ.ЗаказНаПроизводство", Не Объект.УправлениеПроизводством2_2);
...
Процедура ОбновитьНазначения()
	...
	Для Каждого Строка Из Объект.Материалы Цикл
		...
		Если Шаблон = Неопределено Или Не Справочники.Назначения.ШаблонЕстьЧастныйСлучайШаблона(Шаблон, ШаблонНазначенияМатериалы)
			Или Шаблон.Договор <> ШаблонНазначенияМатериалы.Договор
			Или Шаблон.Партнер <> ШаблонНазначенияМатериалы.Партнер Тогда
	...
	ПредставлениеНазначения = Справочники.Назначения.ПредставлениеНазначенияДокумента(ШаблонНазначенияМатериалы, Объект);
	УсловноеОформлениеНазначениеСырьеИМатериалыДляПроизводства();
...
Процедура МатериалыПерезаполнитьПоПотребностямПроизводстваНаСервере(КэшированныеЗначения)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru = 'В заполнении материалов по фактической потребности производства нет необходимости.'")
		  + Символы.ПС + НСтр("ru = 'Отклонений фактической потребности в материалах от заказанного количества не обнаружено.'");
