...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(ХозяйственнаяОперация, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 21.04.2016 11:00:37 
...
Процедура СформироватьЗаписиКнигиПродаж(Ссылка, Период, Организация) Экспорт
	...
	ОтражениеДокументовВРеглУчете.Отбор.Регистратор.Установить(Ссылка);
	...
	Хозрасчетный.Отбор.Регистратор.Установить(Ссылка);
	...
	РеглУчетПроведениеСервер.ДобавитьПараметрыВыборочнойРегистрацииКОтражениюВРеглУчете(
		ДополнительныеСвойства, Организация, Период);
		
	Документ = Новый Структура("Ссылка, Движения", Ссылка, Движения);
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРеализацияБезПереходаПраваСобственности()
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстСебестоимостьОтгруженногоТовара()
	...
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССОтгрузкиБезПереходаПраваСобственности()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССРеализацииПриПереходеПраваСобственности()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССОтгрузкиБезПереходаПраваСобственностиРаботыУслуги()
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстНДССРеализацииПриПереходеПраваСобственностиРаботыУслуги()
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|					ТОГДА 20 / 120
	|				КОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25)
	|					ТОГДА 0.25
	|				КОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25_125)
	|					ТОГДА 25 / 125
	// }
	// 4D
	|
...
#Область РеализацияБезПереходаПраваСобственности // (Дт 45.01 :: Кт 41.01)	
	ТекстРеализацияБезПереходаПраваСобственности = "
...
#Область РеализацияБезПереходаПраваСобственностиСкладПроизводства // (Дт 45.01 :: Кт 20)
	ТекстРеализацияБезПереходаПраваСобственностиСкладПроизводства = "
...
#КонецОбласти
	...
	Возврат ТекстРеализацияБезПереходаПраваСобственности
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстРеализацияБезПереходаПраваСобственностиСкладПроизводства;
...
#Область ЗалоговаяСтоимостьПереданнойТарыСкладПроизводства // (Дт 76 :: Кт 20)
	ТекстЗалоговаяСтоимостьПереданнойТарыСкладПроизводства = "
...
#КонецОбласти
	...
	Возврат ТекстЗалоговаяСтоимостьПереданнойТары
	+ " ОБЪЕДИНИТЬ ВСЕ " + ТекстЗалоговаяСтоимостьПереданнойТарыСкладПроизводства;
...
Функция ТекстНДССРеализацииПриПереходеПраваСобственности()
	...
	|ВЫБРАТЬ //// НДС с релизации при переходе права собственности (Дт 90.03 :: Кт 76.ОТ)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.ДатаПереходаПраваСобственности КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС с релизации при переходе права собственности"" КАК Содержание
	|
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
	|	
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСоздатьНаОсновании) Экспорт
	...
	// ++ НЕ УТ
	Документы.ТаможеннаяДекларацияЭкспорт.ДобавитьКомандуСоздатьНаОсновании(КомандыСоздатьНаОсновании);
	// -- НЕ УТ
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 03.05.2015 20:31:11 
	// Локализация печатных форм, №7762
	// {
	ИмяМакета = "ТН_Вертикальная";
	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
	Если ТекущийКомплект.Количество() > 0 Тогда
		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			Обработки.ПечатьТранспортнойНакладной.СформироватьПечатнуюФормуТН2(ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура("ВыводитьУслуги, ВыводитьГТД", Истина, Истина), ТекущийКомплект, Ложь, Ложь));
	КонецЕсли;
	
	ИмяМакета = "ТН_Горизонтальная";
	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
	Если ТекущийКомплект.Количество() > 0 Тогда
		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			Обработки.ПечатьТранспортнойНакладной.СформироватьПечатнуюФормуТН2(ТекущаяСтруктураТипов, ОбъектыПечати, Новый Структура("ВыводитьУслуги, ВыводитьГТД", Истина, Истина), ТекущийКомплект, Ложь, Истина));
	КонецЕсли;
	
	ИмяМакета = "ТТН_Вертикальная";
	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
	Если ТекущийКомплект.Количество() > 0 Тогда
		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ИмяМакета,
			ТекущийКомплект[0].Представление,
			Обработки.ПечатьТТН.СформироватьПечатнуюФормуТТН_Локализация(ТекущийМассивОбъектов, ОбъектыПечати, ТекущийКомплект));
	КонецЕсли;

	ИмяМакета = "ТТН_Горизонтальная";
	ТекущийКомплект = КомплектПечатныхФорм.Скопировать(Новый Структура("Имя,Печатать,",ИмяМакета,Истина));
	Если ТекущийКомплект.Количество() > 0 Тогда
		Если ТекущийКомплект.Колонки.Найти("Ссылка") <> Неопределено И ЗначениеЗаполнено(ТекущийКомплект[0].Ссылка) Тогда
			РегистрыСведений.НастройкиПечатиОбъектов.СкопироватьПечатнуюФормуВКоллекциюОдинЭкземпляр(КоллекцияПечатныхФорм, ИмяМакета);
...
Функция КомплектПечатныхФорм() Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 03.05.2015 20:20:05 
	// Локализация печатных форм, №7762
	// {
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТН_Вертикальная", НСтр("ru = 'ТН-2 (вертикальная)'"),                     0);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТН_Горизонтальная", НСтр("ru = 'ТН-2 (горизонтальная)'"),                 0);
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТТН") Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТТН_Вертикальная", НСтр("ru = 'ТТН-1 (вертикальная)'"),               0);
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТТН_Горизонтальная", НСтр("ru = 'ТТН-1 (горизонтальная)'"),           0);
	КонецЕсли;
	// }
	// 4D	
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	// Локализация печатных форм, №7762
	// {
	|	РеализацияТоваровУслуг.НомерИсходящегоДокумента КАК НомерИсходящегоДокумента,
	|	РеализацияТоваровУслуг.СерияИсходящегоДокумента КАК СерияИсходящегоДокумента,
	|	РеализацияТоваровУслуг.Отпустил КАК Отпустил,
	|	РеализацияТоваровУслуг.ОтпустилДолжность КАК ОтпустилДолжность,
	|	РеализацияТоваровУслуг.Руководитель.Наименование КАК Разрешил,
	|	РеализацияТоваровУслуг.Руководитель.Должность КАК РазрешилДолжность,
	|	РеализацияТоваровУслуг.Принял КАК Принял,
	|	РеализацияТоваровУслуг.ПереданыДокументы КАК ПереданыДокументы,
	// }
	// 4D
	|
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	Иначе
		...
		// 4D:ERP для Беларуси, Екатерина, 19.04.2016 15:59:20 
		// Расчет скидки к цене, №10910
		// {
		ТоварыРеализации.Колонки.Добавить("ЦенаСоСкидкой", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15, 2)));
		// }
		// 4D
	...
	Запрос.Текст =
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 19.04.2016 15:58:50 
		// Расчет скидки к цене, №10910
		// {
		|	ТоварыРеализации.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 19.04.2016 15:55:55 
		// Расчет скидки к цене, №10910
		// {
		...
		|	ЗаказТовары.Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 19.04.2016 15:56:46 
		// Расчет скидки к цене, №10910
		// {
		|	ЗаказТовары.ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|	ТоварыРеализации.Цена КАК Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 19.04.2016 15:57:58 
		// , №1090
		// {
		|	ТоварыРеализации.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
		// }
