...
Функция СборДанныхДляРегистрацииПерерасчетов(Регистратор, ДанныеДляРегистрацииПерерасчетов, Организация, СборДанныхДляРегистрации = Ложь, ИсключатьНеИзмененные = Ложь, РегистрацияНачалаИлиОкончанияУчета = Ложь, РегистрацияПерерасчетаУдержаний = Ложь) Экспорт
	...
	Если РегистрацияНачалаИлиОкончанияУчета Тогда
		...
		ЗапросПоДокументамНачисленияЗарплаты.Текст =
			...
			|			И НачислениеЗарплаты.МесяцНачисления >= ДанныеДокументов.ПериодДействия
			|ГДЕ
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Начисления.Сотрудник = ДанныеДокументов.Сотрудник", "И Начисления.Регистратор В (&НачисленияЗарплаты)");
...
Функция ТребуетсяПерерасчетПериода(Организация, Месяц, Сотрудники = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ПерерасчетУдержаний.Организация = ТекущиеКадровыеДанныеСотрудников.ТекущаяОрганизация
		|			И ПерерасчетУдержаний.ФизическоеЛицо = ТекущиеКадровыеДанныеСотрудников.ФизическоеЛицо
		|			И (ТекущиеКадровыеДанныеСотрудников.ДатаПриема <= КОНЕЦПЕРИОДА(&Месяц, МЕСЯЦ))
		|			И (ТекущиеКадровыеДанныеСотрудников.ДатаУвольнения >= &Месяц
...
Функция КраткоеПредставлениеДокумента(СсылкаНаДокумент) Экспорт
	...
	КраткоеПредставление = КраткоеПредставление + " " + НСтр("ru='от'") + " " + Формат(РеквизитыДокумента.Дата, "ДФ=dd.MM.yy");
...
Функция ПерерассчитыватьПособия(Организация)
...
Процедура СоздатьВТКонтролируемыеНачисления(МенеджерВременныхТаблиц, Организация = Неопределено, ИмяВТКонтролируемыеНачисления = "ВТКонтролируемыеНачисления")
...
Функция ЗапросВТПериодыРасчетаСотрудников(Регистратор, УдалитьПерерасчетыТекущегоПериода, ИмяВТПериодыРасчетаСотрудников = "")
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ЛьготыСотрудников") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ЛьготыСотрудников");
		Модуль.ДополнитьЛьготамиЗапросВТПериодыРасчетаСотрудников(ТекстыЗапросов);
	КонецЕсли;
...
Функция ПериодНачисленияЗарплатыЗакрыт(Организация, МесяцНачисления, ДокументОснование)
	...
	БылаВыплатаВПериоде = ИсправлениеДокументовЗарплатаКадры.ВыплатаПроизводилась(ДокументОснование, МесяцНачисления);
...
Процедура СоздатьВТПерерасчитываемыеСотрудники(МенеджерВременныхТаблиц, Организация, Подразделение, МесяцНачисления, СотрудникиПериодДействияПерерасчет, РежимДоначисления, Сотрудники = Неопределено, ИмяВТПерерасчитываемыеСотрудники = "ВТПерерасчитываемыеСотрудники")
	...
	Если Не РежимДоначисления Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ПерерасчетЗарплаты.ПериодДействия <= &ПериодДействия", "И ПерерасчетЗарплаты.ПериодДействия < &ПериодДействия");
	КонецЕсли;
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	ПерерасчитываемыеСотрудники.ВидРасчета,
			|	ПерерасчитываемыеСотрудники.ПериодДействия,
			|	ПерерасчитываемыеСотрудники.ПерерасчетНачислений,
			|	ПерерасчитываемыеСотрудники.ПерерасчетЛьгот,
			|	ПерерасчитываемыеСотрудники.ПерерасчетУдержаний
			|ПОМЕСТИТЬ ВТСотрудникиПериодДействияПерерасчет
			|ИЗ
			|	&СотрудникиПериодДействияПерерасчет КАК ПерерасчитываемыеСотрудники
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|ГДЕ
			|	НЕ СотрудникиПериодовПерерасчетов.ПерерасчетУдержаний
			|
			|СГРУППИРОВАТЬ ПО
			|	СотрудникиПериодовПерерасчетов.ПериодДействия,
			|	СотрудникиПериодовПерерасчетов.Организация,
	...
	ЗарплатаКадрыОбщиеНаборыДанных.ЗаменитьИмяСоздаваемойВременнойТаблицы(Запрос.Текст, "ВТПерерасчитываемыеСотрудники", ИмяВТПерерасчитываемыеСотрудники);
	
	Запрос.Выполнить();
	
КонецПроцедуры

Процедура СоздатьВТПерерасчитываемыеУдержанияСотрудников(МенеджерВременныхТаблиц, Организация, Подразделение, Сотрудники, СотрудникиПериодДействияПерерасчет, МесяцНачисления, РежимДоначисления)
	...
	Если СотрудникиПериодДействияПерерасчет <> Неопределено
		...
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
			|	СотрудникиПериодДействияПерерасчет.ПериодДействия,
			...
			|	СотрудникиПериодДействияПерерасчет.ДокументОснование, СотрудникиПериодДействияПерерасчет.ПерерасчетУдержаний
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТПерерасчитываемыеУдержанияСотрудников
		|ИЗ
		|	РегистрСведений.ПерерасчетУдержаний КАК ПерерасчетУдержаний
...
Процедура СоздатьВТСотрудникиОрганизацииКРасчету(МенеджерВременныхТаблиц, ПараметрыПолученияСотрудников, СотрудникиПериодДействияПерерасчет, РежимДоначисления, ИмяВТСотрудникиОрганизацииКПерерасчету) Экспорт
	...
	СоздатьВТПерерасчитываемыеСотрудники(
	...
	Если РежимДоначисления Тогда
		...
		Запрос.Текст =
			...
			|ИЗ
			|	ВТПерерасчитываемыеСотрудники КАК СотрудникиОрганизации
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВТПерерасчитываемыеСотрудники";
...
Процедура ЗаполнитьДанныеПерерасчета(ДанныеПерерасчета, Организация, Подразделение, МесяцНачисления, Сотрудники, СотрудникиПериодДействияПерерасчет, ТекущийРегистратор, РежимДоначисления, МенеджерРасчета) Экспорт
	...
	Запрос.Текст =
		...
		|	ПерерасчитываемыеСотрудники.Сотрудник,
		|	ПерерасчитываемыеСотрудники.ФизическоеЛицо,
		|	ПерерасчитываемыеСотрудники.ПериодДействия,
		|	ПерерасчитываемыеСотрудники.ПерерасчетНачислений,
		|	ПерерасчитываемыеСотрудники.ПерерасчетЛьгот,
		|	ПерерасчитываемыеСотрудники.ВидРасчета,
		...
		|	ПерерасчитываемыеРегистраторы.ФизическоеЛицо,
		|	ПерерасчитываемыеРегистраторы.ПериодДействия,
		|	ПерерасчитываемыеРегистраторы.Регистратор,
		|	ПерерасчитываемыеРегистраторы.ПерерасчетНачислений,
		|	ПерерасчитываемыеРегистраторы.ПерерасчетЛьгот,
		|	ПерерасчитываемыеРегистраторы.ВидРасчета
...
Процедура УстановитьОтборыПерерасчетовУдержаний(ДанныеПерерасчета, МенеджерРасчета, Организация, Подразделение, Сотрудники, СотрудникиПериодДействияПерерасчет, МесяцНачисления, РежимДоначисления) Экспорт
	...
	СоздатьВТПерерасчитываемыеУдержанияСотрудников(
		Запрос.МенеджерВременныхТаблиц,
		Организация,
		Подразделение,
		Сотрудники,
	...
	Запрос.Текст =
		...
		|ИЗ
		|	ВТПерерасчитываемыеУдержанияСотрудников КАК ПерерасчетУдержаний
		|
		|УПОРЯДОЧИТЬ ПО
		|	НачалоПериода,
		|	ФизическоеЛицо,
...
Процедура СвернутьНачисленияПерерасчет(НачисленияПерерасчет)
	...
	КлючТекущейЗаписи = Новый Структура("Сотрудник,Начисление,ДокументОснование,Подразделение,КодВычета,ДатаНачала,ДатаОкончания");
...
// 4D:ERP для Беларуси, Яна, 19.07.2017 14:49:32 
...
Функция ДанныеПерерасчетаНачисленияЗарплаты(Организация, МесяцНачисления, Сотрудники, ТекущийРегистратор, СотрудникиПериодДействияПерерасчет, РежимДоначисления) Экспорт
	...
	// Добавление сотрудников сторнируемых строк текущего документа
	Запрос.УстановитьПараметр("СотрудникиПериодДействияПерерасчет", СотрудникиПериодДействияПерерасчет);
	...
	Запрос.Текст = 
		...
		|			И ПерерасчитываемыеСотрудники.ПериодДействия = Начисления.ПериодДействия
		|ГДЕ
		|	ЕСТЬNULL(Начисления.Регистратор, ЗНАЧЕНИЕ(Документ.НачислениеЗарплаты.ПустаяСсылка)) ССЫЛКА Документ.НачислениеЗарплаты
		|	И (ЕСТЬNULL(Начисления.Регистратор, ЗНАЧЕНИЕ(Документ.НачислениеЗарплаты.ПустаяСсылка)) <> &Регистратор
		|			ИЛИ &Регистратор = ЗНАЧЕНИЕ(Документ.НачислениеЗарплаты.ПустаяСсылка))
		|	И НЕ ЕСТЬNULL(ВЫРАЗИТЬ(Начисления.ВидРасчета КАК ПланВидовРасчета.Начисления).КатегорияНачисленияИлиНеоплаченногоВремени, ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ПустаяСсылка)) В (&КатегорииНачисленийПособий)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|			И (ПерерасчитываемыеРегистраторыСторно.Сторно)
		|ГДЕ
		|	НЕ ПерерасчитываемыеРегистраторы.Сторно
		|	И ПерерасчитываемыеРегистраторыСторно.Сотрудник ЕСТЬ NULL ";
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	НачислениеЗарплатыНачисления.*
		|ПОМЕСТИТЬ ВТСторнируемыеНачисления
		|ИЗ
		|	Документ.НачислениеЗарплаты.НачисленияПерерасчет КАК НачислениеЗарплатыНачисления
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСторнируемыеРегистраторы КАК СторнируемыеРегистраторы
		|		ПО НачислениеЗарплатыНачисления.Ссылка = СторнируемыеРегистраторы.Регистратор
		|			И НачислениеЗарплатыНачисления.Сотрудник = СторнируемыеРегистраторы.Сотрудник
		|			И (НАЧАЛОПЕРИОДА(НачислениеЗарплатыНачисления.ДатаНачала, МЕСЯЦ) = СторнируемыеРегистраторы.ПериодДействия)
		|			И (НЕ ВЫРАЗИТЬ(НачислениеЗарплатыНачисления.Начисление КАК ПланВидовРасчета.Начисления).КатегорияНачисленияИлиНеоплаченногоВремени В (&КатегорииНачисленийПособий))
		|			И (НЕ НачислениеЗарплатыНачисления.Сторно)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Запрос.Текст =
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	СторнируемыеНачисления.ДатаНачала,
		|	СторнируемыеНачисления.Ссылка,
		|	СторнируемыеНачисления.НомерСтроки,
		|	СторнируемыеНачисления.Сотрудник";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	НачислениеЗарплатыРаспределениеРезультатовНачислений.*
			|ИЗ
			|	Документ.НачислениеЗарплаты.РаспределениеРезультатовНачислений КАК НачислениеЗарплатыРаспределениеРезультатовНачислений
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСторнируемыеНачисления КАК СторнируемыеНачисления
			|		ПО НачислениеЗарплатыРаспределениеРезультатовНачислений.Ссылка = СторнируемыеНачисления.Ссылка
			|			И НачислениеЗарплатыРаспределениеРезультатовНачислений.ИдентификаторСтроки = СторнируемыеНачисления.ИдентификаторСтрокиВидаРасчета";
		...
		ИменаКолонокТаблицыРаспределенияНачислений = Неопределено;
		...
		Пока Выборка.Следующий() Цикл
			...
			ПериодДействия = НачалоМесяца(Выборка.ДатаНачала);
			...
			НоваСтрокаСторноНачисленийПерерасчет.ФиксРасчет = Истина;
			НоваСтрокаСторноНачисленийПерерасчет.ФиксРасчетВремени = Истина;
			
			СтруктураПоиска = Новый Структура("Ссылка,ИдентификаторСтроки", Выборка.Ссылка, Выборка.ИдентификаторСтрокиВидаРасчета);
			СтрокиРаспределенияНачислений = ТаблицаРаспределенияНачислений.НайтиСтроки(СтруктураПоиска);
			Если СтрокиРаспределенияНачислений.Количество() > 0 Тогда
				
				Если ИменаКолонокТаблицыРаспределенияНачислений = Неопределено Тогда
				...
				Для каждого СтрокаРаспределенияНачислений Из СтрокиРаспределенияНачислений Цикл
					...
					ЗаполнитьЗначенияСвойств(СтруктураРаспределения, СтрокаРаспределенияНачислений);
				...
				НоваСтрокаСторноНачисленийПерерасчет.РезультатРаспределения = Новый ФиксированныйМассив(РезультатРаспределения);
				
			КонецЕсли; 
			
			НомерПоказателя = 1;
			ИнформацияОНачислении = ЗарплатаКадрыРасширенныйПовтИсп.ПолучитьИнформациюОВидеРасчета(НоваСтрокаСторноНачисленийПерерасчет.Начисление);
			Для каждого ОписаниеПоказателя Из ИнформацияОНачислении.Показатели Цикл
				
				СтруктураПоиска = Новый Структура("Ссылка,ИдентификаторСтроки,Показатель", Выборка.Ссылка, Выборка.ИдентификаторСтрокиВидаРасчета, ОписаниеПоказателя.Показатель);
				СтрокиПоказателей = ТаблицаПоказателей.НайтиСтроки(СтруктураПоиска);
				Если СтрокиПоказателей.Количество() > 0  Тогда
					
					НоваСтрокаСторноНачисленийПерерасчет["Показатель" + НомерПоказателя] = ОписаниеПоказателя.Показатель;
					НоваСтрокаСторноНачисленийПерерасчет["Значение" + НомерПоказателя] = СтрокиПоказателей[0].Значение;
