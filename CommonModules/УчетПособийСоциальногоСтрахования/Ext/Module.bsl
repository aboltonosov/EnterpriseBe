...
// 4D:ERP для Беларуси, Яна, 31.08.2017 12:13:19 
...
Функция ПредельнаяВеличинаБазыСтраховыхВзносовЛокализация(ПериодНачало,ПериодОкончание) Экспорт
	...
	СоздатьВТПредельнаяВеличинаБазыЛокализация(Запрос.МенеджерВременныхТаблиц, ПериодНачало,ПериодОкончание);
...
//		Годы - массив элементов типа число.
// 
Процедура СоздатьВТПредельнаяВеличинаБазыЛокализация(МенеджерВременныхТаблиц,ПериодНачало,ПериодОкончание) Экспорт
	...
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследнихЛокализация(
...
// 4D:ERP для Беларуси, Яна, 09.06.2017 16:29:04 
// Документ "Отпуск по уходу за ребенком", №14900 
// {
Функция ПредельнаяВеличинаБазыСтраховыхВзносовПериод(ПериодНачало,ПериодОкончание) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	СоздатьВТПредельнаяВеличинаБазыПериод(Запрос.МенеджерВременныхТаблиц, ПериодНачало,ПериодОкончание);
...
// 4D:ERP для Беларуси, Яна, 09.06.2017 16:26:43 
// Документ "Отпуск по уходу за ребенком", №14900 
// {
Процедура СоздатьВТПредельнаяВеличинаБазыПериод(МенеджерВременныхТаблиц,ПериодНачало,ПериодОкончание) Экспорт
...
Процедура РассчитатьПособиеПоНетрудоспособности(СтрокиРасчета, ДополнительныеПараметры, ПрименяетсяСторнирование = Истина) Экспорт
	...
	Если НазначитьПособие = Истина Тогда
		НазначитьПособиеЗаСчетФСС = ПараметрыОплаты.ПериодРегистрации < ПрямыеВыплатыПособийСоциальногоСтрахования.ДатаПередачиФССВыплатыПособий(ПараметрыОплаты.Организация);
	...
	Для Каждого СтрокаТаблицы Из СтрокиРасчета Цикл
		...
		Иначе
			...
			// 4D:ERP для Беларуси, Петр, 29.11.2017 15:24:03 
			// Документ "Больничный лист. Доработать заполнение процента оплаты, №15884
			// {
			ПроцентОплаты = СтрокаТаблицы.ПроцентОплаты;
			// }
			// 4D
		...
		Иначе
			...
			// 4D:ERP для Беларуси, Петр, 29.11.2017 15:25:41 
			// Документ "Больничный лист. Доработать заполнение процента оплаты, №15884
			// Неправильный расчет начислений по больничному листу, №17013
			// {
			ДневноеПособие = ПараметрыОплаты.СреднийДневнойЗаработок * ПроцентОплаты / 100;
...
Процедура ОбработатьНачисленияПослеРасчета(МенеджерВременныхТаблиц, Организация, ПериодРегистрации, СтрокиПоИдентификатору) Экспорт
	...
	// Для работодателей, участвующих в пилотном проекте, очищаем результат начислений за счет ФСС после расчета всех начислений.
...
Функция СреднийДневнойЗаработокФСС(ПараметрыРасчета) Экспорт 
	...
	Если ПараметрыРасчета.ПрименятьПредельнуюВеличину Тогда
		...
		// Документ "Отпуск по уходу за ребенком", №14900 
		// {		
		ПараметрыРасчета.ПредельныеВеличины = УчетПособийСоциальногоСтрахования.ПредельнаяВеличинаБазыСтраховыхВзносовПериод(ПараметрыРасчета.ПериодРасчетаСреднегоЗаработкаНачало,ПараметрыРасчета.ПериодРасчетаСреднегоЗаработкаОкончание);
		// }
		// 4D
	КонецЕсли;
...
Функция РеквизитыДокументаДляРасчетаСреднегоЗаработкаФСС() Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 01.03.2018 17:01:07 
	// Доработать печатные формы док-та "Больничный лист", № 17952
	// {
	РеквизитыДокумента.Вставить("РасчетПремии");
	// }
	// 4D
...
Функция ДанныеДокументаДляРасчетаСреднегоЗаработкаФСС(РеквизитыДокумента, ТаблицыДанныхОСреднем) Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 01.03.2018 16:35:33 
	// Доработать печатные формы док-та "Больничный лист", № 17952
	// {
	ДанныеДокумента.ДанныеРасчетаСреднего = УчетПособийСоциальногоСтрахованияКлиентСервер.ДанныеРасчетаСреднегоЗаработкаФСЗН(ДанныеДокумента.ПараметрыРасчета);
	// }
	// 4D
...
Функция ПараметрыРасчетаСреднегоДневногоЗаработкаФССПоДокументу(РеквизитыДокумента, ДанныеОНачислениях, ДанныеОВремени, ДанныеСтрахователей) Экспорт
	...
	// 4D:ERP для Беларуси, Петр, 01.03.2018 16:29:06 
	// Доработать печатные формы док-та "Больничный лист", № 17952
	// {
	ПараметрыРасчета.ПериодыРасчета								= ПериодыРасчетаСреднего(РеквизитыДокумента);
	ПараметрыРасчета.РасчетПремии   							= РеквизитыДокумента.РасчетПремии;
	// }
	// 4D
	
	Если ПараметрыРасчета.ПрименятьПредельнуюВеличину Тогда
		
		// 4D:ERP для Беларуси, Петр, 01.03.2018 16:28:33 
		// Доработать печатные формы док-та "Больничный лист", № 17952
		// {
		ПараметрыРасчета.ПредельныеВеличины = УчетПособийСоциальногоСтрахования.ПредельнаяВеличинаБазыСтраховыхВзносовЛокализация(
			РеквизитыДокумента.ПериодРасчетаСреднегоЗаработкаНачало, РеквизитыДокумента.ПериодРасчетаСреднегоЗаработкаОкончание);
		// }
		// 4D
...
// 4D:ERP для Беларуси, Петр, 01.03.2018 16:29:33 
// Доработать печатные формы док-та "Больничный лист", № 17952
// {
Функция ПериодыРасчетаСреднего(РеквизитыДокумента)
	
	Периоды = Новый Массив;
	Месяц = РеквизитыДокумента.ПериодРасчетаСреднегоЗаработкаНачало;
	
	Пока Месяц <= НачалоМесяца(РеквизитыДокумента.ПериодРасчетаСреднегоЗаработкаОкончание) Цикл
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.МаксимальныйРазмерЕжемесячнойСтраховойВыплаты) Тогда
		...
		Обработчик.Процедура = "РегистрыСведений.МаксимальныйРазмерЕжемесячнойСтраховойВыплаты.ЗаполнитьМаксимальныеРазмерыЕжемесячнойСтраховойВыплаты";
