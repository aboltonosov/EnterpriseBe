...
Процедура ЗаполнитьСписокВыбораКодПричиныПерерасчета(Элемент) Экспорт
	Элемент.СписокВыбора.Добавить("01", Нстр("ru = '1 - представлены дополнительные сведения о выплатах в расчетном периоде'"));
	Элемент.СписокВыбора.Добавить("02", Нстр("ru = '2 - поступление от застрахованного лица заявления о замене одного года или двух лет расчетного периода предшествующими годами'"));
	Элемент.СписокВыбора.Добавить("03", Нстр("ru = '3 - поступление заявления от застрахованного лица о применении норм законодательства при исчислении среднего заработка, действовавшего до 01.01.2011г.'"));
	Элемент.СписокВыбора.Добавить("04", Нстр("ru = '4 - по результатам документальной выездной (камеральной) проверки'"));
	Элемент.СписокВыбора.Добавить("05", Нстр("ru = '5 - перерасчет листка нетрудоспособности при признании несчастного случая на производстве страховым;'"));
	Элемент.СписокВыбора.Добавить("06", Нстр("ru = '6 - перерасчет листка нетрудоспособности в случае ошибки в расчете работодателя'"));
	Элемент.СписокВыбора.Добавить("07", Нстр("ru = '7 - иное'"));
КонецПроцедуры
...
Процедура УстановитьТекстИСсылкуЗаявленияНаВыплатуПособияПоОснованию(Форма, РеквизитСсылка = "Объект.Ссылка", ИмяРеквизитаГиперссылка = "ГиперСсылкаЗаявлениеНаВыплатуПособия", ИмяРеквизитаЗаявление = "ЗаявлениеНаВыплатуПособия") Экспорт
	...
	Основание = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, РеквизитСсылка);
	Если ЗначениеЗаполнено(Основание) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗаявлениеСотрудникаНаВыплатуПособия.Ссылка,
		|	ЗаявлениеСотрудникаНаВыплатуПособия.Дата,
		|	ЗаявлениеСотрудникаНаВыплатуПособия.Номер
		|ИЗ
		...
		|	И НЕ ЗаявлениеСотрудникаНаВыплатуПособия.ПометкаУдаления";
		...
		Если Не Результат.Пустой() Тогда
			...
			Форма[ИмяРеквизитаГиперссылка] = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаГиперСсылки, Выборка.Номер, Формат(Выборка.Дата, "ДЛФ=D"));
			Форма[ИмяРеквизитаЗаявление] = Выборка.Ссылка;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры	
