...
Процедура ПоказатьФормуПредложениеОформитьЗаявлениеНаПодключение(Организация = Неопределено, ПоказыватьКнопки = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 26.09.2017 21:00:31 
	// Регл. отчетность
	// {
...
Процедура ОткрытьФормуМастераЗаявленияНаПодключение(
		...
		ПараметрыОткрытияМастера	= Неопределено) Экспорт
		
	ПараметрыДлительногоОбмена = Новый Структура();
	ПараметрыДлительногоОбмена.Вставить("ЭтоОбновлениеИзМастера", Истина);
	
	ДлительнаяОтправкаКлиент.ОткрытьФормуОжиданияЗагрузкиМодуля(ПараметрыДлительногоОбмена); // обновление модуля
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО", 
		ЭтотОбъект, 
...
Процедура ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповестить("1С-Отчетность. Закрыть ручную форму заявления");
	...
	Иначе
		...
		Иначе
			УжеНастроенаУчетнаяЗаписьДокументооборота = Ложь;
		...
		ОткрытьВторичноеЗаявление = 
			...
			И УжеНастроенаУчетнаяЗаписьДокументооборота 
			И ПоддерживаетсяВторичноеЗаявление
			И НЕ ПринудительноОткрытьПервичноеЗаявление
			ИЛИ ПринудительноОткрытьВторичноеЗаявление; 
		
		ДополнительныеПараметры.Вставить("ОткрытьВторичноеЗаявление", ОткрытьВторичноеЗаявление);
		
		ДополнительныеПараметры.Вставить("КонтекстЭДОКлиент", КонтекстЭДОКлиент);
		ДлительнаяОтправкаКлиент.ЗапомнитьКонтекстДлительнойОперации(ДополнительныеПараметры);
		
		// Выполняется процедура ОткрытьОтложенноФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО
		ПодключитьОбработчикОжидания("Подключаемый_ОткрытьФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО", 0.1, Истина);
				
	КонецЕсли;
	
КонецПроцедуры

Процедура ОткрытьОтложенноФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДО() Экспорт
	
	ДополнительныеПараметры  = ДлительнаяОтправкаКлиент.КонтекстДлительнойОперации();
	
	ПараметрыОткрытияМастера = ДополнительныеПараметры.ПараметрыОткрытияМастера;
	Если ПараметрыОткрытияМастера = Неопределено Тогда
		ПараметрыФормы = Новый Структура();
	Иначе
		ПараметрыФормы = ПараметрыОткрытияМастера;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДополнительныеПараметры.Организация) Тогда
		ПараметрыФормы.Вставить("Организация", ДополнительныеПараметры.Организация);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ЗначениеКопирования) Тогда
		ПараметрыФормы.Вставить("Реквизит", ДополнительныеПараметры.ЗначениеКопирования);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ОткрытьВторичноеЗаявление Тогда
		ИмяФормыМастера = КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.МастерФормированияЗаявкиНаИзменениеПараметровПодключения";
	Иначе
		ИмяФормыМастера = КонтекстЭДОКлиент.ПутьКОбъекту + ".Форма.МастерФормированияЗаявкиНаПодключение";
	КонецЕсли;
...
Процедура ОткрытьОтложенноФормуМастераЗаявленияНаПодключениеПослеПолученияКонтекстаЭДОЗавершение(Результат, ВходящийКонтекст) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		
		// Зайдем в эту ветку, если заявление с первого раза не отправилось
		// и это вторая попытка создания заявления по кнопке "Подготовить новое заявление" 
		
		ПараметрыФормы = Результат;
		
		ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиент.ОткрытьФормуМастераЗаявленияНаПодключение(
			, 
			,
			,
			Результат.ВидЗаявления,
			ПараметрыФормы);
