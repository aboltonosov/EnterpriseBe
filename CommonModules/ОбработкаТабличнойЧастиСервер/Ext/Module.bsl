...
Процедура ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	// {
	КэшированныеЗначения.ПрименятьСкидкиКЦене = СкидкиНаценкиВызовСервера.ПолучитьСпособПримененияСкидки();
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Петр, 20.02.2018 17:08:47 
	// Механизм ввода остатков, № 17856
	// {
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуРеглЗаУпаковкуВСтрокеТЧ(							ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	// }
	// 4D
	...
	//-- НЕ УТКА
	// 4D:ERP для Беларуси, Екатерина, 12.02.2016 15:32:03 
	// Расчет скидки к цене, №10910
	// {
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуСУчетомСкидокВСтрокеТЧ(						ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	// }
	// 4D
	
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуВСтрокеТЧ(								ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
	// 4D:ERP для Беларуси, Петр, 19.02.2018 17:39:21 
	// Механизм ввода остатков, № 17856
	// {
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуРеглПоЦенеРеглВСтрокеТЧ(					ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуУпрПоСуммеРеглВСтрокеТЧ(					ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Екатерина, 12.04.2016 15:36:10 
	// Расчет скидки к цене, №10910
	// {
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьПроцентРучнойСкидкиВПродажахВСтрокеТЧ(			ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	// }
	// 4D
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьНДСРеглВСтрокеТЧ(								ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	
	// 4D:ERP для Беларуси, Петр, 19.02.2018 16:14:58 
	// Механизм ввода остатков, № 17856
	// {
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуНДСРеглВСтрокеТЧ(							ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьСуммуРеглБезНДСВСтрокеТЧ(						ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ОбработкаТабличнойЧастиКлиентСервер.ПересчитатьЦенуРеглПоСуммеРеглВСтрокеТЧ(					ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	// }
	// 4D
...
//-- НЕ УТКА
	...
	// {
	РозничныеПродажи_Локализация.ЗаполнитьСтавкуТорговойНадбавкиВСтрокеТЧ(ТекущаяСтрока,СтруктураДействий, КэшированныеЗначения);
	РозничныеПродажи_Локализация.ЗаполнитьРозничнуюСтавкуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	РозничныеПродажи_Локализация.РассчитатьРозничнуюЦенуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий);
	РозничныеПродажи_Локализация.РассчитатьСтавкуСуммуТорговойНадбавкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий);	
	// }
	// 4D
...
Процедура ПроверитьЗаполнитьОбеспечениеВСтрокеТЧЭтапаПроизводства(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьОбеспечениеВЭтапеПроизводства", СтруктураПараметровДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ВариантОбеспечения = Неопределено;
	ДатаОтгрузки = СтруктураПараметровДействия.ЖелаемаяДатаОтгрузки;
	ДатаОтгрузки = ?(ДатаОтгрузки <> '00010101', ДатаОтгрузки, НачалоДня(ТекущаяДатаСеанса()));
	
	ПараметрыОбеспечения = ПланированиеПроизводства.ПараметрыОбеспеченияМатериалаЭтапа(
								ТекущаяСтрока.Номенклатура,
								ТекущаяСтрока.Характеристика,
								ТекущаяСтрока.Производится,
								СтруктураПараметровДействия.Подразделение,
								ТекущаяСтрока.Спецификация,
								СтруктураПараметровДействия.ЗаказНаПроизводство);
							
	ТекущаяСтрока.ВариантОбеспечения = ПараметрыОбеспечения.ВариантОбеспечения;
	ТекущаяСтрока[СтруктураПараметровДействия.ИмяПоляСклад] = ПараметрыОбеспечения.Склад;
...
Процедура ПроверитьЗаполнитьСкладПроизводстваВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	Если НЕ СтруктураДействий.Свойство("ПроверитьЗаполнитьСкладПроизводства", СтруктураПараметровДействия) Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыОбеспечения = ПланированиеПроизводства.ПараметрыОбеспеченияНоменклатуры(
								ТекущаяСтрока.Номенклатура,
								ТекущаяСтрока.Характеристика,
								СтруктураПараметровДействия.Подразделение);
...
Процедура ЗаполнитьСпособОпределенияСебестоимостиСтрокой(ТекущаяСтрока, СтруктураДействий) 
	
	Перем НалогообложениеНДС;
	
	Если СтруктураДействий.Свойство("ЗаполнитьСпособОпределенияСебестоимостиСтрокой", НалогообложениеНДС)
		И ТекущаяСтрока.Свойство("СпособОпределенияСебестоимостиСтрокой") Тогда
		
		Если ТекущаяСтрока.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзТекущегоДокумента Тогда
			Представление = НСтр("ru='Равна сумме в данной строке'");
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.ИзДокументаПродажи Тогда
			Представление = НСтр("ru='Определяется из документа %1%'");
			Представление = СтрЗаменить(Представление, "%1%", Строка(ТекущаяСтрока.ДокументРеализации));
		ИначеЕсли ТекущаяСтрока.СпособОпределенияСебестоимости = Перечисления.СпособыОпределенияСебестоимости.Вручную Тогда
			ИспользуетсяРегл = ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет");
			Если ИспользуетсяРегл Тогда
				Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС Тогда
			...
			Представление = СтрЗаменить(Представление, "%2%", Константы.ВалютаУправленческогоУчета.Получить());
			Представление = СтрЗаменить(Представление, "%3%", Формат(ТекущаяСтрока.СебестоимостьРегл, "ЧДЦ=2; ЧН=0"));
			Представление = СтрЗаменить(Представление, "%4%", Константы.ВалютаРегламентированногоУчета.Получить());
		КонецЕсли;
...
Процедура ЗаполнитьСпособОпределенияСебестоимости(ТекущаяСтрока, СтруктураДействий) 
