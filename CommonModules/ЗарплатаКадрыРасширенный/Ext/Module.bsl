...
Процедура ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны) Экспорт
	...
	Шаблоны.Добавить("РасчетПоложенныхПравНаОтпускаЗависящихОтСтажа");
...
Функция ОписаниеПрофиляКадровикБезДоступаКЗарплате() Экспорт
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ПодсистемаМотивация", 											"ЗарплатаКадрыКорпоративнаяПодсистемы.Грейды");
...
Функция ОписаниеПрофиляКадровик() Экспорт
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеЭД",													"ЭлектронноеВзаимодействие.ОбменСБанками");
...
Функция ОписаниеПрофиляРасчетчик() Экспорт
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ВыводНаПринтерФайлБуферОбмена");
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ВыполнениеОбменаЭД",											"ЭлектронноеВзаимодействие.ОбменСБанками");
...
Функция ОписаниеПрофиляСтаршийРасчетчик() Экспорт
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "НастройкаВзаиморасчетовССотрудниками");
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "НастройкаПараметровЭД",										"ЭлектронноеВзаимодействие.ОбменСБанками");
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеБухгалтерскогоУчетаЗарплатыРасширенная");
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеВерсийОбъектов",										"СтандартныеПодсистемы.ВерсионированиеОбъектов");
	// 4D:ERP для Беларуси, Яна, 17.05.2017 15:51:57 
	// Ошибка при открытии формы справочника "способы отражения зарплаты", №14904 
	// {
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ДобавлениеИзменениеКлассификаторовНастроекДоходовРасходов"); 
	// }
	// 4D
...
Функция ДействующиеНачисленияСотрудников(СотрудникиДаты, ДокументСсылка, ОписаниеТаблицыВидовРасчета = Неопределено, ФормироватьСПериодичностьДень = Истина, ИсключатьПособия = Ложь) Экспорт
	...
	Если ПолучениеНачислений Тогда
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ДОБАВИТЬКДАТЕ(ПлановыеНачисления.ПериодПредыдущейЗаписи, СЕКУНДА, -1) КАК Период,
	...
	Если ПолучениеНачислений  Тогда
		...
		ЧастиТекстаЗапроса.Добавить(
			...
			|		ПО ДействующиеПлановыеРасчеты.ПериодПредыдущейЗаписи = ДОБАВИТЬКДАТЕ(ПлановыеНачисленияВозвратные.Период, СЕКУНДА, 1)
...
Процедура РедактированиеСоставаНачисленийСотрудниковДействующиеНачисленияВРеквизит(ДокументСсылка, СотрудникиДаты, Форма, ОписаниеТаблицыВидовРасчета, ФормироватьСПериодичностьДень = Истина, ИсключатьПособия = Истина, ИсключатьВременноОтмененные = Ложь) Экспорт
	...
	ДействующиеНачисленияВРеквизит(ДокументСсылка, СотрудникиДаты, ДанныеПлановыхРасчетов, ДанныеПоказателей, ОписаниеТаблицыВидовРасчета, ФормироватьСПериодичностьДень, ИсключатьПособия, ИсключатьВременноОтмененные);
	
КонецПроцедуры

Процедура ДействующиеНачисленияВРеквизит(ДокументСсылка, СотрудникиДаты, ДанныеПлановыхРасчетов, ДанныеПоказателей, ОписаниеТаблицыВидовРасчета, ФормироватьСПериодичностьДень = Истина, ИсключатьПособия = Истина, ИсключатьВременноОтмененные = Ложь) Экспорт
...
Процедура РедактированиеСоставаНачисленийПрочитатьТекущиеДанные(ДокументСсылка, СотрудникиДаты, Форма, ОписанияТаблицВидовРасчета, РежимРаботы, ФормироватьСПериодичностьДень = Ложь, ДатаСобытия = Неопределено, ДатаОкончания = Неопределено) Экспорт
	...
	Если ЕстьКолонкиСведенийПоОкончании Тогда
		...
		Запрос.Текст =
			...
			|	ВТПлановыеНачисленияСрезПоследних КАК ПлановыеНачисления
			|ГДЕ
			|	ПлановыеНачисления.ПериодПредыдущейЗаписи <> ДАТАВРЕМЯ(1, 1, 1)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ДОБАВИТЬКДАТЕ(ПлановыеПоказатели.ПериодПредыдущейЗаписи, СЕКУНДА, -1) КАК Период,
			...
			|ГДЕ
			|	ПлановыеПоказатели.ПериодПредыдущейЗаписи <> ДАТАВРЕМЯ(1, 1, 1)";
	...
	Если ЕстьКолонкиСведенийПоОкончании Тогда
		...
		ЧастиТекстаЗапроса.Добавить("
			...
			|	ПО ЗначенияПериодическихПоказателей.ПериодПредыдущейЗаписи = ДОБАВИТЬКДАТЕ(ЗначенияПериодическихПоказателейВозвратные.Период, СЕКУНДА, 1)
	...
	Если ЕстьКолонкиСведенийПоОкончании Тогда
		...
		Запрос.Текст = Запрос.Текст + "
			...
			|				И ПлановыеНачисленияВозвратные.ДокументОснование = ПлановыйФОТВозвратные.ДокументОснование
			|				И ПлановыеНачисленияВозвратные.ГоловнаяОрганизация = ПлановыйФОТВозвратные.ГоловнаяОрганизация
			|		ПО ПлановыеНачисленияСрезПоследних.ПериодПредыдущейЗаписи = ДОБАВИТЬКДАТЕ(ПлановыеНачисленияВозвратные.Период, СЕКУНДА, 1)
...
Процедура ВводНачисленийДанныеВРеквизит(Форма, ОписанияТаблицВидовРасчета, РежимРаботы, ОтображатьТекущиеЗначения = Ложь, ФормаДополнена = Истина) Экспорт
	...
	Для каждого ОписаниеТаблицыВидовРасчета Из ОписанияТаблиц Цикл
		...
		Для Каждого СтрокаНачислений Из ДанныеНачислений Цикл
			...
			// 4D:ERP для Беларуси, Яна, 21.06.2017 16:03:27 
			// Документ "Кадровый перевод", №15217 
			// {
...
// 4D:ERP для Беларуси, Яна, 22.05.2017 10:50:05 
// Документ "Отпуск по уходу за ребенком", № 14900
// {
Процедура ПериодНачисленияДополнитьФорму(Форма, МассивОписанийТаблицФормы) Экспорт
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьРасчетЗарплатыРасширенная") Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого Таблица Из МассивОписанийТаблицФормы Цикл
		ПутьКДанным = Таблица.Значение.ПутьКДанным;
		ЗарплатаКадры.ЗаполнитьМассивИменРеквизитовФормы(Форма, МассивИменРеквизитовФормы, ПутьКДанным);	
		ИспользоватьПериод = Новый РеквизитФормы("ИспользуетсяПериод", 
		Новый ОписаниеТипов("Булево"),
		ПутьКДанным);
		ДобавляемыеРеквизиты.Добавить(ИспользоватьПериод);
	КонецЦикла;
	...
	Для Каждого Таблица Из МассивОписанийТаблицФормы Цикл
		...
		ПутьКДанным = Таблица.Значение.ПутьКДанным;
		ИмяРеквизитаДатаНачала  = Таблица.Значение.ИмяРеквизитаДатаНачала;
		ИмяРеквизитаДатаОкончания  = Таблица.Значение.ИмяРеквизитаДатаОкончания;
		...
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ПутьКДанным + ".ИспользуетсяПериод");
		ЭлементОтбора.ПравоеЗначение = Ложь;
		
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Доступность", Ложь);
		
		ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить(); 
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ИмяТаблицы + ИмяРеквизитаДатаНачала);
		ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить(); 
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ИмяТаблицы + ИмяРеквизитаДатаОкончания);
...
Процедура ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	НовыеСведения.Объект			= "Документ.НачислениеЗарплаты";
	НовыеСведения.ПоляРегистрации	= "НДФЛ.ФизическоеЛицо";
	// 4D:ERP для Беларуси, Яна, 13.07.2017 17:16:08 
	// Ошибки Зарплатного модуля, № 15414  
	// {
	НовыеСведения.ПоляДоступа		= "НДФЛ.Налог,НДФЛ.ЗачтеноАвансовыхПлатежей,НДФЛ.НалогКЗачетуВозврату,НДФЛ.ПримененныйВычетЛичный,НДФЛ.ПримененныйВычетЛичныйКодВычета,НДФЛ.ПримененныйВычетЛичныйКЗачетуВозврату,НДФЛ.ПримененныйВычетЛичныйКЗачетуВозвратуКодВычета";
	// }
	// 4D	
	НовыеСведения.ОбластьДанных		= "Доходы";
	...
	НовыеСведения.ПоляДоступа		= "Оплаты.СреднедневнойЗаработок,Оплаты.СуммаПособия";
	НовыеСведения.ОбластьДанных		= "Доходы";
	...
	НовыеСведения = ТаблицаСведений.Добавить();
	НовыеСведения.Объект			= "РегистрСведений.ПараметрыИсчисленияПроцентаСевернойНадбавкиФизическихЛиц";
	НовыеСведения.ПоляРегистрации	= "ФизическоеЛицо";
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Медицина.ДоступКНаркотическимСредствам") Тогда
		НовыеСведения = ТаблицаСведений.Добавить();
		НовыеСведения.Объект			= "Справочник.МедицинскиеСправкиФизическихЛиц";
		НовыеСведения.ПоляРегистрации	= "Владелец";
		НовыеСведения.ПоляДоступа		= "ВидСправки,НомерСправки,ДатаС,ДатаПо";
		НовыеСведения.ОбластьДанных		= "ЛичныеДанные";
...
Процедура ДополнитьМассивОписанияВозможныхКонтактов(МассивВозможныеКонтакты) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ПодборПерсоналаКлиентСервер");
		Модуль.ДополнитьМассивОписанияВозможныхКонтактов(МассивВозможныеКонтакты);
	КонецЕсли;
...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(ИсточникиДанных) Экспорт
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.ВосстановлениеВДолжности", 						"ДатаУвольнения", 				"КадровыйУчет", "Организация");
...
Процедура ЗаполнитьСписокТабличныхЧастейИсключаемыхИзКраткогоСостава(Источник, МассивТабличныхЧастей) Экспорт
	...
	Если Источник.Метаданные().ПолноеИмя() = "Документ.УвольнениеСписком" Тогда
		МассивТабличныхЧастей.Добавить("Удержания");
	КонецЕсли;
...
Процедура РабочиеМестаУдержанийПриИзмененииФизическогоЛицаВШапке(Форма) Экспорт
	...
	ФизическоеЛицо = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.ФизическоеЛицо);
	ЕстьСовмещения = КадровыйУчетРасширенный.ЕстьСотрудникиСНесколькимиДоговорами(Объект.Организация, ДатаНачала, ДатаОкончания, ФизическоеЛицо);	
...
Процедура РабочиеМестаУдержанийПриИзмененииФизическогоЛица(Форма, ТекущиеДанные) Экспорт
	...
	ФизическоеЛицо = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.ФизическоеЛицо);
	ЕстьСовмещения = КадровыйУчетРасширенный.ЕстьСотрудникиСНесколькимиДоговорами(Объект.Организация, ДатаНачала, ДатаОкончания, ФизическоеЛицо);	
...
// Создает временную таблицу ВТМаксимальныеПроцентыПорядковНачисления с полями
// ПорядокНачисленияСеверныхНадбавок и МаксимальноеЗначение.
//
// Параметры:
//			МенеджерВременныхТаблиц
//			ИмяВТМаксимальныеПроцентыПорядковНачисления - Строка, желаемое имя временной таблицы.
//
Процедура СоздатьВТМаксимальныеПроцентыПорядковНачисленияСевернойНадбавки(МенеджерВременныхТаблиц,  ИмяВТМаксимальныеПроцентыПорядковНачисления = "ВТМаксимальныеПроцентыПорядковНачисления") Экспорт
	
	Запрос = ЗапросВТМаксимальныеПроцентыПорядковНачисленияСевернойНадбавки(ИмяВТМаксимальныеПроцентыПорядковНачисления);
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.Выполнить();
...
Функция ЗапросВТМаксимальныеПроцентыПорядковНачисленияСевернойНадбавки(ИмяВТМаксимальныеПроцентыПорядковНачисления) Экспорт
	
	Возврат Новый Запрос(
		Перечисления.ПорядокНачисленияСеверныхНадбавок.ТекстЗапросаВТМаксимальныеПроцентыПорядковНачисления(
			ИмяВТМаксимальныеПроцентыПорядковНачисления));
...
Процедура ДобавитьВОписаниеПрофиляОбязательныеРоли(ОписаниеПрофиля) Экспорт 
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "Интеграция1СБухфон");
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеКурсовВалют");
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ЧтениеУчетныхЗаписейЭлектроннойПочты",						"СтандартныеПодсистемы.РаботаСПочтовымиСообщениями");
...
Процедура ДобавитьПравоНаЗащищенныйДокументооборотСКонтролирующимиОрганами(ОписаниеПрофиля)
	...
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ПравоНаЗащищенныйДокументооборотСКонтролирующимиОрганами",	"РегламентированнаяОтчетность.ЭлектронныйДокументооборотСКонтролирующимиОрганами.ЭлектронныйДокументооборотС_ФНС_ПФР_ФСГС");
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ДобавлениеИзменениеСертификатовВХранилище",					"ТехнологияСервиса.РаботаВМоделиСервиса.ЭлектроннаяПодписьВМоделиСервиса");
	ДобавитьВОписаниеПрофиляРоль(ОписаниеПрофиля, "ИспользованиеЭлектроннойПодписиВМоделиСервиса",				"ТехнологияСервиса.РаботаВМоделиСервиса.ЭлектроннаяПодписьВМоделиСервиса");
...
Процедура УстановитьПредставленияРабочихМест(Форма) Экспорт 
	...
	Если Форма.ВыбиратьРабочиеМестаУдержаний Тогда 
		...
		МассивФизическихЛиц = Объект.Удержания.Выгрузить(,"ФизическоеЛицо").ВыгрузитьКолонку("ФизическоеЛицо");
		МассивСовместителей = КадровыйУчетРасширенный.ФизическиеЛицаСНесколькимиДоговорами(Объект.Организация, ДатаНачала, ДатаОкончания, МассивФизическихЛиц);	
...
Процедура УстановитьПредставлениеРабочихМест(Форма) Экспорт
	...
	Если Форма.ВыбиратьРабочиеМестаУдержаний Тогда 
		...
		ФизическоеЛицо = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.ФизическоеЛицо);
		НесколькоРабочихМест = КадровыйУчетРасширенный.ЕстьСотрудникиСНесколькимиДоговорами(Объект.Организация, ДатаНачала, ДатаОкончания, ФизическоеЛицо);	
...
Процедура УстановитьОтображениеНачисленийРедактируемыхВОтдельныхПолях(Форма) Экспорт
	...
	Если ИменаЭлементовРедактирования <> Неопределено Тогда
		...
		УстановитьОтображениеНачисленияРедактируемогоВОтдельныхПолях(
			Форма, "РайонныйКоэффициент", ВидимостьРК, НСтр("ru='Отменено начисление'") + " ""%1"" (%2)", ФОТНеРедактируется);
...
Процедура УстановитьЗначениеСевернойНадбавкиВФорме(Форма, Сотрудник, ДатаСобытия, ФизическоеЛицо) Экспорт
	...
	Если ИменаЭлементовРедактирования <> Неопределено Тогда
		...
		ДанныеСотрудника = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудник, "СеверныйСтажВид,СеверныйСтажЛет,СеверныйСтажМесяцев,СеверныйСтажДней", ДатаСобытия);
...
Процедура ДобавитьРеквизитыНачисленийРедактируемыхВОтдельныхПолях(ДобавляемыеРеквизиты, ОписаниеТаблицыВидовРасчета, РежимРаботы)
	...
	РеквизитЗначениеПоказателяРайонныйКоэффициент = Новый РеквизитФормы(
		"ЗначениеПоказателяРайонныйКоэффициент", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 2)), , , Истина);
	ДобавляемыеРеквизиты.Добавить(РеквизитЗначениеПоказателяРайонныйКоэффициент);
	
	РеквизитЗначениеПоказателяРайонныйКоэффициент = Новый РеквизитФормы("РайонныйКоэффициентСтруктурнойЕдиницы", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(3, 2)));
	ДобавляемыеРеквизиты.Добавить(РеквизитЗначениеПоказателяРайонныйКоэффициент);
...
Процедура ДобавитьЭлементыФормыНачисленийРедактируемыхВОтдельныхПолях(Форма, ОписаниеТаблицыВидовРасчета, РежимРаботы, ДобавлятьЭлементыФормы)
	...
	Если ЭлементТаблица <> Неопределено Тогда
		...
		Если Форма.Элементы.найти(ИмяГруппыРедактированияПлановыхНачислений) = Неопределено Тогда
			...
			ПолеПоказателя.ФорматРедактирования = "ЧЦ=3; ЧДЦ=2";
...
Процедура ДобавитьСтрокиНачисленийРедактируемыхВОтдельныхПолях(Форма, Объект, ОписаниеТаблицыВидовРасчета, РежимРаботы, ИдентификаторСтрокиВидаРасчета)
	...
	Если ЗначениеЗаполнено(НачислениеРайонныйКоэффициент) Тогда
		...
		ИначеЕсли НачислениеРайонныйКоэффициентИспользуется И СодержитПолеДействие Тогда
	...
	Если ЗначениеЗаполнено(НачислениеСевернаяНадбавка) Тогда
		...
		ИначеЕсли НачислениеСевернаяНадбавкаИспользуется И СодержитПолеДействие Тогда
...
// 4D:ERP для Беларуси, Яна, 22.08.2017 17:52:52 
...
Процедура РассчитатьЗначенияСовокупныхТарифныхСтавок(Форма, ДатаСобытия, ОписаниеТаблицыНачислений, ОписаниеТаблицыПересчетТарифныхСтавок = Неопределено, Сотрудник = Неопределено, ГрафикРаботы = Неопределено, КоличествоСтавок = 0) Экспорт 
	...
	// 4D:ERP для Беларуси, Яна, 07.06.2017 16:48:53 
	// Документ "Прием на работу", №14926 
	// {
	...
	Выборка = ЗначенияСовокупныхТарифныхСтавок(ИсходныеДанные, ДатаСобытия).Выбрать();	
	// }
	// 4D
	...
	Иначе
		...
		Иначе 
			...
			Если Найти(ВРег(ПутьКДаннымНачислений), ВРег("Объект.")) = 1 Тогда
...
Процедура СкорректироватьДатыНачисленийБезПериодаДействия(ТаблицаНачислений, ПериодРегистрации, ИмяПоляНачисления = "Начисление", РеквизитыНачислений = Неопределено, ПроверятьСторно = Ложь) Экспорт
	...
	Для Каждого КлючИЗначение Из РеквизитыНачислений Цикл
		...
		// 4D:ERP для Беларуси, Яна, 20.06.2017 11:18:43 
		// Добавление варианта оплаты отпуска по уходу за ребенком инвалидом до 3-х лет, №15145, 14877 
		// {
		Если КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехЛет 
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехЛетНаВторогоРебенка
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДо3Лет50ПроцентовБПМ 
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехНаРебенкаИнвалида 
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаВичИнфицРебенка 
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаДо18ЛетНаРебенкаИнвалида
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаСтарше3ЛетКромеРебенкаИнвалида
			Или КлючИЗначение.Ключ.КатегорияНачисленияИлиНеоплаченногоВремени =  Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаСтарше3Лет Тогда
			Продолжить;		
		КонецЕсли;
		// }
		// 4D
...
Функция СостоянияСотрудниковДляРасчетаОценочныхОбязательствОтпусков(Сотрудники, Период) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	СостоянияСотрудников.СоздатьВТСостоянияСотрудников(Запрос.МенеджерВременныхТаблиц, Сотрудники, , КонецМесяца(Период), КонецМесяца(Период));
...
// 4D:ERP для Беларуси, Яна, 12.06.2017 10:39:27 
// Документ "Прием на работу", №14926 
// {
////////////////////////////////////////////////////////////////////////////////
...
Процедура УстановитьОтображениеПолейРедактированияРКИСН(Форма) Экспорт
	
	ИменаЭлементовРедактированияРКИСН = ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ИменаЭлементовРедактированияРКИСН");
	
	Если ИменаЭлементовРедактированияРКИСН <> Неопределено Тогда
		
		// Отображение полей надбавки за вредность
		ВидимостьНадбавкиЗаВредность = ЗарплатаКадрыРасширенныйКлиентСервер.НадбавкаЗаВредностьВходитВСоставПлановыхНачислений(Форма);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.НачислениеНадбавкаЗаВредность,
			"Видимость",
			ВидимостьНадбавкиЗаВредность);
			
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.РазмерНадбавкаЗаВредность,
			"Видимость",
			ВидимостьНадбавкиЗаВредность);
			
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			// 4D:ERP для Беларуси, АлексейЧ, 29.12.2017 14:20:02 
			// Начисление "Надбавка за работу во вредных", № 17021
			// {
			ИменаЭлементовРедактированияРКИСН.ЗначениеПоказателяНадбавкаЗаВредность,
			// }
			// 4D
			"Видимость",
			ВидимостьНадбавкиЗаВредность);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.ГруппаРасшифровкиНадбавкиЗаВредность,
			"Видимость",
			ВидимостьНадбавкиЗаВредность);
		...
		Если МеняетсяСоставНачислений
			И НЕ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеНадбавкаЗаВредность") Тогда
			
			ВидимостьПолейРедактирования = Ложь;
			ЦветТекстаПоля = ЦветаСтиля.ПоясняющийТекст;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеНадбавкаЗаВредность", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отменено начисление ""%1"" (%2)'"),
					ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеНадбавкаЗаВредность"),
					// 4D:ERP для Беларуси, АлексейЧ, 29.12.2017 14:20:02 
					// Начисление "Надбавка за работу во вредных", № 17021
					// {
					Формат(ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ЗначениеПоказателяНадбавкаЗаВредность"), "ЧЦ=3; ЧДЦ=2")));
					// }
					// 4D
					
		Иначе
			
			ВидимостьПолейРедактирования = ВидимостьНадбавкиЗаВредность;
			ЦветТекстаПоля = ЦветаСтиля.ЦветТекстаФормы;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеНадбавкаЗаВредность",
				ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеНадбавкаЗаВредность"));
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			// 4D
			"ТолькоПросмотр",
			НЕ МеняетсяСоставНачислений ИЛИ Форма.ТолькоПросмотр);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.НачислениеРайонныйКоэффициент,
			"Видимость",
			ВидимостьРК);
			
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.РазмерРайонныйКоэффициент,
			"Видимость",
			ВидимостьРК);
			
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.ЗначениеПоказателяРайонныйКоэффициент,
			"Видимость",
			ВидимостьРК);
			
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.ГруппаРасшифровкиРК,
			"Видимость",
			ВидимостьРК);
		...
		Если МеняетсяСоставНачислений
			И НЕ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеРайонногоКоэффициента") Тогда
			
			ВидимостьПолейРедактирования = Ложь;
			ЦветТекстаПоля = ЦветаСтиля.ПоясняющийТекст;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеРайонныйКоэффициент", 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отменено начисление ""%1"" (%2)'"),
					ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеРайонныйКоэффициент"),
					Формат(ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ЗначениеПоказателяРайонныйКоэффициент"), "ЧЦ=3; ЧДЦ=2")));
					
		Иначе
			
			ВидимостьПолейРедактирования = ВидимостьРК;
			ЦветТекстаПоля = ЦветаСтиля.ЦветТекстаФормы;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеРайонныйКоэффициент",
				ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеРайонныйКоэффициент"));
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.НачислениеСевернаяНадбавка,
			"Видимость",
			ВидимостьСН);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.РазмерСевернаяНадбавка,
			"Видимость",
			ВидимостьСН);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.ЗначениеПоказателяСевернаяНадбавка,
			"Видимость",
			ВидимостьСН);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ИменаЭлементовРедактированияРКИСН.ГруппаРасшифровкиСН,
			"Видимость",
			ВидимостьСН);
		
		Если МеняетсяСоставНачислений
			И НЕ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеСевернойНадбавки") Тогда
			
			ВидимостьПолейРедактирования = Ложь;
			ЦветТекстаПоля = ЦветаСтиля.ПоясняющийТекст;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеСевернаяНадбавка", 
				НСтр("ru='Отменено начисление ""'") 
				+ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеСевернаяНадбавка")
					+ """");
					
		Иначе
					
			ВидимостьПолейРедактирования = ВидимостьСН;
			ЦветТекстаПоля = ЦветаСтиля.ЦветТекстаФормы;
			ОбщегоНазначенияКлиентСервер.УстановитьРеквизитФормыПоПути(Форма, "ПредставлениеНачислениеСевернаяНадбавка",
				ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеСевернаяНадбавка"));
		...
		// 4D:ERP для Беларуси, Яна, 04.08.2017 16:08:39 
		// Результат тестирования конфигурации, №15540 
		// {	
		УстановитьВидимостьЭлементовВкладаВФОТРКИСН(Форма);
		// }
		// 4D
...
Процедура УстановитьЗначениеРайонногоКоэффициентаВФорме(Форма, Знач Сотрудник, Знач ДатаСведений, Знач Подразделение = Неопределено, Знач Организация = Неопределено, Знач Территория = Неопределено, Знач Должность = Неопределено, УстановитьЗначенияПоказателей = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(Должность) Тогда
		ДанныеУсловийТруда = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Должность, "ВыплачиваетсяНадбавкаЗаВредность,ПроцентНадбавкиЗаВредность,КоличествоДнейДополнительногоОтпускаВГод");
		ПрименятьНадбавкуЗаВредность = ДанныеУсловийТруда.ВыплачиваетсяНадбавкаЗаВредность;
		ПроцентНадбавкиЗаВредность = ДанныеУсловийТруда.ПроцентНадбавкиЗаВредность;
	Иначе
		ПрименятьНадбавкуЗаВредность = Ложь;
...
Процедура УстановитьВидимостьЭлементовВкладаВФОТРКИСН(Форма, РедактироватьФОТ = Неопределено) Экспорт
	...
	Если ИменаЭлементовРедактированияРКИСН <> Неопределено Тогда
		...
		ВидимостьНадбавкиЗаВредность = ЗарплатаКадрыРасширенныйКлиентСервер.НадбавкаЗаВредностьВходитВСоставПлановыхНачислений(Форма)
			И НЕ НадбавкаЗаВредностьВыводитсяИзСоставаПлановыхНачислений И ПоказыватьВкладВФОТ;
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			...
			ИменаЭлементовРедактированияРКИСН.РазмерНадбавкаЗаВредность,
			"ТолькоПросмотр",
			НеРазрешеноРедактироватьФОТ ИЛИ Форма.ТолькоПросмотр);
			
		// Районный коэффициент
		РКВыводитсяИзСоставаПлановыхНачислений = 
			МеняетсяСоставНачислений И НЕ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеРайонногоКоэффициента");
			
		ВидимостьРК = ЗарплатаКадрыРасширенныйКлиентСервер.РайонныйКоэффициентВходитВСоставПлановыхНачислений(Форма)
			И НЕ РКВыводитсяИзСоставаПлановыхНачислений И ПоказыватьВкладВФОТ;
		...
		СНВыводитсяИзСоставаПлановыхНачислений = 
			МеняетсяСоставНачислений И НЕ ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеСевернойНадбавки");
...
Процедура УстановитьРежимОтображениеЭлементовРедактированияРКИСН(Форма, РедактированиеПоказателей, РедактированиеВкладаВФОТ) Экспорт
...
Функция РедактироватьНачисленияРКИСНВОтдельныхПоляхФормы(РежимРаботы, ОписаниеТаблицыВидовРасчета) Экспорт
	
	РежимРедактирования = Ложь;
	Если РежимРаботы = 1 И ОписаниеТаблицыВидовРасчета.Свойство("РедактироватьНачисленияРКИСНВОтдельныхПолях") Тогда
		РежимРедактирования = ОписаниеТаблицыВидовРасчета.РедактироватьНачисленияРКИСНВОтдельныхПолях;
	КонецЕсли; 
...
Функция ИзменяетсяПрименениеНачисленийРКИСН(Форма) Экспорт
	
	Возврат ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеРайонногоКоэффициента")
		<> ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "НачислениеРайонныйКоэффициентИспользуется")
		ИЛИ
		ОбщегоНазначенияКлиентСервер.ПолучитьРеквизитФормыПоПути(Форма, "ПрименяетсяНачислениеСевернойНадбавки")
