...
Процедура УстановитьПоляИсправления(Форма, РежимИсправления = "РасчетЗарплаты", ИмяОсновногоРеквизита = "Объект", ПолеПериодРегистрации = "ПериодРегистрации") Экспорт
	...
	Если ИспользоватьИсправлениеДокумента И РежимИсправления = "РасчетЗарплаты" И Форма.Параметры.Ключ.Пустая() Тогда
	...
	Если ИспользоватьИсправлениеДокумента И Форма.ДокументИсправлен Тогда
		Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
			Форма.ИсправлениеИнфоНадпись = НСтр("ru = 'Документ является исправлением документа предыдущего периода. В свою очередь документ повторно исправлен и его редактирование невозможно'");
		Иначе
			Форма.ИсправлениеИнфоНадпись = НСтр("ru = 'Документ исправлен и его редактирование невозможно'");
		КонецЕсли;
	...
	ИначеЕсли ИспользоватьСторнированиеДокумента И Форма.ДокументСторнирован Тогда
		Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
			Форма.ИсправлениеИнфоНадпись = НСтр("ru = 'Документ является исправлением документа предыдущего периода. В свою очередь документ сторнирован и его редактирование невозможно'");
		Иначе
			Форма.ИсправлениеИнфоНадпись = НСтр("ru = 'Документ сторнирован и его редактирование невозможно'");
		КонецЕсли;
		Форма.Элементы.ИсправлениеИнфоНадпись.ЦветТекста = ЦветаСтиля.ЦветОсобогоТекста;
	ИначеЕсли ИспользоватьИсправлениеДокумента И ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
		
		Форма.ИсправлениеИнфоНадпись = НСтр("ru = 'Документ является исправлением документа предыдущего периода'");
	...
	ИначеЕсли РежимИсправления = "РасчетЗарплаты" И (ИспользоватьИсправлениеДокумента Или ИспользоватьСторнированиеДокумента) Тогда
		...
		Если ВидимостьКомандыИсправить И ВидимостьКомандыСторнировать Тогда
			ТекстНадписи = ТекстНадписи + " "
				+ НСтр("ru = 'Редактирование этого документа не рекомендуется. Воспользуйтесь командой Исправить для исправления этого документа или Сторнировать для его отмены'"); 
		ИначеЕсли ВидимостьКомандыИсправить Тогда
			ТекстНадписи = ТекстНадписи + " "
				+ НСтр("ru = 'Редактирование этого документа не рекомендуется. Воспользуйтесь командой Исправить для исправления этого документа'");
		ИначеЕсли ВидимостьКомандыСторнировать Тогда
			ТекстНадписи = ТекстНадписи + " "
				+ НСтр("ru = 'Редактирование этого документа не рекомендуется. Воспользуйтесь командой Сторнировать для отмены этого документа'");
		КонецЕсли;
...
Функция ДоступноИсправлениеРасчетногоДокумента(Документ, Организация, ПериодРегистрации, ПроверятьНаличиеНачислений) Экспорт 
	...
	БылаВыплатаВПериоде = ВыплатаПроизводилась(Документ, ПериодРегистрации);
	...
	Если РежимИсправления = "РасчетЗарплаты" Тогда
		...
		Если ПериодРегистрацииИсправленногоДокумента <> Неопределено
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Неверно указан период: Период должен быть больше, чем в исправляемом документе (%1 г.)'"),
...
Функция ВыплатаПроизводилась(ДокументОснование, ПериодВзаиморасчетов) Экспорт
	
	Если ВзаиморасчетыССотрудникамиРасширенный.РегистрироваласьНачисленнаяЗарплатаДокументом(ДокументОснование) Тогда
		// Движения по документу были, значит определим была ли по этому документу выплата
		Возврат ВзаиморасчетыССотрудникамиРасширенный.ПроизводиласьВыплатаЗарплатыПоДокументу(ДокументОснование);
	Иначе
		// Движений по документу не было, значит определим была ли выплата по периоду
		Возврат ВзаиморасчетыССотрудникамиРасширенный.ПроизводиласьВыплатаЗарплатыПоПериодуВзаиморасчетов(ПериодВзаиморасчетов);
	КонецЕсли;
