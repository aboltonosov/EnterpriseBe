...
Функция ОписаниеДанныхПозицииВХранилище(Форма, ИдентификаторСтрокиТекущейПозиции, ДатаВступленияВСилу, ПодразделениеПозиций = Неопределено, ВыборПодчиненныхПодразделений = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		...
		Модуль.ДополнитьОписаниеДанныхПозицииВХранилище(Форма, ОписаниеПозиции, ДанныеПозиции);
	КонецЕсли;
...
Процедура ПрочитатьДанныеПозицииВФорму(Форма, АдресДанныхПозиции) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.ПрочитатьДанныеПозицииВФорму(Форма, НовыеДанныеПозиции);
	КонецЕсли;
...
Процедура РеквизитВДанные(Форма, ОбъектПриемник, ТолькоДанныеОбъекта = Ложь) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.РеквизитВДанные(Форма, ОбъектПриемник);
	КонецЕсли;
...
Процедура РедактированиеСостоянияШРДополнитьФорму(Форма, ОписаниеТаблицыПозиций, ОписаниеТаблицыВидовРасчета)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.РедактированиеСостоянияШРДополнитьФорму(Форма);
	КонецЕсли;
...
Процедура РедактированиеСостоянияШРДанныеВРеквизиты(ДокументСсылка, Период, Форма, ОписаниеТаблицыПозиций, ОписаниеТаблицыВидовРасчета)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.РедактированиеСостоянияШРДанныеВРеквизиты(ДокументСсылка, Период, Форма, ОписаниеТаблицыПозиций);
	КонецЕсли;
...
Процедура ЗаполнитьТерриториальныеУсловияВСтрокахПозиций(Форма, ДатаВступленияВСилу)
	...
	Подразделения = Форма.Объект.Позиции.Выгрузить(, "Подразделение").ВыгрузитьКолонку("Подразделение");
...
Процедура ЗаполнитьТекущиеДанныеПозиции(Форма, ТекущиеДанные, ДатаВступленияВСилу)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.ЗаполнитьТекущиеДанныеПозиции(Форма, ТекущиеДанные, ДанныеТекущейПозиции);
	КонецЕсли;
...
Процедура УдалитьВсеДанныеПоСтрокеПозиции(Форма, СтрокаПозиции) Экспорт
	
	УдалитьНачисленияИЕжегодныеОтпускаПоИдентификаторуСтрокиПозиции(Форма, СтрокаПозиции.ИдентификаторСтрокиПозиции);
	УдалитьСпециальностиПоИдентификаторуСтрокиПозиции(Форма, СтрокаПозиции.ИдентификаторСтрокиПозиции);
	
	Форма.Объект.Позиции.Удалить(СтрокаПозиции);
...
Процедура ОчиститьДанныеТабличныхЧастейОбъектаФормы(Форма) Экспорт
	...
	Для каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		УправлениеШтатнымРасписаниемФормы.УдалитьВсеДанныеПоСтрокеПозиции(Форма, УдаляемаяСтрока);
	КонецЦикла;
	
	Если Форма.Объект.Позиции.Количество() > 0 Тогда
		УправлениеШтатнымРасписаниемФормы.РеквизитВДанные(Форма, Форма.Объект, Истина);
	КонецЕсли;
	
КонецПроцедуры

Процедура ПрочитатьТекущиеДанныеПозицииШтатногоРасписанияВФормуСправочника(Форма, Ссылка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.ПрочитатьТекущиеДанныеПозицииШтатногоРасписанияВФормуСправочника(Форма, ДанныеПозиции);
	КонецЕсли;
