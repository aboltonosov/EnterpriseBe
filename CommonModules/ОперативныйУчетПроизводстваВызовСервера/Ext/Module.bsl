...
Функция СформироватьМаршрутныеЛистыПоЗаказам(СписокРаспоряжений) Экспорт
	...
	Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, СписокРаспоряжений.Количество());
	...
	Если ТекстПредупреждения = Неопределено  Тогда
		...
		Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоВсего);
		
		КоличествоОбработанных = СформироватьМаршрутныеЛисты(ДанныеМаршрутныхЛистов);
		Производительность.ЗафиксироватьЗамерВремени(ОписаниеЗамера, КоличествоОбработанных);
...
Функция ПараметрыОформленияВыработкиСотрудников(Распоряжения, ТекстПредупреждения) Экспорт
	
	ТекстПредупреждения = Неопределено;
	
	СтруктураОтбора = Документы.ВыработкаСотрудников.ТрудозатратыКОформлениюОтборПоУмолчанию();
	СтруктураОтбора.Распоряжения = Распоряжения;
	...
	Если ТаблицаТрудозатратыКОформлению.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru = 'По выбранным распоряжениям выработка сотрудников уже оформлена или оформлление не требуется.'");
	...
	Для Каждого ДанныеСтроки из ТаблицаТрудозатратыКОформлению Цикл
		...
		ИначеЕсли ТекПодразделение <> ДанныеСтроки.Подразделение Тогда
			ТекстПредупреждения = НСтр("ru = 'Документ может быть сформирован только по одному подразделению.'");
			Отказ = Истина;
		КонецЕсли;
