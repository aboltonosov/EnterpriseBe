...
Процедура ВыполнитьПодключаемуюКомандуОтчетПодготовкаОбъектовОтчет(ДополнительныеПараметры)
	...
	Если ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовМенюОтчеты.Количество() <> 0 Тогда // требуется проверка типа
		ЕстьСоздаваемыеНаОснованииОбъекты = Ложь;
		Для Каждого ОбъектОтчет Из ОбъектыОснований Цикл
			Если ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовМенюОтчеты.Найти(ТипЗнч(ОбъектОтчет)) <> Неопределено Тогда
		...
		Если Не ЕстьСоздаваемыеНаОснованииОбъекты Тогда
			ТекстСообщения = МенюОтчетыВызовСервера.СообщениеОПредназначенииКомандыОтчет(ДополнительныеПараметры.ОписаниеКоманды.ТипыОбъектовМенюОтчеты);
