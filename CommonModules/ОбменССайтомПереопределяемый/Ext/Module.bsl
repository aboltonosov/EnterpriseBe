...
Процедура ЗаполнитьИнформациюОСкладахИТочкахСамовывоза(СписокТочекСамовывоза,
	...
	Пока Выборка.Следующий() Цикл
		...
		// 4D:ERP для Беларуси, Дмитрий, 11.09.2017 5:42:49 
		// Страна Беларусь
		// {
...
Процедура ПолучитьПоСтавкеНДСЗначениеДляВыгрузки(СтавкаНДС, ЗначениеСтавкиНДС) Экспорт
	...
	// Ставка НДС 25%, №11449
	// {
	Если СтавкаНДС = Перечисления.СтавкиНДС.НДС25_125 Тогда
		ЗначениеСтавкиНДС = 25;
	ИначеЕсли СтавкаНДС = Перечисления.СтавкиНДС.НДС25 Тогда
		ЗначениеСтавкиНДС = 25;
...
//
// Параметры:
//  НастройкаПроксиСервера - Соответствие - имеет следующие поля:
