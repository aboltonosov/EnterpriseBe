...
// 4D:ERP для Беларуси, Яна, 08.08.2017 10:32:36 
...
Процедура СформироватьДвиженияЗначенийРазрядовТарифнойСетки(Движения, ДанныеРазрядовТарифнойСетки) Экспорт 
	...
	Для Каждого ТекСтрока Из ДанныеРазрядовТарифнойСетки Цикл
		...
		НоваяСтрока.ТарифныйОклад = ТекСтрока.ТарифныйОклад;
		НоваяСтрока.ДолжностнойОклад = ТекСтрока.ДолжностнойОклад;
...
Функция ДанныеТарифнойСетки(ТарифнаяСетка, ДатаСобытия = '00010101', Регистратор = Неопределено) Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
				   ...
				   |	ЗначенияТарифов.РазрядКатегория.РазрядЕТС КАК РазрядЕТС,
				   |	ЗначенияТарифов.РазрядКатегория.Коэффициент КАК Коэффициент,
				   |	ЗначенияТарифов.РазрядКатегория.ПовышающийКоэффициент КАК ПовышающийКоэффициент,
				   |	ЗначенияТарифов.РазрядКатегория.ПроцентПовышения КАК ПроцентПовышения,
	               |	ЕСТЬNULL(ЗначенияТарифов.РазрядныйКоэффициент, 0) КАК РазрядныйКоэффициент,
				   |	ЕСТЬNULL(ЗначенияТарифов.ТарифныйОклад, 0) КАК ТарифныйОклад,
  				   |	ЕСТЬNULL(ЗначенияТарифов.ДолжностнойОклад, 0) КАК ДолжностнойОклад
	               |ИЗ
...
Функция ЗапросВТЗначенияПоказателейТарифныхСеток(ТолькоРазрешенные, ПараметрыПостроения, ПоляОтбора = Неопределено, ИмяВТЗначенияПоказателейТарифныхСеток = "ВТЗначенияПоказателейТарифныхСеток") Экспорт
	...
	ТекстЗапроса = 
		...
		|	ЕСТЬNULL(ЗначенияТарифовСрезПоследних.ДолжностнойОклад, 0) КАК ЗначениеПоказателя
...
Функция ИнициализироватьЗаголовокФормыИРеквизитов(Объект, ВидТарифнойСетки = Неопределено, ЭтоНовый = Ложь) Экспорт
	...
	Иначе
		...
		// 4D:ERP для Беларуси, Юлия, 14.11.2017 9:28:51 
		// Изменить в синониме имени и представлении элементов конфигурации "группа" на "сетка", № 16581
		// {
		СоответствиеЭлементаИЗаголовка.Вставить("ТарифнаяСеткаСписок", 			НСтр("ru = 'Тарифные сетки'"));	
		СоответствиеЭлементаИЗаголовка.Вставить("ТарифнаяСеткаЭлементТариф",	НСтр("ru = 'Тарифная сетка'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ТарифнаяСеткаСписокТариф", 	НСтр("ru = 'Тарифные сетки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ТарифнаяСеткаЭлементНадбавка",	НСтр("ru = 'Тарифная сетка надбавки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ТарифнаяСеткаСписокНадбавка", 	НСтр("ru = 'Тарифные сетки надбавок'"));
		
		СоответствиеЭлементаИЗаголовка.Вставить("УтверждениеТарифнойСеткиСписок", 		НСтр("ru = 'Утверждения тарифных сеток'"));
		СоответствиеЭлементаИЗаголовка.Вставить("УтверждениеТарифнойСеткиЭлементТариф", НСтр("ru = 'Утверждение тарифной сетки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("УтверждениеТарифнойСеткиСписокТариф",  НСтр("ru = 'Утверждения тарифных сеток'"));
		СоответствиеЭлементаИЗаголовка.Вставить("УтверждениеТарифнойСеткиЭлементНадбавка", НСтр("ru = 'Утверждение тарифных надбавок'"));
		СоответствиеЭлементаИЗаголовка.Вставить("УтверждениеТарифнойСеткиСписокНадбавка",  НСтр("ru = 'Утверждения тарифных надбавок'"));	
		
		
		СоответствиеЭлементаИЗаголовка.Вставить("ИзменениеРазрядаСписок", 	НСтр("ru = 'Изменения квалификационных разрядов сотрудников'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ИзменениеРазрядаЭлемент",  НСтр("ru = 'Изменение квалификационного разряда сотрудника'"));
		
		СоответствиеЭлементаИЗаголовка.Вставить("СправочникРазрядовСписок", 	НСтр("ru = 'Квалификационные разряды'"));
		СоответствиеЭлементаИЗаголовка.Вставить("СправочникРазрядовЭлемент", 	НСтр("ru = 'Квалификационный разряд'"));
					
		СоответствиеЭлементаИЗаголовка.Вставить("РеквизитТарифнаяСеткаТариф", 		НСтр("ru = 'Тарифная сетка'"));
		СоответствиеЭлементаИЗаголовка.Вставить("РеквизитРазрядКатегорияТариф", 	НСтр("ru = 'Разряд'"));
		СоответствиеЭлементаИЗаголовка.Вставить("РеквизитТарифнаяСеткаНадбавка",	НСтр("ru = 'Тарифная сетка надбавки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("РеквизитРазрядКатегорияНадбавка",  НСтр("ru = 'Разряд надбавки'"));
		
		СоответствиеЭлементаИЗаголовка.Вставить("ВсеИзмененияТарифнойСетки",		НСтр("ru = 'Все изменения тарифной сетки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ВсеИзмененияТарифнойСеткиТариф",	НСтр("ru = 'Все изменения тарифной сетки'"));
		СоответствиеЭлементаИЗаголовка.Вставить("ВсеИзмененияТарифнойСеткиНадбавка",	НСтр("ru = 'Все изменения тарифной сетки надбавки'"));

		СоответствиеЭлементаИЗаголовка.Вставить("РеквизитРазрядКатегорияВКадровыхДокументах", НСтр("ru = 'Разряд'"));
...
// 4D:ERP для Беларуси, Яна, 15.05.2017 17:55:32 
// Создание новой позиции штатного расписания, № 14924 
// {
