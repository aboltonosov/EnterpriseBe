...
Функция ПравоНаРазделУчета(Право, ВидУчета) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'В функции ""%1"", в параметре ""%2"" не обслуживается значение ""%3"".'"),
...
Процедура СоздатьВТОсновныеСотрудникиФизическихЛицВПериодах(МенеджерВременныхТаблиц, ТолькоРазрешенные, ГоловнаяОрганизация, ИмяВТФизическиеЛицаПериоды, ИмяВТОсновныеСотрудникиФизическихЛицВПериодах) Экспорт
	...
	ЗарплатаКадрыОбщиеНаборыДанных.СоздатьВТИмяРегистраСрезПоследних(
		...
		ОписаниеФильтра,
		ПараметрыПостроения,
		ИмяВТОсновныеСотрудникиФизическихЛицВПериодах);
...
Функция ПараметрыПолученияОсновныхСотрудниковФизическихЛиц(СписокФизическихЛиц, Организация, НачалоПериода, ОкончаниеПериода) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Иначе
			...
			Для Каждого ОрганизацияСписка Из Организация Цикл
				СписокОрганизаций.Добавить(ЗарплатаКадрыПовтИсп.ГоловнаяОрганизация(ОрганизацияСписка));
			КонецЦикла;
			
			ПараметрыПолученияСотрудниковОрганизаций.Организация = СписокОрганизаций;
	...
	ПараметрыПолученияСотрудниковОрганизаций.КадровыеДанные					= "ГоловнаяОрганизация,ФизическоеЛицо,ВидЗанятости,ДатаПриема,ДатаЗавершенияРаботы";
...
Функция ЗапросВТСотрудникиОрганизацииПоРабочимМестам(ТолькоРазрешенные, ИмяВТСотрудникиОрганизации, ИмяВТСотрудникиПериоды, Параметры, МенеджерВременныхТаблиц) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		ТекстЗапросаСотрудникиОрганизацииДляКадровойИстории =
			СтрЗаменить(ТекстЗапросаСотрудникиОрганизацииДляКадровойИстории, "&ДатаНачала", ТекстДатаОкончания);
	...
	Иначе
		...
		МассивНеобходимыхКадровыхДанных = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок("ФизическоеЛицо,ОформленПоТрудовомуДоговору,ДатаПриема,ДатаЗавершенияРаботы,ГоловнаяОрганизация,Организация,Подразделение");
...
Функция ЗапросВТРабочиеМестаСотрудниковПоВременнойТаблице(ТолькоРазрешенные, ИмяВТРабочиеМестаСотрудников, Параметры, МенеджерВременныхТаблиц) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКадровыйУчет") Тогда
		...
		Если СоздаватьПромежуточныеТаблицы Тогда
			...
			ЗапросДляПроверкиНачалаУчета = Новый Запрос;
			ЗапросДляПроверкиНачалаУчета.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
			ЗапросДляПроверкиНачалаУчета.Текст =
				"ВЫБРАТЬ ПЕРВЫЕ 1
				|	РабочиеМестаСотрудников.ВидСобытия
				|ИЗ
				|	ВТРабочиеМестаСотрудников КАК РабочиеМестаСотрудников
				|ГДЕ
				|	РабочиеМестаСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.НачальныеДанные)";
			
			ЗапросДляПроверкиНачалаУчета.Текст = СтрЗаменить(ЗапросДляПроверкиНачалаУчета.Текст, "ВТРабочиеМестаСотрудников", ИмяВТРабочиеМестаСотрудников);
			
			РезультатЗапроса = ЗапросДляПроверкиНачалаУчета.Выполнить();
			Если Не РезультатЗапроса.Пустой() Тогда
				
				Запрос.Текст =
					"ВЫБРАТЬ
					|	ВЫБОР
					|		КОГДА РабочиеМестаСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.НачальныеДанные)
					|			ТОГДА ВЫБОР
					|					КОГДА КОНЕЦПЕРИОДА(СотрудникиПериоды.ДатаНачала, ДЕНЬ) >= ЕСТЬNULL(ТекущиеКадровыеДанные.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1))
					|						ТОГДА СотрудникиПериоды.ДатаНачала
					|					КОГДА ТекущиеКадровыеДанные.ДатаПриема <= НАЧАЛОПЕРИОДА(РабочиеМестаСотрудников.Период, ДЕНЬ)
					|						ТОГДА ВЫБОР
					|								КОГДА ТекущиеКадровыеДанные.ДатаПриема > СотрудникиПериоды.ДатаНачала
					|									ТОГДА ТекущиеКадровыеДанные.ДатаПриема
					|								ИНАЧЕ СотрудникиПериоды.ДатаНачала
					|							КОНЕЦ
					|					ИНАЧЕ СотрудникиПериоды.ДатаНачала
					|				КОНЕЦ
					|		ИНАЧЕ РабочиеМестаСотрудников.Период
					|	КОНЕЦ КАК Период,
					|	РабочиеМестаСотрудников.*
					|ПОМЕСТИТЬ ВТПериодыРабочихМестСотрудников
					|ИЗ
					|	ВТРабочиеМестаСотрудников КАК РабочиеМестаСотрудников
					|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанные
					|		ПО РабочиеМестаСотрудников.Сотрудник = ТекущиеКадровыеДанные.Сотрудник
					...
					|////////////////////////////////////////////////////////////////////////////////
					|ВЫБРАТЬ
					|	ПериодыРабочихМестСотрудников.*
					|ПОМЕСТИТЬ ВТРабочиеМестаСотрудников
					|ИЗ
					|	ВТПериодыРабочихМестСотрудников КАК ПериодыРабочихМестСотрудников
					|;
					|
					|////////////////////////////////////////////////////////////////////////////////
					|УНИЧТОЖИТЬ ВТПериодыРабочихМестСотрудников";
				...
				Если ПустаяСтрока(Параметры.ИмяПоляСотрудник) Тогда
					Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПО РабочиеМестаСотрудников.Сотрудник = СотрудникиПериоды.Сотрудник", "ПО (ИСТИНА)");
				Иначе
					Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.Сотрудник", "СотрудникиПериоды." + Параметры.ИмяПоляСотрудник);
				КонецЕсли;
				
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаНачала", "СотрудникиПериоды." + Параметры.ИмяПоляНачалоПериода);
				Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаОкончания", "СотрудникиПериоды." + Параметры.ИмяПоляОкончаниеПериода);
...
Функция ПутьКДаннымСведенийОГражданстве(Знач ИмяПоля)
	...
	ИначеЕсли ИмяПоляВВерхнемРегистре = ВРег("Страна") Тогда
		...
		// 4D:ERP для Беларуси, Дмитрий, 11.09.2017 5:40:41 
		// Страна Беларусь
		// {
		ПутьКДанным = "	ЕСТЬNULL(ГражданствоФизическихЛиц.Страна, ЗНАЧЕНИЕ(Справочник.СтраныМира.Беларусь))";
		// }
		// 4D
...
Функция ЗапросВТКадровыеДанныеСотрудников(ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, КадровыеДанные, ПоляОтбораПериодическихДанных, ФормироватьСПериодичностьДень, ИсточникиДанных, ИмяВТКадровыеДанныеСотрудников, ТаблицаОтборовСодержитПолеФизическоеЛицо, УничтожитьВспомогательныеТаблицы) Экспорт
	...
	Для каждого ИмяЗапрашиваемыхДанных Из ИменаКадровыхДанных Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийКадровойИсторииСотрудников(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или ДобавитьПолеСведенийОКлассахУсловийТруда(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
	...
	Иначе
		...
		Если ОписательВТОтборов.ПолучатьСведенияНаПериодНачалаУчета Тогда
			ОписательВТОтборов.ИмяПоляПериод = "ПериодСведений";
		КонецЕсли;
		
		Если Не ФормироватьТаблицуРазличных И ОписательВТОтборов.ПолучатьСведенияНаПериодНачалаУчета Тогда
		
			ТекстЗапросаПолученияРазличныхЗаписей =
				"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
				|	ОтборовРазличныхСотрудников.Период КАК Период,
				|	ОтборовРазличныхСотрудников.Сотрудник КАК Сотрудник,
				|	ОтборовРазличныхСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
				|	МИНИМУМ(КадроваяИсторияСотрудников.Период) КАК ПериодКадровойИстории
				|ПОМЕСТИТЬ ВТПериодыОтбораКадровойИстории
				|ИЗ
				|	ВТОтборовРазличныхСотрудников КАК ОтборовРазличныхСотрудников
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
				|		ПО ОтборовРазличныхСотрудников.Сотрудник = КадроваяИсторияСотрудников.Сотрудник
				|			И (ОтборовРазличныхСотрудников.Период <> ДАТАВРЕМЯ(1, 1, 1))
				|			И ОтборовРазличныхСотрудников.Период <= КадроваяИсторияСотрудников.Период
				|			И (КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.НачальныеДанные))
				|
				|СГРУППИРОВАТЬ ПО
				|	ОтборовРазличныхСотрудников.Период,
				|	ОтборовРазличныхСотрудников.Сотрудник,
				|	ОтборовРазличныхСотрудников.ФизическоеЛицо
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	ВЫБОР
				|		КОГДА ПериодыОтбораКадровойИстории.ПериодКадровойИстории ЕСТЬ NULL 
				|			ТОГДА ПериодыОтбораКадровойИстории.Период
				|		ИНАЧЕ ВЫБОР
				|				КОГДА ЕСТЬNULL(ТекущиеКадровыеДанные.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1)) > ПериодыОтбораКадровойИстории.Период
				|					ТОГДА ПериодыОтбораКадровойИстории.Период
				|				ИНАЧЕ ПериодыОтбораКадровойИстории.ПериодКадровойИстории
				|			КОНЕЦ
				|	КОНЕЦ КАК ПериодСведений,
				|	ПериодыОтбораКадровойИстории.Период,
				|	ПериодыОтбораКадровойИстории.Сотрудник,
				|	ПериодыОтбораКадровойИстории.ФизическоеЛицо КАК ФизическоеЛицо
				|ПОМЕСТИТЬ ВТОтборовСУточненнымПериодом
				|ИЗ
				|	ВТПериодыОтбораКадровойИстории КАК ПериодыОтбораКадровойИстории
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанные
				|		ПО ПериодыОтбораКадровойИстории.Сотрудник = ТекущиеКадровыеДанные.Сотрудник
				|			И (КОНЕЦПЕРИОДА(ПериодыОтбораКадровойИстории.Период, ДЕНЬ) < ЕСТЬNULL(ТекущиеКадровыеДанные.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1)))
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|УНИЧТОЖИТЬ ВТПериодыОтбораКадровойИстории";
				
			ТекстЗапросаПолученияРазличныхЗаписей = СтрЗаменить(ТекстЗапросаПолученияРазличныхЗаписей,
				"ОтборовРазличныхСотрудников.Сотрудник", "ОтборовРазличныхСотрудников." + ОписательВременнойТаблицыОтборов.ИмяПоляСотрудник);
			
			ТекстЗапросаПолученияРазличныхЗаписей = СтрЗаменить(ТекстЗапросаПолученияРазличныхЗаписей,
				"ОтборовРазличныхСотрудников.Период", "ОтборовРазличныхСотрудников." + ОписательВременнойТаблицыОтборов.ИмяПоляПериод);
			
			ТекстЗапросаПолученияРазличныхЗаписей = СтрЗаменить(ТекстЗапросаПолученияРазличныхЗаписей,
				"ВТОтборовРазличныхСотрудников", ОписательВТОтборов.ИмяВременнойТаблицыОтборовСотрудников);
			
			ОписательВТОтборов.ИмяВременнойТаблицыОтборовСотрудников = "ВТОтборовСУточненнымПериодом";
			
			Запрос.Текст = "
				|ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	ТаблицаОтборов.ПериодСведений,
				|	ТаблицаОтборов.Период,
				|	ТаблицаОтборов.Сотрудник,
				|	ТаблицаОтборов.ФизическоеЛицо
				|ПОМЕСТИТЬ ВТКадровыеДанныеСотрудников
				|ИЗ
				|	ВТОтборовСУточненнымПериодом КАК ТаблицаОтборов";
			
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТОтборовСУточненнымПериодом");
	...
	Если ФормироватьТаблицуРазличных Тогда
		...
		Если ОписательВТОтборов.ПолучатьСведенияНаПериодНачалаУчета Тогда
			...
			ТекстЗапросаПолученияРазличныхЗаписей =
				...
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КадроваяИсторияСотрудников КАК КадроваяИсторияСотрудников
				|		ПО ТаблицаОтборовСотрудников.Сотрудник = КадроваяИсторияСотрудников.Сотрудник
				|			И (ТаблицаОтборовСотрудников.Период <> ДАТАВРЕМЯ(1, 1, 1))
				|			И ТаблицаОтборовСотрудников.Период <= КадроваяИсторияСотрудников.Период
				|			И (КадроваяИсторияСотрудников.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.НачальныеДанные))
				|
				|СГРУППИРОВАТЬ ПО
				...
				|ВЫБРАТЬ РАЗРЕШЕННЫЕ
				|	ВЫБОР
				|		КОГДА ТаблицаОтборов.ПериодКадровойИстории ЕСТЬ NULL 
				|			ТОГДА ТаблицаОтборов.Период
				|		ИНАЧЕ ВЫБОР
				|				КОГДА ЕСТЬNULL(ТекущиеКадровыеДанные.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1)) > ТаблицаОтборов.Период
				|					ТОГДА ТаблицаОтборов.Период
				|				ИНАЧЕ ТаблицаОтборов.ПериодКадровойИстории
				|			КОНЕЦ
				|	КОНЕЦ КАК ПериодСведений,
				|	ТаблицаОтборов.Период,
				|	ТаблицаОтборов.Сотрудник,
				...
				|ИЗ
				|	ВТПериодыОтбораКадровойИстории КАК ТаблицаОтборов
				|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТекущиеКадровыеДанныеСотрудников КАК ТекущиеКадровыеДанные
				|		ПО ТаблицаОтборов.Сотрудник = ТекущиеКадровыеДанные.Сотрудник
				|			И (КОНЕЦПЕРИОДА(ТаблицаОтборов.Период, ДЕНЬ) < ЕСТЬNULL(ТекущиеКадровыеДанные.ДатаПриема, ДАТАВРЕМЯ(1, 1, 1)))";
			...
			КадровыйУчет.ДобавитьВКоллекциюИмяКадровыхДанных(ИсточникиДанных, "ВТКУничтожению", "ВТПериодыОтбораКадровойИстории");
...
Функция НеобходимыСведенияКадровойИсторииСотрудников(ИмяПоля) Экспорт
	...
	Возврат ИмяПоля = ВРег("РабочееМестоПериодРегистрации")
		...
		Или КадровыйУчет.НеобходимыСведенияЗависящиеОтФункциональныхОпцийОрганизации(ИмяПоля);
...
Процедура ДобавитьТекстЗапросаВТСведенияКадроваяИсторияСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ПоляОтбораПериодическихДанных, ИсточникиДанных, ИмяВТСведенияКадровойИсторииСотрудников = Неопределено) Экспорт
	...
	Если ПустаяСтрока(ОписательВременнойТаблицыОтборов.ИменаТаблицСКадровойИсториейСотрудников.ИмяВТКадроваяИсторияСотрудниковТаблицаРегистра) Тогда
		...
		Если ОписательВременнойТаблицыОтборов.ПолучатьСведенияНаПериодНачалаУчета
			Или Не ОписательВременнойТаблицыОтборов.ИменаСозданныхВременныхТаблиц.Свойство("ВТСведенияКадровойИсторииСотрудников", ИмяВременнойТаблицы) Тогда
		...
		Иначе
			...
			ЧастиЗапроса.Добавить(Запрос.Текст);
			ИмяВТСведенияКадровойИсторииСотрудников = ИмяВременнойТаблицы;
...
Функция ТекстыОписанияПолейЗапросПоляКадровыхДанныхФизическихЛиц(КадровыеДанные, ИсточникиДанных, ОписательВТОтборов)
	...
	Для каждого ИмяЗапрашиваемыхДанных Из КадровыеДанные Цикл
		...
		ИначеЕсли ДобавитьПолеСведенийОДокументахУдостоверяющихЛичность(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных)
			...
			Или УчетНДФЛФизическиеЛица.ДобавитьПолеСведенийОСтатусахНалогоплательщиков(ИмяКадровыхДанных, ТекстыОписанияПолей, ИсточникиДанных) Тогда
...
Процедура ДобавитьВЗапросСоединенияСИсточникамиПостоянныхКадровыхДанныхСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных, ФормироватьТаблицуРазличных) Экспорт
	...
	КадровыйУчет.ДобавитьТекстЗапросаТекущихКадровыхДанныхСотрудников(Запрос, ТолькоРазрешенные, ОписательВременнойТаблицыОтборов, ИсточникиДанных);
...
Функция СведенияОСреднемЗаработкеДляСправкиПоБезработице(КадровыеДанныеСотрудников) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РасчетЗарплатыДляНебольшихОрганизаций") Тогда
		...
		МодульРасчетЗарплатыДляНебольшихОрганизаций.СоздатьВТПериодыОтсутсвий(Запрос.МенеджерВременныхТаблиц);
		...
		Запрос.Текст =
			...
			|			И ПериодыРаботыФизическихЛиц.ФизическоеЛицо = НачисленияЗарплаты.ФизическоеЛицо
			|
			|СГРУППИРОВАТЬ ПО
			|	ПериодыРаботыФизическихЛиц.ГоловнаяОрганизация,
			|	ПериодыРаботыФизическихЛиц.ФизическоеЛицо,
			...
			|		ПО ПериодыРаботыФизическихЛиц.ГоловнаяОрганизация = НачисленияЗарплаты.ГоловнаяОрганизация
			|			И ПериодыРаботыФизическихЛиц.ФизическоеЛицо = НачисленияЗарплаты.ФизическоеЛицо
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПериодыОтсутсвий КАК ПериодыОтсутсвий
			|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПроизводственногоКалендаря КАК ДанныеПроизводственногоКалендаряОтсутствий
			|			ПО ПериодыОтсутсвий.ДатаНачала <= ДанныеПроизводственногоКалендаряОтсутствий.Дата
			|				И ПериодыОтсутсвий.ДатаОкончания >= ДанныеПроизводственногоКалендаряОтсутствий.Дата
			...
			|		ПО ПериодыРаботыФизическихЛиц.ГоловнаяОрганизация = ПериодыОтсутсвий.ГоловнаяОрганизация
			|			И ПериодыРаботыФизическихЛиц.ФизическоеЛицо = ПериодыОтсутсвий.ФизическоеЛицо
			|
			|СГРУППИРОВАТЬ ПО
			|	ПериодыРаботыФизическихЛиц.ГоловнаяОрганизация,
			|	ПериодыРаботыФизическихЛиц.ФизическоеЛицо,
...
Функция СведенияОПериодахНеРаботыДляСправкиПоБезработице(КадровыеДанныеСотрудников) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.РасчетЗарплатыДляНебольшихОрганизаций") Тогда
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ПериодыОтсутсвий.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ПериодыОтсутсвий.ДатаНачала КАК ДатаНачала,
			|	ПериодыОтсутсвий.ДатаОкончания,
			|	ПериодыОтсутсвий.СсылкаНаДокумент,
			|	ПериодыОтсутсвий.ПричинаНеРаботы
			|ИЗ
			|	ВТПериодыОтсутсвий КАК ПериодыОтсутсвий
			|
			|УПОРЯДОЧИТЬ ПО
			|	ФизическоеЛицо,
			|	ДатаНачала";
		...
		Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
			...
			СтрокиСотрудников = КадровыеДанныеСотрудников.НайтиСтроки(Новый Структура("ФизическоеЛицо", Выборка.ФизическоеЛицо));
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ТипЗнч(Выборка.СсылкаНаДокумент) = Тип("ДокументСсылка.Отпуск") Тогда
					ПериодНеРаботы.Вставить("ПричинаОтсутствия", НСтр("ru='Отпуск основной'"));
				Иначе
					
					Если Выборка.ПричинаНеРаботы = Перечисления.ПричиныНетрудоспособности.ПоБеременностиИРодам Тогда
						ПериодНеРаботы.Вставить("ПричинаОтсутствия", НСтр("ru='Отпуск по беременности и родам'"));
					ИначеЕсли Выборка.ПричинаНеРаботы = Перечисления.ПричиныНетрудоспособности.ПособиеПриДолечивании Тогда
						ПериодНеРаботы.Вставить("ПричинаОтсутствия", НСтр("ru='Отпуск на санаторно-курортное лечение'"));
					Иначе
						ПериодНеРаботы.Вставить("ПричинаОтсутствия", НСтр("ru='Болезнь'"));
					КонецЕсли;
...
Процедура ЗаполнитьОрганизацииСотрудников(ПараметрыОбновления) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(
			ПараметрыОбновления, "РегистрНакопления.ОрганизацииВКоторыхРаботалиСотрудники.НаборЗаписей", "", "");
