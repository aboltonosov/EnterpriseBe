...
// - НеОбособленныеПодразделения (список обособленных подразделений, не выделенных на отдельный баланс, с указанием всех их потомков до 3 колена)
...
Функция ПодготовитьТаблицуРасчетДолейБазыНалогаНаПрибыль(Знач Реквизиты, СписокОрганизаций) Экспорт
	...
	|ГДЕ
	|	Хозрасчетный.Ссылка В ИЕРАРХИИ(&СчетаАмортизируемогоИмущества)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Счет";
	Запрос.Выполнить();
...
//
// Параметры:
//  Расчет							 - см. НалогНаПрибыльДекларация.НовыйРасчетРаспределенияНалога() - коллекция, в которую помещаются собранные данные
...
Функция ВыполнитьЗапросРасчетНалогаНаПрибыль(Организация, КонецПериода) Экспорт
	...
	|			,
	|			,
	// 4D:ERP для Беларуси, Екатерина, 06.08.2015 11:59:08 
	// Локализация плана счетов, №8969
	// {
	|			СчетКт В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НалогНаПрибыль)),
	// }
	// 4D	
	|			&Субконто68_04_1,
	|			Организация В (&СписокОрганизаций)
	...
	// Локализация плана счетов, №8969
	// {
	Запрос.УстановитьПараметр("СчетаРасчетыСБюджетом", БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты));
	// }
	// 4D
