...
Процедура ПроверитьСтатусУтвержден(Объект, Отказ, РежимЗаписи, ТипПлана) Экспорт 
	...
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПродажПоКатегориям И РольДоступна("УтверждениеПлановПродажПоКатегориям") Тогда
		Возврат;
		//++ НЕ УТ
	ИначеЕсли ТипПлана = Перечисления.ТипыПланов.ПланПроизводства И РольДоступна("УтверждениеПлановПроизводства") Тогда
		Возврат;
		//-- НЕ УТ
...
Процедура ПроверкаКоличестваВидовПланов(Сценарий, ТипПлана, Ссылка, Замещающий, Отказ) Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'В рамках сценария %1 %2 Замещающий вид плана, 
											|других видов планов не должно быть.'");
											
		Если Замещающий Тогда
			ТекстСообщенияПараметр = НСтр("ru = 'может быть создан только один'");
		Иначе
			ТекстСообщенияПараметр = НСтр("ru = 'уже создан'");
		КонецЕсли;
		
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, Сценарий, ТекстСообщенияПараметр);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Сценарий);
	КонецЕсли;
...
//           или суммарный вес коэффициентов = 0), тогда возвращается значение Неопределено.
//
Функция РаспределитьКоличествоПропорциональноКоэффициентамИКратностиСтроки(Знач РаспределяемаяСумма, КоэффициентУпаковки, Коэффициенты, КратностьТовара) Экспорт
	...
	КоличествоКратныхУпаковок= Результат[ИндексПоследнегоКоэффициента] * КоэффициентУпаковки / КратностьТовара;
	Если КоличествоКратныхУпаковок - Цел(КоличествоКратныхУпаковок) > 0 Тогда
		Результат[ИндексПоследнегоКоэффициента] = (Цел(КоличествоКратныхУпаковок) + 1) * КратностьТовара / КоэффициентУпаковки;
	КонецЕсли;
