...
Процедура НастроитьКомандуПоказатьСообщения(Объект, ЭлементФормы, Видимость = Истина) Экспорт
	...
	Иначе
		ЭлементФормы.Заголовок = СтрШаблон(НСтр("ru = 'Показать сообщения (%1)'"), СтруктуруСообщений.Сообщения.Количество());
	КонецЕсли;
...
Процедура НазначитьРучнуюСкидку(Объект, ИмяТЧ, СуммаСкидкиКРаспределению, ВходныеПараметры) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:30:25 
	// Расчет скидки к цене, №10910
	// {
	ПрименятьСкидкиКЦене = ПолучитьФункциональнуюОпцию("ПрименятьСкидкиКЦене");	
	// }
	// 4D
	...
	Для каждого СтрокаТЧ Из СтрокиТабличнойЧасти Цикл
		...
		Для Каждого Ограничение Из Данные.Ограничения Цикл
			...
			// 4D:ERP для Беларуси, Екатерина, 26.04.2016 14:50:18 
			// Расчет скидки к цене, №10910
			// {
			Если ПрименятьСкидкиКЦене Тогда 
				...
				СтрокаТЧ.ЦенаСоСкидкой	= СтрокаТЧ.Цена * (1 - (СтрокаТЧ.ПроцентАвтоматическойСкидки + СтрокаТЧ.ПроцентРучнойСкидки)/100);
				
				СтрокаТЧ.Сумма    = СтрокаТЧ[ИмяКоличества] * СтрокаТЧ.ЦенаСоСкидкой; 
			...
			// 4D:ERP для Беларуси, Екатерина, 26.04.2016 14:50:48 
			// Расчет скидки к цене, №10910
			// {
			КонецЕсли;
			// }
...
Процедура ОчиститьСкидкиВТЧ(Объект, ИмяТЧ, РассчитыватьСуммуСНДС=Ложь, РассчитыватьСуммуВзаиморасчетов=Ложь, ОтменитьТолькоАвтоСкидки = Ложь) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:40:57 
	// Расчет скидки к цене, №10910
	// {
	ПрименятьСкидкиКЦене = ПолучитьФункциональнуюОпцию("ПрименятьСкидкиКЦене");
	// }
	...
	Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
		...
		// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:45:46 
		// Расчет скидки к цене, №10910
		// {
		Если ПрименятьСкидкиКЦене Тогда 
			СтрокаТЧ.ЦенаСоСкидкой = ?(СтрокаТЧ[ИмяКоличества] <> 0, СтрокаТЧ.Сумма/СтрокаТЧ[ИмяКоличества], 0);
		КонецЕсли;
		// }
		// 4D
...
Процедура ОтменитьРучныеСкидки(Объект, ИмяТЧ, ИспользуютсяАвтоматическиеСкидки,
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:40:36 
	// Расчет скидки к цене, №10910
	// {
	ПрименятьСкидкиКЦене = ПолучитьФункциональнуюОпцию("ПрименятьСкидкиКЦене");
	// }
	...
	Для Каждого СтрокаТЧ Из СтрокиТабличнойЧасти Цикл
		...
		Если СтрокаТЧ.СуммаРучнойСкидки <> 0 Или СтрокаТЧ.ПроцентРучнойСкидки <> 0 Тогда
			...
			// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:41:25 
			// Расчет скидки к цене, №10910
			// {
			Если ПрименятьСкидкиКЦене Тогда 
				ПроцентСкидки			= ?(ИспользуютсяАвтоматическиеСкидки, СтрокаТЧ.ПроцентАвтоматическойСкидки, 0);
				СтрокаТЧ.ЦенаСоСкидкой	= СтрокаТЧ.Цена * (1 - ПроцентСкидки/100);
				СтрокаТЧ.Сумма			= СтрокаТЧ[ИмяКоличества] * СтрокаТЧ.ЦенаСоСкидкой;
				СтрокаТЧ.СуммаНДС		= РассчитатьСуммуНДС(СтрокаТЧ.Сумма, СтрокаТЧ.СтавкаНДС, Объект.ЦенаВключаетНДС);
			Иначе
...
Функция РассчитатьПоЗаказуКлиента(Объект, ВходныеПараметры) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:46:53 
	// Расчет скидки к цене, №10910
	// {
	|	Товары.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:49:22 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
	// 4D
	|	Таблица.Цена,
	|	Таблица.Сумма
...
Функция РассчитатьПоЗаявкеНаВозвратТоваровОтКлиента(Объект, ВходныеПараметры) Экспорт
	...
	|	Товары.КоличествоУпаковок КАК КоличествоУпаковок,
	|	Товары.Цена                             КАК Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:42:49 
	// , № 10910
	// {
	|	Товары.ЦенаСоСкидкой      КАК ЦенаСоСкидкой,
	// }
	...
	|	Таблица.КоличествоУпаковок,
	|	Таблица.Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:43:20 
	// , №
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция РассчитатьПоКоммерческомуПредложениюКлиенту(Объект, ВходныеПараметры) Экспорт
	...
	|	Товары.КоличествоУпаковок                           КАК КоличествоУпаковок,
	|	Товары.Цена                                         КАК Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:48:29 
	// Расчет скидки к цене, №10910
	// {
	|	Товары.ЦенаСоСкидкой                                КАК ЦенаСоСкидкой,
	// }
	...
	|	Таблица.КоличествоУпаковок,
	|	Таблица.Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:48:49 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция РассчитатьПоРеализацииТоваровУслуг(Объект, ВходныеПараметры) Экспорт
	...
	|	Товары.Количество         КАК Количество,
	|	Товары.КоличествоУпаковок КАК КоличествоУпаковок,
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:50:39 
	// Расчет скидки к цене, №10910
	// {
	|	Товары.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
	// }
	...
	|	Таблица.Количество,
	|	Таблица.КоличествоУпаковок,
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:51:09 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция РассчитатьПоАктуВыполненныхРабот(Объект, ВходныеПараметры) Экспорт
	...
	ТаблицаТовары = Объект.Услуги.Выгрузить(
		...
		|ВидЦены,
		|Количество,
		// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:18:59 
		// Расчет скидки к цене, №10910
		// {
		|ЦенаСоСкидкой,
		// }
	...
	|	Товары.КоличествоУпаковок   КАК КоличествоУпаковок,
	|	Товары.Цена                             КАК Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:10:34 
	// Расчет скидки к цене, №10910
	// {
	|	Товары.ЦенаСоСкидкой        КАК ЦенаСоСкидкой,
	// }
	...
	|	Таблица.КоличествоУпаковок,
	|	Таблица.Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:11:08 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция РассчитатьПоЧекуККМ(Объект, ВходныеПараметры) Экспорт
	...
	|	Товары.КоличествоУпаковок КАК КоличествоУпаковок,
	|	Товары.Цена                             КАК Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:54:49 
	// Расчет скидки к цене, №10910
	// {
	|	Товары.ЦенаСоСкидкой      КАК ЦенаСоСкидкой,
	// }
	...
	|	Таблица.КоличествоУпаковок,
	|	Таблица.Цена,
	// 4D:ERP для Беларуси, Екатерина, 11.04.2016 15:55:16 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция АдресДанныхДляРасчетаРучныхСкидокВоВременномХранилище(Форма, ИмяТаблицы, УникальныйИдентификатор, ТолькоДляВыделенныхСтрок,
	...
	|	ОграниченияРучныхСкидок.МаксимальныйПроцентРучнойСкидки КАК МаксимальныйПроцентРучнойСкидки,
	|	ОграниченияРучныхСкидок.МаксимальныйПроцентРучнойНаценки КАК МаксимальныйПроцентРучнойНаценки,
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:48:37 
	// Расчет скидки к цене, №10910
	// {
	|	ВЫРАЗИТЬ(ЕстьNULL(ОграниченияРучныхСкидок.МаксимальныйПроцентРучнойСкидки, 0) / 100 * Т.СуммаБезСкидки КАК ЧИСЛО(15, 2)) КАК МаксимальнаяСуммаРучнойСкидки,
	|	ВЫРАЗИТЬ(ЕстьNULL(ОграниченияРучныхСкидок.МаксимальныйПроцентРучнойНаценки, 0) / 100 * Т.СуммаБезСкидки КАК ЧИСЛО(15, 2)) КАК МаксимальнаяСуммаРучнойНаценки
	// }
	// 4D
	|ИЗ
	|	(ВЫБРАТЬ
...
Процедура СохранитьДанныеОВыбранныхУправляемыхСкидках(ТаблицаСкидкиНаценки, ПараметрыРасчета)
	...
	Для Каждого УправляемаяСкидка Из ПараметрыРасчета.УправляемыеСкидки Цикл
		...
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:46:17 
		// Расчет скидки к цене, №10910
		// {
...
Функция ОбъединитьРезультатыРасчетаПодчиненныхСтрок(СтрокаДерева)
	...
	РезультатРасчета.Колонки.Добавить("Расшифровка",               Новый ОписаниеТипов("ТаблицаЗначений"));
	РезультатРасчета.Колонки.Добавить("РеквизитДопУпорядочивания", Новый ОписаниеТипов("Число"));
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:38:02 
	// Расчет скидки к цене, №10910
	// {
	РезультатРасчета.Колонки.Добавить("ПроцентСкидкиНаценки",	   Новый ОписаниеТипов("Число"));
	// }
	// 4D
...
Процедура РассчитатьСкидкиПоГруппеСовместногоПримененияСложениеУмножение(СтрокаДерева, ПараметрыРасчета)
	...
	Для Каждого СтрокаТаблицы Из РезультатРасчета Цикл
		...
		Если СтрокаТаблицы.КлючСвязи <> КлючСвязи Тогда
			...
			// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:38:57 
			// Расчет скидки к цене, №10910
			// {
			НоваяСтрокаТЗ.ПроцентСкидкиНаценки = СтрокаТаблицы.ПроцентСкидкиНаценки;
			// }
			// 4D
		...
		Иначе
			...
			// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:54:37 
			// Расчет скидки к цене, №10910
			// {
			НоваяСтрокаТЗ.ПроцентСкидкиНаценки = НоваяСтрокаТЗ.ПроцентСкидкиНаценки + СтрокаТаблицы.ПроцентСкидкиНаценки;
			// }
			// 4D
			Для Каждого СтрокаРасшифровки Из СтрокаТаблицы.Расшифровка Цикл
				ЗаполнитьЗначенияСвойств(НоваяСтрокаТЗ.Расшифровка.Добавить(), СтрокаРасшифровки);
...
// 4D:УПП для Беларуси, Екатерина, 15.04.2016 17:11:21 
// Расчет скидки к цене, №10910
// {
Функция ПолучитьСуммуСкидки(СтрокаДерева, Товар, ПараметрыРасчета, ПроцентСкидкиНаценки, ЭтоУмножение = Истина) Экспорт
	
	Если ЭтоУмножение Тогда
		ЦенаСоСкидкой = Товар.ЦенаСоСкидкой * (1 - ПроцентСкидкиНаценки / 100);
		Товар.ЦенаСоСкидкой = ЦенаСоСкидкой;
	Иначе
		ЦенаСоСкидкой = Товар.Цена * (1 - ПроцентСкидкиНаценки / 100);
	КонецЕсли;
	
	// 4D:ERP для Беларуси, ВладимирР, 05.06.2017 16:13:18 
	// Критичная ошибка в системе (задача №14443) Скидки
	// {
	ЦенаСоСкидкой = Окр(ЦенаСоСкидкой, 2);
	Сумма = Окр(Окр(Товар.Сумма, 2) - (ЦенаСоСкидкой * Товар.Количество), 2); 
	// }
	// 4D
...
// 4D:ERP для Беларуси, Екатерина, 08.04.2016 16:02:36 
// Расчет скидки к цене, №10910
// {
Функция ПолучитьРасшифровкуСкидки(СтрокаДерева, Сумма, ПараметрыРасчета, ЭтоСообщение = Ложь, ПроцентСкидкиНаценки = 0) Экспорт
// }
// 4D
	...
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 16:01:53 
	// Расчет скидки к цене, №10910
	// {
	СтрокаРасшифровки.ПроцентСкидкиНаценки	= ПроцентСкидкиНаценки;
	// }
	// 4D
...
// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:06:17 
// Расчет скидки к цене, №10910
// {
Процедура ПрименитьЗначениеСкидкиКТовару(СтрокаДерева, СуммаСкидки, Товар, РезультатРасчета, ПараметрыРасчета, ПроцентСкидки = 0) Экспорт
// }
// 4D
	...
	НоваяСтрока.Сумма = КонвертироватьСуммуВБонусы(СтрокаДерева, СуммаСкидки, ПараметрыРасчета);
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:07:34 
	// Расчет скидки к цене, №10910
	// {
	...
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 16:03:53 
	// Расчет скидки к цене, №10910
	// {
	НоваяСтрока.Расшифровка = ПолучитьРасшифровкуСкидки(СтрокаДерева, НоваяСтрока.Сумма, ПараметрыРасчета, НоваяСтрока.ПроцентСкидкиНаценки);
	// }
	// 4D
...
Процедура РассчитатьСкидку(СтрокаДерева, ПараметрыРасчета)
	...
	Если СтрокаДерева.СпособПредоставления = Перечисления.СпособыПредоставленияСкидокНаценок.Процент Тогда
		...
		Для Каждого Товар Из Товары Цикл
			...
			// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:05:33 
			// Расчет скидки к цене, №10910
			// {
			ПрименятьСкидкиКЦене = ПолучитьФункциональнуюОпцию("ПрименятьСкидкиКЦене");

			Если ПрименятьСкидкиКЦене Тогда 
				Сумма = ПолучитьСуммуСкидки(СтрокаДерева, Товар, ПараметрыРасчета, ЗначениеСкидкиНаценки(СтрокаДерева, Товар, ПараметрыРасчета), ЭтоУмножение);
				ПрименитьЗначениеСкидкиКТовару(СтрокаДерева, Сумма, Товар, РезультатРасчета, ПараметрыРасчета, ЗначениеСкидкиНаценки(СтрокаДерева, Товар, ПараметрыРасчета));
			Иначе 	
...
Функция РассчитатьДеревоСкидок(ДеревоСкидок, ПараметрыРасчета)
	...
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:23:10 
	// Расчет скидки к цене, №10910
	// {
	ТЗ.Колонки.Добавить("ПроцентСкидкиНаценки",	Новый ОписаниеТипов("Число"));
	// }
	// 4D
	...
	Если ЕстьРезультат Тогда
		Для Каждого СтрокаТаблицы ИЗ РезультатРасчета Цикл
			Для Каждого СтрокаСкидкиНаценки Из СтрокаТаблицы.Расшифровка Цикл
				...
				// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:23:33 
				// Расчет скидки к цене, №10910
				// {
				НоваяСтрока.ПроцентСкидкиНаценки	= СтрокаСкидкиНаценки.ПроцентСкидкиНаценки;
				// }
				// 4D
		...
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 15:24:01 
		// Расчет скидки к цене, №10910
		// {
		ТЗ.Свернуть("КлючСвязи, СкидкаНаценка, ЭтоСообщение", "Сумма, ПроцентСкидкиНаценки");
		// }
		// 4D
...
Процедура ПрименитьРезультатРасчетаКОбъекту(Объект, 
	...
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 15:53:05 
	// Расчет скидки к цене, №10910
	// {
	ПрименятьСкидкиКЦене = ПолучитьФункциональнуюОпцию("ПрименятьСкидкиКЦене");
	// }
	...
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 18:22:50 
	// Расчет скидки к цене, №10910
	// {
	Если ПрименятьСкидкиКЦене Тогда 
		...
		Для Каждого СтрокаТЧ Из Объект[ИмяТЧ] Цикл
			...
			СтруктураОтбора.Вставить("КлючСвязи", СтрокаТЧ.КлючСвязи);
			СтрокиТаблицы = АвтоматическиеСкидкиНаценки.НайтиСтроки(СтруктураОтбора);
			Если СтрокиТаблицы.Количество() > 0 Тогда
				Для сч = 0 по СтрокиТаблицы.Количество() - 1 Цикл 
					СтрокаТаблицы = СтрокиТаблицы[сч];
					Если СтрокаТаблицы.ПроцентСкидкиНаценки <> 0 Тогда 
						ПроцентАвтоматическойСкидки	= ПроцентАвтоматическойСкидки + СтрокаТаблицы.ПроцентСкидкиНаценки;
					Иначе 
						СуммаАвтоматическойСкидки = СуммаАвтоматическойСкидки + СтрокаТаблицы.Сумма;
					КонецЕсли;	
				КонецЦикла;	
			КонецЕсли;
			...
			//сначала рассчитаем скидки процентом
			Если ПроцентАвтоматическойСкидки <> 0 Тогда
				
				ПроцентСкидки = ПроцентАвтоматическойСкидки + СтрокаТЧ.ПроцентРучнойСкидки;
				СтрокаТЧ.ЦенаСоСкидкой	= СтрокаТЧ.Цена * (1 - ПроцентСкидки/100);
				
				СтрокаТЧ.Сумма    = СтрокаТЧ.ЦенаСоСкидкой*СтрокаТЧ[ИмяКоличества];
				СтрокаТЧ.СуммаНДС = РассчитатьСуммуНДС(СтрокаТЧ.Сумма, СтрокаТЧ.СтавкаНДС, Объект.ЦенаВключаетНДС);
				
				СтрокаТЧ.ПроцентАвтоматическойСкидки	= ПроцентАвтоматическойСкидки;
				СтрокаТЧ.СуммаАвтоматическойСкидки		= СтрокаТЧ.Цена*СтрокаТЧ[ИмяКоличества] - СтрокаТЧ.Сумма - СтрокаТЧ.СуммаРучнойСкидки;
				
			КонецЕсли;	
			
			Если СуммаАвтоматическойСкидки <> 0 Тогда 
				
				// 4D:ERP для Беларуси, ВладимирР, 02.03.2017 12:20:07 
				// Критичная ошибка в системе, №14443
				// {
	...
	// 4D:ERP для Беларуси, Екатерина, 08.04.2016 18:23:32 
	// Расчет скидки к цене, №10910
	// {
	КонецЕсли;
	// }
...
Функция ДанныеПоТоварамИНаборам(ТекстЗапроса, ПараметрыЗапроса, ИспользоватьНаборы)
	...
	|		0
	|	КОНЕЦ КАК Цена,
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:26:17 
	// Расчет скидки к цене, №10910
	// {
	|	ВЫБОР КОГДА Таблица.КоличествоУпаковок <> 0 ТОГДА
	|		Таблица.Сумма / Таблица.КоличествоУпаковок
	...
	|	Таблица.КоличествоУпаковок,
	|	Таблица.Цена,
	// 4D:ERP для Беларуси, Екатерина, 09.02.2016 16:27:43 
	// Расчет скидки к цене, №10910
	// {
	|	Таблица.ЦенаСоСкидкой,
	// }
...
Функция ПустаяТаблицаСкидокСРасшифровкой(ПараметрыРасчета = Неопределено) Экспорт
	...
	Если ПараметрыРасчета = Неопределено Тогда
		...
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 14:53:03 
		// Расчет скидки к цене, №10910
		// {
		Таблица.Колонки.Добавить("ПроцентСкидкиНаценки",	Новый ОписаниеТипов("Число"));
		// }
		// 4D
		Возврат Таблица;
	Иначе
...
Функция ПустаяТаблицаРасшифровка(ПараметрыРасчета = Неопределено)
	...
	Если ПараметрыРасчета = Неопределено Тогда
		...
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 14:53:34 
		// Расчет скидки к цене, №10910
		// {
		Таблица.Колонки.Добавить("ПроцентСкидкиНаценки",	Новый ОписаниеТипов("Число"));
		// }
...
Функция ПустаяТаблицаСкидкиНаценки(ПараметрыРасчета = Неопределено)
	...
	Если ПараметрыРасчета = Неопределено Тогда
		...
		Таблица.Колонки.Добавить("СкидкаНаценка", Новый ОписаниеТипов("СправочникСсылка.СкидкиНаценки"));
		Таблица.Колонки.Добавить("Сумма",         Новый ОписаниеТипов("Число"));
		// 4D:ERP для Беларуси, Екатерина, 08.04.2016 14:52:29 
		// Расчет скидки к цене, №10910
		// {
		Таблица.Колонки.Добавить("ПроцентСкидкиНаценки",	Новый ОписаниеТипов("Число"));
		// }
