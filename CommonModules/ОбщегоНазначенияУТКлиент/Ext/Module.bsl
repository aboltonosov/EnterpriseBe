...
//
// Параметры:
//  ИмяКоманды	 - Строка - имя команды, до этого программно сформированное, вида ПриемНаКомиссию_ПоступлениеТоваровУслуг,
//  СтруктураОтбора	- Структура - структура с отборами для заполнения в документе.
//
...
// Открывает форму рабочего места (обработки)
//
// Параметры:
//  ИмяРабочегоМеста			 - 	Строка - имя обработки, в которой реализовано рабочее место
//  ПараметрыВыполненияКоманды	 - 	Структура - стуктура параметров команды
//
Процедура ОткрытьФормуРабочегоМеста(ИмяРабочегоМеста, ПараметрыВыполненияКоманды) Экспорт
	
	ВремМассив = СтрРазделить(ПараметрыВыполненияКоманды.НавигационнаяСсылка,".");
	ИмяКоманды = ВремМассив[ВремМассив.ВГраница()];
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("КлючНазначенияФормы", ИмяКоманды);
	ОткрытьФорму("Обработка." + ИмяРабочегоМеста + ".Форма",
		ПараметрыФормы, 
...
Процедура ПриОпределенииВозможностиОбновления() Экспорт
	...
	//++ НЕ УТ
	МинимальноДопустимаяВерсия = "2.0.2"; // для КА2
	//-- НЕ УТ
	//++ НЕ УТКА
	МинимальноДопустимаяВерсия = "2.1.2"; // для УП2
	//-- НЕ УТКА
...
Процедура ПоказатьВопросНеВыбраноРаспоряжение(Параметры)
	...
	ТекстВопроса = НСтр("ru = 'Не выбрано распоряжение для оформления документа.
                        |Для создания документа без распоряжения необходимо перейти в журнал ""%СинонимЖурнала"".'");
	ТекстВопроса = СтрЗаменить(ТекстВопроса,"%СинонимЖурнала",Параметры.СинонимЖурнала);
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить(КодВозвратаДиалога.ОК,НСтр("ru = 'Перейти в журнал'"));
	Кнопки.Добавить(КодВозвратаДиалога.Отмена);
	ПоказатьВопрос(ОписаниеОповещения,ТекстВопроса,Кнопки,,КодВозвратаДиалога.Отмена,НСтр("ru = 'Не выбрано распоряжение'"));
