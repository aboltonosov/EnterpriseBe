...
Функция ПечатьЭтикетокДоставки(ОписаниеКоманды) Экспорт
	...
	НеДопустимыеОбъектовПечати = Новый Массив;
	
	Если ОписаниеКоманды.ТипыОбъектовПечати.Количество() > 0 Тогда
		Для Каждого Объект Из ОписаниеКоманды.ОбъектыПечати Цикл
			Если ОписаниеКоманды.ТипыОбъектовПечати.Найти(ТипЗнч(Объект)) = Неопределено Тогда
				НеДопустимыеОбъектовПечати.Добавить(Объект);
			Иначе
	...
	Для Каждого Объект Из НеДопустимыеОбъектовПечати Цикл
		ТекстСообщения = НСтр("ru = 'Печать документа ""%Распоряжение%"" не выполнена: выбранная печатная форма недоступна.'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Распоряжение%", Объект);
...
Процедура ПечатьПереоценкаВРозницеЗавершение(ПараметрыПечати, ДополнительныеПараметры) Экспорт 
	...
	Если ПараметрыПечати <> Неопределено Тогда
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(
			...
			"ПереоценкаТоваровВРознице_Локализация",
			ОписаниеКоманды.ОбъектыПечати,
			ОписаниеКоманды.Форма,
			ПараметрыПечати);
...
// Выводит печатную форму описи-акта.
//
// Параметры:
//	ОписаниеКоманды - Структура - структура с описанием команды.
//
...
//	Неопределено
//
Функция ПечатьОписьАкт(ОписаниеКоманды, АдресДанныеДляПечатиВоВременномХранилище = Неопределено) Экспорт
...
//  ДополнительныеПараметры	 - 	Структура - структура дополнительных параметров оповещения
//
Процедура ПечатьОписьАктЗавершение(ПараметрыПечати, ДополнительныеПараметры) Экспорт 
...
Процедура ВыполнитьКомандуПечатиПечатьТТН(КодВозврата, ОписаниеКоманды) Экспорт 
	...
	// 4D:ERP для Беларуси, Дмитрий, 05.12.2014 15:32:05 
	// Локализация печатных форм, №7762
	// {
	Если ОписаниеКоманды.Идентификатор = "ТТН_Горизонтальная" Тогда
		МенеджерПечати = "Обработка.ПечатьТТН";
	...
	ИначеЕсли ОписаниеКоманды.Идентификатор = "ТТН" Тогда
		МенеджерПечати = "Обработка.ПечатьТТН";
		ИменаМакетов = "ТТН";	
	Иначе
		МенеджерПечати = "Обработка.ПечатьТранспортнойНакладной";
