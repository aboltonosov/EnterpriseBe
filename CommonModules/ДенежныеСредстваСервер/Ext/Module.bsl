...
Процедура УстановитьПараметрыВыбораДоговораКредитаДепозита(Объект, ЭлементРасшифровкаПлатежаДоговор, ОчиститьДоговора = Ложь) Экспорт
	...
	|	И ДанныеДоговора.ПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВРублях)");
...
//
// Параметры:
//    Банк - СправочникСсылка.КлассификаторБанковРФ - Банк
...
Функция ПолучитьСписокБанковПоРеквизитам(Поле, Значение, Банк, БИК, КоррСчет) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 06.06.2017 11:01:11 
	// IBAN
	// {
	Если (ЭтоБИК И СтрДлина(Значение) = 8)
	 ИЛИ (ЭтоБИК И СтрДлина(Значение) = 11)
	// }
	// 4D
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	|	Заявка.БанковскийСчетПолучатель КАК БанковскийСчетПолучатель,
	...
	|	Заявка.СтатьяКалькуляции КАК СтатьяКалькуляции,
	...
	|
	// 4D:ERP для Беларуси, Петр, 14.12.2017 12:03:06 
	// В типовую конфигурацию ERP (для казначейства) необходимо добавить справочник с кодами платежей, № 15932
	// {
	|	Заявка.КодПлатежаВБюджет КАК КодПлатежаВБюджет
	// }
	// 4D
	|
	|
	...
	// В типовую конфигурацию ERP (для казначейства) необходимо добавить справочник с кодами платежей, № 15932
	// {
	Если ЗначениеЗаполнено(Выборка.КодПлатежаВБюджет) Тогда
		ДанныеЗаполнения.Вставить("КодПлатежаВБюджет", Выборка.КодПлатежаВБюджет);
	КонецЕсли;
	// }
	// 4D
...
Процедура ЗаполнитьПоОстаткамЗаявокНаРасходованиеДенежныхСредств(ДанныеОтбора, ТабличнаяЧасть) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = ТабличнаяЧасть.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
		НоваяСтрока.ПредставлениеЗаявки = СтрШаблон(НСтр("ru=' %1 от %2'"), Выборка.НомерЗаявки, Формат(Выборка.ДатаЗаявки, "ДЛФ=D"));
	КонецЦикла;
...
// Процедура заполняет подтверждающие документы в договоре или заявке на расходование ДС
//
// Параметры:
//	Объект - ДанныеФормыСтруктура - Для перечисленных типов
...
//		ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств - объект, в котором требуется заполнить табличную часть
//	ПодтверждающиеДокументы - ДанныеФормыКоллекция - заполняемая таблица
//
Процедура ЗаполнитьПодтверждающиеДокументы(Объект, ПодтверждающиеДокументы, Госконтракт = Неопределено) Экспорт
	
	ЭтоДоговор = ТипЗнч(Объект.Ссылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
		Или ТипЗнч(Объект.Ссылка) = Тип("СправочникСсылка.ДоговорыКредитовИДепозитов")
		Или ТипЗнч(Объект.Ссылка) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями");
	
	ПравоЧтениеДоговорыКонтрагентов         = ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКонтрагентов);
	ПравоЧтениеДоговорыКредитовИДепозитов   = ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыКредитовИДепозитов);
	ПравоЧтениеДоговорыМеждуОрганизациями   = ПравоДоступа("Чтение", Метаданные.Справочники.ДоговорыМеждуОрганизациями);
	
	ТекстЗапросДоговорыЗаявки = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РасшифровкаПлатежа.Заказ КАК Ссылка
	...
	|		ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеНаВозмещениеЗатратЗаСчетСобственныхСредств))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РасшифровкаПлатежа.ДоговорКредитаДепозита КАК Ссылка
	|ПОМЕСТИТЬ ДоговорыКредитовДепозитов
	|ИЗ
	|	&РасшифровкаПлатежа КАК РасшифровкаПлатежа
	|ГДЕ
	|	НЕ &ЭтоДоговор
	...
	|ПОМЕСТИТЬ ДоговорыСЗаказчиками
	|ИЗ
	...
	|		ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.ПеречисленияПрибылиИсполнителю), 
	|		ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеНаВозмещениеЗатратЗаСчетСобственныхСредств),
	|		ЗНАЧЕНИЕ(Справочник.ТипыПлатежейФЗ275.СписаниеСуммКредитовПеречисленныхУполномоченнымБанком))
	|	И ДоговорыКонтрагентов.ГосударственныйКонтракт = &Госконтракт
	...
	|
	|ВЫБРАТЬ
	|	ДоговорыКредитовДепозитов.Ссылка
	|ИЗ
	|	ДоговорыКредитовДепозитов КАК ДоговорыКредитовДепозитов
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	ДоговорыСЗаказчиками КАК ДоговорыСЗаказчиками
	|;";
	
	ТекстЗапросаДоговорыКонтрагентов = "
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ПодтверждающиеДокументы.Сумма КАК Сумма
	|ИЗ
	|	ДоговорыЗаявки КАК ДоговорыЗаявки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов.ПодтверждающиеДокументы КАК ПодтверждающиеДокументы
	|		ПО (ПодтверждающиеДокументы.Ссылка = ДоговорыЗаявки.Ссылка)
	|";
	
	ТекстЗапросаДоговорыКредитовИДепозитов = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПодтверждающиеДокументы.ВидДокумента,
	|	ПодтверждающиеДокументы.Файл,
	|	ПодтверждающиеДокументы.Номер,
	|	ПодтверждающиеДокументы.Дата,
	|	ПодтверждающиеДокументы.Сумма
	|ИЗ
	|	ДоговорыЗаявки КАК ДоговорыЗаявки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыКредитовИДепозитов.ПодтверждающиеДокументы КАК ПодтверждающиеДокументы
	|		ПО (ПодтверждающиеДокументы.Ссылка = ДоговорыЗаявки.Ссылка)
	|";
	
	ТекстЗапросаДоговорыМеждуОрганизациями = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПодтверждающиеДокументы.ВидДокумента,
	|	ПодтверждающиеДокументы.Файл,
	|	ПодтверждающиеДокументы.Номер,
	...
	|ИЗ
	|	ДоговорыЗаявки КАК ДоговорыЗаявки
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ДоговорыМеждуОрганизациями.ПодтверждающиеДокументы КАК ПодтверждающиеДокументы
	|		ПО (ПодтверждающиеДокументы.Ссылка = ДоговорыЗаявки.Ссылка)
	|";

	ТекстЗапросТипыПлатежей = "
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТипыПлатежей.ВидДокумента КАК ВидДокумента,
	|	NULL КАК Файл,
	|	NULL КАК Номер,
	|	NULL КАК Дата,
	|	NULL КАК Сумма
	...
	ЧастиЗапросаСУчетомПравЧтения = Новый Массив;
	ТекстЗапросаПоЗаявкам = "";
	
	Если Не ЭтоДоговор Тогда
		ТекстЗапросаПоЗаявкам = ТекстЗапросаПоЗаявкам + ТекстЗапросДоговорыЗаявки;
		
		Если ПравоЧтениеДоговорыКонтрагентов Тогда
			ЧастиЗапросаСУчетомПравЧтения.Добавить(ТекстЗапросаДоговорыКонтрагентов);
		КонецЕсли;
		
		Если ПравоЧтениеДоговорыКредитовИДепозитов Тогда
			ЧастиЗапросаСУчетомПравЧтения.Добавить(ТекстЗапросаДоговорыКредитовИДепозитов);
		КонецЕсли;
		
		Если ПравоЧтениеДоговорыМеждуОрганизациями Тогда
			ЧастиЗапросаСУчетомПравЧтения.Добавить(ТекстЗапросаДоговорыМеждуОрганизациями);
		КонецЕсли;
		
		ТекстЗапросаСУчетомПравЧтения = СтрСоединить(ЧастиЗапросаСУчетомПравЧтения, Символы.ПС + "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС);
		
		ТекстЗапросаПоЗаявкам = ТекстЗапросаПоЗаявкам + ТекстЗапросаСУчетомПравЧтения;
		
		ЧастиЗапроса.Добавить(ТекстЗапросаПоЗаявкам);
	КонецЕсли;
	ЧастиЗапроса.Добавить(ТекстЗапросТипыПлатежей);
	...
	Запрос.УстановитьПараметр("ЭтоДоговор", ЭтоДоговор);
...
Функция ТекстСуммаНДСПлатежа(ВалютаДокумента, РасшифровкаПлатежа, ДобавлятьТекстВТомЧисле) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.СтавкаНДС) ИЛИ СтрокаТаблицы.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС Тогда
			Продолжить;
		КонецЕсли;
		ТекстНазначениеПлатежаНДС = ТекстНазначениеПлатежаНДС
			+ ?(ПустаяСтрока(ТекстНазначениеПлатежаНДС), "", ", ")
			+ НСтр("ru = 'Ставка НДС'") + " " + СтрокаТаблицы.СтавкаНДС + " "
			+ НСтр("ru = 'Сумма НДС'") +
			+ Формат(СтрокаТаблицы.СуммаНДС, "ЧЦ=15; ЧДЦ=2; ЧГ=3,0") + " " + ПредставлениеВалюты;
	КонецЦикла;
		// }
		// 4D
...
//
// Параметры:
//   Ссылка - ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств, ДокументСсылка.СписаниеБезналичныхДенежныхСредств - проверяемый документ
...
//
// Параметры:
//   МассивСсылок - ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств, ДокументСсылка.СписаниеБезналичныхДенежныхСредств - проверяемые документы
//
// Возвращаемое значение:
//   ДанныеДокументовДляКонтроля - Массив - массив результатов запросов для контроля документов за исключением временных таблиц и удалений временных таблиц
//
...
//
Функция ТекстЗапросаПровекиОтдельногоСчета(ИсточникБанковскогоСчета) Экспорт
...
// Проверяет корретность заполнения подтверждающих документов
//
// Параметры:
...
// Процедура формирования движений по последовательности "Переоценка денежных средств".
//
// Параметры:
//	ДополнительныеСвойства - Структура - Свойства проведения
//	ПринадлежностьПоследовательностям - Структура - Значения принадлежности последовательностям
//	Отказ - Булево - Признак отказа от проведения документа
//
...
Процедура ЗарегистрироватьПеречислениеНалоговИВзносов(МенеджерВременныхТаблиц = Неопределено, Объект = Неопределено, Отказ = Ложь) Экспорт
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// 4D:ERP для Беларуси, Екатерина, 30.06.2015 9:50:40 
	// Локализация плана счетов, №8969
	// Локализовать документ Списание безналичных ДС, № 16881
	// {
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПФР),
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудников.ПенсионныйФонд)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ФСС),
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОбязательногоСтрахованияСотрудников.СС)
	// }
	// 4D
	|;
	|
	...
	// 4D:ERP для Беларуси, Петр, 31.01.2018 17:03:52 
	// Локализовать документ Списание безналичных ДС, № 16881
	// {
	//|	Реквизиты.Субконто1 КАК ВидПлатежа,
	//|	Реквизиты.Субконто2 КАК НалоговыйОрган,
	|	ПорядокОтражения.Субконто1 КАК ВидПлатежа,
	|	ПорядокОтражения.Субконто2 КАК НалоговыйОрган,
	// }
	// 4D
	
	|	ПорядокОтражения.Сумма КАК Сумма,
	|	Реквизиты.Номер КАК Номер,
	|	Реквизиты.ДатаПроведенияБанком КАК Дата,
	...
	// 4D:ERP для Беларуси, Петр, 31.01.2018 17:16:06 
	// Локализовать документ Списание безналичных ДС, № 16881
	// {
	//|		ПО Реквизиты.СчетУчета = ВтСчета.СчетУчета
	|		ПО ПорядокОтражения.СчетУчета = ВтСчета.СчетУчета
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Петр, 31.01.2018 17:17:18 
	// Локализовать документ Списание безналичных ДС, № 16881
	// {
	//|	И Реквизиты.Субконто1 В(&МассивНазначенийПлатежа)
	|	И ПорядокОтражения.Субконто1 В(&МассивНазначенийПлатежа)
	// }
	// 4D
...
Процедура СформироватьВозмещениеРасходовПоСоциальномуСтрахованию(МенеджерВременныхТаблиц = Неопределено, Объект = Неопределено, Отказ = Ложь) Экспорт
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// 4D:ERP для Беларуси, Дмитрий, 06.01.2016 10:38:00 
	// Локализация плана счетов, №8969
	// {
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ФСС),
...
// 4D:ERP для Беларуси, Петр, 13.09.2017 15:23:04 
// ERP 2.1.3 Ошибки, №16021 
// {
// }
// 4D
//   Строка
...
Функция НаименованиеОрганизации(Знач Наименование) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 11.12.2017 9:20:42 
	// Проверка контрагентов, №14822 
	// {
	ИначеЕсли УбратьИзНаименованияОрганизационнуюФорму(Наименование, НСтр("ru='Частное предприятие'")) Тогда
		ОрганизационноПравоваяФорма				= НСтр("ru='Частное предприятие'");
		ОрганизационноПравоваяФормаСокращенно	= НСтр("ru='ЧП'");
	ИначеЕсли УбратьИзНаименованияОрганизационнуюФорму(Наименование, НСтр("ru='Унитарное предприятие'")) Тогда
		ОрганизационноПравоваяФорма				= НСтр("ru='Унитарное предприятие'");
		ОрганизационноПравоваяФормаСокращенно	= НСтр("ru='УП'");
	// }
	// 4D
		...
		Если НайденныеСтроки.Количество() >= 1 Тогда
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
				...
				Если НайденнаяСтрока.СуммаДокумента + НайденнаяСтрока.СуммаФакт > НайденнаяСтрока.СуммаПлан Тогда
					...
					Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
						Формат(НайденнаяСтрока.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.СуммаФакт, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.СуммаПлан, "ЧЦ=15; ЧДЦ=2; ЧН="));
					ОписаниеМассивСтрок.Добавить(Описание);
				КонецЕсли;
			КонецЦикла;
...
Процедура РезультатКонтроля_Раздел1_275_83_1_2_з(РезультатыПроверок, ИндексыРезультатовЗапросов, Ссылка, Знач ДанныеДокументовДляКонтроля)
	...
	Если Ссылка.ТипПлатежаФЗ275 <> Справочники.ТипыПлатежейФЗ275.ОплатаИныхРасходовНаСуммуНеБолееТрехМиллионовРублейВМесяц Тогда
		Описание = НСтр("ru= 'Контроль не требуется для типа платежа 275-ФЗ ""%1"".'");
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, Ссылка.ТипПлатежаФЗ275);
		ОписаниеМассивСтрок.Добавить(Описание);
	Иначе
		...
		Если НайденныеСтроки.Количество() >= 1 Тогда
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
				...
				Если НайденнаяСтрока.СуммаДокумента + НайденнаяСтрока.СуммаРасходовВТекущемМесяце > 3000000 Тогда
					...
					Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
						Формат(НайденнаяСтрока.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.СуммаРасходовВТекущемМесяце, "ЧЦ=15; ЧДЦ=2; ЧН="));
					ОписаниеМассивСтрок.Добавить(Описание);
				КонецЕсли;
...
Функция ТекстЗапросаРезультатКонтроля_Раздел1_275_83_1_2_е()
	...
	|	ДоговорыСписаний.Договор,
	|	ДоговорыКонтрагентовКалькуляцияЗатрат.СуммаКВозмещению
	|ПОМЕСТИТЬ ДоговорыСписаний
	|ИЗ
	|	(ВЫБРАТЬ ПЕРВЫЕ 1
	|		РасшифровкаПлатежа.Ссылка КАК Ссылка,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов.КалькуляцияЗатрат КАК ДоговорыКонтрагентовКалькуляцияЗатрат
	|		ПО ДоговорыСписаний.Договор = ДоговорыКонтрагентовКалькуляцияЗатрат.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
		...
		Если НайденныеСтроки.Количество() >= 1 Тогда
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если НайденнаяСтрока.СуммаДокумента > НайденнаяСтрока.ОстатокКредита Тогда
					...
					Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
						Формат(НайденнаяСтрока.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.ОстатокКредита, "ЧЦ=15; ЧДЦ=2; ЧН="));
					ОписаниеМассивСтрок.Добавить(Описание);
				КонецЕсли;
...
Процедура РезультатКонтроля_Раздел1_275_84_10(РезультатыПроверок, ИндексыРезультатовЗапросов, Ссылка, Знач ДанныеДокументовДляКонтроля)
	...
	Если Ссылка.ТипПлатежаФЗ275 <> Справочники.ТипыПлатежейФЗ275.СписаниеСвязаннноеСРазмещениемНаДепозитеУполномоченногоБанка Тогда
		Описание = НСтр("ru= 'Контроль не требуется для типа платежа 275-ФЗ ""%1"".'");
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, Ссылка.ТипПлатежаФЗ275);
		ОписаниеМассивСтрок.Добавить(Описание);
	Иначе
...
Процедура РезультатКонтроля_Раздел1_275_85_1_3(РезультатыПроверок, ИндексыРезультатовЗапросов, Ссылка, Знач ДанныеДокументовДляКонтроля)
	...
	Если НайденныеСтроки.Количество() >= 1 Тогда
		...
		Если СуммаДокумента + СуммаФакт > СуммаКонтрактов Тогда
			...
			Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
				Формат(СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
				Формат(СуммаФакт, "ЧЦ=15; ЧДЦ=2; ЧН="), 
				Формат(СуммаКонтрактов, "ЧЦ=15; ЧДЦ=2; ЧН="));
			ОписаниеМассивСтрок.Добавить(Описание);
		КонецЕсли;
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
				Если НайденнаяСтрока.СуммаДокумента + НайденнаяСтрока.СуммаФакт > 0.5 * НайденнаяСтрока.СуммаКонтракта Тогда
					...
					Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
						Формат(НайденнаяСтрока.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.СуммаФакт, "ЧЦ=15; ЧДЦ=2; ЧН="), 
						Формат(НайденнаяСтрока.СуммаКонтракта, "ЧЦ=15; ЧДЦ=2; ЧН="));
					ОписаниеМассивСтрок.Добавить(Описание);
				КонецЕсли;
			КонецЦикла;
	...
	Если НайденныеСтроки.Количество() >= 1 Тогда
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
			Если Макс(НайденнаяСтрока.СуммаЗаявок, НайденнаяСтрока.СуммаОплат) > НайденнаяСтрока.ПодтверждающийДокументСумма Тогда
				...
				Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Описание, 
					Формат(НайденнаяСтрока.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН="), 
					Формат(НайденнаяСтрока.СуммаЗаявок, "ЧЦ=15; ЧДЦ=2; ЧН="), 
					Формат(НайденнаяСтрока.СуммаОплат, "ЧЦ=15; ЧДЦ=2; ЧН="), 
					Формат(НайденнаяСтрока.ПодтверждающийДокументСумма, "ЧЦ=15; ЧДЦ=2; ЧН="), 
					ПредставлениеДокумента);
				ОписаниеМассивСтрок.Добавить(Описание);
			КонецЕсли;
		КонецЦикла;
...
Процедура УстановитьПараметрыВыбораСтавкиНДС(Элемент) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 10.09.2017 15:04:15 
	// Ставки НДС 20% и 25%
	// {
	ИсключаемыеЗначения.Добавить(Перечисления.СтавкиНДС.НДС20_120);
	ИсключаемыеЗначения.Добавить(Перечисления.СтавкиНДС.НДС25_125);
	// }
	// 4D
...
Функция РасшифровкаПлатежаНДС(ДатаДокумента, ВалютаДокумента, ОбъектыРасчетов, ОплатаОтКлиента = Ложь) Экспорт
	...
	// Ставки НДС 20% и 25%
	// {
	|	КОГДА ТаблицаСуммНДС.СтавкаНДС В (&СтавкаНДС20) ТОГДА
	|		ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	|	КОГДА ТаблицаСуммНДС.СтавкаНДС В (&СтавкаНДС25) ТОГДА
	...
	|	КОГДА ТаблицаСуммНДС.СтавкаНДС В (&СтавкаНДС10) ТОГДА
	|		ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10)
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 11:01:20 
	// Ставки НДС 20% и 25%
	// {
	|	КОГДА ТаблицаСуммНДС.СтавкаНДС В (&СтавкаНДС20) ТОГДА
	|		ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 11:02:48 
	// Ставки НДС 20% и 25%
	// {
	...
	СтавкаНДС25 = Новый Массив;
	СтавкаНДС25.Добавить(Перечисления.СтавкиНДС.НДС25);
	СтавкаНДС25.Добавить(Перечисления.СтавкиНДС.НДС20_120);
	
	Запрос.УстановитьПараметр("СтавкаНДС20", СтавкаНДС20);
	Запрос.УстановитьПараметр("СтавкаНДС25", СтавкаНДС25);
	// }
	// 4D
...
Процедура ЗаполнитьНДСВРасшифровке(РасшифровкаПлатежа, ТаблицаНДС) Экспорт
	...
	Для каждого Платеж Из Платежи Цикл
		...
		Если ТипЗнч(Платеж.Ссылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			...
			// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 11:04:50 
			// Ставка НДС 20%
			// {
			ИначеЕсли Платеж.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 Тогда
				Платеж.СуммаНДС = Платеж.Сумма * 20 / 120;
			ИначеЕсли Платеж.СтавкаНДС = Перечисления.СтавкиНДС.НДС25 Тогда
				Платеж.СуммаНДС = Платеж.Сумма * 25 / 125;
			// }
			// 4D
			КонецЕсли;
		Иначе
...
Процедура ЗаполнитьТекстНазначенияПлатежа(Объект) Экспорт
	...
	Иначе
		...
		// 4D:ERP для Беларуси, Петр, 27.11.2017 10:56:36 
		// Списание безналичных ДС, №16657
		// {
		ТекстСумма = "Сумма " + Формат(Объект.СуммаДокумента, "ЧЦ=15; ЧДЦ=2; ЧН=0,00; ЧГ=");
		// }
		// 4D
...
Функция НазначениеПлатежа(Объект)
	...
	// Локализация печатных форм, №7762
	// {
	Если РеквизитыВалюты.Код = "933" Тогда
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.Документ) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
			...
			// 4D:ERP для Беларуси, Петр, 27.11.2017 10:54:52 
			// Списание безналичных ДС, №16657
			// {
			...
			+ " " + Формат(Выборка.Сумма, "ЧЦ=15; ЧДЦ=2; ЧН=0,00; ЧГ=")
			+ " " + ВалютаДляПечати;
			// }
			// 4D
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ВидДокумента) Тогда
				...
				// 4D:ERP для Беларуси, Петр, 27.11.2017 10:58:02 
				// Списание безналичных ДС, №16657
				// {
	...
	Если СуммаКРаспределению > 0 Тогда
		...
		// 4D:ERP для Беларуси, Петр, 27.11.2017 10:57:22 
		// Списание безналичных ДС, №16657
		// {
		ТекстНазначенияПлатежа = ТекстНазначенияПлатежа + ", "
			+ НСтр("ru = 'без указания назначения'")
			+ " " + Формат(СуммаКРаспределению, "ЧЦ=15; ЧДЦ=2; ЧН=0,00; ЧГ=")
			+ " " + ВалютаДляПечати;
		// }
		// 4D
...
Функция ПолучитьПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("ПоступлениеТоваровУслуг",			   НСтр("ru='накладной'"));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеУслугПрочихАктивов",		   НСтр("ru='акту'"));
...
Функция ДокументыКЗаполнениюНДС(ИмяДокумента = Неопределено) Экспорт
	...
	|
	|ГДЕ
	|	Документ.СтавкаНДС В (&СтавкиНДС)
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	|ВЫБРАТЬ
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 11:06:22 
	// Ставки НДС 20% и 25%
	// {
