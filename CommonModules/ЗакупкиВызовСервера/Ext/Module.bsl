...
Функция СформироватьДанныеЗаполненияПоступления(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	ВЫБОР КОГДА ЗаказПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден)
		...
		|		  ЗаказПоставщику.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Подтвержден)
	...
	Запрос.УстановитьПараметр("ОписаниеПоступленияТУ", НСтр("ru = 'поступления товаров и услуг'"));
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ОписаниеПоступленияСД", НСтр("ru = 'поступления сырья от давальца'"));
	//-- НЕ УТКА
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	...
	Если Отказ Тогда
		...
		ТекстОшибки = НСтр("ru='Ввод одного %ОписаниеПоступления% на основании выделенных распоряжений невозможен'");
