...
Процедура ИсправлениеПериодическихСведений(Источник, Отказ, РежимПроведения, СсылкаДокументаИсправления = Неопределено, ИсправленныйДокумент = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(Источник.Ссылка) Тогда
		...
		|	#ТаблицаДокумента КАК Документы
		|ГДЕ
		|	Документы.ИсправленныйДокумент = &Ссылка
		|	И Документы.Проведен";
		...
		Выборка = Запрос.Выполнить().Выбрать();
		Если Выборка.Следующий() Тогда
			ПредставлениеИсправленного = "№" + Источник.Номер + " (" + Источник.Дата + ")";
			ПредставлениеИсправления = "№" + Выборка.Номер + " (" + Выборка.Дата + ")";
			ВызватьИсключение 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Документ %1 уже исправлен документом %2. Проведение невозможно.'"),
	...
	Если ЗначениеЗаполнено(ИсправленныйДокумент) Тогда
		...
		Если МетаданныеДокумента.Реквизиты.Найти("ИсправленныйДокумент") <> Неопределено Тогда
			...
			Если Выборка.Следующий() Тогда
				...
				ВызватьИсключение 
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Документ %1 уже исправлен документом %2. Проведение еще одного исправления невозможно.'"),
						ПредставлениеИсправленного, 
						ПредставлениеИсправления);
			КонецЕсли;
...
Процедура ОтменаИсправленияПериодическихСведений(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("Сторнирование") И Источник.ДополнительныеСвойства.Сторнирование Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ТаблицаДокумента.ИсправленныйДокумент", "ТаблицаДокумента.СторнируемыйДокумент");
	...
	Если Выборка.Следующий() Тогда
		...
		ВызватьИсключение 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Отмена исправления по документу %1 невозможна. Сначала нужно отменить исправление по документу %2.'"),
		ПредставлениеИсправления,
		ПредставлениеИсправленного);
	КонецЕсли;
