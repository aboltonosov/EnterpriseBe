	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "АдрИО_XML" Тогда
			...
			Если НЕ Организация.ИностраннаяОрганизация Тогда
				Значение = Неопределено;
			Иначе
				// 4D:ERP для Беларуси, Яна, 22.08.2017 16:25:13 
				// Результат тестирования конфигурации, №15769
				// {
				Значение = АдресВXML(Организация.КонтактнаяИнформация, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации); //УчредительныйАдресИностраннойОрганизации
				// }
				// 4D
			КонецЕсли;
...
// 4D:ERP для Беларуси, Екатерина, 25.11.2015 15:06:10 
// , №10629
// {
Функция ПолучитьСписокРегламентированныхОтчетов_Локализация() Экспорт
	...
	// Сведения об отсутствии объектов налогообложения
    СписокРегламентированныхОтчетов.Добавить("РегламентированныйОтчетСведенияОбОтстутствииОбъектовНалогообложенияДляКоммерческихОрганизаций_Локализация");
    СписокРегламентированныхОтчетов.Добавить("РегламентированныйОтчетСведенияОбОтсутствииОбъектовНалогообложенияДляНекоммерческихОрганизаций_Локализация");
	
	// Прочие местные налоги и сборы
    СписокРегламентированныхОтчетов.Добавить("РегламентированныйОтчетПоСборуСЗаготовителей");
    СписокРегламентированныхОтчетов.Добавить("РегламентированныйОтчетПоСборуСПользователейЗаВладениеСобаками");
