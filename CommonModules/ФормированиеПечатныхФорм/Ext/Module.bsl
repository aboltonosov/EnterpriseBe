...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 03.12.2015 16:39:34 
	// , №10696
	// {
	// Пояснения к некоторым видам реквизитов, выводимые в описание
	ПрефиксыРеквизитов = Новый Структура;
	ПрефиксыРеквизитов.Вставить("ИНН",			НСтр("ru='УНП %1'"));
	...
	ПрефиксыРеквизитов.Вставить("НомерСчета",	НСтр("ru='р/с %1 в банке %2 БИК %3'"));
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, Знач БанковскийСчет = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 26.01.2017 1:37:49 
	// Добавим наши реквизиты
	// {
	Сведения = Новый Структура("Представление, СокращенноеНаименование, ПолноеНаименование, НаименованиеДляПечатныхФорм,
		|КодПоОКПО, КодОКВЭД, ИНН, УНП, КПП, Телефоны, ЮридическийАдрес, Банк, БИК, КоррСчет, НомерСчета");
	...
	Если ЗначениеЗаполнено(ЮрФизЛицо)
	   И (ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации")
		...
		Сведения.Вставить("УНП", 						 Реквизиты.ИНН);
		...
		Сведения.Вставить("ЮридическийАдресЗначенияПолей", УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияXMLПоПредставлению(Сведения.ЮридическийАдрес, Перечисления.ТипыКонтактнойИнформации.Адрес));
		
		Сведения.Вставить("ФактическийАдрес", ПолучитьАдресИзКонтактнойИнформации(ЮрФизЛицо, "Фактический"));
		Сведения.Вставить("ФактическийАдресЗначенияПолей", УправлениеКонтактнойИнформациейСлужебныйВызовСервера.КонтактнаяИнформацияXMLПоПредставлению(Сведения.ФактическийАдрес, Перечисления.ТипыКонтактнойИнформации.Адрес));
	КонецЕсли;
...
Функция ПолучитьСоответствиеСтавокНДС() Экспорт
	...
	СоответствиеСтавокНДС.Вставить(Перечисления.СтавкиНДС.НДС25, 0);
	СоответствиеСтавокНДС.Вставить(Перечисления.СтавкиНДС.НДС25_125, 0);
	// }
	// 4D
