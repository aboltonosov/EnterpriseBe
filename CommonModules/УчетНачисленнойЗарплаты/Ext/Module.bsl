...
Процедура ЗарегистрироватьНачисленияУдержания(Движения, Отказ, Организация, ПериодРегистрации, Начисления, Удержания, БазаУдержаний, ПрочиеДоходы, ХарактерВыплаты = Неопределено, ЗаписыватьДвижения = Ложь) Экспорт
	...
	ТаблицаНачислений = ТаблицаЗначенийПоИмениРегистраНакопления("НачисленияУдержанияПоСотрудникам");
	ТаблицаУдержанийПрочихНачислений = ТаблицаЗначенийПоИмениРегистраНакопления("НачисленияУдержанияПоСотрудникам");
	...
	Если Удержания <> Неопределено Тогда
		...
		Иначе
			...
			Если БазаУдержаний = Неопределено Тогда
				...
				ДанныеДляУчетаУдержаний = ДанныеДляУчетаУдержаний(Движения, МассивФизическихЛиц, ПериодРегистрации, Организация,, ДанныеМежрасчетногоПериода);
			КонецЕсли;
...
Процедура ПодготовитьДанныеНДФЛКРегистрации(ТаблицаНДФЛ) Экспорт
	...
	КонецЕсли;
	Если ЗаполнитьПодразделениеСотрудника Или ВычестьЗачтеннуюСумму Тогда
...
Процедура ЗарегистрироватьКорректировкиВыплаты(Движения, Отказ, Организация, ПериодРегистрации, КорректировкиВыплаты, МВТ, ХарактерВыплаты = Неопределено, ЗаписыватьДвижения = Ложь, РаспределятьПоРабочимМестамБезусловно = Ложь) Экспорт
	...
	Иначе
		...
		Если УдержанияКРаспределениюВторичные.Количество() > 0 Тогда
			...
			ТекущиеКорректировкиВыплаты = ВзаиморасчетыССотрудниками.КорректировкиВыплаты(Организация, ПериодРегистрации, ФизическиеЛицаМассив, РегистраторОграниченияБазы);
			ТекущиеКорректировкиВыплаты.Колонки.КорректировкаВыплаты.Имя = "Сумма";
...
//		УжеУдержано
//
Функция ДанныеДляУчетаУдержаний(Движения, МассивФизическихЛиц, ПериодРегистрации, Организация, Авансом = Ложь, ДанныеМежрасчетногоПериода = Ложь) Экспорт
...
//
Процедура ПриКомпоновкеОтчетаАнализНачисленийИУдержаний(Объект, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, КлючВарианта, НаАванс = Ложь) Экспорт
	УчетНачисленнойЗарплатыВнутренний.ПриКомпоновкеОтчетаАнализНачисленийИУдержаний(Объект, ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка, КлючВарианта, НаАванс);
КонецПроцедуры
...
Функция ТаблицаРаспределенияПоРабочимМестам(Авансом = Ложь) Экспорт
	...
	УдержанияПоРабочимМестам = ТаблицаЗначенийПоИмениРегистраНакопления(ИмяРегистраНачисленийУдержаний);
...
Функция ТаблицаЗначенийПоИмениРегистраНакопления(ИмяРегистра)
	...
	Иначе
		...
		Попытка
			...
			ДокументРезультат = Отчеты.АнализНачисленийИУдержаний.РасчетныйЛисток(
				ФизическиеЛицаОтчета, ДокументОбъект.Организация, ДатаОтчета, ОтчетРасчетныйЛисток);
...
// 4D:ERP для Беларуси, АлексейЧ, 10.01.2018 11:42:26 
