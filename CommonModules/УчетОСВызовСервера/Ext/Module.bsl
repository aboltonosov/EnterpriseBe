...
Функция ТекстЗапросаВТОтраженияВРеглУчетеНачисленнойАмортизации(ИмяДокумента) Экспорт
	...
	ТекстВТ = ТекстЗапросаВТНачисленнаяАмортизация(ИмяДокумента)
		...
		+ ТекстЗапросаВТПервоначальныеСведения()
		+ ТекстЗапросаВТМестонахожденияОС()
		+ ТекстЗапросаВТНачисленнаяАмортизацияЦелевогоФинансирования()
		+ ТекстЗапросаВТНачисленнаяАмортизационнаяПремия();
...
Функция ТекстОтраженияВРеглУчетеНачисленнойАмортизации() Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 04.01.2016 19:56:08 
	// Ошибка при проведении всех документов по ОС и по НМА (задача №10830)
	// {
	ТекстОтражения = ТекстОтраженияАмортизационнаяПремияКВ()
		+ Разделитель + ТекстОтраженияИзнос()
		
		+ Разделитель + ТекстОтраженияАмортизация()
		+ Разделитель + ТекстОтраженияАмортизацияБезНУ()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремия()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияБезНУ()
		
		+ Разделитель + ТекстОтраженияАмортизацияОтправитель()
		+ Разделитель + ТекстОтраженияАмортизацияБезНУОтправитель()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияОтправитель()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияБезНУОтправитель()
		
		+ Разделитель + ТекстОтраженияАмортизацияПолучатель()
		+ Разделитель + ТекстОтраженияАмортизацияБезНУПолучатель()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияПолучатель()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияБезНУПолучатель()
		
		+ Разделитель + ТекстОтраженияАмортизацияНаПрочиеОперации()
		+ Разделитель + ТекстОтраженияАмортизацияБезНУНаПрочиеОперации()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияНаПрочиеОперации()
		+ Разделитель + ТекстОтраженияАмортизационнаяПремияБезНУНаПрочиеОперации();
...
// Возвращает данные для заполнения табличной части "Целевое финансирование" документов принятия к учета НМА/ОС и модернизации ОС
//
// Параметры:
...
//
// Возвращаемое значение:
// 		ТаблицаЗначений - Таблица с даныым для заполнения ТЧ
//
...
//
Процедура ЗаполнитьСлужебныеРеквизитыВТаблицеЦелелевогоФинансирования(ОбъектФормы, Строка=Неопределено) Экспорт
...
Функция ТекстЗапросаВТПервоначальныеСведения()
...
Функция ТекстОтраженияАмортизацияПолучатель()
	...
	|	&Ссылка КАК Ссылка,
	|	&Дата КАК Период,
	|	ЕСТЬNULL(ОрганизациПолучатель.Ссылка, ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК Организация,
	|	"""" КАК ИдентификаторСтроки,
	|	
	|	Строки.СуммаБУ КАК Сумма,
	|	
	...
	|	втНачисленнаяАмортизация КАК Строки
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК ОрганизациПолучатель
	|		ПО Строки.ОрганизацияПолучательРасходов = ОрганизациПолучатель.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиСтроительства
	|	ПО Строки.СтатьяРасходов = СтатьиСтроительства.Ссылка
	|		И СтатьиСтроительства.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
...
Процедура ВременнаяТаблицаСтоимостиУпрПрочиеАктивыДанныеДляГруппировки(ТекстыЗапроса)
	...
	|ГДЕ
	|	&ПогашениеУпрЗатрат
	|	И ПрочиеАктивыПассивы.Статья В (ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.НематериальныеАктивы), ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиАктивовПассивов.ОсновныеСредства))
...
Процедура ВременнаяТаблицаСтоимостиУпрПрочиеРасходыДанныеДляГруппировки(ТекстыЗапроса)
	...
	|			И ПрочиеРасходы.СтатьяРасходов.ВариантРаспределенияРасходов = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|ГДЕ
	|	&ПогашениеУпрЗатрат
...
Процедура ВременнаяТаблицаЦелевыеСредстваРеглДанныеДляГруппировки(ТекстыЗапроса)
	...
	// TODO Надо проверить 
	// {
	|			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДоходыПолученныеВСчетБудущихПериодов),
	// }
	// 4D
	|			ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.ОсновныеСредства),
	|			Организация = &Организация
	...
	// TODO Надо проверить 
	// {
	|	И Хозрасчетный.СчетДт = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДоходыПолученныеВСчетБудущихПериодов)
	// }
	// 4D
	|
	|ОБЪЕДИНИТЬ ВСЕ
...
Функция ПрочиеРасходы_АмортизацияРеглСтоимости()
	...
	|	0 КАК Сумма,
	|	0 КАК СуммаБезНДС,
	|	втНачисленнаяАмортизация.СуммаБУ КАК СуммаРегл,
	...
	|	втНачисленнаяАмортизация КАК втНачисленнаяАмортизация
	|ГДЕ
	|	втНачисленнаяАмортизация.СтатьяРасходов.ПринятиеКНалоговомуУчету = ИСТИНА
	|	И НЕ втНачисленнаяАмортизация.НачислятьИзнос
	...
	|	0 КАК СуммаБезНДС,
	|	втНачисленнаяАмортизация.СуммаБУ КАК СуммаРегл,
	|	втНачисленнаяАмортизация.СуммаНУ + втНачисленнаяАмортизация.СуммаПР КАК ПостояннаяРазница,
	|	втНачисленнаяАмортизация.СуммаВР КАК ВременнаяРазница,
	|	
	|	ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПустаяСсылка) КАК ХозяйственнаяОперация,
	|	ЗНАЧЕНИЕ(Справочник.КлючиАналитикиУчетаНоменклатуры.ПустаяСсылка) КАК АналитикаУчетаНоменклатуры,
	...
	|	втНачисленнаяАмортизация КАК втНачисленнаяАмортизация
	|ГДЕ
	|	втНачисленнаяАмортизация.СтатьяРасходов.ПринятиеКНалоговомуУчету = ЛОЖЬ
	|	И НЕ втНачисленнаяАмортизация.НачислятьИзнос
	|	И втНачисленнаяАмортизация.СтатьяРасходов.ВариантРаспределенияРасходов В (
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПроизводственныеЗатраты),
	|		ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаНаправленияДеятельности),
...
Функция ПрочиеРасходы_АмортизацияУпрСтоимостиИзПрочихРасходов()
	...
	|	СУММА(ВЫРАЗИТЬ(втСтоимостиУпрПрочиеРасходы.Сумма * втНачисленнаяАмортизация.Коэффициент КАК ЧИСЛО(15, 2))) КАК Сумма,
	|	
	// 4D:ERP для Беларуси, Петр, 13.11.2017 12:48:57 
	// Не работает печатная форма акта на списание НМА, №16612
	// {
...
Функция ПрочиеРасходы_ЧастичнаяЛиквидацияОстаточнойУпрСтоимостиИзПрочихАктивов()
	...
	|	* (ЕСТЬNULL(втОбъектыДокумента.СуммаЧастичнойЛиквидации, 0) / ЕСТЬNULL(втТекущееСостояниеОбъектов.ПолнаяВосстановительнаяСтоимостьБУ, 0)) КАК Сумма,
	|	(втСтоимостиУпрПрочиеАктивы.Сумма - ЕСТЬNULL(втАмортизацияУпрПрочиеАктивы.Сумма, 0))
	|	* (ЕСТЬNULL(втОбъектыДокумента.СуммаЧастичнойЛиквидации, 0) / ЕСТЬNULL(втТекущееСостояниеОбъектов.ПолнаяВосстановительнаяСтоимостьБУ, 0)) КАК СуммаБезНДС,
	|	0 КАК СуммаРегл,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	
...
Функция ПартииПрочихРасходов_АмортизацияРеглСтоимости()
	...
	|	втНачисленнаяАмортизация.СтатьяРасходов.ПринятиеКНалоговомуУчету = ИСТИНА
	|	И НЕ втНачисленнаяАмортизация.НачислятьИзнос
	|	И втНачисленнаяАмортизация.СтатьяРасходов.ВариантРаспределенияРасходов В (ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
//
Процедура СформироватьЗаданиеАмортизацияОС(ДанныеТаблиц, НачислениеАмортизацииЗаданоДокументом = Ложь) Экспорт
	...
	ТекстОбъединенияДанных = СформироватьТекстОбъединенияДанныхДляФормированияЗадания(СписокТаблиц, Истина, ДанныеТаблиц);
	...
	Запрос.УстановитьПараметр("Операция", Перечисления.ОперацииЗакрытияМесяца.АмортизацияОС);
...
//
Процедура СформироватьЗаданиеАмортизацияНМА(ДанныеТаблиц) Экспорт
	...
	ТекстОбъединенияДанных = СформироватьТекстОбъединенияДанныхДляФормированияЗадания(СписокТаблиц, Ложь, ДанныеТаблиц);
	...
	|				ИНАЧЕ ДОБАВИТЬКДАТЕ(ИзмененныеДанные.Период, МЕСЯЦ, 1)
	|			КОНЕЦ) КАК Месяц
	|ИЗ
	|	(" + ТекстОбъединенияДанных + ") КАК ИзмененныеДанные
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВводОстатковВнеоборотныхАктивов КАК ВводОстатковВнеоборотныхАктивов
	...
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.МенеджерВременныхТаблиц = ДанныеТаблиц.МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Операция", Перечисления.ОперацииЗакрытияМесяца.АмортизацияНМА);
...
Функция ЗаданияКРасчетуАмортизацииОСНаСледующийПериод(Организации, НачалоПериода) Экспорт
	...
	"ВЫБРАТЬ
	|	НачислениеАмортизации.Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ОперацииЗакрытияМесяца.АмортизацияОС) КАК Операция,
...
Функция ЗаданияКРасчетуАмортизацииНМАНаСледующийПериод(Организации, НачалоПериода) Экспорт
	...
	"ВЫБРАТЬ
	|	НачислениеАмортизации.Организация,
	|	ЗНАЧЕНИЕ(Перечисление.ОперацииЗакрытияМесяца.АмортизацияНМА) КАК Операция,
	|	ЕСТЬNULL(МИНИМУМ(НачислениеАмортизации.Период), ДАТАВРЕМЯ(1, 1, 1)) КАК Месяц
	|ИЗ
	|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|		НачислениеАмортизации.Организация КАК Организация,
...
Функция СформироватьТекстОбъединенияДанныхДляФормированияЗадания(СписокТаблиц, ЕстьОсновныеСредства, ДанныеТаблиц) Экспорт
	...
	ТекстОбъединенияДанных = СтрСоединить(МассивСтрок, РазделительЗапросов);
...
Процедура ЗафиксироватьРасчетАмортизации(ВыполненныеЗадания, ЗаданияНаСледующийПериод) Экспорт
	...
	ТекущийНомерЗадания = ЗакрытиеМесяцаУТВызовСервера.ТекущийНомерЗадания();
...
Функция ПодготовитьПараметрыРегистрацияПервоначальныхСведенийОСУСН(ТаблицаПервоначальныеСведенияОСУСН, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаПервоначальныеСведенияОСУСН",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаПервоначальныеСведенияОСУСН, СписокОбязательныхКолонок));
...
Функция ПодготовитьПараметрыРегистрацияОплатОсновныхСредствДляУСН(ТаблицаОплатыОсновныхСредствДляУСН, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаРегистрацияОплатыОсновныхСредствДляУСН",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаОплатыОсновныхСредствДляУСН, СписокОбязательныхКолонок));
...
Функция ПодготовитьПараметрыОтражениеАмортизационнойПремии(ТаблицаАмортизационнойПремии, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаАмортизационнойПремии",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаАмортизационнойПремии, СписокОбязательныхКолонок));
...
Функция ПодготовитьПараметрыСуммАмортизационнойПремии(ТаблицаАмортизации, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаАмортизации",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаАмортизации, СписокОбязательныхКолонок));
...
Функция ПодготовитьПараметрыНачислениеАмортизационнойПремии(ТаблицаНачисленияАмортизационнойПремии, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаНачисленияАмортизационнойПремии",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаНачисленияАмортизационнойПремии, СписокОбязательныхКолонок));
...
Функция ПодготовитьТаблицыРасходовПоАренднымПлатежамНУ(ТаблицаОС, ТаблицаРеквизиты, ТаблицаЗатрат, КоличествоОбработанныхДанных, Отказ) Экспорт
	...
	|	ВТ_РанееНачисленнаяАмортизация.НаправлениеДеятельности,
...
Процедура СформироватьДвиженияРасходыПоАренднымПлатежамНУиКорректировкиАмортизации(ПараметрыРасходыПоАреднымПлатежамНУ, Движения, Отказ) Экспорт
...
Функция ПолучитьПустуюТаблицуРасходыПоАренднымПлатежамНУ()
	...
	ТаблицаРасходыПоАренднымПлатежамНУ.Колонки.Добавить("ВалютаВзаиморасчетов", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
		
	ТаблицаРасходыПоАренднымПлатежамНУ.Колонки.Добавить("СчетРасходовПоАренднымПлатежам", ОписаниеТиповХозрасчетный);
	ТаблицаРасходыПоАренднымПлатежамНУ.Колонки.Добавить("СчетРасчетовСКонтрагентом", ОписаниеТиповХозрасчетный);
	ТаблицаРасходыПоАренднымПлатежамНУ.Колонки.Добавить("СчетЗадолженностиПоАренде", ОписаниеТиповХозрасчетный);
...
Функция ПодготовитьТаблицуАрендныхПлатежей(ТаблицаОС, Реквизиты, КоличествоОбработанныхДанных, Отказ)
	...
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДоговорыЛизинга.СчетУчетаАрендныеОбязательства КАК Счет
	|ПОМЕСТИТЬ ВТ_СчетаАрендныеОбязательства
	|ИЗ
	...
	|			&НачалоПериода,
	|			&КонецПериодаГраница, , , 
	|			Счет В (ВЫБРАТЬ Счет ИЗ ВТ_СчетаАрендныеОбязательства), 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоступлениеУслугПоЛизингу.Договор.СчетУчетаАрендныеОбязательства КАК СчетЗадолженностиПоАренде,
	|	ПоступлениеУслугПоЛизингу.Договор.СчетУчетаУслугиПоЛизингу КАК СчетРасчетовСКонтрагентом,
	|	ПоступлениеУслугПоЛизингу.Контрагент КАК Контрагент,
	|	ПоступлениеУслугПоЛизингу.Договор КАК ДоговорКонтрагента,
	|	ПоступлениеУслугПоЛизингу.Подразделение КАК Подразделение,
	...
	|	ВЫБОР
	|		КОГДА ПоступлениеУслугПоЛизингу.Договор.СчетУчетаУслугиПоЛизингу.Валютный
	|			ТОГДА ПоступлениеУслугПоЛизингу.ВалютаВзаиморасчетов
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПоступлениеУслугПоЛизингу.Начисления КАК ТаблицаНачисления
	|		ПО ТаблицаНачисления.Ссылка = ПоступлениеУслугПоЛизингу.Ссылка
	|			И ТаблицаНачисления.ТипНачисления В (ЗНАЧЕНИЕ(Перечисление.ТипыНачисленийПоЛизингу.УслугаПоЛизингу),
	...
	|ГДЕ
	|	ПоступлениеУслугПоЛизингу.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ПоступлениеУслугПоЛизингу.Организация = &Организация
	|	И ПоступлениеУслугПоЛизингу.Проведен
	|
	|СГРУППИРОВАТЬ ПО
	|	ПоступлениеУслугПоЛизингу.Договор.СчетУчетаАрендныеОбязательства,
	|	ПоступлениеУслугПоЛизингу.Договор.СчетУчетаУслугиПоЛизингу,
	|	ПоступлениеУслугПоЛизингу.Контрагент,
	|	ПоступлениеУслугПоЛизингу.Договор,
	|	ПоступлениеУслугПоЛизингу.Подразделение,
	|	ВЫБОР
	|		КОГДА ПоступлениеУслугПоЛизингу.Договор.СчетУчетаУслугиПоЛизингу.Валютный
	|			ТОГДА ПоступлениеУслугПоЛизингу.ВалютаВзаиморасчетов
	|		ИНАЧЕ &ВалютаРегламентированногоУчета
	...
	|					ИЗ
	|						НеСнятыеСУчета)
	|				И ДоговорЛизинга <> ЗНАЧЕНИЕ(Справочник.ДоговорыЛизинга.ПустаяСсылка)) КАК ПервоначальныеСведенияОСБухгалтерскийУчет
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Контрагент,
	|	ДоговорКонтрагента
...
Функция ПолучитьПустуюТаблицуАрендныхПлатежей()
	...
	ТаблицаАрендныхПлатежей.Колонки.Добавить("ВалютаВзаиморасчетов", Новый ОписаниеТипов("СправочникСсылка.Валюты"));
...
Функция ПодготовитьТаблицуАмортизацияОС(ТаблицаОС, ТаблицаРеквизиты, КоличествоОбработанныхДанных = 0, Отказ = Ложь) Экспорт
	...
	|	+ ЕСТЬNULL(АмортизацияЦФОСНачалоПериода.СуммаОстатокКт, 0) КАК АмортизацияНачальныйОстаток,
	...
	|	(ЕСТЬNULL(ДокументАП_ПринятиеКУчету.Ссылка, ДокументАП_ВводОстатков.Ссылка) = СписокОС.СпособОтраженияРасходовДокументОснование) КАК СпособОтраженияРасходовАмпортизационнойПремииЗаданДокументом,
	|	
	...
	Пока ВыборкаПоОС.Следующий() Цикл
			...
			Если НЕ Пропустить Тогда
				...
				ИначеЕсли ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
					...
					Иначе
						...
						// 4D:ERP для Беларуси, АлексейЧ , 27.10.2017 5:00:05 
						// механизм начисления амортизации ОС по методу уменьшаемого остатка, № 16353
						// {
						Если СтоимостьИтоговая = 0 И ВыборкаПоОС.АмортизацияНачальныйОстаток <> ВыборкаПоОС.ПервоначальнаяСтоимость Тогда
							
							ЗапросПриВводеОстатковОС = Новый Запрос;
							ЗапросПриВводеОстатковОС.Текст = 
							"ВЫБРАТЬ ПЕРВЫЕ 1
							|	ВводОстатковОС.НакопленнаяАмортизацияБУ КАК НакопленнаяАмортизацияБУ
							|ИЗ
							|	Документ.ВводОстатковВнеоборотныхАктивов.ОС КАК ВводОстатковОС
							|ГДЕ
							|	ВводОстатковОС.ОсновноеСредство = &ОсновноеСредство
							|	И ВводОстатковОС.ИнвентарныйНомер = &ИнвентарныйНомер
							|	И ВводОстатковОС.Ссылка.Проведен = &Проведение";

							ЗапросПриВводеОстатковОС.УстановитьПараметр("ОсновноеСредство",ВыборкаПоОС.ОсновноеСредство);
							ЗапросПриВводеОстатковОС.УстановитьПараметр("ИнвентарныйНомер",ВыборкаПоОС.ИнвентарныйНомер);
							ЗапросПриВводеОстатковОС.УстановитьПараметр("Проведение",ИСТИНА);
							
							РезультатЗапросаПриВводеОстатковОС = ЗапросПриВводеОстатковОС.Выполнить().Выбрать();;
							
							Пока РезультатЗапросаПриВводеОстатковОС.Следующий() Цикл
								СтоимостьИтоговая = ВыборкаПоОС.ПервоначальнаяСтоимость - РезультатЗапросаПриВводеОстатковОС.НакопленнаяАмортизацияБУ; 	
							КонецЦикла;
		...
		Если ОтражатьВНалоговомУчете Тогда
			...
			Если ПорядкиПогашенияСтоимости.Получить(ВыборкаПоОС.ПорядокВключенияСтоимостиВСоставРасходов) <> Неопределено
				...
				Если НЕ Пропустить Тогда
					...
					// 4D:ERP для Беларуси, Алексей4Д, 25.10.2017 23:51:08 
					// Задача № 16334 Исключить различие расчета амортизации для налогового учета
					// {
					...
					СтоимостьДляВычисления = ВыборкаПоОС.СтоимостьДляВычисленияАмортизации;
					СрокИспользованияДляВычисления = Макс(1, ВыборкаПоОС.СрокИспользованияДляВычисленияАмортизации);
					// }
					// 4D
					...
					ИначеЕсли МетодНачисленияАмортизации = Перечисления.МетодыНачисленияАмортизации.Нелинейный Тогда
						...
						Если НЕ (НормаАмортизации = Неопределено) Тогда
							...
							* СпециальныйКоэффициент;
							...
							// 4D:ERP для Беларуси, АлексейЧ, 30.10.2017 13:35:27 
							// механизм начисления амортизации ОС по методу уменьшаемого остатка, № 16353
							// {
							Если ВыборкаПоОС.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
								СуммаАмортизации = СуммаАмортизацииДляНалоговогоУчета;
							КонецЕсли;
							// }
							// 4D
		...
		Если СуммаАмортизационнойПремии <> 0 Тогда
			Если Не ВыборкаПоОС.СпособОтраженияРасходовАмпортизационнойПремииЗаданДокументом Тогда
				СпособОтражения = Неопределено;
			КонецЕсли;
			НовыеСтроки = ПодготовитьСтрокиВТаблицуАмортизации(
				ВыборкаПоОС,
...
Процедура ДобавитьСтрокиВТаблицуАмортизации(Таблица, ВыборкаДанных, НовыеСтроки, ТаблицаРаспределения)
	...
	Для Каждого Строка Из НовыеСтроки Цикл
		...
		Иначе
			...
			СуммыПР = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Строка.СуммаПР, МассивКоэффицентов);
			СуммыВР = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Строка.СуммаВР, МассивКоэффицентов);
			Коэффициенты = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Строка.Коэффициент, МассивКоэффицентов, 36);
...
Функция ПолучитьТекстЗапросаАмортизацияОСНеСнятыеСУчетаОСВсе()
	...
	|			И КОНЕЦПЕРИОДА(ДокВводОстатков.Дата, МЕСЯЦ) < &Период)
	|	И СостоянияОСОрганизаций.Организация = &Организация
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПодготовитьПараметрыНачислениеИзноса(ТаблицаИзноса, ТаблицаРеквизиты)
	...
	Параметры.Вставить("ТаблицаИзноса",
		УправлениеВнеоборотнымиАктивамиПереопределяемый.ПолучитьТаблицуПараметровПроведения(ТаблицаИзноса, СписокОбязательныхКолонок));
...
Функция ПодготовитьТаблицыСведенийПоВыбытиюОС(ТаблицаОС, ТаблицаРеквизиты, Отказ, ВыдаватьСообщения = Ложь) Экспорт
	...
	ТаблицаНачислениеАмортизацииОСНУ = ТаблицаПараметрыСписания.Скопировать(
		Новый Структура("ПрекратитьНачислениеАмортизацииНУ", Истина), "ОсновноеСредство");
...
Процедура СформироватьДвиженияПеремещениеОС(ТаблицаОС, ТаблицаРеквизиты, ПараметрыНачисленияАмортизации, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОстатковПоОС Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.СчетУчета)
			...
			Проводка.Период = Реквизиты.Период;
			Проводка.Организация = Реквизиты.Организация;
			Проводка.Содержание = НСтр("ru = 'Перемещение ОС'");
...
Функция ПодготовитьПараметрыМодернизацияОС(ТаблицаМодернизации, ТаблицаРеквизиты)
	...
	+ "ПодразделениеПоАмортизационнойПремии,"   // Подразделение, на котороео нужно отнести затраты по отражению амортизационной премии
	+ "Регистратор,"                            // <ДокументСсылка.*>
...
Функция ПодготовитьТаблицыАмортизацииОСИСуммАмортизационнойПремии(ТаблицаОС, ТаблицаРеквизиты, КоличествоОбработанныхДанных, Отказ) Экспорт
	
	Параметры = ПараметрыТаблицыАмортизацииОСИСуммАмортизационнойПремии(ТаблицаОС, ТаблицаРеквизиты);
	
	Реквизиты = Параметры.Реквизиты[0];
	
	Содержание = НСтр("ru='Амортизация ОС'");
	
	ПараметрыНачисленияАмортизации = Новый Структура;
	
	Параметры.Реквизиты.Колонки.Добавить("Содержание");
	Параметры.Реквизиты.ЗаполнитьЗначения(Содержание, "Содержание");
	ТаблицаАмортизации = ПодготовитьТаблицуАмортизацияОС(Параметры.ТаблицаОС, Параметры.Реквизиты, КоличествоОбработанныхДанных, Отказ);
	...
	ТаблицаРаспределения = УправлениеВнеоборотнымиАктивамиПереопределяемый.ПодготовитьТаблицуРаспределениеАмортизацииПоНаправлениямРегл(
		ТаблицаАмортизации, КоличествоОбработанныхДанных, Отказ);
...
Функция ПараметрыТаблицыАмортизацииОСИСуммАмортизационнойПремии(ТаблицаОС, ТаблицаРеквизиты)
...
// Регистрирует необходимость выполнения регл. операции "Признание в НУ лизинговых платежей" при закрытии месяца.
//  Вызывается при изменении данных влияющих на выполнение регл. операции.
//
// Параметры:
//  Объект				 - ДокументОбъект	 - документ, который повлиял на необходимость выполнения регл. операции.
//  ОпределитьПериод	 - Булево - Истина, если нужно определить период задания (начиная с которого есть ОС принятые по договору лизинга).
//
Процедура ЗарегистрироватьЗаданиеПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца(Объект, ОпределитьПериод = Ложь) Экспорт
	Перем ДанныеДляЗадания;
	...
	ВыполнитьРегистрациюЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца(ТипОбъекта, Объект, Объект.Ссылка, ОпределитьПериод);
	
	Если Объект.ДополнительныеСвойства.Свойство("ДанныеДляЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца", ДанныеДляЗадания) Тогда
		Если ДанныеДляЗадания.Проведен
			И (ДанныеДляЗадания.Организация <> Объект.Организация 
				ИЛИ НачалоМесяца(ДанныеДляЗадания.Дата) <> НачалоМесяца(Объект.Дата)) Тогда
			
			ВыполнитьРегистрациюЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца(ТипОбъекта, ДанныеДляЗадания, Объект.Ссылка, ОпределитьПериод);
		КонецЕсли; 
	КонецЕсли;
	
КонецПроцедуры

// Подготовливает данные для регистрации задания к закрытию месяца
//
// Параметры:
//  Объект	 - ДокументОбъект - документ для которого требуется подготовить данные
//
Процедура ПодготовитьДанныеДляЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца(Объект) Экспорт

	Если Объект.ЭтоНовый() Тогда
		Возврат;
	КонецЕсли;
	
	СписокРеквизитов = "Дата,Организация,Проведен";
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.ПринятиеКУчетуОС") Тогда
		СписокРеквизитов = СписокРеквизитов + ",СпособПоступления";
	КонецЕсли;
	ДанныеДляЗадания = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.Ссылка, СписокРеквизитов);
	Объект.ДополнительныеСвойства.Вставить("ДанныеДляЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца", ДанныеДляЗадания);
	
КонецПроцедуры

Процедура ВыполнитьРегистрациюЗаданияПризнаниеВНУЛизинговыхПлатежейКЗакрытиюМесяца(ТипОбъекта, РеквизитыОбъекта, Ссылка, ОпределитьПериод)

	Если НЕ РеквизитыОбъекта.Проведен Тогда
		Возврат;
	КонецЕсли;
	
	Если ТипОбъекта = Тип("ДокументОбъект.ПринятиеКУчетуОС") 
		И РеквизитыОбъекта.СпособПоступления <> Перечисления.СпособыПоступленияАктивов.ПоДоговоруЛизинга Тогда
		Возврат;
	КонецЕсли; 
	
	Период = ?(ТипОбъекта = Тип("ДокументОбъект.ПринятиеКУчетуОС"), КонецМесяца(РеквизитыОбъекта.Дата) + 1, РеквизитыОбъекта.Дата);
	
	СфрмироватьЗаданиеПризнаниеВНУЛизинговыхПлатежей(
		РеквизитыОбъекта.Организация, Период, Ссылка, ОпределитьПериод);

КонецПроцедуры

Процедура СфрмироватьЗаданиеПризнаниеВНУЛизинговыхПлатежей(Организация, Дата, Ссылка, ОпределитьПериод) Экспорт
	...
	Если ПараметрыУчетнойПолитики <> Неопределено 
		И ПараметрыУчетнойПолитики.СхемаУчетаСтоимостиОСвНУ = Перечисления.СхемыУчетаСтоимостиОСвНУ.СФормированиемВР Тогда
		
		Если ОпределитьПериод Тогда
			ПериодЗадания = СледующийПериодПризнанияВНУЛизинговыхПлатежей(Организация, Дата);
		Иначе
			ПериодЗадания = Дата;
		КонецЕсли; 
