	...
	Если ТипЗнч(СписокИменВидовКонтактнойИнформации) = Тип("СписокЗначений") Тогда
		...
		Для каждого ЭлементСпискаВидов Из СписокИменВидовКонтактнойИнформации Цикл
			...
			Если ТипЗнч(ТекущееИмяВидаКонтактнойИнформации) = Тип("Строка") 
				...
				Если РезЗапросаПредопределенных.Найти(ТекущееИмяВидаКонтактнойИнформации, "ИмяПредопределенныхДанных") = Неопределено Тогда
					Продолжить;
				Иначе		 
...
Функция ОпределитьКонтрагентаИзЗапросаАЛКО(ТаблицаРезультатаЗапроса, КПП, 
							ИмяКолонкиКонтрагент = "Контрагент", ЕстьСтранаРегистрации = Неопределено) Экспорт
	...
	Если ТаблицаРезультатаЗапроса.Количество() > 0 Тогда
		...
		Иначе	
			...
			Для каждого СтрокаТЗ Из ТаблицаРезультатаЗапроса Цикл
			    // Ищем контрагента без КПП.
				Если НЕ ЗначениеЗаполнено(СтрокаТЗ[ИмяКолонкиКонтрагент].КПП) Тогда
				
					НайденКонтрагент = Истина;
					Контрагент = СтрокаТЗ[ИмяКолонкиКонтрагент];
					Прервать;
			...
			Если НЕ НайденКонтрагент Тогда
				...
				Для каждого СтрокаТЗ Из ТаблицаРезультатаЗапроса Цикл
					...
					Если ЕстьСтранаРегистрации Тогда
					    // Не должно быть КПП, предполагаем что иностранец.
						Если НЕ СтрокаТЗ[ИмяКолонкиКонтрагент].СтранаРегистрации = ПредопределенноеЗначение("Справочник.СтраныМира.Россия") Тогда
...
//   Число.
//
Функция ПолучитьВерсиюЧислом(СтрокаВерсии)
...
Процедура ОпределитьПараметрыОбработчикаОжиданияАЛКО(МаксимальныйИнтервал, КоэффициентУвеличенияИнтервала) Экспорт
	...
	Иначе
		...
		ИначеЕсли КоличествоСеансов < 21 Тогда
...
Процедура ОчиститьРегистрСведенийАЛКООтМусора(ДокументСсылкаАлкоОтчет, СтруктураСведенийОтчета)
	...
	ТекстЗапроса = 	"ВЫБРАТЬ
					|	РегистрСведенийАлкоОтчет.ИндексСтраницы КАК ИндексСтраницыРегистра
					|ИЗ
					|	РегистрСведений." + ИмяРегистраСведений + " КАК РегистрСведенийАлкоОтчет
					|ГДЕ
					|	РегистрСведенийАлкоОтчет.Документ = &Документ
					|   И НЕ (РегистрСведенийАлкоОтчет.ИндексСтраницы В (&МассивИндексовСтраниц))";
	...
	ЗапросОшибочныхДанных.Параметры.Вставить("МассивИндексовСтраниц", МассивИндексовСтраниц);
	...
	Для каждого СтрокаРезультата Из РезультатЗапросаОшибочныхДанных Цикл
		...
		УдаляемыйИндексРегистра = СтрокаРезультата.ИндексСтраницыРегистра;
		НаборЗаписей.Отбор.ИндексСтраницы.Установить(УдаляемыйИндексРегистра);
...
Процедура ОбновитьЦветаСсылокТитульногоАЛКО(Форма)
	...
	Если НЕ Форма.Элементы.Найти("МестоОсуществленияДеятельности") = Неопределено Тогда
		// Заполнение необязательно, поэтому даже если не заполнено - обычная ссылка.
