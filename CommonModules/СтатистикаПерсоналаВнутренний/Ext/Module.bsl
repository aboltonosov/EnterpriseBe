////////////////////////////////////////////////////////////////////////////////
// Подсистема "Статистика персонала".
// Процедуры и функции, предназначенные для обслуживания форм статистической отчетности.
//  
////////////////////////////////////////////////////////////////////////////////

#Область СлужебныеПроцедурыИФункции

// П-4

Процедура ОписаниеПоказателей_СтатистикаФормаП4_2013Кв1(ПоказателиОтчета) Экспорт
	
	СтатистикаПерсоналаРасширенный.ОписаниеПоказателей_СтатистикаФормаП4_2013Кв1(ПоказателиОтчета);
	
КонецПроцедуры

Процедура ЗначенияПоказателей_СтатистикаФормаП4_2013Кв1(ПараметрыОтчета, Контейнер) Экспорт
	
	СтатистикаПерсоналаРасширенный.ЗначенияПоказателей_СтатистикаФормаП4_2013Кв1(ПараметрыОтчета, Контейнер);
	
КонецПроцедуры

Процедура ЗначенияПоказателей_СтатистикаФормаП4_2017Кв1(ПараметрыОтчета, Контейнер) Экспорт
	
	СтатистикаПерсоналаРасширенный.ЗначенияПоказателей_СтатистикаФормаП4_2017Кв1(ПараметрыОтчета, Контейнер);
	
КонецПроцедуры

// П-4 (НЗ)

Процедура ОписаниеПоказателей_СтатистикаФормаП4НЗ_2015Кв1(ПоказателиОтчета) Экспорт
	
	СтатистикаПерсоналаРасширенный.ОписаниеПоказателей_СтатистикаФормаП4НЗ_2015Кв1(ПоказателиОтчета);
	
КонецПроцедуры

Процедура ЗначенияПоказателей_СтатистикаФормаП4НЗ_2015Кв1(ПараметрыОтчета, Контейнер) Экспорт
	
	СтатистикаПерсоналаРасширенный.ЗначенияПоказателей_СтатистикаФормаП4НЗ_2015Кв1(ПараметрыОтчета, Контейнер);
	
КонецПроцедуры

#КонецОбласти
