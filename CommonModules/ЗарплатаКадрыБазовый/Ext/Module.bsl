...
Процедура УстановитьНачальныеЗначения(ИменаОбъектовМетаданных) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 31.05.2017 14:38:54 
	...
	Для каждого ОбрабатываемоеИмя Из ОбрабатываемыеИменаОбъектов Цикл
		...
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("Справочник.ЕдинаяТарифнаяСетка")  Тогда
			
			Справочники.ЕдинаяТарифнаяСетка.НачальноеЗаполнение();
			ПроверитьСправочникНаЗадвоенныеЗначения("Справочник.ЕдинаяТарифнаяСетка");
			
		КонецЕсли;
		
		//СоставыВоеннослужащих
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("Справочник.СоставыВоеннослужащих")  Тогда
			
			Справочники.СоставыВоеннослужащих.НачальноеЗаполнение();
			ПроверитьСправочникНаЗадвоенныеЗначения("Справочник.СоставыВоеннослужащих");
			
		КонецЕсли;
		
		//ЗванияГражданскогоВоинскогоУчета
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("Справочник.ЗванияГражданскогоВоинскогоУчета")  Тогда
			
			Справочники.ЗванияГражданскогоВоинскогоУчета.НачальноеЗаполнение();
			ПроверитьСправочникНаЗадвоенныеЗначения("Справочник.ЗванияГражданскогоВоинскогоУчета");
		...
		// 4D:ERP для Беларуси, , 21.08.2017 11:33:54 
		// Справочник "Виды документов физ. лиц", № 15718
		// {
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("Справочник.ВидыДокументовФизическихЛиц")  Тогда
			
			ПроверитьСправочникНаЗадвоенныеЗначения("Справочник.ВидыДокументовФизическихЛиц");
			Справочники.ВидыДокументовФизическихЛиц.НачальноеЗаполнение();
			ПроверитьСправочникНаЗадвоенныеЗначения("Справочник.ВидыДокументовФизическихЛиц");
		...
		// 4D:ERP для Беларуси, Яна, 22.08.2017 12:43:02 
		// Результат тестирования конфигурации, №15769 
		// {
		// РасчетЗарплаты
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.ВеличинаПрожиточногоМинимума") Тогда
			РегистрыСведений.ВеличинаПрожиточногоМинимума.НачальноеЗаполнение();
		КонецЕсли;
		// }
		// 4D
		...
		Если ПустаяСтрока(ОбрабатываемоеИмя) ИЛИ ВРег(ОбрабатываемоеИмя) =  ВРег("РегистрСведений.РазмерыГосударственныхПособий") Тогда
			РегистрыСведений.РазмерыГосударственныхПособий.ЗаполнитьГосударственныеПособия();
		КонецЕсли;	
		// }
		// 4D
...
Процедура ЗарегистрироватьОплатуВедомостей(ПлатежныйДокумент, Организация, Ведомости, ФизическиеЛица, ДатаОперации, Отказ) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 22.08.2017 12:19:31 
	// Результат тестирования конфигурации, №15769 
	// {
	//Данный фрагмент будет помещен в комментарий, в настоящее время мы не регистируем полученную из внешних источников зарплату.	
