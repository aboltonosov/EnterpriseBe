...
Процедура РаспределитьВсеРасчетыСКлиентами(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	КлючеваяОперация = "РаспределениеВзаиморасчетов.РаспределениеРасчетовСКлиентами";
	...
	Пока НачалоПериода < ОкончаниеПериодаРасчета Цикл
		...
		Если ПервыйПроход Тогда
			НомерЗаданияДоРасчета = УвеличитьНомерЗадания(ИспользуемыеМетаданные.ИмяКонстантыЗаданий);
			ПервыйПроход = Ложь;
		КонецЕсли;
	...
	//++ НЕ УТ
	УчетУСНСервер.ОбновитьЗаданияКЗакрытиюМесяцаПриВыполненииРеглОперации(ПервыйПериод, СписокОрганизаций, Истина, КоличествоПовторяющихсяДанных);
	//-- НЕ УТ
...
Процедура РаспределитьВсеРасчетыСПоставщиками(ОкончаниеПериодаРасчета, АналитикиРасчета = Неопределено) Экспорт
	...
	КлючеваяОперация = "РаспределениеВзаиморасчетов.РаспределениеРасчетовСПоставщиками";
...
Процедура РаспределитьРасчетыФоновымЗаданием(КонецРасчета = Неопределено, АналитикиРасчета = Неопределено, РасчетКВосстановлению = Неопределено) Экспорт
	...
	Если РасчетКВосстановлению = "РасчетыСКлиентами" Тогда
		РаспределитьРасчетыСКлиентамиФоновымЗаданием(КонецРасчета, АналитикиРасчета, РасчетКВосстановлению);
	ИначеЕсли РасчетКВосстановлению = "РасчетыСПоставщиками" Тогда
		РаспределитьРасчетыСПоставщикамиФоновымЗаданием(КонецРасчета, АналитикиРасчета, РасчетКВосстановлению);
	Иначе
		РаспределитьРасчетыСКлиентамиФоновымЗаданием(КонецРасчета, АналитикиРасчета, РасчетКВосстановлению);
		РаспределитьРасчетыСПоставщикамиФоновымЗаданием(КонецРасчета, АналитикиРасчета, РасчетКВосстановлению);
	КонецЕсли;
...
Процедура РаспределитьРасчетыСПоставщикамиФоновымЗаданием(Период, АналитикиКРасчету, РасчетКВосстановлению)
	...
	ФоновыеЗадания.Выполнить("РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации", ПараметрыЗадания, , НаименованиеЗадания);
...
Процедура ЗаполнитьРасчетыСПоставщиками(РасчетнаяПартия, Расход, Приход, Валюты) // РасчетысПоставщикамиПоДокументам
	...
	ЭтоКорректировка = ?(ТипЗнч(Расход.Регистратор) = Тип("ДокументСсылка.КорректировкаПоступления"), Истина, Ложь);
