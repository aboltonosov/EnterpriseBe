...
Процедура ЗаполнитьТаблицуУдержаннаяЗарплата(ТаблицаУдержаннаяЗарплата, ПериодРегистрации, Организация, НачисленияУдержания) 
	...
	Иначе
		...
		|	УдержанияПоСотрудникам.ФизическоеЛицо,
		|	УдержанияПоСотрудникам.Подразделение,
		|	УдержанияПоСотрудникам.ДатаНачала,
		|	УдержанияПоСотрудникам.НачислениеУдержание,
		|	УдержанияПоСотрудникам.Сумма,
		|	НАЧАЛОПЕРИОДА(УдержанияПоСотрудникам.Период, МЕСЯЦ),
		|	УдержанияПоСотрудникам.Период,
...
Процедура ЗаполнитьДанныеВзносов(ТаблицаВзносов, ПериодРегистрации, Организация, СтраховыеВзносы)
	...
	// 4D:ERP для Беларуси, Яна, 04.09.2017 12:12:32 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПоЗарплате.ФСЗН);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПоЗарплате.ПенсионныйФонд);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПоЗарплате.Белгосстрах);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПоЗарплате.ПС);
	ВидыОпераций.Добавить(Перечисления.ВидыОперацийПоЗарплате.СС);
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Яна, 04.09.2017 12:15:20 
	// Локализовать документ "Отражение зарплаты в финансовом учете", №15826
	// {
	|			КОГДА ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ФСЗН)
	|				ТОГДА СтраховыеВзносы.ФСЗН
	|			КОГДА ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ПенсионныйФонд)
	|				ТОГДА СтраховыеВзносы.ПенсионныйФонд	
	|			КОГДА ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.Белгосстрах)
	|				ТОГДА СтраховыеВзносы.Белгосстрах
	|			КОГДА ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ПС)
	|				ТОГДА СтраховыеВзносы.ПС
	|			КОГДА ВидыОперацийПоЗарплате.Ссылка = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.СС)
	|				ТОГДА СтраховыеВзносы.СС
	|			ИНАЧЕ 0
	// }
	// 4D	
	|		КОНЕЦ) КАК Сумма
