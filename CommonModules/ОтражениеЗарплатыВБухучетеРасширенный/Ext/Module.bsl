...
Процедура ПлановыеНачисленияПриЗаписи(НаборЗаписей) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДатыПоследнихДвижений.Регистратор,
...
Процедура СоздатьВТБухучетНачисленийПоСтатьямФинансирования(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, МассивФизическихЛиц = Неопределено,
																ИсключаемыйРегистратор = Неопределено, ИсключатьМежрасчетныеВыплаты = Ложь, ТолькоДоходыСотрудников = Ложь)
...
Функция ВыполнитьОтражениеНачисленийВБухучете(ИсходныеДанные) Экспорт
	...
	|	БухучетНачислений.КодСтатьиФинансирования
	|ИЗ
...
// Выполняет отражение начислений по договорам ГПХ в бухучете, создает временную таблицу с именем указанным в ИсходныеДанные - ИмяВТБухучетНачислений
//
// Параметры:
// 		ИсходныеДанные - структура, см ОписаниеИсходныхДанныхДляОтраженияНачисленийВБухучете
//
...
Функция ВыполнитьОтражениеНДФЛИКорректировокВыплатыВБухучете(ИсходныеДанные) Экспорт
	...
	Если НДФЛЗачтено.Количество()>0 Тогда
		...
		БазаРасчетаНДФЛ.Колонки.КорректировкаВыплаты.Имя = "Сумма";
...
Функция ВыполнитьОтражениеУдержанийИЗаймовВБухучете(ИсходныеДанные) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:31:11 
...
// 4D:ERP для Беларуси, Яна, 09.08.2017 11:50:31 
// Ошибки документа "Единовременное пособие за счет ФСС", №15698 
// {
#Область СлужебныйПрограммныйИнтерфейс_РаспределениеРезультатовРасчетаПоВидамДеятельностиЕНВД

//	Распределяет начисления по договорам из переданной таблицы по видам деятельности ЕНВД и не ЕНВД.
//	
// 	Структура таблицы
//		Сотрудник
//		Подразделение
...
//		СуммаДохода
//		СуммаВычетаВзносы
//		ОблагаетсяЕНВД, тип Булево.
//		
// Параметры:
//		Организация
//		МесяцНачисления
//		ИменаВременныхТаблиц - структура, содержит имена временных таблиц
//			ИмяВременнойТаблицыИсточник.
//			ИмяВременнойТаблицы - имя временной таблицы, в которую надо поместить результат
//		МенеджерВременныхТаблиц содержит временную таблицу с именем в параметре ИмяВременнойТаблицыИсточник.
//				Сотрудник
//				Начисление
//				Сумма
//				СуммаВычета
...
//				ДатаНачала
//
Процедура РаспределитьНачисленияПоДоговорамПоВидамДеятельностиЕНВДСоздатьВременнуюТаблицу(Организация, МесяцНачисления, МенеджерВременныхТаблиц, ИменаВременныхТаблиц) Экспорт

	ИмяВременнойТаблицыИсточник = ИменаВременныхТаблиц.ИмяВременнойТаблицыИсточник;
	ИмяВременнойТаблицы = ИменаВременныхТаблиц.ИмяВременнойТаблицы;
	...
	|ПОМЕСТИТЬ ВТНачисленияПоДоговорамВременная
	|ИЗ
	|	" + ИмяВременнойТаблицыИсточник + " КАК Начисления";
	Запрос.Выполнить();
	
	// Получим сведения о настройке бухучета начислений для договорников.
	СоздатьВТСведенияОБухучетеЗарплатыСотрудников(МенеджерВременныхТаблиц, "ВТНачисленияПоДоговорамВременная", "Сотрудник,МесяцНачисления", Организация);
	...
	|			И (СведенияОБухучетеЗарплатыСотрудников.Период = &МесяцНачисления)
	|
	|СГРУППИРОВАТЬ ПО
	|	Начисления.Подразделение,
	|	Начисления.Сотрудник,
	...
	|		КОГДА &ЕстьЕНВД
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(НачисленияПоДоговорам.ОтношениеКЕНВД, УсловияДоговораГПХСрезПоследних.ОтношениеКЕНВД) ЕСТЬ NULL 
	|							ИЛИ ЕСТЬNULL(НачисленияПоДоговорам.ОтношениеКЕНВД, УсловияДоговораГПХСрезПоследних.ОтношениеКЕНВД) = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ПустаяСсылка)
	|						ТОГДА БухучетСотрудниковПоДоговорам.ОтношениеКЕНВД
	|					ИНАЧЕ ЕСТЬNULL(НачисленияПоДоговорам.ОтношениеКЕНВД, УсловияДоговораГПХСрезПоследних.ОтношениеКЕНВД)
	|				КОНЕЦ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.НеЕНВД)
	|	КОНЕЦ КАК ОтношениеКЕНВД,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НачисленияПоДоговорам.Сумма, УсловияДоговораГПХСрезПоследних.Сумма) = 0
	|			ТОГДА 0
	|		КОГДА Начисления.Сумма = ЕСТЬNULL(НачисленияПоДоговорам.Сумма, УсловияДоговораГПХСрезПоследних.Сумма)
	|			ТОГДА ЕСТЬNULL(НачисленияПоДоговорам.СуммаЕНВД, УсловияДоговораГПХСрезПоследних.СуммаЕНВД)
	|		ИНАЧЕ ВЫРАЗИТЬ(ЕСТЬNULL(НачисленияПоДоговорам.СуммаЕНВД, УсловияДоговораГПХСрезПоследних.СуммаЕНВД) * (ВЫРАЗИТЬ(Начисления.Сумма / ЕСТЬNULL(НачисленияПоДоговорам.Сумма, УсловияДоговораГПХСрезПоследних.Сумма) КАК ЧИСЛО(25, 10))) КАК ЧИСЛО(15, 2))
	|	КОНЕЦ КАК СуммаЕНВД,
	|	ЕСТЬNULL(НачисленияПоДоговорам.ЗаключенСоСтудентомРаботающимВСтудотряде, УсловияДоговораГПХСрезПоследних.ЗаключенСоСтудентомРаботающимВСтудотряде) КАК ЗаключенСоСтудентомРаботающимВСтудотряде,
	|	ЕСТЬNULL(НачисленияПоДоговорам.КодДоходаСтраховыеВзносы, УсловияДоговораГПХСрезПоследних.КодДоходаСтраховыеВзносы) КАК КодДоходаСтраховыеВзносы
	|ПОМЕСТИТЬ ВТНачисленияПоДоговорамДляРаспределения
	|ИЗ
	|	#Начисления КАК Начисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановыеНачисленияПоДоговорам КАК НачисленияПоДоговорам
	|		ПО Начисления.Договор = НачисленияПоДоговорам.ДоговорАкт
	|			И (НачисленияПоДоговорам.МесяцНачисления = &МесяцНачисления)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТБухучетСотрудниковПоДоговорам КАК БухучетСотрудниковПоДоговорам
	|		ПО Начисления.Сотрудник = БухучетСотрудниковПоДоговорам.Сотрудник
	|			И Начисления.Подразделение = БухучетСотрудниковПоДоговорам.Подразделение
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УсловияДоговораГПХ.СрезПоследних(КОНЕЦПЕРИОДА(&МесяцНачисления, МЕСЯЦ), ) КАК УсловияДоговораГПХСрезПоследних
	|		ПО Начисления.Договор = УсловияДоговораГПХСрезПоследних.Договор
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ВЫБОР
	|				КОГДА Начисления.ОтношениеКЕНВД = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ОпределяетсяЕжемесячноПроцентом)
	|					ТОГДА ОтношениеКЕНВДЗатратНаЗарплату.Ссылка
	|				ИНАЧЕ Начисления.ОтношениеКЕНВД
	...
	|						ТОГДА Начисления.СуммаЕНВД
	|					ИНАЧЕ Начисления.Сумма - Начисления.СуммаЕНВД
	|				КОНЕЦ
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ОтношениеКЕНВДЗатратНаЗарплату.Ссылка = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ЕНВД)
	|						ТОГДА ВЫРАЗИТЬ(Начисления.СуммаВычета * Начисления.СуммаЕНВД / Начисления.Сумма КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ Начисления.СуммаВычета - (ВЫРАЗИТЬ(Начисления.СуммаВычета * Начисления.СуммаЕНВД / Начисления.Сумма КАК ЧИСЛО(15, 2)))
	|				КОНЕЦ
	|		ИНАЧЕ Начисления.СуммаВычета
	|	КОНЕЦ КАК СуммаВычетаВзносы,
	|	Начисления.КодДоходаСтраховыеВзносы КАК КодДоходаСтраховыеВзносы,
	|	Начисления.ЗаключенСоСтудентомРаботающимВСтудотряде КАК ЗаключенСоСтудентомРаботающимВСтудотряде
	|ПОМЕСТИТЬ ВТНачисленияСтраховыеВзносы
	|ИЗ
	|	ВТНачисленияПоДоговорамДляРаспределения КАК Начисления
	...
	|			И (Начисления.ОтношениеКЕНВД = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ОпределяетсяЕжемесячноПроцентом))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНачисленияПоДоговорамДляРаспределения
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТНачисленияПоДоговорамВременная
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТСведенияОБухучетеЗарплатыСотрудников";   
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#Начисления", ИмяВременнойТаблицыИсточник);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ВТСписокНачислений", ИмяВременнойТаблицы);
	Запрос.Текст = ТекстЗапроса;
	Запрос.Выполнить();

КонецПроцедуры

// Создает ВТ распределения единовременных пособий ФСС по видам деятельности ЕНВД и не ЕНВД.
//	
// 	Структура временной таблицы, которая будет создана.
//		Сотрудник
//		Период
//		Начисление
//		ДоляЕНВД, тип Число
...
//		Организация
//		МесяцНачисления
//		МенеджерВременныхТаблиц содержит временную таблицу ВТНачисления.
//				Сотрудник
//				Подразделение
//				ДатаНачала - дата регистрации.
//				Начисление
//
Процедура СоздатьВТНачисленияСДаннымиЕНВДЕдиновременныхПособий(Организация, МесяцНачисления, МенеджерВременныхТаблиц) Экспорт
	...
	Если ЕстьЕНВД Тогда
		...
		Запрос.УстановитьПараметр("ЕжемесячнаяДоляЕНВДПособий", ЕжемесячнаяДоляЕНВДПособий(Организация, ПериодРегистрации));
		...
		|			ТОГДА &ЕжемесячнаяДоляЕНВДПособий
		...
		|			И Начисления.ТерриторияВыполненияРаботВОрганизации = СведенияОБухучетеЗарплаты.ТерриторияВыполненияРаботВОрганизации
...
Функция НачисленияСДаннымиЕНВДДляСтраховыхВзносов(Организация, МесяцНачисления, ДополнительныеПараметры) Экспорт

	НачисленияПоСотрудникам = ДополнительныеПараметры.НачисленияСРаспределениемПоЕНВД;
	Если НачисленияПоСотрудникам = Неопределено Или НачисленияПоСотрудникам.Количество() = 0 Тогда
	...
	ПроцентЕНВД = ОтражениеЗарплатыВБухучете.ПроцентЕНВД(Организация, МесяцНачисления);
	
	Если ДополнительныеПараметры.ПроверятьЕНВД И ПроцентЕНВД = Неопределено Тогда
		...
		ОтражениеЗарплатыВБухучете.ПроверитьНеобходимостьЗаполненияПроцентаЕНВДНаТекущийМесяц(Организация, МесяцНачисления, ДополнительныеПараметры.МенеджерВременныхТаблиц, "ВТНачисления", "Сотрудник,ДатаНачала");
	КонецЕсли;
	...
	НачисленияИсходная = Запрос.Выполнить().Выгрузить();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Начисления", НачисленияПоСотрудникам);
	Запрос.УстановитьПараметр("НачисленияИсходная", НачисленияИсходная);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СписокНачислений.Сотрудник КАК Сотрудник,
	|	СписокНачислений.Подразделение КАК Подразделение,
	...
	|ИЗ
	|	&Начисления КАК СписокНачислений
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СписокНачислений.Начисление КАК Начисление,
	|	СписокНачислений.ДатаНачала КАК ДатаНачала
	|ПОМЕСТИТЬ ВТНачисленияИсходная
	|ИЗ
	|	&НачисленияИсходная КАК СписокНачислений
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник,
	|	Подразделение,
	...
	|	Начисления.ДатаНачала КАК ДатаНачала,
	|	СУММА(Начисления.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ВТНачисленияПоСотрудникамСвод
	|ИЗ
	|	ВТНачисленияПоСотрудникам КАК Начисления
	|
	|СГРУППИРОВАТЬ ПО
	|	Начисления.Подразделение,
	|	Начисления.Начисление,
	...
	|ИЗ
	|	ВТНачисленияПоСотрудникамСвод КАК НачисленияСвод
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачисленияПоСотрудникам КАК Начисления
	...
	|	КОНЕЦ КАК ДоляЕНВД
	|ИЗ
	|	ВТНачисленияИсходная КАК НачисленияИсходная
...
Функция БухучетПервичногоДокумента(ДокументОснование) Экспорт
	...
	|	БухучетЗарплатыПервичныхДокументов.ОтношениеКЕНВД
	|ИЗ
...
Функция ЕжемесячнаяДоляЕНВДПособий(Организация, Дата)
	
	ЕжемесячнаяДоляПособийЕНВД = 0;
	
	ПроцентЕНВД = ОтражениеЗарплатыВБухучете.ПроцентЕНВД(Организация, Дата);
	Если ПроцентЕНВД = 100 Тогда
		ЕжемесячнаяДоляПособийЕНВД = 1;
	КонецЕсли;
	
	Возврат ЕжемесячнаяДоляПособийЕНВД;
...
Процедура СоздатьВТНастройкиНачислений(Запрос)
	...
	|		КОГДА НЕ &ИспользоватьСтатьиФинансирования
	|			ТОГДА ЛОЖЬ
	|		КОГДА Начисления.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоФактическимНачислениям)
	|			ТОГДА ЛОЖЬ
	|		КОГДА Начисления.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоБазовымРасчетам)
	|				И Начисления.ВидОперацииПоЗарплате В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюБюджет), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюФСС), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюФССНС), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ПособиеНаПогребение))
	|			ТОГДА ИСТИНА
...
Процедура СоздатьВТНастройкиБухучетаСотрудников(Запрос, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная)
	...
	Если НЕ ИспользоватьОбособленныеТерритории Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Начисления.ТерриторияВыполненияРаботВОрганизации = СведенияОБухучетеНачисленийСотрудников.ТерриторияВыполненияРаботВОрганизации", "");
	КонецЕсли;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТНачисленияДляРаспределения", ИмяВТНачисленияИсходная);
	Запрос.Выполнить();
...
Процедура СоздатьВТБухучетОсновногоЗаработка(Запрос, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная)
	...
	|	РаспределениеОсновногоЗаработка.ОблагаетсяЕНВД,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СтрокиСРаспределением.ИдентификаторСтроки,
	|	СтрокиСРаспределением.СтатьяФинансирования,
	|	СтрокиСРаспределением.СпособОтраженияЗарплатыВБухучете,
	|	&Статья211 КАК СтатьяРасходов,
	|	СтрокиСРаспределением.ОблагаетсяЕНВД,
	...
	|	БухучетОсновногоЗаработка.ОблагаетсяЕНВД,
	|	НачислениеВидНачисленияОплатыТрудаДляНУ.ВидНачисленияОплатыТрудаДляНУ
	|ПОМЕСТИТЬ ВТБухучетОсновногоЗаработка
	|ИЗ
	|	ВТБухучетОсновногоЗаработкаВременная КАК БухучетОсновногоЗаработка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНачислениеВидНачисленияОплатыТрудаДляНУ КАК НачислениеВидНачисленияОплатыТрудаДляНУ
	|		ПО БухучетОсновногоЗаработка.Начисление = НачислениеВидНачисленияОплатыТрудаДляНУ.Начисление
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура СоздатьВТБухучетНачисленийПоДокументамИПлановые(Запрос, ИмяВТНачисленияИсходная)
	...
	|	БухучетПервичныхДокументов.ОтношениеКЕНВД
	...
	|	БухучетПлановыхНачислений.ОтношениеКЕНВД
...
Процедура СоздатьВТБухучетНачисленийИзНастроекНачисления(Запрос, ИмяВТНачисленияИсходная)
	...
	|		КОГДА НЕ &ЕстьЕНВД
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.НеЕНВД)
	|		КОГДА СведенияОБухучетеНачислений.ОтношениеКЕНВД <> ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ПустаяСсылка)
	|			ТОГДА СведенияОБухучетеНачислений.ОтношениеКЕНВД
	|		ИНАЧЕ НастройкиБухучетаСотрудников.ОтношениеКЕНВД
	|	КОНЕЦ КАК ОтношениеКЕНВД,
	|	ЕСТЬNULL(НастройкиНачислений.СтатьяФинансированияОпределяетсяПоСреднемуЗаработку, ЛОЖЬ) КАК СтатьяФинансированияОпределяетсяПоСреднемуЗаработку,
...
Процедура СоздатьВТБухучетРасходыПоСтрахованию(Запрос, КоэффициентыСреднегоЗаработкаФССДокумента, КоэффициентыСреднегоЗаработкаДокумента, СтрокиКоэффициентыСреднегоЗаработка, ИмяВТНачисленияИсходная)
	...
	|		КОГДА НЕ &РаботаВБюджетномУчреждении
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
	|		КОГДА НЕ ЕСТЬNULL(НастройкиНачислений.ЭтоРасходыПоСтрахованиюРаботодатель, ЛОЖЬ)
	|				И НЕ ЕСТЬNULL(НастройкиНачислений.ЭтоРасходыПоСтрахованиюБюджет, ЛОЖЬ)
	|			ТОГДА &Статья213
	|		ИНАЧЕ ВЫБОР
	...
	|	КОНЕЦ КАК СтатьяРасходов,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(НастройкиНачислений.ЭтоРасходыПоСтрахованиюРаботодатель, ЛОЖЬ)
	|				ИЛИ ЕСТЬNULL(НастройкиНачислений.ЭтоРасходыПоСтрахованиюБюджет, ЛОЖЬ)
	|			ТОГДА ВЫБОР
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.НеЕНВД)
	|		ИНАЧЕ ВЫБОР
	|				КОГДА НЕ СведенияОБухучетеНачислений.ОтношениеКЕНВД ЕСТЬ NULL
	|						И СведенияОБухучетеНачислений.ОтношениеКЕНВД <> ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ПустаяСсылка)
	|					ТОГДА СведенияОБухучетеНачислений.ОтношениеКЕНВД
	|				ИНАЧЕ НастройкиБухучетаСотрудников.ОтношениеКЕНВД
	|			КОНЕЦ
	|	КОНЕЦ КАК ОтношениеКЕНВД,
	|	ЕСТЬNULL(НастройкиНачислений.СтатьяФинансированияОпределяетсяПоСреднемуЗаработку, ЛОЖЬ) КАК СтатьяФинансированияОпределяетсяПоСреднемуЗаработку,
	|	ЕСТЬNULL(НастройкиНачислений.СтратегияОтраженияВУчете, ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПустаяСсылка)) КАК СтратегияОтраженияВУчете,
	|	НастройкиНачислений.ЭтоОплатаДнейУходаЗаДетьмиИнвалидами,
	|	Начисления.Сумма
	...
	|	БухучетРасходыПоСтрахованиюВременная.ОтношениеКЕНВД,
	|	БухучетРасходыПоСтрахованиюВременная.СтатьяФинансированияОпределяетсяПоСреднемуЗаработку,
	|	БухучетРасходыПоСтрахованиюВременная.ЭтоОплатаДнейУходаЗаДетьмиИнвалидами,
	...
	|	ВТБухучетРасходыПоСтрахованиюВременная КАК БухучетРасходыПоСтрахованиюВременная
	|ГДЕ
	|	БухучетРасходыПоСтрахованиюВременная.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоФактическимНачислениям)";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТНачисленияДляРаспределения", ИмяВТНачисленияИсходная);
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	РезультатПоБазе = РезультатЗапроса[1];
	РезультатПоФактическим = РезультатЗапроса[2];
	
	ТаблицаБухучетПоСреднемуЗаработку = НоваяТаблицаБухучетНачисленияУдержанияПоСотрудникам();
	ТаблицаБухучетПоФактическимНачислениям = ТаблицаБухучетПоСреднемуЗаработку.СкопироватьКолонки();
	...
	Если Не РезультатПоБазе.Пустой() Тогда
		...
		Для каждого СтрокаНачисления Из НачисленияПоБазе Цикл
			...
			Если СтрокиОтражения = Неопределено Тогда
				...
				Если СтрокаНачисления.ЭтоОплатаДнейУходаЗаДетьмиИнвалидами Тогда
					СтрокиОтражения = КоэффициентыСреднегоЗаработкаДокумента[Перечисления.СпособыРасчетаНачислений.ПустаяСсылка()];
				Иначе
	...
	Если Не РезультатПоФактическим.Пустой() Тогда
		...
		Если Месяц(ПериодРегистрации) > 1 Тогда
			...
			НачисленияПоБазе = РезультатПоФактическим.Выгрузить();
			...
			Для каждого СтрокаНачисления Из НачисленияПоБазе Цикл
				...
				Если СтрокаНачисления.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюРаботодатель Тогда
				...
				Для Каждого СтрокаОтражения Из СтрокиОтражения Цикл
					...
					Если СтрокаНачисления.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюРаботодатель Тогда
						...
						СтрокаТаблицы.ОтношениеКЕНВД = ?(СтрокаОтражения.ОблагаетсяЕНВД, Перечисления.ОтношениеКЕНВДЗатратНаЗарплату.ЕНВД, Перечисления.ОтношениеКЕНВДЗатратНаЗарплату.НеЕНВД);
	...
	|ПОМЕСТИТЬ ВТБухучетПоСреднемуЗаработку
	|ИЗ
	...
	|	БухучетНачислений.Сумма,
	|	БухучетНачислений.ОтношениеКЕНВД
	|ПОМЕСТИТЬ ВТБухучетПоФактическимНачислениям
	|ИЗ
	...
	|ИЗ
	|	ВТБухучетПоСреднемуЗаработку КАК БухучетНачислений
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	БухучетНачислений.ОтношениеКЕНВД
	|ИЗ
	|	ВТБухучетПоФактическимНачислениям КАК БухучетНачислений
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И НЕ БухучетНачислений.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоФактическимНачислениям)
	|	И НЕ(БухучетНачислений.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.РасходыПоСтрахованиюРаботодатель)
	|				И БухучетНачислений.СтратегияОтраженияВУчете = ЗНАЧЕНИЕ(Перечисление.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоДаннымОСотрудникеИЕгоПлановыхНачислениях))";
	Запрос.Выполнить();
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТБухучетПоФактическимНачислениям
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТБухучетПоСреднемуЗаработку";
	Запрос.Выполнить();
	
КонецПроцедуры

Процедура СоздатьВТБухучетНачисленийБезРаспределяемыхПоБазе(Запрос, КоэффициентыСреднегоЗаработкаДокумента, СтрокиКоэффициентыСреднегоЗаработка, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная)
	...
	|	БухучетНачисленийВременная.ОтношениеКЕНВД,
	...
	Если НЕ ИспользоватьОбособленныеТерритории Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И БухучетНачисленийВременная.ТерриторияВыполненияРаботВОрганизации = Начисления.ТерриторияВыполненияРаботВОрганизациии", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (Начисления.ТерриторияВыполненияРаботВОрганизации = НастройкиБухучетаСотрудников.ТерриторияВыполненияРаботВОрганизации)", "");
	КонецЕсли;
	...
	|	БухучетНачислений.Сумма,
	|	БухучетНачислений.ОтношениеКЕНВД
	|ПОМЕСТИТЬ ВТБухучетНачисленияСтатьяФинансированияПоБазе
	|ИЗ
	...
	|ИЗ
	|	ВТБухучетНачисленияСтатьяФинансированияПоБазе КАК БухучетНачислений
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ОтношениеКЕНВДЗатратНаЗарплату.Ссылка = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ЕНВД)
	|						ТОГДА ВЫРАЗИТЬ(БухучетНачислений.Сумма * &ПроцентЕНВД / 100 КАК ЧИСЛО(15, 2))
	|					ИНАЧЕ БухучетНачислений.Сумма - (ВЫРАЗИТЬ(БухучетНачислений.Сумма * &ПроцентЕНВД / 100 КАК ЧИСЛО(15, 2)))
	|				КОНЕЦ
	|		ИНАЧЕ БухучетНачислений.Сумма
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА ВЫБОР
	|				КОГДА БухучетНачислений.ОтношениеКЕНВД = ЗНАЧЕНИЕ(Перечисление.ОтношениеКЕНВДЗатратНаЗарплату.ОпределяетсяЕжемесячноПроцентом)
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТБухучетНачисленияСтатьяФинансированияПоБазе";
	
	Запрос.Выполнить(); 	
	
КонецПроцедуры

Процедура СоздатьВТБухучетНачисленияПоБазе(Запрос, ИспользоватьОбособленныеТерритории, УчитыватьСторноЗаписи, МенеджерКадровогоУчета)
	...
	Если Не РезультатОчередностьРаспределенияПоБазе.Пустой() Тогда
		...
		|	ИЗ
		|		ВТБухучетНачисленийСторно КАК БухучетНачисленийФормы
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		БухучетНачисления.ОблагаетсяЕНВД
		|	ИЗ
		|		ВТБухучетОсновногоЗаработка КАК БухучетНачисления) КАК БухучетНачислений
		|
		|СГРУППИРОВАТЬ ПО
		|	БухучетНачислений.Организация,
		|	БухучетНачислений.Сотрудник,
		...
		РасчетЗарплатыРасширенный.СоздатьВТРасчетнаяБазаНачисленийПоВременнойТаблице(Запрос.МенеджерВременныхТаблиц, Запрос.Параметры.ИсключаемыйРегистратор, МенеджерКадровогоУчета);
...
Процедура СоздатьВТБухучетОсновногоЗаработкаРаспределениеПоПодразделению(Запрос, ИмяВТНачисленияИсходная)
	...
	Если Распределение.Количество() = 0 Тогда
		...
		|	НЕОПРЕДЕЛЕНО КАК ДокументОснование,
		|	0 КАК Сумма,
		|	ЛОЖЬ КАК ВидНачисленияОплатыТрудаДляНУ,
		|	ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка) КАК СтатьяФинансирования,
		|	ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка) КАК СтатьяРасходов,
		|	ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка) КАК СпособОтраженияЗарплатыВБухучете,
		|	ЛОЖЬ КАК ОблагаетсяЕНВД
	...
	Иначе
		...
		|	БухучетНачисления.СтатьяРасходов,
		|	БухучетНачисления.Сумма,
		|	БухучетНачисления.ОтношениеКЕНВД
		...
		|		КОГДА НЕ &ИспользоватьСтатьиФинансирования
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиФинансированияЗарплата.ПустаяСсылка)
		|		ИНАЧЕ БухучетОсновногоЗаработка.СтатьяФинансирования
		|	КОНЕЦ КАК СтатьяФинансирования,
		|	ВЫБОР
		|		КОГДА НЕ &РаботаВБюджетномУчреждении
		|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтатьиРасходовЗарплата.ПустаяСсылка)
		|		ИНАЧЕ БухучетОсновногоЗаработка.СтатьяРасходов
		|	КОНЕЦ КАК СтатьяРасходов,
...
Процедура СоздатьВТБухучетНачисленийБезДоговоровГПХ(ИсходныеДанные, ИмяВТБухучетНачислений) Экспорт
	...
	СоздатьВТНастройкиБухучетаСотрудников(Запрос, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная);
	...
	УдалитьВТ.Добавить("ВТСведенияОБухучетеНачислений");
	...
	СоздатьВТБухучетНачисленийПоДокументамИПлановые(Запрос, ИмяВТНачисленияИсходная);
	УдалитьВТ.Добавить("ВТБухучетНачисленийПоДокументамИПлановые");
	
	// ВТБухучетРасходыПоСтрахованию 
	СоздатьВТБухучетРасходыПоСтрахованию(Запрос, КоэффициентыСреднегоЗаработкаФССДокумента, КоэффициентыСреднегоЗаработкаДокумента, СтрокиКоэффициентыСреднегоЗаработка, ИмяВТНачисленияИсходная);
	УдалитьВТ.Добавить("ВТБухучетРасходыПоСтрахованию");
	...
	СоздатьВТБухучетНачисленийИзНастроекНачисления(Запрос, ИмяВТНачисленияИсходная);
	УдалитьВТ.Добавить("ВТБухучетНачисленийИзНастроекНачисления");
	...
	|	СписокНачислений.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	СписокНачислений.Сторно,
	|	СписокНачислений.ФиксСторно,
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТНачисленияДляРаспределения", ИмяВТНачисленияИсходная);
	Если Не УчитыватьСторноЗаписи Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СписокНачислений.Сторно,", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СписокНачислений.ФиксСторно,", "");
	КонецЕсли;
	Запрос.Выполнить();
	...
	СоздатьВТБухучетНачисленийБезРаспределяемыхПоБазе(Запрос, КоэффициентыСреднегоЗаработкаДокумента, СтрокиКоэффициентыСреднегоЗаработка, ИспользоватьОбособленныеТерритории, ИмяВТНачисленияИсходная);
	УдалитьВТ.Добавить("ВТБухучетНачисленийБезРаспределяемыхПоБазе");
	
	// ВТБухучетНачисленияПоБазе
	СоздатьВТБухучетНачисленияПоБазе(Запрос, ИспользоватьОбособленныеТерритории, УчитыватьСторноЗаписи, ИсходныеДанные.МенеджерКадровогоУчета);
...
// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:56:18 
// Ошибка в документе "Материальная помощь", № 17211
// {
...
Функция НоваяТаблицаРезультатРасчетаНДФЛ() Экспорт
	...
	Таблица.Колонки.Добавить("Территория", Новый ОписаниеТипов(Метаданные.ОпределяемыеТипы.ТерриторииВыполненияРаботВОрганизации.Тип));
	Таблица.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число", , , Новый КвалификаторыЧисла(15, 2)));
...
// 4D:ERP для Беларуси, Яна, 22.08.2017 14:24:49 
...
Функция БазаДляРаспределенияНДФЛ(МенеджерВременныхТаблиц, МассивФизическихЛиц, ПериодРегистрации, Организация, ОкончательныйРасчет, ИсключаемыйРегистратор)
	...
	Если ОкончательныйРасчет Тогда
		...
		|	База.ДатаПолученияДохода,
		|	База.ДоходМежрасчетногоПериода
		|ПОМЕСТИТЬ ВТБаза
		|ИЗ
		|	(ВЫБРАТЬ
		...
		|		И НЕ СведенияОДоходах.Регистратор В
		|					(ВЫБРАТЬ
		|						ИсключаемыеРегистраторы.Регистратор
		|					ИЗ
		|						ВТИсключаемыеРегистраторы КАК ИсключаемыеРегистраторы)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|		И СведенияОДоходах.Период МЕЖДУ &НачалоМесяца И &КонецМесяца
		|		И СведенияОДоходах.КодДохода.СтавкаНалогообложенияРезидента = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13)
		|		И СведенияОДоходах.ФизическоеЛицо В(&ФизическиеЛица)) КАК База
		|
		|СГРУППИРОВАТЬ ПО
		|	База.ФизическоеЛицо,
		|	База.Сотрудник,
...
Процедура РаспределитьНДФЛПоБазе(РезультатыРаспределения, ТаблицаДляРаспределения, База, БазаВсеНачисления, СтрокиУжеУдержаноПоФизическимЛицам, Организация, ПериодРегистрации, ОтборТолькоПоФизлицу = Ложь)
	...
	Для Каждого СтрокаНДФЛ Из ТаблицаДляРаспределения Цикл
		...
		СтарыеДанные = НЕ ЗначениеЗаполнено(СтрокаНДФЛ.ДатаПолученияДохода) Или СтрокаНДФЛ.ДатаПолученияДохода < УчетНДФЛ.ДатаИзмененияПорядкаИсчисленияНалогаДляИностранцев();
		УчитыватьУжеРаспределенные = (СтрокаНДФЛ.Сумма <> 0) И Не СтарыеДанные;
		...
		Иначе // распределение строк НДФЛ
			...
			НайденныеСтрокиПоФизлицу = База.НайтиСтроки(Отбор);
			
			Если СтарыеДанные Тогда
				
				СтрокиРаспределенияБезТерритории = Новый Массив;
				Для каждого СтрокаТЗ Из НайденныеСтрокиПоФизлицу Цикл
			...
			Иначе
				...
				Если СтрокиРаспределения.Количество() = 0 Тогда
					...
					Если СтрокиРаспределения.Количество() = 0 Тогда
						СтрокиРаспределения = НайденныеСтрокиПоФизлицу;
					КонецЕсли;
				КонецЕсли;
			...
			Если СтрокиРаспределения.Количество() = 0 Тогда
				...
				НайденныеСтрокиПоФизлицу = БазаВсеНачисления.НайтиСтроки(Отбор);
				Для каждого СтрокаТЗ Из НайденныеСтрокиПоФизлицу Цикл
					Если СтрокаТЗ.Территория = СтрокаНДФЛ.Территория Тогда
						СтрокиРаспределения.Добавить(СтрокаТЗ);
					КонецЕсли;
		...
		Если УчитыватьУжеРаспределенные И СтрокиУжеУдержаноПоФизическомуЛицу <> Неопределено Тогда
			Для Каждого СтрокаУжеУдержаноПоФизическомуЛицу Из СтрокиУжеУдержаноПоФизическомуЛицу Цикл
				Если СтрокаУжеУдержаноПоФизическомуЛицу.ВидУдержания = СтрокаНДФЛ.ВидУдержания 
						...
						И СтрокаУжеУдержаноПоФизическомуЛицу.Территория = СтрокаНДФЛ.Территория Тогда
...
Процедура ВыполнитьРаспределениеУдержаний(БухучетУдержаний, ИсходныеДанные, БазовыеНачисления, СтрокиУжеУдержаноПоФизическимЛицам)
	...
	// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:33:11 
	// Ошибка в документе "Материальная помощь", № 17211
	// {
	ТаблицаУдержаний = ИсходныеДанные.ТаблицаУдержаний;
...
// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:29:50 
// Ошибка в документе "Материальная помощь", № 17211
// {
Процедура РаспределитьУдержанияПоБазе(ТаблицаРаспределениеРезультатовУдержаний, ТаблицаУдержаний, БазаУдержаний, Организация, ПериодРегистрации)
...
Функция БухучетНачисленийСторноИзИсходнойТаблицы(ТаблицаНачислений, СтрокиНачисленийСторно, ИспользоватьСтатьиФинансирования, РаботаВБюджетномУчреждении, ФиксСторно)
	...
	ТаблицаРаспределение.Колонки.Добавить("Организация", Новый ОписаниеТипов("СправочникСсылка.Организации"));
...
Функция УпорядоченнаяТаблицаРаспределениеРезультатовУдержаний(ТаблицаРаспределениеРезультатовУдержаний)
	...
	|	ТаблицаРаспределения.ВидУдержания,
	...
	|	ТаблицаРаспределения.КодСтатьиФинансирования
...
Процедура ЗаполнитьПредставлениеРаспределения(ДанныеЗаполнения, ОписаниеДокумента, ИмяТаблицы, ВидРасчетаИзШапки, РаботаВБюджетномУчреждении, НачислениеУдержаниеВидОперации) 
	...
	Для Каждого СтрокаТаблицы Из ДанныеЗаполнения Цикл
		...
		// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:38:07 
		// Ошибка в документе "Материальная помощь", № 17211
		// {
		РезультатПроверкиСтроки = ПроверитьРезультатыРаспределенияНачисленийУдержанийВСтроке(
			СтрокаТаблицы, ИмяТаблицы, ОписаниеДокумента, ВидРасчетаИзШапки, НачислениеУдержаниеВидОперации);
		// }
		// 4D
...
// 4D:ERP для Беларуси, Юлия, 08.01.2018 14:37:29 
// Ошибка в документе "Материальная помощь", № 17211
// {
Функция ПроверитьРезультатыРаспределенияНачисленийУдержанийВСтроке(ПроверяемаяСтрока, ИмяТаблицы, ОписаниеДокумента, ВидРасчетаИзШапки, НачислениеУдержаниеВидОперации)
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ОтражениеЗарплатыВБухучетеРасширенный.ОбновитьСтратегиюОтраженияВБухучетеДнейУходаЗаДетьмиИнвалидами";
...
Процедура ОбновитьСтратегиюОтраженияВБухучетеДнейУходаЗаДетьмиИнвалидами() Экспорт
	...
	|	И Начисления.СтатьяФинансированияОпределяетсяПоСреднемуЗаработку";
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ВРОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ВРОбъект.СтатьяФинансированияОпределяетсяПоСреднемуЗаработку = Ложь;
		ВРОбъект.СтратегияОтраженияВУчете = Перечисления.СтратегииОтраженияВУчетеНачисленийУдержаний.ПоБазовымРасчетам;
...
Процедура ЗаполнитьДвиженияБухучетНачисленияУдержанияПоСотрудникамПерерасчетНДФЛ(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("Регистратор") Цикл
		...
		Если Не ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ПодготовитьОбновлениеДанных(ПараметрыОбновления, "РегистрНакопления.БухучетНачисленияУдержанияПоСотрудникам", "Регистратор", Выборка.Регистратор) Тогда
			Продолжить;
		КонецЕсли;
...
// 4D:ERP для Беларуси, Юлия, 08.01.2018 9:47:36 
// Ошибка в документе "Материальная помощь", № 17211
// {
Функция ОписаниеПараметровДляРаспределенияДанныхЗаполнения() Экспорт

	ПараметрыДляРаспределения = Новый Структура;
	ПараметрыДляРаспределения.Вставить("НачисленияСРаспределениемПоСтатьям", Неопределено);
	ПараметрыДляРаспределения.Вставить("ТаблицаРаспределениеРезультатовНачислений", Неопределено);
	...
	ПараметрыДляРаспределения.Вставить("НДФЛРассчитывался", Ложь);
...
//
Процедура ПолучитьРаспределениеДанныеФормыПоСотрудникам(СотрудникиФизическиеЛица, Форма, ОписаниеДокумента, ОписаниеТаблицы = Неопределено, Движения = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ДополнительныеПараметры = Неопределено Тогда
		ДополнительныеПараметры = ОписаниеПараметровДляРаспределенияДанныхЗаполнения();
		РаспределятьНачисления = Истина;
		РаспределятьУдержания  = Истина;
		НДФЛРассчитывался = Ложь;
	Иначе
		...
		НДФЛРассчитывался = ДополнительныеПараметры.НДФЛРассчитывался;
	КонецЕсли;
	...
	КонецЕсли;
	ВыплатаВМежрасчетныйПериод = ПорядокВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Межрасчет;
	...
	Если ОписаниеТаблицы = Неопределено Тогда
		// Не меняем значения параметров РаспределятьНачисления и РаспределятьУдержания.
		РаспределятьПогашениеЗаймов = Истина;
		
	Иначе
		
		ИменаТаблиц = ОписаниеТаблицы.ИмяТаблицы;
		ОбрабатыватьНачисления = (Найти(ИменаТаблиц,"Начисления") > 0);
		ОбрабатыватьНачисления = ОбрабатыватьНачисления Или (Найти(ИменаТаблиц,"ОплатаТруда") > 0);
		ОбрабатыватьНачисления = ОбрабатыватьНачисления Или (Найти(ИменаТаблиц,"Пособия") > 0);
		ОбрабатыватьУдержания  = ОбрабатыватьНачисления Или (Найти(ИменаТаблиц,"Удержания") > 0) Или (Найти(ИменаТаблиц,"НДФЛ") > 0);
		РаспределятьПогашениеЗаймов = ОбрабатыватьНачисления Или (Найти(ИменаТаблиц,"ПогашениеЗаймов") > 0);
		
		РаспределятьНачисления = РаспределятьНачисления И ОбрабатыватьНачисления;
		РаспределятьУдержания  = РаспределятьУдержания И ОбрабатыватьУдержания;
	...
	Если РаспределятьНачисления Тогда
		...
		Если ОписаниеДокумента.ЕстьОплатаПоСреднему Тогда
			...
			Если ОписаниеДокумента.СпособыРасчетаСреднегоЗаработка <> Неопределено Тогда
				...
				ОписаниеРасчетаДляОтпусков = УчетСреднегоЗаработкаКлиентСервер.ОписаниеРасчетаДляОтпусков();
				
				Для Каждого СпособРасчета Из ОписаниеДокумента.СпособыРасчетаСреднегоЗаработка Цикл
					
					ОписаниеРасчетаДляОтпусков.СпособРасчетаОтпуска = СпособРасчета.Значение;
					ОписаниеРасчетаДляОтпусков.ДатаНачалаСобытия 	= Объект[ОписаниеДокумента.ДатаНачалаСобытияИмя];
					
					ДанныеДляРасчетаСреднегоЗаработка = УчетСреднегоЗаработкаКлиентСервер.ДанныеДляРасчетаСреднегоЗаработка(Объект.СреднийЗаработокОбщий, Объект.ОтработанноеВремяДляСреднегоОбщий, 
																	Объект.ДанныеОбИндексации, Объект.ПериодРасчетаСреднегоЗаработкаНачало, Объект.ПериодРасчетаСреднегоЗаработкаОкончание, 
																	ОписаниеДокумента.ЭтоСреднеЧасовойЗаработок, ОписаниеРасчетаДляОтпусков, Истина); 
																														
					Для Каждого СтрокаНачислений Из ДанныеДляРасчетаСреднегоЗаработка["Заработок"] Цикл
											
						НоваяСтрока	 = КоэффициентыРаспределенияСреднегоЗаработка.Добавить();
						НоваяСтрока.СпособРасчета 		 = СпособРасчета.Значение;
	...
	Если РаспределятьУдержания Или РаспределятьПогашениеЗаймов Тогда
		...
		Для каждого ОписаниеКлючИЗначение Из ОписанияУдержаний Цикл
			...
			Если ИмяТаблицы <> Неопределено Тогда
				...
				Если Не РаспределятьПогашениеЗаймов И ОписаниеКлючИЗначение.Ключ = "ПогашениеЗаймов" Тогда
					Продолжить;
				ИначеЕсли Не РаспределятьУдержания И (ОписаниеКлючИЗначение.Ключ = "УдержанияИмя" Или ОписаниеКлючИЗначение.Ключ = "НДФЛИмя") Тогда
					Продолжить;
				КонецЕсли;
				
				ТаблицаКРаспределению = СоздатьТаблицуЗначенийПоОбразуКоллекции(Объект[ИмяТаблицы]);
				СтрокиКРаспределению  = СтрокиНачисленийУдержанийКРаспределению(Объект[ИмяТаблицы], "ФизическоеЛицо", Физлица, ТаблицаКРаспределению, ОписаниеКлючИЗначение.Значение);
				
				Если ТаблицаКРаспределению.Количество() > 0 Тогда
					ДанныеФормы.Вставить(ИмяТаблицы, ТаблицаКРаспределению);
					ДанныеФормы.Вставить(ИмяТаблицы + "СтрокиКРаспределению", СтрокиКРаспределению);
					Если ОписаниеКлючИЗначение.Ключ = "НДФЛИмя" Тогда
						РаспределятьНДФЛ = Истина;
					КонецЕсли;
				ИначеЕсли ОписаниеКлючИЗначение.Ключ = "ПогашениеЗаймов" Тогда
	...
	ПараметрыДляРаспределения.ОкончательныйРасчет = НЕ ВыплатаВМежрасчетныйПериод;
	
	Если (Не НДФЛРассчитывался Или ВыплатаВМежрасчетныйПериод) И РаспределятьНДФЛ Тогда
		//необходимо собрать доходы для учета НДФЛ
		ВсеСотрудники = КадровыйУчетРасширенный.СотрудникиФизическихЛиц(Физлица, Организация, Ложь);
	...
	Если РаспределятьНачисления Тогда
		...
		ДополнительныеПараметры.НачисленияСРаспределениемПоСтатьям = ТаблицаРаспределениеРезультатовНачислений.Скопировать(,"Сотрудник,Подразделение,Начисление,ДатаНачала,ОблагаетсяЕНВД,Сумма");
		
		// Перенесем результаты распределения начислений в строки таблиц структуры ДанныеЗаполнения.
		ИменаКолонок = "СтатьяФинансирования,СтатьяРасходов,Результат,СпособОтраженияЗарплатыВБухучете,ОблагаетсяЕНВД";
		Для каждого ОписаниеКлючИЗначение Из ОписанияНачислений Цикл
			ИмяТаблицы = ОписаниеДокумента[ОписаниеКлючИЗначение.Ключ];
	...
	Если РаспределятьУдержания Или РаспределятьПогашениеЗаймов Тогда
		...
		ВсеСотрудники = КадровыйУчетРасширенный.СотрудникиФизическихЛиц(Физлица, Организация, Ложь);
		ПараметрыДляРаспределения.БухучетНачисленийИзФормы = БухучетНачисленийИзФормыПоСотрудникам(Форма, ВсеСотрудники, СоответствиеСотрудникФизическоеЛицо, ОписаниеДокумента.ОписанияТаблицДляРаспределенияРезультата, НачислениеИзШапкиДокумента);
...
Функция СотрудникиФизическиеЛица(Знач СотрудникиФизическиеЛица, Организация)
	...
	Иначе
		...
		Сотрудники = КадровыйУчетРасширенный.СотрудникиФизическихЛиц(Физлица, Организация);
	КонецЕсли;
...
Функция ЗарегистрироватьДвиженияДокументаДляРаспределенияНачисленийУдержаний(Форма, Сотрудники, ОписаниеДокумента, РегистрироватьНачисления, РегистрироватьНДФЛ, ЗаписыватьДвижения)
	...
	Для каждого ОписаниеТаблицыФормы Из ОписаниеТаблицФормы Цикл
		...
		Если Найти(ВозможныеИменаТаблиц,ИмяТаблицы) > 0 Тогда
	...
	Если РегистрироватьНДФЛ Тогда
		...
		Начисления.Колонки.Добавить("ДатаПолученияДохода", Новый ОписаниеТипов("Дата"));
		Начисления.ЗаполнитьЗначения(МесяцНачисления, "ДатаПолученияДохода");
		Начисления.Колонки.Добавить("КодДохода", Новый ОписаниеТипов("СправочникСсылка.ВидыДоходовНДФЛ"));
		Начисления.ЗаполнитьЗначения(Справочники.ВидыДоходовНДФЛ.КодДоходаПоУмолчанию, "КодДохода");
		Начисления.Колонки.Результат.Имя = "СуммаДохода";
		
		Начисления.Колонки.Добавить("ПодразделениеСотрудника", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
		Для каждого СтрокаТЗ Из Начисления Цикл
			СтрокаТЗ.ПодразделениеСотрудника = СтрокаТЗ.Подразделение;
		КонецЦикла;
				
		НачисленияПоДоговорам.Колонки.Добавить("ДатаПолученияДохода", Новый ОписаниеТипов("Дата"));
		НачисленияПоДоговорам.ЗаполнитьЗначения(МесяцНачисления, "ДатаПолученияДохода");
		НачисленияПоДоговорам.Колонки.Результат.Имя = "СуммаДохода";
		НачисленияПоДоговорам.Колонки.Добавить("ПодразделениеСотрудника", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
		Для каждого СтрокаТЗ Из НачисленияПоДоговорам Цикл
			СтрокаТЗ.ПодразделениеСотрудника = СтрокаТЗ.Подразделение;
		КонецЦикла;
		
		// регистрация доходов
		УчетНДФЛ.СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(НаборыЗаписей, Ложь, Организация, МесяцНачисления, Начисления, ЗаписыватьДвижения, ЗаписыватьДвижения);
		УчетНДФЛ.СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(НаборыЗаписей, Ложь, Организация, МесяцНачисления, НачисленияПоДоговорам, ЗаписыватьДвижения, ЗаписыватьДвижения);
...
Процедура ОбновитьРезультатыРаспределенияВСтрокахОсновнойТаблицы(КоллекцияСтрок, ТаблицаРаспределения, ИменаКолонок, ИмяТаблицы, ОписаниеДокумента, ВидРасчетаИзШапки = Неопределено) 
	...
	Для каждого КлючИЗначение Из КоллекцияСтрок Цикл
		...
		ОтборСтрок.ИдентификаторСтроки = ИдентификаторСтроки;
		...
		////Юля (тут не хватало факт параметров)
		РезультатПроверкиСтроки = ПроверитьРезультатыРаспределенияНачисленийУдержанийВСтроке(
			СтрокаТаблицы, ИмяТаблицы, ОписаниеДокумента, ВидРасчетаИзШапки, НачислениеУдержаниеВидОперации);
...
Функция БухучетНачисленийИзФормыПоСотрудникам(Форма, Сотрудники, СоответствиеСотрудникФизическоеЛицо, ОписаниеТаблицФормы, ВидРасчетаИзШапки = Неопределено)
...
Функция РаспределениеРезультатовУдержанийПоИсточникамФинансирования(ПараметрыДляРаспределения)
	...
	Колонки = "ФизическоеЛицо,Удержание,Результат,ДокументОснование,ПлатежныйАгент,ДатаНачала,ДатаОкончания,ИдентификаторСтроки";
	...
	///Юля (тут не хватало пареметров)
	РаспределитьУдержанияПоБазе(ТаблицаРаспределениеРезультатовУдержаний, ТаблицаУдержаний, БазаУдержаний, Организация, ПериодРегистрации);
...
Процедура РассчитатьБазыНДФЛДляРаспределенияПоСтатьямФинансирования(БазаРасчетаНДФЛ, БазаВсеНачисления, БухучетНачисления, СведенияОДоходахНДФЛ, МассивФизическихЛиц, ПериодРегистрации, Организация, ОкончательныйРасчет, ДокументСсылка)
	...
	Иначе
		...
		|ПОМЕСТИТЬ ВТБаза
		|ИЗ
		|	ВТСведенияОДоходахНДФЛ КАК СведенияОДоходах
		|
		|СГРУППИРОВАТЬ ПО
		|	СведенияОДоходах.ФизическоеЛицо,
		|	СведенияОДоходах.Сотрудник,
