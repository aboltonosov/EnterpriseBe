...
Процедура СформироватьВТНачисленияСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ИсключаемыйРегистратор) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		|ГДЕ
		|	СведенияОДоходах.Регистратор В
		|			(ВЫБРАТЬ
		...
		|	НачисленияСоСведениямиРегистра.ДатаНачала,
		|	НачисленияСоСведениямиРегистра.ВидДохода,
		|	НачисленияСоСведениямиРегистра.Сумма,
		|	НачисленияСоСведениямиРегистра.Скидка,
		|	НачисленияСоСведениямиРегистра.ОблагаетсяВзносамиНаДоплатуЛетчикам,
		|	НачисленияСоСведениямиРегистра.ОблагаетсяВзносамиНаДоплатуШахтерам,
		|	НачисленияСоСведениямиРегистра.ОблагаетсяВзносамиЗаЗанятыхНаРаботахСДосрочнойПенсией,
		|	НачисленияСоСведениямиРегистра.КлассУсловийТруда,
		|	НачисленияСоСведениямиРегистра.ЯвляетсяДоходомФармацевта,
		|	НачисленияСоСведениямиРегистра.ЯвляетсяДоходомЧленаЭкипажаСуднаПодФлагомРФ,
		|	НачисленияСоСведениямиРегистра.ДатаПолученияДохода,
		|	НачисленияСоСведениямиРегистра.ЕстьСторнируемыеДанныеУчета,
		|	НачисленияСоСведениямиРегистра.СуммаВУчете,
		|	НачисленияСоСведениямиРегистра.СкидкаВУчете
		|ИЗ
		|	ВТНачисленияСоСведениямиРегистра КАК НачисленияСоСведениямиРегистра
...
// 4D:ERP для Беларуси, Яна, 16.08.2017 16:42:37 
// Локализация ЗП
// {
// Помещает в переданный МенеджерВременныхТаблиц таблицу 
//	ВТНачисленияСДаннымиУчета, которая содержит следующие поля.
...
//		ИсключаемыйРегистратор - ДокументСсылка -
//
Процедура СформироватьВТНачисленияСДаннымиУчета_Локализация(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ИсключаемыйРегистратор) Экспорт
	...
	"ВЫБРАТЬ
	|	СведенияОДоходах.ФизическоеЛицо,
	|	СведенияОДоходах.ВидДохода.ВходитВБазуФСЗН КАК ВходитВБазуФСЗН,
	|	СведенияОДоходах.КлассУсловийТруда,
	|	СУММА(СведенияОДоходах.Результат) КАК Результат
	|ПОМЕСТИТЬ ВТДоходыЗаПериод
	|ИЗ
	...
	|		ДанныеУчета.ВидДохода,
	|		ДанныеУчета.КлассУсловийТруда,
	|		ВЫРАЗИТЬ(СУММА(СтраховыеВзносыСведенияОДоходах.СуммаДохода * ЕСТЬNULL(ДанныеУчета.КоэффициентУчетаСтроки, 1)) КАК ЧИСЛО(15, 2))
	|	ИЗ
	|		ВТНачисления КАК СтраховыеВзносыСведенияОДоходах
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТСДаннымиУчета КАК ДанныеУчета
	|			ПО СтраховыеВзносыСведенияОДоходах.Сотрудник = ДанныеУчета.Сотрудник
	|				И СтраховыеВзносыСведенияОДоходах.Начисление = ДанныеУчета.Начисление
	|				И СтраховыеВзносыСведенияОДоходах.ПодразделениеОрганизации = ДанныеУчета.Подразделение
	|				И СтраховыеВзносыСведенияОДоходах.ДатаНачала = ДанныеУчета.ДатаНачала
	|				И СтраховыеВзносыСведенияОДоходах.УсловияТруда = ДанныеУчета.УсловияТруда
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ДанныеУчета.ВидДохода,
	|		ДанныеУчета.КлассУсловийТруда,
	|		ДанныеУчета.ФизическоеЛицо) КАК СведенияОДоходах
	|
	|СГРУППИРОВАТЬ ПО
	|	СведенияОДоходах.ФизическоеЛицо,
	|	СведенияОДоходах.КлассУсловийТруда,
	|	СведенияОДоходах.ВидДохода.ВходитВБазуФСЗН
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	СторнированиеНеТребуется = Запрос.Выполнить().Пустой();
	Если СторнированиеНеТребуется Тогда
		
		УчетСтраховыхВзносовБазовый.СформироватьВТНачисленияСДаннымиУчета_Локализация(Организация, ПериодРегистрации, МенеджерВременныхТаблиц, ПолучитьФункциональнуюОпцию("ИспользоватьОсобыеУсловияТруда", Новый Структура("Организация",Организация)));
	...
	Иначе	
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыЗаПериод КАК ДоходыЗаПериод
		|			ПО ДанныеУчета.ФизическоеЛицо = ДоходыЗаПериод.ФизическоеЛицо
		|				И ДанныеУчета.ВидДохода.ВходитВБазуФСЗН = ДоходыЗаПериод.ВходитВБазуФСЗН
		|				И ДанныеУчета.КлассУсловийТруда = ДоходыЗаПериод.КлассУсловийТруда
		|		ПО ВременнаяТаблица.Сотрудник = ДанныеУчета.Сотрудник
		|			И ВременнаяТаблица.Начисление = ДанныеУчета.Начисление
		|			И ВременнаяТаблица.ПодразделениеОрганизации = ДанныеУчета.Подразделение
	...
	Если Не Запрос.Выполнить().Пустой() Тогда // Попробуем скомпенсировать уже имеющиеся сторно-записи прошлых периодов.
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТДоходыЗаПериод КАК ДоходыЗаПериод
		|		ПО СтраховыеВзносыСведенияОДоходах.ФизическоеЛицо = ДоходыЗаПериод.ФизическоеЛицо
		|			И СтраховыеВзносыСведенияОДоходах.ВидДохода.ВходитВБазуФСЗН = ДоходыЗаПериод.ВходитВБазуФСЗН
		|			И СтраховыеВзносыСведенияОДоходах.КлассУсловийТруда = ДоходыЗаПериод.КлассУсловийТруда
		|ГДЕ
		|	СтраховыеВзносыСведенияОДоходах.Регистратор <> &ИсключаемыйРегистратор
		|	И СтраховыеВзносыСведенияОДоходах.ГоловнаяОрганизация = &ГоловнаяОрганизация
		...
		Если Запрос.Выполнить().Выгрузить()[0].Количество = 0 Тогда // Компенсировать нечего, сторно-записей ранее не было.
			Запрос.Текст = "УНИЧТОЖИТЬ ВТЗаписиЗаПрошлыйПериод";
		Иначе 	
			Запрос.Текст =
			"ВЫБРАТЬ
			|	ЗаписиЗаПрошлыйПериод.ФизическоеЛицо,
			...
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ВТЗаписиЗаПрошлыйПериод
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
	...
	Запрос.Текст = "УНИЧТОЖИТЬ ВТДоходыЗаПериод";
	Запрос.Выполнить();
...
Процедура СформироватьВТСДаннымиУчета(Организация, ПериодРегистрации, МенеджерВременныхТаблиц) Экспорт 
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьОсобыеУсловияТруда", Новый Структура("Организация",Организация)) Или ПериодРегистрации < УчетСтраховыхВзносовКлиентСервер.ДатаОбъединенияСтраховойИНакопительнойЧастейВзносовПФР() Тогда
		...
		// 4D:ERP для Беларуси, Яна, 16.08.2017 16:30:23 
	...
	|	СписокНачислений.Начисление,
	|	СписокНачислений.ПодразделениеОрганизации КАК Подразделение,
	|	СписокНачислений.УсловияТруда КАК УсловияТруда,
