...
Процедура ЗарегистрироватьТарифицируемыеУслуги_ОбработчикОбновления() Экспорт
	...
	Попытка
		...
		ТекстИнформации = НСтр("ru = 'Услуги успешно отправлены в Менеджер сервиса'", КодЯзыка());
...
Процедура ПроверитьСтруктуруПоставщиковУслуг(ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru = 'Имеется несколько поставщиков услуг с идентифкатором: '", КодЯзыка()) + Поставщик.Идентификатор;
		КонецЕсли;
	КонецЦикла;
