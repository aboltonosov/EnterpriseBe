...
// 4D:ERP для Беларуси, Яна, 07.08.2017 11:11:19 
// Результат тестирования конфигурации, №15540 
// {
////////////////////////////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции для отображения начислений в документах, выполняющих расчет.

// Процедура - часть механизма обработки периода (действия или базового) начисления в строках табличных частей.
// Если в текущей строке выбрано начисление, не использующее период - то очищаем его и делаем недоступными поля ввода.
//
// Параметры:
//				Данные - Текущие данные строки.
//				
//				ОписаниеТекущихДанных - Структура, содержащая поля
//				                        ИмяРеквизитаВидРасчета,ИмяРеквизитаДатаНачала,ИмяРеквизитаДатаОкончания.
//
Процедура ПериодНачисленияОбработкаПериодаПриИзмененииНачисления(Форма, ДанныеСтроки, ОписаниеДокумента, ОписаниеТаблицы) Экспорт
	...
	Если ОписаниеДокумента.Окончание <> Неопределено Тогда
		ДатаОкончания = Форма.Объект[ОписаниеДокумента.Окончание];
	КонецЕсли;
	
	ДанныеСтроки.ИспользуетсяПериод = ПериодНачисленияВидРасчетаИспользуетПериод(ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаВидРасчета]);
	
	Если Не ЗначениеЗаполнено(ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаДатаНачала])
		И Не ЗначениеЗаполнено(ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаДатаОкончания]) Тогда
		ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаДатаНачала] = ДатаНачала;
		ДанныеСтроки[ОписаниеТаблицы.ИмяРеквизитаДатаОкончания] = ДатаОкончания;
	КонецЕсли;
	
КонецПроцедуры

// Функция определяет использование периода конкретным видом расчета.
Функция ПериодНачисленияВидРасчетаИспользуетПериод(ВидРасчета) Экспорт
	
	ИспользуетсяПериод = Истина;
	...
	Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Начисления") Тогда
		Если ИнфоОВидеРасчета.ИспользованиеПериода = ПредопределенноеЗначение("Перечисление.ВариантыИспользованияПериодаНачисления.НеИспользовать") Тогда
			ИспользуетсяПериод = Ложь;
		Иначе
			ИспользуетсяПериод = Истина;
		КонецЕсли;   
	ИначеЕсли ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Удержания") Тогда
		Если ИнфоОВидеРасчета.ТребуетсяРасчетБазы Тогда
			ИспользуетсяПериод = Истина;
		Иначе	
			ИспользуетсяПериод = Ложь;
		КонецЕсли;
	...
	Если ДанныеТекущейСтрокиНачисления <> Неопределено Тогда
		...
		Если ДанныеТекущейСтрокиНачисления.Действие = ПредопределенноеЗначение("Перечисление.ДействияСНачислениямиИУдержаниями.Отменить") Тогда
			...
			Для НомерПоказателя = 1 По КоличествоПоказателей Цикл
				...
				Если ДанныеТекущейСтрокиНачисления.Свойство("ТекущееЗначение" + НомерПоказателя) Тогда
