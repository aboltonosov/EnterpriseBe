...
// Возвращает правила заполнения полей в первичных движениях указанного документа по регистру "Себестоимость товаров".
//
// Параметры:
//	ИмяДокумента - Строка - имя документа как оно указано в метаданных
//	ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - требуется получить правила для конкретной хоз. операции документа
//
// Возвращаемое значение:
//	Соответствие - хоз. операция и правила заполнения записей по этой хоз. операции
//
Функция ПравилаЗаполненияПоляТипЗаписиДокумента(ИмяДокумента, ХозяйственнаяОперация = Неопределено) Экспорт
	
	Правила = УниверсальныеМеханизмыПартийИСебестоимостиПовтИсп.ПравилаЗаполненияПоляТипЗаписи();
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ПустоеЗначениеРегистратора", Документы[ИмяДокумента].ПустаяСсылка());
	Если ХозяйственнаяОперация <> Неопределено Тогда
		ПараметрыОтбора.Вставить("ХозяйственнаяОперация", ХозяйственнаяОперация);
	КонецЕсли;
		
	ПравилаДляДокумента = Новый Соответствие;
	
	Для Каждого ТекущаяСтрока Из Правила.НайтиСтроки(ПараметрыОтбора) Цикл
		
		Если ТекущаяСтрока.ПоложительноеКоличество = Неопределено Тогда
			
			ТекущееПравило = Новый Структура("КорПартияВРасходе, ТипЗаписиПриход, ТипЗаписиРасход");
			
		Иначе
			
			ТекущееПравило = ПравилаДляДокумента[ТекущаяСтрока.ХозяйственнаяОперация];
			Если ТекущееПравило = Неопределено Тогда
				ТекущееПравило = Новый Структура(
					"КорПартияВРасходе, ТипЗаписиПриходПлюс, ТипЗаписиРасходПлюс, ТипЗаписиПриходМинус, ТипЗаписиРасходМинус");
			КонецЕсли;
			
			Если ТекущаяСтрока.ПоложительноеКоличество Тогда
				ТекущееПравило.ТипЗаписиПриходПлюс  = ТекущаяСтрока.ТипЗаписиПриход;
				ТекущееПравило.ТипЗаписиРасходПлюс  = ТекущаяСтрока.ТипЗаписиРасход;
			Иначе
				ТекущееПравило.ТипЗаписиПриходМинус = ТекущаяСтрока.ТипЗаписиПриход;
				ТекущееПравило.ТипЗаписиРасходМинус = ТекущаяСтрока.ТипЗаписиРасход;
			КонецЕсли;
			
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ТекущееПравило, ТекущаяСтрока);
		ТекущееПравило.Вставить("БезЗнакаКоличества", (ТекущаяСтрока.ПоложительноеКоличество = Неопределено));
		
		ПравилаДляДокумента.Вставить(ТекущаяСтрока.ХозяйственнаяОперация, ТекущееПравило);		
		
	КонецЦикла;
	
	Возврат ПравилаДляДокумента;
