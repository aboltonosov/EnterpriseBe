...
Процедура ПроверитьОграничениеИспользованияДокумента(Источник, Отказ, РежимЗаписи, РежимПроведения) Экспорт
	...
	ДокументОграничен = Ложь;
	
	Если Не Источник.ЭтоНовый() И Не Отказ Тогда
		ДокументОграничен = ЕстьОграниченныеДокументы(Источник.Ссылка, Истина);
	КонецЕсли;
	
	Отказ = ДокументОграничен;
