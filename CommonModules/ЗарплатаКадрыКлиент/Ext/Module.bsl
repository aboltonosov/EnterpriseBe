...
// Возвращает требует ли динамический список обновления на форме списка журнала
//
// Параметры:
//		Форма - Форма журнала документов с динамическим списком
//
// Возвращаемое значение:
//		Булево - Истина, если форма требует обновления динамического списка, иначе Ложь
//
Функция ОбновитьДинамическийСписок(Форма, ИмяСобытия, Параметр, Источник) Экспорт
	
	Если ИмяСобытия = "ОбновитьДинамическийСписокЖурнала"
		Или (ИмяСобытия = "ЗаписьДокумента" И Форма.ТипыОбъектовОповещения.НайтиПоЗначению(ТипЗнч(Параметр)) <> Неопределено) Тогда
		Если Форма.ИспользоватьКритерийОтбора Тогда
			Возврат Истина;
		Иначе
			Форма.ОбновитьОтображениеДанных();
