...
Функция ДанныеДляНачисленияРезерва(ОтчетнаяДата, Организации, МенеджерВременныхТаблиц, ПолучатьСостояниеРезерва = Ложь, ПолучатьСостояниеРезерваНачалоГода = Ложь)
	...
	|			И ЗаказКлиента <> НЕОПРЕДЕЛЕНО) КАК Задолженность
	|ГДЕ
	|	Задолженность.Валюта = &ВалютаРеглУчета
	|	И Задолженность.ДолгРеглОстаток > 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	Иначе
		...
		|	ВЫБОР КОГДА ОстаткиРезервов.Субконто2 = НЕОПРЕДЕЛЕНО ТОГДА
		|		ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
		|	ИНАЧЕ
		|		ОстаткиРезервов.Субконто2
