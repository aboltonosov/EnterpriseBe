...
Функция СоздатьТаблицаКадровыхДанных() Экспорт
	...
	ТаблицаКадровыхДанных.Колонки.Добавить("ГрафикРаботы", Новый ОписаниеТипов("СправочникСсылка.ГрафикиРаботыСотрудников"));
...
Процедура СоздатьВТЗначенияПоказателейСтажаПоПериодам(МенеджерВременныхТаблиц, ДатаОкончания = '00010101')
	...
	// 4D:ERP для Беларуси, Яна, 21.08.2017 16:19:54 
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТОтборНачислений.Сотрудник,
	|	ДОБАВИТЬКДАТЕ(ВТОтборНачислений.Период, МЕСЯЦ, -1) КАК Период,
	|	КОНЕЦПЕРИОДА(ВТОтборНачислений.Период, МЕСЯЦ) КАК ДатаОкончания
...
// На основании ТИзмененияИсточниковВторичныхДанныхПоДатам (содержащей даты изменений источников данных, описание изменившихся свойств)
//  а также таблиц содержащих интервалы дейтсвия ресурсов регистров ПлановыеНачисления,ЗначенияПериодическихПоказателейРасчетаЗарплатыСотрудников,
//  формирует выборку, где на каждую дату изменения источников вторичных данных плановых начислений, получены все дейтсвующие плаеновыне начисления
//  значения периодических и стажевых показаителей, используемыых в плановых начислениях, а так же описание того, какие данные изменились на эту дату
//  
...
Функция ПараметрыЗаписиНаборовПоСотрудникам(ДанныеПлановыхНачислений, АктуальныеДанныеФОТ, АктуальныСводныеДанные, СотрудникиСУдаляемымиДанными, ЗаписыватьБезусловно = Ложь)	
	...
	Для Каждого СтрокаТаблицы Из ДанныеПлановыхНачислений Цикл
		...
		Если СтрокаТаблицы.НеобходимПерерасчетФОТ Тогда 
...
Функция СоздатьТаблицаДанныхГрафиков()
	...
	ТаблицаДанныхГрафиков.Колонки.Добавить("ГрафикРаботы", Новый ОписаниеТипов("СправочникСсылка.ГрафикиРаботыСотрудников"));
