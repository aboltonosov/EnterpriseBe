...
Функция РазмерГосударственногоПособия(ВидПособия, Период) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 07.06.2017 10:41:57 
	// Документ "Единовременные пособия за счет ФСЗН", №14887 
	// {
	|	РазмерПособия.ВСвязиСоСмертью,
	|	РазмерПособия.ПриРожденииВторогоИПоследующихДетей
	// }
	// 4D
	|ИЗ
...
Процедура ЗаполнитьСведенияОПособияхПоУходуЗаРебенком(Ссылка, ПособиеПлатитУчастникПилотногоПроекта, ДанныеДляПроведения, ИмяТабличнойЧасти = "Пособия", ИмяТабличнойЧастиПерерасчет = Неопределено, СписокФизическихЛиц = Неопределено) Экспорт
	...
	|	ВТУсловияОплатыПоСтраховымСлучаям.ФинансированиеФедеральнымБюджетом,
	|	ВЫБОР
...
Функция ДанныеОНачисленияхДляРасчетаСреднегоЗаработкаПослеПереносаЗатрат(НачисленияСотрудников, ТаблицаПереносы) Экспорт
	...
	ОтборТекущиеДанныеОНачислениях = Новый Структура("Сотрудник,Регистратор,СтатьяФинансирования");
	// отбор для поиска строк в таблице ВремТаблицаДобавить
	ОтборСтрокДобавить = Новый Структура("Сотрудник,Регистратор,СтатьяФинансирования");
	...
	ВремТаблицаСписать.Свернуть("Сотрудник,Регистратор,СтатьяФинансирования","Сумма");
	ВремТаблицаДобавить.Свернуть("Сотрудник,Регистратор,СтатьяФинансирования,НоваяСтатьяФинансирования","Сумма");
...
// {
Процедура СоздатьВТДатыИзмененияЗаписейРасчетаПособияДоТрехЛет(МенеджерВременныхТаблиц, Начало, Окончание, ИменаВТ) Экспорт
	...
	|ПОМЕСТИТЬ ВТДатыИзмененияНачисленийПособияДоТрехЛет
	|ИЗ
	...
	|			И (Начисления.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДоТрехЛет))";
	...
	ИменаВТ.Добавить("ВТДатыИзмененияНачисленийПособияДоТрехЛет");
...
Функция ОстатокОплачиваемыхДнейУходаЗаБольнымРодственником(Родственник, НачалоПериодаОплаты, СлучайУходаЗаБольнымРебенком, Ссылка) Экспорт
	...
	МаксимальноеКоличествоДнейУходаЗаБольнымРодственникомВГод = МаксимальноеКоличествоДнейУходаЗаБольнымРодственникомВГод(СлучайУходаЗаБольнымРебенком);
	
	Если Не МаксимальноеКоличествоДнейУходаЗаБольнымРодственникомВГод = Неопределено Тогда
		КоличествоИспользованныхДнейУходаЗаРодственником = ОплаченоДнейУходаЗаБольнымРодственником(Родственник, НачалоГода(НачалоПериодаОплаты), НачалоПериодаОплаты, СлучайУходаЗаБольнымРебенком, Ссылка);
		Остаток = МаксимальноеКоличествоДнейУходаЗаБольнымРодственникомВГод - КоличествоИспользованныхДнейУходаЗаРодственником;
	КонецЕсли;		
...
Функция ОплаченоДнейУходаЗаБольнымРодственником(Родственник, НачалоПериода, ОкончаниеПериода, СлучайУходаЗаБольнымРебенком, ИсключаемыйРегистратор = Неопределено)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПериодыУходаЗаБольнымиРодственникамиОбороты.ОплаченоДнейОборот КАК ОплаченоДней
	|ПОМЕСТИТЬ ВТИспользованоДней
	|ИЗ
	|	РегистрНакопления.ПериодыУходаЗаБольнымиРодственниками.Обороты(&НачалоПериода, &ОкончаниеПериода, Период, Родственник = &Родственник И СлучайУходаЗаБольнымРебенком В (&СлучаиУходаЗаБольнымРебенком)) КАК ПериодыУходаЗаБольнымиРодственникамиОбороты";
	...
	Если ЗначениеЗаполнено(ИсключаемыйРегистратор) Тогда
		...
		Запрос.Текст = Запрос.Текст + 
		"ВЫБРАТЬ
		|	СУММА(ПериодыУходаЗаБольнымиРодственниками.ОплаченоДней) * -1 КАК ОплаченоДней
		|ИЗ
		|	РегистрНакопления.ПериодыУходаЗаБольнымиРодственниками КАК ПериодыУходаЗаБольнымиРодственниками
		|ГДЕ
		|	ПериодыУходаЗаБольнымиРодственниками.Регистратор = &ИсключаемыйРегистратор
		...
		|	И ПериодыУходаЗаБольнымиРодственниками.Период МЕЖДУ &НачалоПериода И &ОкончаниеПериода";
	...
	иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И СлучайУходаЗаБольнымРебенком В (&СлучаиУходаЗаБольнымРебенком)", "");
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	СУММА(ВТИспользованоДней.ОплаченоДней) КАК ОплаченоДней
	|ИЗ
	|	ВТИспользованоДней КАК ВТИспользованоДней";
	
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		Выборка = Результат.Выбрать();
		Если Выборка.Следующий() 
			И ЗначениеЗаполнено(Выборка.ОплаченоДней) Тогда
			ОплаченоДней = Выборка.ОплаченоДней;
		КонецЕсли;	
	КонецЕсли;
	
	Возврат ОплаченоДней;	
	
КонецФункции

Функция МаксимальноеКоличествоДнейУходаЗаБольнымРодственникомВГод(СлучайУходаЗаБольнымРебенком = Неопределено)
	...
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуЗаРебенкомИнвалидомАмбулаторно 
		...
		КоличествоДней = 120;		
	ИначеЕсли СлучайУходаЗаБольнымРебенком = Перечисления.СлучаиУходаЗаБольнымиДетьми.ПоУходуДо15тиЛетПриПоствакцинальномОсложненииАмбулаторно 
...
// 4D:ERP для Беларуси, Яна, 31.08.2017 14:36:00 
...
//			ОтработаноДнейКалендарных
//
Функция ДанныеДляРасчетаСреднегоЗаработкаФСССотрудникаЛокализация(Сотрудник, ДополнительныеПараметры) Экспорт
	...
	ДанныеДляРасчета = ДанныеДляРасчетаСреднегоЗаработкаФСССотрудниковЛокализация(ИсходныеДанные, ДополнительныеПараметры);
...
//	Аппроксимировать - необязательный, по умолчанию Ложь, 
//		если Истина, отсутствующие в учете значения заполняются приближенными, "угаданными".
//
...
//
Функция ДанныеДляРасчетаСреднегоЗаработкаФСССотрудниковЛокализация(ИсходныеДанные, ДополнительныеПараметры) Экспорт
		
	ДокументСсылка = ДополнительныеПараметры.ДокументСсылка;
	ОтборМесяцев = Неопределено; 
	Аппроксимировать = Ложь;
	...
	ДанныеДляРасчета = Новый Структура("ДанныеОНачислениях, ДанныеОВремени, ДанныеСтрахователей");
	...
	Запрос.УстановитьПараметр("Аппроксимировать", Аппроксимировать);
	
	// Получаем данные о начислениях
	// исключаем данные по текущему регистратору.
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ДанныеУчетаНачислений.Период,
	|	ДанныеУчетаНачислений.ФизическоеЛицо,
	|	ДанныеУчетаНачислений.Сотрудник,
	|	ДанныеУчетаНачислений.ПорядокРасчета,
	|	ДанныеУчетаНачислений.СтатьяФинансирования,
	|	ДанныеУчетаНачислений.СоставнаяЧасть,
	|	СУММА(ДанныеУчетаНачислений.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ВТДанныеУчетаНачисленийСреднегоЗаработкаФСС
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДанныеОНачисленияхОбороты.Период КАК Период,
	|		ДанныеОНачисленияхОбороты.ФизическоеЛицо КАК ФизическоеЛицо,
	|		ДанныеОНачисленияхОбороты.Сотрудник КАК Сотрудник,
	|		ДанныеОНачисленияхОбороты.ПорядокРасчета КАК ПорядокРасчета,
	|		ДанныеОНачисленияхОбороты.СтатьяФинансирования КАК СтатьяФинансирования,
	|		ДанныеОНачисленияхОбороты.СоставнаяЧасть КАК СоставнаяЧасть,
	|		ДанныеОНачисленияхОбороты.СуммаОборот КАК Сумма,
	|		ЛОЖЬ КАК ПереносДанных
	|	ИЗ
	|		РегистрНакопления.ДанныеОНачисленияхДляРасчетаСреднегоЗаработкаФСС.Обороты(
	|				&ДатаНачала,
	|				&ДатаОкончания,
	|				Месяц,
	|				(Сотрудник, ГоловнаяОрганизация, ПорядокРасчета) В
	|					(ВЫБРАТЬ
	|						ВТИсходныеДанные.Сотрудник,
	|						ВТИсходныеДанные.ГоловнаяОрганизация,
	|						ВТИсходныеДанные.ПорядокРасчета
	|					ИЗ
	|						ВТИсходныеДанные)) КАК ДанныеОНачисленияхОбороты
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		НАЧАЛОПЕРИОДА(ДанныеОНачисленияхРегистратора.Период, МЕСЯЦ),
	|		ДанныеОНачисленияхРегистратора.ФизическоеЛицо,
	|		ДанныеОНачисленияхРегистратора.Сотрудник,
	|		ДанныеОНачисленияхРегистратора.ПорядокРасчета,
	|		ДанныеОНачисленияхРегистратора.СтатьяФинансирования,
	|		ДанныеОНачисленияхРегистратора.СоставнаяЧасть,
	|		-ДанныеОНачисленияхРегистратора.Сумма,
	|		ЛОЖЬ
	|	ИЗ
	|		РегистрНакопления.ДанныеОНачисленияхДляРасчетаСреднегоЗаработкаФСС КАК ДанныеОНачисленияхРегистратора
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК ИсходныеДанные
	|			ПО ДанныеОНачисленияхРегистратора.ФизическоеЛицо = ИсходныеДанные.ФизическоеЛицо
	|				И ДанныеОНачисленияхРегистратора.Сотрудник = ИсходныеДанные.Сотрудник
	|				И ДанныеОНачисленияхРегистратора.ГоловнаяОрганизация = ИсходныеДанные.ГоловнаяОрганизация
	|				И ДанныеОНачисленияхРегистратора.ПорядокРасчета = ИсходныеДанные.ПорядокРасчета
	|				И (НАЧАЛОПЕРИОДА(ДанныеОНачисленияхРегистратора.Период, МЕСЯЦ) = ИсходныеДанные.Период)
	|				И (ДанныеОНачисленияхРегистратора.Регистратор = &Регистратор)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				И ДанныеОНачисленияхРегистратора.ПорядокРасчета = ИсходныеДанные.ПорядокРасчета
	|				И (НАЧАЛОПЕРИОДА(ДанныеОНачисленияхРегистратора.Период, МЕСЯЦ) = ИсходныеДанные.Период)
	|				И (ДанныеОНачисленияхРегистратора.Регистратор ССЫЛКА Документ.ПереносДанных)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				И ДанныеОНачисленияхРегистратора.ПорядокРасчета = ИсходныеДанные.ПорядокРасчета
	|				И (НАЧАЛОПЕРИОДА(ДанныеОНачисленияхРегистратора.Период, МЕСЯЦ) = ИсходныеДанные.Период)
	|				И (ДанныеОНачисленияхРегистратора.Регистратор ССЫЛКА Документ.ПереносДанных)) КАК ДанныеУчетаНачислений
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеУчетаНачислений.Период,
	|	ДанныеУчетаНачислений.ФизическоеЛицо,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ДанныеУчетаНачислений.Сумма) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК ИсходныеДанные
	|		ПО СведенияНачислений.Сотрудник = ИсходныеДанные.ФизическоеЛицо
	...
	|	СотрудникиМесяцы.Период КАК ДатаНачалаПериода,
	|	КОНЕЦПЕРИОДА(СотрудникиМесяцы.Период, МЕСЯЦ) КАК ДатаОкончанияПериода
	|ИЗ
	|	ВТИсходныеДанные КАК СотрудникиМесяцы";
	...
	|		ДанныеВремени.ОтработаноДнейКалендарныхОборот КАК ОтработаноДнейКалендарных
	|	ИЗ
	|		РегистрНакопления.ДанныеОВремениДляРасчетаСреднегоФСЗН.Обороты(
	|				&ДатаНачала,
	|				&ДатаОкончания,
	|				Месяц,
	...
	|						ВТИсходныеДанные.ПорядокРасчета
	|					ИЗ
	|						ВТИсходныеДанные)) КАК ДанныеВремени
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		-ДанныеРегистратора.ОтработаноДнейКалендарных
	|	ИЗ
	|		РегистрНакопления.ДанныеОВремениДляРасчетаСреднегоФСЗН КАК ДанныеРегистратора
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК ПериодыРасчетаСреднегоСотрудников
	...
	|				И (ДанныеРегистратора.Регистратор = &Регистратор)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК СотрудникиМесяцы
	|		ПО (СотрудникиМесяцы.Сотрудник = ДанныеВремени.Сотрудник)
	|			И (СотрудникиМесяцы.Период = ДанныеВремени.Период)
	...
	|	ИСТИНА
	|ИЗ
	|	РегистрСведений.СведенияОВремениДляРасчетаСреднегоФСЗН КАК СведенияВремени
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК СотрудникиМесяцы
	|		ПО (СотрудникиМесяцы.Сотрудник = СведенияВремени.Сотрудник)
	|			И (СотрудникиМесяцы.Период = СведенияВремени.Месяц)
	...
	|	ИЗ
	|		РегистрСведений.ДанныеОВремениДляРасчетаСреднегоФСЗНКорректировка КАК ДанныеВремениКорректировка
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТИсходныеДанные КАК СотрудникиМесяцы
	...
	|				И (СотрудникиМесяцы.Период = ДанныеВремениКорректировка.Месяц)
	...
	|	СУММА(ДанныеВремени.НормаДней) КАК НормаДнейПолная,
	|	СУММА(ДанныеВремени.НормаЧасов) КАК НормаЧасовПолная
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДанныеВремени.Сотрудник КАК Сотрудник,
	|		ДанныеВремени.ПорядокРасчета КАК ПорядокРасчета,
	...
	|		СотрудникиМесяцы.Период,
	|		0,
	|		0,
	|		0,
	|		КалендарноеВремя.НормаДней,
	|		КалендарноеВремя.НормаЧасов,
	|		КалендарноеВремя.НормаДнейПолная,
	|		КалендарноеВремя.НормаЧасовПолная
	|	ИЗ
	|		ВТИсходныеДанные КАК СотрудникиМесяцы
	...
	|				И (НАЧАЛОПЕРИОДА(КалендарноеВремя.ДатаНачалаПериода, МЕСЯЦ) = СотрудникиМесяцы.Период)) КАК ДанныеВремени
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеВремени.Сотрудник,
	|	ДанныеВремени.ПорядокРасчета,
...
Процедура РассчитатьПособиеДоПолутораЛет(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.РасчетПоПравилам2010Года Тогда
			...
			РазмерОграничения = Окр(РазмерОграниченияДля2010года() / 12, 2);
		...
		Если КоэффициентЛьготы > 1 Тогда
			...
			Иначе
				...
				Если КоличествоДетейВсего > 1 Тогда
					ДоляПервогоРебенка = 
					Макс(Окр(РазмерПособияИзСреднегоЗаработка * 40 / 100, 2), МинимумПособияПоУходуЗаПервымРебенкомДоПолутораЛет * Макс(РайонныйКоэффициентРФОрганизации, 1)) /
					Макс(Окр(РазмерПособияИзСреднегоЗаработка * Мин(40 * КоличествоДетейВсего, 100) / 100, 2), (МинимумПособияПоУходуЗаПервымРебенкомДоПолутораЛет + МинимумПособияПоУходуЗаПоследующимРебенкомДоПолутораЛет * (КоличествоДетейВсего - 1)) * Макс(РайонныйКоэффициентРФОрганизации, 1));
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если КоличествоПервыхДетей = 1 Тогда
			...
			ОграничениеПособияСнизу = МинимумПособияПоУходуЗаПервымРебенкомДоПолутораЛет * Макс(РайонныйКоэффициентРФОрганизации, 1) * КоэффициентЛьготы;
			ОграниченныйРазмер = Макс(РазмерПособия, ОграничениеПособияСнизу);
			ОграниченныйРазмерНаПервогоРебенка = ОграниченныйРазмер;
			Если КоличествоДетейВсего > 1 Тогда
				...
				ОграничениеПособияСнизу = МинимумПособияПоУходуЗаПоследующимРебенкомДоПолутораЛет * (КоличествоДетейВсего - 1) * Макс(РайонныйКоэффициентРФОрганизации, 1) * КоэффициентЛьготы;
				...
				ОграниченныйРазмер = ОграниченныйРазмер + Макс(РазмерПособия, ОграничениеПособияСнизу);
				// Из-за суммирования отдельно рассчитанных пособий минимальный и максимальный размер пособия приходится проверять повторно.
				ОграниченныйРазмер = Макс(Мин(ОграниченныйРазмер, РазмерПособияИзСреднегоЗаработка), МинимальныйРазмерПособия);
				Если КоэффициентЛьготы > 1 Тогда
		...
		Иначе
			...
			// Пособие за месяц с учетом ограничений.
			ОграниченныйРазмер = Макс(Мин(РазмерПособия, РазмерПособияИзСреднегоЗаработка), ОграничениеПособияСнизу);
		КонецЕсли;
		...
		ИначеЕсли КоэффициентФедеральногоБюджета = 0.5 Тогда // пособие за счет федерального бюджета выплачивается частично
			ОграниченныйРазмер = Макс(Окр(РазмерПособияИзСреднегоЗаработка * Мин(40 * КоличествоДетейВсего, 100) / 100, 2), (МинимумПособияПоУходуЗаПервымРебенкомДоПолутораЛет * КоличествоПервыхДетей + МинимумПособияПоУходуЗаПоследующимРебенкомДоПолутораЛет * (КоличествоДетейВсего - КоличествоПервыхДетей)) * Макс(РайонныйКоэффициентРФОрганизации, 1));
			Если ОплачиваемыеДни < КалендарныхДнейВМесяце Тогда
...
Процедура ЗарегистрироватьДанныеСреднегоЗаработкаФСС(Движения, Отказ, МенеджерВременныхТаблиц, ЗаписыватьДвижения = Ложь, ИмяВТНачисления = "ВТНачисления") Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	&ИспользоватьСтатьиФинансирования = ИСТИНА
		|
		|СГРУППИРОВАТЬ ПО
		|	Начисления.Сотрудник,
	...
	ТаблицаДвижений.Свернуть("Период, ФизическоеЛицо, ГоловнаяОрганизация, ПорядокРасчета, Сотрудник, СтатьяФинансирования", "Сумма");
...
// 4D:ERP для Беларуси, Яна, 09.06.2017 15:10:15 
// Документ "Отпуск по уходу за ребенком", №14900 
// {
// Процедура выполняет запись сведений о среднем заработке ФСС
//  на основе данных, введенных пользователем.
...
//	КорректировкиВремени	- таблица значений, структуры аналогичной набору записей 
//								регистра сведений СведенияОВремениДляРасчетаСреднегоФСС.
//	ПереноситьДанныеВДругойУчетСреднегоЗаработка - признак необходимости преобразования и записи сведения для расчета
//	                                               общего среднего заработка.
//	ПереноситьВДругойПорядокРасчета - признак необходимости переноса данных для других правил расчета среднего заработка.
//
Процедура ЗаписатьКорректировкиСреднегоЗаработкаФССОтпускПоУходуЗаРебенком(Организация, Сотрудник, КорректировкиНачислений, НачалоПериода, ОкончаниеПериода, КорректировкиВремени = Неопределено, ПереноситьДанныеВДругойУчетСреднегоЗаработка = Ложь, ПереноситьВДругойПорядокРасчета = Истина) Экспорт
	...
	Если ПереноситьДанныеВДругойУчетСреднегоЗаработка Тогда
		УчетСреднегоЗаработка.ПеренестиКорректировкиВСреднийЗаработокОбщий(КорректировкиНачислений, КорректировкиВремени, Организация, НачалоПериода, ОкончаниеПериода);	
	КонецЕсли;
...
Функция ТаблицыДанныхСреднегоЗаработкаФСС(ИмяДокумента, МассивСсылок) Экспорт
	...
	ТекстЗапроса = 
		...
		// 4D:ERP для Беларуси, Петр, 01.03.2018 16:42:51 
		// Доработать печатные формы док-та "Больничный лист", № 17952
		// {
		|	СреднийЗаработокФСС.СоставнаяЧасть,
		// }
		// 4D
		...
		// 4D:ERP для Беларуси, Юлия, 25.09.2017 9:18:06 
		// Ошибка в документе "Больничный лист", № 16086
		// {
		|	ОтработанноеВремяДляСреднегоФСС.ОтработаноДней,
		|	ОтработанноеВремяДляСреднегоФСС.НормаДней,
		|	ОтработанноеВремяДляСреднегоФСС.ОтработаноЧасов,
		|	ОтработанноеВремяДляСреднегоФСС.НормаЧасов
		// }
		// 4D
...
Процедура ОбновитьДанныеУчетаСреднегоЗаработкаФСС(Сотрудник, НачалоПериода, ОкончаниеПериода, ДополнительныеПараметры) Экспорт
	...
	ПеренестиРанееОтсутствующиеДанные(ДанныеДляРасчета.ДанныеОНачислениях, ДанныеОНачислениях);
	ПеренестиРанееОтсутствующиеДанные(ДанныеДляРасчета.ДанныеОВремени, ДанныеОВремени);
	
КонецПроцедуры

Процедура ПеренестиРанееОтсутствующиеДанные(КоллекцияИсточник, КоллекцияПриемник)
	
	// Если на какой-то месяц сведения уже введены, то пропускаем, если нет, то переносим все строки за этот месяц.
	Месяцы = ОбщегоНазначения.ВыгрузитьКолонку(КоллекцияИсточник, "Период", Истина);
	
	// Из массива месяцев удаляем месяцы, которые есть в коллекции приемника.
	МесяцыПриемника = ОбщегоНазначения.ВыгрузитьКолонку(КоллекцияПриемник, "Период", Истина);
	Месяцы = ОбщегоНазначенияКлиентСервер.СократитьМассив(Месяцы, МесяцыПриемника);
	
	Для Каждого СтрокаКоллекции Из КоллекцияИсточник Цикл
		Если Месяцы.Найти(СтрокаКоллекции.Период) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(КоллекцияПриемник.Добавить(), СтрокаКоллекции);
	КонецЦикла;
...
// 4D:ERP для Беларуси, Яна, 30.08.2017 17:46:44 
// Ошибка при вводе данных для расчета среднего заработка, №15832 
// {
// Выполняет упаковку данных среднего заработка ФСС формы документа для передачи в форму редактирования.
//
Процедура ЗаполнитьТаблицыДанныхСреднегоЗаработкаФССПоДокументуЛокализация(ДанныеФормыОбъект, ПараметрыРедактирования) Экспорт
	...
	//ДанныеПредельныеВеличины  = ПустаяТаблицаПредельныеВеличины();
	ПредельныеВеличины = ПредельнаяВеличинаБазыСтраховыхВзносовЛокализация(ДанныеФормыОбъект.ПериодРасчетаСреднегоЗаработкаНачало,ДанныеФормыОбъект.ПериодРасчетаСреднегоЗаработкаОкончание);
	//ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ПредельныеВеличины, ДанныеПредельныеВеличины);
	ПараметрыРедактирования.ПредельныеВеличины = ПоместитьВоВременноеХранилище(ПредельныеВеличины);

	// - отработанное время
	ОтработанноеВремя = ПустаяТаблицаОтработанноеВремяСреднийЗаработокФССЛокализация();
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеФормыОбъект.ОтработанноеВремяДляСреднегоФСС, ОтработанноеВремя);
	ПараметрыРедактирования.ДанныеОВремени = ПоместитьВоВременноеХранилище(ОтработанноеВремя);

	Если ДанныеФормыОбъект.УчитыватьЗаработокПредыдущихСтрахователей Тогда
		// - данные страхователей
		ДанныеСтрахователей = ПустаяТаблицаДанныеСтрахователейСреднийЗаработокФСС();
		ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ДанныеФормыОбъект.СреднийЗаработокДанныеСтрахователей, ДанныеСтрахователей);
		ПараметрыРедактирования.ДанныеСтрахователей = ПоместитьВоВременноеХранилище(ДанныеСтрахователей);
	КонецЕсли;
...
//
Функция ПредельнаяВеличинаБазыСтраховыхВзносовЛокализация(ПериодНачало,ПериодОкончание) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	
	СоздатьВТПредельнаяВеличинаБазы(Запрос.МенеджерВременныхТаблиц, ПериодНачало,ПериодОкончание);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПредельнаяВеличинаБазыСтраховыхВзносов.Месяц КАК Период,
	...
	Пока Выборка.Следующий() Цикл
		ПредельнаяВеличина.Вставить(Выборка.Период, Выборка.Размер);
	КонецЦикла;
...
//		Годы - массив элементов типа число.
// 
Процедура СоздатьВТПредельнаяВеличинаБазы(МенеджерВременныхТаблиц,ПериодНачало,ПериодОкончание) Экспорт

	ИзмеренияДаты = Новый ТаблицаЗначений;
	ИзмеренияДаты.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	
	Период = НачалоМесяца(ПериодНачало-1);	
	Пока Период <= НачалоМесяца(ПериодОкончание) Цикл
		ИзмеренияДаты.Добавить().Период = Период;	
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПредельнаяВеличинаБазыСтраховыхВзносов.Период КАК Месяц,
	|	ЕСТЬNULL(ПредельнаяВеличинаБазыСтраховыхВзносов.РазмерФСЗН, 0) КАК Размер
...
//
Функция ПустаяТаблицаОтработанноеВремяСреднийЗаработокФССЛокализация() Экспорт
	...
	ТаблицаДанных.Колонки.Добавить("ОтработаноДней", Новый ОписаниеТипов("Число"), Новый КвалификаторыЧисла(7, 0));
	ТаблицаДанных.Колонки.Добавить("ОтработаноЧасов", Новый ОписаниеТипов("Число"), Новый КвалификаторыЧисла(7, 0));
	ТаблицаДанных.Колонки.Добавить("НормаДней", Новый ОписаниеТипов("Число"), Новый КвалификаторыЧисла(7, 0));
	ТаблицаДанных.Колонки.Добавить("НормаЧасов", Новый ОписаниеТипов("Число"), Новый КвалификаторыЧисла(7, 0)); 	
...
Функция ПустаяТаблицаДанныеСтрахователейСреднийЗаработокФСС() Экспорт
	...
	ТаблицаДанных.Колонки.Добавить("Страхователь", Новый ОписаниеТипов("СправочникСсылка.Работодатели,СправочникСсылка.Организации"));
...
Процедура СоздатьВТИсходныеДанные(МенеджерВременныхТаблиц, ИсходныеДанные, ОтборМесяцев = Неопределено)
	...
	ОписательВременныхТаблиц = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеСотрудников(
		МенеджерВременныхТаблиц,
		"ВТТаблицаСотрудники");
	// 4D:ERP для Беларуси, Яна, 31.08.2017 16:35:30 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Истина, "Сотрудник, ФизическоеЛицо, ГоловнаяОрганизация, ДатаПриема");
	// }
	// 4D
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Сотрудники.ФизическоеЛицо,
	// 4D:ERP для Беларуси, Яна, 31.08.2017 16:43:18 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
...
Функция ПустаяТаблицаНачисленийСреднийЗаработокФСС() Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 31.08.2017 10:26:03 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	ТаблицаДанных.Колонки.Добавить("СоставнаяЧасть", Новый ОписаниеТипов("ПеречислениеСсылка.УчетНачисленийВСреднемЗаработкеФСС"));
	// }
	// 4D
...
Функция ПустаяТаблицаОтработанноеВремяСреднийЗаработокФСС() Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 31.08.2017 12:34:08 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	ТаблицаДанных = Новый ТаблицаЗначений;
	ТаблицаДанных.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
...
Функция ПустаяТаблицаИсходныхДанныхРасчетаСреднегоЗаработкаФСС()
	...
	// 4D:ERP для Беларуси, Яна, 31.08.2017 16:12:14 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	ИсходныеДанные.Колонки.Добавить("СоставнаяЧасть", Новый ОписаниеТипов("ПеречислениеСсылка.УчетНачисленийВСреднемЗаработкеФСС"));
	// }
	// 4D
...
// 4D:ERP для Беларуси, Яна, 06.07.2017 14:35:21 
// Ошибка Поле не найдено МесяцНачисления КАК Период, №15329 
// {
Процедура ЗарегистрироватьДанныеСреднегоЗаработкаФСС_Локализация(Движения, Отказ, МенеджерВременныхТаблиц, ЗаписыватьДвижения = Ложь, ИмяВТНачисления = "ВТНачисления") Экспорт  
	
	// Регистрация сумм начислений для среднего заработка ФСЗН. 	
	...
	|	НастройкиНачислений.Значение
	|ПОМЕСТИТЬ ВТНастройкиНачислений
	|ИЗ
	|	ПланВидовРасчета.Начисления.СреднийЗаработокФСС КАК НастройкиНачислений
	|ГДЕ
	|	НастройкиНачислений.Значение <> ЗНАЧЕНИЕ(Перечисление.УчетНачисленийВСреднемЗаработкеФСС.НеВключать)
	|	И НастройкиНачислений.ПорядокРасчета = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетаСреднегоЗаработкаФСС.Локализация)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(Начисления.ДатаДействия, МЕСЯЦ) КАК Период,
	|	ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетаСреднегоЗаработкаФСС.Локализация) КАК ПорядокРасчета,
	|	Начисления.ФизическоеЛицо,
	|	Начисления.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
	|	Начисления.Сотрудник,
	|	Начисления.СтатьяФинансирования,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(Начисления.ДатаДействия, МЕСЯЦ),
	|	Начисления.Сотрудник,
	|	Начисления.Организация.ГоловнаяОрганизация,
	|	Начисления.СтатьяФинансирования,
	|	Начисления.ФизическоеЛицо,
	...
	// Регистрация отработанного времени для среднего заработка	
	
	Запрос = Новый Запрос;
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("Регистратор", Движения.ДанныеОВремениДляРасчетаСреднегоФСЗН.Отбор.Регистратор.Значение);
	Запрос.УстановитьПараметр("КатегорииНеявокДляБольничного", ПланыВидовРасчета.Начисления.КатегорииНеявокДляБольничных());
	Запрос.УстановитьПараметр("КатегорииОтпуска", ПланыВидовРасчета.Начисления.КатегорииОтпуска());
	...
	|	ПланВидовРасчета.Начисления КАК Начисления
	|ГДЕ
	|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииНеявокДляБольничного)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПланВидовРасчета.Начисления КАК Начисления
	|ГДЕ
	|	Начисления.КатегорияНачисленияИлиНеоплаченногоВремени В(&КатегорииОтпуска)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Начисления.Ссылка
	|;
	...
	|		СУММА(ВЫБОР
	|				КОГДА ФПД.Сторно = ИСТИНА
	|					ТОГДА -(ДанныеКалендаряПоФПД.ДнейКалендарных - ДанныеКалендаряПоФПД.ДнейКалендарныхБезУчетаПраздников)
	|				ИНАЧЕ ДанныеКалендаряПоФПД.ДнейКалендарных - ДанныеКалендаряПоФПД.ДнейКалендарныхБезУчетаПраздников
	|			КОНЕЦ)
	|	ИЗ
	|		ВТФПДСотрудников КАК ФПД
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДанныеПроизводственногоКалендаряПоГрафикам КАК ДанныеКалендаряПоФПД
	...
	|	(СУММА(ДанныеВремени.ОтработаноДней) <> 0
	|		ИЛИ СУММА(ДанныеВремени.ОтработаноЧасов) <> 0
	|		ИЛИ СУММА(ДанныеВремени.ДнейКалендарных) <> 0)";
	
	
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		Движения.ДанныеОВремениДляРасчетаСреднегоФСЗН.Записывать = Истина;
	КонецЕсли;
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		ЗаполнитьЗначенияСвойств(Движения.ДанныеОВремениДляРасчетаСреднегоФСЗН.Добавить(), Выборка);
	КонецЦикла;
	
	Если ЗаписыватьДвижения Тогда
		Движения.ДанныеОВремениДляРасчетаСреднегоФСЗН.Записать();
		Движения.ДанныеОВремениДляРасчетаСреднегоФСЗН.Записывать = Ложь;
	КонецЕсли;
