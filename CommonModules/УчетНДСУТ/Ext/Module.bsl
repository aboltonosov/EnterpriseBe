...
Функция СтранаЯвляетсяЧленомЕАЭС(Страна, Период = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 18.10.2017 12:35:38 
	// Добавим РФ
	// {
	Если Период >= '2015-01-01' 
			и (КодСтраны = "643" или КодСтраны = "398") Тогда // Казахстан, Россия
		Возврат Истина;
	КонецЕсли;
	// }
	// 4D
...
Процедура ПолучитьПартии(ПараметрыРасчета)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:50:11 
	// Ставки НДС 20% и 25%
	// {
	|					КОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20) ТОГДА Партии.НДСРегл * 100 / 20
	|					КОГДА ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25) ТОГДА Партии.НДСРегл * 100 / 25
	// }
	// 4D
	|					ИНАЧЕ 0 КОНЕЦ)
...
Процедура РассчитатьНДСЗаписиКнигиПродажПоНДСПредъявленный(ПараметрыРасчета)
	...
	// 4D:ERP для Беларуси, Дмитрий, 12.09.2017 23:49:02 
	// Ставки НДС 20% и 25%
	// {
	...
	|		КОГДА НДСПредъявленный.СуммаБезНДС = 0 И НДСПредъявленный.СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС25)
	|			ТОГДА НДСПредъявленный.НДС * 100 / 25
	// }
	// 4D
