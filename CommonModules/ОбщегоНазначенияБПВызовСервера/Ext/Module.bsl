...
Функция ПолучитьСоответствиеСубконтоПараметрамУчета(ТолькоМПЗ = Ложь) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 30.06.2015 17:28:09 
	...
	СтрокаСчета = Счета.Добавить();
	СтрокаСчета.Счет = ПланыСчетов.Хозрасчетный.ОборудованиеКУстановкеПереданноеВМонтаж;
	СтрокаСчета.СПодчиненными = Истина;
	...
	СтрокаСчета = Счета.Добавить();
	// 4D:ERP для Беларуси, Екатерина, 01.07.2015 16:05:14 
	// Локализация плана счетов, №8969
	// {
	СтрокаСчета.Счет = ПланыСчетов.Хозрасчетный.КассаФилиала;
	// }
	// 4D
	СтрокаСчета.СПодчиненными = Ложь;
	...
	СтрокаСчета = Счета.Добавить();
	// 4D:ERP для Беларуси, Екатерина, 01.07.2015 17:32:39 
	// Локализация плана счетов, №8969
	// {
	СтрокаСчета.Счет = ПланыСчетов.Хозрасчетный.СчетаВДрагоценныхМеталлах;
	// }
	// 4D
	СтрокаСчета.СПодчиненными = Ложь;
