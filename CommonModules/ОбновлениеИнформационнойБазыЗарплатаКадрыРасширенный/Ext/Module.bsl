...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru='Заполнение даты окончания в регистре ""Периоды стажа ПФР"" по договорма ГПХ'");
...
Процедура ЗаменитьСсылкиНаБанки() Экспорт
	...
	Если Метаданные.Справочники.Найти("УдалитьБанки") <> Неопределено Тогда
		...
		Если Не ОбщегоНазначенияПовтИсп.РазделениеВключено()
			И ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.КлассификаторБанковРФ) Тогда
			...
			Если Не РезультатЗапроса.Пустой() Тогда
				...
				Пока Выборка.Следующий() Цикл
					...
					КлассификаторБанковРФОбъект.ОбменДанными.Загрузка = Истина;
					КлассификаторБанковРФОбъект.Записать();
		...
		Запрос.Текст =
			...
			|	РегистрСведений.ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам КАК ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам
			|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлассификаторБанковРФ КАК КлассификаторБанковРФ
			|		ПО (ВЫРАЗИТЬ(ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.УдалитьБанк КАК Справочник.УдалитьБанки).Код = КлассификаторБанковРФ.Код)
			|ГДЕ
			|	ЛицевыеСчетаСотрудниковПоЗарплатнымПроектам.УдалитьБанк <> ЗНАЧЕНИЕ(Справочник.УдалитьБанки.ПустаяСсылка)
...
Процедура ЗаполнитьЦелевоеВремяКлючевыхОпераций() Экспорт
	...
	#Область ОтчетОтчетыПоСотрудникамТ2
	КлючеваяОперация.Вставить("ИмяКлючевойОперации", "Отчет.ОтчетыПоСотрудникам.Т2.Формирование");
	КлючеваяОперация.Вставить("ЦелевоеВремя", 5);
	КлючевыеОперации.Добавить(КлючеваяОперация);
	КлючеваяОперация = Новый Структура;
	#КонецОбласти
