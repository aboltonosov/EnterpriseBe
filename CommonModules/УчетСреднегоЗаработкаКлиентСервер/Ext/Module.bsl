...
Функция ДоляОтработанногоВремени(ОтработанноеВремя) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 18.07.2017 15:24:57 
	// Ошибка документа "Простой сотрудников", №15213 
	// {
	Норма 		= ИтогиПоПолю(ОтработанноеВремя, "Норма");
	Отработано 	= ИтогиПоПолю(ОтработанноеВремя, "Отработано");
	// }
	// 4D
...
Функция ПериодРасчетаОбщегоСреднегоЗаработка(ДатаНачалаСобытия, ВидРасчета = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВидРасчета) Тогда	
		...
		// 4D:ERP для Беларуси, Петр, 21.12.2017 10:22:15 
		// Внести изменения в документ "Оплата дней ухода за детьми-инвалидами", № 17078
		// {
		Если  ИнфоОВидеРасчета.КоличествоМесяцевРасчетаСреднегоЗаработка > 0 Тогда
...
Функция ПараметрыРедактированияОбщегоСреднегоЗаработкаПоДокументу() Экспорт
	// 4D:ERP для Беларуси, Яна, 01.09.2017 14:17:56 
	...
	ПараметрыРедактирования.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОтпуск.Локализация");
...
// 4D:ERP для Беларуси, Яна, 26.06.2017 17:45:37 
// Ошибка документа "Отпуск", №15211 
// {
Функция РассчитатьСреднийЗаработокОтпуск(Начисления, Время, Индексации, НачалоПериода, ОкончаниеПериода, ПоЧасам, ОписаниеРасчетаДляОтпусков = Неопределено) Экспорт
				
	ДанныеДляРасчета = ДанныеДляРасчетаСреднегоЗаработкаОтпуска(Начисления, Время, Индексации, НачалоПериода, ОкончаниеПериода, ПоЧасам, ОписаниеРасчетаДляОтпусков); 
...
Функция ДанныеДляРасчетаСреднегоЗаработкаОтпуска(Начисления, Время, Индексации, НачалоПериода, ОкончаниеПериода, ПоЧасам, ОписаниеРасчетаДляОтпусков = Неопределено, ПоСтатьямФинансирования = Ложь) Экспорт
		
	Заработок 			= Новый Массив;
	ОтработанноеВремя 	= Новый Массив;
	
	// 4D:ERP для Беларуси, Юлия, 07.09.2017 11:21:29 
	// Ошибка при вводе данных для расчета среднего заработка, № 15832
	// {	
	...
	СреднемесячноеКоличествоДней		= КоличествоДнейВМесяцеДляОплатыОтпуска();
			
	КоличествоМесяцевПериодаРасчета = УчетСреднегоЗаработкаКлиентСервер.КоличествоМесяцевПериодаРасчетаСреднегоОтпуска(НачалоПериода, ОкончаниеПериода);
	...
	Для Каждого ПервичнаяСтрокаВремени Из Время Цикл
		...
		Если ПервичнаяСтрокаВремени.НеУчитывать Тогда
			Продолжить;	
		КонецЕсли;	
		
		Отбор = ОтборЭлементаВремени(ПервичнаяСтрокаВремени.Период);
		
		СтрокаОтработанноеВремя 							= ЭлементВремениПоОтборуОтпуск(ОтработанноеВремя, Отбор);
		
		СтрокаОтработанноеВремя.КалендарныхДней 			= СреднемесячноеКоличествоДней;
		ПервичнаяСтрокаВремени.ОтработаноДнейКалендарных    = СреднемесячноеКоличествоДней;
		...
		СтрокаОтработанноеВремя.НормаДней					= ПервичнаяСтрокаВремени.НормаДней;
		СтрокаОтработанноеВремя.НормаЧасов 					= ПервичнаяСтрокаВремени.НормаЧасов;
	...
	Для Каждого СтрокаНачислений Из Начисления Цикл
		...
		Если СтрокаНачислений.НеУчитывать Тогда
			Продолжить;	
		КонецЕсли;
				
		СтатьяФинансирования = ?(ПоСтатьямФинансирования, СтрокаНачислений.СтатьяФинансирования, Неопределено);
		
		Если Не ЭтоПремияГодоваяОтпуск(СтрокаНачислений.СоставнаяЧасть)  Тогда
			// не является премией
			...
			Отбор = ОтборЭлементаЗаработкаОтпуск(СтрокаНачислений.Период, СтрокаНачислений.СоставнаяЧасть, КоэффициентИндексации, Неопределено, Неопределено, СтатьяФинансирования);
		...
		КоличествоМесяцев = 1;
		Если ЭтоПремияГодоваяОтпуск(СтрокаНачислений.СоставнаяЧасть) Тогда
		...
		Отбор = ОтборЭлементаЗаработкаОтпуск(СтрокаНачислений.Период, СтрокаНачислений.СоставнаяЧасть, КоэффициентИндексации, КоличествоМесяцев,  СтатьяФинансирования);
	
		СтрокаПремии 				= ЭлементЗаработкаПоОтбору(Заработок, Отбор);
		СтрокаПремии.Сумма 			= СтрокаПремии.Сумма 			+ СтрокаНачислений.Сумма;
		СтрокаПремии.Учтено 		= СтрокаПремии.Учтено 			+ Окр(СтрокаНачислений.Сумма * КоэффициентПремии * КоэффициентИндексации, 2);
...
Функция ОтборЭлементаЗаработкаОтпуск(Месяц, СоставнаяЧасть, КоэффициентИндексации = Неопределено, КоличествоМесяцев = Неопределено, КоэффициентНормирования = Неопределено, СтатьяФинансирования = Неопределено)
...
Функция ЭлементВремениПоОтборуОтпуск(Коллекция, Отбор)
	
	ПодходящийЭлемент = ЭлементКоллекцииПоОтбору(Коллекция, Отбор);
	
	Если ПодходящийЭлемент = Неопределено Тогда
		ПодходящийЭлемент = СтруктураВремениДляРасчетаСреднегоЗаработкаОтпуска();
...
Функция ЭтоПремияГодоваяОтпуск(СоставнаяЧастьЗаработка) Экспорт
	Возврат СоставнаяЧастьЗаработка = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОтпуск.ПремияГодовая")
КонецФункции

Функция СтруктураВремениДляРасчетаСреднегоЗаработкаОтпуска()
	...
	ТипСоставнаяЧастьЗаработка = Новый ОписаниеТипов("ПеречислениеСсылка.УчетНачисленийВСреднемЗаработкеОтпуск");
	...
	ОтработанноеВремя.Вставить("НормаДней", 				ПустоеЧисло);
	ОтработанноеВремя.Вставить("НормаЧасов", 				ПустоеЧисло);
...
Функция КоличествоДнейВМесяцеДляОплатыОтпуска() Экспорт 
...
//
Функция КоличествоМесяцевПериодаРасчетаСреднегоОтпуска(ДатаНачалаПериода, ДатаОкончанияПериода) Экспорт
...
Процедура РассчитатьУчитываемоеВремя(Время, УчитываемоеВремя, ДополнительныеПараметры )
	...
	Для Каждого ПервичнаяСтрокаВремени Из Время Цикл
		...
		// 4D:ERP для Беларуси, Яна, 18.07.2017 15:13:37 
		// Ошибка документа "Простой сотрудников", №15213
		// 	
		СтрокаОтработанноеВремя.НормаДней 					= ПервичнаяСтрокаВремени.НормаДней;
		СтрокаОтработанноеВремя.НормаЧасов 					= ПервичнаяСтрокаВремени.НормаЧасов;
		...
		Если ДополнительныеПараметры.ПоЧасам Тогда
			СтрокаОтработанноеВремя.Отработано 				= ПервичнаяСтрокаВремени.ОтработаноЧасов;
			СтрокаОтработанноеВремя.Норма 					= ПервичнаяСтрокаВремени.НормаЧасов;
		Иначе 	
			СтрокаОтработанноеВремя.Отработано 				= ПервичнаяСтрокаВремени.ОтработаноДней;
			СтрокаОтработанноеВремя.Норма 					= ПервичнаяСтрокаВремени.НормаДней;
		...
		ИначеЕсли СпособРасчетаОтпуска = ПредопределенноеЗначение("Перечисление.СпособыРасчетаНачислений.ОплатаОтпускаПоКалендарнымДням") Тогда
			// 4D:ERP для Беларуси, Яна, 27.06.2017 9:26:57 
			// Ошибка документа "Отпуск", №15211 
			// {
			СреднемесячноеКоличествоДней 		= КоличествоДнейВМесяцеДляОплатыОтпуска();
			// }
		...
		ИначеЕсли СпособРасчетаОтпуска = ПредопределенноеЗначение("Перечисление.СпособыРасчетаНачислений.ОплатаОтпускаПоШестидневке") Тогда
			// 4D:ERP для Беларуси, Яна, 24.07.2017 10:56:16 
			// Ошибка конфигурации "Поле объекта не обнаружено (норма дней)" , № 15529 
			// {
			СтрокаОтработанноеВремя.Учтено = СтрокаОтработанноеВремя.ОтработаноДней;
			// }
			// 4D
...
Функция СтруктураВремениДляРасчетаСреднегоЗаработка()
	...
	// 4D:ERP для Беларуси, Яна, 18.07.2017 15:07:30 
	// Ошибка документа "Простой сотрудников", № 15213 
	// {
	ОтработанноеВремя.Вставить("НормаДней", 				ПустоеЧисло);
	ОтработанноеВремя.Вставить("НормаЧасов", 				ПустоеЧисло);
	
	ОтработанноеВремя.Вставить("Отработано",				ПустоеЧисло);
	ОтработанноеВремя.Вставить("Норма", 					ПустоеЧисло);
	// }
	// 4D
...
Функция КоличествоДнейВМесяцеДляОплатыОтпусков() Экспорт 

	// 4D:ERP для Беларуси, Яна, 01.09.2017 15:14:23 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
...
Функция ОписаниеСтрокиДанныхОНачисленияхОбщегоСреднегоЗаработка() Экспорт
	...
	// 4D:ERP для Беларуси, , 31.08.2017 9:42:07 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {	
