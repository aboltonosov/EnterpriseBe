...
Процедура ЗаполнитьОтчетКосвенныеНалогиТамСоюзФормаОтчета2010Кв3(ПараметрыОтчета, Контейнер,СохранятьТаблицуРасшифровок = Истина) Экспорт
	...
	Если Контейнер.Свойство("Раздел1") Тогда   
		...
		ТекстЗапросаЗаявленияОВвозеТоваров = УчетНДСПереопределяемый.ТекстЗапросаЗаявленияОВвозеТоваров();
		
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЗаявленияОВвозе.ЗаявлениеОВвозе,
		|	ЗаявленияОВвозе.НалоговыйПериод
		|ПОМЕСТИТЬ ЗаявленияПоНалоговымПериодам
		|ИЗ
		|	(" + ТекстЗапросаЗаявленияОВвозеТоваров + ") КАК ЗаявленияОВвозе 
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗаявленияОВвозе.НалоговыйПериод
		|;
		...
		ТекстЗапросаЗаявленияОВвозеТоваров = УчетНДСПереопределяемый.ТекстЗапросаЗаявленияОВвозеТоваров();
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ЗаявленияОВводеТоваров", "(" + ТекстЗапросаЗаявленияОВвозеТоваров + ")");
...
//	См. НовыйСтруктураДокументовОтчетности().
//
...
// Возвращаемое значение:
//	Структура - Содержит ключи:
//		* ИННФЛ, Фамилия, Имя, Отчество - Для физического лица.
//		* ИННЮЛ, КПП, НаимОрг - Для юридического лица.
//
...
//				или РегистрНакопленияНаборЗаписей.НДСЗаписиКнигиПродаж.
//
