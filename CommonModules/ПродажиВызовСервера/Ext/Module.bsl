...
Функция СформироватьДанныеЗаполненияРеализации(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ЗаказПереработчику) Тогда
		...
		ТекстЗаказы = ТекстЗаказы + "
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ЗаказыКлиентов.Остатки(, ЗаказКлиента В (&МассивСсылок)) КАК ЗаказыКлиентов
			|		ПО ЗаказыКлиентов.ЗаказКлиента = ЗаказПереработчику.Ссылка
			|ГДЕ
	...
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("ИмяДокументаПД", НСтр("ru = 'передачи давальцу'"));
	//-- НЕ УТКА
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ИмяДокументаПСП", НСтр("ru = 'передачи сырья переработчику'"));
	//-- НЕ УТ
