...
Процедура УстановитьПараметрыЗапросаПоПараметрамУказанияСерий(Запрос,ПараметрыУказанияСерий)
	...
	КонецЕсли;
	Если ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ПриемкаИзПроизводства) <> Неопределено Тогда
		Запрос.УстановитьПараметр("ПриемкаИзПроизводства", Истина);
	Иначе
		Запрос.УстановитьПараметр("ПриемкаИзПроизводства", Ложь);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ЕстьВидНоменклатурыТребующийОбязательностиОрдернойСхемыПоИзлишкамНедостачам() Экспорт
	...
	|	И ВидыНоменклатуры.ВариантНастройкиСерийДляСкладов = ЗНАЧЕНИЕ(Перечисление.ВариантыНастроекСерий.ЕдинаяНастройка)
...
Функция ДанныеДляЗаполненияСерийПоFEFOВЭтапе(Объект, ПараметрыУказанияСерий, Запрос)
	...
	|				И ЕСТЬNULL(ЭтапПроизводства2_2.НазначениеМатериалы.ДвиженияПоСкладскимРегистрам, ЛОЖЬ)
	|			ТОГДА ЭтапПроизводства2_2.НазначениеМатериалы
	...
	|		ПО (ЭтапПроизводства2_2.Ссылка = &Ссылка)
	|ГДЕ
	...
	|		И (ТоварыНаСкладах.Склад, ТоварыНаСкладах.Номенклатура, ТоварыНаСкладах.Характеристика, ТоварыНаСкладах.Назначение) В
	|				(ВЫБРАТЬ
	|					ТаблицаТоваровДляЗапроса.Склад,
	|					ТаблицаТоваровДляЗапроса.Номенклатура,
	|					ТаблицаТоваровДляЗапроса.Характеристика,
