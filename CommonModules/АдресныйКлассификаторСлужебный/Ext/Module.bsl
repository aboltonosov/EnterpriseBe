...
Процедура УстановитьИдентификаторыНаселенногоПункта(ЧастиАдреса) Экспорт
	...
	// {
	Если ЧастиАдреса.Свойство("Округ") Тогда
...
Функция ИндексИОКТМОАдресаПоЧастямАдреса(Адрес, Идентификатор = Неопределено) Экспорт
	...
	Если ПустаяСтрока(Источник) Тогда
		...
		Если Идентификатор <> Неопределено Тогда
			...
			// {
			|	ВЫБОР
			|		КОГДА АдресныеОбъекты.КодСубъектаРФ = 0
			|			ТОГДА АдресныеОбъекты.КодКЛАДР
			|		ИНАЧЕ ДополнительныеАдресныеСведения.ОКТМО
			|	КОНЕЦ КАК ОКТМО,
			// }
			// 4D
...
Процедура ЗагрузитьКлассификаторАдресов(КодыСубъектовРФ, ОписаниеФайлов, ОповещатьОПрогрессе = Истина) Экспорт
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	ИндексБеларусь = КодыСубъектовРФ.Найти(0);
	Если ИндексБеларусь <> Неопределено Тогда
		КодыСубъектовРФ.Удалить(0);
		...
		Для каждого ЗначИмяФайла Из СписокДополнительныхФайлов Цикл
			ИмяНовогоФайла = КаталогФайлов + ЗначИмяФайла.Значение;
			
			ДополнительныйФайл = Новый Файл(ИмяНовогоФайла);
			Если Не ДополнительныйФайл.Существует() Тогда
				ЕстьДополнительныеФайлы = Ложь;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		...
		Если ЕстьДополнительныеФайлы Тогда
			Если ОповещатьОПрогрессе Тогда
				...
				ДлительныеОперации.СообщитьПрогресс(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загрузка региона ""%1 - %2"" (осталось %3) ...'"),
					0, АдресныйКлассификатор.НаименованиеРегионаПоКоду(0), Формат(ВсегоСубъектов - НомерПоПорядку, "ЧН=")));
			КонецЕсли;
			
			Попытка
				Набор = РегистрыСведений.АдресныеОбъекты.СоздатьНаборЗаписей();
				Набор.Отбор.КодСубъектаРФ.Установить(0);
				Набор.Записать();
				
				РегистрыСведений.АдресныеОбъекты.ОбновитьСоставСубъектовРФПоКлассификатору(Истина);
				
				АдресныйКлассификаторСлужебный_Локализация.ЗагрузитьАдресныйКлассификаторРБ(КаталогФайлов);
				
				Для каждого Стр Из ОписаниеФайлов Цикл
					Если Стр.Свойство("КодСубъекта") И Стр.Свойство("Версия") И Стр.Свойство("ДатаОбновления") Тогда
						Если Стр.КодСубъекта = 0 Тогда
							УстановитьИнформациюОЗагрузкеВерсии(0, Стр.Версия, Стр.ДатаОбновления, ТекущаяДата());
							Прервать;
						КонецЕсли;
					Иначе
						УстановитьИнформациюОЗагрузкеВерсии(0, "Локальная", '00010101', ТекущаяДата());
						Прервать;
					КонецЕсли;
				КонецЦикла;
			Исключение
				КодыРегионов = Новый Массив;
				КодыРегионов.Добавить(0);
				ОчиститьКлассификаторАдресов(КодыРегионов);
				ВызватьИсключение;
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если КодыСубъектовРФ.Количество() > 0 Тогда
...
Функция ИдентификаторыАдресаПоКлассификатору(ПоляАдреса, ИскатьПоУлице, ВсеИдентификаторыАдреса = Ложь)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	Если ПоляАдреса.Получить("Округ") = Неопределено Или ПустаяСтрока(ПоляАдреса["Округ"].Наименование) Тогда
...
Процедура ЗагрузитьКлассификаторАдресовССайта(КодыСубъектовРФ = Неопределено, ИсходнаяАвторизация = Неопределено)
	...
	// Адресный и банковский классификаторы, №10377 (2)
	// {
	АдресныйКлассификаторСлужебный_Локализация.ДобавитьДоступныеВерсииАдресныхСведений(ВсеВерсии);
	// }
	// 4D
	Для Каждого КодСубъекта Из КодыСубъектовРФ Цикл
		...
		// Адресный и банковский классификаторы, №10377 (2)
		// {
		ОписаниеФайлов.Добавить(Новый Структура("Имя, Хранение, КодСубъекта, Версия, ДатаОбновления", ПутьДляСохранения, ПутьДляСохранения, КодСубъекта, ОписаниеРегиона.Версия, ОписаниеРегиона.ДатаОбновления));
