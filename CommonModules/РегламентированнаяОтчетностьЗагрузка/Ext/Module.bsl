...
Функция ЗагрузкаОтчетаИзФайла(ФормаОтчета, ПараметрыОтчета, ДанныеДляЗагрузки, ДеревоДляЗагрузки) Экспорт
	...
	РезультатДлительнойОперации = ВыполнитьВФоне(ИмяПроцедуры, ПараметрыВыполнения, ПараметрыЗапуска);
...
Функция ВыполнитьВФоне(ИмяПроцедуры, ПараметрыПроцедуры, ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.Свойство("ЗапуститьНеВФоне") И ПараметрыВыполнения.ЗапуститьНеВФоне = Истина Тогда
		Попытка
			...
			Для Каждого СтрокаУровня1 Из СоставДопСтрок.Значение.Строки Цикл
				...
				Если СтрокаУровня1.Строки.Количество() > 0 Тогда
					...
					Для Каждого СтрокаУровня2 Из СтрокаУровня1.Строки Цикл
						...
						ИмяПоказателя01 = "П00010" + СтрЗаменить(ИмяОбластиДопСтроки, "ДопСтрока", "") + "01";
						ИмяПоказателя02 = "П00010" + СтрЗаменить(ИмяОбластиДопСтроки, "ДопСтрока", "") + "02";
						ЗначениеПоказателя01 = Неопределено;
						РазделОтчета.Свойство(ИмяПоказателя01, ЗначениеПоказателя01);
						ЗначениеПоказателя02 = Неопределено;
						РазделОтчета.Свойство(ИмяПоказателя02, ЗначениеПоказателя02);
						Если ЗначениеЗаполнено(ЗначениеПоказателя01) ИЛИ ЗначениеЗаполнено(ЗначениеПоказателя02) Тогда
							СтрокаУровня2.ВключатьВОтчет = Истина;
						Иначе
							ВсеВключены = Ложь;
						КонецЕсли;
					КонецЦикла;
					СтрокаУровня1.ВключатьВОтчет = ВсеВключены;
				Иначе
					ИмяПоказателя = "П100100" + СтрокаУровня1.Код + "02" + "_1";
					ЗначениеПоказателя = Неопределено;
					...
					Для НомСтроки = 1 По 3 Цикл
						...
						ИмяПоказателя = "П00010" + СтрокаУровня1.Код + Формат(НомСтроки, "ЧГ=") + "02";
						ЗначениеПоказателя = Неопределено;
						Если РазделОтчета.Свойство(ИмяПоказателя, ЗначениеПоказателя) И ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
