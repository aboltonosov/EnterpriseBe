...
Функция НачислитьНалогНаПрибыль(Движения, Знач Реквизиты, Знач ПараметрРаспределениеНалогаНаПрибыль) Экспорт
	...
	СтавкаФедеральныйБюджет  = НалоговыйУчет.НалоговаяСтавкаФедеральныйБюджет(Реквизиты.Период);
	...
	// Локализация плана счетов, №8969
	// {
	СчетДт = ?(ПрименяетсяПБУ18, ПланыСчетов.Хозрасчетный.НалогНаПрибыль, ПланыСчетов.Хозрасчетный.ПрибылиИУбыткиНеЕНВД);
	// }
	// 4D
	...
	Для Каждого ДанныеПроводки Из НачислениеНалога Цикл
		...
		// Локализация плана счетов, №8969
		// {
		Проводка.СчетКт = ПланыСчетов.Хозрасчетный.НалогНаПрибыль;
		// }
		// 4D
...
Процедура ДобавитьПроводкуСписанияОтклоненийПриОкругленииНалогаНаПрибыль(
	...
	// Локализация плана счетов, №8969
	// {
	СчетРасчетНалогаНаПрибыль = ПланыСчетов.Хозрасчетный.НалогНаПрибыль;
	// }
	// 4D
	...
	Если ОстатокОкругления > 0 Тогда
		...
		// Локализация плана счетов, №8969
		// {
		Проводка.СчетДт      = ПланыСчетов.Хозрасчетный.НалогНаПрибыль;  // 68.04.2
		// }
		// 4D
	...
	Иначе
		...
		// Локализация плана счетов, №8969
		// {
		Проводка.СчетКт      = ПланыСчетов.Хозрасчетный.НалогНаПрибыль;   // 68.04.2
		// }
		// 4D
...
Функция НачислениеНалогаНаПрибыль(Период, Организация, НалоговаяБаза, СтавкаФедеральныйБюджет, СтавкаРегиональныйБюджет, РаспределениеНалогаНаПрибыль, ПрименяетсяПБУ18 = Истина)
	...
	Если Месяц(Период) > 1 Тогда
		...
		Если ЗначениеЗаполнено(РаспределениеНалогаНаПрибыль) Тогда
			...
			// Локализация плана счетов, №8969
			// {
			|			Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты),
			// }
			// 4D
			|			&ВидыСубконто,
			|			Организация = &Организация
			...
			// Локализация плана счетов, №8969
			// {
			|			КорСчет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НалогНаПрибыль),
			// }
			// 4D
			|			) КАК ХозрасчетныйОбороты
			|
...
Функция ОборотыВременныхРазницПоВидуАктивовОбязательствСформироватьПакет(СтрокаВидАктиваОбязательства, ТекИндекс = 0, СоответствиеСтрокДляСвертки = "")
	...
	Если СтрокаВидАктиваОбязательства.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.РасходыБудущихПериодов Тогда
		ТекстЗапросаШаблон = СтрЗаменить(ТекстЗапросаШаблон, "ХозрасчетныйОстаткиИОбороты.СуммаВРОборотДт",
			"ВЫБОР КОГДА ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРБП = ЗНАЧЕНИЕ(Перечисление.ВидыРБП.УбыткиПрошлыхЛет) ТОГДА 0 ИНАЧЕ ХозрасчетныйОстаткиИОбороты.СуммаВРОборотДт КОНЕЦ");
		ТекстЗапросаШаблон = СтрЗаменить(ТекстЗапросаШаблон, "ХозрасчетныйОстаткиИОбороты.СуммаВРКонечныйОстатокДт",
			"ХозрасчетныйОстаткиИОбороты.СуммаВРКонечныйОстатокДт - ВЫБОР КОГДА ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРБП = ЗНАЧЕНИЕ(Перечисление.ВидыРБП.УбыткиПрошлыхЛет) ТОГДА ХозрасчетныйОстаткиИОбороты.СуммаВРОборотДт ИНАЧЕ 0 КОНЕЦ");
	КонецЕсли;
...
Функция ОборотыПостоянныхРазницПоВидуАктивовОбязательствСформироватьПакет(СтрокаВидАктиваОбязательства, ТекИндекс = 0, СоответствиеСтрокДляСвертки = "")
	...
	Если СтрокаВидАктиваОбязательства.ВидАктивовОбязательств = Перечисления.ВидыАктивовИОбязательств.РасходыБудущихПериодов Тогда
		ТекстЗапросаШаблон = СтрЗаменить(ТекстЗапросаШаблон, "ХозрасчетныйОстаткиИОбороты.СуммаВРОборотДт",
			"ВЫБОР КОГДА ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРБП = ЗНАЧЕНИЕ(Перечисление.ВидыРБП.УбыткиПрошлыхЛет) ТОГДА 0 ИНАЧЕ ХозрасчетныйОстаткиИОбороты.СуммаПРОборотДт КОНЕЦ");
		ТекстЗапросаШаблон = СтрЗаменить(ТекстЗапросаШаблон, "ХозрасчетныйОстаткиИОбороты.СуммаПРКонечныйОстатокДт",
			"ХозрасчетныйОстаткиИОбороты.СуммаПРКонечныйОстатокДт - ВЫБОР КОГДА ВЫРАЗИТЬ(Субконто1 КАК ПланВидовХарактеристик.СтатьиРасходов).ВидРБП = ЗНАЧЕНИЕ(Перечисление.ВидыРБП.УбыткиПрошлыхЛет) ТОГДА ХозрасчетныйОстаткиИОбороты.СуммаПРОборотДт ИНАЧЕ 0 КОНЕЦ");
		КонецЕсли;
...
Процедура СформироватьДвиженияРасчетНалогаНаПрибыль(ТаблицаРасчетНалога, ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для каждого Запись Из Параметры.ТаблицаРасчетНалога Цикл
		...
		Если Запись.Субконто68_4 <> Неопределено Тогда
			...
			// Локализация плана счетов, №8969
			// {
			Счет68_04   = ПланыСчетов.Хозрасчетный.ВнутрихозяйственныеРасчеты;
			// }
			// 4D
		...
		Иначе
			...
			// Локализация плана счетов, №8969
			// {
			Счет68_04   = ПланыСчетов.Хозрасчетный.НалогНаПрибыль;
			// }
			// 4D
