...
Функция КомандыОтчетовФормы(Форма, СписокОбъектов, ИмяМетодаМенеджераОтчеты) Экспорт
...
Функция СоздатьКоллекциюКомандОтчетов() Экспорт
	...
	// менеджер
	Результат.Колонки.Добавить("ТипыОбъектовМенюОтчеты", Новый ОписаниеТипов("Массив"));
		...
		Для Каждого ДобавляемаяКомандаОтчет Из КомандыОтчетовФормы Цикл
			...
			Если НайденныеКоманды.Количество() > 0 Тогда
				Для Каждого ИмеющаясяКомандаОтчет Из НайденныеКоманды Цикл
					...
					Если ИмеющаясяКомандаОтчет.ТипыОбъектовМенюОтчеты.Найти(ТипОбъекта) = Неопределено Тогда
						ИмеющаясяКомандаОтчет.ТипыОбъектовМенюОтчеты.Добавить(ТипОбъекта);
					КонецЕсли;
				КонецЦикла;
				Продолжить;
			КонецЕсли;
			
			Если ДобавляемаяКомандаОтчет.ТипыОбъектовМенюОтчеты.Количество() = 0 Тогда
				ДобавляемаяКомандаОтчет.ТипыОбъектовМенюОтчеты.Добавить(Тип(СтрЗаменить(ОбъектМетаданных.ПолноеИмя(), ".", "Ссылка.")));
			КонецЕсли;
