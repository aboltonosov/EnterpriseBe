...
// 4D:ERP для Беларуси, Екатерина, 14.12.2015 11:40:33 // , №10533
// {
// Процедура-обработчик события "ПриЗаписи" регистра накопления ФактическиеОтпуска.
//
Процедура ПроверитьОстаткиОтпусковПередЗаписью(Источник, Отказ, Замещение) Экспорт
	...
	ТаблицаСотрудниковИОтпусков = Источник.Выгрузить(,"Сотрудник, ВидЕжегодногоОтпуска, Период, Компенсация");
	
	Если ТаблицаСотрудниковИОтпусков.Количество() > 0
		И СуществуютРанееВведенныеОстаткиОтпусков(ТаблицаСотрудниковИОтпусков) Тогда
		
		ТекстОшибки = НСтр("ru = 'Нельзя проводить отпуска ранее уже введенной информации по остаткам отпусков.'");
...
Функция СуществуютРанееВведенныеОстаткиОтпусков(ТаблицаСотрудниковИОтпусков)
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(ТЗ.Компенсация КАК БУЛЕВО) КАК Компенсация,
		|	НАЧАЛОПЕРИОДА(ТЗ.Период, ДЕНЬ) КАК Период
		|ПОМЕСТИТЬ СотрудникиИОтпуска
		|ИЗ
		|	&ТЗ КАК ТЗ
		|ГДЕ
		|	НЕ ТЗ.Компенсация
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СотрудникиИОтпуска КАК СотрудникиИОтпуска
		|		ПО НачальныеОстаткиОтпусков.Сотрудник = СотрудникиИОтпуска.Сотрудник
		|			И НачальныеОстаткиОтпусков.ВидЕжегодногоОтпуска = СотрудникиИОтпуска.ВидЕжегодногоОтпуска
		|			И НачальныеОстаткиОтпусков.ДатаОстатка > СотрудникиИОтпуска.Период";
	
	Запрос.УстановитьПараметр("ТЗ", ТаблицаСотрудниковИОтпусков);
