...
Функция НазначитьОсновныеСпецификацииДляИзделий(НаборДанных, МассивОшибок = Неопределено) Экспорт
	...
	Для каждого Строка Из НаборДанных Цикл
		...
		КонецЕсли;
	КонецЦикла;
	СвойстваСпецификаций = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивСпецификаций, "Статус");
...
Функция ЗаполнитьЭтапыПоРесурснойСпецификации(Приемник, Источник, СсылкаПриемника)
	...
	Пока Выборка.Следующий() Цикл
		...
		НайденныеСтроки = Приемник.МатериалыИУслуги.НайтиСтроки(Новый Структура("ИсточникПолученияПолуфабриката", Выборка.Ссылка));
		
		Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			
			НайденнаяСтрока.ИсточникПолученияПолуфабриката = ЭтапОбъект.Ссылка;
