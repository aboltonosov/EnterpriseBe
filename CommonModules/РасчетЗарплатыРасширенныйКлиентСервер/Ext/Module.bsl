...
Процедура УстановитьПланируемуюДатуВыплаты(Форма, ОписаниеДокумента) Экспорт
	...
	ДлинаСуток = 86400;
	// 4D:ERP для Беларуси, Яна, 22.05.2017 16:51:11 
	// Документ "Отпуск по уходу за ребенком", № 14900 
	// {	
	Если Форма.Объект.ПорядокВыплаты = Форма.ХарактерВыплатыЗарплата Тогда
		Форма.Объект.ПланируемаяДатаВыплаты = ПланируемаяДатаВыплатыЗарплатыПоНастройкам(МесяцНачисления, Настройки);
	ИначеЕсли Форма.Объект.ПорядокВыплаты = Форма.ХарактерВыплатыАванс Тогда
		Форма.Объект.ПланируемаяДатаВыплаты = НачалоМесяца(МесяцНачисления) + (Настройки.ДатаВыплатыАвансаНеПозжеЧем - 1) * ДлинаСуток;
	Иначе
		Форма.Объект.ПланируемаяДатаВыплаты = НачалоМесяца(МесяцНачисления) + (Настройки.ДатаВыплатыМежрасчетаНеПозжеЧем - 1) * ДлинаСуток;
	КонецЕсли;
	// }
	// 4D
...
Процедура УточнитьВидимостьНормыВремениПоВидуРасчета(ВидимостьПолей, Форма, ОписаниеТаблицы) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 17.07.2017 15:32:22 
...
//
Функция УдержаниеВыполняетсяВЦеломЗаМесяц(УдержаниеОбъект, ЕстьПоказательРасчетнаяБаза, ЕстьОперативныеПоказатели, ЕстьПоказательРасчетнаяБазаСтраховыеВзносы) Экспорт
