...
// 4D:ERP для Беларуси, Яна, 17.07.2017 16:51:50 
// Ошибка документа "Простой сотрудников", №15213 
// {
// Обработчик события, возникающего при преобразовании данных формы в объект (например, перед записью формы).
//
Процедура РаспределениеПоТерриториямУсловиямТрудаПриПреобразованииДанныхФормыВОбъект(Форма, ТекущийОбъект) Экспорт
	...
	Для Каждого СтрокаРаспределения Из Форма.Объект.РаспределениеПоТерриториямУсловиямТруда Цикл
		ИдентификаторСтрокиПоказателей = ИдентификаторСтрокиПоказателей + 1;
		СтрокаРаспределения.ИдентификаторСтрокиПоказателей = ИдентификаторСтрокиПоказателей;
		Для НомерПоказателя = 1 По КоличествоПоказателей Цикл
			Показатель = СтрокаРаспределения["Показатель" + НомерПоказателя];
			Если Не ЗначениеЗаполнено(Показатель) Тогда
				Продолжить;
			...
			НоваяСтрока.Показатель = Показатель;
			НоваяСтрока.Значение = СтрокаРаспределения["Значение" + НомерПоказателя];
			НоваяСтрока.ИдентификаторСтрокиВидаРасчета = СтрокаРаспределения.ИдентификаторСтрокиПоказателей;
		КонецЦикла;
	КонецЦикла;
...
// 4D:ERP для Беларуси, Яна, 14.06.2017 12:22:39 
// Документ "Отпуск по уходу за ребенком", № 14900,15145 
// {
#Область ОбслуживаниеВводаПоказателейПособияДоТрехЛетНаВторогоРебенкаВФормеДокументов

Процедура ОбновитьПоляВводаПоказателейПособияДоТрехЛетНаВторогоРебенка(Форма) Экспорт
	
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	// Если вид пособия не заполнен.
	Если Не ЗначениеЗаполнено(Объект.ПособиеДоТрехЛетНаВторогоРебенка) Тогда
		// "Включаем" пустую декорацию, чтобы отображать страницу без элементов.
		Элементы.ГруппаПособиеДоТрехЛетНаВторогоРебенкаРассчитываетсяДекорация.Видимость = Истина;
		Элементы.ГруппаПоказателиПособияДоТрехЛетНаВторогоРебенка.ТекущаяСтраница = Элементы.ГруппаПособиеДоТрехЛетНаВторогоРебенкаРассчитывается;
		Возврат;
	КонецЕсли;
	
	// Для ввода запрашиваемых показателей пособия до трех лет нужно оформить необходимое количество полей.
	ВидРасчетаИнфо = ЗарплатаКадрыРасширенныйПовтИсп.ПолучитьИнформациюОВидеРасчета(Объект.ПособиеДоТрехЛетНаВторогоРебенка);
	
	// Если не рассчитывается, то отображаем заранее подготовленное поле Размер.
	Если Не ВидРасчетаИнфо.Рассчитывается Тогда
		Элементы.ГруппаПоказателиПособияДоТрехЛетНаВторогоРебенка.ТекущаяСтраница = Элементы.ГруппаПособиеДоТрехЛетНаВторогоРебенкаНеРассчитывается;
		Возврат;
	КонецЕсли;
	...
	// Обнуляем фиксированный размер.
	Объект.РазмерПособияДоТрехЛетНаВторогоРебенкаФиксированнойСуммой = 0;
	...
	Для Каждого ОписаниеПоказателя Из ВидРасчетаИнфо.Показатели Цикл
		...
		// Показатель запрашивается - создаем для него поле ввода.
		// Проверяем, возможно для такого показателя поле уже существует.
		Если Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка.НайтиСтроки(Новый Структура("Показатель", ОписаниеПоказателя.Показатель)).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		ИмяРеквизита = "Показатель_" + ЗарплатаКадрыРасширенныйКлиентСервер.УникальноеИмяРеквизита();
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы(
				...
				ОписаниеПоказателя.КраткоеНаименование, 
				Истина));
		НовыйПоказатель = Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка.Добавить();		
		НовыйПоказатель.Показатель = ОписаниеПоказателя.Показатель;
		НовыйПоказатель.ИмяРеквизита = ИмяРеквизита;
	КонецЦикла;
	...
	УдаляемыеРеквизиты = Новый Массив;
	УдаляемыеСтроки = Новый Массив;
	Для Каждого СтрокаПоказателя Из Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка Цикл
		Если ПоказателиПособия[СтрокаПоказателя.Показатель]	= Неопределено Тогда
			УдаляемыеРеквизиты.Добавить(СтрокаПоказателя.ИмяРеквизита);
			УдаляемыеСтроки.Добавить(СтрокаПоказателя);
		КонецЕсли;
	КонецЦикла;
	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка.Удалить(УдаляемаяСтрока);
	КонецЦикла;
	...
	Для Каждого СтрокаПоказателя Из Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка Цикл
		Если Элементы.Найти(СтрокаПоказателя.ИмяРеквизита) <> Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		ПолеВвода = Элементы.Добавить(СтрокаПоказателя.ИмяРеквизита, Тип("ПолеФормы"), Элементы.ГруппаПособиеДоТрехЛетНаВторогоРебенкаРассчитывается);
		ПолеВвода.ПутьКДанным = СтрокаПоказателя.ИмяРеквизита;
		ПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
		ПолеВвода.КнопкаВыбора = Истина;
	КонецЦикла;
	...
	// "Включаем" пустую декорацию, чтобы отображать страницу без элементов.
	Элементы.ГруппаПособиеДоТрехЛетНаВторогоРебенкаРассчитываетсяДекорация.Видимость = Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка.Количество() = 0;
	
	ПрочитатьЗначенияПоказателейПособияДоТрехЛетНаВторогоРебенка(Форма);
	
КонецПроцедуры

Процедура ПрочитатьЗначенияПоказателейПособияДоТрехЛетНаВторогоРебенка(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	// Заполним поля ввода значениями показателей.
	Для Каждого СтрокаТаблицы Из ТекущийОбъект.Показатели Цикл
		НайденныеСтроки = Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка.НайтиСтроки(Новый Структура("Показатель", СтрокаТаблицы.Показатель));
		Если НайденныеСтроки.Количество() > 0 Тогда
			Форма[НайденныеСтроки[0].ИмяРеквизита] = СтрокаТаблицы.Значение;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаписатьЗначенияПоказателейПособияДоТрехЛетНаВторогоРебенка(Форма, ТекущийОбъект) Экспорт
	
	Для Каждого СтрокаПоказателей Из Форма.ПоказателиПособияДоТрехЛетНаВторогоРебенка Цикл
		НоваяСтрока = ТекущийОбъект.Показатели.Добавить();
		НоваяСтрока.Показатель = СтрокаПоказателей.Показатель;
		НоваяСтрока.Значение = Форма[СтрокаПоказателей.ИмяРеквизита];
		НоваяСтрока.ИдентификаторСтрокиВидаРасчета = Документы.ОтпускПоУходуЗаРебенком.ИдентификаторСтрокПоказателейПособияДоТрехЛет();
...
#Область ОбслуживаниеВводаПоказателейПособияДоТрехЛетНаРебенкаИнвалидаВФормеДокументов

Процедура ОбновитьПоляВводаПоказателейПособияДоТрехЛетНаРебенкаИнвалида(Форма) Экспорт 
	
	Объект = Форма.Объект;
	Элементы = Форма.Элементы;
	
	// Если вид пособия не заполнен.
	Если Не ЗначениеЗаполнено(Объект.ПособиеДоТрехЛетНаРебенкаИнвалида) Тогда
		// "Включаем" пустую декорацию, чтобы отображать страницу без элементов.
		Элементы.ГруппаПособиеДоТрехЛетНаРебенкаИнвалидаРассчитываетсяДекорация.Видимость = Истина;
		Элементы.ГруппаПоказателиПособияДоТрехЛетНаРебенкаИнвалида.ТекущаяСтраница = Элементы.ГруппаПособиеДоТрехЛетНаРебенкаИнвалидаРебенкаРассчитывается;
		Возврат;
	КонецЕсли;
	
	// Для ввода запрашиваемых показателей пособия до трех лет на ребенка ивалида нужно оформить необходимое количество полей.
	ВидРасчетаИнфо = ЗарплатаКадрыРасширенныйПовтИсп.ПолучитьИнформациюОВидеРасчета(Объект.ПособиеДоТрехЛетНаРебенкаИнвалида);
	...
	// Обнуляем фиксированный размер.
	Объект.РазмерПособияДоТрехЛетНаРебенкаИнвалидаФиксированнойСуммой = 0;
	...
	Для Каждого ОписаниеПоказателя Из ВидРасчетаИнфо.Показатели Цикл
		...
		// Показатель запрашивается - создаем для него поле ввода.
		// Проверяем, возможно для такого показателя поле уже существует.
		Если Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида.НайтиСтроки(Новый Структура("Показатель", ОписаниеПоказателя.Показатель)).Количество() > 0 Тогда
			Продолжить;
		КонецЕсли;
		ИмяРеквизита = "Показатель_" + ЗарплатаКадрыРасширенныйКлиентСервер.УникальноеИмяРеквизита();
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы(
				...
				ОписаниеПоказателя.КраткоеНаименование, 
				Истина));
		НовыйПоказатель = Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида.Добавить();		
		НовыйПоказатель.Показатель = ОписаниеПоказателя.Показатель;
		НовыйПоказатель.ИмяРеквизита = ИмяРеквизита;
	КонецЦикла;
	...
	УдаляемыеРеквизиты = Новый Массив;
	УдаляемыеСтроки = Новый Массив;
	Для Каждого СтрокаПоказателя Из Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида Цикл
		Если ПоказателиПособия[СтрокаПоказателя.Показатель]	= Неопределено Тогда
			УдаляемыеРеквизиты.Добавить(СтрокаПоказателя.ИмяРеквизита);
			УдаляемыеСтроки.Добавить(СтрокаПоказателя);
		КонецЕсли;
	КонецЦикла;
	Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида.Удалить(УдаляемаяСтрока);
	КонецЦикла;
	...
	Для Каждого СтрокаПоказателя Из Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида Цикл
		Если Элементы.Найти(СтрокаПоказателя.ИмяРеквизита) <> Неопределено Тогда
			...
			Продолжить;
		КонецЕсли;
		ПолеВвода = Элементы.Добавить(СтрокаПоказателя.ИмяРеквизита, Тип("ПолеФормы"), Элементы.ГруппаПособиеДоТрехЛетНаРебенкаИнвалидаНеРассчитывается);
		ПолеВвода.ПутьКДанным = СтрокаПоказателя.ИмяРеквизита;
		ПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
		ПолеВвода.КнопкаВыбора = Истина;
	КонецЦикла;
	...
	// "Включаем" пустую декорацию, чтобы отображать страницу без элементов.
	Элементы.ГруппаПособиеДоТрехЛетНаРебенкаИнвалидаРассчитываетсяДекорация.Видимость = Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида.Количество() = 0;
	
	ПрочитатьЗначенияПоказателейПособияДоТрехЛетНаРебенкаИнвалида(Форма);

КонецПроцедуры

Процедура ПрочитатьЗначенияПоказателейПособияДоТрехЛетНаРебенкаИнвалида(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	// Заполним поля ввода значениями показателей.
	Для Каждого СтрокаТаблицы Из ТекущийОбъект.Показатели Цикл
		НайденныеСтроки = Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида.НайтиСтроки(Новый Структура("Показатель", СтрокаТаблицы.Показатель));
		Если НайденныеСтроки.Количество() > 0 Тогда
			Форма[НайденныеСтроки[0].ИмяРеквизита] = СтрокаТаблицы.Значение;
		КонецЕсли;
	КонецЦикла;	
...
Процедура ЗаписатьЗначенияПоказателейПособияДоТрехЛетНаРебенкаИнвалида(Форма, ТекущийОбъект) Экспорт
	
	Для Каждого СтрокаПоказателей Из Форма.ПоказателиПособияДоТрехЛетНаРебенкаИнвалида Цикл
		НоваяСтрока = ТекущийОбъект.Показатели.Добавить();
		НоваяСтрока.Показатель = СтрокаПоказателей.Показатель;
		НоваяСтрока.Значение = Форма[СтрокаПоказателей.ИмяРеквизита];
		НоваяСтрока.ИдентификаторСтрокиВидаРасчета = Документы.ОтпускПоУходуЗаРебенком.ИдентификаторСтрокПоказателейПособияДоТрехЛет();
...
Процедура ЗаполнитьНачислениеВФормеДокументаПоКатегории(Форма, РеквизитНачисления, КатегорияНачисления, ДополнительныйОтбор = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 09.01.2018 11:35:11 
...
Процедура РасчетЗарплатыНДФЛВДанныеФормы(ТаблицаНДФЛ, ТаблицаВычетов, ТаблицаИсточник, НачальныеПозицииВставки = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 24.08.2017 12:48:56 
	// Ошибка при формировании документа увольнения, №14902 
	// {
	Если  НДФЛ.Колонки.Найти("ИдентификаторСтрокиНДФЛ") = Неопределено Тогда
		НДФЛ.Колонки.Добавить("ИдентификаторСтрокиНДФЛ");
	КонецЕсли;
	// }
	// 4D
...
Процедура ВзносыВДанныеМенеджераРасчета(ТаблицаИсточник, МенеджерРасчета, Отбор = Неопределено, НачальныеПозицииВставки = Неопределено) Экспорт
	...
	Если Отбор <> Неопределено Тогда
		...
		КонецЦикла;
		МенеджерРасчета.Зарплата.Взносы = ТаблицаИсточник.Выгрузить(СтрокиРасчета);
		НачальныеПозицииВставки.Вставить("Взносы", НачальныеПозицииСотрудников);
		
	Иначе
		МенеджерРасчета.Зарплата.Взносы = ТаблицаИсточник.Выгрузить();
	КонецЕсли;
