...
Процедура СформироватьОписаниеДокументовСКонтрагентомВШапке(Форма, Описание) Экспорт
	...
	Если ПроверкаКонтрагентовКлиентСервер.ЭтоДокументСКонтрагентомВШапке(Форма) Тогда
		...
		Если ТипЗнч(ДокументСсылка) <> Тип("ДокументСсылка.ЗаписьКнигиПокупок")
		  ...
		  //++ НЕ УТ
		  И ТипЗнч(ДокументСсылка) <> Тип("ДокументСсылка.ПоступлениеУслугПоЛизингу")
		  //-- НЕ УТ
...
Процедура СформироватьОписаниеДокументовСТабличнымиЧастями(Форма, Описание) Экспорт
	...
	Если ПроверкаКонтрагентовКлиентСервер.ЭтоДокументСКонтрагентомВТабличнойЧасти(Форма) Тогда
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураКомиссионеру")
			Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураКомитента") Тогда
...
Процедура СформироватьОписаниеСчетовФактур(Форма, Описание) Экспорт
	...
	Если ТипДокумента = Тип("ДокументСсылка.СчетФактураВыданный")
		...
		Или ТипДокумента = Тип("ДокументСсылка.СчетФактураПолученныйАванс") Тогда
	...
	Если ТипДокумента = Тип("ДокументСсылка.СчетФактураКомиссионеру") Тогда
		...
		СтруктураПараметровОформляемогоЭлемента.ГруппаСтраниц                =  Форма.Элементы.СтраницыПроблемаПроверкаКонтрагентаКонтрагент;
		СтруктураПараметровОформляемогоЭлемента.ЭлементФормыПодсказка        =  Форма.Элементы.ПроблемаПроверкаКонтрагента;
		СтруктураПараметровОформляемогоЭлемента.Имя                          =  "Комиссионер";
		
		ПроверкаКонтрагентов.ДополнитьОписание(
			Описание, 
			ДокументСсылка,
			СтруктураПараметровОформляемогоЭлемента,
			ДокументОбъект.Комиссионер);
...
Процедура ПолучитьИменаРеквизитовКонтрагентовВСчетеФактуре(Тип, ИменаРеквизитов) Экспорт
	...
	ИначеЕсли Тип = Тип("ДокументСсылка.СчетФактураКомиссионеру") Тогда
		
		ИменаРеквизитов.Добавить("Комиссионер");
...
Процедура ОпределитьДатуВСчетеФактуре(СчетФактураОбъект, Дата) Экспорт
	...
	Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураПолученный")
		...
		Дата = ?(ЗначениеЗаполнено(СчетФактураОбъект.Исправление), НачалоДня(СчетФактураОбъект.ДатаИсправления), НачалоДня(СчетФактураОбъект.Дата));
	...
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураВыданный")
		...
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураКомиссионеру") Тогда
		
		Дата = ?(ЗначениеЗаполнено(СчетФактураОбъект.ДатаВыставления), НачалоДня(СчетФактураОбъект.ДатаВыставления), НачалоДня(СчетФактураОбъект.Дата));
		
	ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураПолученныйАванс")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураНалоговыйАгент")
		Или ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СчетФактураНаНеподтвержденнуюРеализацию0") Тогда
...
// Параметры:
//  СтандартнаяОбработка - Булево - необходимо установить в Ложь, если стандартная обработка переопределяется.
//  ДанныеКонтрагентов   - ТаблицаЗначений - подготоваливаемая таблица с данными о контрагентах
//  Форма                - УправляемаяФорма, Неопределено - Форма из которой выполняется вызов.
//
Процедура ЗапомнитьРезультатПроверкиКонтрагентовСчетФактурыДокументе(СтандартнаяОбработка, ДанныеКонтрагентов, Форма) Экспорт
	
	//++ НЕ ЕГАИС
	СтандартнаяОбработка = Ложь;
	
	ТаблицаКонтрагентовСчетовФактур = ПроверкаКонтрагентовКлиентСервер.СчетФактура(Форма);
	ДанныеПоСчетамФактурам = ДанныеКонтрагентов.СкопироватьКолонки();
	ДанныеПоСчетамФактурам.Колонки.Добавить("ТипСчетФактуры");
	СписокСчетФактур       = Новый СписокЗначений;
	
	Если ТаблицаКонтрагентовСчетовФактур <> Неопределено Тогда
		Для Каждого СтрокаСчетФактуры Из ТаблицаКонтрагентовСчетовФактур Цикл
			
			НайденныеСтроки = ДанныеКонтрагентов.НайтиСтроки(Новый Структура("Ссылка", СтрокаСчетФактуры.Ссылка));
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				НоваяСтрока = ДанныеПоСчетамФактурам.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, НайденнаяСтрока);
				НоваяСтрока.ТипСчетФактуры = ТипЗнч(НоваяСтрока.Ссылка);
				Если ПроверкаКонтрагентовКлиентСерверПовтИсп.ЭтоСостояниеНедействующегоКонтрагента(Форма.РеквизитыПроверкиКонтрагентов.СостояниеКонтрагента, Истина) Тогда
					СтрокаСчетФактуры.ЕстьОшибки = Истина;
				КонецЕсли;
			КонецЦикла;
			
			СтрокаСпискаСчетФактур = СписокСчетФактур.НайтиПоЗначению(СтрокаСчетФактуры.Ссылка);
			Если СтрокаСпискаСчетФактур = Неопределено Тогда
				СтрокаСпискаСчетФактур = СписокСчетФактур.Добавить(СтрокаСчетФактуры.Ссылка);
			КонецЕсли;
			
			Если СтрокаСчетФактуры.ЕстьОшибки Тогда
				СтрокаСпискаСчетФактур.Пометка = Истина;
			КонецЕсли;
			
		КонецЦикла;
	КонецЕсли;
	
	Форма.РеквизитыПроверкиКонтрагентов.СписокСчетФактур = СписокСчетФактур;
	
	Если Форма.РеквизитыПроверкиКонтрагентов.СчетаФактурыНесколькихТипов Тогда
		Для каждого ЭлементСписка Из Форма.РеквизитыПроверкиКонтрагентов.СписокТиповСчетовФактур Цикл
			
			СтруктураОтбора = Новый Структура("ТипСчетФактуры", Тип("ДокументСсылка." + ЭлементСписка.Представление));
			СостояниеПоТипуСчетФактуры = ПроверкаКонтрагентов.ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеПоСчетамФактурам, СтруктураОтбора);
			ЭлементСписка.Значение     = СостояниеПоТипуСчетФактуры.СостояниеКонтрагента;
			
		КонецЦикла;
	Иначе
		
		ИтоговоеСостояние = ПроверкаКонтрагентов.ИтоговоеСостояниеКонтрагентаВДокументе(ДанныеПоСчетамФактурам);
