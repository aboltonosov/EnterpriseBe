	...
	Иначе
		...
		ИначеЕсли НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(ТекущиеДанные.СтатусУказанияСерий, ПараметрыУказанияСерий)
			...
			Если ПараметрыУказанияСерий.ПолноеИмяОбъекта = "Документ.ЭтапПроизводства2_2"
				И ТекущиеДанные.Свойство("ВариантОбеспечения")
				И ТипЗнч(ИсточникДанныхФормы) <> Тип("УправляемаяФорма")
				И ИсточникДанныхФормы.Свойство("НазначениеМатериалы") Тогда
				Если ТекущиеДанные.ВариантОбеспечения = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.ОтгрузитьОбособленно") Тогда
					ПараметрыПодбораСерий.Вставить("Назначение", ИсточникДанныхФормы.НазначениеМатериалы);
				Иначе
					ПараметрыПодбораСерий.Вставить("Назначение", ПредопределенноеЗначение("Справочник.Назначения.ПустаяСсылка"));
				КонецЕсли;
			КонецЕсли;
