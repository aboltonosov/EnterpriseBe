			...
			Если ЗначениеЗаполнено(СобытиеОтпусков.ФактическиеОтпуска) Тогда
				...
				Если ЕстьКомпенсация И ЕстьРеальныйОтпуск Тогда
					ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Период отпуска: %1 - %2 (+ %3 дн. компенсации)'"),
...
// Функция рассчитывает сводные остатки отпусков для списка сотрудников.
//
// Параметры:
...
Процедура ПеренестиРезультатыРасчетаОтпускаЧАЭС(СтрокаДанных, ИсходныеДанные) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 26.06.2017 14:58:15 
	// Ошибка УчетНДФЛ, №15123
	// {	
	ВидОтпуска = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДанных.Начисление, "ВидОтпуска");
	// }
	// 4D
...
// Формирует временную таблицу, на основе переданной, с признаком того, попадает ли сотрудник под ст. 291 и ст. 295 ТК РФ на дату среза.
// Параметры
...
Процедура СоздатьВТНеРассчитанныхСотрудников(ТаблицаДанных, МенеджерВременныхТаблиц, РассчитатьПравоНаОтпускаВсемСотрудникам)
	...
	Запрос.Текст =
		...
		|	ПраваНаОтпуска.ДатаОкончанияРасчета
		...
		|	СотрудникиСДаннымиДляРасчета.ВидЕжегодногоОтпуска,
		|	СотрудникиСДаннымиДляРасчета.ДатаНачалаРасчета,
		|	СотрудникиСДаннымиДляРасчета.ДатаОкончанияРасчета,
		|	ТаблицаДанных.ЭтоРасчетПриУвольнении
	...
	Иначе
		Запрос.Текст = Запрос.Текст + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов() +
			...
			|	ПроверяемыеСотрудники.ВидЕжегодногоОтпуска,
			|	ВЫБОР
			...
			|	ПроверяемыеСотрудники.ЭтоРасчетПриУвольнении
			...
			|	ПроверяемыеСотрудники.Сотрудник,
			|	ПроверяемыеСотрудники.ВидЕжегодногоОтпуска,
			|	ПроверяемыеСотрудники.ДатаНачала,
			|	ПроверяемыеСотрудники.ДатаОкончания,
			|	ПроверяемыеСотрудники.ЭтоРасчетПриУвольнении
			...
			//|
			//|////////////////////////////////////////////////////////////////////////////////
			//|УНИЧТОЖИТЬ ВТПоследниеРассчитанныеЗаписи
			//|;
			//|
			//|////////////////////////////////////////////////////////////////////////////////
			//|УНИЧТОЖИТЬ ВТСотрудникиРегистр
			//|;
			//|
			//|////////////////////////////////////////////////////////////////////////////////
			//|УНИЧТОЖИТЬ ВТРегистрНеРассчитанныеСотрудники";
	КонецЕсли;
	...
	Запрос.Текст = Запрос.Текст + ЗарплатаКадрыОбщиеНаборыДанных.РазделительЗапросов() +
		...
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТПервыеДействующиеПрава
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТУказателиПоследнихДействующихПрав
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТДействующиеПраваДатаОкончанияРасчета
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТДействующиеПраваНаОтпуска
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТСотрудникиСДаннымиДляРасчета
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ ВТСотрудникиПолнаяТаблицаДляРасчета";
...
Процедура СоздатьВТИсходныхДанныхДляРасчетаОстатков(ТаблицаДанных, МенеджерВременныхТаблиц)
	...
	|			И ПериодыПрерывающиеПравоНаОтпуск.ДатаНачалаОтсутствия < СотрудникиБезПраваНаОтпуск.ДатаОстатков
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
		...
		Для Каждого СтрокаВидаОтпуска Из ТаблицаВидовОтпуска Цикл
			...
			Если ПраваОтпускаВРегистре.Количество() = 0 Тогда
				...
				Для каждого СтрокаНабора Из НаборЗаписей Цикл
					ЗаполнитьЗначенияСвойств(ИтоговаяТаблицаЗаработанныхПрав.Добавить(), СтрокаНабора);
				КонецЦикла;
...
Процедура СформироватьВТОтпускаШтатного(Запрос)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьИсториюИзмененияШтатногоРасписания") Тогда
		Запрос.Текст = 
			...
			|	ИсторияЕжегодныхОтпусковПоШтатномуРасписанию.ВидЕжегодногоОтпуска,
			|	ИсторияЕжегодныхОтпусковПоШтатномуРасписанию.Дата,
			|	ИсторияЕжегодныхОтпусковПоШтатномуРасписанию.КоличествоДнейВГод
			...
			|ВЫБРАТЬ
			|	ВТИсторияЕжегодныхОтпусковПоШтатномуРасписанию.ПозицияШР,
			|	ВТИсторияЕжегодныхОтпусковПоШтатномуРасписанию.ВидЕжегодногоОтпуска,
			|	ВТИсторияЕжегодныхОтпусковПоШтатномуРасписанию.КоличествоДнейВГод
			...
			|ГДЕ
			|	ВТИсторияЕжегодныхОтпусковПоШтатномуРасписанию.КоличествоДнейВГод <> 0
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
...
Процедура РассчитатьКоличествоДнейОтпуска(СотрудникБезПрава, СтрокаВидаОтпуска, РабочиеГода, МассивПравНаОтпуск, ТаблицаПереквалификацииДней)
	...
	// В случае, если действовали ст. 291 или 295 ТК РФ и есть "отложенные дни" - запишем их в первый месяц, следующий после окончания ее действия
	Для Каждого РабочийГод Из РабочиеГода Цикл
