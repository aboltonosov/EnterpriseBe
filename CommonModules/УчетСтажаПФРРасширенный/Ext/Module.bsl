...
Функция ВыборкаДанныхУчетаСтажаПоДокументамПереносДанных(МассивСсылок)
	...
	|	СоответствиеКатегорийНачисленийСостояниям.Состояние
	...
	|	НачисленияОтклонения.ВидСтажаПФР,
	|	Начисления.Регистратор,
	|	Начисления.Сторно,
	|	НачисленияОтклонения.Начисление,
	|	НачисленияОтклонения.Состояние
...
Процедура ЗаполнитьПолеСотрудникВРегистрахУчетаИспр(ПараметрыОбновления = Неопределено) Экспорт 
	...
	|	РегистрСведений.ПараметрыПериодовСтажаПФРИспр КАК ПараметрыПериодовСтажаПФРИспр
	|ГДЕ
	|	ПараметрыПериодовСтажаПФРИспр.Приоритет = ЗНАЧЕНИЕ(Перечисление.СостоянияСотрудника.Работа)
	|	И ПараметрыПериодовСтажаПФРИспр.ИспользованиеРесурсаВнутреннееСовместительство = ЗНАЧЕНИЕ(Перечисление.ЗначениеРесурсаРегистровСостоянийСотрудника.ПустаяСсылка)
	|	И ПараметрыПериодовСтажаПФРИспр.ИспользованиеРесурсаПодразделение = ЗНАЧЕНИЕ(Перечисление.ЗначениеРесурсаРегистровСостоянийСотрудника.Установлено)
	|	И ПараметрыПериодовСтажаПФРИспр.ТипДоговора = ЗНАЧЕНИЕ(Перечисление.ТипыДоговоровСЗВ63.Трудовой)";
	...
	УчетСтажаПФР.ЗаполнитьПолеСотрудникВРегистре("ПараметрыПериодовСтажаПФРИспр", "РегистраторИзмерение", ПараметрыОбновления);	
КонецПроцедуры	
