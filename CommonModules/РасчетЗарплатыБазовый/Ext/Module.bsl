...
Функция РезультатРасчетаКорректировкиВыплаты(Объект, МесяцНачисленияИмя = "МесяцНачисления") Экспорт
	...
	ТекущиеОстаткиВзаиморасчетов = ВзаиморасчетыССотрудниками.ОстаткиВзаиморасчетов(Объект.Организация, НачалоМесяца(Объект[МесяцНачисленияИмя]), ИсчисленныйНалогФизическиеЛица, Объект.Ссылка);
