...
Процедура ДобавитьКомандуПечатиПриказаОбУвольнении(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ПФ_MXL_Т8_Локализация";
...
Процедура ДобавитьКомандуПечатиПриказаОПриеме(КомандыПечати) Экспорт   	
	...
	// 4D:ERP для Беларуси, Юлия, 04.08.2017 16:09:52 
	// Локализованные макеты по кадрам и ЗП, № 15656
	// { 	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Обработчик = "ЗарплатаКадрыКлиент.ВыполнитьКомандуПечати";
	КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказов";
	КомандаПечати.Идентификатор = "ПФ_MXL_Т1_Локализация";
...
Процедура ДобавитьКомандуПечатиРегистраНалоговогоУчетаПоНДФЛ(КомандыПечати) Экспорт
	...
	// {
	КомандаПечати.Представление = НСтр("ru = 'Регистр налогового учета по подоходному налогу'");	
	// }
	// 4D
	КомандаПечати.Порядок = 60;
...
Процедура ПрочитатьРазмерАвансаПоУмолчанию(Форма) Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 19.10.2015 18:48:33 
	// , №10496
	// {
	//Если Форма.ДоступенПросмотрДанныхДляНачисленияЗарплаты И НЕ Форма.ПолучитьФункциональнуюОпциюФормы("ИспользоватьКадровыйУчет") Тогда
...
Функция ПолучитьСведенияОВычетахНДФЛ(ФизическоеЛицо) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 02.08.2017 11:05:01 
	// 267 релиз: Ошибки в Справочнике Сотрудники, Документе Отпуск, №15624 
	// {
	...
	Если Пользователи.РолиДоступны("ЧтениеНалоговИВзносов,ДобавлениеИзменениеНалоговИВзносов") Тогда
		...
		|	СтандартныеВычетыФизическихЛицНДФЛСрезПоследних.КодВычетаЛичныйДополнительный
		|ИЗ
		...
		|	СтандартныеВычетыНаДетейНДФЛ.ДатаДействия КАК ДействуетС
		|ИЗ
		|	РегистрСведений.СтандартныеВычетыНаДетейНДФЛ КАК СтандартныеВычетыНаДетейНДФЛ
		...
		|	ИмущественныеВычетыНДФЛИРасходыНаОбразованиеОстаток.СуммаОстаток КАК РасходыНаОбразование,
		|	ИмущественныеВычетыНДФЛИмущественныйВычетОстатки.СуммаОстаток КАК РасходыНаСтроительствоПриобретение,
		|	ИмущественныеВычетыНДФЛИРасходыНаСтрахованиеОстаток.СуммаОстаток КАК РасходыНаСтрахование
		|ИЗ
		|	ИмущественныйВычетМаксимум КАК ИмущественныйВычетМаксимум
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИмущественныеВычетыНДФЛ.Остатки(
		|				&ТекущаяДата,
		|				ФизическоеЛицо = &ФизическоеЛицо
		|					И КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640)) КАК ИмущественныеВычетыНДФЛИРасходыНаОбразованиеОстаток
		|		ПО ИмущественныйВычетМаксимум.ФизическоеЛицо = ИмущественныеВычетыНДФЛИРасходыНаОбразованиеОстаток.ФизическоеЛицо
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИмущественныеВычетыНДФЛ.Остатки(
		|				&ТекущаяДата,
		|				ФизическоеЛицо = &ФизическоеЛицо
		|					И КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640Страх)) КАК ИмущественныеВычетыНДФЛИРасходыНаСтрахованиеОстаток
		|		ПО ИмущественныйВычетМаксимум.ФизическоеЛицо = ИмущественныеВычетыНДФЛИРасходыНаСтрахованиеОстаток.ФизическоеЛицо
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИмущественныеВычетыНДФЛ.Остатки(
		|				&ТекущаяДата,
		|				ФизическоеЛицо = &ФизическоеЛицо
		|					И КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код650)) КАК ИмущественныеВычетыНДФЛИмущественныйВычетОстатки
		|		ПО ИмущественныйВычетМаксимум.ФизическоеЛицо = ИмущественныеВычетыНДФЛИмущественныйВычетОстатки.ФизическоеЛицо";
