...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура           = "ОбновлениеИнформационнойБазыБППереопределяемый.КонтактнаяИнформацияОбновлениеИБ";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий 		   = НСтр("ru = 'Обновление регистра сведений ""Журнал учета счетов-фактур""'");
	Обработчик.РежимВыполнения 	   = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("7403552a-cc5f-469b-b43f-66d4d093d9f0");
	...
	Обработчик.РежимВыполнения 	   = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("0a79e2f0-2f3d-413e-aad4-748b674f1d9b");
...
Процедура ЗаполнитьПустуюИБ() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Константы.ВалютаРегламентированногоУчета.Получить()) Тогда
		...
		ВалютаРегламентированногоУчета = НайтиСоздатьВалюту(
			"933", 
			НСтр("ru = 'BYN'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			НСтр("ru = 'Белорусский рубль'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			"белорусский рубль, белорусских рубля, белорусских рублей, м, копейка, копейки, копеек, ж, 2");
		// }
		// 4D
...
Процедура УстановитьСуммовойУчетНаСкладах()
	...
	// 4D:ERP для Беларуси, Екатерина, 30.06.2015 17:17:29 
	// Локализация плана счетов, №8969
	// {
	Счета.Добавить("ОборудованиеКУстановке");
	Счета.Добавить("ОборудованиеКУстановкеПереданноеВМонтаж");
	Счета.Добавить("СтроительныеМатериалы");
...
Процедура УстановитьСубконтоСтатьиДвиженияДенежныхСредств()
	...
	// 4D:ERP для Беларуси, Екатерина, 01.07.2015 16:05:32 
	// Локализация плана счетов, №8969
	// {
	ОбрабатываемыеСчета.Добавить(ПланыСчетов.Хозрасчетный.КассаФилиала);
	...
	ОбрабатываемыеСчета.Добавить(ПланыСчетов.Хозрасчетный.СчетаВДрагоценныхМеталлах);
...
Процедура УстановитьСубконтоНоменклатура(ИмяСчета, ИмяПланаСчетов, ЕстьКоличество, ЕстьКонтрагент)
	...
	Попытка
		...
		Для каждого Счет Из Счета Цикл
			...
			// 4D:ERP для Беларуси, Екатерина, 01.07.2015 15:32:11 
			// Локализация плана счетов, №8969
			// {
			Если Объект.Код = "41.12" ИЛИ Объект.Код = "45.04" ИЛИ Объект.Код = "41.ДР" ИЛИ Объект.Код = "10.ДР" ИЛИ Объект.Код = "10.12" 
				ИЛИ Объект.Код = "08.01.2" ИЛИ Объект.Код = "08.01" 
				ИЛИ Объект.Код = "08.02.1" ИЛИ Объект.Код = "08.02.2" ИЛИ Объект.Код = "08.02" 
				ИЛИ Объект.Код = "08.05.2" ИЛИ Объект.Код = "08.05" Тогда
				Продолжить;
			КонецЕсли;
			// }
			// 4D
