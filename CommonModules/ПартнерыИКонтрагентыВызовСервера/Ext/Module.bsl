...
//                  ДанныеЕдиныхГосРеестров.НовыеРеквизитыЮридическогоЛица() и 
//                  ДанныеЕдиныхГосРеестров.НовыеРеквизитыПредпринимателя()
//
...
Функция ФоновоеЗаданиеДанныеПартнераПоИННЗапустить(ИНН, ЮрФизЛицо, УникальныйИдентификатор, ИдентификаторЗадания) Экспорт
	...
	Попытка
		РезультатФоновогоЗадания = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			...
			// 4D:ERP для Беларуси, Дмитрий, 31.03.2017 15:45:25 
			// Загрузка клиентов по УНП, №14614 
			// {
			НСтр("ru = 'Работа с контрагентами: получение реквизитов по УНП.'"));
			// }
			// 4D
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер) Экспорт
	...
	ПартнерыИКонтрагенты.ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер);
