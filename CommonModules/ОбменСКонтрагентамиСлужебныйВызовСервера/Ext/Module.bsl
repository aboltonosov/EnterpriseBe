...
Процедура ОбновитьВерсиюЭД(ЭлектронныйДокумент, ПринудительнаяСменаСостоянияВерсии = Ложь, ВерсияФорматаПакета = Неопределено, 
	...
	// 4D:ERP для Беларуси, ВалентинМ, 14.08.2017
	// Перенести изменения по ЭСЧФ из 2.1.3.266 в 2.2.3.138, 15728
	// {
	Если ТипЗнч(СвойстваЭД.ВладелецФайла) <> Тип("ДокументСсылка.ЭлектронныйДокументВходящий") Или ТипЗнч(СвойстваЭД.ВладелецФайла) <> Тип("ДокументСсылка.ЭлектронныйДокументИсходящий") Тогда
		Возврат;
	КонецЕсли; 
	// }
	// 4D 
