...
Процедура ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	// {
	РозничныеПродажиКлиентСервер_Локализация.ЗаполнитьЦенуИзЦеныИзготовителяВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий);
	РозничныеПродажиКлиентСервер_Локализация.РассчитатьСтавкуСуммуТорговойНадбавкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий);
	РозничныеПродажиКлиентСервер_Локализация.РассчитатьРозничнуюЦенуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий);
	// }
	// 4D
...
Функция НеобходимВызовСервера(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	// 4D:ERP для Беларуси, Екатерина, 12.04.2016 16:24:55 
	// Расчет скидки к цене, №10910
	// {
	Если СтруктураДействий.Свойство("ПересчитатьЦенуСкидкуПоСуммеВПродажах", СтруктураПараметровДействия) 
		ИЛИ СтруктураДействий.Свойство("ПересчитатьПроцентРучнойСкидкиВПродажах", СтруктураПараметровДействия) Тогда
	// }
	// 4D
...
Функция ПолучитьТекущуюСтрокуСтруктурой(ТекущаяСтрока, СтруктураДействий)
	...
	// 4D:ERP для Беларуси, Петр, 20.02.2018 21:58:00 
	// Механизм ввода остатков, № 17856
	// {
	Если СтруктураДействий.Свойство("ПересчитатьСуммуУпрПоСуммеРегл") Тогда
		СтруктураПолейТЧ.Вставить("Сумма"    , 0);
		СтруктураПолейТЧ.Вставить("СуммаБезНДС", 0);
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДСРегл") Тогда
		СтруктураПолейТЧ.Вставить("СуммаРегл"    , 0);
		СтруктураПолейТЧ.Вставить("СтавкаНДС", 0);
		СтруктураПолейТЧ.Вставить("НДСРегл" , 0);
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ПересчитатьЦенуРеглЗаУпаковку") Тогда
		СтруктураПолейТЧ.Вставить("ЦенаРегл"      , 0);
		СтруктураПолейТЧ.Вставить("Количество", 0);
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРеглПоЦенеРегл") Тогда
		СтруктураПолейТЧ.Вставить("СуммаРегл"             , 0);
		СтруктураПолейТЧ.Вставить("ЦенаРегл"              , 0);
	КонецЕсли;
	// }
	...
	// 4D:ERP для Беларуси, Екатерина, 12.04.2016 16:32:30 
	// Расчет скидки к цене, №10910
	// {
	Если СтруктураДействий.Свойство("ПересчитатьЦенуСкидкуПоСуммеВПродажах") 
		ИЛИ СтруктураДействий.Свойство("ПересчитатьПроцентРучнойСкидкиВПродажах") Тогда
	// }
	// 4D
		СтруктураПолейТЧ.Вставить("Цена"                       , 0);
		СтруктураПолейТЧ.Вставить("Сумма"                      , 0);
	...
	// 4D:ERP для Беларуси, Екатерина, 05.04.2016 15:07:59 
	// Расчет скидки к цене, №10910
	// {
	...
	// Розничная торговля, №15729
	// {
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуТорговойНадбавки") Тогда
		СтруктураПолейТЧ.Вставить("Номенклатура");    
		СтруктураПолейТЧ.Вставить("Характеристика");
		СтруктураПолейТЧ.Вставить("СтавкаТН");
		СтруктураПолейТЧ.Вставить("СуммаТН");
	КонецЕсли; 
	
	Если СтруктураДействий.Свойство("ЗаполнитьРозничнуюСтавкуНДС") Тогда
		СтруктураПолейТЧ.Вставить("Номенклатура");
		СтруктураПолейТЧ.Вставить("РозничнаяСтавкаНДС");
	КонецЕсли;
	
	Если СтруктураДействий.Свойство("РассчитатьРозничнуюЦену") 
		ИЛИ СтруктураДействий.Свойство("РассчитатьСтавкуСуммуТорговойНадбавки")Тогда
		СтруктураПолейТЧ.Вставить("Номенклатура");
		СтруктураПолейТЧ.Вставить("РозничнаяСтавкаНДС");
		СтруктураПолейТЧ.Вставить("СуммаТН");
		СтруктураПолейТЧ.Вставить("СтавкаТН");
		СтруктураПолейТЧ.Вставить("РозничнаяСуммаНДС");
		СтруктураПолейТЧ.Вставить("РозничнаяЦенаБезНДС");
		СтруктураПолейТЧ.Вставить("РозничнаяЦена");
		СтруктураПолейТЧ.Вставить("Цена");
		СтруктураПолейТЧ.Вставить("Сумма");
		СтруктураПолейТЧ.Вставить("СуммаНДС");
		СтруктураПолейТЧ.Вставить("РозничнаяЦенаСУчетомОкругления");
		СтруктураПолейТЧ.Вставить("Округление");		
	КонецЕсли;	
	// }
	// 4D
