...
Процедура ПроверитьКорректностьЗаполненияДокументаЗакупки(ДокументЗакупки, Отказ) Экспорт
	...
	ИначеЕсли ИмяТаблицы = "Документ.ПоступлениеТоваровУслуг" Тогда
		...
		ЕстьОшибкиСоглашениеНеУказано = ИспользоватьСоглашенияСПоставщиками
		                                ...
		                                И ДокументЗакупки.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо
		                                И НужноСогласовать;
...
Функция ПолучитьДоговорПоУмолчанию(
	...
	Запрос.УстановитьПараметр("УчетАгентскогоНДС",			Налогообложение = Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС);
...
Функция ЗаполнитьТоварыПоФактическойПриемке(Товары, ПараметрыЗаполнения, ЕстьПринятыеТовары, ДополнительныеПараметрыПоиска = Неопределено) Экспорт
	...
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеТоваровУслуг") Тогда
...
Процедура ЗаполнитьСписокВыбораНалогообложения(Поле, ХозяйственнаяОперация) Экспорт
	...
	//++ НЕ УТ
	ДоступноОблагаетсяЕНВД = ПолучитьФункциональнуюОпцию("ИспользоватьУведомленияОКонтролируемыхСделках");
	ЗначениеОблагаетсяЕНВД = СписокНалогообложений.НайтиПоЗначению(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД);
	Если ДоступноОблагаетсяЕНВД И ЗначениеОблагаетсяЕНВД = Неопределено Тогда
		СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД, НСтр("ru = 'Закупка облагается ЕНВД'"));
	ИначеЕсли Не ДоступноОблагаетсяЕНВД И Не ЗначениеОблагаетсяЕНВД = Неопределено Тогда
		СписокНалогообложений.Удалить(ЗначениеОблагаетсяЕНВД);
	КонецЕсли;
	//-- НЕ УТ
...
Функция ДопустимыеОтклоненияМерныхТоваров(МассивРаспоряжений,РаспоряжениеЗаказ = Истина) Экспорт
	...
	ИначеЕсли ТипЗнч(МассивРаспоряжений[0]) = Тип("ДокументСсылка.ЗаказПоставщику")
		...
		ИЛИ ТипЗнч(МассивРаспоряжений[0]) = Тип("ДокументСсылка.ЗаказНаПроизводство")
		//-- НЕ УТКА
		//++ НЕ УТ
...
Функция ТекстЗапросаПроверкаПринимающихсяТоваровПоРаспоряжениям(ТипПоступления)
	...
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеОтПереработчика") Тогда
		...
		|		ТоварыПоступления.Количество	    КАК Количество
		|	ИЗ
		|		Документ.ПоступлениеОтПереработчика.ВозвратныеОтходы КАК ТоварыПоступления
		|	ГДЕ
		|		ТоварыПоступления.Распоряжение В (&МассивРаспоряжений)
		|		И ТоварыПоступления.Ссылка <> &ДокументПоступления
		|		И ТоварыПоступления.Ссылка.Проведен) КАК ТоварыПоступления
		|
		|СГРУППИРОВАТЬ ПО
		|	ТоварыПоступления.Номенклатура,
		|	ТоварыПоступления.Характеристика,
...
Функция ТекстЗапросаПоРаспоряжениям(ТипПоступления)
	...
	ИначеЕсли ТипПоступления = Тип("ДокументСсылка.ПоступлениеСырьяОтДавальца") Тогда
		...
		ТекстЗапросаПоРаспоряжениям = 
		"ВЫБРАТЬ
		|	ТоварыПоступления.ЗаказДавальца		КАК Распоряжение,
		|	ТоварыПоступления.Номенклатура		КАК Номенклатура,
		|	ТоварыПоступления.Характеристика	КАК Характеристика,
		|	ТоварыПоступления.Серия				КАК Серия,
		|	ТоварыПоступления.Склад				КАК Склад,
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	ТоварыПоступления.ЗаказДавальца,
		|	ТоварыПоступления.Номенклатура,
		|	ТоварыПоступления.Характеристика,
		|	ТоварыПоступления.Серия,
		|	ТоварыПоступления.Склад
...
Процедура СформироватьЗапросВременнаяТаблицаТоварыДокументаЗакупки(ТекстЗапроса,
	...
	ИначеЕсли ИмяТаблицы = "Документ.ЗаказПереработчику" Тогда
		...
		ПредставлениеТабличнойЧасти = НСтр("ru='Выпускаемая продукция'");
...
Функция ПредставлениеСчетаФактуры(Номер, Дата, Проведен) Экспорт
	...
	Возврат НСтр("ru='Счет-фактура'") + " " +"№ " + СокрЛП(Номер) + " от " + Формат(Дата, "ДЛФ=D") + " г." + ?(НЕ Проведен, НСтр("ru=' (не проведен)'"),"");
