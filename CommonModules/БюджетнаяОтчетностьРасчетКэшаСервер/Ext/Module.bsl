...
Функция ПолучитьСтруктуруКолонокТаблицыДанныхВидаБюджета(ВидБюджета) Экспорт
	...
	РегистрыСведений.КэшВспомогательныхДанныхВидаБюджета.УстановитьЗначениеКэша(ВидБюджета, ВидЭлементаКэша, ВозвращаемоеЗначение);
...
Функция ПолучитьСтруктуруОписанияВидаБюджета(ВидБюджета, СтруктураКолонокТаблицыДанных, РежимФормирования) Экспорт
	...
	ОписаниеЗамера = Производительность.НачатьЗамерВремени("Бюджетирование.РасчетКэшаПолучениеСтруктурыОписания");
...
Функция ТаблицаОперандовТребующихРасчетаДанныхЯчеек(СтруктураОписаниеОтчета) Экспорт
	...
	|					И РасчетныеИИтоговыеЭлементы.ВидЭлемента В (&СписокПроизольныхИГрупп)
	...
	СписокПроизольныхИГрупп = Новый СписокЗначений;
	СписокПроизольныхИГрупп.Добавить(Перечисления.ВидыЭлементовФинансовогоОтчета.ПроизводныйПоказатель);
	СписокПроизольныхИГрупп.Добавить(Перечисления.ВидыЭлементовФинансовогоОтчета.Группа);
	СписокПроизольныхИГрупп.Добавить(Перечисления.ВидыЭлементовФинансовогоОтчета.ФормулаПоГруппе);
	...
	Запрос.УстановитьПараметр("СписокПроизольныхИГрупп", СписокПроизольныхИГрупп);
...
Функция ИсточникиЗначенийЭлемента(Кэш, СтрокаДерева, Заполнение = Ложь, БезПроизводных = Ложь) Экспорт
	...
	Если Не СложнаяТаблица Тогда
		...
		ПодчиненныеИсточникиЗначенийЭлемента(ФинансоваяОтчетностьКлиентСервер.ПодчиненныеСтроки(СтрокаДерева), 
																			ТаблицаИсточников, ВидыЭлементовЗначений);
...
Процедура ДобавитьИсточник(СтруктураКэша, Источники, ОтобраннаяСтрока, СтруктураКолонок, ИсточникДокумент)
	...
	СтруктураИсточника.Вставить("ПривилегированныйРасчет", 
		?(ОтобраннаяСтрока.ПривилегированныйРасчет = Неопределено, Ложь, ОтобраннаяСтрока.ПривилегированныйРасчет));
...
Функция СравнитьИсточники(СтруктураКэша, Источник, ОтобраннаяСтрока, СтруктураКолонок, ИсточникДокумент)
	...
	ОтобраннаяСтрокаПривилегированныйРасчет = ФинансоваяОтчетностьСервер.ЗначениеДополнительногоРеквизита(ЭлементОтчета, 
																ДопРеквизиты.ПривилегированныйРасчет,
																КэшДопРеквизитов);
																
	Если ОтобраннаяСтрокаПривилегированныйРасчет = Неопределено Тогда
		ОтобраннаяСтрокаПривилегированныйРасчет = Ложь;
	КонецЕсли;
																
	Если ОтобраннаяСтрокаПривилегированныйРасчет <> Источник.ПривилегированныйРасчет Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если Не ДляПоказателя Тогда
		Для Каждого Аналитика из Аналитики Цикл
			...
			Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, ИмяПараметра,, Истина) Тогда
				...
				ТекстПоля = ПолучитьТекстПоляВидаАналитики(ИмяПараметра);
	...
	Если ДляПоказателя Тогда
		Для Каждого Измерение из СтруктураКолонокТаблицыДанных.ВидыАналитик Цикл
			...
			Если Источник.Свойство(ИмяАналитики, ОтборПоИзмерениям)
				...
				ТекстАналитики = ПолучитьТекстПоляВидаАналитики(ИмяАналитики, "ПредварительныйРасчет.");
				ТекстАналитики = СтрЗаменить(ТекстАналитики, "СтатьяБюджетов.", "");
		...
		Для Каждого Аналитика из Аналитики Цикл
			...
			Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, ИмяПараметра,, Истина) Тогда
				...
				ТекстПоля = ПолучитьТекстПоляВидаАналитики(ИмяПараметра, "ПредварительныйРасчет.");
...
Функция ПолучитьСКДПлановыеДанные(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, Аналитики, ПараметрыКУстановке, РежимыФормирования)
	...
	Если РежимыФормирования.Свойство("ПривилегированныйРасчет") Тогда 
		ПривилегированныйРасчет = РежимыФормирования.ПривилегированныйРасчет;
	Иначе
		ПривилегированныйРасчет = Ложь;
	КонецЕсли;
	...
	Для Каждого Источник из ИсточникиДанных Цикл
		...
		Если Источник.ПривилегированныйРасчет = Неопределено И ПривилегированныйРасчет 
			ИЛИ (Не Источник.ПривилегированныйРасчет = Неопределено И Не Источник.ПривилегированныйРасчет = ПривилегированныйРасчет) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Источник.ТипИсточника = "Обороты"
			...
			ДобавитьПодчиненныйНаборПоБюджетномуРегистру(СКДПлановыеДанные, БазовыйОбъединенныйНабор, 
														...
														ПараметрыКУстановке, РежимФормирования, РежимПолученияДанных);
...
Функция ПолучитьСКДФактическиеДанные(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, Аналитики, ПараметрыКУстановке, РежимыФормирования)
	...
	Для Каждого Источник из ИсточникиДанных Цикл
		...
		Если Источник.ПривилегированныйРасчет = Неопределено И ПривилегированныйРасчет 
			ИЛИ Не Источник.ПривилегированныйРасчет = ПривилегированныйРасчет Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Источник.ТипИсточника = "Обороты"
			...
			ДобавитьПодчиненныйНаборПоБюджетномуРегистру(СКДФактическиеДанные, БазовыйОбъединенныйНабор, 
														Источник, СтруктураКолонокТаблицыДанных, Аналитики, 
														ПараметрыКУстановке, РежимФормирования, РежимПолученияДанных, Истина);
...
Функция ПолучитьСКДИтоговыеДанныеОбороты(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, ЕстьХранимыеПравила, ЕстьПривилегированныйРасчет)
	...
	Если ЕстьПривилегированныйРасчет Тогда
...
Функция ПолучитьСКДПредварительныйИтоговыйРасчет(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, ЕстьХранимыеПравила, ЕстьПривилегированныйРасчет)
...
Функция ПолучитьСКДПоОписаниюИсточников(ВидБюджета, СтруктураОписанияОтчета, СтруктураКолонокТаблицыДанных, РежимФормирования)
	...
	ОписаниеЗамера = Производительность.НачатьЗамерВремени("Бюджетирование.РасчетКэшаПолучениеСКДИсточников");
...
Функция РасчетКэшаПолучениеСКДИсточников(ВидБюджета, СтруктураОписанияОтчета, СтруктураКолонокТаблицыДанных, РежимФормирования, ВидЭлементаКэша, Постфикс)
	...
	ЕстьПривилегированныйРасчет = Ложь;
	...
	Для Каждого Источник из ИсточникиДанных Цикл
		...
		ЕстьПривилегированныйРасчет = ЕстьПривилегированныйРасчет ИЛИ 
			(Не Источник.ПривилегированныйРасчет = Неопределено И Источник.ПривилегированныйРасчет);
		Если ЕстьОстатки И ЕстьХранимыеПравила И ЕстьПривилегированныйРасчет Тогда
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Если ЕстьПривилегированныйРасчет Тогда
		...
		РежимыФормирования.Вставить("ПривилегированныйРасчет", Истина); 
	...
	Если ЕстьОстатки Тогда
		СКДПредварительныйРасчет = ПолучитьСКДПредварительныйИтоговыйРасчет(ИсточникиДанных, ПоляИсточников, СтруктураКолонокТаблицыДанных, 
																			ЕстьХранимыеПравила, ЕстьПривилегированныйРасчет);
	...
	Поле = ФинансоваяОтчетностьСервер.НовоеВычисляемоеПоле(СКДИтоговыеДанные, "СтатьяБюджетов", 
					"ВЫБОР КОГДА НЕ ЗначениеЗаполнено(СтатьяБюджетов) ТОГДА Неопределено ИНАЧЕ СтатьяБюджетов КОНЕЦ");
...
Функция ПолучитьДеревоДляВыводаОтчета(ВидБюджета, СтруктураЯчеек, ПараметрыЯчеек)
	...
	ОписаниеЗамера = Производительность.НачатьЗамерВремени("Бюджетирование.РасчетКэшаПолучениеДереваДляВыводаОтчета");
...
Функция НастройкиВводаЯчейкиСписка(СведенияОБюджетеДляРасчета, СтрокаСпискаВвода, Измерения, НакопленныйОтбор)
	...
	Для Каждого Измерение из МассивИзмерений Цикл
		...
		Выборка = БюджетнаяОтчетностьВыводСервер.ПолучитьВыборкуЯчеек(СтруктураОписанияОтчета.СтруктураЯчеек, Измерение, НакопленныйОтбор, Истина, , СписокСтрокСУчетомИерархии, , Истина);
...
Процедура ТипизироватьКолонкиАналитики(СКД, Описание)
	...
	Для Каждого Аналитика из Описание.ВидыАналитик Цикл
		...
		Разделитель = " " + "ИЛИ" + " ";
		УсловияПоТипамЗначений = Новый Массив;
		ТипыЗначений = Новый Массив;
		Для каждого Тип Из ТипКолонки.Типы() Цикл
			МетаданныеТипа = Метаданные.НайтиПоТипу(Тип);
			УсловияПоТипамЗначений.Добавить("ТипЗначения(" + ИмяПоля + ") = Тип(""" + МетаданныеТипа.ПолноеИмя() + """)");
			ТипыЗначений.Добавить("ЗНАЧЕНИЕ(" + МетаданныеТипа.ПолноеИмя() + ".ПустаяСсылка)");
		КонецЦикла;
		ТипыЗначений.Добавить("НЕОПРЕДЕЛЕНО");
		
		ПроверкаНаТипЗначения = "(" + СтрСоединить(УсловияПоТипамЗначений, Разделитель) + ")";
		СписокПустыхТипов     = СтрСоединить(ТипыЗначений, ", ");
		ВыражениеПоля = "ВЫБОР
		                |	КОГДА &ИмяПоля ЕСТЬ NULL
		                |		ТОГДА &ПустоеЗначение
		                |	КОГДА &ИмяПоля В (&ПустыеТипы)
		                |		ТОГДА &ПустоеЗначение
		                |	КОГДА &ПроверкаНаТипЗначения
		                |		ТОГДА &ИмяПоля
		                |	ИНАЧЕ &ПустоеЗначение
		                |КОНЕЦ";
		
		ВыражениеПоля = СтрЗаменить(ВыражениеПоля, "&ИмяПоля",                       ИмяПоля);
		ВыражениеПоля = СтрЗаменить(ВыражениеПоля, "&ПустоеЗначение",                ПустоеЗначение);
		ВыражениеПоля = СтрЗаменить(ВыражениеПоля, "&ПроверкаНаТипЗначения",         ПроверкаНаТипЗначения);
		ВыражениеПоля = СтрЗаменить(ВыражениеПоля, "&ПустыеТипы",                    СписокПустыхТипов);
		
		ФинОтчеты.НовоеВычисляемоеПоле(СКД, ИмяПоля, ВыражениеПоля, Строка(Аналитика),,ТипКолонки);
	КонецЦикла;
