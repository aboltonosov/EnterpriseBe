...
// 4D:ERP для Беларуси, Яна, 13.07.2017 17:28:01 
// Ошибки Зарплатного модуля, №15414
// Ошибка при открытии интерфейса редактирования подоходного налога, №15972
// {
Процедура ДополнитьФормуПанельюВычетов(Форма, ОписаниеПанелиВычеты = Неопределено, ДобавлятьЭлементыФормы = Истина, ДобавлятьРеквизитыФормы = Истина, ОтложенноеИзменение = Ложь) Экспорт
	...
	Если ГруппаФормыПанельВычеты = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найден элемент формы (группа ""%1"") для размещения элементов управления панелью Подоходный налог'"),
	...
	Если ВычетыЛичные <> Неопределено Тогда
		...
		ГруппаВычетЛичный       = Форма.Элементы.Найти(ГруппаФормыПанельВычеты.Имя + "ГруппаВычетЛичный");
		ГруппаВычетЛичныйВторой = Форма.Элементы.Найти(ГруппаФормыПанельВычеты.Имя + "ГруппаВычетЛичныйВторой");
		Если ГруппаВычетыЛичные <> Неопределено И ГруппаВычетыЛичные.ПодчиненныеЭлементы.Количество() = 2 Тогда
			...
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйКодВычета", Тип("ПолеФормы"), ГруппаВычетЛичный);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.Заголовок = НСтр("ru = 'Код'");
			Поле.Ширина = 5;
			Поле.РастягиватьПоГоризонтали = Ложь;
			...
			Поле.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);
			МассивПараметровВыбора = Новый Массив;
			МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", Справочники.ВидыВычетовНДФЛ.Код600));
			Поле.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
			
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичный", Тип("ПолеФормы"), ГруппаВычетЛичный);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			...
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйКЗачетуВозвратуКодВычета", Тип("ПолеФормы"), ГруппаВычетЛичный);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.Заголовок = НСтр("ru = 'Код'");
			Поле.Ширина = 5;
			Поле.РастягиватьПоГоризонтали = Ложь;
			...
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйКЗачетуВозврату", Тип("ПолеФормы"), ГруппаВычетЛичный);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
			Поле.Ширина = 8;
			Поле.РастягиватьПоГоризонтали = Ложь;
			...
			// 4D:ERP для Беларуси, Яна, 07.09.2017 15:20:12 
			// Локализация документа "Увольнение ", №15180 
			// {
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКодВычета", Тип("ПолеФормы"), ГруппаВычетЛичныйВторой);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.Заголовок = НСтр("ru = 'Код'");
			Поле.Ширина = 5;
			Поле.РастягиватьПоГоризонтали = Ложь;
			Поле.КнопкаОткрытия = Ложь;
			Поле.ПутьКДанным = ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКодВычета";
			Поле.УстановитьДействие("ПриИзменении", "Подключаемый_ВычетыЛичныеПриИзменении");
			Поле.БыстрыйВыбор = Истина;
			Поле.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);
			МассивПараметровВыбора = Новый Массив;
			МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", Справочники.ВидыВычетовНДФЛ.Код620));
			Поле.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметровВыбора);
			
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторой", Тип("ПолеФормы"), ГруппаВычетЛичныйВторой);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
			Поле.Ширина = 8;
			Поле.РастягиватьПоГоризонтали = Ложь;
			Поле.КнопкаВыбора = Ложь;
			Поле.ПутьКДанным = ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторой";
			Поле.УстановитьДействие("ПриИзменении", "Подключаемый_ВычетыЛичныеПриИзменении");
			
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКЗачетуВозвратуКодВычета", Тип("ПолеФормы"), ГруппаВычетЛичныйВторой);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.Заголовок = НСтр("ru = 'Код'");
			Поле.Ширина = 5;
			Поле.РастягиватьПоГоризонтали = Ложь;
			Поле.КнопкаОткрытия = Ложь;
			Поле.ПутьКДанным = ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКЗачетуВозвратуКодВычета";
			Поле.УстановитьДействие("ПриИзменении", "Подключаемый_ВычетыЛичныеПриИзменении");
			Поле.БыстрыйВыбор = Истина;
			Поле.СвязиПараметровВыбора = Новый ФиксированныйМассив(СвязиПараметровВыбора);
			МассивПараметровВыбора = Новый Массив;
			...
			Поле = Форма.Элементы.Добавить(ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКЗачетуВозврату", Тип("ПолеФормы"), ГруппаВычетЛичныйВторой);
			Поле.Вид = ВидПоляФормы.ПолеВвода;
			Поле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
			Поле.Ширина = 8;
			Поле.РастягиватьПоГоризонтали = Ложь;
			Поле.КнопкаВыбора = Ложь;
			Поле.ПутьКДанным = ГруппаФормыПанельВычеты.Имя + "ПримененныйВычетЛичныйВторойКЗачетуВозврату";
			Поле.УстановитьДействие("ПриИзменении", "Подключаемый_ВычетыЛичныеПриИзменении");
			// }
			// 4D
...
Процедура ДополнитьМассивРеквизитовПанелиВычетов(Форма, МассивДобавляемыхРеквизитов, МассивИменРеквизитовФормы, ОписаниеПанелиВычеты = Неопределено) Экспорт
	...
	Если ВычетыЛичные <> Неопределено Тогда
		...
		// 4D:ERP для Беларуси, Петр, 27.09.2017 10:58:33 
		// Ошибка при открытии интерфейса редактирования подоходного налога, №15972
		// {
		Реквизит = Новый РеквизитФормы(ИмяГруппыФормыПанелиВычеты + "ПримененныйВычетЛичныйВторой", 
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(12, 2)),
			,
			,
			Истина);
		...
		Реквизит = Новый РеквизитФормы(ИмяГруппыФормыПанелиВычеты + "ПримененныйВычетЛичныйВторойКодВычета", 
			Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"),
			,
			,
			Истина);
		...
		Реквизит = Новый РеквизитФормы(ИмяГруппыФормыПанелиВычеты + "ПримененныйВычетЛичныйВторойКЗачетуВозврату", 
			Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(12, 2)),
			,
			,
			Истина);
		...
		Реквизит = Новый РеквизитФормы(ИмяГруппыФормыПанелиВычеты + "ПримененныйВычетЛичныйВторойКЗачетуВозвратуКодВычета", 
			Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"),
			,
			,
			Истина);
