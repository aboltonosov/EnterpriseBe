...
Процедура СоздатьВТДанныеУчетаСтажаПФР(МенеджерВременныхТаблиц, ПараметрыОтбора, ИмяВТРезультат = "ВТДанныеУчетаСтажаПФР") Экспорт
	...
	ПерсонифицированныйУчет.СоздатьВТИнтервалыПоТаблицеДат(
		...
		"МЕСЯЦ",
	// 4D:ERP для Беларуси, Яна, 08.09.2017 14:27:35 
	// Локализация документов зарплатного модуля, №15480
	// {
		"ФизическоеЛицо, ЗастрахованФСЗН, ГоловнаяОрганизация",
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, Яна, 08.09.2017 14:28:42 
	// Локализация документов зарплатного модуля, №15480
	// {
	|	ДанныеОПравахНаСтрахование.ЗастрахованФСЗН,
	...
	// *Обновление*, Братухин М.В., 2017.10.23 (
	//|	ЗначениеПериодическихПараметровСтажа.ЗастрахованПФР,
	|	ЗначениеПериодическихПараметровСтажа.ЗастрахованФСЗН,
	// *Обновление*, Братухин М.В., 2017.10.23 )
...
Процедура СкорректироватьПересекающиесяИнтервалыВторичногоРегистра(ПараметрыОбновления = Неопределено) Экспорт 
	...
	Запрос.Текст = 
		...
		|			И (ПериодыСтажаПФР.Начало МЕЖДУ ПериодыСтажаПФРПересекающие.Начало И ПериодыСтажаПФРПересекающие.Окончание)
		|ГДЕ
