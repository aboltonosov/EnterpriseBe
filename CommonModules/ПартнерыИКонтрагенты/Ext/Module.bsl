...
Функция ИННКППУжеИспользуетсяВИнформационнойБазе(ИНН,КПП,ИсключаяСсылку = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 22.08.2017 13:57:04 
	// Уникальность УНП, №15601 
	// {
	Возврат Неопределено;
	// }
	// 4D
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер) Экспорт
	...
	Если Объект.ОбособленноеПодразделение И ЗначениеЗаполнено(Объект.ГоловнойКонтрагент) Тогда
		...
		Иначе
			...
			ИначеЕсли ЗначениеЗаполнено(СвойстваГоловногоКонтрагента.ИНН) И СвойстваГоловногоКонтрагента.ИНН <> Объект.ИНН Тогда
				...
				// {
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'УНП контрагента не равен указанному в головном подразделении'"),
					СвойстваГоловногоКонтрагента.Наименование);
		...
		Если ВыборкаКонтрагент <> Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			                                СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                                  НСтр("ru='Данный УНП уже указан для другого контрагента, ответственный - %1.'"),
...
Процедура ДанныеКонтрагентаПоИННФоновоеЗадание(ПараметрыФормирования, АдресХранилища) Экспорт
	...
	Если ПараметрыФормирования.ЭтоЮридическоеЛицо Тогда
		РезультатФормирования = ДанныеЕдиныхГосРеестров.РеквизитыЮридическогоЛицаПоИНН(ПараметрыФормирования.ИНН);
	Иначе
		РезультатФормирования = ДанныеЕдиныхГосРеестров.РеквизитыПредпринимателяПоИНН(ПараметрыФормирования.ИНН);
	КонецЕсли;
...
Функция НастройкиПодключенияКСервисуИППЗаданы() Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 31.03.2017 15:46:22 
