...
Функция РасчетЗаймовСотрудникам(Организация, ДатаОперации, Регистратор, ДоговорыЗайма = Неопределено, Сотрудники = Неопределено, Подразделение = Неопределено, ИсключатьУволенныхСотрудников = Ложь) Экспорт
	...
	КадровыйУчет.СоздатьВТОсновныеСотрудникиФизическихЛицВПериодах(
		МенеджерВременныхТаблиц, Истина, Организация, "ВТДоговорыПериоды", "ВТРабочиеМестаСотрудников");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидПлатежей = Перечисления.ВидыПлатежейПогашенияЗаймаСотруднику.АннуитетныеПлатежи Тогда
			...
			НоваяСтрока.РазмерПлатежа = Мин(Выборка.РазмерПлатежа / Выборка.ДнейВМесяце * Выборка.Дней, СуммаПогашения);
		...
		ПроцентПогашения = ПроцентПогашения - НоваяСтрока.ПогашениеПроцентов;
