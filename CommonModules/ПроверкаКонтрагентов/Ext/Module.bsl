...
Функция ТестовыйРежимРаботыСервисаЗавершился() Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 21.04.2017 17:18:07 
...
Процедура УстановитьНадписиВПанелиОтчетаПриСозданииНаСервере(Форма) Экспорт
	...
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Проверка контрагентов по данным МНС выполнена успешно'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Есть некорректные контрагенты.
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриНекорректныхКонтрагентах");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Обнаружены недействующие контрагенты по данным МНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Выполняется проверка
	ЭлементФормы = Форма.Элементы.Найти("ТекстПриПроверкеВПроцессеВыполнения");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Выполняется проверка контрагентов согласно данным МНС'"), СсылкаНаИнструкцию);
	КонецЕсли;
	
	// Нет доступа к сервису
	ЭлементФормы = Форма.Элементы.Найти("ТекстНетДоступаКСервису");
	Если ЭлементФормы <> Неопределено Тогда
		ЭлементФормы.Заголовок = Новый ФорматированнаяСтрока(НСтр("ru = 'Не удалось произвести проверку контрагентов: сервис МНС временно недоступен'"), СсылкаНаИнструкцию);
	КонецЕсли;
	// }
	// 4D
...
Функция ЕстьДоступКВебСервисуФНС() Экспорт
	...
	// 4D:ERP для Беларуси, Дмитрий, 26.04.2017 11:15:20 
	// Проверка контрагентов, №14822 
	// {
	Возврат Истина;
	// }
...
Функция ПараметрыРаботыСервиса()
	...
	// 4D:ERP для Беларуси, Дмитрий, 26.04.2017 11:13:54 
	// Проверка контрагентов, №14822 
	// {
	АдресСервиса = "portal.nalog.gov.by/grp/";
...
Функция ПроксиСервиса(Таймаут = 120)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка контрагентов.Ошибка доступа к веб-сервису'", КодОсновногоЯзыка),
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор)
	...
	// 4D:ERP для Беларуси, Дмитрий, 26.04.2017 11:14:33 
	// Проверка контрагентов, №14822 
	// {
	...
	Для Каждого ДанныеКонтрагента ИЗ ДанныеКонтрагентовДляПроверкиСервисом Цикл
		
		ОбъектXDTO     = Неопределено;
		
		Попытка
			
			HTTPСоединение = Новый HTTPСоединение(Параметры.АдресСервиса);
			HTTPЗапрос = Новый HTTPЗапрос("getData?unp=" + ДанныеКонтрагента.ИНН + "&charset=UTF-8"); 
			HTTPОтвет = HTTPСоединение.Получить(HTTPЗапрос);
			
			Если HTTPОтвет.КодСостояния <> 200 Тогда
				ДанныеКонтрагента.Состояние	= Перечисления.СостоянияСуществованияКонтрагента.КонтрагентОтсутствуетВБазеФНС;
				Продолжить;
			КонецЕсли;	
			...
			Если ДанныеКонтрагента.Состояние <> Перечисления.СостоянияСуществованияКонтрагента.КонтрагентСодержитОшибкиВДанных Тогда
				Если ОбъектXDTO.ROW.CKODSOST = "L" Тогда
					ДанныеКонтрагента.Состояние	= Перечисления.СостоянияСуществованияКонтрагента.НеДействуетИлиИзмененКПП;
				Иначе	
					ДанныеКонтрагента.Состояние	= Перечисления.СостоянияСуществованияКонтрагента.КонтрагентЕстьВБазеФНС;
				КонецЕсли;
			КонецЕсли;
...
Функция ОтветСервисаФНС(Прокси, WSЗапрос, ПроверкаДоступностиСервиса = Ложь)
	...
	Исключение
		...
		Если ПроверкаДоступностиСервиса Тогда
			ШаблонОшибки = НСтр("ru = 'Веб-сервис %1 недоступен по причине:'");
		Иначе
			ШаблонОшибки = НСтр("ru = 'Веб-сервис %1 не вернул результат проверки данных по причине:'")
		КонецЕсли;
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
			Прокси.ТочкаПодключения.Местоположение);
		
		ТекстОшибки = ТекстОшибки + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		
		// Страхуемся от ошибки "Timeout was reached".
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка контрагентов.NdsRequest2'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Предупреждение,
			,
			,
			ТекстОшибки);
...
Процедура ДобавитьКонтрагентаВЗапросКСервису(WSЗапрос, Прокси, ДанныеКонтрагента)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка контрагентов.Формирование данных для обращения к сервису ФНС'", КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Предупреждение,
			,
			,
			Комментарий);
