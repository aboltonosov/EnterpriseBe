...
//
// Параметры:
//  РаспоряженияСсылки	 - Массив	 - состоит из ДокументСсылка, ссылок на документы, по которым
...
Процедура ОбновитьСписокРаспоряженийНаДоставку(Форма, ОбновляемыеРаспоряжения = Неопределено) Экспорт
	...
	ОтгрузкаПоОрдерам 			= СкладыСервер.ЕстьОрдерныйНаОтгрузкуСклад(Склад);
	...
	Если Форма.ВидДоставки = Перечисления.ВидыДоставки.СоСклада Тогда
		...
		ТекстЗапроса = ?(ОтгрузкаПоОрдерам,
