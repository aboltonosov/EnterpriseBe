...
// Функция возвращает регистрировалась начисленная зарплата документом или нет
//
// Параметры:
//		ДокументОснование - Ссылка - Документ, по которому требуется проверить регистрацию начисленной зарплаты
//
// Возвращаемое значение:
//		Булево - Истина, если зарплата регистрировалась документов, иначе Ложь
//
Функция РегистрироваласьНачисленнаяЗарплатаДокументом(ДокументОснование) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ЗарплатаКВыплате.Регистратор = &ДокументОснование";
...
// Функция возвращает производилась выплата зарплаты по документу или нет
//
// Параметры:
//		ДокументОснование - Ссылка - Документ, по которому требуется проверить выплату зарплаты
//
// Возвращаемое значение:
//		Булево - Истина, если зарплата выплачивалась по документу, иначе Ложь
//
Функция ПроизводиласьВыплатаЗарплатыПоДокументу(ДокументОснование) Экспорт
...
// Функция возвращает производилась выплата зарплаты по периоду взаиморасчетов или нет
//
// Параметры:
//		ПериодВзаиморасчетов - Дата - Период, за который требуется проверить выплату зарплаты
//
// Возвращаемое значение:
//		Булево - Истина, если зарплата выплачивалась за период взаиморасчетов, иначе Ложь
//
Функция ПроизводиласьВыплатаЗарплатыПоПериодуВзаиморасчетов(ПериодВзаиморасчетов) Экспорт
...
//
Процедура ЗарегистрироватьОплатуВедомостейВУчетеНДФЛ(ПлатежныйДокумент, Организация, Ведомости, ФизическиеЛица = Неопределено, ДатаОперации = Неопределено, Отказ = Ложь) Экспорт
	...
	ЗарегистрироватьОплатуВедомостейВУчетеНДФЛСоздатьВТНалогУдержанный(МенеджерВременныхТаблиц, Организация, Ведомости, ФизическиеЛица);
	...
	ЗарегистрироватьУдержанныйНалогПоВременнымТаблицам(ПлатежныйДокумент, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц);
		
КонецПроцедуры

Процедура ЗарегистрироватьОплатуВедомостейВУчетеНДФЛСоздатьВТНалогУдержанный(МенеджерВременныхТаблиц, Организация, Ведомости, ФизическиеЛица)
	...
	|ИЗ
	|	#НФДЛВедомостей КАК НФДЛВедомостей";
	ТекстЗапросаВТНалогУдержанный = СтрЗаменить(ТекстЗапросаВТНалогУдержанный, "#НФДЛВедомостей", ЗапросНФДЛВедомостей);
	
	Запрос.Текст = ТекстЗапросаВТНалогУдержанный;
	Запрос.Выполнить();
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ВзаиморасчетыССотрудникамиРасширенный.ДобавитьСпособВыплатыАванс";
...
Процедура СпособыВыплатыЗарплатыНачальноеЗаполнение() Экспорт
	...
	СтруктураПоиска = Новый Структура("ХарактерВыплаты, ВидДокументаОснования");
...
Процедура ДобавитьСпособВыплатыАванс() Экспорт
	
	// Выбираем авансовые способы выплаты, не являющиеся Авансом
	ОписаниеЭлементов = ОписаниеСпособовВыплатыЗарплатыПоУмолчанию();
	ОписаниеАванса = ОписаниеЭлементов.Найти(Перечисления.ХарактерВыплатыЗарплаты.Аванс, "ХарактерВыплаты"); 
...
Функция СпособыВыплатыЗарплатыДобавитьПоОписаниямКОбработке(Описания)
	...
	|		ПО Поставляемые.ВидДокументаОснования = СпособыВыплатыЗарплаты.ВидДокументаОснования
	|			И Поставляемые.ХарактерВыплаты = СпособыВыплатыЗарплаты.ХарактерВыплаты
	|ГДЕ
...
Функция ОписаниеСпособовВыплатыЗарплатыПоУмолчанию() Экспорт
	
	// 4D:ERP для Беларуси, Яна, 08.09.2017 12:58:11 
	// Локализация документов зарплатного модуля, №15480
	// {	
	
	НаименованияМежрасчетныхВыплат = Новый Соответствие;
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.БольничныйЛист,					НСтр("ru = 'Больничные листы'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ВозвратНДФЛ,						НСтр("ru = 'Возврат подоходного налога'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ЕдиновременноеПособиеЗаСчетФСС,	НСтр("ru = 'Единовременные пособия за счет ФСЗН'"));
	// }
	// 4D
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.Командировка,						НСтр("ru = 'Командировки'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.КомпенсацияЗаЗадержкуЗарплаты,	НСтр("ru = 'Компенсации за задержку зарплаты'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.МатериальнаяПомощь,				НСтр("ru = 'Материальная помощь'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.НачислениеЗарплаты,				НСтр("ru = 'Доначисление'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ОплатаДнейУходаЗаДетьмиИнвалидами,НСтр("ru = 'Оплата дней ухода за детьми-инвалидами'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ОплатаПоСреднемуЗаработку,		НСтр("ru = 'Отсутствие с сохранением оплаты'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.Отпуск,							НСтр("ru = 'Отпуска'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ОтпускПоУходуЗаРебенком,			НСтр("ru = 'Отпуска по уходу за ребенком'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.Премия,							НСтр("ru = 'Премии'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.ПростойСотрудников,				НСтр("ru = 'Простои'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.РазовоеНачисление,				НСтр("ru = 'Разовые начисления'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.Увольнение,						НСтр("ru = 'Увольнения'"));
	НаименованияМежрасчетныхВыплат.Вставить(Перечисления.ВидыДокументовМежрасчетныхНачислений.НачислениеПоДоговорам,			НСтр("ru = 'Начисления по договорам'"));

	ОписаниеЭлементов = СпособыВыплатыЗарплатыНовыеОписания();
	
	ОписаниеЭлемента = ОписаниеЭлементов.Добавить();
	ОписаниеЭлемента.ИмяПредопределенныхДанных		= "Зарплата";
	ОписаниеЭлемента.Наименование					= НСтр("ru = 'Зарплата за месяц'");
	ОписаниеЭлемента.ХарактерВыплаты				= Перечисления.ХарактерВыплатыЗарплаты.Зарплата;
	
	ОписаниеЭлемента = ОписаниеЭлементов.Добавить();
	ОписаниеЭлемента.Наименование					= НСтр("ru = 'Аванс'");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда 
		...
		Модуль.СпособыВыплатыЗарплатыДобавитьОписанияВыплатВоеннослужащим(НаименованияМежрасчетныхВыплат, ВидыДокументовСОкончательнымРасчетом);
	КонецЕсли;
		
	Для Каждого ВидДокументаОснования Из Перечисления.ВидыДокументовМежрасчетныхНачислений.ДоступныеПоМетаданным() Цикл
		...
		Если ВидыДокументовСОкончательнымРасчетом.Найти(ВидДокументаОснования) <> Неопределено Тогда
			ОписаниеЭлемента.ХарактерВыплаты= Перечисления.ХарактерВыплатыЗарплаты.Зарплата;	
		Иначе	
...
Функция СпособыВыплатыЗарплатыКлюч()
	Возврат Новый Структура("ХарактерВыплаты, ВидДокументаОснования")
КонецФункции
...
Функция НастройкиВзаиморасчетовССотрудниками() Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	НастройкиВзаиморасчетов.ПроверятьЗаполнениеФинансированияВВедомостях
		|ИЗ
...
//
Процедура ЗарегистрироватьУдержанныйНалогПоВременнымТаблицам(Регистратор, Отказ = Ложь, Организация, Дата, МенеджерВременныхТаблиц) Экспорт
	
	УчетНДФЛ.ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор.Ссылка);	
	УчетНДФЛ.СформироватьУдержанныйНалогПоВременнойТаблице(Регистратор.Движения, Отказ, Организация, Дата, МенеджерВременныхТаблиц, , Истина);
...
Процедура СоздатьВТЗарплатаКВыплате(МенеджерВременныхТаблиц, ТолькоРазрешенные, Параметры, ИмяВТСотрудники) Экспорт
	...
	Если Параметры.ХарактерВыплаты = Перечисления.ХарактерВыплатыЗарплаты.Межрасчет Тогда 
...
Функция ВедомостьНДФЛПоТаблицеЗарплат(Ведомость, ТаблицаЗарплат) Экспорт
	...
	// Для межрасчетов оставляем только налоги с текущих выплат  
	Если РеквизитыСпособаВыплаты.ХарактерВыплаты <> Перечисления.ХарактерВыплатыЗарплаты.Зарплата Тогда
		
		КолонкиОтбораНДФЛ = "ФизическоеЛицо, ДокументОснование";
		ИндексОтбораНДФЛ = ТаблицаЗарплат.Индексы.Добавить(КолонкиОтбораНДФЛ);
		ПараметрыОтбораНДФЛ = Новый Структура(КолонкиОтбораНДФЛ); 
		
		УдаляемыеСтроки = Новый Массив;
		Для Каждого СтрокаНДФЛ Из НДФЛ Цикл
			ЗаполнитьЗначенияСвойств(ПараметрыОтбораНДФЛ, СтрокаНДФЛ); 
			Если ТаблицаЗарплат.НайтиСтроки(ПараметрыОтбораНДФЛ).Количество() = 0 Тогда
				УдаляемыеСтроки.Добавить(СтрокаНДФЛ);
			КонецЕсли	
		КонецЦикла;	
		
		Для Каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
			НДФЛ.Удалить(УдаляемаяСтрока)
		КонецЦикла;	
		
		ТаблицаЗарплат.Индексы.Удалить(ИндексОтбораНДФЛ);
...
Процедура ВедомостьЗарегистрироватьУдержанныеНалоги(Ведомость, Отказ = Ложь)
	...
	|ГДЕ
	|	ВедомостьНДФЛ.Ссылка = &Ссылка";
	Запрос.Текст = СтрЗаменить(ТекстЗапроса, "#ВедомостьНДФЛ", Ведомость.Метаданные().ПолноеИмя() + ".НДФЛ");
	Запрос.Выполнить();
	
	ЗарегистрироватьУдержанныйНалогПоВременнымТаблицам(Ведомость, Отказ, Ведомость.Организация, Ведомость.Дата, МенеджерВременныхТаблиц);

КонецПроцедуры

Процедура ВедомостьЗарегистрироватьПеречислениеНДФЛ(Ведомость, Отказ = Ложь)
	
	УчетНДФЛРасширенный.ЗарегистрироватьНДФЛПеречисленныйПоПлатежномуДокументу(Ведомость.Движения, Отказ, Ведомость.Организация, Ведомость.Дата, Ведомость.ПеречислениеНДФЛРеквизиты);
...
Процедура ВедомостьВКассуДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 21.12.2017 11:57:48 
	// Редактировать печатные формы из док-та "Ведомость в кассу", № 17225
	// {	
