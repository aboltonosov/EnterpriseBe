...
Функция ПолучитьИнформациюОВидеРасчета(ВидРасчета) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 21.06.2017 11:04:47 
	...
	Если ТипЗнч(ВидРасчета) = Тип("ПланВидовРасчетаСсылка.Начисления") Тогда
		...
		// 4D:ERP для Беларуси, Петр, 28.08.2017 10:08:20 
		// Локализация документа "Премии", №15462
		// Ошибка в документе "Назначение планового начисления", № 16493
		// {
		...
		|Рассчитывается,
		|ЯвляетсяЛьготой,		
		// 4D:ERP для Беларуси, Яна, 07.08.2017 15:12:30 
		// Ошибка открытия документа "Отпуск", №15668
		// Ошибка при изменении реквизита "Причина нетрудоспособности", №15210
		// Создание новой позиции штатного распиания, №15711 
		// {
		|УдалитьЛьготаДоступнаДляВыбораСотрудником,
		|ФОТНеРедактируется,
		|ВключатьВФОТ,
		// }
		// 4D
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "#ИмяТаблицы", ВидРасчета.Метаданные().ПолноеИмя());
	...
	ЗначенияРеквизитов.Вставить("ЭтоНачисление", ЭтоНачисление);
	
	// Заполняем по данным результата запроса.
	ЗаполнитьЗначенияСвойств(ЗначенияРеквизитов, Выборка);
	
	// Добавляем значение кода вычета НДФЛ для начислений.
	Если ЭтоНачисление Тогда 
		КодВычетаНДФЛ = УчетНДФЛ.КодВычетаПоКодуДоходаНДФЛ(ЗначенияРеквизитов.КодДоходаНДФЛ);
		ЗначенияРеквизитов.Вставить("КодВычетаНДФЛ", КодВычетаНДФЛ);
	КонецЕсли;
	
	// Получим также и сведения о самих показателях.
	...
	Если Выборка.Показатели <> Неопределено Тогда
		...
		Пока ВыборкаПоказателей.Следующий() Цикл
			Строка = Новый Структура(ИменаПолей);
			ЗаполнитьЗначенияСвойств(Строка, ВыборкаПоказателей);
			МассивОписаний.Добавить(Строка);
	...
	// Дополняем описание в массиве описаний с учетом полученных сведений о показателях.
...
// Возращает значение настройки "ОплачиватьДлительныеКомандировкиПомесячно" расчета зарплаты
// 
