...
Функция ТекстПорядокОтраженияДоходов()
	Возврат "
		...
		|	(ВЫБОР
		|
		// 4D:ERP для Беларуси, ВладимирР, 17.08.2016 16:47:09 
		// Отражение в регл.учете дохода по сч.90.07, №12315
		// {
		|		КОГДА ПорядокУчета.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Продажи_ПрочиеДоходы) ТОГДА
		|			ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Продажи_ПрочиеДоходы)
		// }
		// 4D
		|
...
Функция ТекстСчетаОтраженияНоменклатурыПоУмолчанию()
	...
	|
	// 4D:ERP для Беларуси, Екатерина, 01.10.2015 12:31:47 
	// Локализация плана счетов, №8969
	// {
	|				ИЛИ ПорядокУчета.СчетУчетаНаСкладе = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПриобретениеОбъектовОсновныхСредств) 
	// }
	// 4D
	|
...
Функция ТекстСчетаОтраженияДоходовПоУмолчанию()
	Возврат "
		...
		|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НДСПриДоходах) КАК ВидСчетаРеглУчета,
		|	(ВЫБОР
		// 4D:ERP для Беларуси, ВладимирР, 22.08.2017 12:47:21 
		// Ошибки ERP, №15567
		// {
		|		КОГДА ПорядокУчета.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеДоходы) ИЛИ ПорядокУчета.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Операционные_НДС) ТОГДА
		|			ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Операционные_НДС)
		// }
		// 4D
		|		КОГДА ПорядокУчета.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ВыручкаНеЕНВД) ТОГДА
		|			ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Продажи_НДС)
