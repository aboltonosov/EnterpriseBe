...
Процедура СоздатьВТДанныеПроизводственногоКалендаряПоГрафикам(МенеджерВТ) Экспорт
	...
	Запрос.Текст = 
	// 4D:ERP для Беларуси, Яна, 06.07.2017 14:21:48 
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТИзмеренияДатыДляТаблицыРегистра
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТОбщиеГрафикиСотрудниковСИндивидуальнымиГрафиками
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТПериодыПроизводственныхКалендарей
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТДанныеПроизводственныхКалендарей";
...
Процедура СоздатьВТВремяПоГрафикамСотрудников(ТаблицаСотрудников, МенеджерВременныхТаблиц, УчитыватьДанныеПослеУвольнения = Ложь) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 01.09.2017 9:52:18 
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	|	СУММА(ДанныеГрафиков.НормаДней) КАК НормаДнейПолная,
	|	СУММА(ДанныеГрафиков.НормаЧасов) КАК НормаЧасовПолная,	
	// }
	// 4D	
	...
	// Ошибка при вводе данных для расчета среднего заработка, №15832
	// {
	|		ЕСТЬNULL(ДанныеГрафиковСотрудников.ОсновноеЗначениеНорма, 0) КАК НормаДнейПолная,
	|		ЕСТЬNULL(ДанныеГрафиковСотрудников.ДополнительноеЗначениеНорма, 0) КАК НормаЧасовПолная,
	// }
	// 4D		
...
Процедура ЗаписатьДанныеГрафика(ГрафикРаботы, ДанныеГрафика, НомерТекущегоГода, СохранитьСреднемесячныеНормыВремени = Истина) Экспорт
	...
	ПоместитьТекущиеДанныеОбщегоГрафикаВВТПередЗаписью(МенеджерВременныхТаблиц, ГрафикРаботы, Дата(НомерТекущегоГода, 1, 1));
	...
	// получим виды времени которые включаются в норму
	// и для укорения поиска поместим их в соответствие
	ВидыВремениВключаемыеВНорму = ВидыВремениВключаемыеВНорму();
	ВидыВремениВключаемыеВНормуСтруктураПоиска = Новый Соответствие;
	
	Для Каждого ВидВремениДляПолученияНормы Из ВидыВремениВключаемыеВНорму Цикл
		ВидыВремениВключаемыеВНормуСтруктураПоиска.Вставить(ВидВремениДляПолученияНормы, Истина);
	КонецЦикла;	
		...
		Пока Выборка.СледующийПоЗначениюПоля("Дата") Цикл
			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.Часы <> 0 Тогда
					...
					Если ВидыВремениВключаемыеВНормуСтруктураПоиска.Получить(Выборка.ОсновноеВремя) = Истина Тогда
						РабочееВремяДни = 1;
				...
				Если Выборка.Часы > 0
					И ВидыВремениВключаемыеВНормуСтруктураПоиска.Получить(Выборка.ОсновноеВремя) = Истина Тогда
	...
	Если СохранитьСреднемесячныеНормыВремени Тогда
		...
		Для каждого СтрокаСреднемесячныеНормыВремени Из СреднемесячныеНормыВремени Цикл
			...
			НаборСреднемесячныеНормы.Отбор.Год.Установить(СтрокаСреднемесячныеНормыВремени.Год);
			НаборСреднемесячныеНормы.Отбор.ГрафикРаботыСотрудников.Установить(СтрокаСреднемесячныеНормыВремени.ГрафикРаботыСотрудников);
			...
			Если НЕ СтрокаНабораСреднемесячныеНормы.УстановленыВРучную Тогда
				...
				СтрокаНабораСреднемесячныеНормы.Год = СтрокаСреднемесячныеНормыВремени.Год;
				СтрокаНабораСреднемесячныеНормы.ГрафикРаботыСотрудников = СтрокаСреднемесячныеНормыВремени.ГрафикРаботыСотрудников;
				СтрокаНабораСреднемесячныеНормы.СреднемесячноеЧислоДней = СтрокаСреднемесячныеНормыВремени.СреднемесячноеЧислоДней / 12;
				СтрокаНабораСреднемесячныеНормы.СреднемесячноеЧислоЧасов = СтрокаСреднемесячныеНормыВремени.СреднемесячноеЧислоЧасов / 12;
...
Процедура ПоместитьТекущиеДанныеОбщегоГрафикаВВТПередЗаписью(МенеджерВременныхТаблиц, ГрафикРаботы, Год)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ГрафикРаботы", ГрафикРаботы);
...
Процедура ОбработатьИзмененияОбщегоГрафикаПослеЗаписи(МенеджерВременныхТаблиц, ГрафикРаботы, Год)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИзменившиесяДанныеОбщихГрафиков.ГрафикРаботы,
...
Процедура ПроверитьСоответствиеФактическогоВремениПлановому(МенеджерВременныхТаблиц, ОписанияОшибокВводаВремени, ПериодРегистрации = Неопределено, Отказ = Ложь) Экспорт 
	...
	|			И (НЕ ФактическоеВремя.План И ПлановоеВремя.ДниПлан > 0)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыИспользованияРабочегоВремени КАК ОписаниеПлановыхВидовВремени
...
Функция ЗапросВТИсточникиПолученияДанныхУчетаВремени(ПараметрыФормированияДанных)
	...
	|	ИспользуемыеИсточникиДанныхНеотфильтрованные.ПолучатьРассчитанныеДанныеФакт
...
Функция ТекстУсловияЕстьНевытесненныеДанные()
	...
	Для Сч = 1 По 31 Цикл
		ТекстСуммированиеПолей = ТекстСуммированиеПолей + " + ИспользуемыеИсточникиДанныхНесгруппированные.УстановленДень" + Формат(Сч, "ЧГ=");
	КонецЦикла;	
	
	ТекстСуммированиеПолей = Сред(ТекстСуммированиеПолей, 3);
...
Функция ТекстУсловияПолученияВремениЗаДень()
	...
	Для Сч = 1 По 31 Цикл
		ТекстСуммированиеПолей = ТекстСуммированиеПолей + " + ИспользуемыеИсточникиДанныхНеотфильтрованные.УстановленДень" + Формат(Сч, "ЧГ=") + " * ВспомогательнаяТаблицаФильтрации.УстановленДень" +  Формат(Сч, "ЧГ=");
	КонецЦикла;	
...
Функция ТекстУсловияПолученияВремениЗаДеньПлан()
	...
	ТекстУсловия =  "(НЕ ИспользуемыеИсточникиДанныхНеотфильтрованные.ВыполнятьФильтрациюДанных И ИспользуемыеИсточникиДанныхНеотфильтрованные.ИспользоватьДанныеИсточникаПлан) Или " + ТекстСуммированиеПолей + " В (1, 3)";
...
Функция ТекстУсловияПолученияВремениЗаДеньФакт()
	...
	ТекстУсловия = "(НЕ ИспользуемыеИсточникиДанныхНеотфильтрованные.ВыполнятьФильтрациюДанных И ИспользуемыеИсточникиДанныхНеотфильтрованные.ИспользоватьДанныеИсточникаФакт) Или " + ТекстСуммированиеПолей + " >= 2";
...
// Возвращает объект (Обработка.МенеджерДанныхУчетаВремениСотрудников), реализующий набор методов для получения данных учета времени
// оптимально использовать, когда данные запрашиваются несколько раз. На вход передается описание набора данных,
...
// Возвращает объект (Обработка.МенеджерДанныхУчетаВремениСотрудников), реализующий набор методов для получения данных учета времени.
// оптимально использовать, когда данные запрашиваются один раз или заранее неизвестен набор данных, по которому будут выполянться запросы
// 
// Параметры:
//		МенеджерВременныхТаблиц - МенеджерВременныхТаблиц
//		ДатаАкутальности - дата, ограничивающая перио опредения источников данных учета времени. Обычно ПериодРегистрации документа-начислятеля 
