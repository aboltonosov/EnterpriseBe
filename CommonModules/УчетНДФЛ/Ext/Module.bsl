...
Функция РеестрПеречисленныхСуммНалога(РеквизитыПлатежа) Экспорт 
	...
	//*************************************************************************************
	// 4D:ERP для Беларуси, Екатерина, 06.10.2015 12:35:20 
	// Локализация ЗП, №10360
	// {
	...
	//*****************************************************************************************************
	// 4D:ERP для Беларуси, Екатерина, 06.10.2015 12:34:39 
	// Локализация ЗП, №10360
	// {
	|			И ДанныеДокументов.МесяцНалоговогоПериода = УплатаПоФизическимЛицам.МесяцНалоговогоПериода
	// }
	...
	//|					И УплатаПоФизическимЛицам.МесяцНалоговогоПериода >= ДАТАВРЕМЯ(2015, 1, 1, 0, 0, 0))
	//*****************************************************************************************************
	// 4D:ERP для Беларуси, Екатерина, 06.10.2015 12:33:39 
	// Локализация ЗП, №10360
	// {
	|			И ДанныеДокументов.МесяцНалоговогоПериода = УплатаПоФизическимЛицам.МесяцНалоговогоПериода
	// }
...
Процедура СформироватьДоходыНДФЛПоКодамДоходовИзТаблицыЗначений(Движения, Отказ, Организация, ДатаОперации, Начисления, Записывать = Ложь, ОкончательныйРасчет = Истина, Регистратор = Неопределено) Экспорт
	...
	Для Каждого СтрокаДанных Из НачисленияСРегистрациями Цикл
		...
		// 4D:ERP для Беларуси, АлексейЧ, 21.02.2018 13:02:49 
		// Локализовать документ "Начисление зарплаты и взносов", № 15928
		// {
		
		// В РБ расчет подоходного производится в месяц начисления, 
		// не зависимо от того когда производится реальная выплата дохода. 
		// Таким образом, для всех документов (мат. помощь, премия, разовое начисление, отпуск, больничный и т.д.)
		// ПН должен быть рассчитан в месяц начисления. Это значит, что в регистр "Учет доходов для исчисления 
		// подоходного налога" месяц налогового периода должен писаться в соответствии с месяцем начисления ЗП.
		
		//НоваяСтрока.МесяцНалоговогоПериода = НачалоМесяца(НоваяСтрока.ДатаПолученияДохода);
		
		НоваяСтрока.МесяцНалоговогоПериода = НачалоМесяца(НоваяСтрока.Период);
		// }
		// 4D
	КонецЦикла;
...
Процедура СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперации, ДатаВыплаты, МенеджерВременныхТаблиц, МесяцНачисления = Неопределено, Записывать = Ложь, ОкончательныйРасчет = Истина, ИмяВТНачисления = "ВТНачисления", Регистратор = Неопределено, МенеджерКадровогоУчета = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, АлексейЧ, 14.02.2018 17:32:16 
	// Локализовать документ "Начисление зарплаты и взносов", №  15928
	// По законодательству РБ месяцем налогового периода считается месяц начисления дохода,
	// а не месяц получения дохода
	// {
	...
	//|	НАЧАЛОПЕРИОДА(Начисления.Период, МЕСЯЦ) КАК МесяцНалоговогоПериода,
	
	|	НАЧАЛОПЕРИОДА(&КонецМесяцаНачисления, МЕСЯЦ) КАК МесяцНалоговогоПериода,
...
Процедура СформироватьНалогиВычеты(Движения, Отказ, Организация, ДатаОперации, ДанныеДокумента, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	КонецЕсли;
	// 4D:ERP для Беларуси, Яна, 07.09.2017 15:04:28 
	// Локализация документа "Увольнение ", №15180 
	// {
	Если КолонкиДанных.Найти("ПримененныйВычетЛичныйВторой") <> Неопределено Тогда
		СформироватьСтандартныеВычетыПоТаблицеЗначений(Движения, Отказ, Организация, ДатаОперации, ЛичныйВычетИзДанныхДокумента(ДанныеСРегистрациями, "ПримененныйВычетЛичныйВторой", "ПримененныйВычетЛичныйВторойКодВычета"), , РегистрацияВНалоговомОргане, Ложь, ОкончательныйРасчет);
	КонецЕсли;
	// }
	// 4D
...
Функция РассчитатьНалогПоОсновнойСтавке(Регистратор, Организация, МесяцРасчета, МенеджерВременныхТаблиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, ИсключатьДоходыРегистратора = Истина, ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, АлексейЧ, 06.02.2018 10:09:42 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
	...
	Если ЭтоРасчетПоДоходамТекущегоМесяца Тогда
		...
		// 4D:ERP для Беларуси, АлексейЧ, 14.02.2018 14:18:54 
		// Локализовать документ "Начисление зарплаты и взносов", № 15928
		// {
		//|			И Доходы.КодДохода.СтавкаНалогообложенияРезидента = ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13)
		|			И Доходы.КодДохода.СтавкаНалогообложенияРезидента В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13), ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка09))
		// }
		// 4D
	...
	// В РБ все работающие уплачивают подоходный налог на общих основаниях
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СтатусНалогоплательщика.Статус, ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент)) В (ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Нерезидент), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.ИностранныйГражданин), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент) )
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|	КОНЕЦ КАК Резидент,
	// В РБ Иностранцы уплачивают подоходный нало по общим ставкам
	|	ЛОЖЬ КАК ПрименяетсяСтавкаПункта1Статьи224НК
	...
	|	Обороты.КодДохода.СтавкаНалогообложенияРезидента В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13), ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка09) )
	...
	|	И ( СведенияОДоходахНДФЛ.КодДохода.СтавкаНалогообложенияРезидента В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13), ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка09) ) )
	...
	|		КОГДА Обороты.СтавкаНалогообложенияРезидента В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13),ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка09))
	...
	|				И СтавкаНДФЛПоСтавкеРезидента.Статус В (ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.ИностранныйГражданин), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Нерезидент))
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|						КОГДА СтавкаНДФЛПоСтавкеРезидента.Статус В (ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Резидент), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.ИностранныйГражданин), ЗНАЧЕНИЕ(Справочник.СтатусыНалогоплательщиковПоНДФЛ.Нерезидент))
	...
	|ГДЕ
	|	Доходы.ДатаПолученияДохода ЕСТЬ NULL 
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	СУММА(ВЫБОР
	|			КОГДА ВычетыНаДетей.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код610)
	|				ТОГДА ЕСТЬNULL(ВычетыНаДетей.КоличествоДетей, 0) * ЕСТЬNULL(РазмерВычетовНаДетей.Размер, 0)
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ВычетНаДетей,
	|	СУММА(ВЫБОР
	|			КОГДА ВычетыНаДетей.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код611)
	|				ТОГДА ЕСТЬNULL(ВычетыНаДетей.КоличествоДетей, 0) * ЕСТЬNULL(РазмерВычетовНаДетей.Размер, 0)
	|			ИНАЧЕ 0
	...
	|	ВычетыЛичныеВторой.КодВычета КАК КодВычетаЛичныйВторой,
	|	ВЫБОР
	|		КОГДА ПрименениеВычетов.ПрименятьВычеты
	|				И Периоды.Резидент
	|				И (ДоходыРезидентовПоМесяцам.ОблагаемыйДоход <= ЕСТЬNULL(РазмерВычетовЛичный.ОграничениеПоДоходам, 0)
	...
	|	ЕСТЬNULL(РазмерВычетовНаДетей.Размер, 0) * ЕСТЬNULL(ВычетыНаДетей.КоличествоДетей, 0) КАК СтандартныеВычетыДетские,
	|	ЕСТЬNULL(РазмерВычетовНаДетейДвойные.Размер, 0) * ЕСТЬNULL(ВычетыНаДетейДвойные.КоличествоДетей, 0) КАК СтандартныеВычетыДетскиеДвойные,
	|	ЕСТЬNULL(РазмерВычетовЛичныйВторой.Размер, 0) КАК СтандартныеВычетыЛичныеВторой,
	...
	|		ПО (ВычетыЛичные.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код600))
	|			И Периоды.МесяцНалоговогоПериода = ВычетыЛичные.МесяцНалоговогоПериода
	|			И Периоды.ФизическоеЛицо = ВычетыЛичные.ФизическоеЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПраваНаВычетыНаДетей КАК ВычетыНаДетей
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТРазмерыСтандартныхВычетов КАК РазмерВычетовНаДетей
	|			ПО ВычетыНаДетей.КодВычета = РазмерВычетовНаДетей.КодВычета
	|				И ВычетыНаДетей.НачалоГодаНалоговогоПериода = РазмерВычетовНаДетей.НачалоНалоговогоПериода
	|		ПО (ВычетыНаДетей.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код610))
	...
	|		ПО (ВычетыНаДетейДвойные.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код611))
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПраваНаЛичныеВычеты КАК ВычетыЛичныеВторой
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТРазмерыСтандартныхВычетов КАК РазмерВычетовЛичныйВторой
	|			ПО ВычетыЛичныеВторой.КодВычета = РазмерВычетовЛичныйВторой.КодВычета
	|				И ВычетыЛичныеВторой.НачалоГодаНалоговогоПериода = РазмерВычетовЛичныйВторой.НачалоНалоговогоПериода
	|		ПО (ВычетыЛичныеВторой.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код620))
	|			И Периоды.МесяцНалоговогоПериода = ВычетыЛичныеВторой.МесяцНалоговогоПериода
	|			И Периоды.ФизическоеЛицо = ВычетыЛичныеВторой.ФизическоеЛицо
	...
	|		ПО Периоды.МесяцНалоговогоПериода = ДоходыРезидентовПоМесяцам.МесяцНалоговогоПериода
	|			И Периоды.ФизическоеЛицо = ДоходыРезидентовПоМесяцам.ФизическоеЛицо
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			КОНЕЦ * СтавкиНалогаВПроцентах.Размер КАК ЧИСЛО(15, 2))) КАК НалогНерезидента
	...
	|	ЛОЖЬ КАК ИсчислятьНалогНарастающимИтогом,
	...
	|	ВЫРАЗИТЬ(СУММА(Доходы.ДоходыБезВычетов) * Доходы.СтавкаЧислом КАК ЧИСЛО(15, 2)),
	|	ЛОЖЬ,
	...
	|ГДЕ
	|	Доходы.ФизическоеЛицо ЕСТЬ НЕ NULL 
	|
	|СГРУППИРОВАТЬ ПО
	|	Периоды.ФизическоеЛицо,
	|	Периоды.МесяцНалоговогоПериода,
	...
	|	ЕСТЬNULL(СтавкаНДФЛПоСтавкеРезидента.СтавкаЧислом, 0.13) КАК СтавкаДляРезидента,
	|	ПравоНаСтандартныеВычеты.КодВычетаЛичный,
	|	ПравоНаСтандартныеВычеты.СтандартныеВычетыЛичные,
	|	ПравоНаСтандартныеВычеты.КодВычетаЛичныйВторой,
	|	ПравоНаСтандартныеВычеты.СтандартныеВычетыЛичныеВторой,
	...
	|			И Периоды.ФизическоеЛицо = Доходы.ФизическоеЛицо
	|ГДЕ
	|	(НЕ Доходы.ИсчислятьНалогНарастающимИтогом
	...
	|		ПО (СтавкиНалогаВПроцентах.СтавкаНалогообложения В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавки.Ставка13), ЗНАЧЕНИЕ(Перечисление.НДФЛСтавки.Ставка09)) )  //**************************************************************
	|
	|УПОРЯДОЧИТЬ ПО
	|	ФизическоеЛицо,
	|	НалоговыйПериод,
	...
	|	Периоды.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
	|	СУММА(ВЫБОР
	|			КОГДА ПравоНаИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код650)
	|				ТОГДА ПравоНаИмущественныеВычеты.Размер
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ВычетИмущественный,
	|	СУММА(ВЫБОР
	|			КОГДА ПравоНаИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640)
	|				ТОГДА ПравоНаИмущественныеВычеты.Размер
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ВычетСоциальный,
	|	СУММА(ВЫБОР
	|			КОГДА ПравоНаИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640Страх)
	|				ТОГДА ПравоНаИмущественныеВычеты.Размер
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ВычетСоциальныйСтраховой
	|ИЗ
	|	ВТПериодыФизлиц КАК Периоды
	...
	|		ПериодыФизлиц.МесяцНалоговогоПериода КАК МесяцНалоговогоПериода,
	|		СведенияОДоходахНДФЛ.Подразделение КАК Подразделение,
	|		ЕСТЬNULL(СведенияОДоходахНДФЛ.СуммаДохода, 0) - ЕСТЬNULL(СведенияОДоходахНДФЛ.СуммаВычета, 0) КАК СуммаДохода
	|	ИЗ
	|		ВТПериодыФизлиц КАК ПериодыФизлиц
	...
	|	И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СтавкаНалогообложенияРезидента В (ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка13), ЗНАЧЕНИЕ(Перечисление.НДФЛСтавкиНалогообложенияРезидента.Ставка09))
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код600)
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код620)
	|				ТОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета
	|			ИНАЧЕ ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.ПустаяСсылка)
	|		КОНЕЦ) КАК КодВычетаЛичныйВторой,
	|	СУММА(ВЫБОР
	|			КОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код600)
	...
	|				ТОГДА -НДФЛПредоставленныеСтандартныеВычеты.СуммаОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПримененныйВычетЛичныйВторой,
	|	СУММА(ВЫБОР
	|			КОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код610)
	|				ТОГДА -НДФЛПредоставленныеСтандартныеВычеты.СуммаОборот
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПримененныйВычетНаДетей,
	|	СУММА(ВЫБОР
	|			КОГДА НДФЛПредоставленныеСтандартныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код611)
	|				ТОГДА -НДФЛПредоставленныеСтандартныеВычеты.СуммаОборот
	|			ИНАЧЕ 0
	...
	|	СУММА(ВЫБОР
	|			КОГДА ИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код650)
	|				ТОГДА -ИмущественныеВычеты.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПримененныйВычетИмущественныйРасходы,
	|	СУММА(ВЫБОР
	|			КОГДА ИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640)
	|				ТОГДА -ИмущественныеВычеты.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПримененныйВычетСоциальный,
	|	СУММА(ВЫБОР
	|			КОГДА ИмущественныеВычеты.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640Страх)
	|				ТОГДА -ИмущественныеВычеты.Сумма
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ПримененныйВычетСоциальныйСтраховой
	|ИЗ
	|	ВТПериодыФизлиц КАК Периоды
	...
	// 4D:ERP для Беларуси, АлексейЧ, 08.02.2018 15:51:09 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928 
	// {
	...
	// Расчет по каждой дате получения дохода
		...
		Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
			...
			Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("НалоговыйПериод") Цикл
				...
				ВычетЛичныйВторойОстаток = 0;
				ВычетНаДетейОстаток = 0;
				ВычетНаДетейДвойнойОстаток = 0;
				ВычетИмущественныйОстаток = 0;
				ВычетСоциальныйОстаток = 0;
				ВычетСоциальныйСтраховойОстаток = 0;
				ОблагаемыйДоход = 0;
				...
				Пока ВыборкаРасчетов.СледующийПоЗначениюПоля("МесяцНалоговогоПериода") Цикл
					...
					ВписатьВРезультатЗарегистрированныеВУчетеДанные(ВспомогательнаяТаблицаМесяца, СтандартныхВычетовРанее.НайтиСтроки(СтруктураПоиска), "ПримененныйВычетЛичный,ПримененныйВычетЛичныйВторой," + РассчитываемыеРесурсыВычетовНаДетей());
					...
					ВычетЛичныйВторойЗаМесяц = 0;
					
					ВычетНаДетейЗаМесяц = 0;
					ВычетНаДетейДвойнойЗаМесяц = 0;
					
					ВычетИмущественныйЗаМесяц = 0;
					ВычетСоциальныйЗаМесяц = 0;
					ВычетСоциальныйСтраховойЗаМесяц = 0;
					...
					Если СтрокиПравНаИмущественныеВычеты.Количество() > 0 Тогда
						...
						ВычетИмущественныйОстаток = ВычетИмущественныйОстаток + СтрокиПравНаИмущественныеВычеты[0].ВычетИмущественный;
						ВычетСоциальныйОстаток = ВычетСоциальныйОстаток + СтрокиПравНаИмущественныеВычеты[0].ВычетСоциальный;
						ВычетСоциальныйСтраховойОстаток = ВычетСоциальныйСтраховойОстаток + СтрокиПравНаИмущественныеВычеты[0].ВычетСоциальныйСтраховой;
					КонецЕсли;
					...
					ВычетЛичныйВторойОстаток = ВыборкаРасчетов.СтандартныеВычетыЛичныеВторой;										
					...
					Если ДоходЗаМесяцОстаток > 0 И (ВыборкаРасчетов.ВсеДоходыЗаМесяц <> 0 Или ЭтоПерерасчет) Тогда
						...
						ВычетЛичныйВторойЗаМесяц = Мин(ДоходЗаМесяцОстаток,ВычетЛичныйВторойОстаток);
						ВычетЛичныйВторойОстаток = ВычетЛичныйОстаток - ВычетЛичныйВторойЗаМесяц;
						ДоходЗаМесяцОстаток = ДоходЗаМесяцОстаток - ВычетЛичныйВторойЗаМесяц;
						...
						ДоходЗаМесяцДляИмущественныхВычетов = Макс(0, ВыборкаРасчетов.ОблагаемыйДоходЗаМесяц 
																		- ВычетЛичныйЗаМесяц 
																		- ВычетЛичныйВторойЗаМесяц
																		- ВычетНаДетейЗаМесяц 
																		- ВычетНаДетейДвойнойЗаМесяц); 
							
						Если ДоходЗаМесяцДляИмущественныхВычетов > 0 Тогда
							ВычетСоциальныйСтраховойЗаМесяц		=	Мин(ДоходЗаМесяцДляИмущественныхВычетов, ВычетСоциальныйСтраховойОстаток);
							ВычетСоциальныйСтраховойОстаток		=	ВычетСоциальныйСтраховойОстаток - ВычетСоциальныйСтраховойЗаМесяц;
							ДоходЗаМесяцДляИмущественныхВычетов =	ДоходЗаМесяцДляИмущественныхВычетов - ВычетСоциальныйСтраховойЗаМесяц;
							ДоходЗаМесяцОстаток 				=	ДоходЗаМесяцОстаток - ВычетСоциальныйСтраховойЗаМесяц;
														
							ВычетСоциальныйЗаМесяц				=	Мин(ДоходЗаМесяцДляИмущественныхВычетов, ВычетСоциальныйОстаток);
							...
							ВычетИмущественныйЗаМесяц 			=	Мин(ДоходЗаМесяцДляИмущественныхВычетов, ВычетИмущественныйОстаток);
							ВычетИмущественныйОстаток 			=	ВычетИмущественныйОстаток - ВычетИмущественныйЗаМесяц;
							ДоходЗаМесяцОстаток 				=	ДоходЗаМесяцОстаток - ВычетИмущественныйЗаМесяц;
						КонецЕсли;
						ВсегоВычетовЗаМесяц = ВычетЛичныйЗаМесяц 
											+ ВычетЛичныйВторойЗаМесяц 
											+ ВычетНаДетейЗаМесяц 
											+ ВычетНаДетейДвойнойЗаМесяц
						                    + ВычетСоциальныйСтраховойЗаМесяц 
											+ ВычетСоциальныйЗаМесяц 
											+ ВычетИмущественныйЗаМесяц;
											
						ЕстьВычетыЗаМесяц = ВычетЛичныйЗаМесяц					<> 0 
											Или ВычетЛичныйВторойЗаМесяц		<> 0 
											Или ВычетНаДетейЗаМесяц				<> 0
											Или ВычетНаДетейДвойнойЗаМесяц		<> 0 
											Или ВычетНаДетейДвойнойЗаМесяц		<> 0 
						                    Или ВычетСоциальныйСтраховойЗаМесяц	<> 0 
											Или ВычетСоциальныйЗаМесяц			<> 0 
											Или ВычетИмущественныйЗаМесяц		<> 0;
						...
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ВыборкаРасчетов.МесяцНалоговогоПериода, ВычетЛичныйВторойЗаМесяц, "ПримененныйВычетЛичныйВторой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);

						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ВыборкаРасчетов.МесяцНалоговогоПериода, ВычетСоциальныйСтраховойЗаМесяц, "ПримененныйВычетСоциальныйСтраховой", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ВыборкаРасчетов.МесяцНалоговогоПериода, ВычетСоциальныйЗаМесяц, "ПримененныйВычетСоциальный", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);
						ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ВыборкаРасчетов.МесяцНалоговогоПериода, ВычетИмущественныйЗаМесяц, "ПримененныйВычетИмущественныйРасходы", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений);	
						
					КонецЕсли;
					
					ОблагаемыйДоход = Макс(0, ВыборкаРасчетов.ОблагаемыйДоходЗаМесяц - (ВычетЛичныйЗаМесяц + ВычетЛичныйВторойЗаМесяц 
								+ ВычетНаДетейЗаМесяц + ВычетНаДетейДвойнойЗаМесяц + ВычетСоциальныйСтраховойЗаМесяц + ВычетСоциальныйЗаМесяц + ВычетИмущественныйЗаМесяц));
						
					НалогЗаМесяц = Окр(ОблагаемыйДоход * ВыборкаРасчетов.СтавкаДляРезидента,2); 
					НалогИсчисленный = НалогЗаМесяц;
						
					ВписатьРесурсВРезультатыРасчетов(ВспомогательнаяТаблицаМесяца, ВыборкаРасчетов.МесяцНалоговогоПериода, НалогЗаМесяц, "НалогПоСтавке13", ВыборкаРасчетов, Доходы, КоэффициентыПодразделений, 2); //1с-минск //берем с копейками
	...
	// 4D:ERP для Беларуси, АлексейЧ, 14.02.2018 14:42:08 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
	ВписатьВРезультатЗачтенныеАвансы(Регистратор, Организация, МесяцРасчета, РезультатыРасчетов, Перечисления.НДФЛСтавкиНалогообложенияРезидента.Ставка09, "НалогПоСтавке09", "ЗачтеноАвансовыхПлатежейПоСтавке09", ИсключатьДоходыРегистратора);
	// }
	// 4D
...
Процедура ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор) Экспорт
	...
	|		Доходы.КодДохода,
	|		Доходы.Подразделение,
	|		Доходы.УчитыватьСтатьиФинансирования) КАК Доходы
...
Функция РассчитатьУдержанныеНалоги(Ссылка, Организация, ДатаОперации, МенеджерТаблиц, ПериодРегистрации = Неопределено, ОкончательныйРасчет = Истина) Экспорт
	...
	// 4D:ERP для Беларуси, АлексейЧ, 15.02.2018 17:46:46 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
	РезультатыРасчетов.Колонки.Добавить("Сумма",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(15,2)));
	РезультатыРасчетов.Колонки.Добавить("НачисленоНалога",Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(15,2)));
	// }
	// 4D
	...
	// 4D:ERP для Беларуси, АлексейЧ, 15.02.2018 17:39:37 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
	...
	|				КОГДА (ВЫРАЗИТЬ(ДанныеУчета.НачисленоНалога * ДанныеУчета.Доля КАК ЧИСЛО(15, 2))) - ДанныеУчета.УдержаноНалога > 0
	|					ТОГДА (ВЫРАЗИТЬ(ДанныеУчета.НачисленоНалога * ДанныеУчета.Доля КАК ЧИСЛО(15, 2))) - ДанныеУчета.УдержаноНалога
	|				ИНАЧЕ 0
	|			КОНЕЦ
	|			ТОГДА ДанныеУчета.СуммаОстаток
	|		ИНАЧЕ ВЫБОР
...
Процедура СоздатьВТДанныеНДФЛПоДокументу(МенеджерВременныхТаблиц, ДокументСсылка, ОписаниеТаблицДокумента = Неопределено, ФизическиеЛица = Неопределено) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 08.09.2017 11:44:01 
	// Локализация документа "Увольнение ", №15180 
	// Локализовать документ "Начисление зарплаты и взносов", №15928
	// {
	"ВЫБРАТЬ
	...
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйКЗачетуВозвратуКодВычета,
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйКЗачетуВозврату,
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйВторойКодВычета,
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйВторой,
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйВторойКЗачетуВозвратуКодВычета,
	|	ТаблицаНДФЛ.ПримененныйВычетЛичныйВторойКЗачетуВозврату,
	|	СУММА(ЕСТЬNULL(ПримененныйВычетИмущественныйРасходы.РазмерВычета, 0)) КАК ПримененныйВычетИмущественныйРасходы,
	|	СУММА(ЕСТЬNULL(ПримененныйВычетСоциальный.РазмерВычета, 0)) КАК ПримененныйВычетСоциальный,
	|	СУММА(ЕСТЬNULL(ПримененныйВычетСоциальныйСтраховой.РазмерВычета, 0)) КАК ПримененныйВычетСоциальныйСтраховой,
	...
	|			И (ПримененныйВычетИмущественныйРасходы.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код650))
	|		ЛЕВОЕ СОЕДИНЕНИЕ #ТаблицаПримененныеВычеты КАК ПримененныйВычетСоциальный
	|		ПО ТаблицаНДФЛ.Ссылка = ПримененныйВычетСоциальный.Ссылка
	|			И ТаблицаНДФЛ.ИдентификаторСтрокиНДФЛ = ПримененныйВычетСоциальный.ИдентификаторСтрокиНДФЛ
	|			И (ПримененныйВычетСоциальный.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640))
	|		ЛЕВОЕ СОЕДИНЕНИЕ #ТаблицаПримененныеВычеты КАК ПримененныйВычетСоциальныйСтраховой
	|		ПО ТаблицаНДФЛ.Ссылка = ПримененныйВычетСоциальныйСтраховой.Ссылка
	|			И ТаблицаНДФЛ.ИдентификаторСтрокиНДФЛ = ПримененныйВычетСоциальныйСтраховой.ИдентификаторСтрокиНДФЛ
	|			И (ПримененныйВычетСоциальныйСтраховой.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код640СТрах))
	...
	|			И (ПримененныйВычетНаДетей.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код610))
	...
	|			И (ПримененныйВычетНаДетейДвойной.КодВычета = ЗНАЧЕНИЕ(Справочник.ВидыВычетовНДФЛ.Код611))
	|ГДЕ
	|	ТаблицаНДФЛ.Ссылка = &Ссылка
	|	И ТаблицаНДФЛ.ФизическоеЛицо В(&ФизическиеЛица)
	|
...
Функция ВычетыКДоходам(НалоговыйПериод) Экспорт
	...
	// 4D:ERP для Беларуси, , 15.01.2018 11:02:23 
	// Локализовать документ "Начисление зарплаты и взносов",  № 15928
	// {
		МассивДоступныхВычетов = Новый Массив;
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код163113		);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код571			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код520			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код510			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код163120_1		);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код16313выхпос	);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код163113		);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код3117			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код500			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код500иныеорг	);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код501			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код540			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код1503			);
		МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код504			);
		
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163113, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код163113));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163115, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код571));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163118, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код520));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163120, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код510));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163120_1, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код163120_1));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код16313выхпос, 	ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код16313выхпос));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код163117, 		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код3117));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код500, 			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код500));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код500иныеорг, 	ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код500иныеорг));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код501, 			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код501));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код502, 			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код540));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код503, 			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код1503));
		СоответствиеДоступныхВычетовДоходам.Вставить(Справочники.ВидыДоходовНДФЛ.Код504, 			ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Справочники.ВидыВычетовНДФЛ.Код504));
	// }
	// 4D
...
Функция ВычетыНалогоплательщика(НалоговыйПериод, Знач ГруппаВычета = Неопределено) Экспорт
	...
	//*********************************************************************************
	// 4D:ERP для Беларуси, Яна, 26.05.2017 10:57:35 
	// Заявление на вычеты по подоходному налогу в ERP, № 15008 
	// {
	// Личные вычеты
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код600);
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код620);
		
	// Вычеты на детей
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код610);
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код611);

	// Имущественные вычеты
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код640);
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код640Страх);
	МассивДоступныхВычетов.Добавить(Справочники.ВидыВычетовНДФЛ.Код650);
...
Функция РассчитываемыеРесурсыПоОсновнойСтавке() Экспорт
	...
	//***********************************************************************************************************************************************
	// 4D:ERP для Беларуси, Яна, 07.09.2017 17:28:01 
	// Ошибки Зарплатного модуля, №15414 
	// Локализация документа "Увольнение ", №15180 
	// {
	
	Возврат "НалогПоСтавке13,ПримененныйВычетЛичный,ПримененныйВычетЛичныйВторой," + РассчитываемыеРесурсыВычетовНаДетей() + "," + РассчитываемыеРесурсыИмущественныхВычетов();
...
Функция РассчитываемыеРесурсыВычетовНаДетей() 
	...
	// 4D:ERP для Беларуси, Екатерина, 06.10.2015 11:11:21 
	// Локализация ЗП, №10360
	// Ошибка при проведении документа "Увольнение", №16145
	// {
	Возврат "ПримененныйВычетНаДетей,ПримененныйВычетНаДетейДвойной";
	// }
	// 4D
...
Функция РассчитываемыеРесурсыИмущественныхВычетов() 
	...
	// Ошибка при проведении документа "Увольнение", №16145
	// {
	Возврат "ПримененныйВычетИмущественныйРасходы,ПримененныйВычетСоциальный,ПримененныйВычетСоциальныйСтраховой";
	// }
	// 4D
...
Функция ВидыВычетовПоПолямТаблицы() Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 06.10.2015 11:09:48 
	// Локализация ЗП, №10360
	// Ошибка при проведении документа "Увольнение", №16145
	// {
	
	ВидыВычетов.Вставить("ПримененныйВычетИмущественныйРасходы", Справочники.ВидыВычетовНДФЛ.Код650);
	ВидыВычетов.Вставить("ПримененныйВычетСоциальный", Справочники.ВидыВычетовНДФЛ.Код640);
	ВидыВычетов.Вставить("ПримененныйВычетСоциальныйСтраховой", Справочники.ВидыВычетовНДФЛ.Код640Страх);
	
	ВидыВычетов.Вставить("ПримененныйВычетНаДетей", Справочники.ВидыВычетовНДФЛ.Код610);
	ВидыВычетов.Вставить("ПримененныйВычетНаДетейДвойной", Справочники.ВидыВычетовНДФЛ.Код611);
	// }
	// 4D
...
Функция ТаблицаНДФЛ() Экспорт
	...
	// 4D:ERP для Беларуси, АлексейЧ, 15.01.2018 14:13:56 
	// Локализовать документ "Начисление зарплаты и взносов", № 15928
	// {
	ТипЗначенияНалогСКопейками = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2));
	// }
	// 4D
	...
	НДФЛ.Колонки.Добавить("КодВычетаЛичныйВторой", Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"));
	НДФЛ.Колонки.Добавить("ПримененныйВычетЛичный", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("ПримененныйВычетЛичныйВторой", ТипЗначенияНалогСКопейками);
	// }
	// 4D
	...
	Для Каждого НазваниеВычета Из НазванияВычетов Цикл
		...
		// Локализовать документ "Начисление зарплаты и взносов", № 15928
		// {
		НДФЛ.Колонки.Добавить(НазваниеВычета, ТипЗначенияНалогСКопейками);
		// }
		// 4D
	КонецЦикла;
	...
	НДФЛ.Колонки.Добавить("Налог", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("ЗачтеноАвансовыхПлатежей", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("НалогКЗачетуВозврату", ТипЗначенияНалогСКопейками);
	
	НДФЛ.Колонки.Добавить("НалогПоСтавке13", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("ЗачтеноАвансовыхПлатежейПоСтавке13", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("НалогПоСтавке13КЗачетуВозврату", ТипЗначенияНалогСКопейками);
	
	НДФЛ.Колонки.Добавить("НалогПоСтавке09", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("ЗачтеноАвансовыхПлатежейПоСтавке09", ТипЗначенияНалогСКопейками);
	НДФЛ.Колонки.Добавить("НалогПоСтавке09КЗачетуВозврату", ТипЗначенияНалогСКопейками);
	...
	// <описание>, №
	// {
	НДФЛ.Колонки.Добавить("ПримененныйВычетЛичныйКодВычетаВторой", Новый ОписаниеТипов("СправочникСсылка.ВидыВычетовНДФЛ"));
	НДФЛ.Колонки.Добавить("Сумма", ТипЗначенияНалогСКопейками);
	// }
	// 4D
...
Функция ДанныеОРезультатахРасчетаНДФЛДляДокумента(НДФЛ) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 07.09.2017 15:11:20 
	// Локализация документа "Увольнение ", №15180 
	// {
	НДФЛ.Колонки.Добавить("ПримененныйВычетЛичныйВторойКодВычета");
	...
	Для Каждого СтрокаНДФЛ Из НДФЛ Цикл
		...
		СтрокаНДФЛ.ПримененныйВычетЛичныйВторойКодВычета = СтрокаНДФЛ.КодВычетаЛичныйВторой;
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 31.05.2017 15:53:46 
	// Результат тестирования конфигурации, №15769 
	// Регистр сведений Бюджет прожиточного минимума в ERP, № 15052 
	// {
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.РегистрыСведений.ВеличинаПрожиточногоМинимума) Тогда
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.0.24.68";
		Обработчик.Процедура = "РегистрыСведений.ВеличинаПрожиточногоМинимума.НачальноеЗаполнение";
		Обработчик.ОбщиеДанные =  ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.ВеличинаПрожиточногоМинимума);
		Обработчик.НачальноеЗаполнение = Истина;
	КонецЕсли;

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.0.24.68";
	Обработчик.Процедура = "РегистрыСведений.ВеличинаПрожиточногоМинимума.ЗаполнитьБПМ_РБ";
	Обработчик.ОбщиеДанные = ЗарплатаКадры.ЭтоОбщиеДанные(Метаданные.РегистрыСведений.ВеличинаПрожиточногоМинимума);
	// }
	// 4D
...
Функция СформироватьПечатнуюФорму2НДФЛ(ОбъектыПечати, МассивОбъектов, ДанныеДокументов, ДанныеСотрудниковДокументов, ДанныеОДоходахДокументов, ДанныеОВычетахДокументов, ДанныеОбУведомленияхДокументов) Экспорт
	...
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			// 4D:ERP для Беларуси, Юлия, 24.08.2017 11:12:48 
			// Печать справки подоходного налога, № 15798
			// {
			...
			Для Каждого СтавкаНДФЛ Из Перечисления.НДФЛСтавки Цикл
				...
				Если ПерваяСтавка Тогда
					...
					Иначе
						...
						// 4D:ERP для Беларуси, Юлия, 24.08.2017 11:13:43 
						// Печать справки подоходного налога, № 15798
						// {
