...
Процедура ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	...
	Если ПараметрыЗагрузки = Неопределено Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			// 4D:ERP для Беларуси, ВладимирР, 04.12.2015 9:22:57 
			// Загрузка курсов валют, №10621
			// {
			...
			Для каждого текСтрока Из СписокВалют Цикл
				...
				новаяСтрока.КодВалюты = текСтрока.КодВалюты;
			КонецЦикла;
			
			РаботаСКурсамиВалютВызовСервера_Локализация.ЗагрузитьКурсыВалютВебСервис(ТаблицаВалют, НачалоПериода, КонецПериода, ПриЗагрузкеВозниклиОшибки);
			// }
			// 4D
		КонецЦикла;
	Иначе
		...
		Для каждого текСтрока Из ПараметрыЗагрузки.СписокВалют Цикл
			новаяСтрока = ТаблицаВалют.Добавить();
			новаяСтрока.Валюта = текСтрока.Валюта;
			новаяСтрока.КодВалюты = текСтрока.КодВалюты;
		КонецЦикла;
		
		РаботаСКурсамиВалютВызовСервера_Локализация.ЗагрузитьКурсыВалютВебСервис(ТаблицаВалют, ПараметрыЗагрузки.НачалоПериода, ПараметрыЗагрузки.КонецПериода, ПриЗагрузкеВозниклиОшибки);
		// }
		// 4D
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Справочники.Валюты.ВключитьЗагрузкуКурсаВалюты643ИзИнтернета";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("dc79c561-8657-4852-bbc5-38ced6996fff");
	Обработчик.Комментарий = НСтр("ru = 'Включает ошибочно выключенную загрузку курсов валюты ""Российский рубль (643)"" из интернета.'");
