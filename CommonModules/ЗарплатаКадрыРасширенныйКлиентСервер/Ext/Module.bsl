...
Процедура ВводНачисленийИнициализироватьСтрокиНачислений(Форма, СтрокиНачислений, ОписаниеТаблицы, РежимРаботы, Сотрудник = Неопределено, ДатаСведений = Неопределено) Экспорт
	...
	Для Каждого СтрокаНачислений Из СтрокиНачислений Цикл
		...
		// 4D:ERP для Беларуси, Яна, 23.06.2017 10:16:42 
		// Документ "Изменение оплаты труда" сумма ФОТ, №15215 
		// {
		Если РежимРаботы = 2 Тогда
...
Функция ИтогиПоФОТ(Форма, ОписаниеТаблицыВидовРасчета) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 21.06.2017 11:10:51 
	// На форме документа "Прием на работу" не рассчитана сумма ФОТ, №15146 
	// {
	Для каждого СтрокаНачисления Из ДанныеНачислений Цикл
		
		Если ТабличнаяЧастьСодержитПолеДействие(ОписаниеТаблицыВидовРасчета)
			И СтрокаНачисления.Действие = ПредопределенноеЗначение("Перечисление.ДействияСНачислениямиИУдержаниями.Отменить") Тогда
...
Функция ПродолжительностьСтажа(Знач ОписаниеСтажа, Знач ДатаАктуальности) Экспорт 
	...
	ДатаОтсчета = НачалоДня(ОписаниеСтажа.ДатаОтсчета);
	
	Если ДатаАктуальности > ДатаОтсчета Тогда
		
		Лет = Год(ДатаАктуальности) - Год(ДатаОтсчета);
		Если Месяц(ДатаОтсчета) > Месяц(ДатаАктуальности) Тогда
			Лет = Лет - 1;
		ИначеЕсли Месяц(ДатаОтсчета) = Месяц(ДатаАктуальности)
			И День(ДатаОтсчета) > День(ДатаАктуальности) Тогда
			
			Лет = Лет - 1;
			
		КонецЕсли;
		
		НачалоТекущегоГода = ДобавитьМесяц(ДатаОтсчета, Лет * 12);
		Если Год(НачалоТекущегоГода) < Год(ДатаАктуальности) Тогда
			Месяцев = 12 - Месяц(НачалоТекущегоГода) + Месяц(ДатаАктуальности);
		Иначе
			Месяцев = Месяц(ДатаАктуальности) - Месяц(НачалоТекущегоГода);
		КонецЕсли;
		
		Если День(НачалоТекущегоГода) > День(ДатаАктуальности) Тогда
			Месяцев = Месяцев - 1;
		КонецЕсли;
		
		Дней = (ДатаАктуальности - ДобавитьМесяц(НачалоТекущегоГода, Месяцев)) / 86400;
		
		Лет = Лет + ОписаниеСтажа.Лет;
		Месяцев = Месяцев + ОписаниеСтажа.Месяцев;
		Дней = Дней + ОписаниеСтажа.Дней;
		
		Если Дней >= 30 Тогда
			
			Если ОписаниеСтажа.Лет = 0
				И ОписаниеСтажа.Месяцев = 0 
				И ОписаниеСтажа.Дней = 1 Тогда
				
				Если Дней >= День(КонецМесяца(ДатаАктуальности)) Тогда
			...
			Иначе
				...
				ЧислоМесяцевВДнях = Цел(Дней / 30);
				Дней = Дней - ЧислоМесяцевВДнях * 30;
				
				Месяцев = Месяцев + ЧислоМесяцевВДнях;
		...
		Если Дней >= День(КонецМесяца(ДатаАктуальности)) Тогда
			...
			Дней = Дней - День(КонецМесяца(ДатаАктуальности));
			Месяцев = Месяцев + 1;
		...
		Если Месяцев >= 12 Тогда
			...
			ЧислоЛетВМесяцах = Цел(Месяцев / 12);
			Месяцев = Месяцев - ЧислоЛетВМесяцах * 12;
			
			Лет = Лет + ЧислоЛетВМесяцах;
			
		КонецЕсли;
		
	ИначеЕсли ДатаАктуальности = ДатаОтсчета Тогда
		
		Лет = ОписаниеСтажа.Лет;
		Месяцев = ОписаниеСтажа.Месяцев;
		Дней = ОписаниеСтажа.Дней;
...
Процедура ЗаполнитьЗначенияПоказателейТарифныхСетокВСтрокеНачислений(Форма, СтрокаНачислений, ОписаниеТаблицыВидовРасчета, РежимРаботы, ДополнительныеПараметры) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 01.06.2017 10:44:10 
	...
	Если ЗначениеЗаполнено(ВидРасчета) Тогда
		Если (ЗначениеЗаполнено(ТарифнаяСетка) ИЛИ ЗначениеЗаполнено(ТарифнаяСеткаНадбавки)) И (РежимРаботы = 0 ИЛИ ЗначениеЗаполнено(РазрядКатегория) 
			ИЛИ ЗначениеЗаполнено(РазрядКатегорияНадбавки)) Тогда
			...
			Для СчетчикПоказателей = 1 По ЗарплатаКадрыРасширенныйКлиентСервер.МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицыВидовРасчета) Цикл
