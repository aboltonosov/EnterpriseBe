...
Функция ЗаполнитьОтчетОРозничныхПродажах(ОтчетОРозничныхПродажахОбъект, КассоваяСмена, ОписаниеОшибки = "") Экспорт
	...
	Попытка
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:47:44 
		...
		|	ЧекККМТовары.Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:51:32 
		// Расчет скидки к цене, №10910
		// {
		|	0,
		// }
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 17:13:01 
		// Расчет скидки к цене, №10910
		// {
		|	СписокТоваров.Цена КАК ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:53:23 
		// Расчет скидки к цене, №10910
		// {
		|	ТаблицаИндексов.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|	Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:53:48 
		// Расчет скидки к цене, №10910
		// {
		|	ЦенаСоСкидкой,
		// }
		...
		|	СписокТоваров.Цена КАК Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:54:45 
		// Расчет скидки к цене, №10910
		// {
		|	СписокТоваров.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|		СписокТоваров.Цена КАК Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:55:19 
		// Расчет скидки к цене, №10910
		// {
		|		СписокТоваров.ЦенаСоСкидкой КАК ЦенаСоСкидкой,
		// }
		...
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:55:44 
		// Расчет скидки к цене, №10910
		// {
		|			И СписокТоваров.ЦенаСоСкидкой = ИндексыПоНоменклатуре.ЦенаСоСкидкой
		// }
		// 4D
		|
		...
		|	СписокТоваров.Цена,
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:56:17 
		// Расчет скидки к цене, №10910
		// {
		|	СписокТоваров.ЦенаСоСкидкой,
		// }
		// 4D
		|
		...
		|	МАКСИМУМ(Цена),
		|
		// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:56:40 
		// Расчет скидки к цене, №10910
		// {
		|	МАКСИМУМ(ЦенаСоСкидкой),
		// }
		// 4D
		|
		|	МАКСИМУМ(Партнер),
			...
			Пока ВыборкаПоАналитикеНоменклатуры.Следующий() Цикл
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Если ВыборкаДетальныеЗаписи.Количество <> 0 Тогда
						...
						// 4D:ERP для Беларуси, Екатерина, 21.04.2016 16:59:25 
						// Расчет скидки к цене, №10910
						// {
...
Функция ВыполнитьОперациюЗакрытияКассовойСмены(Объект, ОшибкаПриСнятииZОтчета, ОписаниеОшибки = "") Экспорт
	...
	Если Не ЗначениеЗаполнено(КассаККМ)
		Или ВыполнитьАвтоматическуюИнкассациюДС(КассаККМ, ОшибкаПриСнятииZОтчета, ОписаниеОшибки) Тогда
		ЗафиксироватьТранзакцию();
	Иначе
		ОтменитьТранзакцию();
		Возврат МассивДокументов;
