...
//
Процедура СоздатьВТПериодыИзмененияСтажа(МенеджерВременныхТаблиц) Экспорт
	...
	|	СотрудникиПериоды.ДатаОкончания,
	|	СтажиФизическихЛиц.ВидСтажа КАК ВидСтажа,
	|	ДОБАВИТЬКДАТЕ(СтажиФизическихЛиц.ДатаОтсчета, ДЕНЬ, -СтажиФизическихЛиц.РазмерДней) КАК ДатаСтажа,
	|	СтажиФизическихЛиц.РазмерМесяцев + РАЗНОСТЬДАТ(ДОБАВИТЬКДАТЕ(СтажиФизическихЛиц.ДатаОтсчета, ДЕНЬ, -СтажиФизическихЛиц.РазмерДней), СотрудникиПериоды.Период, МЕСЯЦ) КАК ЗначениеСтажаНаНачало
	|ИЗ
	|	ВТСотрудникиПериоды КАК СотрудникиПериоды
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтажиФизическихЛиц КАК СтажиФизическихЛиц
	|		ПО (СтажиФизическихЛиц.ФизическоеЛицо = ВЫРАЗИТЬ(СотрудникиПериоды.Сотрудник КАК Справочник.Сотрудники).ФизическоеЛицо)
	|			И (СтажиФизическихЛиц.ДатаОтсчета <= СотрудникиПериоды.ДатаОкончания)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Сотрудник,
	|	ВидСтажа,
	...
	// Дата стажа определяется как дата, на которую приходится целое значение месяцев.
	// Значение стажа определяется как размер в месяцах на дату отсчета + целое значение месяцев на дату начала.
	...
	// Составляется таблица с датами изменения стажа за интервалы для сотрудников.
	ДатыИзмененияСтажаСотрудников = Новый ТаблицаЗначений;
	ДатыИзмененияСтажаСотрудников.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ДатыИзмененияСтажаСотрудников.Колонки.Добавить("ВидСтажа", Новый ОписаниеТипов("СправочникСсылка.ВидыСтажа"));
	ДатыИзмененияСтажаСотрудников.Колонки.Добавить("ДатаСтажа", Новый ОписаниеТипов("Дата"));
	ДатыИзмененияСтажаСотрудников.Колонки.Добавить("ЗначениеСтажа", Новый ОписаниеТипов("Число"));
	
	Пока Выборка.Следующий() Цикл
		// Для каждого сотрудника и вида стажа помесячно добавляем строки с датами изменения стажа.
		ДатаСтажа = Выборка.ДатаСтажа;
		ЗначениеСтажа = Выборка.ЗначениеСтажаНаНачало;
		Если ДатаСтажа <= Выборка.ДатаНачала Тогда
			// Первая строка - это значение на начало интервала.
			НоваяСтрока = ДатыИзмененияСтажаСотрудников.Добавить();
			НоваяСтрока.Сотрудник = Выборка.Сотрудник;
			НоваяСтрока.ВидСтажа = Выборка.ВидСтажа;
			НоваяСтрока.ДатаСтажа = Выборка.ДатаНачала;
			НоваяСтрока.ЗначениеСтажа = ЗначениеСтажа - ?(День(Выборка.ДатаСтажа) > День(Выборка.ДатаНачала), 1, 0);
			ЗначениеСтажа = НоваяСтрока.ЗначениеСтажа + 1;
			ДатаСтажа = ДобавитьМесяц(ДатаСтажа, 1);
		КонецЕсли;
		// Далее добавляем значения попадающие в интервал.
		Пока ДатаСтажа < Выборка.ДатаОкончания Цикл
			Если ДатаСтажа > Выборка.ДатаНачала Тогда
				НоваяСтрока = ДатыИзмененияСтажаСотрудников.Добавить();
				НоваяСтрока.Сотрудник = Выборка.Сотрудник;
				НоваяСтрока.ВидСтажа = Выборка.ВидСтажа;
				НоваяСтрока.ДатаСтажа = ДатаСтажа;
				НоваяСтрока.ЗначениеСтажа = ЗначениеСтажа;
			КонецЕсли;
			ЗначениеСтажа = НоваяСтрока.ЗначениеСтажа + 1;
			ДатаСтажа = ДобавитьМесяц(ДатаСтажа, 1);
		КонецЦикла;
	...
	// Результат помещаем во временную таблицу.
	ТекстЗапроса = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|ИЗ
	|	&ДатыИзмененияСтажаСотрудников КАК ПериодыИзмененияСтажа";
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("ДатыИзмененияСтажаСотрудников", ДатыИзмененияСтажаСотрудников);
	Запрос.Выполнить();
	...
	Если НеобходимыСведенияОбОбразовании Тогда
		...
		Если УсловиеПоиска.ОтносительныйПуть
			...
			Если УсловиеВидаСравнения = "<"
				...
				// 4D:ERP для Беларуси, Дмитрий, 07.10.2015 4:36:31 
				// TODO, №
				// {
...
Функция НеобходимыСведенияОВоинскомУчетеФизическихЛиц(Знач ИмяПоля)
	...
	// 4D:ERP для Беларуси, Яна, 04.05.2017 6:11:39 
	// Исправление справочников по подсистемам "Кадры" и "Зарплата", №14769 
	// {
	Возврат ИмяПоля = ВРег("ВоинскийУчетГруппаУчета")
		...
		// 4D:ERP для Беларуси, Юлия, 23.08.2017 11:17:01 
		// Ошибки в печатных формах со спр. Сотрудники, № 15785
		// { 	
		ИЛИ ИмяПоля = ВРег("ВоинскийУчетПериодРегистрации");
		// }
		// 4D
...
Функция ПутьКДаннымСведенийОВоинскомУчетеФизическихЛиц(Знач ИмяПоля)
	...
	// {
	ПутьКДанным = "";
	Если ИмяПоля = ВРег("ВоинскийУчетГруппаУчета") Тогда
...
Процедура СоздатьВТДанныеДокументовКадровыхПеремещений(МенеджерВременныхТаблиц, МассивОбъектов) Экспорт
	...
	Запрос.Текст =
		...
		|	
		// 4D:ERP для Беларуси, Юлия, 19.10.2017 9:22:40 
		...
		|	
		// 4D:ERP для Беларуси, Юлия, 04.11.2017 12:18:51 
...
Функция ПараметрыВыбораКатегорииНачислений(ПолныйСписокКатегорий = Ложь) Экспорт
	...
	// 4D:ERP для Беларуси, Яна, 15.06.2017 10:48:07 
	// РС "Размер гос. пособий", №14877, 15145  
	// {
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеПоУходуЗаРебенкомДо3Лет50ПроцентовБПМ);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаВичИнфицРебенка);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаДо18ЛетНаРебенкаИнвалида);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаСтарше3ЛетКромеРебенкаИнвалида);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаСтарше3ЛетНаРебенкаИнвалида);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаСтарше3Лет);
	МассивКатегорийНачислений.Добавить(Перечисления.КатегорииНачисленийИНеоплаченногоВремени.ПособиеНаРебенкаДо3ЛетНаРебенкаИнвалида);
	// }
	// 4D
