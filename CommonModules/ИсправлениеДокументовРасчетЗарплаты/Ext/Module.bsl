...
Процедура СформироватьДанныеПерерасчета(Организация, ПериодРегистрации, Начисления, НачисленияПерерасчет, ДокументСсылка, КонтролируемыеПоля, ОписаниеДокумента, ОтборПоСотрудникам = Неопределено, Объект = Неопределено) Экспорт
	...
	СформироватьДанныеПерерасчетаСлужебный(Организация, ПериодРегистрации, Начисления, НачисленияПерерасчет, ДокументСсылка, КонтролируемыеПоля, ОписаниеДокумента, ОтборПоСотрудникам, Объект);
...
//
Процедура СформироватьДанныеПерерасчетаДляИсправления(Организация, ПериодРегистрации, ИсправленныйДокумент, Начисления, НачисленияПерерасчет, ДокументСсылка, ОписаниеТаблицы, КонтролируемыеПоля, ОписаниеДокумента, Сотрудник = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	СформироватьДанныеПерерасчетаДляИсправленияСлужебный(Организация, ПериодРегистрации, ИсправленныйДокумент, Начисления, НачисленияПерерасчет, ДокументСсылка, ОписаниеТаблицы, КонтролируемыеПоля, ОписаниеДокумента, Сотрудник);
	УстановитьПривилегированныйРежим(Ложь);
...
Функция ОписаниеТиповСторнируемогоДокумента() Экспорт
	Возврат Метаданные.ОпределяемыеТипы.СторнируемыеДокументы.Тип;
КонецФункции
...
Процедура ЗаполнитьНачисленияПерерасчетПоНачисленияЗаднимЧислом(Организация, ПериодРегистрации, НачисленияЗаднимЧислом, НачисленияПерерасчет, ИсключаемыйРегистратор, ДанныеПерерасчетов = Неопределено, Показатели = Неопределено) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НачисленияЗаднимЧислом.ВидРасчета,
	...
	|				И (НЕ ПланВидовРасчетаНачисления.ТребуетсяРасчетБазы)
...
Процедура СформироватьДанныеПерерасчетаСлужебный(Организация, ПериодРегистрации, Начисления, НачисленияПерерасчет, ДокументСсылка, КонтролируемыеПоля, ОписаниеДокумента, ОтборПоСотрудникам = Неопределено, Объект = Неопределено)
	...
	// Фиксированные сторнированные строки всегда перезаполняем
	СтруктураОтбора = Новый Структура("ФиксРасчет, ФиксСторно", Истина, Истина);
	НайденныеСтроки = НачисленияПерерасчет.НайтиСтроки(СтруктураОтбора);
	УдалитьСтрокиСОтборомПоСотрудникам(НачисленияПерерасчет, НайденныеСтроки, ОтборПоСотрудникам);
...
Функция ТребуетсяПерерасчет(Регистратор, ПериодРегистрации, Начисления)
	...
	ПараметрНачисления = Начисления.Выгрузить(, "Сотрудник,ПериодДействия");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Регистратор", Регистратор);
	Запрос.УстановитьПараметр("ПериодРегистрации", ПериодРегистрации);
	Запрос.УстановитьПараметр("Начисления", ПараметрНачисления);
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Начисления.Сотрудник,
		|	Начисления.ПериодДействия
...
Процедура СформироватьДанныеПерерасчетаДляИсправленияСлужебный(Организация, ПериодРегистрации, ИсправленныйДокумент, Начисления, НачисленияПерерасчет, ДокументСсылка, ОписаниеТаблицы, КонтролируемыеПоля, ОписаниеДокумента, Сотрудник = Неопределено) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "РеквизитПериодРегистрации", ОписаниеТаблицы.ИмяРеквизитаПериод);
	Если ПустаяСтрока(ОписаниеТаблицы.ИмяРеквизитаДокументОснование) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (РегистрНачисления.ДокументОснование = Начисления.ДокументОснование)", "");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (РегистрНачисления.ДокументОснование = Начисления.ДокументОснование)", "И (РегистрНачисления.ДокументОснование = Начисления." + ОписаниеТаблицы.ИмяРеквизитаДокументОснование + ")");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ОписаниеТаблицы.ИмяРеквизитаОтработаноДней) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.ОтработаноДней + ЕСТЬNULL(РегистрНачисления.ОтработаноДней, 0) КАК ОтработаноДней", "0 КАК ОтработаноДней");
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ОписаниеТаблицы.ИмяРеквизитаОтработаноЧасов) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.ОтработаноЧасов + ЕСТЬNULL(РегистрНачисления.ОтработаноЧасов, 0) КАК ОтработаноЧасов", "0 КАК ОтработаноЧасов");
	КонецЕсли;
	Если ДокументБезДатаНачала Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.ДатаНачала", "Начисления.Ссылка." + ОписаниеТаблицы.ИмяРеквизитаПериод);
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.ДатаОкончания", "КОНЕЦПЕРИОДА(Начисления.Ссылка." + ОписаниеТаблицы.ИмяРеквизитаПериод + ", МЕСЯЦ)");
	КонецЕсли;
	Если ОписаниеТаблицы.СодержитПолеВидРасчета Тогда
		...
		|	Начисления.Ссылка.ВидРасчета КАК Начисление", "");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (РегистрНачисления.ВидРасчета = Начисления.Ссылка.ВидРасчета)", "И (РегистрНачисления.ВидРасчета = Начисления." + ОписаниеТаблицы.ИмяРеквизитаВидРасчета + ")");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "
		|
		|СГРУППИРОВАТЬ ПО
		|	Начисления.Ссылка.ВидРасчета", "");
	Иначе
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Начисления.Ссылка.ВидРасчета", "Начисления.Ссылка." + ОписаниеТаблицы.ИмяРеквизитаВидРасчета);
	КонецЕсли;
	...
	// Не нашлось в исправленном документе сотрудников по которыым формируются перерасчеты
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТНовыйФПД КАК НовыйФПД
