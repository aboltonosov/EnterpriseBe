...
Процедура ТорговыеПредложенияНажатиеГиперссылки(Форма, Объект, Элемент, СтандартнаяОбработка) Экспорт 
	...
	Если Элемент.Имя = "ТорговыеПредложенияСостояниеОбмена" Тогда
		...
		ОткрываемаяФорма = "Обработка.ТорговыеПредложения.Форма.ФормаСписка";
		ОткрытьФорму(ОткрываемаяФорма,, Объект);
...
Процедура ПриИзмененииПубликоватьТорговыеПредложения(Форма, Объект, Элемент, СтандартнаяОбработка) Экспорт
	...
	Форма.ТорговыеПредложенияСостояниеОбмена = НСтр("ru = 'Для смены статуса публикации нажмите Записать'");
	...
	Если ПустаяСтрока(Форма.ТорговыеПредложенияСостояниеОбмена)
		И Форма.ТорговыеПредложенияПубликовать = Истина Тогда
		
		Форма.Элементы.ТорговыеПредложенияОткрытьНастройкиПубликации.Видимость = Истина;
	КонецЕсли;
