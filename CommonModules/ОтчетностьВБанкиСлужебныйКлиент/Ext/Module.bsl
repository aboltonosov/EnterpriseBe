...
Процедура ПолучитьАдресВнешнейКомпоненты(ДополнительныеПараметры)
	...
	АдресВК = ДополнительныеВнешниеКомпонентыВызовСервера.АдресВК("VPNKeyTLS");
	...
	Иначе
		...
		ДополнительныеВнешниеКомпонентыВызовСервера.ОбновитьВнешнююКомпоненту("VPNKeyTLS");
	КонецЕсли;
...
Процедура ПодключитьВнешнююКомпонентуСбербанка(АдресВК, ДополнительныеПараметры)
	...
	ДополнительныеВнешниеКомпонентыКлиент.ПодключитьВнешнююКомпонентуПоСсылке(Оповещение, АдресВК, "VPNKeyTLS");
	...
	Иначе
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru = 'Не удалось выполнить аутентификацию на банковском ключе.
										|Необходимо выполнить перезапуск банковского ключа'");
