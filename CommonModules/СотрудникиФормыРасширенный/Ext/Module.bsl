...
Процедура ДобавитьКомандыПечатиСправочникуСотрудники(КомандыПечати) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 22.08.2017 11:31:57 
	// Ошибки в печатных формах со спр. Сотрудники, № 15785
	// {	
...
Процедура ДобавитьКомандуПечатиСправкиОДоходахПроизвольнаяФорма(КомандыПечати)
	...
	Если Пользователи.РолиДоступны("ДобавлениеИзменениеНачисленнойЗарплатыРасширенная,ЧтениеНачисленнойЗарплатыРасширенная", , Ложь) Тогда
		...
		КомандаПечати.Идентификатор = "ПФ_MXL_СправкаОДоходахПроизвольнаяФорма_Локализация";
...
Процедура ДобавитьКомандуПечатиСправкиСМестаРаботы(КомандыПечати)
	...
	Если Пользователи.РолиДоступны("ДобавлениеИзменениеКадровогоСостоянияРасширенная,ЧтениеКадровогоСостоянияРасширенная", , Ложь) Тогда
		...
		КомандаПечати.Обработчик = "ЗарплатаКадрыКлиент.ВыполнитьКомандуПечати";
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьКадровыхПриказовРасширенная";
		КомандаПечати.Идентификатор = "ПФ_MXL_СправкаСМестаРаботы_Локализация";
...
Процедура ДобавитьДополнительноеПоллеСортировки(Коллекция, Поле, Тип) Экспорт
	Если Коллекция.ОписаниеКоллекции.ДополнительынеПоляСортировки = Неопределено Тогда
		Коллекция.ОписаниеКоллекции.ДополнительынеПоляСортировки = Новый Массив;
	КонецЕсли;
	
	ОписаниеПоля = ОписаниеПоляДопУпорядовачиванияКоллекцииСотрудников(Поле, Тип);
	Коллекция.ОписаниеКоллекции.ДополнительынеПоляСортировки.Добавить(ОписаниеПоля);	
КонецПроцедуры	
...
Функция ОписаниеСортируемойКоллекцииСотрудников() Экспорт
	...
	ОписаниеСортируемойКоллекции.Вставить("ДополнительынеПоляСортировки");	
	
	Возврат ОписаниеСортируемойКоллекции;
КонецФункции	

Функция ПорядокСтрокКоллекцийСотрудников(СортируемыеКоллекции, НастрокиСортировки = Неопределено) Экспорт
	...
	Если НастрокиСортировки = Неопределено Тогда
		НастрокиСортировки = НастройкиСортировкиПоУмолчанию();	
	КонецЕсли;	
	
	СортируемаяТаблица = ТаблицаСортировкиСотрудников(ОписаниеСортируемыхКоллекция);
	ТаблицаОтбраКадровыхДанных = ТаблицаОтбраКадровыхДанныхДляСортировкиКоллекцийСотрудников();
	
	Для Каждого СортируемыеДанные Из СортируемыеКоллекции Цикл
		НеобходимоПолучатьКадровыеДанные = НеобходимоПолучатьКадровыеДанныеДляСортировки(СортируемыеДанные.ОписаниеКоллекции, НастрокиСортировки);
		...
		Для Каждого СтрокаСортируемойТаблицы Из СортируемыеДанные.Коллекция Цикл	
			...
			Если НеобходимоПолучатьКадровыеДанные Тогда	
				ДобавитьСтрокуВТаблицуОтбраКадровыхДанныхДляСортировки(ТаблицаОтбраКадровыхДанных, СтрокаСортируемойТаблицы, СортируемыеДанные.ОписаниеКоллекции);
			КонецЕсли;	
	...
	РезультатЗапросаСортировки = РезультатЗапросаСортирокиКоллекцииСотрудников(СортируемаяТаблица, НастрокиСортировки, ТаблицаОтбраКадровыхДанных, ОписаниеСортируемыхКоллекция);
...
Функция ОписаниеПоляДопУпорядовачиванияКоллекцииСотрудников(Поле, Тип) Экспорт 
	Возврат Новый Структура("Поле, Тип", Поле, Тип);	
КонецФункции

Функция РезультатЗапросаСортирокиКоллекцииСотрудников(СортируемаяТаблица, НастрокиСортировки, ТаблицаОтбораКадровыхДанных, ОписаниеСортируемыхКоллекций)
	Запрос = Новый Запрос;
	...
	КадровыеДанныеВоВнешейТаблице = ТаблицаОтбораКадровыхДанных.Количество() > 0;
	ПоляУпорядовачивания = ПоляУпорядовачиванияЗапросаСортировкиКоллекцииСотрудников(КадровыеДанныеВоВнешейТаблице, НастрокиСортировки, ОписаниеСортируемыхКоллекций);
	
	ПоляПорядка = Новый Массив;
	БлокУпорядочить = Новый Массив;	
	
	Для Каждого ОписаниеПоля Из ПоляУпорядовачивания Цикл
		ПоляПорядка.Добавить(ОписаниеПоля.Текст + " КАК " + ОписаниеПоля.Псевдоним);
		БлокУпорядочить.Добавить(ОписаниеПоля.Псевдоним);
	КонецЦикла;	
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПоляУпорядовачивания", СтрСоединить(ПоляПорядка,  "," + Символы.ПС));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Порядок", СтрСоединить(БлокУпорядочить,  "," + Символы.ПС));
	
	Возврат Запрос.Выполнить();
КонецФункции	

Функция ПоляУпорядовачиванияЗапросаСортировкиКоллекцииСотрудников(КадровыеДанныеВоВнешейТаблице, НастрокиСортировки, ОписаниеСортируемыхКоллекций)
	ПоляУпорядовачивания = Новый ТаблицаЗначений;
	ПоляУпорядовачивания.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число"));
	ПоляУпорядовачивания.Колонки.Добавить("Текст", Новый ОписаниеТипов("Строка"));
	ПоляУпорядовачивания.Колонки.Добавить("Псевдоним", Новый ОписаниеТипов("Строка"));
	
	Если КадровыеДанныеВоВнешейТаблице Тогда
		ШаблонПоляПодразделение = 
		"ВЫБОР
		|	КОГДА ИсходныеДанные.ЗаданоПолеПодразделение
		|			ИЛИ КадровыеДанныеСотрудников.Подразделение ЕСТЬ NULL 
		|		ТОГДА ИсходныеДанные.ПодразделениеПорядка.РеквизитУпорядовачивания
		|	ИНАЧЕ КадровыеДанныеСотрудников.Подразделение.РеквизитУпорядовачивания
		|КОНЕЦ";	
		ШаблонПоляДолжность = 
		"ВЫБОР
		|	КОГДА ИсходныеДанные.ЗаданоПолеДолжность
		|			ИЛИ КадровыеДанныеСотрудников.Должность ЕСТЬ NULL 
		|		ТОГДА ИсходныеДанные.ДолжностьПорядка.РеквизитУпорядовачивания
		|	ИНАЧЕ КадровыеДанныеСотрудников.Должность.РеквизитУпорядовачивания
		|КОНЕЦ";
	Иначе
	...
	Для Каждого ЭлементСортировки Из НастрокиСортировки Цикл
		Если ЭлементСортировки.Значение.Использовать <> Истина Тогда
			Продолжить;
		КонецЕсли;
		
		СтрокаТаблицы = ПоляУпорядовачивания.Добавить();
		...
		Если ЭлементСортировки.Ключ = "ПодразделениеПриоритет" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляПодразделение, "РеквизитУпорядовачивания", "РеквизитДопУпорядочиванияИерархического");	
		ИначеЕсли ЭлементСортировки.Ключ = "ПодразделениеНаименование" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляПодразделение, "РеквизитУпорядовачивания", "Наименование");
		ИначеЕсли ЭлементСортировки.Ключ = "ПодразделениеНомер" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляПодразделение, "РеквизитУпорядовачивания", "Код");
		ИначеЕсли ЭлементСортировки.Ключ = "ДолжностьПриоритет" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляДолжность, "РеквизитУпорядовачивания", "РеквизитДопУпорядочивания");
		ИначеЕсли ЭлементСортировки.Ключ = "ДолжностьНаименование" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляДолжность, "РеквизитУпорядовачивания", "Наименование");
		ИначеЕсли ЭлементСортировки.Ключ = "ДолжностьКраткоеНаименование" Тогда	
			СтрокаТаблицы.Текст = СтрЗаменить(ШаблонПоляДолжность, "РеквизитУпорядовачивания", "НаименованиеКраткое");	
	...
	ПоляУпорядовачивания.Сортировать("Порядок");
	Если ПоляУпорядовачивания.Количество() > 0 Тогда
		Порядок = ПоляУпорядовачивания[ПоляУпорядовачивания.Количество() - 1].Порядок;
	Иначе
		Порядок = 0;
	КонецЕсли;	
	
	ДополнителныеПоляУпорядовачивания = ДополнителныеПоляУпорядовачиванияЗапросаСортировкиКоллекцииСотрудников(ОписаниеСортируемыхКоллекций);
		
	Для Каждого СтрокаТаблицыДопПорядка Из ДополнителныеПоляУпорядовачивания Цикл
		Порядок = Порядок + 1;
		СтрокаТаблицы = ПоляУпорядовачивания.Добавить();
	...
	Возврат ПоляУпорядовачивания;
КонецФункции

Функция ДополнителныеПоляУпорядовачиванияЗапросаСортировкиКоллекцииСотрудников(ОписаниеСортируемыхКоллекций)
	ПоляУпорядовачивания = Новый ТаблицаЗначений;
	ПоляУпорядовачивания.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число"));
	ПоляУпорядовачивания.Колонки.Добавить("Текст", Новый ОписаниеТипов("Строка"));
	ПоляУпорядовачивания.Колонки.Добавить("Псевдоним", Новый ОписаниеТипов("Строка"));
	
	ОписаниеПолейДопУпорядовачивания = Новый Массив;
	
	Для Каждого ОписаниеКоллекции Из ОписаниеСортируемыхКоллекций Цикл
		Если ОписаниеКоллекции.ДополнительынеПоляСортировки <> Неопределено Тогда
			Сч = 1;
			Для Каждого ДополнительноеПоле Из ОписаниеКоллекции.ДополнительынеПоляСортировки Цикл
				Если ОписаниеПолейДопУпорядовачивания.Количество() >= Сч Тогда
					ОписаниеПоляСортировки = ОписаниеПолейДопУпорядовачивания[Сч - 1];
				Иначе 
					ОписаниеПоляСортировки = Новый Структура;
					ОписаниеПолейДопУпорядовачивания.Добавить(ОписаниеПоляСортировки);
				КонецЕсли;	
	...
	Порядок = 1;	
	Для Каждого Описание Из ОписаниеПолейДопУпорядовачивания Цикл	
		...
		СтркаТаблицы = ПоляУпорядовачивания.Добавить();
		СтркаТаблицы.Текст = СтрСоединить(ЧастиТелаПоля, Символы.ПС);
		СтркаТаблицы.Псевдоним = "ПолеПорядка" + Порядок;
		СтркаТаблицы.Порядок = Порядок;
...
Функция ТаблицаСортировкиСотрудников(ОписаниеСортируемыхКоллекций)
	...
	Для Каждого ОписаниеКоллекции Из ОписаниеСортируемыхКоллекций Цикл
		Если ОписаниеКоллекции.ДополнительынеПоляСортировки <> Неопределено Тогда
			Для Каждого ОписаниеПоля Из ОписаниеКоллекции.ДополнительынеПоляСортировки Цикл
	...
	Для Каждого ДобалвяемойПоле Из ТипыДополнительныхПолей Цикл
		ТаблицаСортировки.Колонки.Добавить(ДобалвяемойПоле.Ключ, Новый ОписаниеТипов(ДобалвяемойПоле.Значение)); 		
	КонецЦикла;	
			
	Возврат ТаблицаСортировки;
КонецФункции

Процедура ДобавитьСтрокуВТаблицуСортировкиСотрудников(ТаблицаСортировки, СтрокаСортируемойКоллекциия, ИндексСтрокиКоллекции, ОписаниеКоллекции)
	СтрокаТаблицы = ТаблицаСортировки.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаСортируемойКоллекциия); 
	
	СтрокаТаблицы.Сотрудник = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляСотрудник];
	
	Если ОписаниеКоллекции.ИмяПоляПодразделение <> Неопределено Тогда 
		СтрокаТаблицы.ПодразделениеПорядка = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляПодразделение];
		СтрокаТаблицы.ЗаданоПолеПодразделение = Истина;
	КонецЕсли;
	Если ОписаниеКоллекции.ИмяПоляДолжность <> Неопределено Тогда
		СтрокаТаблицы.ДолжностьПорядка = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляДолжность];
	...
	Если ОписаниеКоллекции.ИмяПоляПериод <> Неопределено Тогда
		СтрокаТаблицы.Период = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляПериод];
	Иначе	
...
Функция ТаблицаОтбраКадровыхДанныхДляСортировкиКоллекцийСотрудников()
	ТаблицаОтбра = Новый ТаблицаЗначений;
	ТаблицаОтбра.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаОтбра.Колонки.Добавить("Период", Новый ОписаниеТипов("Дата"));
	
	Возврат ТаблицаОтбра;
КонецФункции

Функция ДобавитьСтрокуВТаблицуОтбраКадровыхДанныхДляСортировки(ТаблицаОтбраКадровыхДанных, СтрокаСортируемойКоллекциия, ОписаниеКоллекции)
	СтрокаТаблицыОтбора = ТаблицаОтбраКадровыхДанных.Добавить();
	СтрокаТаблицыОтбора.Сотрудник = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляСотрудник];
	
	Если ОписаниеКоллекции.ИмяПоляПериод <> Неопределено Тогда
		СтрокаТаблицыОтбора.Период = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляПериод];
	Иначе	
...
Функция ПорядокСортировкиКоллекцияФизическихЛиц(СортируемыеКоллекции, Организация, ПериодПолученияКадровыхДанных, НастрокиСортировки = Неопределено) Экспорт 
	РезультатСортировки = Новый Структура;
	...
	СортируемаяТаблица = ТаблицаСортировкиФизичекихЛиц(ОписаниеСортируемыхКоллекций);
	
	Для Каждого СортируемыеДанные Из СортируемыеКоллекции Цикл	
		ИндексСтроки = 0;
		Для Каждого СтрокаСортируемойТаблицы Из СортируемыеДанные.Коллекция Цикл	
			ДобавитьСтрокуВТаблицуСортировкиФизичекихЛиц(СортируемаяТаблица, СтрокаСортируемойТаблицы, ИндексСтроки, СортируемыеДанные.ОписаниеКоллекции, ПериодПолученияКадровыхДанных);
						
			ИндексСтроки = ИндексСтроки + 1;
		КонецЦикла;		
	КонецЦикла;	
	
	ЗаполнитьСотрудникаВТаблицеСортировкиФизическизЛиц(СортируемаяТаблица, Организация, ПериодПолученияКадровыхДанных);
	
	ТаблицаОтбораКадровыхДанных = ТаблицаОтбраКадровыхДанныхДляСортировкиКоллекцийСотрудников();
	...
	РезультатЗапросаСортировки = РезультатЗапросаСортирокиКоллекцииСотрудников(СортируемаяТаблица, НастрокиСортировки, ТаблицаОтбораКадровыхДанных, ОписаниеСортируемыхКоллекций);
	Выборка = РезультатЗапросаСортировки.Выбрать();
...
Функция ТаблицаСортировкиФизичекихЛиц(ОписаниеСортируемыхКоллекция)
...
Процедура ДобавитьСтрокуВТаблицуСортировкиФизичекихЛиц(ТаблицаСортировки, СтрокаСортируемойКоллекциия, ИндексСтрокиКоллекции, ОписаниеКоллекции, ПериодПолученияКадровыхДанных)
	СтрокаТаблицы = ТаблицаСортировки.Добавить();
	ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаСортируемойКоллекциия); 	
	СтрокаТаблицы.ФизическоеЛицо = СтрокаСортируемойКоллекциия[ОписаниеКоллекции.ИмяПоляФизическоеЛицо];
	СтрокаТаблицы.Период = ПериодПолученияКадровыхДанных;	
	СтрокаТаблицы.ПсевдонимТаблицы = ОписаниеКоллекции.Псевдоним;
	СтрокаТаблицы.ИндексСтрокиИсходнойтаблицы = ИндексСтрокиКоллекции;
КонецПроцедуры	

Процедура ЗаполнитьСотрудникаВТаблицеСортировкиФизическизЛиц(ТаблицаСортировки, Организация, ПериодПолученияКадровыхДанных)
...
Функция СообщениеОКонфликтахВидовЗанятостиСотрудников(ТаблицаПроверяемыхСотрудников, Организация, ИсключаемыеРегистраторы) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаСотрудников.ДатаСобытия
		...
		|ИЗ
		|	ВТСотрудникиПериодыПредварительно КАК СотрудникиПериоды
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТФизическиеЛицаСНесколькимиСотрудниками КАК ФизическиеЛицаСНесколькимиСотрудниками
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(
			Описатель, Ложь, КадровыеДанные, ПоляОтбора);
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Пока Выборка.СледующийПоЗначениюПоля("ПроверяемыйСотрудник") Цикл
				...
				ИначеЕсли НЕ ЕстьОсновноеМестоРаботы
					...
					И Выборка.ПроверяемыйСотрудникТекущийВидЗанятости <> Перечисления.ВидыЗанятости.ВнутреннееСовместительство Тогда
...
Функция ПредставлениеПроцентаСевернойНадбавки(ФизическоеЛицо) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ИначеЕсли Выборка.ПроцентСевернойНадбавки <> 0 Тогда
			ПредставлениеПроцента = НСтр("ru = 'Процент северной надбавки'") + " " + Выборка.ПроцентСевернойНадбавки + "%";
		КонецЕсли;
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	// 4D:ERP для Беларуси, Юлия, 15.12.2017 14:36:49 
	// Переименовать НДФЛ на "Подоходный налог", № 17145
	// {	
	ОписаниеМеню.Пункты.Добавить(
		СотрудникиФормы.РазделМеню(НСтр("ru='Подоходный налог'"), ПунктыМеню));
	// }
	// 4D
...
Функция ТекстыЗапросовФормыВыбораСотрудников()
	...
	Если ПараметрыСеанса.АльтернативныйПодборСпискаСотрудников Тогда
		...
		ТекстыЗапросов.Текст =
			...
			|			И КОНЕЦПЕРИОДА(ДанныеДляПодбора.Начало, День) МЕЖДУ ДанныеОбАвансеДляПодбора.ДатаНачала И  ДанныеОбАвансеДляПодбора.ДатаОкончания}
			|ГДЕ
	...
	Иначе
		...
		ТекстыЗапросов.Текст =
			...
			|			И (СостоянияСотрудников.Период В
			|				(ВЫБРАТЬ ПЕРВЫЕ 1
			|					РегистрСостоянияСотрудников.Период КАК Период
			|				ИЗ
			|					РегистрСведений.СостоянияСотрудников КАК РегистрСостоянияСотрудников
			|				ГДЕ
			|					ДанныеДляПодбора.Сотрудник = РегистрСостоянияСотрудников.Сотрудник
			...
			|					И &ОтборСостоянийСотрудника
			|				УПОРЯДОЧИТЬ ПО
			|					РегистрСостоянияСотрудников.ДействуетДо УБЫВ,
			|					РегистрСостоянияСотрудников.Состояние,
			|					Период УБЫВ))}
			|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановыйФОТИтоги КАК ДанныеОбОплатеТрудаДляПодбора
			|		ПО ДанныеДляПодбора.Сотрудник = ДанныеОбОплатеТрудаДляПодбора.Сотрудник
			|			И ДанныеДляПодбора.Организация = ДанныеОбОплатеТрудаДляПодбора.ГоловнаяОрганизация
