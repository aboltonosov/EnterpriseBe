...
//
Процедура ЗафиксироватьОшибкиВСформированныхДвижениях(ПараметрыРасчета, ОписаниеРегистра, РезультатЗапроса, ОписаниеОшибки) Экспорт
	...
	Регистраторы = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Регистратор");
...
Процедура ЗафиксироватьОшибкуРасчета(ПараметрыРасчета, ТипОшибки,
			ТекстДляПротокола = Неопределено, ТекстДляЖурналаРегистрации = Неопределено) Экспорт
...
Процедура ОтметкаВЖурналеРегистрации(ПараметрыРасчета, ПредставлениеЭтапа, Уровень = Неопределено, Комментарий = "")
	...
	ЗаписьЖурналаРегистрации(
		УниверсальныеМеханизмыПартийИСебестоимости.ИмяСобытияЖурналаРегистрации(
			ПараметрыРасчета, НСтр("ru='Этап'") + "." + ПредставлениеЭтапа),
		Уровень,
		,
		,
		Комментарий);
