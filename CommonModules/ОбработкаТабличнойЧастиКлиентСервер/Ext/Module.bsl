...
Функция ПолучитьСтруктуруКэшируемыеЗначения() Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 12.02.2016 17:15:26 
	// Расчет скидки к цене, №10910
	// {
	КэшированныеЗначения.Вставить("ПрименятьСкидкиКЦене", Неопределено);
	// }
	// 4D
...
Функция ПолучитьСтруктуруКэшируемыеЗначенияУслуг() Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 16.02.2016 11:42:49 
	// Расчет скидки к цене, №10910
	// {
	КэшированныеЗначения.Вставить("ПрименятьСкидкиКЦене", СкидкиНаценкиВызовСервера.ПолучитьСпособПримененияСкидки());
	// }
	// 4D
...
// 4D:ERP для Беларуси, Екатерина, 12.04.2016 15:37:33 
// Расчет скидки к цене, №10910
// {
Процедура ПересчитатьПроцентРучнойСкидкиВПродажахВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	СтруктураПараметровДействия = Неопределено;
	
	Если СтруктураДействий.Свойство("ПересчитатьПроцентРучнойСкидкиВПродажах", СтруктураПараметровДействия) Тогда
		...
		Если ИспользоватьРучныеСкидки И КэшированныеЗначения.ИспользоватьРучныеСкидкиВПродажах Тогда
			...
			Иначе
				Если ТекущаяСтрока.Цена = 0 Тогда
					...
					ТекущаяСтрока.ПроцентРучнойСкидки = 0;
				Иначе
					СуммаБезСкидки = Окр(ТекущаяСтрока.Цена * ТекущаяСтрока[ИмяКоличества], 2);
...
Процедура ПересчитатьСуммуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСумму", ИмяКоличества) Тогда
		...
		// 4D:ERP для Беларуси, Екатерина, 12.02.2016 15:38:22 
		// Расчет скидки к цене, №10910
		// {
		...
		Если КэшированныеЗначения.ПрименятьСкидкиКЦене Тогда 
			
			//ПЕРЕДЕЛАТЬ!!!!!!!
			Попытка
				Цена = ТекущаяСтрока.ЦенаСоСкидкой;
			Исключение
				Цена = ТекущаяСтрока.Цена;
			КонецПопытки;	
			
			ТекущаяСтрока.Сумма = Цена * ТекущаяСтрока[ИмяКоличества];
	...
	#КонецЕсли
		...
		Если ИспользоватьРучныеСкидки И КэшированныеЗначения.ИспользоватьРучныеСкидкиВПродажах Тогда
			...
			Если ТекущаяСтрока.Сумма = 0 Или ТекущаяСтрока[ИмяКоличества] = 0 Тогда
				...
				// 4D:ERP для Беларуси, Екатерина, 12.04.2016 16:05:31 
				// Расчет скидки к цене, №10910
				// {
			...
			Иначе
				...
				ИначеЕсли ТекущаяСтрока.Цена = 0 Тогда
					Если ЗначениеЗаполнено(ТекущаяСтрока.ПроцентРучнойСкидки) Тогда
						...
						// 4D:ERP для Беларуси, Екатерина, 12.02.2016 15:58:36 
						// Расчет скидки к цене, №10910
						// {
						Если КэшированныеЗначения.ПрименятьСкидкиКЦене Тогда 
							ТекущаяСтрока.СуммаРучнойСкидки = ТекущаяСтрока.Сумма - ТекущаяСтрока[ИмяКоличества] * ТекущаяСтрока.ЦенаСоСкидкой; 
						Иначе 
				...
				Иначе
					...
					// 4D:ERP для Беларуси, Екатерина, 12.02.2016 16:12:51 
					// Расчет скидки к цене, №10910
					// {
					Если КэшированныеЗначения.ПрименятьСкидкиКЦене Тогда
						СуммаБезСкидки = Окр(ТекущаяСтрока.Цена * ТекущаяСтрока[ИмяКоличества], 2);
						ТекущаяСтрока.ЦенаСоСкидкой = Окр(ТекущаяСтрока.Сумма / ТекущаяСтрока[ИмяКоличества], 2);
					Иначе 
...
Процедура ПересчитатьСуммуСУчетомРучнойСкидкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуСУчетомРучнойСкидки", СтруктураПараметровДействия) Тогда
		...
		// 4D:ERP для Беларуси, Екатерина, 12.02.2016 16:49:56 
		// Расчет скидки к цене, №10910
		// {
		Если КэшированныеЗначения.ПрименятьСкидкиКЦене Тогда
			...
			Если ЦенаСоСкидкой <> 0 Тогда 
				ТекущаяСтрока.Сумма = Окр(ЦенаСоСкидкой * ТекущаяСтрока[ИмяКоличества], 2); 
...
// 4D:ERP для Беларуси, Петр, 19.02.2018 16:21:47 
// Механизм ввода остатков, № 17856
// {
// Пересчитывает цену (регл) за упаковку товара в текущей строке табличной части документа.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьЦенуРеглЗаУпаковкуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт

	Перем КоличествоЕдиницДоПересчета;

	Если СтруктураДействий.Свойство("ПересчитатьЦенуРеглЗаУпаковку", КоличествоЕдиницДоПересчета)
	   И КоличествоЕдиницДоПересчета <> 0 Тогда
		ТекущаяСтрока.ЦенаРегл = ТекущаяСтрока.ЦенаРегл
		   / КоличествоЕдиницДоПересчета
		   * ТекущаяСтрока.Количество;
	КонецЕсли;
...
// Пересчитывает сумму (регл) товара в текущей строке табличной части документа, в зависимости от того включает ли цена НДС.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьСуммуРеглБезНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ЦенаВключаетНДС;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРеглБезНДС") И СтруктураДействий.Свойство("ПересчитатьСуммуНДСРегл", ЦенаВключаетНДС) Тогда
		
		Если ЦенаВключаетНДС.ЦенаВключаетНДС Тогда
			
			ТекущаяСтрока.СуммаРегл = ТекущаяСтрока.СуммаРегл - ТекущаяСтрока.НДСРегл;
...
// Пересчитывает сумму товара в текущей строке табличной части документа, относительно цены (регл).
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьСуммуУпрПоСуммеРеглВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт

	Перем КоэффициентПересчетаИзВалютыРеглВУпр;
	
	Перем ЦенаВключаетНДС;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуУпрПоСуммеРегл", КоэффициентПересчетаИзВалютыРеглВУпр) И СтруктураДействий.Свойство("ПересчитатьСуммуНДС", ЦенаВключаетНДС)Тогда
		Если ЦенаВключаетНДС.ЦенаВключаетНДС Тогда
			ТекущаяСтрока.Сумма = Окр((ТекущаяСтрока.СуммаРегл + ТекущаяСтрока.НДСРегл) * КоэффициентПересчетаИзВалютыРеглВУпр, 2, 1)
		Иначе
			ТекущаяСтрока.Сумма = Окр(ТекущаяСтрока.СуммаРегл * КоэффициентПересчетаИзВалютыРеглВУпр, 2, 1)
		КонецЕсли;
	ИначеЕсли СтруктураДействий.Свойство("ПересчитатьСуммуУпрПоСуммеРегл", КоэффициентПересчетаИзВалютыРеглВУпр) Тогда
		ТекущаяСтрока.Сумма = Окр(ТекущаяСтрока.СуммаРегл * КоэффициентПересчетаИзВалютыРеглВУпр, 2, 1)
	КонецЕсли;

КонецПроцедуры

// Пересчитывает цену (регл) товара относительно суммы (регл) в текущей строке табличной части документа.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьЦенуРеглПоСуммеРеглВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем ЦенаВключаетНДС;
	
	ИмяКоличества = Неопределено;
	
	Если СтруктураДействий.Свойство("ПересчитатьЦенуРеглПоСуммеРегл",ИмяКоличества) И СтруктураДействий.Свойство("ПересчитатьСуммуНДСРегл", ЦенаВключаетНДС) Тогда
		Если Не ЗначениеЗаполнено(ИмяКоличества) Тогда
			ИмяКоличества = "КоличествоУпаковок";
		КонецЕсли;
		Если ТекущаяСтрока[ИмяКоличества] = 0 Тогда
			Если ЦенаВключаетНДС.ЦенаВключаетНДС Тогда
				ТекущаяСтрока.ЦенаРегл = ТекущаяСтрока.СуммаРегл + ТекущаяСтрока.НДСРегл;
			Иначе
				ТекущаяСтрока.ЦенаРегл = ТекущаяСтрока.СуммаРегл
			КонецЕсли;
		Иначе
			Если ЦенаВключаетНДС.ЦенаВключаетНДС Тогда
				ТекущаяСтрока.ЦенаРегл = Окр((ТекущаяСтрока.СуммаРегл + ТекущаяСтрока.НДСРегл)/ ТекущаяСтрока[ИмяКоличества], 2);
			Иначе
				ТекущаяСтрока.ЦенаРегл = Окр(ТекущаяСтрока.СуммаРегл/ ТекущаяСтрока[ИмяКоличества], 2);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
// Пересчитывает сумму (регл) товара относительно цены (регл) в текущей строке табличной части документа.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьСуммуРеглПоЦенеРеглВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	ИмяКоличества = Неопределено;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРеглПоЦенеРегл", ИмяКоличества) Тогда
		
		Если НЕ ЗначениеЗаполнено(ИмяКоличества) Тогда
			ИмяКоличества = "КоличествоУпаковок"
		Конецесли;
		
		ТекущаяСтрока.СуммаРегл = ТекущаяСтрока.ЦенаРегл * ТекущаяСтрока[ИмяКоличества]
...
// в зависимости от того включает ли цена НДС.
//
// Параметры:
//	ТекущаяСтрока - Структура - структура со свойствами строки документа.
//	СтруктураДействий - Структура - структура с действиями, которые нужно произвести.
//	КэшированныеЗначения - Структура - кэшированные значения.
//
Процедура ПересчитатьСуммуНДСРеглВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	
	Перем СтруктураПараметровДействия;
	Перем СтавкаНДС;
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуНДСРегл", СтруктураПараметровДействия) Тогда
		...
		ТекущаяСтрока.НДСРегл = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(ТекущаяСтрока.СуммаРегл, ТекПроцентНДС, СтруктураПараметровДействия.ЦенаВключаетНДС);
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			...
			Если ДанныеСпецификации <> Неопределено Тогда
				ТекущаяСтрока.Спецификация = ДанныеСпецификации.Спецификация;
			Иначе
...
// 4D:ERP для Беларуси, Екатерина, 12.02.2016 15:33:24 
// Расчет скидки к цене, №10910
// {
Процедура ПересчитатьЦенуСУчетомСкидокВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт   
	...
	Если Не КэшированныеЗначения.ПрименятьСкидкиКЦене Тогда 
		Возврат
	КонецЕсли;	
	
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРучнойСкидки", ИмяКоличества) или
		СтруктураДействий.Свойство("ПересчитатьСуммуСУчетомАвтоматическойСкидки")
		...
		Если СтруктураДействий.Свойство("ПересчитатьСуммуСУчетомАвтоматическойСкидки", СтруктураПараметровДействия) Тогда
			...
			Если НЕ Очищать Тогда
				ПроцентСкидки = ПроцентСкидки + ТекущаяСтрока.ПроцентАвтоматическойСкидки;
			КонецЕсли;

		КонецЕсли;
		
		Если СтруктураДействий.Свойство("ПересчитатьСуммуСУчетомРучнойСкидки") или СтруктураДействий.Свойство("ПересчитатьСуммуРучнойСкидки") Тогда 
			ПроцентСкидки = ПроцентСкидки + ТекущаяСтрока.ПроцентРучнойСкидки;
		КонецЕсли;			
		
		ТекущаяСтрока.ЦенаСоСкидкой	= Окр(ТекущаяСтрока.Цена * (1 - ПроцентСкидки/100), 2);
