...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// 4D:ERP для Беларуси, Яна, 31.05.2017 10:09:10 
	// Ошибка конфигурации при создании нового вида расчета Удержания, № 15011 
	// {
	УдерживаетсяВЦеломЗаМесяц = Не ТребуетсяРасчетБазы 
		Или СпособВыполненияУдержания = Перечисления.СпособыВыполненияУдержаний.ПоЗначениюПоказателяПриОкончательномРасчете;
	УчаствуетВРасчетеПервойПоловиныМесяца = Не УдерживаетсяВЦеломЗаМесяц;
	
	Если УчаствуетВРасчетеПервойПоловиныМесяца  
		И РасчетЗарплатыРасширенный.ЕстьПоказательВКоллекции(Показатели, "РасчетнаяБазаСтраховыеВзносы") Тогда 
		ТекстСообщения = НСтр("ru = 'Показатель РасчетнаяБазаСтраховыеВзносы допустимо использовать только в удержаниях, выполняемых в целом за месяц.'");
