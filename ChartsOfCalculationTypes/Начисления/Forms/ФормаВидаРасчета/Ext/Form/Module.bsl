...
Процедура КодДоходаСтраховыеВзносыПриИзменении(Элемент)
	...
	// 4D:ERP для Беларуси, Юлия, 19.09.2017 11:51:43 
	// Исправить закладку "Средний заработок" для Начислений в Плане видов расчета, №15950
	// {
...
// 4D:ERP для Беларуси, Юлия, 05.12.2017 17:25:29 
// Редактировать форму Плана видов расчета Начисления, закладка "Налоги, взносы, бухучет", № 16988
// {
...
// 4D:ERP для Беларуси, Юлия, 04.10.2017 16:28:33 
// // Исправить закладку "Средний заработок" для Начислений в Плане видов расчета, № 15950
// {
&НаКлиенте
...
&НаКлиенте
Процедура СреднийЗаработокОтпускИспользоватьПриИзменении(Элемент)
	
	УстановитьСвойстваЭлементовОтпускСреднегоЗаработка(ЭтаФорма);
...
Процедура УстановитьДоступностьЭлементовПоКатегории(Форма)
	...
	// 4D:ERP для Беларуси, Петр, 14.12.2017 15:31:04 
	...
	// {
		...
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокОбщий" Тогда
			...
			УстановитьСвойстваЭлементовОбщегоСреднегоЗаработка(Форма);
		ИначеЕсли ИмяСвойства = "СреднийЗаработокОтпуск" Тогда
			Форма.СреднийЗаработокОтпускДоступностьПоКатегории = ДоступностьСвойства;
			УстановитьСвойстваЭлементовОтпускСреднегоЗаработка(Форма);
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокОтпуск" Тогда
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Форма.Элементы, "СреднийЗаработокОтпускИспользовать", "Доступность", ДоступностьСвойства);
			УстановитьСвойстваЭлементовОтпускСреднегоЗаработка(Форма);
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокФСС" Тогда
			Форма.СреднийЗаработокФССДоступностьПоКатегории = ДоступностьСвойства;
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
...
Процедура ЗаполнитьСвойстваПоКатегории(Форма)
	...
	Для Каждого КлючИЗначение Из СвойстваНачисления Цикл
		...
		// 4D:ERP для Беларуси, Петр, 15.12.2017 11:08:52 
		// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
		// {
		Если ИмяСвойства = "СреднийЗаработокОбщий" Тогда
			ЗаполнитьСреднийЗаработокОбщий(Форма, КлючИЗначение.Значение);
		ИначеЕсли ИмяСвойства = "СреднийЗаработокОтпуск" Тогда
			ЗаполнитьСреднийЗаработокОтпуск(Форма, КлючИЗначение.Значение);
		ИначеЕсли ИмяСвойства = "СреднийЗаработокФСС" Тогда
			ЗаполнитьСреднийЗаработокФСС(Форма, КлючИЗначение.Значение);
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокОбщий" Тогда
			Форма.СреднийЗаработокОбщийИспользовать = КлючИЗначение.Значение;
		ИначеЕсли ИмяСвойства = "ИндексируетсяСреднийЗаработокОбщий" Тогда
			Продолжить;
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокФСС" Тогда
			Форма.СреднийЗаработокФССИспользовать = КлючИЗначение.Значение;
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокФСС2010" Тогда
			Продолжить;
		ИначеЕсли ИмяСвойства = "ВключатьВСреднийЗаработокОтпуск" Тогда
			Форма.СреднийЗаработокОтпускИспользовать = КлючИЗначение.Значение;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСреднийЗаработокФСС(Форма, ЗначениеУчетаСреднего)
	
	// 4D:ERP для Беларуси, Петр, 14.12.2017 16:19:40 
	// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
	// {
	Форма.Объект.СреднийЗаработокФСС.Очистить();
	
	Форма.СреднийЗаработокФССЗначение = Неопределено;
	
	Если  ЗначениеУчетаСреднего <> Неопределено Тогда
		НоваяСтрока = Форма.Объект.СреднийЗаработокФСС.Добавить();
		НоваяСтрока.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаФСС.Локализация");
		НоваяСтрока.Значение = ЗначениеУчетаСреднего;
		Форма.СреднийЗаработокФССЗначение = ЗначениеУчетаСреднего;
	КонецЕсли;
	// }
	// 4D
...
Процедура ЗаполнитьСреднийЗаработокОбщий(Форма, ЗначениеУчетаСреднего)
	...
	Если ЗначениеУчетаСреднего <> Неопределено Тогда
		...
		НоваяСтрока.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОбщий.Локализация");
		НоваяСтрока.Значение = ЗначениеУчетаСреднего;
...
// 4D:ERP для Беларуси, Петр, 14.12.2017 16:21:22 
// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
// {
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьСреднийЗаработокОтпуск(Форма, ЗначениеУчетаСреднего)
	
	Форма.Объект.СреднийЗаработокОтпуск.Очистить();
	
	Форма.СреднийЗаработокОтпускЗначение = Неопределено;
	
	Если ЗначениеУчетаСреднего <> Неопределено Тогда
		НоваяСтрока = Форма.Объект.СреднийЗаработокОтпуск.Добавить();
		НоваяСтрока.ПорядокРасчета = ПредопределенноеЗначение("Перечисление.ПорядокРасчетаСреднегоЗаработкаОтпуск.Локализация");
		НоваяСтрока.Значение = ЗначениеУчетаСреднего;
		Форма.СреднийЗаработокОтпускЗначение = ЗначениеУчетаСреднего;
 	КонецЕсли;
...
Процедура ПрочитатьНастройкиСреднегоЗаработка(ТекущийОбъект = Неопределено)
	...
	// 4D:ERP для Беларуси, Петр, 14.12.2017 15:59:19 
	// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
	// {
	ПрочитатьНастройкиСреднегоЗаработкаОтпуск(ЭтаФорма, ТекущийОбъект, ЭтоНовый, Копирование);
	// }
	// 4D
...
Процедура ЗаписатьНастройкиСреднегоЗаработка(ТекущийОбъект)
	...
	ЗаписатьНастройкиСреднегоЗаработкаОтпуск(ТекущийОбъект);
...
Процедура ПрочитатьНастройкиСреднегоЗаработкаОбщий(Форма, ТекущийОбъект, ЭтоНовый = Ложь, Копирование = Ложь)
	...
	Иначе
		...
		// 4D:ERP для Беларуси, Петр, 14.12.2017 15:44:46 
		// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
		// {
		// Действие не требуется
		// }
...
// 4D:ERP для Беларуси, Петр, 14.12.2017 15:42:50 
// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
// {
&НаКлиентеНаСервереБезКонтекста
Процедура ПрочитатьНастройкиСреднегоЗаработкаОтпуск(Форма, ТекущийОбъект, ЭтоНовый = Ложь, Копирование = Ложь)
	
	Если Форма.СреднийЗаработокОтпускМножественныйВвод Тогда
		// Заполняем коллекцию формы данными табличной части.
		Для Каждого СтрокаДанныхФормы Из Форма.СреднийЗаработокОтпуск Цикл
			НайденныеСтроки = ТекущийОбъект.СреднийЗаработокОтпуск.НайтиСтроки(Новый Структура("ПорядокРасчета", СтрокаДанныхФормы.ПорядокРасчета));
			Если НайденныеСтроки.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(СтрокаДанныхФормы, НайденныеСтроки[0]);
			Иначе
				// значения по умолчанию
				Если ЭтоНовый И Не Копирование Тогда
					СтрокаДанныхФормы.Значение = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОтпуск.ОбщийЗаработок");
					СтрокаДанныхФормы.Индексируется = Истина;
				Иначе
					СтрокаДанныхФормы.Значение = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОтпуск.НеВключать");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Заполняем коллекцию формы данными единственной строки табличной части.
		Если ТекущийОбъект.СреднийЗаработокОтпуск.Количество() > 0 Тогда
			Форма.СреднийЗаработокОтпускИспользовать	= Истина;
			Форма.СреднийЗаработокОтпускЗначение		= ТекущийОбъект.СреднийЗаработокОтпуск[0].Значение;
			//Форма.СреднийЗаработокОтпускИндексируется	= ТекущийОбъект.СреднийЗаработокОбщий[0].Индексируется;
		Иначе
			// значения по умолчанию
			Форма.СреднийЗаработокОтпускИспользовать	= ЭтоНовый И Не Копирование;
			Если Не ЗначениеЗаполнено(Форма.СреднийЗаработокОтпускЗначение) Тогда
				Форма.СреднийЗаработокОтпускЗначение	= ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеОтпуск.ОбщийЗаработок");
			КонецЕсли;
			//Форма.СреднийЗаработокОтпускИндексируется = ЭтоНовый И Не Копирование;
		КонецЕсли;
		УстановитьСвойстваЭлементовОтпускСреднегоЗаработка(Форма);
	КонецЕсли;
...
Процедура ПрочитатьНастройкиСреднегоЗаработкаФСС(Форма, ТекущийОбъект, ЭтоНовый = Ложь, Копирование = Ложь)
	...
	// 4D:ERP для Беларуси, Петр, 14.12.2017 15:46:26 
	// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
	// {
	Если Форма.СреднийЗаработокФССМножественныйВвод Тогда
		...
		Для Каждого СтрокаНастройки Из ТекущийОбъект.СреднийЗаработокФСС Цикл
			НайденныеСтроки = ТекущийОбъект.СреднийЗаработокФСС.НайтиСтроки(Новый Структура("ПорядокРасчета", СтрокаНастройки.ПорядокРасчета));
			Если НайденныеСтроки.Количество() > 0 Тогда
				ЗаполнитьЗначенияСвойств(СтрокаНастройки, НайденныеСтроки[0]);
			Иначе
				// значения по умолчанию
				Если ЭтоНовый И Не Копирование Тогда
					СтрокаНастройки.Значение = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеФСС.ОбщийЗаработок");
				Иначе
					СтрокаНастройки.Значение = ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеФСС.НеВключать");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	...
	Иначе
		...
		Если ТекущийОбъект.СреднийЗаработокФСС.Количество() > 0 Тогда
			Если  ТекущийОбъект.СреднийЗаработокФСС[0].Значение <> ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеФСС.НеВключать") Тогда 
				Форма.СреднийЗаработокФССИспользовать	= Истина;
				Форма.СреднийЗаработокФССЗначение		= ТекущийОбъект.СреднийЗаработокФСС[0].Значение;
			Иначе
				Форма.СреднийЗаработокФССИспользовать	= Ложь; 	
			КонецЕсли;
		Иначе
			// значения по умолчанию
			Форма.СреднийЗаработокФССИспользовать	= ЭтоНовый И Не Копирование;
			Если Не ЗначениеЗаполнено(Форма.СреднийЗаработокФССЗначение) Тогда
				Форма.СреднийЗаработокОбщийЗначение	= ПредопределенноеЗначение("Перечисление.УчетНачисленийВСреднемЗаработкеФСС.ОбщийЗаработок");
			КонецЕсли;
			Форма.СреднийЗаработокОбщийИндексируется = ЭтоНовый И Не Копирование;
		КонецЕсли;
		УстановитьСвойстваЭлементовСреднегоЗаработкаФСС(Форма);
...
Процедура ЗаписатьНастройкиСреднегоЗаработкаОбщий(ТекущийОбъект)
	...
	Иначе
		...
		Если СреднийЗаработокОбщийИспользовать Тогда
			...
			// 4D:ERP для Беларуси, Петр, 14.12.2017 15:33:47 
			// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
			// {
			СтрокаНастройки.ПорядокРасчета	= Перечисления.ПорядокРасчетаСреднегоЗаработкаОбщий.Локализация;
...
Процедура ЗаписатьНастройкиСреднегоЗаработкаФСС(ТекущийОбъект)
	...
	// 4D:ERP для Беларуси, Петр, 14.12.2017 15:35:34 
	// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
	// {
	Если СреднийЗаработокФССМножественныйВвод Тогда
		ТекущийОбъект.СреднийЗаработокФСС.Очистить();
		Для Каждого СтрокаДанныхФормы Из СреднийЗаработокФСС Цикл
			Если СтрокаДанныхФормы.Значение <> Перечисления.УчетНачисленийВСреднемЗаработкеФСС.НеВключать Тогда
				ЗаполнитьЗначенияСвойств(ТекущийОбъект.СреднийЗаработокФСС.Добавить(), СтрокаДанныхФормы);
			КонецЕсли;
		КонецЦикла;
	Иначе
		ТекущийОбъект.СреднийЗаработокФСС.Очистить();
		Если СреднийЗаработокФССИспользовать Тогда
			...
			СтрокаНастройки.ПорядокРасчета	= Перечисления.ПорядокРасчетаСреднегоЗаработкаФСС.Локализация;
			СтрокаНастройки.Значение		= СреднийЗаработокФССЗначение;
		КонецЕсли;
	КонецЕсли;
	// }
	// 4D
...
// 4D:ERP для Беларуси, Петр, 14.12.2017 15:36:54 
// Не сохраняются настройки вида начисления на закладке "Средний заработок", № 17074
// {
&НаСервере
Процедура ЗаписатьНастройкиСреднегоЗаработкаОтпуск(ТекущийОбъект)
	
	Если СреднийЗаработокОтпускМножественныйВвод Тогда
		ТекущийОбъект.СреднийЗаработокОтпуск.Очистить();
		Для Каждого СтрокаДанныхФормы Из СреднийЗаработокОтпуск Цикл
			Если СтрокаДанныхФормы.Значение <> Перечисления.УчетНачисленийВСреднемЗаработкеОтпуск.НеВключать Тогда
				ЗаполнитьЗначенияСвойств(ТекущийОбъект.СреднийЗаработокОтпуск.Добавить(), СтрокаДанныхФормы);
			КонецЕсли;
		КонецЦикла;
	Иначе
		ТекущийОбъект.СреднийЗаработокОтпуск.Очистить();
		Если СреднийЗаработокОтпускИспользовать Тогда
			СтрокаНастройки = ТекущийОбъект.СреднийЗаработокОтпуск.Добавить();
			СтрокаНастройки.ПорядокРасчета	= Перечисления.ПорядокРасчетаСреднегоЗаработкаОтпуск.Локализация;
			СтрокаНастройки.Значение		= СреднийЗаработокОтпускЗначение;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьСвойстваЭлементовСреднегоЗаработкаФСС(Форма)
	...
	ДоступностьИзмененияСреднегоЗаработкаФСС = Форма.СреднийЗаработокФССИспользовать И Форма.СреднийЗаработокФССДоступностьПоКатегории;
	Форма.Элементы.СреднийЗаработокФССЗначениеЕдинственный.Доступность		= ДоступностьИзмененияСреднегоЗаработкаФСС;
	
КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьСвойстваЭлементовОтпускСреднегоЗаработка(Форма)
	
	ДоступностьИзмененияСреднегоЗаработка = Форма.СреднийЗаработокОтпускИспользовать И Форма.СреднийЗаработокОтпускДоступностьПоКатегории;
	ДоступностьИзмененияИндексируемости = ДоступностьИзмененияСреднегоЗаработка;
	
	Форма.Элементы.СреднийЗаработокОтпускЗначениеЕдинственный.Доступность		= ДоступностьИзмененияСреднегоЗаработка;
...
Процедура НастроитьЭлементыВводаСреднегоЗаработкаОбщий()
	...
	// 4D:ERP для Беларуси, Юлия, 07.09.2017 14:52:28 
	// Исправить закладку "Средний заработок" для Начислений в Плане видов расчета, № 15950
	// {
	// Действие не требуется.
	// }
...
Процедура УстановитьСвойстваПериодаРасчетаСреднегоЗаработка(Форма)
	...
	// 4D:ERP для Беларуси, Петр, 16.01.2018 17:05:51 
...
Процедура УстановитьТекстНадписиОЗаполненностиФормулы(Форма)
	...
	Иначе 
		...
		Форма.ФормулаНеЗаполненаТекст = НСтр("ru = 'Формула не заполнена'");
	КонецЕсли;
