...
Функция ПолучитьСчетаИсключения() Экспорт
	...
	// 4D:ERP для Беларуси, Екатерина, 05.08.2015 10:46:48 
	// Локализация плана счетов, №8969
	// {
	МассивСчетовИсключений.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоИмущественномуИЛичномуСтрахованию);				// 76.02
	МассивСчетовИсключений.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоПричитающимсяДивидендам); 						// 76.04
	МассивСчетовИсключений.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоИмущественномуИЛичномуСтрахованиюВал);      		// 76.22
	// }
	// 4D
...
Процедура ПереименоватьСчетНалогаУСН() Экспорт
	...
	Попытка
		...
		Если НЕ РеквизитыСчета.ЗапретитьИспользоватьВПроводках
			...
			// 4D:ERP для Беларуси, Екатерина, 06.08.2015 16:11:02 
			// Локализация плана счетов, №8969
			// 
			И РеквизитыСчета.Код = "68.05.2"
			// }
			// 4D
	...
	Исключение
		ТекстСообщения = НСтр("ru = 'Не удалось установить новое наименование
		    |
			// 4D:ERP для Беларуси, Екатерина, 06.08.2015 16:11:48 
			// Локализация плана счетов, №8969
			// 
			|для счета 68.05.2 ""Единый налог при применении упрощенной системы налогообложения"".
			// }
			// 4D
			|
			|%1'");
