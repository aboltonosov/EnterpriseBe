...
Функция НастройкаОтборовНаУзлеБазыКорреспондента(ВерсияКорреспондента, ИмяФормы, ИдентификаторНастройки = "") Экспорт
	...
	Если ИдентификаторНастройки = "ОбменУП2ЗУП3" Тогда
		СтруктураНастроек.Вставить("ВыгружатьАналитикуПоСкладам", Ложь);
		СтруктураНастроек.Вставить("ВерсияФорматаОбмена", "1.3");		
	КонецЕсли;
	//-- НЕ УТ
...
Функция ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента(ВерсияКорреспондента, ИмяФормы, ИдентификаторНастройки = "") Экспорт
	...
	СтруктураТабличнойЧастиШаблоны = Новый Структура;
	СтруктураТабличнойЧастиШаблоны.Вставить("Шаблон",      Новый Массив);
	СтруктураТабличнойЧастиШаблоны.Вставить("Шаблон_Ключ", Новый Массив);
	СтруктураНастроек.Вставить("ШаблоныВводаИсходныхДанных", СтруктураТабличнойЧастиШаблоны);
	//-- НЕ УТ
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки = "") Экспорт
	...
	ИначеЕсли ИдентификаторНастройки = "ОбменУПБП" Тогда
		...
		|и ""Бухгалтерия предприятия, редакция 3.0"".  В синхронизации участвуют документы и нормативно-справочной информация.'");
	ИначеЕсли ИдентификаторНастройки = "ОбменУП2ЗУП3" Тогда
		ПоясняющийТекст = НСтр("ru = 'Данная настройка позволит синхронизировать данные между программами ""1С:ERP Управление предприятием 2"",
...
// Предназначена для получения дополнительных данных, используемых при настройке обмена в базе-корреспонденте.
//
// Параметры:
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
		...
		|	И СинхронизацияДанныхЧерезУниверсальныйФормат.ЭтотУзел = ЛОЖЬ
		|	И СинхронизацияДанныхЧерезУниверсальныйФормат.ПравилаСозданияДоговоровКонтрагентов = """"
		|	И СинхронизацияДанныхЧерезУниверсальныйФормат.УдалитьПравилаСозданияДоговоровКонтрагентов <> """"");
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			УзелПланаОбменаОбъект.ПравилаСозданияДоговоровКонтрагентов        = Выборка.УзелПланаОбмена.УдалитьПравилаСозданияДоговоровКонтрагентов;
			УзелПланаОбменаОбъект.УдалитьПравилаСозданияДоговоровКонтрагентов = "";
